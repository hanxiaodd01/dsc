...
if ($_REQUEST['act'] == 'coupons_index') {
    ...
    foreach ($cou_goods as $k => $v) {
        if ($v['cou_ok_goods']) {
            ...
            $cou_goods[$k]['cou_ok_goods_name'][0]['goods_thumb'] = get_image_path($v['cou_id'],
                'images/coupons_default.png');
        }
    ...
    if ($_REQUEST['act'] == 'coupons_list') {
        ...
        $page_url = strstr($_SERVER['QUERY_STRING'], '&p', true) ? strstr($_SERVER['QUERY_STRING'], '&p',
            true) : $_SERVER['QUERY_STRING'];
            ...
            foreach ($cou_goods as $k => $v) {
                if ($v['cou_ok_goods']) {
                    ...
                    if (!empty($cou_goods_arr)) {
                        foreach ($cou_goods_arr as $g_key => $g_val) {
                            if ($g_val['goods_thumb']) {
                                $cou_goods_arr[$g_key]['goods_thumb'] = get_image_path($v['cou_id'],
                                    $g_val['goods_thumb']);
                            }
                        }
                    }
            ...
            if ($_REQUEST['act'] == 'coupons_receive') {
                ...
                if ($cou_data['cou_user_num'] <= $cou_user_num) {
                    exit(json_encode(array(
                        'status' => 'error',
                        'msg' => sprintf($_LANG['lang_coupons_user_receive'], $cou_data['cou_user_num'])
                    )));
                } else {
                    $result['is_over'] = 1;
                }

                if (strpos(',' . $cou_data['cou_ok_user'] . ',',
                        ',' . $_SESSION['user_rank'] . ',') === false && $cou_data['cou_type'] != 3) {
                    $rank_name = $db->getOne('SELECT GROUP_CONCAT(rank_name) FROM ' . $ecs->table('user_rank') . ' WHERE rank_id IN(' . $cou_data['cou_ok_user'] . ')');
                    exit(json_encode(array(
                        'status' => 'error',
                        'msg' => sprintf($_LANG['lang_coupons_user_rank'], $rank_name)
                    )));
                }
                ...
                if ($db->query($sql)) {
                    ...
                    exit(json_encode(array(
                        'status' => 'ok',
                        'msg' => $_LANG['lang_coupons_receive_succeed'],
                        'content' => $result['content'],
                        'content_kill' => $result['content_kill']
                    )));
                }
            }
        }
