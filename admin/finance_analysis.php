...
if ($_REQUEST['act'] == 'settlement_stats_query') {
    ...
    make_json_result($smarty->fetch('settlement_stats.dwt'), '',
...
if ($_REQUEST['act'] == 'settlement_total_stats') {
    ...
    if ($_REQUEST['act'] == 'balance_stats') {
        ...
        if ($_REQUEST['act'] == 'balance_stats_query') {
            $order_list = balance_stats();
            $smarty->assign('order_list', $order_list['orders']);
            $smarty->assign('filter', $order_list['filter']);
            $smarty->assign('record_count', $order_list['record_count']);
            ...
            $sort_flag = sort_flag($order_list['filter']);
            $smarty->assign($sort_flag['tag'], $sort_flag['img']);
            make_json_result($smarty->fetch('balance_stats.dwt'), '',
                array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
        }
    }
}
...
if ($_REQUEST['act'] == 'balance_total_stats') {
    ...
    if ($_REQUEST['act'] == 'download') {
        ...
        $thead = array(
            $_LANG['record_id'],
            $_LANG['user_name'],
            $_LANG['finance_analysis'][5],
            $_LANG['finance_analysis'][6],
            $_LANG['finance_analysis'][7],
            $_LANG['finance_analysis'][8],
            $_LANG['finance_analysis'][9]
        );
        $tbody = array(
            'user_id',
            'user_name',
        ...
        if ($_REQUEST['act'] == 'download_settlement') {
            $_GET['uselastfilter'] = 1;
            $order_list = merchants_commission_list();
            ...
            $thead = array(
                ...
                $_LANG['finance_analysis'][1],
                $_LANG['finance_analysis'][2],
                $_LANG['finance_analysis'][3],
                $_LANG['finance_analysis'][4]
            );
            $tbody = array(
                'user_id',
                'store_name',
