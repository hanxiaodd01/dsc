...
function get_sale_list($is_pagination = true)
{
    ...
    $filter['order_status'] = isset($_REQUEST['order_status']) && !($_REQUEST['order_status'] == '') ? explode(',',
        $_REQUEST['order_status']) : '';
    $filter['shipping_status'] = isset($_REQUEST['shipping_status']) && !($_REQUEST['shipping_status'] == '') ? explode(',',
    ...
    $sql = 'SELECT COUNT(og.goods_id) FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS oi,' . $GLOBALS['ecs']->table('order_goods') . ' AS og ' . $leftJoin . $where;
    $filter['record_count'] = $GLOBALS['db']->getOne($sql);
    $filter = page_and_size($filter);
    $sql = 'SELECT og.goods_id, og.goods_sn, og.goods_name, og.goods_number AS goods_num, og.ru_id, og.goods_price ' . 'AS sales_price, oi.add_time AS sales_time, oi.order_id, oi.order_sn, (og.goods_number * og.goods_price) AS total_fee ' . 'FROM ' . $GLOBALS['ecs']->table('order_goods') . ' AS og, ' . $GLOBALS['ecs']->table('order_info') . ' AS oi ' . $leftJoin . $where . (' ORDER BY ' . $filter['sort_by'] . ' DESC');
    ...
    foreach ($sale_list_data as $key => $item) {
        ...
        $sale_list_data[$key]['sales_time'] = local_date($GLOBALS['_CFG']['time_format'],
            $sale_list_data[$key]['sales_time']);
    }
...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' || $_REQUEST['act'] == 'download')) {
    ...
    if ($_REQUEST['act'] == 'download') {
        ...
        echo ecs_iconv(EC_CHARSET, 'GB2312',
                $_REQUEST['start_date'] . $_LANG['to'] . $_REQUEST['end_date'] . $_LANG['sales_list']) . '	
';
...
';
    ...
    make_json_result($smarty->fetch('sale_list.dwt'), '',
