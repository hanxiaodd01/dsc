...
function get_orderinfo($start_date, $end_date, $type = 0)
{
    ...
    if ($type == 1) {
        ...
        $sql = 'SELECT IFNULL(SUM(' . order_amount_field('o.') . '), 0) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . ' WHERE o.order_status = \'' . OS_CONFIRMED . '\' AND o.shipping_status NOT ' . db_create_in(array(
                SS_SHIPPED,
                SS_RECEIVED
            )) . ' AND o.pay_status NOT' . db_create_in(array(
                PS_PAYED,
                PS_PAYING
            )) . (' AND o.add_time >= \'' . $start_date . '\'') . ' AND o.add_time < \'' . ($end_date + 86400) . '\'' . $where . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
        $order_info['confirmed_num'] = $GLOBALS['db']->getOne($sql);
        $sql = 'SELECT IFNULL(SUM(' . order_amount_field('o.') . '), 0) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . ' WHERE 1 ' . order_query_sql('finished',
                'o.') . (' AND o.add_time >= \'' . $start_date . '\' AND o.add_time < \'') . ($end_date + 86400) . '\'' . $where . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
        $order_info['succeed_num'] = $GLOBALS['db']->getOne($sql);
        $sql = 'SELECT IFNULL(SUM(' . order_amount_field('o.') . '), 0) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . ' WHERE o.order_status ' . db_create_in(array(
                OS_CANCELED,
                OS_INVALID
            )) . (' AND o.add_time >= \'' . $start_date . '\' AND o.add_time < \'') . ($end_date + 86400) . '\'' . $where . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
        ...
        $sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . ' WHERE o.order_status = \'' . OS_CONFIRMED . '\' AND o.shipping_status NOT ' . db_create_in(array(
                SS_SHIPPED,
                SS_RECEIVED
            )) . ' AND o.pay_status NOT' . db_create_in(array(
                PS_PAYED,
        ...
        $sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . ' WHERE 1 ' . order_query_sql('finished',
                'o.') . (' AND o.add_time >= \'' . $start_date . '\' AND o.add_time < \'') . ($end_date + 86400) . '\'' . $where . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
        $order_info['succeed_num'] = count($GLOBALS['db']->getAll($sql));
        $sql = 'SELECT o.order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o ' . ' WHERE o.order_status ' . db_create_in(array(
                OS_CANCELED,
                OS_INVALID
            )) . (' AND o.add_time >= \'' . $start_date . '\' AND o.add_time < \'') . ($end_date + 86400) . '\'' . $where . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
...
function get_order_general($type = 0)
{
    ...
    $sql = 'SELECT count(*) as total_order_num, ' . $total_fee . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' as o' . ' WHERE 1 ' . order_query_sql('finished',
            'o.') . $where . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0';
...
function get_area($countries, $pro = 0)
{
    ...
    $where .= ' AND i.order_status ' . db_create_in(array(
            OS_CONFIRMED,
            OS_SPLITED
        )) . '  AND i.shipping_status = \'' . SS_RECEIVED . '\' AND i.pay_status ' . db_create_in(array(
            PS_PAYED,
            PS_PAYING
        ));
...
if ($_REQUEST['act'] == 'list') {
    ...
    if (!$is_multi) {
        ...
        $smarty->assign('action_link', array(
            'text' => $_LANG['down_order_statistics'],
            'href' => 'order_stats.php?act=download&start_date=' . $start_date . '&end_date=' . $end_date . '&filename=' . $filename
        ));
    }
    ...
    if ($_REQUEST['act'] == 'list_old') {
        ...
        if ($is_multi) {
            ...
            if ($pay_item1) {
                ...
                foreach ($pay_arr as $row) {
                    $pay_xml .= '<set value=\'' . count($row['pay_arr']) . '\' name=\'' . strip_tags($row['pay_name']) . '\' color=\'' . $color_array[mt_rand(0,
                            7)] . '\'/>';
                }
            }
            ...
            foreach ($order_area as $val) {
                $order_area_xml .= '<set value=\'' . $val['area_num'] . '\' name=\'' . $val['region_name'] . '\' color=\'' . $color_array[mt_rand(0,
                        7)] . '\' />';
            }
            ...
            if ($ship_res1) {
                ...
                foreach ($ship_arr as $row) {
                    $ship_xml .= '<set value=\'' . count($row['ship_arr']) . '\' name=\'' . $row['ship_name'] . '\' color=\'' . $color_array[mt_rand(0,
                            7)] . '\' />';
                }
            }
