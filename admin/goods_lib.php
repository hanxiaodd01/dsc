...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            if (isset($GLOBALS['shop_id']) && 0 < $GLOBALS['shop_id']) {
                foreach ($img_list as $key => $gallery_img) {
                    ...
                    if (!empty($gallery_img['external_url'])) {
                        ...
                        $gallery_img['thumb_url'] = get_image_path($gallery_img['goods_id'], $gallery_img['thumb_url'],
                            true);
        ...
        $smarty->assign('ur_here',
            $is_add ? (empty($code) ? $_LANG['02_goods_add'] : $_LANG['51_virtual_card_add']) : ($_REQUEST['act'] == 'edit' ? $_LANG['edit_goods'] : $_LANG['copy_goods']));
        ...
        $smarty->assign('goods_type_name',
            $GLOBALS['db']->getOne(' SELECT cat_name FROM ' . $GLOBALS['ecs']->table('goods_type') . (' WHERE cat_id = \'' . $goods['goods_type'] . '\' ')));
        ...
        if ($_REQUEST['act'] == 'get_select_category_pro') {
            ...
            $smarty->assign('category_list',
                get_category_list($cat_id, 2, $seller_shop_cat, $goods['user_id'], $cat_level + 1));
            $result['content'] = $smarty->fetch('templates/library/get_select_category.lbi');
            exit(json_encode($result));
        } else {
            if ($_REQUEST['act'] == 'set_common_category_pro') {
                $cat_id = empty($_REQUEST['cat_id']) ? 0 : intval($_REQUEST['cat_id']);
                $result = array('error' => 0, 'message' => '', 'content' => '');
                $level_limit = 3;
                $category_level = array();
                ...
                if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                    ...
                    if (!empty($_POST['goods_img_url']) && $_POST['goods_img_url'] != 'http://' && (strpos($_POST['goods_img_url'],
                                'http://') !== false || strpos($_POST['goods_img_url'],
                                'https://') !== false) && $is_img_url == 1) {
                        ...
                        if ($proc_thumb && 0 < $image->gd_version() || $is_url_goods_img) {
                            if (empty($is_url_goods_img)) {
                                $img_wh = $image->get_width_to_height($goods_img, $GLOBALS['_CFG']['image_width'],
                                ...
                                $goods_img = $image->make_thumb(array('img' => $goods_img, 'type' => 1),
                                    $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height']);

                                if ($goods_img === false) {
                                    sys_msg($image->error_msg(), 1, array(), false);
                                }

                                $gallery_img = $image->make_thumb(array('img' => $gallery_img, 'type' => 1),
                                    $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height']);
                                ...
                                if (0 < intval($_CFG['watermark_place']) && !empty($GLOBALS['_CFG']['watermark'])) {
                                    if ($image->add_watermark($goods_img, '', $GLOBALS['_CFG']['watermark'],
                                    ...
                                    if ($_CFG['auto_generate_gallery']) {
                                        if ($image->add_watermark($img, '', $GLOBALS['_CFG']['watermark'],
                                                $GLOBALS['_CFG']['watermark_place'],
                                                $GLOBALS['_CFG']['watermark_alpha']) === false) {
                                            sys_msg($image->error_msg(), 1, array(), false);
                                        }
                            ...
                            if ($_CFG['auto_generate_gallery']) {
                                if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0) {
                                    $gallery_thumb = $image->make_thumb(array('img' => $img, 'type' => 1),
                                        $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height']);
                        ...
                        if ($proc_thumb && !empty($original_img)) {
                            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0) {
                                $goods_thumb = $image->make_thumb(array('img' => $original_img, 'type' => 1),
                                    $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height']);
                    ...
                    if ($is_insert) {
                        ...
                        get_goods_file_content($_REQUEST['goods_id'], $GLOBALS['_CFG']['goods_file'], $adminru['ru_id'],
                            $review_goods, $model_attr);
                    ...
                    $db->autoExecute($ecs->table('goods_extend'), $extend_arr, 'UPDATE',
                        'goods_id = \'' . $goods_id . '\'');
                    ...
                    if ($_REQUEST['act'] == 'batch') {
                        ...
                        if ($_REQUEST['act'] == 'edit_goods_name') {
                            ...
                            if ($exc->edit('goods_name = \'' . $goods_name . '\', last_update=' . gmtime(),
                                $goods_id)) {
                                clear_cache_files();
                            ...
                            if ($_REQUEST['act'] == 'check_goods_sn') {
                                ...
                                if ($_REQUEST['act'] == 'edit_goods_price') {
                                    ...
                                    if ($goods_price < 0 || $goods_price == 0 && $_POST['val'] != $goods_price) {
                                        ...
                                        if ($exc->edit('shop_price = \'' . $goods_price . '\', market_price = \'' . $price_rate . '\', last_update=' . gmtime(),
                                            $goods_id)) {
                                            clear_cache_files();
                                    ...
                                    if ($_REQUEST['act'] == 'toggle_on_sale') {
                                        ...
                                        if ($exc->edit('is_on_sale = \'' . $on_sale . '\', last_update=' . gmtime(),
                                            $goods_id)) {
                                            clear_cache_files();
                                            make_json_result($on_sale);
                                        }
                                    } else {
                                        if ($_REQUEST['act'] == 'edit_img_desc') {
                                            check_authz_json('goods_lib');
                                            ...
                                            if ($_REQUEST['act'] == 'main_dsc') {
                                                ...
                                                if ($_REQUEST['act'] == 'edit_sort_order') {
                                                    ...
                                                    if ($exc->edit('sort_order = \'' . $sort_order . '\', last_update=' . gmtime(),
                                                        $goods_id)) {
                                                        clear_cache_files();
                                                    ...
                                                    if ($_REQUEST['act'] == 'query') {
                                                        ...
                                                        $smarty->assign('record_count', $goods_list['record_count']);
                                                        $smarty->assign('page_count', $goods_list['page_count']);
                                                        $smarty->assign('use_storage',
                                                            empty($_CFG['use_storage']) ? 0 : 1);
                                                        $sort_flag = sort_flag($goods_list['filter']);
                                                        $smarty->assign($sort_flag['tag'], $sort_flag['img']);
                                                        $specifications = get_goods_type_specifications();
                                                        ...
                                                        if ($_REQUEST['act'] == 'remove') {
                                                            ...
                                                            $arr = array();
                                                            if (!empty($goods['goods_thumb']) && strpos($goods['goods_thumb'],
                                                                    'data/gallery_album') === false) {
                                                                $arr[] = $goods['goods_thumb'];
                                                                @unlink('../' . $goods['goods_thumb']);
                                                            }

                                                            if (!empty($goods['goods_img']) && strpos($goods['goods_img'],
                                                                    'data/gallery_album') === false) {
                                                                $arr[] = $goods['goods_img'];
                                                                @unlink('../' . $goods['goods_img']);
                                                            }

                                                            if (!empty($goods['original_img']) && strpos($goods['original_img'],
                                                                    'data/gallery_album') === false) {
                                                            ...
                                                            if ($exc->drop($goods_id)) {
                                                                ...
                                                                while ($row = $db->fetchRow($res)) {
                                                                    $arr = array();
                                                                    if (!empty($row['img_url']) && strpos($row['img_url'],
                                                                            'data/gallery_album') === false) {
                                                                        $arr[] = $row['img_url'];
                                                                        @unlink('../' . $row['img_url']);
                                                                    }

                                                                    if (!empty($row['thumb_url']) && strpos($row['thumb_url'],
                                                                            'data/gallery_album') === false) {
                                                                        $arr[] = $row['thumb_url'];
                                                                        @unlink('../' . $row['thumb_url']);
                                                                    }

                                                                    if (!empty($row['img_original']) && strpos($row['img_original'],
                                                                            'data/gallery_album') === false) {
                                                                ...
                                                                clear_cache_files();
                                                                $url = 'goods_lib.php?act=query&' . str_replace('act=remove',
                                                            ...
                                                            if ($_REQUEST['act'] == 'get_goods_list') {
                                                                ...
                                                                if ($_REQUEST['act'] == 'addImg') {
                                                                    ...
                                                                    handle_gallery_image_add($goods_id,
                                                                        $_FILES['img_url'], $img_desc, $img_file, '',
                                                                        '', 'ajax');
                                                                    clear_cache_files();
                                                                    ...
                                                                    if (isset($GLOBALS['shop_id']) && 0 < $GLOBALS['shop_id']) {
                                                                        foreach ($img_list as $key => $gallery_img) {
                                                                            $gallery_img[$key]['img_url'] = get_image_path($gallery_img['goods_id'],
                                                                                $gallery_img['img_original'], false,
                                                                                'gallery');
                                                                            $gallery_img[$key]['thumb_url'] = get_image_path($gallery_img['goods_id'],
                                                                                $gallery_img['img_original'], true,
                                                                                'gallery');
                                                                        }
                                                                    } else {
                                                                    ...
                                                                    if ($_REQUEST['act'] == 'img_default') {
                                                                                ...
                                                                                if (isset($GLOBALS['shop_id']) && 0 < $GLOBALS['shop_id']) {
                                                                                    foreach ($img_list as $key => $gallery_img) {
                                                                                        ...
                                                                                        if (!empty($gallery_img['external_url'])) {
                                                                                            ...
                                                                                            $img_list[$key]['img_url'] = get_image_path($gallery_img['goods_id'],
                                                                                                $gallery_img['img_original'],
                                                                                                false, 'gallery');
                                                                                            $img_list[$key]['thumb_url'] = get_image_path($gallery_img['goods_id'],
                                                                                                $gallery_img['img_original'],
                                                                                                true, 'gallery');
                                                                                        }
                                                                                    }
                                                                        ...
                                                                        if ($_REQUEST['act'] == 'remove_consumption') {
                                                                            require ROOT_PATH . '/includes/cls_json.php';
                                                                            $json = new JSON();
                                                                            $result = array(
                                                                                'error' => 0,
                                                                            ...
                                                                            if ($_REQUEST['act'] == 'gallery_album_dialog') {
                                                                                require ROOT_PATH . '/includes/cls_json.php';
                                                                                $json = new JSON();
                                                                                $result = array(
                                                                                    'error' => 0,
                                                                                ...
                                                                                if ($_REQUEST['act'] == 'gallery_album_pic') {
                                                                                    require ROOT_PATH . '/includes/cls_json.php';
                                                                                    $json = new JSON();
                                                                                    $result = array(
                                                                                        'error' => 0,
                                                                                    ...
                                                                                    if ($_REQUEST['act'] == 'scan_code') {
                                                                                        ...
                                                                                        $data = get_jsapi(array(
                                                                                            'appkey' => $config['js_appkey'],
                                                                                            'barcode' => $bar_code
                                                                                        ));
                                                                                        ...
                                                                                        if ($data['status'] != 0) {
                                                                                            ...
                                                                                            if (strpos($data['result']['grossweight'],
                                                                                                    $_LANG['unit_kg']) !== false) {
                                                                                                $goods_weight = floatval(str_replace($_LANG['unit_kg'],
                                                                                                    '',
                                                                                                    $data['result']['grossweight']));
                                                                                            } else {
                                                                                                if (strpos($data['result']['grossweight'],
                                                                                                        $_LANG['unit_g']) !== false) {
                                                                                                    $goods_weight = floatval(str_replace($_LANG['unit_kg'],
                                                                                                            '',
                                                                                                            $data['result']['grossweight'])) / 1000;
                                                                                                }
                                                                                            }
                                                                                        ...
                                                                                        if ($_REQUEST['act'] == 'drop_image') {
                                                                                            ...
                                                                                            $arr = array();
                                                                                            if ($row['img_url'] != '' && is_file($img_url) && strpos($row['img_url'],
                                                                                                    'data/gallery_album') === false) {
                                                                                                $arr[] = $row['img_url'];
                                                                                                @unlink($img_url);
                                                                                            }

                                                                                            if ($row['thumb_url'] != '' && is_file($thumb_url) && strpos($row['img_url'],
                                                                                                    'data/gallery_album') === false) {
                                                                                                $arr[] = $row['thumb_url'];
                                                                                                @unlink($thumb_url);
                                                                                            }

                                                                                            if ($row['img_original'] != '' && is_file($img_original) && strpos($row['img_url'],
                                                                                                    'data/gallery_album') === false) {
                                                                                                $arr[] = $row['img_original'];
                                                                                                @unlink($img_original);
                                                                                            }
                                                                                            ...
                                                                                            if ($_REQUEST['act'] == 'import_seller_goods') {
                                                                                                ...
                                                                                                if ($_REQUEST['act'] == 'import_action') {
                                                                                                    ...
                                                                                                    if ($_REQUEST['act'] == 'import_action_list') {
                                                                                                        admin_priv('goods_lib_list');
                                                                                                        $user_id = $_REQUEST['user_id'] ? intval($_REQUEST['user_id']) : 0;
                                                                                                        include_once ROOT_PATH . 'includes/cls_json.php';
                                                                                                        $json = new JSON();
                                                                                                        ...
                                                                                                        $goods_list = $ecs->page_array($page_size,
                                                                                                            $page,
                                                                                                            $goods_list);
                                                                                                        ...
                                                                                                        if ($_REQUEST['act'] == 'get_shopname') {
                                                                                                            ...
                                                                                                            foreach ($seller_ids as $k => $v) {
                                                                                                                if (is_numeric(stripos(get_shop_name($v,
                                                                                                                        1),
                                                                                                                        $shop_name)) || empty($shop_name)) {
                                                                                                                    $seller_list[$k]['shop_name'] = get_shop_name($v,
                                                                                                                        1);
                                                                                                                    $seller_list[$k]['user_id'] = $v;
                                                                                                                }
