...
function services_list()
{
    ...
    if ($filter['extension_code'] == 'platform') {
        ...
        if ($filter['extension_code'] == 'seller') {
            $sql .= ' AND a.ru_id <> 0 AND s.status = 1';
            $sqlCount .= ' AND a.ru_id <> 0 AND s.status = 1';
        } else {
...
');
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $smarty->assign('action_link',
                array('text' => $_LANG['services_list'], 'href' => 'services.php?act=list' . $pix));
            $admins = admin_list();
...
');
                ...
                if ($_REQUEST['act'] == 'remove') {
                    ...
                    $sql = 'UPDATE' . $ecs->table('im_dialog') . ' SET services_id = 0  WHERE services_id = ' . $id;
                    $res = $db->query($sql);
                                        ...
                                        if ($_REQUEST['act'] == 'generage_word') {
                                            ...
                                            if ($list) {
                                                ...
                                                foreach ($list as $k => $v) {
                                                    ...
                                                    $excel->setActiveSheetIndex(0)->setCellValue('A' . $num,
                                                        $userName)->setCellValue('B' . $num,
                                                        strip_tags($v['message']))->setCellValue('C' . $num,
                                                        $v['add_time']);
                                                }
                                                ...
                                                $objWriter = \PhpOffice\PhpSpreadsheet\IOFactory::createWriter($excel,
                                                    'Xls');
