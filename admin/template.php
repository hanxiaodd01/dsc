...
if ($_REQUEST['act'] == 'setup') {
    ...
    while ($row = $db->FetchRow($rc)) {
        if (0 < $row['type']) {
            $db_dyna_libs[$row['region']][$row['library']][] = array(
                'id' => $row['id'],
                'number' => $row['number'],
                'type' => $row['type'],
                'floor_tpl' => $row['floor_tpl']
            );
        } else {
    ...
    foreach ($temp_libs as $val) {
        if ($val['lib'] == 'cat_goods') {
            ...
            if ($val['lib'] == 'brand_goods') {
                ...
                if ($val['lib'] == 'cat_articles') {
                    if (isset($db_dyna_libs[$val['region']][$val['library']]) && ($row = array_shift($db_dyna_libs[$val['region']][$val['library']]))) {
                        $cat_articles[] = array(
                            'region' => $val['region'],
                            'sort_order' => $val['sort_order'],
                            'number' => $row['number'],
                            'cat_articles_id' => $row['id'],
                            'cat' => article_cat_list_new(0, $row['id'])
                        );
                    } else {
                        $cat_articles[] = array(
                            'region' => $val['region'],
                            ...
                            'number' => 0,
                            'cat_articles_id' => $row['id'],
                            'cat' => article_cat_list_new(0)
                        );
                    }
                } else {
                    if ($val['lib'] == 'ad_position') {
                        if (isset($db_dyna_libs[$val['region']][$val['library']]) && ($row = array_shift($db_dyna_libs[$val['region']][$val['library']]))) {
                            $ad_positions[] = array(
                                'region' => $val['region'],
                                'sort_order' => $val['sort_order'],
...
if ($_REQUEST['act'] == 'setting') {
    ...
    foreach ($_POST['regions'] as $key => $val) {
        ...
        if (!in_array($key,
                $GLOBALS['dyna_libs']) && ((isset($_POST['display'][$key]) && ($_POST['display'][$key] == 1)) || (0 < $number))) {
    ...
    $template_content = read_static_flie_cache($_POST['template_file'], 'dwt',
        ROOT_PATH . '/themes/' . $curr_template . '/');
    ...
    foreach ($org_regions as $region) {
        ...
        foreach ($post_regions as $lib) {
            if ($lib['region'] == $region) {
                ...
                $lib_content = preg_replace('/<meta\\shttp-equiv=["|\']Content-Type["|\']\\scontent=["|\']text\\/html;\\scharset=.*["|\']>/i',
                    '', $lib_content);
    ...
    $template_file = write_static_file_cache($_POST['template_file'], $template_content, 'dwt',
        ROOT_PATH . '/themes/' . $curr_template . '/');
    ...
    if ($template_file) {
        ...
        $lnk[] = array(
            'text' => $_LANG['go_back'],
            'href' => 'template.php?act=setup&template_file=' . $_POST['template_file']
        );
        sys_msg($_LANG['setup_success'], 0, $lnk);
    } else {
        sys_msg(sprintf($_LANG['modify_dwt_failed'],
            'themes/' . $curr_template . '/' . $_POST['template_file'] . '.dwt'), 1, null, false);
    }
}
...
if ($_REQUEST['act'] == 'update_library') {
    ...
    $lib_file_html = write_static_file_cache($_POST['lib'], $html, 'lbi',
        ROOT_PATH . '/themes/' . $_CFG['template'] . '/library/');
    if ((@file_exists($lib_file) === true) && $lib_file_html) {
        write_static_file_cache($_CFG['template'] . '-' . $_POST['lib'], $org_html, 'lbi',
            ROOT_PATH . '/temp/backup/library/');
...
if ($_REQUEST['act'] == 'act_backup_setting') {
    ...
    $sql = 'INSERT INTO ' . $ecs->table('template') . ' (filename, region, library, sort_order, id, number, type, theme, remarks)' . ' SELECT filename, region, library, sort_order, id, number, type, theme, \'' . $remarks . '\'' . ' FROM ' . $ecs->table('template') . ' WHERE remarks = \'\' AND theme = \'' . $_CFG['template'] . '\'' . ' AND ' . db_create_in($files,
            'filename');
    $db->query($sql);
        ...
        if ($arr) {
            ...
            foreach ($arr as $val) {
                if (file_exists(ROOT_PATH . 'themes/' . $_CFG['template'] . $val['library'])) {
                    ...
                    $lib_content = preg_replace('/<meta\\shttp-equiv=["|\']Content-Type["|\']\\scontent=["|\']text\\/html;\\scharset=utf-8"|\']>/i',
                        '', $lib_content);
                    $lib_content = str_replace("\xef\xbb\xbf", '', $lib_content);
            ...
            foreach ($data as $file => $regions) {
                $pattern = '/(?:<!--\\s*TemplateBeginEditable\\sname="(' . implode('|',
                        array_keys($regions)) . ')"\\s*-->)(?:.*?)(?:<!--\\s*TemplateEndEditable\\s*-->)/se';
                $template_content = read_static_flie_cache($file, 'dwt',
                    ROOT_PATH . 'themes/' . $_CFG['template'] . '/');
                $match = array();
                $template_content = preg_replace($pattern,
                    "'<!-- TemplateBeginEditable name=\"\\1\" -->\r\n' . \$regions['\\1'] . '\r\n<!-- TemplateEndEditable -->';",
                    $template_content);
                write_static_file_cache($file, $template_content, 'dwt',
                    ROOT_PATH . 'themes/' . $_CFG['template'] . '/');
            }

            $sql = 'DELETE FROM ' . $ecs->table('template') . ' WHERE remarks = \'\' AND  theme = \'' . $_CFG['template'] . '\'' . ' AND ' . db_create_in(array_keys($data),
                    'filename');
            $db->query($sql);
