...
function merchants_commission_list()
{
    ...
    if ($result === false) {
        ...
        $filter['start_time'] = empty($_REQUEST['start_time']) ? '' : (0 < strpos($_REQUEST['start_time'],
            '-') ? local_strtotime($_REQUEST['start_time']) : $_REQUEST['start_time']);
        $filter['end_time'] = empty($_REQUEST['end_time']) ? '' : (0 < strpos($_REQUEST['end_time'],
            '-') ? local_strtotime($_REQUEST['end_time']) : $_REQUEST['end_time']);
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if (file_exists(MOBILE_DRP)) {
            ...
            if ($commission_info['commission_model']) {
                ...
                $row[$i]['formated_brokerage_amount'] = price_format($order_commission + $row[$i]['shipping_fee'],
                    true);
                $effective_amount_price = $order_commission;
                $row[$i]['effective_amount_price'] = number_format($effective_amount_price, 2, '.', '');
            } else {
                $row[$i]['formated_brokerage_amount'] = price_format($total_return_amount * $percent_value + $row[$i]['shipping_fee'] + $brokerage_amount['should_amount'],
                    true);
            ...
            if ($commission_info['commission_model']) {
                ...
                $row[$i]['formated_brokerage_amount'] = price_format($total_return_amount * $percent_value + $row[$i]['shipping_fee'] + $goods_rate['should_amount'],
                    true);
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link3',
        array('href' => 'javascript:download_list();', 'text' => $_LANG['export_all_suppliers']));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'commission_amount') {
            $result = array('error' => 0, 'message' => '', 'content' => '');
            $seller = isset($_REQUEST['seller']) && !empty($_REQUEST['seller']) ? dsc_addslashes($_REQUEST['seller'],
                1) : 0;
            ...
            if ($_REQUEST['act'] == 'edit_suppliers_ser_name') {
                check_authz_json('merchants_commission');
                $id = intval($_POST['id']);
            ...
            WHERE server_id = \'' . $id . '\'');
                    $db->query($sql);
                    clear_cache_files();
                    $url = 'merchants_commission.php?act=query&' . str_replace('act=remove', '',
                ...
                WHERE suppliers_ser_id ' . db_create_in($ids);
                        ...
                        if (in_array($_REQUEST['act'], array('add', 'edit'))) {
                            ...
                            $smarty->assign('menu_select',
                                array('action' => '17_merchants', 'current' => 'suppliers_list_server'));
                            $smarty->assign('action_link', array(
                                'href' => 'merchants_commission.php?act=list',
                                'text' => $_LANG['suppliers_list_server']
                            ));
...
');
                            ...
                            if (in_array($_REQUEST['act'], array('insert', 'update'))) {
                                ...
                                if ($_REQUEST['act'] == 'insert') {
                                    ...
                                    $db->autoExecute($ecs->table('merchants_server'), $server, 'UPDATE',
                                        'user_id = \'' . $user_id . '\'');
                                }
                                ...
                                $links[] = array(
                                    'href' => 'merchants_commission.php?act=edit&id=' . $user_id,
                                    'text' => $_LANG['back_suppliers_server_list']
                                );
                                ...
                                if ($_REQUEST['act'] == 'order_list') {
                                    admin_priv('merchants_commission');
                                    $smarty->assign('menu_select',
                                        array('action' => '17_merchants', 'current' => 'brokerage_order_list'));
                                    $user_id = isset($_REQUEST['id']) && !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
...
');
                                    ...
                                    $smarty->assign('action_link', array(
                                        'href' => 'javascript:order_downloadList();',
                                        'text' => $_LANG['export_merchant_commission']
                                    ));
                                    ...
                                    if ($_REQUEST['act'] == 'order_query') {
                                        ...
                                        make_json_result($smarty->fetch('merchants_order_list.dwt'), '', array(
                                            'filter' => $order_list['filter'],
                                            'page_count' => $order_list['page_count']
                                        ));
                                    } else {
                                        if ($_REQUEST['act'] == 'edit_percent_value') {
                                            check_authz_json('merchants_commission');
                                            ...
                                            if ($_REQUEST['act'] == 'toggle_on_settlement') {
                                                check_authz_json('merchants_commission');
                                                $order_id = intval($_POST['id']);
                                                ...
                                                $amount = get_seller_settlement_amount($order_id,
                                                    $order_goods['ru_id']);
                                                ...
                                                $sql = 'UPDATE ' . $ecs->table('seller_shopinfo') . (' SET seller_money = seller_money + ' . $amount . ' WHERE ru_id = \'') . $order_goods['ru_id'] . '\'';
                                                $db->query($sql);
                                                $change_desc = sprintf($_LANG['01_admin_settlement'],
                                                    $_SESSION['admin_name']);
                                                ...
                                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_account_log'),
                                                    $user_account_log, 'INSERT');
                                                ...
                                                if ($_REQUEST['act'] == 'toggle_on_frozen') {
                                                    check_authz_json('merchants_commission');
                                                    $order_id = intval($_POST['id']);
                                                    $on_sale = intval($_POST['val']);
                                                    $nowTime = gmtime();
                                                    ...
                                                    if ($_REQUEST['act'] == 'query_merchants_info') {
                                                        check_authz_json('merchants_commission');
                                                        $user_id = empty($_REQUEST['user_id']) ? 0 : intval($_REQUEST['user_id']);
                                                        $date = array('shoprz_brandName, shopNameSuffix');
                                                        $user = get_table_date('merchants_shop_information',
...
if ($_REQUEST['act'] == 'merchant_download') {
    ...
    if (!empty($merchants_download_content)) {
        foreach ($merchants_download_content as $k => $merchants_order_list) {
            ...
            $content .= ecs_iconv(EC_CHARSET, 'GB2312',
                    $merchants_order_list['orders']['brokerage_amount']['all_price']) . '	
';
            $content .= ecs_iconv(EC_CHARSET, 'GB2312', $_LANG['is_settlement_amount']);
            $content .= ecs_iconv(EC_CHARSET, 'GB2312',
                    $merchants_order_list['orders']['brokerage_amount']['is_settlement_price']) . '	
';
            $content .= ecs_iconv(EC_CHARSET, 'GB2312', $_LANG['no_settlement_amount']);
            $content .= ecs_iconv(EC_CHARSET, 'GB2312',
                    $merchants_order_list['orders']['brokerage_amount']['no_settlement_price']) . '	
';
    ...
    if ($_REQUEST['act'] == 'handle_log') {
        ...
        $smarty->assign('action_link', array(
            'text' => $_LANG['brokerage_order_list'],
            'href' => 'merchants_commission.php?act=order_list&id=' . $user_id
        ));
        $id = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
        ...
        if ($_REQUEST['act'] == 'search_users') {
            ...
            if ($_REQUEST['act'] == 'Ajax_handle_log') {
                ...
                if ($_REQUEST['act'] == 'commission_bill') {
                    admin_priv('merchants_commission');
...
');
                    ...
                    if ($_REQUEST['act'] == 'commission_bill_query') {
                        check_authz_json('merchants_commission');
                        $result = commission_bill_list();
                        $smarty->assign('bill_list', $result['bill_list']);
                        $smarty->assign('filter', $result['filter']);
                        ...
                        if ($_REQUEST['act'] == 'bill_remove') {
                            ...
                            clear_cache_files();
                            $url = 'merchants_commission.php?act=commission_bill_query&id=' . $seller_id . '&' . str_replace('act=bill_remove',
                                    '', $_SERVER['QUERY_STRING']);
                            ecs_header('Location: ' . $url . '
');
                            exit();
                        } else {
                            if ($_REQUEST['act'] == 'bill_detail') {
                                admin_priv('merchants_commission');
                                $smarty->assign('menu_select',
                                    array('action' => '17_merchants', 'current' => 'commission_bill'));
...
');
                                ...
                                $smarty->assign('action_link', array(
                                    'href' => 'merchants_commission.php?act=commission_bill&id=' . $bill['seller_id'],
                                    'text' => $_LANG['commission_bill']
                                ));
                                ...
                                if ($_REQUEST['act'] == 'bill_detail_query') {
                                    check_authz_json('merchants_commission');
                                    $result = bill_detail_list();
                                    $smarty->assign('bill_list', $result['bill_list']);
                                    $smarty->assign('filter', $result['filter']);
...
');
                                        ...
                                        if ($_REQUEST['act'] == 'bill_goods_query') {
                                            check_authz_json('merchants_commission');
                                            $result = bill_goods_list();
                                            $smarty->assign('goods_list', $result['goods_list']);
                                            $smarty->assign('filter', $result['filter']);
                                            ...
                                            $sort_flag = sort_flag($result['filter']);
                                            $smarty->assign($sort_flag['tag'], $sort_flag['img']);
                                            make_json_result($smarty->fetch('merchants_bill_goods.dwt'), '', array(
                                                'filter' => $result['filter'],
                                                'page_count' => $result['page_count']
                                            ));
                                        } else {
                                            if ($_REQUEST['act'] == 'apply_for') {
                                                admin_priv('merchants_commission');
                                                $smarty->assign('menu_select',
                                                    array('action' => '17_merchants', 'current' => 'commission_bill'));
...
');
                                                    ...
                                                    if ($type == 'bill_frozen' || $type == 'bill_unfreeze') {
                                                        ...
                                                        if ($type == 'bill_frozen') {
                                                            ...
                                                            if ($bill['chargeoff_status'] == 2) {
                                                                ...
                                                                $db->query($sql);
                                                                $change_desc = sprintf($_LANG['seller_bill_unfreeze'],
                                                                    $_SESSION['admin_name']);
                                                                $user_account_log = array(
                                                        ...
                                                        if ($check_status == 1 && $bill['bill_apply'] == 1 && $bill['chargeoff_status'] <= 1) {
                                                            ...
                                                            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_commission_bill'),
                                                                $other, 'UPDATE', 'id = \'' . $bill_id . '\'');
                                                            $sys_msg = $_LANG['update_bill_success'];
                                                            $apply_sn = sprintf($_LANG['seller_bill_account'],
                                                                $bill['bill_sn']);
                                                            ...
                                                            $db->autoExecute($ecs->table('seller_account_log'), $other,
                                                                'INSERT');
                                                            $sql = 'UPDATE ' . $ecs->table('seller_shopinfo') . ' SET seller_money = seller_money + \'' . $bill['should_amount'] . '\' WHERE ru_id = \'' . $bill['seller_id'] . '\'';
                                                            $db->query($sql);
                                                            $change_desc = sprintf($_LANG['seller_bill_settlement'],
                                                                $_SESSION['admin_name'] . '【' . $bill['bill_sn'] . '】');
                                                            $user_account_log = array(
                                                                'user_id' => $bill['seller_id'],
                                                            ...
                                                            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_bill_order'),
                                                                $order_other, 'UPDATE',
                                                                'bill_id = \'' . $bill_id . '\'');
                                                    ...
                                                    $link[0] = array(
                                                        'href' => 'merchants_commission.php?act=commission_bill&id=' . $seller_id,
                                                        'text' => $_LANG['commission_bill']
                                                    );
                                                    sys_msg($sys_msg, 0, $link);
                                                } else {
                                                    if ($_REQUEST['act'] == 'commission_chang') {
                                                        ...
                                                        if ($_REQUEST['act'] == 'chang_commission_list') {
                                                            ...
                                                            $seller_list = read_static_cache('seller_list',
                                                                '/data/sc_file/');
                                                            $seller_list = $ecs->page_array($page_size, $page,
                                                                $seller_list);
                                                            $result['list'] = $seller_list['list'][0];

                                                            if ($result['list']) {
                                                                $result['list']['shop_name'] = get_shop_name($result['list']['seller_id'],
                                                                    1);
                                                            ...
                                                            if ($_REQUEST['act'] == 'replacement_order') {
                                                                ...
                                                                $order_list = read_static_cache('commission_replacement_order' . $seller_id,
                                                                    '/data/sc_file/');
                                                                ...
                                                                if (!$order_list) {
                                                                    ...
                                                                    write_static_cache('commission_replacement_order' . $seller_id,
                                                                        $order_list, '/data/sc_file/');
                                                                }
                                                                ...
                                                                if (0 < $record_count) {
                                                                    ...
                                                                    $last_year_end = local_strtotime(local_date('Y-m-d',
                                                                        gmtime()));
                                                                    ...
                                                                    if (!$bill_id) {
                                                                        ...
                                                                        $other = array(
                                                                            'seller_id' => $seller_id,
                                                                            'bill_sn' => $bill_sn,
                                                                            'proportion' => $seller_info['percent_value'],
                                                                            'start_time' => $last_year_start,
                                                                            'end_time' => $last_year_end,
                                                                            'bill_cycle' => $seller_info['cycle'],
                                                                ...
                                                                if ($_REQUEST['act'] == 'chang_replacement_order') {
                                                                    include_once ROOT_PATH . 'includes/cls_json.php';
                                                                    $json = new JSON();
                                                                    ...
                                                                    $server = $GLOBALS['db']->getRow($sql);
                                                                    $order_list = $ecs->page_array($page_size, $page,
                                                                        $order_list);
                                                                    ...
                                                                    if ($order_list['filter']['page_count'] < $page) {
                                                                        $bill = array(
                                                                            ...
                                                                            'bill_id' => $bill_id,
                                                                            'commission_model' => $server['commission_model'],
                                                                            'proportion' => $server['percent_value']
                                                                        );
