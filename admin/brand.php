    ...
    if ($_REQUEST['act'] == 'add') {
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            if ($_REQUEST['act'] == 'edit') {
                ...
                $brand = $db->GetRow($sql);
                $brand['brand_logo'] = !empty($brand['brand_logo']) ? get_image_path($brand['brand_id'],
                    DATA_DIR . '/brandlogo/' . $brand['brand_logo']) : '';
                $brand['index_img'] = !empty($brand['index_img']) ? get_image_path($brand['brand_id'],
                    DATA_DIR . '/indeximg/' . $brand['index_img']) : '';
                $brand['brand_bg'] = !empty($brand['brand_bg']) ? get_image_path($brand['brand_id'],
                    DATA_DIR . '/brandbg/' . $brand['brand_bg']) : '';
                $smarty->assign('ur_here', $_LANG['brand_edit']);
                $smarty->assign('action_link', array(
                    'text' => $_LANG['06_goods_brand_list'],
                    'href' => 'brand.php?act=list&' . list_link_postfix()
                ));
                ...
                if ($_REQUEST['act'] == 'updata') {
                    ...
                    if ($_REQUEST['act'] == 'brand_separate') {
                        admin_priv('brand_manage');
                        ...
                        if ($_REQUEST['act'] == 'brand_separate_initial') {
                            ...
                            if ($result['list']) {
                                ...
                                if ($result['list']['user_id']) {
                                    $db->autoExecute($ecs->table('goods'), $other, 'UPDATE',
                                        'user_id = \'' . $result['list']['user_id'] . '\' AND brand_id = \'' . $result['list']['bid'] . '\' AND user_brand = 0');
                                    $db->autoExecute($ecs->table('collect_brand'), $other, 'UPDATE',
                                        'ru_id = \'' . $result['list']['user_id'] . '\' AND brand_id = \'' . $result['list']['bid'] . '\' AND user_brand = 0');
                                    $sql = 'SELECT GROUP_CONCAT(act_id) AS act_id FROM ' . $GLOBALS['ecs']->table('favourable_activity') . ' WHERE act_range = 2 AND user_id = \'' . $result['list']['user_id'] . '\' ' . 'AND FIND_IN_SET(\'' . $result['list']['bid'] . '\', act_range_ext) AND ' . db_create_in($result['list']['bid'],
                                            'user_range_ext', 'NOT') . ' LIMIT 1';
                            ...
                            if ($_REQUEST['act'] == 'create_brand_letter') {
                                admin_priv('brand_manage');
                                $smarty->assign('ur_here', $_LANG['06_goods_brand_list']);
                                ...
                                if ($_REQUEST['act'] == 'create_brand_initial') {
                                    include_once ROOT_PATH . 'includes/cls_json.php';
                                    $json = new JSON();
                                    $page = !empty($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
                                    $page_size = isset($_REQUEST['page_size']) ? intval($_REQUEST['page_size']) : 1;
                                    ...
                                    if ($result['list']) {
                                        $arr = array();
                                        if (!empty($result['list']['brand_first_char']) && in_array(strtoupper($result['list']['brand_first_char']),
                                                range('A', 'Z'))) {
                                            ...
                                            if ($ecs->preg_is_letter($str_first)) {
                                                ...
                                                foreach ($letters as $key => $val) {
                                                    ...
                                                    if (in_array($str, range('A', 'Z'))) {
                                                        ...
                                                        if (strtolower($val) == substr($pin->Pinyin($result['list']['brand_name'],
                                                                EC_CHARSET), 0, 1)) {
                                                            $arr['brand_id'] = $result['list']['brand_id'];
                                                            $arr['brand_name'] = $result['list']['brand_name'];
                                                            $arr['letter'] = $val;
                                    ...
                                    if ($_REQUEST['act'] == 'edit_brand_name') {
                                        ...
                                        if ($_REQUEST['act'] == 'edit_brand_letter') {
                                            check_authz_json('brand_manage');
                                            $id = intval($_POST['id']);
                                            $name = json_str_iconv(trim($_POST['val']));
                                            ...
                                            if ($_REQUEST['act'] == 'add_brand') {
                                                ...
                                                if ($_REQUEST['act'] == 'edit_sort_order') {
                                                    ...
                                                    if ($_REQUEST['act'] == 'toggle_show') {
                                                        ...
                                                        if ($_REQUEST['act'] == 'toggle_recommend') {
                                                            check_authz_json('brand_manage');
                                                            ...
                                                            if ($_REQUEST['act'] == 'remove') {
                                                                check_authz_json('brand_manage');
                                                                $id = intval($_GET['id']);
                                                                get_del_batch('', $id, array('brand_logo'), 'brand_id',
                                                                    'brand', 0, DATA_DIR . '/brandlogo/');
                                                                $exc->drop($id);
                                                                $sql = 'UPDATE ' . $ecs->table('goods') . (' SET brand_id=0 WHERE brand_id=\'' . $id . '\'');
                                                                $db->query($sql);
                                                                $url = 'brand.php?act=query&' . str_replace('act=remove',
                                                                ...
                                                                if ($_REQUEST['act'] == 'drop_logo') {
                                                                    ...
                                                                    get_del_batch('', $brand_id, array('brand_logo'),
                                                                        'brand_id', 'brand', 0,
                                                                        DATA_DIR . '/brandlogo/');
                                                                    ...
                                                                    $link = array(
                                                                        array(
                                                                            'text' => $_LANG['brand_edit_lnk'],
                                                                            'href' => 'brand.php?act=edit&id=' . $brand_id
                                                                        ),
                                                                        array(
                                                                            'text' => $_LANG['brand_list_lnk'],
                                                                            'href' => 'brand.php?act=list'
                                                                        )
                                                                    );
                                                                    sys_msg($_LANG['drop_brand_logo_success'], 0,
                                                                        $link);
                                                                } else {
                                                                    if ($_REQUEST['act'] == 'query') {
                                                                        ...
                                                                        $smarty->assign('record_count',
                                                                            $brand_list['record_count']);
                                                                        $smarty->assign('page_count',
                                                                            $brand_list['page_count']);
                                                                        make_json_result($smarty->fetch('brand_list.dwt'),
                                                                            '', array(
                                                                                'filter' => $brand_list['filter'],
                                                                                'page_count' => $brand_list['page_count']
                                                                            ));
                                                                    }
                                                                }
                                                            }
