    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $smarty->assign('action_link',
                array('href' => 'touch_ad_position.php?act=list', 'text' => $_LANG['ad_position']));
            $smarty->assign('posit_arr',
                array('position_style' => '{foreach $ads as $ad}' . "\n" . '<div class="swiper-slide">{$ad}</div>' . "\n" . '{/foreach}'));
            $smarty->assign('type', '1');
            assign_query_info();
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                if ($exc->num('position_name', $position_name, 0, $where) == 0) {
                    ...
                    sys_msg($_LANG['add'] . '&nbsp;' . stripslashes($position_name) . '&nbsp;' . $_LANG['attradd_succed'],
                        0, $link);
                } else {
                    ...
                    if ($_REQUEST['act'] == 'update') {
                        ...
                        if ($db->getOne($sql) == 0) {
                            ...
                            if ($db->query($sql)) {
                                ...
                                clear_cache_files();
                                $link[] = array(
                                    'text' => $_LANG['back_position_list'],
                                    'href' => 'touch_ad_position.php?act=list'
                                );
                                sys_msg($_LANG['edit'] . ' ' . stripslashes($position_name) . ' ' . $_LANG['attradd_succed'],
                                    0, $link);
                            }
                        } else {
                            $link[] = array('text' => $_LANG['go_back'], 'href' => 'javascript:history.back(-1)');
                        ...
                        if ($_REQUEST['act'] == 'edit_position_name') {
                            ...
                            if ($exc->num('position_name', $position_name, $id, 'theme=\'' . $template . '\'') != 0) {
                                ...
                                if ($exc->edit('position_name = \'' . $position_name . '\', theme=\'' . $template . '\'',
                                    $id)) {
                                    admin_log($position_name, 'edit', 'ads_position');
                            ...
                            if ($_REQUEST['act'] == 'edit_ad_width') {
                                ...
                                if ($_REQUEST['act'] == 'edit_ad_height') {
                                    ...
                                    if ($_REQUEST['act'] == 'remove') {
                                        ...
                                        $url = 'touch_ad_position.php?act=query&' . str_replace('act=remove', '',
