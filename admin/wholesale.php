...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link2',
        array('href' => 'wholesale.php?act=batch_add', 'text' => $_LANG['add_batch_wholesale']));
    ...
    if (isset($_POST['type'])) {
        ...
        elseif ($_POST['type'] == 'review_to') {
            ...
            if ($db->query($sql)) {
                $lnk[] = array(
                    'text' => $_LANG['back_wholesale_list'],
                    'href' => 'wholesale.php?act=list&seller_list=1&' . list_link_postfix()
                );
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
    ...
    $smarty->assign('transport_list',
        get_table_date("goods_transport", "ru_id='$adminru[ru_id]'", array('tid, title'), 1)); //商品运费 by wu
    assign_query_info();
...
//        sys_msg($_LANG['pls_set_user_rank']);
    ...
    if (intval($_POST['price_model']) && isset($_POST['volume_number']) && isset($_POST['volume_price'])) {
        handle_wholesale_volume_price($goods_id, intval($_POST['price_model']), $_POST['volume_number'],
            $_POST['volume_price'], $_POST['id']);
    }
    ...
    foreach ($product['product_sn'] as $key => $value) {
        ...
        if ($product['product_id'][$key]) {
            ...
            $goods_attr_id = handle_wholesale_goods_attr($product['goods_id'], $id_list, $is_spec_list,
                $value_price_list);
    ...
    if (count($attr_arr) > 0) {
        ...
        if ($_REQUEST['act'] == 'set_attribute_table') {
            ...
            make_json_result($smarty->fetch('library/wholesale_attribute_query.lbi'), '',
                array('filter' => $products_list['filter'], 'page_count' => $products_list['page_count']));
        }
        /* ajax分页 end */
    }
        ...
        if (!empty($goods_list)) {
            foreach ($goods_list as $k => $v) {
                ...
                if ($v['promote_price'] > 0) {
                    $goods_list[$k]['promote_price'] = bargain_price($v['promote_price'], $v['promote_start_date'],
                        $v['promote_end_date']);
                } else {
                    $goods_list[$k]['promote_price'] = 0;
                }
                if ($v['goods_id'] > 0 && in_array($v['goods_id'],
...
//-- 添加属性图片 //ecmoban模板堂 --zhuo
    ...
    elseif ($_REQUEST['act'] == 'add_attr_img') {
        ...
        $goods_attr_info = get_table_date('wholesale_goods_attr',
            "goods_id = '$goods_id' and attr_id = '$attr_id' and goods_attr_id = '$goods_attr_id'", $goods_attr_date);
...
//-- 添加属性图片插入数据 //ecmoban模板堂 --zhuo
    ...
    elseif ($_REQUEST['act'] == 'insert_attr_img') {
        ...
        if ($other['attr_checked'] == 1) {
            $db->autoExecute($ecs->table('wholesale_goods_attr'), array('attr_checked' => 0), 'UPDATE',
                'attr_id = ' . $attr_id . ' and goods_id = ' . $goods_id);
            $result['is_checked'] = 1;
        }

        $db->autoExecute($ecs->table('wholesale_goods_attr'), $other, 'UPDATE',
...
//-- 删除属性图片 //ecmoban模板堂 --zhuo
    ...
    elseif ($_REQUEST['act'] == 'drop_attr_img') {
        ...
        $other['attr_img_flie'] = '';
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('wholesale_goods_attr'), $other, "UPDATE",
