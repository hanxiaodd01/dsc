...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_lib_cat'), $args, 'update',
        'cat_id=\'' . $cat_id . '\'');
}
...
if ($_REQUEST['act'] == 'list') {
    ...
    if (isset($_REQUEST['back_level']) && 0 < $_REQUEST['back_level']) {
        ...
        $parent_id = $db->getOne('SELECT parent_id FROM ' . $ecs->table('goods_lib_cat') . (' WHERE cat_id = \'' . $parent_id . '\''),
            true);
    } else {
    ...
    if ($adminru['ru_id'] == 0) {
        $smarty->assign('action_link',
            array('href' => 'goods_lib_cat.php?act=add', 'text' => $_LANG['04_category_add']));
    }
...
if ($_REQUEST['act'] == 'update') {
    ...
    if ($db->autoExecute($ecs->table('goods_lib_cat'), $cat, 'UPDATE', 'cat_id=\'' . $cat_id . '\'')) {
        ...
        $link[] = array(
            ...
            'href' => 'goods_lib_cat.php?act=list&parent_id=' . $cat['parent_id'] . '&level=' . $cat['level']
        );
