...
function steps_users_list()
{
    ...
    if ($result === false) {
        ...
        if ($filter['shopinfo_check'] == 1) {
            ...
            if ($filter['shopinfo_check'] == 2) {
                $ex_where .= ' AND ss.review_status = \'2\'';
            } else {
                if ($filter['shopinfo_check'] == 3) {
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if ($field['bool'] == 1) {
            $users_list[$i]['company_type'] = $GLOBALS['db']->getOne('SELECT company_type FROM ' . $GLOBALS['ecs']->table('merchants_steps_fields') . ' WHERE user_id = \'' . $users_list[$i]['user_id'] . '\'',
                true);
        }
    }
...
function get_user_steps_title($id = 0, $user_id, $ec_shop_bid, $action = 'add_shop')
{
    ...
    foreach ($res as $key => $row) {
        ...
        $cententFields = get_fields_centent_info($centent['id'], $centent['textFields'], $centent['fieldsDateType'],
            ...
            $centent['fieldsForm'], $centent['fields_sort'], $centent['will_choose'], 'root', $copy_user_id);
                    ...
                    if (0 < $ec_shop_bid) {
                        ...
                        if (0 < $user_id || $addImg == 'addImg') {
                            ...
                            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_brand'), $parent,
                                'UPDATE', 'user_id = \'' . $user_id . ('\' and bid = \'' . $ec_shop_bid . '\''));
                            get_shop_brand_file($qualificationNameInput, $qualificationImg, $expiredDateInput, $b_fid,
                                $ec_shop_bid);
                        }
                    } else {
                        if (0 < $user_id || $addImg == 'addImg') {
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['01_merchants_user_add'], 'href' => 'merchants_users_list.php?act=add_shop'));
    $smarty->assign('action_link2', array(
        'text' => $_LANG['02_initialize_seller_rank'],
        'href' => 'merchants_users_list.php?act=create_initialize_rank'
    ));
    ...
    if ($_REQUEST['act'] == 'check_shop_name') {
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($smarty->fetch('merchants_users_list.dwt'), '',
                array('filter' => $users_list['filter'], 'page_count' => $users_list['page_count']));
        }
    }
}
...
if ($_REQUEST['act'] == 'add_shop' || $_REQUEST['act'] == 'edit_shop' || $_REQUEST['act'] == 'copy_shop') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['02_merchants_users_list'], 'href' => 'merchants_users_list.php?act=list'));
    ...
    if ($_REQUEST['act'] == 'toggle_is_street') {
        ...
        if ($_REQUEST['act'] == 'toggle_is_IM') {
            ...
            if ($_REQUEST['act'] == 'insert_shop' || $_REQUEST['act'] == 'update_shop') {
                ...
                if ($old_merchants_audit != $merchants_audit) {
                    ...
                    if ($merchants_audit == 1) {
                        ...
                        if (empty($action_list)) {
                            ...
                            $db->autoExecute($ecs->table('admin_user'), $action, 'UPDATE',
                                'ru_id = \'' . $user_id . '\'');
                        }
                    } else {
                        if (0 < $grade) {
                ...
                if ($_REQUEST['act'] == 'update_shop') {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_steps_fields'), $parent, 'UPDATE',
                        'user_id = \'' . $user_id . '\'');
                } else {
                    $parent['user_id'] = $user_id;
                    $parent['agreement'] = 1;
                    ...
                    if (0 < $fid) {
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_steps_fields'), $parent,
                            'INSERT');
                    }
                }
                ...
                if ($merchants_allow == 1) {
                    ...
                    if (0 < $adminId) {
                        if ($_REQUEST['act'] == 'update_shop') {
                            $link[] = array(
                                'text' => $_LANG['go_back'],
                                'href' => 'merchants_users_list.php?act=edit_shop&id=' . $user_id
                            );
                        } else {
                            $link[] = array(
                                'text' => $_LANG['go_back'],
                                'href' => 'merchants_users_list.php?act=add_shop'
                            );
                        }
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_information'), $info, 'UPDATE',
                    'user_id = \'' . $user_id . '\'');
                $seller_shopinfo = array('shopname_audit' => $shopname_audit, 'shop_close' => $shop_close);
                $shopinfo = get_seller_shopinfo($user_id);

                if ($shopinfo) {
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_shopinfo'), $seller_shopinfo, 'UPDATE',
                        'ru_id = \'' . $user_id . '\'');
                } else {
                    if ($merchants_audit == 1) {
                        ...
                        $field = get_table_file_name($GLOBALS['ecs']->table('merchants_steps_fields'),
                            'company_adress');
                ...
                if ($review_goods == 0 && $shop_close == 1) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods'), $goods_date, 'UPDATE',
                        'user_id = \'' . $user_id . '\'');
                }
                ...
                if ($_REQUEST['act'] == 'allot') {
                    ...
                    $smarty->assign('action_link', array(
                        'text' => $_LANG['restore_default_priv'],
                        'href' => 'merchants_users_list.php?act=restore_default_priv&user_id=' . $user_id
                    ));
                                ...
                                if ($adminru['ru_id'] == 0 && $template['template_content'] != '') {
                                    if ($shopinfo['seller_email']) {
                                        ...
                                        $smarty->assign('send_date',
                                            local_date($GLOBALS['_CFG']['time_format'], gmtime()));
                                        $content = $smarty->fetch('str:' . $template['template_content']);
                                        send_mail($user_name, $shopinfo['seller_email'], $template['template_subject'],
                                            $content, $template['is_html']);
                                    }
                                }
                            }
                        }
                    ...
                    $smarty->assign('users',
                        get_table_date('merchants_shop_information', 'user_id=\'' . $user_id . '\'',
                            array('user_id', 'hopeLoginName', 'merchants_audit')));
                    $smarty->assign('menu_select',
                        array('action' => 'seller_shopinfo', 'action' => 'templates', 'current' => 'allot'));
                    assign_query_info();
                    $smarty->display('merchants_user_allot.dwt');
                } else {
                    if ($_REQUEST['act'] == 'restore_default_priv') {
                        admin_priv('users_merchants');
                                    ...
                                    if ($adminru['ru_id'] == 0 && $template['template_content'] != '') {
                                        ...
                                        if ($shopinfo['seller_email'] && ($seller_name != '' || $seller_psw != '')) {
                                            ...
                                            $smarty->assign('send_date',
                                                local_date($GLOBALS['_CFG']['time_format'], gmtime()));
                                            $content = $smarty->fetch('str:' . $template['template_content']);
                                            send_mail($seller_name, $shopinfo['seller_email'],
                            ...
                            if ($_REQUEST['act'] == 'remove') {
                                ...
                                if ($_REQUEST['act'] == 'addChildCate') {
                                    ...
                                    if ($_REQUEST['act'] == 'addChildCate_checked') {
                                        ...
                                        $category_info = get_fine_category_info($child_category['cat_id'],
                                            $cat->user_id);
                                        $smarty->assign('category_info', $category_info);
                                        ...
                                        if ($_REQUEST['act'] == 'deleteChildCate_checked') {
                                            check_authz_json('users_merchants');
                                            include_once ROOT_PATH . 'includes/cls_json.php';
                                                        ...
                                                        if ($_REQUEST['act'] == 'get_user_name') {
                                                            ...
                                                            if ($_REQUEST['act'] == 'addImg') {
                                                                ...
                                                                if ($_REQUEST['act'] == 'edit_sort_order') {
                                                                    ...
                                                                    if ($exc->edit('sort_order = \'' . $sort_order . '\'',
                                                                        $shop_id)) {
                                                                        clear_cache_files();
                                                                        make_json_result($sort_order);
                                                                    }
                                                                } else {
                                                                    if ($_REQUEST['act'] == 'create_initialize_rank') {
                                                                        admin_priv('users_merchants');
                                                                        $smarty->assign('ur_here',
                                                                            $_LANG['create_seller_grade']);
                                                                        ...
                                                                        if ($_REQUEST['act'] == 'ajax_initialize_rank') {
                                                                            include_once ROOT_PATH . 'includes/cls_json.php';
                                                                            $json = new JSON();
                                                                            $page = !empty($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
                                                                            $page_size = isset($_REQUEST['page_size']) ? intval($_REQUEST['page_size']) : 1;
                                                                            $seller_grade_list = seller_grade_list();
                                                                            $grade_list = $ecs->page_array($page_size,
                                                                                $page, $seller_grade_list);
                                                                            $arr = array();
                                                                            ...
                                                                            foreach ($grade_list['list'] as $key => $row) {
                                                                                ...
                                                                                $seller_list[$key]['shop_name'] = get_shop_name($row['user_id'],
                                                                                    1);
                                                                                $arr = array(
                                                                                    'user_id' => $row['user_id'],
                                                                                    'shop_name' => $seller_list[$key]['shop_name'],
                                                                                    'grade_name' => $seller_grade['grade_name']
                                                                                );
                                                                            }
                                                                            ...
                                                                            if ($_REQUEST['act'] == 'create_seller_grade') {
                                                                                admin_priv('users_merchants');
                                                                                $smarty->assign('menu_select', array(
                                                                                    'action' => '17_merchants',
                                                                                    'current' => '04_create_seller_grade'
                                                                                ));
                                                                                $smarty->assign('ur_here',
                                                                                    $_LANG['create_seller_grade']);
                                                                                $seller_grade_list = seller_grade_list();
                                                                                ...
                                                                                if ($_REQUEST['act'] == 'ajax_seller_grade') {
                                                                                    include_once ROOT_PATH . 'includes/cls_json.php';
                                                                                    $json = new JSON();
                                                                                    $page = !empty($_REQUEST['page']) ? intval($_REQUEST['page']) : 1;
                                                                                    $page_size = isset($_REQUEST['page_size']) ? intval($_REQUEST['page_size']) : 1;
                                                                                    ...
                                                                                    if ($grade_list['list']) {
                                                                                        foreach ($grade_list['list'] as $key => $row) {
                                                                                            ...
                                                                                            write_static_cache('seller_comment_' . $row['user_id'],
                                                                                                $seller_list[$key],
                                                                                                '/data/sc_file/');
                                                                                            $arr = array(
                                                                                                'user_id' => $row['user_id'],
                                                                                    ...
                                                                                    if ($_REQUEST['act'] == 'seller_shopinfo') {
                                                                                        admin_priv('users_merchants');
                                                                                        ...
                                                                                        $smarty->assign('users',
                                                                                            get_table_date('merchants_shop_information',
                                                                                                'user_id=\'' . $user_id . '\'',
                                                                                                array(
                                                                                                    'user_id',
                                                                                                    'hopeLoginName',
                                                                                        ...
                                                                                        $seller_shop_info = array_replace($seller_shop_info,
                                                                                            $diff_data);

                                                                                        if ($seller_shop_info['shop_logo']) {
                                                                                            $seller_shop_info['shop_logo'] = str_replace('../',
                                                                                                '',
                                                                                                $seller_shop_info['shop_logo']);
                                                                                            $seller_shop_info['shop_logo'] = get_image_path(0,
                                                                                                $seller_shop_info['shop_logo']);
                                                                                        }

                                                                                        if ($seller_shop_info['logo_thumb']) {
                                                                                            $seller_shop_info['logo_thumb'] = str_replace('../',
                                                                                                '',
                                                                                                $seller_shop_info['logo_thumb']);
                                                                                            $seller_shop_info['logo_thumb'] = get_image_path(0,
                                                                                                $seller_shop_info['logo_thumb']);
                                                                                        }

                                                                                        if ($seller_shop_info['street_thumb']) {
                                                                                            $seller_shop_info['street_thumb'] = str_replace('../',
                                                                                                '',
                                                                                                $seller_shop_info['street_thumb']);
                                                                                            $seller_shop_info['street_thumb'] = get_image_path(0,
                                                                                                $seller_shop_info['street_thumb']);
                                                                                        }

                                                                                        if ($seller_shop_info['brand_thumb']) {
                                                                                            $seller_shop_info['brand_thumb'] = str_replace('../',
                                                                                                '',
                                                                                                $seller_shop_info['brand_thumb']);
                                                                                            $seller_shop_info['brand_thumb'] = get_image_path(0,
                                                                                                $seller_shop_info['brand_thumb']);
                                                                                        }

                                                                                        $smarty->assign('shop_info',
                                                                                            $seller_shop_info);
                                                                                        $shop_information = get_shop_name($adminru['ru_id']);
                                                                                        $adminru['ru_id'] == 0 ? $shop_information['is_dsc'] = true : $shop_information['is_dsc'] = false;
                                                                                        $smarty->assign('shop_information',
                                                                                            $shop_information);
                                                                                        ...
                                                                                        $smarty->assign('http',
                                                                                            $ecs->http());
                                                                                        $smarty->assign('data_op',
                                                                                            $action);
                                                                                        assign_query_info();
                                                                                        $smarty->assign('ur_here',
                                                                                            $_LANG['04_self_basic_info']);
                                                                                        $smarty->display('seller_shopinfo.dwt');
                                                                                    } else {
                                                                                        if ($_REQUEST['act'] == 'save_seller_shopinfo') {
                                                                                            ...
                                                                                            if (empty($adminru['ru_id'])) {
                                                                                                $lnk[] = array(
                                                                                                    ...
                                                                                                    'href' => 'merchants_users_list.php?act=seller_shopinfo&id=' . $adminru['ru_id']
                                                                                                );
                                                                                                sys_msg($_LANG['invalid_data'],
                                                                                                    0, $lnk);
                                                                                            }
                                                                                            ...
                                                                                            $db->autoExecute($ecs->table('admin_user'),
                                                                                                $admin_user, 'UPDATE',
                                                                                            ...
                                                                                            if ($data_op == 'add') {
                                                                                                ...
                                                                                                if (!$store) {
                                                                                                    ...
                                                                                                    if ($review_status == 3) {
                                                                                                        $diff_data = get_seller_shopinfo_changelog($adminru['ru_id']);
                                                                                                        $shop_info = array_replace($shop_info,
                                                                                                            $diff_data);
                                                                                                ...
                                                                                                sys_msg($_LANG['add_store_info_success'],
                                                                                                    0, $lnk);
                                                                                            } else {
                                                                                                ...
                                                                                                if ($GLOBALS['_CFG']['open_oss'] == 1) {
                                                                                                    ...
                                                                                                    if (1 < $count) {
                                                                                                        ...
                                                                                                        if ($real_path == SELLER_PATH) {
                                                                                                            ...
                                                                                                            $url = substr($GLOBALS['ecs']->seller_url(),
                                                                                                                0,
                                                                                                                $str_len);
                                                                                                        }
                                                                                                    }
                                                                                                ...
                                                                                                sys_msg($_LANG['update_store_info_success'],
                                                                                                    0, $lnk);
                                                                                            }
                                                                                        } else {
                                                                                            if ($_REQUEST['act'] == 'see_shopinfo') {
                                                                                                admin_priv('users_merchants');
                                                                                                $user_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
                                                                                                $adminru['ru_id'] = $user_id;
                                                                                                ...
                                                                                                $shop_information['add_time'] = local_date('Y-m-d H:i:s',
                                                                                                    $shop_information['add_time']);
                                                                                                ...
                                                                                                $shop_information['grade_add_time'] = local_date('Y-m-d H:i:s',
                                                                                                    $grade_info['add_time']);
                                                                                                $shop_information['grade_end_time'] = local_date('Y-m-d H:i:s',
                                                                                                    $grade_info['add_time'] + 365 * 24 * 60 * 60 * $grade_info['year_num']);
                                                                                                ...
                                                                                                if ($_REQUEST['act'] == 'ajax_send_message') {
                                                                                                    check_authz_json('users_merchants');
                                                                                                    $user_id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
                                                                                                    ...
                                                                                                    $grade_info = get_seller_grade($user_id);
                                                                                                    $grade_end_time = local_date('Y-m-d H:i:s',
                                                                                                        $grade_info['add_time'] + 365 * 24 * 60 * 60 * $grade_info['year_num']);
                                                                                                    $shop_name = $_CFG['shop_name'];
                                                                                                    $send_date = local_date($GLOBALS['_CFG']['time_format'],
                                                                                                        gmtime());
            ...
            {$send_date}'
                                                                                                        ...
                                                                                                        $smarty->assign('user_name',
                                                                                                            $user_name);
                                                                                                        $smarty->assign('grade_end_time',
                                                                                                            $grade_end_time);
                                                                                                        $smarty->assign('shop_name',
                                                                                                            '<a href=\'' . $ecs->url() . '\'>' . $shop_name . '</a>');
                                                                                                        $smarty->assign('send_date',
                                                                                                            $send_date);
                                                                                                        $content = $smarty->fetch('str:' . $template['template_content']);

                                                                                                        if (send_mail($user_name,
                                                                                                            $to_email,
                                                                                                    ...
                                                                                                    if ($type == 0 && !empty($to_mobile)) {
                                                                                                        ...
                                                                                                        if ($GLOBALS['_CFG']['sms_type'] == 0) {
                                                                                                            $resp = huyi_sms($pt_smsParams,
                                                                                                                'sms_seller_grade_time');
                                                                                                        } else {
                                                                                                            if (1 <= $GLOBALS['_CFG']['sms_type']) {
