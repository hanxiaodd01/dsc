...
function order_goods_warehouse_list($order_id, $warehouse_id)
{
    ...
    for ($i = 0; $i < $count; $i++) {
        $order_goods_list[$i]['add_time'] = local_date($GLOBALS['_CFG']['date_format'],
            $order_goods_list[$i]['add_time']);
        ...
        $user_address = get_user_address_order($order_goods_list[$i]['user_id'], $order_goods_list[$i]['province_id'],
            $order_goods_list[$i]['city_id'], $order_goods_list[$i]['district_id']);
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('order_goods_warehouse_list.htm'), '',
            array('filter' => $order_goods_list['filter'], 'page_count' => $order_goods_list['page_count']));
    }
}
