...
if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
    ...
    if (isset($width_height['pic']['width']) && isset($width_height['pic']['height'])) {
        $smarty->assign('width_height', sprintf($_LANG['tips_width_height'], $width_height['pic']['width'] . 'px',
            $width_height['pic']['height'] . 'px'));
    }

    if (isset($width_height['title_pic']['width']) && isset($width_height['title_pic']['height'])) {
        $smarty->assign('title_width_height',
            sprintf($_LANG['tips_title_width_height'], $width_height['title_pic']['width'] . 'px',
                $width_height['title_pic']['height'] . 'px'));
    }
                    ...
                    if ($_REQUEST['act'] == 'remove_img') {
                        ...
                        if ($_REQUEST['act'] == 'downloadModal') {
                            ...
                            if ($_REQUEST['act'] == 'backmodal') {
                                ...
                                if ($_REQUEST['act'] == 'get_hearder_body') {
                                    require ROOT_PATH . '/includes/cls_json.php';
                                    $json = new JSON();
                                    $result = array('error' => '', 'message' => '');
                                    ...
                                    if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                                        ...
                                        if ($is_insert) {
                                            ...
                                            $db->AutoExecute($ecs->table('topic'), $record, 'UPDATE',
                                                'topic_id=\'' . $topic_id . '\'');
                                        }
                                        ...
                                        if ($_REQUEST['act'] == 'batch') {
                                            ...
                                            if (isset($_POST['type'])) {
                                                if ($_POST['type'] == 'batch_remove') {
                                                    get_del_batch($_POST['checkboxes'], intval($_GET['id']),
                                                        array('topic_img', 'title_pic'), 'topic_id', 'topic', 1);
                                                    ...
                                                    if (!empty($_REQUEST['is_ajax'])) {
                                                        $url = 'topic.php?act=query&' . str_replace('act=delete', '',
...
');
                                                    ...
                                                    if ($_POST['type'] == 'review_to') {
                                                        $ids = !empty($_POST['checkboxes']) ? join(',',
                                                            $_POST['checkboxes']) : 0;
                                                        ...
                                                        if ($db->query($sql)) {
                                                            $lnk[] = array(
                                                                'text' => $_LANG['back_list'],
                                                                'href' => 'topic.php?act=list&seller_list=1&' . list_link_postfix()
                                                            );
...
');
                                                ...
                                                if ($_REQUEST['act'] == 'query') {
                                                    ...
                                                    make_json_result($smarty->fetch($tpl), '', array(
                                                        'filter' => $topic_list['filter'],
                                                        'page_count' => $topic_list['page_count']
                                                    ));
                                                }
                                            }
                                        }
