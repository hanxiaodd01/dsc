...
if ($_REQUEST['act'] == 'pc_list') {
    ...
    $smarty->assign('action_link',
        array('href' => 'pay_card.php?act=export_pc_list&id=' . $id, 'text' => $_LANG['export_pc_list']));
    $list = get_bonus_list();
...
if ($_REQUEST['act'] == 'export_pc_list') {
    ...
    foreach ($row as $key => $val) {
        ...
        $arr[$key]['use_end_date'] = $val['use_end_date'] == 0 ? $GLOBALS['_LANG']['no_use'] : local_date($GLOBALS['_CFG']['date_format'],
            $val['use_end_date']);
        $arr[$key]['user_name'] = !empty($val['user_name']) ? $val['user_name'] : $_LANG['no_use'];
        $arr[$key]['used_time'] = $val['used_time'] == 0 ? $GLOBALS['_LANG']['no_use'] : local_date($GLOBALS['_CFG']['date_format'],
            $val['used_time']);
    }
