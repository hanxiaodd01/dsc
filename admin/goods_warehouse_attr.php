...
if ($_REQUEST['act'] == 'warehouse_list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('goods_warehouse_attr_list.htm'), '',
        ...
        if ($_REQUEST['act'] == 'product_list') {
            ...
            $region_name = get_table_date('region_warehouse', 'region_id = \'' . $warehouse_id . '\'',
                array('region_name'), 2);
                ...
                if ($_REQUEST['act'] == 'edit_product_sn') {
                    ...
                    if ($_REQUEST['act'] == 'edit_bar_code') {
                        ...
                        if ($_REQUEST['act'] == 'edit_product_number') {
                            ...
                            $product = get_product_warehouse_info($product_id,
                                'product_number, warehouse_id, goods_id');
                            ...
                            if ($product['product_number'] != $product_number) {
                                ...
                                $goods = get_admin_goods_info($product['goods_id'],
                                    array('model_inventory', 'model_attr'));
                                $logs_other = array(
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_inventory_logs'), $logs_other,
                                    'INSERT');
                            }
                            ...
                            if ($_REQUEST['act'] == 'product_remove') {
                                ...
                                if ($result) {
                                    $url = 'goods_warehouse_attr.php?act=product_query&warehouse_id=' . $_SESSION['product_warehouse'] . '&' . str_replace('act=product_remove',
                                ...
                                if ($_REQUEST['act'] == 'product_add_execute') {
                                    ...
                                    if (empty($product['goods_id'])) {
                                        sys_msg($_LANG['sys']['wrong'] . $_LANG['cannot_found_goods'], 1, array(),
                                            false);
                                    }

                                    $insert = true;

                                    if (0 < product_warehouse_number_count($product['goods_id'], '',
                                    ...
                                    foreach ($product['product_sn'] as $key => $value) {
                                        ...
                                        $product_count = product_warehouse_number_count($product['goods_id'], '',
                                            $product['warehouse_id']);

                                        if (update_warehouse_goods($product['goods_id'], 'goods_number',
                                            $product_count)) {
                                    ...
                                    if ($insert) {
                                        ...
                                        $link[] = array(
                                            'href' => 'goods_warehouse_attr.php?act=product_list&goods_id=' . $product['goods_id'] . '&warehouse_id=' . $product['warehouse_id'],
                                            'text' => $_LANG['18_product_list']
                                        );
                                    } else {
                                        $link[] = array(
                                            'href' => 'goods.php?act=list&uselastfilter=1',
                                            'text' => $_LANG['01_goods_list']
                                        );
                                        $link[] = array(
