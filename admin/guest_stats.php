...
if ($_REQUEST['act'] == 'list') {
    ...
    $sql = 'SELECT COUNT(*) AS order_num, ' . $total_fee . 'FROM ' . $ecs->table('order_info') . ' as o ' . ' WHERE o.user_id > 0 ' . order_query_sql('finished',
            'o.') . ' AND (select count(*) from ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ';
    ...
    if ($_GET['flag'] == 'download') {
        ...
        $data .= $user_num . '	' . $have_order_usernum . '	' . $user_all_order['order_num'] . '	' . sprintf('%0.2f',
                (0 < $user_num ? $have_order_usernum / $user_num : 0) * 100) . "\n\n";
    ...
    $smarty->assign('ave_user_ordernum',
        0 < $user_num ? sprintf('%0.2f', $user_all_order['order_num'] / $user_num) : 0);

    if ($user_all_order['order_num']) {
        $smarty->assign('ave_user_turnover', 0 < $user_num ? price_format($user_all_order['turnover'] * sprintf('%0.2f',
                $user_all_order['order_num'] / $user_num)) : 0);
    } else {
    ...
    $smarty->assign('guest_order_amount',
        0 < $guest_all_order['order_num'] ? price_format($guest_all_order['turnover'] / $guest_all_order['order_num']) : 0);
    $smarty->assign('all_order', $user_all_order);
    $smarty->assign('ur_here', $_LANG['report_guest']);
    $smarty->assign('lang', $_LANG);
    $smarty->assign('action_link',
        array('text' => $_LANG['down_guest_stats'], 'href' => 'guest_stats.php?flag=download'));
