...
if ($_REQUEST['act'] == 'list_article') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['article_add'], 'href' => 'shophelp.php?act=add&cat_id=' . $cat_id));
    $smarty->assign('full_page', 1);
    $smarty->assign('cat', article_cat_list($cat_id, true, 'cat_id', 0,
        'onchange="location.href=\'?act=list_article&cat_id=\'+this.value"'));
...
if ($_REQUEST['act'] == 'edit') {
    ...
    $smarty->assign('action_link', array(
        'text' => $_LANG['article_list'],
        'href' => 'shophelp.php?act=list_article&cat_id=' . $article['cat_id']
    ));
...
if ($_REQUEST['act'] == 'update') {
    ...
    if ($exc_article->edit('title = \'' . $title . '\', cat_id = \'' . $cat_id . '\', article_type = \'' . $article_type . '\', content = \'' . $editor . '\'',
        $id)) {
        clear_cache_files();
        $link[0]['text'] = $_LANG['back_list'];
        $link[0]['href'] = 'shophelp.php?act=list_article&cat_id=' . $cat_id;
...
');
                ...
                if ($_REQUEST['act'] == 'remove_art') {
                    ...
                    $url = 'shophelp.php?act=query_art&cat=' . $cat_id . '&' . str_replace('act=remove_art', '',
