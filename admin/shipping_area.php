...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('href' => 'shipping_area.php?act=add&shipping=' . $shipping_id, 'text' => $_LANG['new_area']));
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            if (0 < $db->getOne($sql)) {
                ...
                $lnk[] = array(
                    ...
                    'href' => 'shipping_area.php?act=list&shipping=' . $shipping_id
                );
                $lnk[] = array(
            ...
            if ($_REQUEST['act'] == 'edit') {
                ...
                if ($_REQUEST['act'] == 'update') {
                    ...
                    if (0 < $db->getOne($sql, true)) {
                        ...
                        $db->query($sql);
                        $point_id = isset($_POST['point_id']) && !empty($_POST['point_id']) ? dsc_addslashes($_POST['point_id'],
                            0) : array();
                        $point_name = isset($_POST['point_name']) ? $_POST['point_name'] : array();
                        $user_name = isset($_POST['user_name']) ? $_POST['user_name'] : array();
                        $mobile = isset($_POST['mobile']) ? $_POST['mobile'] : array();
                            ...
                            if ($_REQUEST['act'] == 'remove_area') {
                                ...
                                if ($_REQUEST['act'] == 'remove_point') {
                                    ...
                                    if ($db->query($sql)) {
                                        $data = array(
                                            'error' => 2,
                                            'message' => $_LANG['carddrop_succeed'],
                                            'content' => ''
                                        );
                                        admin_log($name, 'remove', 'shipping_area');
                                    } else {
