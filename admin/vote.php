    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $vote = array(
                ...
                'end_time' => local_date('Y-m-d', local_strtotime('+2 weeks'))
            );
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                if ($db->getOne($sql) == 0) {
                    ...
                    sys_msg($_LANG['add'] . '&nbsp;' . $_POST['vote_name'] . '&nbsp;' . $_LANG['attradd_succed'], 0,
                        $link);
                } else {
                    ...
                    if ($_REQUEST['act'] == 'update') {
                        ...
                        if ($_REQUEST['act'] == 'option') {
                            ...
                            $smarty->assign('action_link',
                                array('href' => 'vote.php?act=list', 'text' => $_LANG['list_vote']));
                            $smarty->assign('full_page', 1);
                            $smarty->assign('id', $id);
                                    ...
                                    if ($_REQUEST['act'] == 'new_option') {
                                        ...
                                        if (!empty($option_name)) {
                                            ...
                                            if ($db->getOne($sql) != 0) {
                                                ...
                                                $url = 'vote.php?is_ajax=1&act=query_option&vid=' . $vote_id . '&' . str_replace('act=new_option',
                                        ...
                                        if ($_REQUEST['act'] == 'remove') {
                                            ...
                                            $url = 'vote.php?act=query&' . str_replace('act=remove', '',
                                                    $_SERVER['QUERY_STRING']);
                                            ecs_header('Location: ' . $url . "\n");
                                            exit();
                                        } else {
                                            if ($_REQUEST['act'] == 'remove_option') {
                                                ...
                                                $url = 'vote.php?act=query_option&vid=' . $vote_id . '&' . str_replace('act=remove_option',
                                                        '', $_SERVER['QUERY_STRING']);
                                                ecs_header('Location: ' . $url . "\n");
                                                exit();
                                            }
