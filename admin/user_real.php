...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'edit') {
        ...
        $smarty->assign('action_link',
            array('text' => $_LANG['16_users_real'], 'href' => 'user_real.php?act=list&' . list_link_postfix()));
        $smarty->assign('user_type', $user_type);
        ...
        if ($_REQUEST['act'] == 'update') {
            ...
            if (0 < $user_id) {
                ...
                if ($real_id) {
                    if ($db->autoExecute($ecs->table('users_real'), $post_user_real, 'UPDATE',
                        'real_id = \'' . $real_id . '\'')) {
            ...
            if ($_REQUEST['act'] == 'query') {
                ...
                make_json_result($smarty->fetch('users_real_list.dwt'), '',
                    array('filter' => $users_real_list['filter'], 'page_count' => $users_real_list['page_count']));
            } else {
                        ...
                        if ($_POST['type'] == 'batch_remove') {
                            ...
                            if ($db->query($sql)) {
                                $lnk[] = array(
                                    ...
                                    'href' => 'user_real.php?act=list' . $type
                                );
