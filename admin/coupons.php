...
if ($_REQUEST['act'] == 'edit') {
    ...
    $smarty->assign('action_link',
        array('href' => 'coupons.php?act=list&' . list_link_postfix(), 'text' => $_LANG['cou_list']));
    $smarty->assign('form_act', 'update');

    if ($cou_arr['spec_cat']) {
        $cou_arr['cats'] = get_choose_cat($cou_arr['spec_cat']);
    } else {
        if ($cou_arr['cou_goods']) {
    ...
    $smarty->assign('type',
        $cou_arr['cou_type'] == 1 ? 'register' : ($cou_arr['cou_type'] == 2 ? 'buy' : ($cou_arr['cou_type'] == 3 ? 'all' : ($cou_arr['cou_type'] == 4 ? 'member' : ''))));
...
if ($_REQUEST['act'] == 'query_coupons') {
    ...
    if ($_REQUEST['act'] == 'remove_coupons') {
        $cou_id = intval($_GET['cou_id']);
        ...
        if ($uc_id) {
            ...
            $url = 'coupons.php?act=user_query_coupons&cou_id=' . $cou_id . str_replace('act=remove_coupons', '',
                    ...
                    if ($_POST['type'] == 'review_to') {
                        ...
                        if ($db->query($sql)) {
                            $lnk[] = array(
                                ...
                                'href' => 'coupons.php?act=list&seller_list=1&' . list_link_postfix()
                            );
