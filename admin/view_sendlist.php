...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('view_sendlist.dwt'), '',
            array('filter' => $listdb['filter'], 'page_count' => $listdb['page_count']));
    } else {
        if ($_REQUEST['act'] == 'del') {
            $id = (int)$_REQUEST['id'];
            ...
            if ($_REQUEST['act'] == 'batch_remove') {
                ...
                if ($_REQUEST['act'] == 'batch_send') {
                    if (isset($_POST['checkboxes'])) {
                        ...
                        if (empty($row['id'])) {
                            $links[] = array(
                                'text' => $_LANG['05_view_sendlist'],
                                'href' => 'view_sendlist.php?act=list'
                            );
                        ...
                        while ($row = $db->fetchRow($res)) {
                            ...
                            if ($rt['template_id'] && $rt['template_content']) {
                                if (send_mail('', $row['email'], $rt['template_subject'], $rt['template_content'],
                                    $rt['is_html'])) {
                                    $sql = 'DELETE FROM ' . $ecs->table('email_sendlist') . ' WHERE id = \'' . $row['id'] . '\'';
                                    $db->query($sql);
                                } else {
