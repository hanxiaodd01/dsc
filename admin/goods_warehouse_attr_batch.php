        ...
        if ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
                ...
                if ($GLOBALS['db']->getOne($sql, true)) {
                    ...
                    $db->autoExecute($ecs->table('warehouse_attr'), $other, 'UPDATE',
                        'goods_id = \'' . $result['list']['goods_id'] . '\' AND warehouse_id = \'' . $result['list']['warehouse_id'] . '\' AND goods_attr_id = \'' . $result['list']['goods_attr_id'] . ('\' ' . $where));
