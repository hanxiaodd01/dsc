...
function vc_list()
{
    ...
    foreach ($row as $key => $val) {
        $row[$key]['bind_time'] = 0 < $val['bind_time'] ? local_date($GLOBALS['_CFG']['date_format'],
            $val['bind_time']) : $GLOBALS['_LANG']['no_use'];
    }
...
if ($_REQUEST['act'] == 'query') {
    ...
    $sort_flag = sort_flag($list['filter']);
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('vc_type_list.dwt'), '',
...
if ($_REQUEST['act'] == 'vc_query') {
    ...
    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
    make_json_result($smarty->fetch('value_card_view.dwt'), '',
        array('filter' => $vc_list['filter'], 'page_count' => $vc_list['page_count']));
}
...
if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
    ...
    $spec_goods = !empty($_POST['vc_goods']) && $use_condition == 2 ? implode(',',
        array_unique($_POST['vc_goods'])) : '';
            ...
            if (isset($_POST['drop'])) {
                if (0 < $row) {
                    $links[] = array(
                        ...
                        'href' => 'value_card.php?act=list&' . list_link_postfix()
                    );
...
if ($_REQUEST['act'] == 'vc_list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['export_vc_list'], 'href' => 'value_card.php?act=export_vc_list&id=' . $id));
    $vc_list = vc_list();
    $smarty->assign('value_card_list', $vc_list['item']);
    $smarty->assign('filter', $vc_list['filter']);
    $smarty->assign('record_count', $vc_list['record_count']);
...
if ($_REQUEST['act'] == 'export_vc_list') {
    ...
    foreach ($row as $key => $val) {
        ...
        $arr[$key]['bind_time'] = 0 < $val['bind_time'] ? local_date($GLOBALS['_CFG']['date_format'],
            $val['bind_time']) : $GLOBALS['_LANG']['no_use'];
    }
    ...
    $prev = array(
        $_LANG['record_id'],
        $_LANG['value_card_sn'],
        $_LANG['value_card_password'],
        $_LANG['value_card_type'],
        $_LANG['value_card_value'],
        $_LANG['bind_user'],
        $_LANG['bind_time']
    );
