...
if ($_REQUEST['act'] == 'new') {
    ...
    if ($_REQUEST['act'] == 'get_chart_data') {
        $search_data = array();
        ...
        if ($_REQUEST['act'] == 'user_analysis') {
            ...
            if ($_REQUEST['act'] == 'user_analysis_query') {
                $order_list = user_sale_stats();
                $smarty->assign('order_list', $order_list['orders']);
                $smarty->assign('filter', $order_list['filter']);
                $smarty->assign('record_count', $order_list['record_count']);
                ...
                make_json_result($smarty->fetch('user_analysis.dwt'), '',
                    array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
            } else {
                if ($_REQUEST['act'] == 'user_area_analysis') {
                    ...
                    if ($_REQUEST['act'] == 'user_area_analysis_query') {
                        $order_list = user_area_stats();
                        $smarty->assign('order_list', $order_list['orders']);
                        $smarty->assign('filter', $order_list['filter']);
                        $smarty->assign('record_count', $order_list['record_count']);
                        ...
                        $sort_flag = sort_flag($order_list['filter']);
                        $smarty->assign($sort_flag['tag'], $sort_flag['img']);
                        make_json_result($smarty->fetch('user_area_analysis.dwt'), '',
                            array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
                    } else {
                        if ($_REQUEST['act'] == 'user_rank_analysis') {
                            ...
                            if ($_REQUEST['act'] == 'user_consumption_rank') {
                                $order_list = user_sale_stats();
                                $smarty->assign('full_page', 1);
                                $smarty->assign('order_list', $order_list['orders']);
                                $smarty->assign('filter', $order_list['filter']);
                                ...
                                if ($_REQUEST['act'] == 'user_consumption_rank_query') {
                                    $order_list = user_sale_stats();
                                    $smarty->assign('order_list', $order_list['orders']);
                                    $smarty->assign('filter', $order_list['filter']);
                                    $smarty->assign('record_count', $order_list['record_count']);
                                    ...
                                    $sort_flag = sort_flag($order_list['filter']);
                                    $smarty->assign($sort_flag['tag'], $sort_flag['img']);
                                    make_json_result($smarty->fetch('user_consumption_rank.dwt'), '', array(
                                        'filter' => $order_list['filter'],
                                        'page_count' => $order_list['page_count']
                                    ));
                                } else {
                                    if ($_REQUEST['act'] == 'download_area') {
                                        ...
                                        $thead = array(
                                            ...
                                            $_LANG['city'],
                                            $_LANG['area_alt'],
                                            $_LANG['user_sale_stats'][0],
                                            '下单总金额',
                                            $_LANG['user_sale_stats'][1]
                                        );
                                        $tbody = array(
                                            'province_name',
                                            'city_name',
                                        ...
                                        if ($_REQUEST['act'] == 'download_rank') {
                                            ...
                                            $thead = array(
                                                $_LANG['record_id'],
                                                $_LANG['user_name'],
                                                $_LANG['user_sale_stats'][2],
                                                $_LANG['user_sale_stats'][3],
                                                $_LANG['user_sale_stats'][4],
                                                $_LANG['user_sale_stats'][5],
                                                $_LANG['user_sale_stats'][6],
                                                $_LANG['user_sale_stats'][7]
                                            );
                                            $tbody = array(
                                                'user_id',
                                                'user_name',
