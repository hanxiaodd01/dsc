...
if ($_REQUEST['act'] == 'new') {
    ...
    if ($_REQUEST['act'] == 'get_chart_data') {
        ...
        if ($_REQUEST['act'] == 'shop_sale_stats') {
...
if ($_REQUEST['act'] == 'shop_sale_stats_query') {
    ...
    make_json_result($smarty->fetch('shop_sale_stats.dwt'), '',
...
if ($_REQUEST['act'] == 'get_total_stats') {
    ...
    if ($_REQUEST['act'] == 'shop_area') {
        ...
        if ($_REQUEST['act'] == 'shop_area_query') {
            $order_list = shop_area_stats();
            $smarty->assign('order_list', $order_list['orders']);
            $smarty->assign('filter', $order_list['filter']);
            $smarty->assign('record_count', $order_list['record_count']);
            ...
            $sort_flag = sort_flag($order_list['filter']);
            $smarty->assign($sort_flag['tag'], $sort_flag['img']);
            make_json_result($smarty->fetch('shop_area_distribution.dwt'), '',
                array('filter' => $order_list['filter'], 'page_count' => $order_list['page_count']));
        } else {
            if ($_REQUEST['act'] == 'get_area_chart_data') {
                $search_data = array();
                $search_data['start_date'] = empty($_REQUEST['start_date']) ? '' : (0 < strpos($_REQUEST['start_date'],
                    '-') ? local_strtotime($_REQUEST['start_date']) : $_REQUEST['start_date']);
                $search_data['end_date'] = empty($_REQUEST['end_date']) ? '' : (0 < strpos($_REQUEST['end_date'],
