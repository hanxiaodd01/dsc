                        ...
                        if (isset($_POST['fail'])) {
                            ...
                            if (isset($_POST['mail'])) {
                                ...
                                if (0 < $group_buy_id) {
                                    ...
                                    $db->autoExecute($ecs->table('goods_activity'), $group_buy, 'UPDATE',
                                        'act_id = \'' . $group_buy_id . '\' AND act_type = ' . GAT_GROUP_BUY);
                                    admin_log(addslashes($goods_name) . '[' . $group_buy_id . ']', 'edit', 'group_buy');
                                    $links = array(
                                        array(
                                            'href' => 'group_buy.php?act=list&' . list_link_postfix(),
                        ...
                        if ($_POST['type'] == 'batch_remove') {
                            ...
                            if ($_POST['type'] == 'review_to') {
                                ...
                                if ($db->query($sql)) {
                                    $lnk[] = array(
                                        'text' => $_LANG['back_list'],
                                        'href' => 'group_buy.php?act=list&seller_list=1&' . list_link_postfix()
                                    );
                            ...
                            if ($_REQUEST['act'] == 'edit_deposit') {
                                ...
                                if ($_REQUEST['act'] == 'edit_restrict_amount') {
                                    ...
                                    if ($_REQUEST['act'] == 'remove') {
                                        ...
                                        $db->query($sql);
                                        admin_log(addslashes($group_buy['goods_name']) . '[' . $id . ']', 'remove',
                                            'group_buy');
                                        clear_cache_files();
                                        $url = 'group_buy.php?act=query&' . str_replace('act=remove', '',
