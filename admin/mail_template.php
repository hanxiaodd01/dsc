...
if ($_REQUEST['act'] == 'list') {
    ...
    while ($row = $db->FetchRow($res)) {
        ...
        $templates[$row['template_id']] = $len < 18 ? $_LANG[$row['template_code']] . str_repeat('&nbsp;',
                (18 - $len) / 2) . (' [' . $row['template_code'] . ']') : $_LANG[$row['template_code']] . (' [' . $row['template_code'] . ']');
    }
    ...
    if ($_REQUEST['act'] == 'loat_template') {
        ...
        if ($_REQUEST['act'] == 'save_template') {
            ...
            $sql = 'UPDATE ' . $ecs->table('mail_templates') . ' SET ' . 'template_subject = \'' . str_replace('\\\'\\\'',
                    '\\\'', $subject) . '\', ' . 'template_content = \'' . str_replace('\\\'\\\'', '\\\'',
                    $content) . '\', ' . ('is_html = \'' . $type . '\', ') . 'last_modify = \'' . gmtime() . '\' ' . ('WHERE template_id=\'' . $tpl_id . '\'');
