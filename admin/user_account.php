...
function account_list()
{
    ...
    if ($result === false) {
        ...
        $filter['add_start_date'] = empty($_REQUEST['add_start_date']) ? '' : (0 < strpos($_REQUEST['add_start_date'],
            '-') ? local_strtotime($_REQUEST['add_start_date']) : $_REQUEST['add_start_date']);
        $filter['add_end_date'] = empty($_REQUEST['add_end_date']) ? '' : (0 < strpos($_REQUEST['add_end_date'],
            '-') ? local_strtotime($_REQUEST['add_end_date']) : $_REQUEST['add_end_date']);
        $where = ' WHERE 1 ';
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['surplus_add'], 'href' => 'user_account.php?act=add&process_type=' . $process_type));
    $list = account_list();
        ...
        if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                if ($process_type == 1) {
                    ...
                    log_account_change($user_id, $amount, 0 - $amount, 0, 0, $change_desc, $change_type);
                }
            } else {
...
');
                    ...
                    if ($account['is_paid'] == 0) {
                        ...
                        if ($is_paid == 1 && $account['process_type'] == 1) {
                            ...
                            log_account_change($account['user_id'], 0, $amount + $account['deposit_fee'], 0, 0,
                                '【' . $_LANG['surplus_type_1'] . '-' . $_LANG['offline_transfer'] . '】' . $admin_note,
                                ACT_DRAWING);
                            $order_note = !empty($account['admin_note']) ? explode('：',
                                $account['admin_note']) : array();
                            ...
                            if ($user_info['email'] != '') {
                                ...
                                send_mail($_CFG['shop_name'], $user_info['email'], $tpl['template_subject'], $content,
                                    $tpl['is_html']);
                            }
                        } else {
                            if ($is_paid == 1 && $account['process_type'] == 0) {
                                update_user_account($id, $amount, $admin_note, $is_paid);
                                log_account_change($account['user_id'], $amount, 0, 0, 0,
                                    '【' . $_LANG['user_name'] . $_LANG['surplus_type_0'] . '】' . $admin_note,
                                    ACT_SAVING);
                                $smsParams['user_money'] = $user_surplus['user_money'] + $amount;
                                $smsParams['usermoney'] = $user_surplus['user_money'] + $amount;
                                ...
                                if ($user_info['email'] != '') {
                                    ...
                                    $smarty->assign('smsParams', $smsParams);
                                    $content = $smarty->fetch('str:' . $tpl['template_content']);
                                    send_mail($_CFG['shop_name'], $user_info['email'], $tpl['template_subject'],
                                        $content, $tpl['is_html']);
                                }
                            } else {
                                if ($is_paid == 0 || $is_paid == 2) {
                                    ...
                                    if ($db->query($sql) && $is_paid == 2) {
                                        ...
                                        $row = $db->getRow($sql);
                                        log_account_change($row['user_id'], abs($row['amount']), $row['amount'], 0, 0,
                                            '【' . $_LANG['Application_withdrawal'] . $_LANG['dismiss_application'] . '】',
                                            ACT_ADJUSTING, 2);
                                    }
                                }
                            }
                    ...
                    if ($_REQUEST['act'] == 'query') {
                        ...
                        make_json_result($smarty->fetch('user_account_list.dwt'), '',
                        ...
                        if ($_REQUEST['act'] == 'remove') {
                            ...
                            if ($db->query($sql, 'SILENT')) {
                                ...
                                $url = 'user_account.php?act=query&' . str_replace('act=remove', '',
                                    ...
                                    foreach ($checkboxes as $id) {
                                        ...
                                        if ($account['is_paid'] == 0) {
                                            ...
                                            if ($account['process_type'] == '1') {
                                                ...
                                                update_user_account($id, $amount, $admin_note, $is_paid);
                                                log_account_change($account['user_id'], 0,
                                                    $amount + $account['deposit_fee'], 0, 0, $_LANG['surplus_type_1'],
                                                    ACT_DRAWING);
                                                ...
                                                if ($user_info['email'] != '') {
                                                    ...
                                                    $smarty->assign('smsParams', $smsParams);
                                                    $content = $smarty->fetch('str:' . $tpl['template_content']);
                                                    send_mail($_CFG['shop_name'], $user_info['email'],
                                            ...
                                            admin_log('(' . addslashes($_LANG['check']) . ')' . $admin_note, 'edit',
                                                'user_surplus');
                                        }
                                    }
