...
if ($_REQUEST['act'] == 'query') {
    ...
    if ($_REQUEST['act'] == 'toggle_display') {
                ...
                if (isset($_POST['checkboxes'])) {
                    switch ($action) {
                        case 'remove':
                            ...
                            $db->query('DELETE FROM ' . $ecs->table('zc_topic') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                                    'topic_id'));
                            break;

                        case 'allow':
                            $db->query('UPDATE ' . $ecs->table('zc_topic') . ' SET topic_status = 1  WHERE ' . db_create_in($_POST['checkboxes'],
                                    'topic_id'));
                            break;

                        case 'deny':
                            $db->query('UPDATE ' . $ecs->table('zc_topic') . ' SET topic_status = 0  WHERE ' . db_create_in($_POST['checkboxes'],
                                    'topic_id'));
                            break;
