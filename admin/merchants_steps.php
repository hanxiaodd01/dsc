...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['02_merchants_add'], 'href' => 'merchants_steps.php?act=add'));
    ...
    if ($_REQUEST['act'] == 'edit_sort_order') {
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            make_json_result($smarty->fetch('merchants_steps_list.dwt'), '',
                array('filter' => $process_list['filter'], 'page_count' => $process_list['page_count']));
        }
    }
}
...
if ($_REQUEST['act'] == 'title_list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['04_merchants_add_info'], 'href' => 'merchants_steps.php?act=title_add&id=' . $id));
    $smarty->assign('action_link2',
        array('text' => $_LANG['01_merchants_list'], 'href' => 'merchants_steps.php?act=list'));
    ...
    if ($_REQUEST['act'] == 'query_title') {
        ...
        $smarty->assign($sort_flag['tag'], $sort_flag['img']);
        make_json_result($smarty->fetch('merchants_steps_title_list.dwt'), '',
            array('filter' => $title_list['filter'], 'page_count' => $title_list['page_count']));
    } else {
        if ($_REQUEST['act'] == 'toggle_steps_show') {
            check_authz_json('merchants_setps');
            $id = intval($_POST['id']);
            ...
            if ($_REQUEST['act'] == 'add') {
                ...
                if ($_REQUEST['act'] == 'insert') {
                    ...
                    if (0 < $res) {
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_steps_process'), $parent,
                            'INSERT');
                    ...
                    if ($_REQUEST['act'] == 'title_add') {
                        ...
                        $smarty->assign('action_link', array(
                            'text' => $_LANG['01_merchants_list'],
                            'href' => 'merchants_steps.php?act=title_list&id=' . $id
                        ));
                        ...
                        if ($_REQUEST['act'] == 'title_insert') {
                            ...
                            $res = get_merchants_steps_title_insert_update($fields_steps, $fields_titles,
                                $titles_annotation, $steps_style, $fields_special, $special_type, 'insert', $tid);
                            $sql = 'select fields_steps from ' . $ecs->table('merchants_steps_title') . ' where tid = \'' . $res['tid'] . '\'';
                            $fields_steps = $db->getOne($sql);

                            if ($res['true']) {
                                $steps = get_merchants_steps_fields_admin('merchants_steps_fields', $date, $dateType,
                                    $length, $notnull, $coding, $formName, $fields_sort, $res['tid']);
                                get_merchants_steps_fields_centent_insert_update($steps['textFields'],
                                    $steps['fieldsDateType'], $steps['fieldsLength'], $steps['fieldsNotnull'],
                                    $steps['fieldsFormName'], $steps['fieldsCoding'], $steps['fields_sort'],
                                    $steps['will_choose'], $form_choose['chooseForm'], $res['tid']);
                            ...
                            $link[] = array(
                                'text' => $_LANG['go_back'],
                                'href' => 'merchants_steps.php?act=title_list&id=' . $fields_steps
                            );
                            ...
                            if ($_REQUEST['act'] == 'edit') {
                                ...
                                $smarty->assign('action_link', array(
                                    'text' => $_LANG['01_merchants_list'],
                                    'href' => 'merchants_steps.php?act=list'
                                ));
                                assign_query_info();
                                ...
                                if ($_REQUEST['act'] == 'title_edit') {
                                    ...
                                    $smarty->assign('action_link', array(
                                        'text' => $_LANG['01_merchants_list'],
                                        'href' => 'merchants_steps.php?act=title_list&id=' . $title_info['fields_steps']
                                    ));
                                    $sql = 'select id, process_title from ' . $ecs->table('merchants_steps_process') . ' where 1';
                                    $process_list = $db->getAll($sql);
                                    $smarty->assign('process_list', $process_list);
                                    ...
                                    $cententFields = get_fields_centent_info($centent['id'], $centent['textFields'],
                                        $centent['fieldsDateType'], $centent['fieldsLength'], $centent['fieldsNotnull'],
                                        $centent['fieldsFormName'], $centent['fieldsCoding'], $centent['fieldsForm'],
                                        $centent['fields_sort'], $centent['will_choose']);
                                    ...
                                    if ($_REQUEST['act'] == 'update') {
                                        ...
                                        if (0 < $res) {
                                            ...
                                            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_steps_process'),
                                                $parent, 'UPDATE', 'id = \'' . $id . '\'');
                                        ...
                                        if ($_REQUEST['act'] == 'title_update') {
                                            ...
                                            $form_choose = get_steps_form_choose($form_array);
                                            $steps = get_merchants_steps_fields_admin('merchants_steps_fields', $date,
                                                $dateType, $length, $notnull, $coding, $formName, $fields_sort, $tid);
                                            get_merchants_steps_fields_centent_insert_update($steps['textFields'],
                                                $steps['fieldsDateType'], $steps['fieldsLength'],
                                                $steps['fieldsNotnull'], $steps['fieldsFormName'],
                                                $steps['fieldsCoding'], $steps['fields_sort'], $steps['will_choose'],
                                                $form_choose['chooseForm'], $tid);
                                            $res = get_merchants_steps_title_insert_update($fields_steps,
                                                $fields_titles, $titles_annotation, $steps_style, $fields_special,
                                                $special_type, 'update', $tid);
                                            ...
                                            if ($_REQUEST['act'] == 'batch_remove') {
                                                ...
                                                if (isset($_POST['checkboxes'])) {
                                                    ...
                                                    if ($db->query($sql) == true) {
                                                        ...
                                                        sys_msg(sprintf($_LANG['batch_remove_success'], $count), 0,
                                                            $lnk);
                                                    }
                                                } else {
                                                    $lnk[] = array(
                                                ...
                                                if ($_REQUEST['act'] == 'remove') {
                                                    ...
                                                    if ($_REQUEST['act'] == 'titleList_remove') {
                                                        admin_priv('merchants_setps_drop');
                                                        ...
                                                        if ($_REQUEST['act'] == 'title_remove') {
                                                            ...
                                                            $fields_date_all = get_fields_date_title_remove($tid,
                                                                $objName);

                                                            if (count($fields_date_all) == 1) {
                                                                $sql = 'delete from ' . $ecs->table('merchants_steps_fields_centent') . ' where tid = \'' . $tid . '\'';
                                                                $db->query($sql);
                                                                get_Add_Drop_fields($objName, '',
                                                                    'merchants_steps_fields', 'delete');
                                                            } else {
                                                                $fields_date = get_fields_date_title_remove($tid,
                                                                    $objName, 1);
                                                            ...
                                                            $link[] = array(
                                                                'text' => $_LANG['go_back'],
                                                                'href' => 'merchants_steps.php?act=title_edit&id=' . $tid
                                                            );
                                                            sys_msg($_LANG['remove_success'], 0, $link);
                                                        } else {
