    ...
    if ($bt_log) {
        foreach ($bt_log as $key => $val) {
            ...
            if ($bt_log[$key]['repayed_date']) {
                $bt_log[$key]['repayed_date'] = local_date($GLOBALS['_CFG']['time_format'],
                    $bt_log[$key]['repayed_date']);
            }
        }
    }
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'log_list') {
        ...
        $smarty->assign('baitiao_page',
            array('total' => $total, 'page_total' => $page_total, 'page_one_num' => $page_one_num));
        $smarty->assign('ur_here', $_LANG['bt_details']);
        $smarty->display('baitiao_log_list.dwt');
    } else {
        if ($_REQUEST['act'] == 'log_list_query') {
            admin_priv('baitiao_manage');
            $baitiao_id = isset($_REQUEST['bt_id']) ? intval($_REQUEST['bt_id']) : 0;
            $user_id = isset($_REQUEST['user_id']) ? intval($_REQUEST['user_id']) : 0;
            ...
            $smarty->assign('action_link',
                array('text' => $_LANG['bt_list'], 'href' => 'user_baitiao_log.php?act=list'));
            $smarty->assign('baitiao_page',
                array('total' => $total, 'page_total' => $page_total, 'page_one_num' => $page_one_num));
            $smarty->assign('ur_here', $_LANG['bt_details']);
            make_json_result($smarty->fetch('baitiao_log_list.dwt'), '',
                array('filter' => $bt_log['filter'], 'page_count' => $bt_log['page_count']));
        } else {
            if ($_REQUEST['act'] == 'bt_add_tp') {
                admin_priv('baitiao_manage');
                ...
                if (0 < $user_id) {
                    ...
                    $smarty->assign('action_link2',
                        array('href' => 'users.php?act=list', 'text' => $_LANG['03_users_list']));
                }

                $smarty->assign('ur_here', $_LANG['bt_ur_here']);
                $smarty->assign('action_link',
                    array('href' => 'baitiao_batch.php?act=add', 'text' => $_LANG['baitiao_batch_set']));
                ...
                if ($_REQUEST['act'] == 'bt_edit') {
                    ...
                    if (0 < $user_id) {
                        ...
                        if ($bt_info) {
                            ...
                            if ($up_ok) {
                                $links[] = array(
                                    'text' => $_LANG['go_back'],
                                    'href' => 'user_baitiao_log.php?act=bt_add_tp&user_id=' . $user_id
                                );
                    ...
                    if ($_REQUEST['act'] == 'query') {
                        ...
                        make_json_result($smarty->fetch('baitiao_list.dwt'), '',
                            array('filter' => $baitiao_list['filter'], 'page_count' => $baitiao_list['page_count']));
                    } else {
                        if ($_REQUEST['act'] == 'batch_remove') {
                            ...
                            $sql = 'DELETE FROM ' . $ecs->table('baitiao') . ' WHERE baitiao_id ' . db_create_in(join(',',
                                    $_POST['checkboxes']));
                            $del_ok = $db->query($sql);
                            ...
                            if ($del_ok) {
                                $lnk[] = array(
                                    ...
                                    'href' => 'user_baitiao_log.php?act=list'
                                );
                                sys_msg($_LANG['baitiao_remove_success'], 0, $lnk);
                            }
                        } else {
                            if ($_REQUEST['act'] == 'batch_remove_log') {
                                admin_priv('baitiao_manage');
                                if (!isset($_POST['checkboxes']) || !is_array($_POST['checkboxes'])) {
                                    sys_msg($_LANG['not_select_data'], 1);
                                }

                                $sql = 'SELECT log_id,is_repay,baitiao_id,user_id FROM ' . $ecs->table('baitiao_log') . ' WHERE log_id ' . db_create_in(join(',',
                                        $_POST['checkboxes']));
                                ...
                                if ($bt_log) {
                                    ...
                                    if (0 < $no_del_num) {
                                        $lnk[] = array(
                                            'text' => $_LANG['back_list'],
                                            'href' => 'user_baitiao_log.php?act=log_list&bt_id=' . $bt_log[0]['baitiao_id'] . '&user_id=' . $bt_log[0]['user_id']
                                        );
                                        ...
                                        if (0 < $bt_log['log_id']) {
                                            if ($bt_log['is_repay'] || $bt_log['is_refund']) {
                                                ...
                                                if ($del_ok) {
                                                    ...
                                                    $link[] = array(
                                                        'text' => $_LANG['go_back'],
                                                        'href' => 'user_baitiao_log.php?act=log_list&bt_id=' . $bt_log['baitiao_id'] . '&user_id=' . $bt_log['user_id']
                                                    );
