...
if ($_REQUEST['act'] == 'single_reply') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['single_manage'], 'href' => 'comment_manage.php?act=single_list'));
    $single_edit_delete = get_merchants_permissions($user_action_list, 'single_edit_delete');
    $smarty->assign('single_edit_delete', $single_edit_delete);
    assign_query_info();
    $smarty->display('single_info.htm');
} else {
    if ($_REQUEST['act'] == 'drop_single_image') {
...
if ($_REQUEST['act'] == 'single_check') {
    ...
    if ($_REQUEST['check'] == 'allow') {
        ...
        if ($integ) {
            log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = $integ,
                $change_desc = $_LANG['single_reward']);
        }
        ...
        if (!empty($_REQUEST['integ'])) {
            log_account_change($user_id, $user_money = 0, $frozen_money = 0, $rank_points = 0, $pay_points = 0 - $integ,
                $change_desc = $_LANG['single_prohibit_integral']);
        }
...
if ($_REQUEST['act'] == 'reply') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['05_comment_manage'], 'href' => 'comment_manage.php?act=list'));
    assign_query_info();
...
if ($_REQUEST['act'] == 'action') {
    ...
    if (!empty($reply_info['content']) && $adminru['ru_id'] == $comment_info['ru_id']) {
        ...
        if ($adminru['ru_id'] == $comment_info['ru_id']) {
    ...
    if (!empty($send_email_notice) || isset($remail) && !empty($remail)) {
        ...
        if (send_mail($comment_info['user_name'], $comment_info['email'], $template['template_subject'], $content,
            $template['is_html'])) {
            $send_ok = 0;
        } else {
            $send_ok = 1;
...
if ($_REQUEST['act'] == 'batch') {
    ...
    if (isset($_POST['checkboxes'])) {
        switch ($action) {
            case 'remove':
                $sql = 'SELECT comment_img, img_thumb FROM ' . $ecs->table('comment_img') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                        'comment_id');
                $img = $db->getAll($sql);
                ...
                $db->query('DELETE FROM ' . $ecs->table('comment_img') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                        'comment_id'));
                $db->query('DELETE FROM ' . $ecs->table('comment') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                        'comment_id'));
                $db->query('DELETE FROM ' . $ecs->table('comment') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                        'parent_id'));
                break;
            ...
            case 'allow':
                $db->query('UPDATE ' . $ecs->table('comment') . ' SET status = 1  WHERE ' . db_create_in($_POST['checkboxes'],
                        'comment_id'));
                break;

            case 'deny':
                $db->query('UPDATE ' . $ecs->table('comment') . ' SET status = 0  WHERE ' . db_create_in($_POST['checkboxes'],
                        'comment_id'));
                break;
