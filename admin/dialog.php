...
if ($_REQUEST['act'] == 'dialog_warehouse') {
    ...
    if ($_REQUEST['act'] == 'extension_category') {
        $json = new JSON();
        $result = array('content' => '', 'sgs' => '');
        $temp = !empty($_REQUEST['temp']) ? $_REQUEST['temp'] : '';
        $smarty->assign('temp', $temp);
        ...
        if ($_REQUEST['act'] == 'add_attr_img') {
            ...
            $goods_attr_info = get_table_date('goods_attr',
                'goods_id = \'' . $goods_id . '\' and attr_id = \'' . $attr_id . '\' and goods_attr_id = \'' . $goods_attr_id . '\'',
                $goods_attr_date);

            if ($goods_attr_info) {
                if ($goods_attr_info['attr_img_flie']) {
                    $goods_attr_info['attr_img_flie'] = get_image_path($goods_attr_id,
                        $goods_attr_info['attr_img_flie'], true);
                }

                if ($goods_attr_info['attr_img_site']) {
                    $goods_attr_info['attr_img_site'] = get_image_path($goods_attr_id,
                        $goods_attr_info['attr_img_site'], true);
                }

                if ($goods_attr_info['attr_gallery_flie']) {
                    $goods_attr_info['attr_gallery_flie'] = get_image_path($goods_attr_id,
                        $goods_attr_info['attr_gallery_flie'], true);
                }
            }
            ...
            if ($_REQUEST['act'] == 'insert_attr_img') {
                ...
                if ($other['attr_checked'] == 1) {
                    $db->autoExecute($ecs->table('goods_attr'), array('attr_checked' => 0), 'UPDATE',
                        'attr_id = ' . $attr_id . ' and goods_id = ' . $goods_id);
                    $result['is_checked'] = 1;
                }

                $db->autoExecute($ecs->table('goods_attr'), $other, 'UPDATE',
                    'goods_attr_id = ' . $goods_attr_id . ' and attr_id = ' . $attr_id . ' and goods_id = ' . $goods_id);
                $result['goods_attr_id'] = $goods_attr_id;

                if ($other['attr_checked'] == 1) {
                    $goods = get_admin_goods_info($goods_id,
                        array('promote_price', 'promote_start_date', 'promote_end_date', 'user_id', 'model_attr'));
                    if ($GLOBALS['_CFG']['add_shop_price'] == 0 && $goods['model_attr'] == 0) {
                        ...
                        if (!empty($goodsAttrId)) {
                            $products = get_warehouse_id_attr_number($goods_id, $goodsAttrId, $goods['user_id'], 0, 0,
                                $goods['model_attr']);
                            ...
                            if ($products) {
                                ...
                                if (0 < $row['promote_price']) {
                                    $promote_price = bargain_price($row['promote_price'], $row['promote_start_date'],
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods'), $other, 'UPDATE',
                                    'goods_id = \'' . $goods_id . '\'');
                            }
                        }
                    }
                    ...
                    if (0 < $goods['model_attr']) {
                        ...
                        $db->autoExecute($ecs->table('goods'), $goods_other, 'UPDATE',
                            'goods_id = \'' . $goods_id . '\'');
                    }
                }
                ...
                if ($_REQUEST['act'] == 'drop_attr_img') {
                    ...
                    $other['attr_img_flie'] = '';
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_attr'), $other, 'UPDATE',
                        'goods_attr_id = \'' . $goods_attr_id . '\'');
                    $result['goods_attr_id'] = $goods_attr_id;
                    exit($json->encode($result));
                } else {
                    if ($_REQUEST['act'] == 'choose_attrImg') {
                        $json = new JSON();
                        $result = array('error' => 0, 'message' => '', 'content' => '');
                        $admin_id = get_admin_id();
                        $goods_id = empty($_REQUEST['goods_id']) ? 0 : intval($_REQUEST['goods_id']);
                        ...
                        if ($_REQUEST['act'] == 'insert_gallery_attr') {
                            $json = new JSON();
                            $result = array('error' => 0, 'message' => '', 'content' => '');
                            ...
                            if ($_REQUEST['act'] == 'manual_intervention') {
                                $json = new JSON();
                                $result = array('error' => 0, 'message' => '', 'content' => '');
                                $goods_id = !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
                                $goods = get_admin_goods_info($goods_id);
                                ...
                                if ($_REQUEST['act'] == 'add_goods_model_price') {
                                    $json = new JSON();
                                    $result = array('error' => 0, 'message' => '', 'content' => '');
                                    $goods_id = !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
                                    $warehouse_id = 0;
                                    ...
                                    $list = get_goods_warehouse_area_list($goods_id, $goods['model_attr'],
                                        $warehouse_id);
                                    ...
                                    if ($_REQUEST['act'] == 'goods_wa_query') {
                                        $json = new JSON();
                                        $result = array('error' => 0, 'message' => '', 'content' => '');
                                        ...
                                        $smarty->assign('goods', $goods);
                                        make_json_result($smarty->fetch('goods_price_list.lbi'), '', array(
                                            'pb_filter' => $list['filter'],
                                            'pb_page_count' => $list['page_count'],
                                            'class' => 'goodslistDiv'
                                        ));
                                    } else {
                                        if ($_REQUEST['act'] == 'add_warehouse_price') {
                                            $json = new JSON();
                                            $result = array('error' => 0, 'message' => '', 'content' => '');
                                            $goods_id = !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
                                            ...
                                            $goods_attr_name = !empty($_REQUEST['goods_attr_name']) ? trim($_REQUEST['goods_attr_name']) : '';
                                            $action_link = array(
                                                'href' => 'goods.php?act=edit&goods_id=' . $goods_id . '&extension_code=',
                                                'text' => $_LANG['goods_info']
                                            );

                                            if (empty($goods_attr_id)) {
                                                $goods_attr_id = get_goods_attr_nameId($goods_id, $attr_id,
                                                    $goods_attr_name);
                                            }

                                            if (empty($attr_id)) {
                                                $attr_id = get_goods_attr_nameId($goods_id, $goods_attr_id,
                                                    $goods_attr_name, 'attr_id', 1);
                                            }

                                            $goods_date = array('goods_name');
                                            $goods_info = get_table_date('goods', 'goods_id = \'' . $goods_id . '\'',
                                                $goods_date);
                                            $attr_date = array('attr_name');
                                            $attr_info = get_table_date('attribute', 'attr_id = \'' . $attr_id . '\'',
                                                $attr_date);
                                            ...
                                            if ($_REQUEST['act'] == 'insert_warehouse_price') {
                                                $json = new JSON();
                                                $result = array('error' => 0, 'message' => '', 'content' => '');
                                                $goods_id = !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
                                                ...
                                                $goods_attr_name = !empty($_REQUEST['goods_attr_name']) ? $_REQUEST['goods_attr_name'] : '';
                                                get_warehouse_area_attr_price_insert($warehouse_name, $goods_id,
                                                    $goods_attr_id, 'warehouse_attr');
                                                $result['goods_attr_id'] = $goods_attr_id;
                                                exit($json->encode($result));
                                            } else {
                                                if ($_REQUEST['act'] == 'del_goods_attr') {
                                                    $json = new JSON();
                                                    $result = array('error' => 0, 'message' => '', 'content' => '');
                                                    ...
                                                    $ecs->get_del_find_in_set($goods_attr_id, $where, $table,
                                                        'goods_attr', '|');
                                                    exit($json->encode($result));
                                                } else {
                                                    if ($_REQUEST['act'] == 'add_area_price') {
                                                        $json = new JSON();
                                                        $result = array('error' => 0, 'message' => '', 'content' => '');
                                                        $goods_id = !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
                                                        $attr_id = isset($_REQUEST['attr_id']) && !empty($_REQUEST['attr_id']) ? intval($_REQUEST['attr_id']) : 0;
                                                        ...
                                                        $goods_date = array('goods_name');
                                                        $goods_info = get_table_date('goods',
                                                            'goods_id = \'' . $goods_id . '\'', $goods_date);
                                                        $attr_date = array('attr_name');
                                                        $attr_info = get_table_date('attribute',
                                                            'attr_id = \'' . $attr_id . '\'', $attr_date);
                                                        $warehouse_area_list = get_fine_warehouse_area_all(0, $goods_id,
                                                            $goods_attr_id);
                                                        $smarty->assign('goods_info', $goods_info);
                                                        $smarty->assign('attr_info', $attr_info);
                                                        $smarty->assign('goods_attr_name', $goods_attr_name);
                                                        ...
                                                        if ($_REQUEST['act'] == 'insert_area_price') {
                                                            $json = new JSON();
                                                            $result = array(
                                                                'error' => 0,
                                                                'message' => '',
                                                            ...
                                                            $goods_attr_name = !empty($_REQUEST['goods_attr_name']) ? $_REQUEST['goods_attr_name'] : '';
                                                            get_warehouse_area_attr_price_insert($area_name, $goods_id,
                                                                $goods_attr_id, 'warehouse_area_attr');
                                                            $result['goods_attr_id'] = $goods_attr_id;
                                                            exit($json->encode($result));
                                                        } else {
                                                            if ($_REQUEST['act'] == 'add_sku') {
                                                                $json = new JSON();
                                                                $result = array(
                                                                    'error' => 0,
                                                                    'message' => '',
                                                                ...
                                                                $smarty->assign('goods_id', $goods_id);
                                                                $smarty->assign('user_id', $user_id);
                                                                $smarty->assign('goods_attr_price',
                                                                    $GLOBALS['_CFG']['goods_attr_price']);
                                                                $product_list = get_goods_product_list($goods_id,
                                                                    $goods['model_attr'], $warehouse_id, $area_id, true,
                                                                    $city_id);
                                                                $smarty->assign('product_list',
                                                                    $product_list['product_list']);
                                                                $smarty->assign('sku_filter', $product_list['filter']);
                                                                $smarty->assign('sku_record_count',
                                                                    $product_list['record_count']);
                                                                $smarty->assign('sku_page_count',
                                                                ...
                                                                if ($_REQUEST['act'] == 'sku_query') {
                                                                    ...
                                                                    $smarty->assign('goods', $goods);
                                                                    make_json_result($smarty->fetch('goods_attr_list.lbi'),
                                                                        '', array(
                                                                            'pb_filter' => $product_list['filter'],
                                                                            'pb_page_count' => $product_list['page_count'],
                                                                            'class' => 'attrlistDiv'
                                                                        ));
                                                                } else {
                                                                    if ($_REQUEST['act'] == 'add_attr_sku') {
                                                                        $json = new JSON();
                                                                        $result = array(
                                                                            'error' => 0,
                                                                            'message' => '',
                                                                        ...
                                                                        $product = get_product_info($product_id,
                                                                            'product_id, product_number, goods_id, product_sn, goods_attr' . $filed,
                                                                            $goods_info['model_attr'], 1);
                                                                        $smarty->assign('goods_info', $goods_info);
                                                                        ...
                                                                        $warehouse = get_area_info($warehouse_area_id,
                                                                            1);
                                                                        $smarty->assign('warehouse_id', $warehouse_id);
                                                                        $smarty->assign('area_id', $area_id);
                                                                        ...
                                                                        if ($_REQUEST['act'] == 'getload_url') {
                                                                            $smarty->assign('temp', 'load_url');
                                                                            $smarty->display('library/dialog.lbi');
                                                                        } else {
                                                                            if ($_REQUEST['act'] == 'dialog_upgrade') {
                                                                                $json = new JSON();
                                                                                $result = array(
                                                                                    'content' => '',
                                                                                    'sgs' => ''
                                                                                );
                                                                                $smarty->assign('cat_belongs',
                                                                                    $GLOBALS['_CFG']['cat_belongs']);
                                                                                $smarty->assign('brand_belongs',
                                                                                    $GLOBALS['_CFG']['brand_belongs']);
                                                                                ...
                                                                                if ($_REQUEST['act'] == 'attr_input_type') {
                                                                                    ...
                                                                                    $attr_id = isset($_REQUEST['attr_id']) && !empty($_REQUEST['attr_id']) ? intval($_REQUEST['attr_id']) : 0;
                                                                                    $goods_id = isset($_REQUEST['goods_id']) && !empty($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
                                                                                    $smarty->assign('attr_id',
                                                                                        $attr_id);
                                                                                    $smarty->assign('goods_id',
                                                                                        $goods_id);
                                                                                    $goods_attr = get_dialog_goods_attr_type($attr_id,
                                                                                        $goods_id);
                                                                                    $smarty->assign('goods_attr',
                                                                                        $goods_attr);
                                                                                    $result['content'] = $GLOBALS['smarty']->fetch('library/attr_input_type.lbi');
                                                                                    exit($json->encode($result));
                                                                                } else {
                                                                                    if ($_REQUEST['act'] == 'insert_attr_input') {
                                                                                        $json = new JSON();
                                                                                        $result = array(
                                                                                            'content' => '',
                                                                                            'sgs' => ''
                                                                                        ...
                                                                                        $goods_attr_id = isset($_REQUEST['attr_id_val']) ? explode(',',
                                                                                            $_REQUEST['attr_id_val']) : $goods_attr_id;
                                                                                        $attr_value_list = isset($_REQUEST['value_list_val']) ? explode(',',
                                                                                            $_REQUEST['value_list_val']) : $attr_value_list;
                                                                                        ...
                                                                                        if ($_REQUEST['act'] == 'del_input_type') {
                                                                                            $json = new JSON();
                                                                                            $result = array(
                                                                                                'content' => '',
                                                                                                'sgs' => ''
                                                                                            ...
                                                                                            $where = ' AND goods_id = \'' . $goods_id . '\'';
                                                                                            $ecs->get_del_find_in_set($goods_attr_id,
                                                                                                $where, $table,
                                                                                                'goods_attr', '|');
                                                                                            $goods_attr = get_dialog_goods_attr_type($attr_id,
                                                                                                $goods_id);
                                                                                            ...
                                                                                            if ($_REQUEST['act'] == 'del_volume') {
                                                                                                ...
                                                                                                if ($_REQUEST['act'] == 'del_wholesale_volume') {
                                                                                                    $json = new JSON();
                                                                                                    $result = array(
                                                                                                        'content' => '',
                                                                                                        'sgs' => ''
                                                                                                    ...
                                                                                                    if ($_REQUEST['act'] == 'del_cfull') {
                                                                                                        ...
                                                                                                        $db->query($sql);
                                                                                                        $consumption_list = get_goods_con_list($goods_id,
                                                                                                            'goods_consumption');
                                                                                                        ...
                                                                                                        if ($_REQUEST['act'] == 'add_external_url') {
                                                                                                            $json = new JSON();
                                                                                                            $result = array(
                                                                                                                'content' => '',
                                                                                                                'sgs' => '',
                                                                                                            ...
                                                                                                            if ($_REQUEST['act'] == 'insert_external_url') {
                                                                                                                $json = new JSON();
                                                                                                                $result = array(
                                                                                                                    'content' => '',
                                                                                                                    'sgs' => '',
                                                                                                                ...
                                                                                                                if ($external_url_list) {
                                                                                                                    ...
                                                                                                                    foreach ($external_url_list as $key => $image_urls) {
                                                                                                                        if ($image_urls) {
                                                                                                                            if (!empty($image_urls) && $image_urls != $GLOBALS['_LANG']['img_file'] && $image_urls != 'http://' && (strpos($image_urls,
                                                                                                                                        'http://') !== false || strpos($image_urls,
                                                                                                                                        'https://') !== false)) {
                                                                                                                                if (get_http_basename($image_urls,
                                                                                                                                    $admin_temp_dir)) {
                                                                                                                                    $image_url = trim($image_urls);
                                                                                                                                    $down_img = $admin_temp_dir . '/' . basename($image_url);
                                                                                                                                    $img_wh = $GLOBALS['image']->get_width_to_height($down_img,
                                                                                                                                    ...
                                                                                                                                    $goods_img = $image->make_thumb(array(
                                                                                                                                        'img' => $down_img,
                                                                                                                                        'type' => 1
                                                                                                                                    ),
                                                                                                                                        $GLOBALS['_CFG']['image_width'],
                                                                                                                                    ...
                                                                                                                                    if ($proc_thumb) {
                                                                                                                                        $thumb_url = $GLOBALS['image']->make_thumb(array(
                                                                                                                                            'img' => $down_img,
                                                                                                                                            'type' => 1
                                                                                                                                        ),
                                                                                                                                            $GLOBALS['_CFG']['thumb_width'],
                                                                                                                                            $GLOBALS['_CFG']['thumb_height']);
                                                                                                                                        $thumb_url = reformat_image_name('gallery_thumb',
                                                                                                                                            $goods_id,
                                                                                                                                            $thumb_url,
                                                                                                                                            'thumb');
                                                                                                                                    } else {
                                                                                                                                    ...
                                                                                                                                    $img_original = reformat_image_name('gallery',
                                                                                                                                        $goods_id,
                                                                                                                                        $down_img,
                                                                                                                                        'source');
                                                                                                                                    $img_url = reformat_image_name('gallery',
                                                                                                                                        $goods_id,
                                                                                                                                        $goods_img,
                                                                                                                                        'goods');
                                                                                                                                    $desc += 1;
                                                                                                                    ...
                                                                                                                    if (!empty($_SESSION['thumb_img_id' . $_SESSION['admin_id']])) {
                                                                                                                        $_SESSION['thumb_img_id' . $_SESSION['admin_id']] = array_merge($thumb_img_id,
                                                                                                                            $_SESSION['thumb_img_id' . $_SESSION['admin_id']]);
                                                                                                                    } else {
                                                                                                                ...
                                                                                                                if (isset($GLOBALS['shop_id']) && 0 < $GLOBALS['shop_id']) {
                                                                                                                    foreach ($img_list as $key => $gallery_img) {
                                                                                                                        ...
                                                                                                                        $gallery_img['img_original'] = get_image_path($gallery_img['goods_id'],
                                                                                                                            $gallery_img['img_original'],
                                                                                                                            true);
                                                                                                                        $img_list[$key]['img_url'] = $gallery_img['img_original'];
                                                                                                                        $gallery_img['thumb_url'] = get_image_path($gallery_img['goods_id'],
                                                                                                                            $gallery_img['thumb_url'],
                                                                                                                            true);
                                                                                                                ...
                                                                                                                if ($_REQUEST['act'] == 'insert_gallery_url') {
                                                                                                                    $json = new JSON();
                                                                                                                    $result = array(
                                                                                                                        'content' => '',
                                                                                                                        'sgs' => '',
                                                                                                                    ...
                                                                                                                    if (!empty($external_url)) {
                                                                                                                        ...
                                                                                                                        $thumb_url = $db->getOne($sql,
                                                                                                                            true);
                                                                                                                        $thumb_url = get_image_path($img_id,
                                                                                                                            $thumb_url,
                                                                                                                            true);
                                                                                                                    ...
                                                                                                                    if ($_REQUEST['act'] == 'pic_album') {
                                                                                                                        ...
                                                                                                                        $smarty->assign('album_id',
                                                                                                                            $album_id);
                                                                                                                        $smarty->assign('temp',
                                                                                                                            $_REQUEST['act']);
                                                                                                                        $album_info = get_goods_gallery_album(2,
                                                                                                                            $album_id,
                                                                                                                            array('suppliers_id,ru_id'));
                                                                                                                        $cat_select = gallery_cat_list(0,
                                                                                                                            0,
                                                                                                                            false,
                                                                                                                            0,
                                                                                                                            true,
                                                                                                                            $album_info['ru_id'],
                                                                                                                            $album_info['suppliers_id']);
                                                                                                                        ...
                                                                                                                        $smarty->assign('cat_select',
                                                                                                                            $cat_select);
                                                                                                                        $album_mame = get_goods_gallery_album(0,
                                                                                                                            $album_id,
                                                                                                                            array('album_mame'));
                                                                                                                        $smarty->assign('album_mame',
                                                                                                                            $album_mame);
                                                                                                                        $result['content'] = $GLOBALS['smarty']->fetch('library/dialog.lbi');
                                                                                                                        exit($json->encode($result));
                                                                                                                    } else {
                                                                                                                        if ($_REQUEST['act'] == 'album_move') {
                                                                                                                            $json = new JSON();
                                                                                                                            $result = array(
                                                                                                                                'content' => '',
                                                                                                                                'pic_id' => '',
                                                                                                                            ...
                                                                                                                            if ($_REQUEST['act'] == 'update_review_status') {
                                                                                                                                $json = new JSON();
                                                                                                                                $result = array(
                                                                                                                                    'error' => 0,
                                                                                                                                    'message' => '',
                                                                                                                                ...
                                                                                                                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods'),
                                                                                                                                    $other,
                                                                                                                                    'UPDATE',
                                                                                                                                    'goods_id = \'' . $goods_id . '\'');
                                                                                                                                $result['goods_id'] = $goods_id;
                                                                                                                                ...
                                                                                                                                if ($_REQUEST['act'] == 'shop_banner') {
                                                                                                                                    ...
                                                                                                                                    if ($is_vis == 0) {
                                                                                                                                        ...
                                                                                                                                        $link = !empty($spec_attr['link']) && $spec_attr['link'] != ',' ? explode(',',
                                                                                                                                            $spec_attr['link']) : array();
                                                                                                                                        ...
                                                                                                                                        for ($i = 0; $i < $count; $i++) {
                                                                                                                                            if ($pic_src[$i]) {
                                                                                                                                                $arr[$i + 1]['pic_src'] = get_image_path($i + 1,
                                                                                                                                                    $pic_src[$i]);

                                                                                                                                                if ($link[$i]) {
                                                                                                                                                    $arr[$i + 1]['link'] = str_replace(array('＆'),
                                                                                                                                                        '&',
                                                                                                                                                        $link[$i]);
                                                                                                                                                } else {
                                                                                                                                                    $arr[$i + 1]['link'] = $link[$i];
                                                                                                                                    ...
                                                                                                                                    $smarty->assign('cat_select',
                                                                                                                                        $cat_select);
                                                                                                                                    $smarty->assign('is_vis',
                                                                                                                                        $is_vis);

                                                                                                                                    if ($is_vis == 0) {
                                                                                                                                        if ($result['mode'] == 'fh-haohuo' || $result['mode'] == 'h-phb') {
                                                                                                                                            $titleup = 1;
                                                                                                                                            $smarty->assign('hierarchy',
                                                                                                                                                $result['hierarchy']);
                                                                                                                                            $smarty->assign('lift',
                                                                                                                                                $lift);
                                                                                                                                        }

                                                                                                                                        $smarty->assign('pic_number',
                                                                                                                                            $pic_number);
                                                                                                                                        $smarty->assign('mode',
                                                                                                                                            $result['mode']);
                                                                                                                                        $smarty->assign('spec_attr',
                                                                                                                                            $spec_attr);
                                                                                                                                        $smarty->assign('uploadImage',
                                                                                                                                            $uploadImage);
                                                                                                                                        $smarty->assign('titleup',
                                                                                                                                            $titleup);
                                                                                                                                        $smarty->assign('suffix',
                                                                                                                                            $result['suffix']);
                                                                                                                                        $result['content'] = $GLOBALS['smarty']->fetch('library/shop_banner.lbi');
                                                                                                                                    } else {
                                                                                                                                        $smarty->assign('image_type',
                                                                                                                                            0);
                                                                                                                                        $smarty->assign('log_type',
                                                                                                                                            'image');
                                                                                                                                        $smarty->assign('image_type',
                                                                                                                                            $image_type);
                                                                                                                                        $smarty->assign('inid',
                                                                                                                                    ...
                                                                                                                                    if ($_REQUEST['act'] == 'add_albun_pic') {
                                                                                                                                        ...
                                                                                                                                        if ($_REQUEST['act'] == 'goods_info') {
                                                                                                                                            ...
                                                                                                                                            if ($goods_id && $spec_attr['ru_id'] == -1) {
                                                                                                                                                ...
                                                                                                                                                if (-1 < $spec_attr['ru_id']) {
                                                                                                                                                    $adminru['ru_id'] = $spec_attr['ru_id'];
                                                                                                                                                }
                                                                                                                                            }

                                                                                                                                            set_default_filter(0,
                                                                                                                                                $cat_id,
                                                                                                                                                $adminru['ru_id']);
                                                                                                                                            $smarty->assign('parent_category',
                                                                                                                                                get_every_category($cat_id));
                                                                                                                                            $smarty->assign('select_category_html',
                                                                                                                                                $select_category_html);
                                                                                                                                            $smarty->assign('arr',
                                                                                                                                                $spec_attr);
                                                                                                                                            $smarty->assign('temp',
                                                                                                                                                'goods_info');
                                                                                                                                            $smarty->assign('goods_type',
                                                                                                                                                $goods_type);
                                                                                                                                            $smarty->assign('mode',
                                                                                                                                                $result['mode']);
                                                                                                                                            $smarty->assign('cat_id',
                                                                                                                                                $cat_id);
                                                                                                                                            $smarty->assign('lift',
                                                                                                                                                $lift);
                                                                                                                                            $smarty->assign('good_number',
                                                                                                                                                $good_number);
                                                                                                                                            ...
                                                                                                                                            if ($_REQUEST['act'] == 'custom') {
                                                                                                                                                ...
                                                                                                                                                if ($GLOBALS['_CFG']['open_oss'] == 1) {
                                                                                                                                                    ...
                                                                                                                                                    if ($custom_content) {
                                                                                                                                                        $desc_preg = get_goods_desc_images_preg($bucket_info['endpoint'],
                                                                                                                                                            $custom_content);
                                                                                                                                                        $custom_content = $desc_preg['goods_desc'];
                                                                                                                                                    }
                                                                                                                                                }

                                                                                                                                                $FCKeditor = create_ueditor_editor('custom_content',
                                                                                                                                                    $custom_content,
                                                                                                                                                    486,
                                                                                                                                                    1);
                                                                                                                                                ...
                                                                                                                                                if ($_REQUEST['act'] == 'home_adv') {
                                                                                                                                                    ...
                                                                                                                                                    if (!empty($_REQUEST['spec_attr'])) {
                                                                                                                                                        $spec_attr = json_decode($_REQUEST['spec_attr'],
                                                                                                                                                            true);
                                                                                                                                                    }
                                                                                                                                                    ...
                                                                                                                                                    if ($result['mode'] == 'h-brand') {
                                                                                                                                                        $spec_attr['brand_ids'] = resetBarnd($spec_attr['brand_ids'],
                                                                                                                                                            'brand',
                                                                                                                                                            'brand_id');
                                                                                                                                                        $brand = getBrandList($spec_attr['brand_ids']);
                                                                                                                                                        $smarty->assign('filter',
                                                                                                                                                            $brand['filter']);
                                                                                                                                                        $smarty->assign('recommend_brands',
                                                                                                                                                            $brand['list']);
                                                                                                                                                    } else {
                                                                                                                                                        if ($result['mode'] == 'h-promo') {
                                                                                                                                                            $spec_attr['goods_ids'] = resetBarnd($spec_attr['goods_ids']);
                                                                                                                                                            $time = gmtime();
                                                                                                                                                            ...
                                                                                                                                                            if (!empty($goods_list)) {
                                                                                                                                                                foreach ($goods_list as $key => $val) {
                                                                                                                                                                    $goods_list[$key]['goods_thumb'] = get_image_path($val['goods_id'],
                                                                                                                                                                        $val['goods_thumb']);
                                                                                                                                                                    if (0 < $val['promote_price'] && $val['promote_start_date'] <= $time && $time <= $val['promote_end_date']) {
                                                                                                                                                                        $goods_list[$key]['promote_price'] = price_format(bargain_price($val['promote_price'],
                                                                                                                                                                            $val['promote_start_date'],
                                                                                                                                                                            $val['promote_end_date']));
                                                                                                                                                                    } else {
                                                                                                                                                                    ...
                                                                                                                                                                    if (!empty($goods_ids)) {
                                                                                                                                                                        if (0 < $val['goods_id'] && in_array($val['goods_id'],
                                                                                                                                                                                $goods_ids)) {
                                                                                                                                                            ...
                                                                                                                                                            if ($result['mode'] == 'h-sepmodule') {
                                                                                                                                                                ...
                                                                                                                                                                if (!empty($spec_attr['PromotionType'])) {
                                                                                                                                                                    if ($spec_attr['PromotionType'] == 'exchange') {
                                                                                                                                                                        ...
                                                                                                                                                                        if ($spec_attr['PromotionType'] == 'presale') {
                                                                                                                                                                            ...
                                                                                                                                                                            if ($spec_attr['PromotionType'] == 'is_new') {
                                                                                                                                                                                $spec_attr['goods_ids'] = resetBarnd($spec_attr['goods_ids']);

                                                                                                                                                                                if ($spec_attr['goods_ids']) {
                                                                                                                                                                                    $spec_attr['goods_ids'] = explode(',',
                                                                                                                                                                                        $spec_attr['goods_ids']);
                                                                                                                                                                                }

                                                                                                                                                                                $where .= ' AND g.is_new = 1 AND  g.is_on_sale=1 AND g.is_delete=0';
                                                                                                                                                                            } else {
                                                                                                                                                                                if ($spec_attr['PromotionType'] == 'is_best') {
                                                                                                                                                                                    $spec_attr['goods_ids'] = resetBarnd($spec_attr['goods_ids']);
                                                                                                                                                                                    ...
                                                                                                                                                                                    if ($spec_attr['PromotionType'] == 'is_hot') {
                                                                                                                                                                                        $spec_attr['goods_ids'] = resetBarnd($spec_attr['goods_ids']);
                                                                                                                                                                                        ...
                                                                                                                                                                                        if ($spec_attr['PromotionType'] == 'snatch') {
                                                                                                                                                                                            ...
                                                                                                                                                                                            if ($spec_attr['PromotionType'] == 'auction') {
                                                                                                                                                                                                ...
                                                                                                                                                                                                if ($spec_attr['PromotionType'] == 'group_buy') {
                                                                                                                                                                    ...
                                                                                                                                                                    if (!empty($goods_list)) {
                                                                                                                                                                        foreach ($goods_list as $key => $val) {
                                                                                                                                                                            ...
                                                                                                                                                                            if ($spec_attr['PromotionType'] == 'exchange') {
                                                                                                                                                                                $goods_list[$key]['url'] = build_uri('exchange_goods',
                                                                                                                                                                                    array('gid' => $val['goods_id']),
                                                                                                                                                                                    $val['goods_name']);
                                                                                                                                                                ...
                                                                                                                                                                $smarty->assign('activity_dialog',
                                                                                                                                                                    $result['activity_dialog']);
                                                                                                                                                            } else {
                                                                                                                                                                if ($result['mode'] == 'h-seckill') {
                                                                                                                                                                    if (!empty($spec_attr['goods_ids'])) {
                                                                                                                                                                        foreach ($spec_attr['goods_ids'] as $k => $v) {
                                                                                                                                                                            $spec_attr['goods_ids'][$k] = resetBarnd($v,
                                                                                                                                                                                'seckill');
                                                                                                                                                                        }
                                                                                                                                                                    }
                                                                                                                                                                    ...
                                                                                                                                                                    $smarty->assign('seckill_time_bucket',
                                                                                                                                                                        $arr);
                                                                                                                                                                    $smarty->assign('time_bucket',
                                                                                                                                                                        $time_bucket);
                                                                                                                                                                }
                                                                                                                                                            }
                                                                                                                                                        }
                                                                                                                                                    ...
                                                                                                                                                    $smarty->assign('lift',
                                                                                                                                                        $lift);
                                                                                                                                                    $smarty->assign('needColor',
                                                                                                                                                        $needColor);
                                                                                                                                                    $smarty->assign('spec_attr',
                                                                                                                                                        $spec_attr);
                                                                                                                                                    $smarty->assign('hierarchy',
                                                                                                                                                        $result['hierarchy']);
                                                                                                                                                    $smarty->assign('masterTitle',
                                                                                                                                                        $masterTitle);
                                                                                                                                                    $smarty->assign('suffix',
                                                                                                                                                        $result['suffix']);
                                                                                                                                                    $result['content'] = $GLOBALS['smarty']->fetch('library/shop_banner.lbi');
                                                                                                                                                    ...
                                                                                                                                                    if ($_REQUEST['act'] == 'changedpromotegoods') {
                                                                                                                                                        require ROOT_PATH . '/includes/lib_goods.php';
                                                                                                                                                        $json = new JSON();
                                                                                                                                                        $result = array(
                                                                                                                                                            'content' => '',
                                                                                                                                                        ...
                                                                                                                                                        $goods_ids = !empty($_REQUEST['goods_ids']) ? explode(',',
                                                                                                                                                            $_REQUEST['goods_ids']) : array();
                                                                                                                                                        ...
                                                                                                                                                        if (!empty($PromotionType)) {
                                                                                                                                                            ...
                                                                                                                                                            if ($PromotionType == 'exchange') {
                                                                                                                                                                ...
                                                                                                                                                                if ($PromotionType == 'presale') {
                                                                                                                                                                    ...
                                                                                                                                                                    if ($PromotionType == 'is_new') {
                                                                                                                                                                        $where .= ' AND g.is_new = 1 AND  g.is_on_sale=1 AND g.is_delete=0';
                                                                                                                                                                    } else {
                                                                                                                                                                        if ($PromotionType == 'is_best') {
                                                                                                                                                                            $where .= ' AND g.is_best = 1 AND  g.is_on_sale=1 AND g.is_delete=0 ';
                                                                                                                                                                        } else {
                                                                                                                                                                            if ($PromotionType == 'is_hot') {
                                                                                                                                                                                $where .= ' AND g.is_hot = 1  AND  g.is_on_sale=1 AND g.is_delete=0';
                                                                                                                                                                            } else {
                                                                                                                                                            ...
                                                                                                                                                            if ($_REQUEST['act'] == 'homeFloor') {
                                                                                                                                                                ...
                                                                                                                                                                if ($spec_attr['leftBannerLink']) {
                                                                                                                                                                    foreach ($spec_attr['leftBannerLink'] as $k => $v) {
                                                                                                                                                                        $spec_attr['leftBannerLink'][$k] = str_replace(array('＆'),
                                                                                                                                                                            '&',
                                                                                                                                                                            $v);
                                                                                                                                                                    }
                                                                                                                                                                }
                                                                                                                                                                ...
                                                                                                                                                                if ($spec_attr['rightAdvLink']) {
                                                                                                                                                                    foreach ($spec_attr['rightAdvLink'] as $k => $v) {
                                                                                                                                                                        $spec_attr['rightAdvLink'][$k] = str_replace(array('＆'),
                                                                                                                                                                            '&',
                                                                                                                                                                            $v);
                                                                                                                                                                    }
                                                                                                                                                                }
                                                                                                                                                                ...
                                                                                                                                                                if ($spec_attr['leftAdvLink']) {
                                                                                                                                                                    foreach ($spec_attr['leftAdvLink'] as $k => $v) {
                                                                                                                                                                        $spec_attr['leftAdvLink'][$k] = str_replace(array('＆'),
                                                                                                                                                                            '&',
                                                                                                                                                                            $v);
                                                                                                                                                                    }
                                                                                                                                                                }
                                                                                                                                                                ...
                                                                                                                                                                if (0 < $cat_id) {
                                                                                                                                                                    $parent = get_cat_info($spec_attr['cat_id'],
                                                                                                                                                                        array('parent_id'));
                                                                                                                                                                ...
                                                                                                                                                                $smarty->assign('hierarchy',
                                                                                                                                                                    $result['hierarchy']);
                                                                                                                                                                $smarty->assign('floor_style',
                                                                                                                                                                    $floor_style);
                                                                                                                                                                $smarty->assign('imgNumberArr',
                                                                                                                                                                    $imgNumberArr);
                                                                                                                                                                $smarty->assign('suffix',
                                                                                                                                                                    $suffix);
                                                                                                                                                                $result['content'] = $GLOBALS['smarty']->fetch('library/shop_banner.lbi');
                                                                                                                                                                    ...
                                                                                                                                                                    if ($_REQUEST['act'] == 'edit_cmsarti') {
                                                                                                                                                                        ...
                                                                                                                                                                        if ($_REQUEST['act'] == 'edit_cmsgoods') {
                                                                                                                                                                            ...
                                                                                                                                                                            set_default_filter(0,
                                                                                                                                                                                ...
                                                                                                                                                                                $adminru['ru_id']);
                                                                                                                                                                            $smarty->assign('parent_category',
                                                                                                                                                                                get_every_category($spec_attr['cat_id']));
                                                                                                                                                                            $smarty->assign('select_category_html',
                                                                                                                                                                                $select_category_html);
                                                                                                                                                                            $smarty->assign('cat_select',
                                                                                                                                                                                article_cat_list_new(0,
                                                                                                                                                                            ...
                                                                                                                                                                            if ($_REQUEST['act'] == 'vipEdit') {
                                                                                                                                                                                ...
                                                                                                                                                                                $quick_url = isset($spec_attr['quick_url']) ? explode(',',
                                                                                                                                                                                    $spec_attr['quick_url']) : array();
                                                                                                                                                                                ...
                                                                                                                                                                                $smarty->assign('quick',
                                                                                                                                                                                    $arr);
                                                                                                                                                                                $smarty->assign('index_article_cat',
                                                                                                                                                                                    $index_article_cat);
                                                                                                                                                                                $result['content'] = $GLOBALS['smarty']->fetch('library/shop_banner.lbi');
                                                                                                                                                                                exit($json->encode($result));
                                                                                                                                                                            } else {
                                                                                                                                                                                if ($_REQUEST['act'] == 'nav_mode') {
                                                                                                                                                                                    $json = new JSON();
                                                                                                                                                                                    $result = array(
                                                                                                                                                                                        'content' => '',
                                                                                                                                                                                        'mode' => ''
                                                                                                                                                                                    ...
                                                                                                                                                                                    $smarty->assign('system',
                                                                                                                                                                                        $system);
                                                                                                                                                                                    $smarty->assign('topic_type',
                                                                                                                                                                                        $result['topic']);
                                                                                                                                                                                    $smarty->assign('navigator',
                                                                                                                                                                                        $spec_attr);
                                                                                                                                                                                    $smarty->assign('suffix',
                                                                                                                                                                                        $suffix);
                                                                                                                                                                                    $result['content'] = $GLOBALS['smarty']->fetch('library/shop_banner.lbi');
                                                                                                                                                                                    ...
                                                                                                                                                                                    if ($_REQUEST['act'] == 'template_information') {
                                                                                                                                                                                        $json = new JSON();
                                                                                                                                                                                        $result = array(
                                                                                                                                                                                            'content' => '',
                                                                                                                                                                                            'mode' => ''
                                                                                                                                                                                        ...
                                                                                                                                                                                        if ($code) {
                                                                                                                                                                                            $smarty->assign('template',
                                                                                                                                                                                                get_seller_template_info($code,
                                                                                                                                                                                                    0,
                                                                                                                                                                                                    $theme));
                                                                                                                                                                                        }

                                                                                                                                                                                        $smarty->assign('template_mall_info',
                                                                                                                                                                                            $template_mall_info);
                                                                                                                                                                                        $smarty->assign('template_type',
                                                                                                                                                                                            $template_type);
                                                                                                                                                                                        $smarty->assign('code',
                                                                                                                                                                                            $code);
                                                                                                                                                                                        $smarty->assign('temp',
                                                                                                                                                                                                ...
                                                                                                                                                                                                if ($_REQUEST['act'] == 'navigator') {
                                                                                                                                                                                                    ...
                                                                                                                                                                                                    if ($_REQUEST['act'] == 'merchant_download') {
                                                                                                                                                                                                        ...
                                                                                                                                                                                                        $smarty->assign('filename',
                                                                                                                                                                                                            $filename);
                                                                                                                                                                                                        $smarty->assign('fileaction',
                                                                                                                                                                                                            $fileaction);
                                                                                                                                                                                                        $smarty->assign('lastfilename',
                                                                                                                                                                                                            $lastfilename);
                                                                                                                                                                                                        $smarty->assign('lastaction',
                                                                                                                                                                                                            $lastaction);
                                                                                                                                                                                                        unset($_SESSION['merchants_download_content']);
                                                                                                                                                                                                        $result['content'] = $GLOBALS['smarty']->fetch('library/merchant_download.lbi');
                                                                                                                                                                                                        exit($json->encode($result));
                                                                                                                                                                                                    } else {
                                                                                                                                                                                                        if ($_REQUEST['act'] == 'set_free_shipping') {
                                                                                                                                                                                                            $json = new JSON();
                                                                                                                                                                                                            $result = array('content' => '');
                                                                                                                                                                                                            $region_ids = !empty($_REQUEST['region_ids']) ? explode(',',
                                                                                                                                                                                                                trim($_REQUEST['region_ids'])) : array();
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            for ($i = 0; $i < $count; $i++) {
                                                                                                                                                                                                                $region_list[$i]['add_time'] = local_date('Y-m-d H:i:s',
                                                                                                                                                                                                                    $region_list[$i]['add_time']);
                                                                                                                                                                                                                $area = ajax_get_area_list($region_list[$i]['ra_id'],
                                                                                                                                                                                                                    $region_ids);
                                                                                                                                                                                                                $region_list[$i]['area_list'] = $area;
                                                                                                                                                                                                            }

                                                                                                                                                                                                            $smarty->assign('region_list',
                                                                                                                                                                                                                $region_list);
                                                                                                                                                                                                            $smarty->assign('temp',
                                                                                                                                                                                                                'set_free_shipping');
                                                                                                                                                                                                            $result['content'] = $GLOBALS['smarty']->fetch('library/dialog.lbi');
                                                                                                                                                                                                            ...
                                                                                                                                                                                                            if ($_REQUEST['act'] == 'add_goods_type_cat') {
                                                                                                                                                                                                                $json = new JSON();
                                                                                                                                                                                                                $result = array('content' => '');
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                if ($type == 'add_goods_type_cat' || $type == 'add_goods_type') {
                                                                                                                                                                                                                    $cat_level = get_type_cat_arr(0,
                                                                                                                                                                                                                        0,
                                                                                                                                                                                                                        0,
                                                                                                                                                                                                                        $user_id);
                                                                                                                                                                                                                    $smarty->assign('cat_level',
                                                                                                                                                                                                                        $cat_level);
                                                                                                                                                                                                                } else {
                                                                                                                                                                                                                    if ($type == 'attribute_add') {
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        $smarty->assign('attr',
                                                                                                                                                                                                                            $attr);
                                                                                                                                                                                                                        $smarty->assign('attr_groups',
                                                                                                                                                                                                                            get_attr_groups($attr['cat_id']));
                                                                                                                                                                                                                        $smarty->assign('goods_type_list',
                                                                                                                                                                                                                            goods_type_list($attr['cat_id']));
                                                                                                                                                                                                                    }
                                                                                                                                                                                                                }
                                                                                                                                                                                                                ...
                                                                                                                                                                                                                if ($_REQUEST['act'] == 'ajaxBrand') {
                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                    $smarty->assign('is_need',
                                                                                                                                                                                                                        $_CFG['template'] == 'ecmoban_dsc2017' ? 1 : 0);
                                                                                                                                                                                                                    $smarty->assign('temp',
                                                                                                                                                                                                                        $_REQUEST['act']);
                                                                                                                                                                                                                    $smarty->assign('lang',
                                                                                                                                                                                                                        $_LANG);
                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                    if ($_REQUEST['act'] == 'brand_insert') {
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        $img_name = basename($image->upload_image($_FILES['brand_logo'],
                                                                                                                                                                                                                            'brandlogo'));
                                                                                                                                                                                                                        get_oss_add_file(array(DATA_DIR . '/brandlogo/' . $img_name));
                                                                                                                                                                                                                        $index_img = basename($image->upload_image($_FILES['index_img'],
                                                                                                                                                                                                                            'indeximg'));
                                                                                                                                                                                                                        get_oss_add_file(array(DATA_DIR . '/indeximg/' . $index_img));
                                                                                                                                                                                                                        $brand_bg = basename($image->upload_image($_FILES['brand_bg'],
                                                                                                                                                                                                                            'brandbg'));
                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                        if ($_REQUEST['act'] == 'ajaxTransport') {
                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                            if ($tid) {
                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                if (0 < $tid) {
                                                                                                                                                                                                                                    $transport_info = $trow;
                                                                                                                                                                                                                                    $shipping_tpl = get_transport_shipping_list($tid,
                                                                                                                                                                                                                                        $adminru['ru_id']);
                                                                                                                                                                                                                                }
                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                            $smarty->assign('shipping_tpl',
                                                                                                                                                                                                                                $shipping_tpl);
                                                                                                                                                                                                                            $smarty->assign('tid',
                                                                                                                                                                                                                                $tid);
                                                                                                                                                                                                                            $smarty->assign('transport_info',
                                                                                                                                                                                                                                $transport_info);
                                                                                                                                                                                                                            $smarty->assign('transport_area',
                                                                                                                                                                                                                                get_transport_area($tid));
                                                                                                                                                                                                                            $smarty->assign('transport_express',
                                                                                                                                                                                                                                get_transport_express($tid));
                                                                                                                                                                                                                            $shipping_list = shipping_list();

                                                                                                                                                                                                                            foreach ($shipping_list as $key => $val) {
                                                                                                                                                                                                                                if (substr($row['shipping_code'],
                                                                                                                                                                                                                                        0,
                                                                                                                                                                                                                                        5) == 'ship_') {
                                                                                                                                                                                                                                    unset($arr[$key]);
                                                                                                                                                                                                                                    continue;
                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                            if ($_REQUEST['act'] == 'transport_insert' || $_REQUEST['act'] == 'transport_update') {
                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                if ($_REQUEST['act'] == 'transport_update') {
                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                    $db->autoExecute($ecs->table('goods_transport'),
                                                                                                                                                                                                                                        $data,
                                                                                                                                                                                                                                        'UPDATE',
                                                                                                                                                                                                                                        'tid = \'' . $data['tid'] . '\'');
                                                                                                                                                                                                                                    $tid = $s_tid;
                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                    $db->autoExecute($ecs->table('goods_transport'),
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        'INSERT');
                                                                                                                                                                                                                                    $tid = $db->insert_id();
                                                                                                                                                                                                                                    $db->autoExecute($ecs->table('goods_transport_extend'),
                                                                                                                                                                                                                                        array('tid' => $tid),
                                                                                                                                                                                                                                        'UPDATE',
                                                                                                                                                                                                                                        'tid = \'0\' AND admin_id = \'' . $admin_id . '\' ');
                                                                                                                                                                                                                                    $db->autoExecute($ecs->table('goods_transport_express'),
                                                                                                                                                                                                                                        array('tid' => $tid),
                                                                                                                                                                                                                                        'UPDATE',
                                                                                                                                                                                                                                        'tid = \'0\' AND admin_id = \'' . $admin_id . '\' ');
                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                $smarty->assign('temp',
                                                                                                                                                                                                                                    'transport_reload');
                                                                                                                                                                                                                                $smarty->assign('transport_list',
                                                                                                                                                                                                                                    get_table_date('goods_transport',
                                                                                                                                                                                                                                        'ru_id=\'' . $adminru['ru_id'] . '\'',
                                                                                                                                                                                                                                        array('tid, title'),
                                                                                                                                                                                                                                        1));
                                                                                                                                                                                                                                $result['content'] = $GLOBALS['smarty']->fetch('library/dialog.lbi');
                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                if ($_REQUEST['act'] == 'ajaxArticle') {
                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                    $db->query($sql);
                                                                                                                                                                                                                                    $smarty->assign('filter_category_list',
                                                                                                                                                                                                                                        get_category_list());

                                                                                                                                                                                                                                    if (isset($_GET['id'])) {
                                                                                                                                                                                                                                        $smarty->assign('cur_id',
                                                                                                                                                                                                                                            $_GET['id']);
                                                                                                                                                                                                                                    }
                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                    if ($_REQUEST['act'] == 'article_insert') {
                                                                                                                                                                                                                                        $json = new JSON();
                                                                                                                                                                                                                                        $result = array(
                                                                                                                                                                                                                                            'content' => '',
                                                                                                                                                                                                                                            'message' => '',
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        if (isset($_FILES['file']['error']) && $_FILES['file']['error'] == 0 || !isset($_FILES['file']['error']) && isset($_FILES['file']['tmp_name']) && $_FILES['file']['tmp_name'] != 'none') {
                                                                                                                                                                                                                                            if (!check_file_type($_FILES['file']['tmp_name'],
                                                                                                                                                                                                                                                $_FILES['file']['name'],
                                                                                                                                                                                                                                                $allow_file_types)) {
                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                        if ($_REQUEST['act'] == 'ajaxArea') {
                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                            $smarty->assign('parent_id',
                                                                                                                                                                                                                                                $region_id);

                                                                                                                                                                                                                                            if ($region_id == 0) {
                                                                                                                                                                                                                                                $region_type = 0;
                                                                                                                                                                                                                                            } else {
                                                                                                                                                                                                                                                $region_type = $exc->get_name($region_id,
                                                                                                                                                                                                                                                        'region_type') + 1;
                                                                                                                                                                                                                                            }

                                                                                                                                                                                                                                            $smarty->assign('region_type',
                                                                                                                                                                                                                                                $region_type);
                                                                                                                                                                                                                                            $region_arr = area_list($region_id);
                                                                                                                                                                                                                                            $smarty->assign('region_arr',
                                                                                                                                                                                                                                                $region_arr);
                                                                                                                                                                                                                                            $area_top = '-';
                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                            $smarty->assign('area_top',
                                                                                                                                                                                                                                                $area_top);
                                                                                                                                                                                                                                            $smarty->assign('area_here',
                                                                                                                                                                                                                                                $area);

                                                                                                                                                                                                                                            if (0 < $region_id) {
                                                                                                                                                                                                                                                $parent_id = $exc->get_name($region_id,
                                                                                                                                                                                                                                                    'parent_id');
                                                                                                                                                                                                                                                $is_ajax = 0;
                                                                                                                                                                                                                                                $action_link = array(
                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                            if ($_REQUEST['act'] == 'area_insert') {
                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                if ($_REQUEST['act'] == 'ajaxCate') {
                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                    $type_level = get_type_cat_arr(0,
                                                                                                                                                                                                                                                        0,
                                                                                                                                                                                                                                                        0,
                                                                                                                                                                                                                                                        $adminru['ru_id']);
                                                                                                                                                                                                                                                    $smarty->assign('type_level',
                                                                                                                                                                                                                                                        $type_level);
                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                    $smarty->assign('goods_type_list',
                                                                                                                                                                                                                                                        goods_type_list(0));
                                                                                                                                                                                                                                                    $smarty->assign('attr_list',
                                                                                                                                                                                                                                                        $list);
                                                                                                                                                                                                                                                    $smarty->assign('cat_info',
                                                                                                                                                                                                                                                        array('is_show' => 1));
                                                                                                                                                                                                                                                    $smarty->assign('ru_id',
                                                                                                                                                                                                                                                        $adminru['ru_id']);
                                                                                                                                                                                                                                                    $smarty->assign('temp',
                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                    if ($_REQUEST['act'] == 'cate_insert') {
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        $exc = new exchange($ecs->table('category'),
                                                                                                                                                                                                                                                            $db,
                                                                                                                                                                                                                                                            'cat_id',
                                                                                                                                                                                                                                                            'cat_name');
                                                                                                                                                                                                                                                        $cat['cat_id'] = !empty($_POST['cat_id']) ? intval($_POST['cat_id']) : 0;
                                                                                                                                                                                                                                                        $cat['parent_id'] = isset($_POST['parent_id']) ? intval($_POST['parent_id']) : 0;
                                                                                                                                                                                                                                                        $cat['level'] = count(get_select_category($cat['parent_id'],
                                                                                                                                                                                                                                                                1,
                                                                                                                                                                                                                                                                true)) - 2;
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        if (!empty($_FILES['cat_icon']['name'])) {
                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                            $imgNamePrefix = time() . mt_rand(1001,
                                                                                                                                                                                                                                                                    9999);
                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                            move_uploaded_file($_FILES['cat_icon']['tmp_name'],
                                                                                                                                                                                                                                                                $imgName);
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        if (!empty($_FILES['touch_icon']['name'])) {
                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                            $touch_iconPrefix = time() . mt_rand(1001,
                                                                                                                                                                                                                                                                    9999);
                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                            move_uploaded_file($_FILES['touch_icon']['tmp_name'],
                                                                                                                                                                                                                                                                $touchimgName);
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        $pin = new pin();
                                                                                                                                                                                                                                                        $pinyin = $pin->Pinyin($cat['cat_name'],
                                                                                                                                                                                                                                                            'UTF8');
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        $cat['filter_attr'] = !empty($_POST['filter_attr']) ? implode(',',
                                                                                                                                                                                                                                                            array_unique(array_diff($_POST['filter_attr'],
                                                                                                                                                                                                                                                                array(0)))) : 0;
                                                                                                                                                                                                                                                        $cat['cat_recommend'] = !empty($_POST['cat_recommend']) ? $_POST['cat_recommend'] : array();

                                                                                                                                                                                                                                                        if (cat_exists($cat['cat_name'],
                                                                                                                                                                                                                                                            $cat['parent_id'])) {
                                                                                                                                                                                                                                                            $result['error'] = 2;
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        if (1 < count($cat_name)) {
                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                            get_bacth_category($cat_name,
                                                                                                                                                                                                                                                                $cat,
                                                                                                                                                                                                                                                                $adminru['ru_id']);
                                                                                                                                                                                                                                                            clear_cache_files();
                                                                                                                                                                                                                                                        } else {
                                                                                                                                                                                                                                                            if ($db->autoExecute($ecs->table('category'),
                                                                                                                                                                                                                                                                    $cat) !== false) {
                                                                                                                                                                                                                                                                $cat_id = $db->insert_id();
                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                if ($cat['show_in_nav'] == 1) {
                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                    $vieworder += 2;
                                                                                                                                                                                                                                                                    $sql = 'INSERT INTO ' . $ecs->table('nav') . ' (name,ctype,cid,ifshow,vieworder,opennew,url,type)' . ' VALUES(\'' . $cat['cat_name'] . '\', \'c\', \'' . $db->insert_id() . ('\',\'1\',\'' . $vieworder . '\',\'0\', \'') . build_uri('category',
                                                                                                                                                                                                                                                                            array('cid' => $cat_id),
                                                                                                                                                                                                                                                                            $cat['cat_name']) . '\',\'middle\')';
                                                                                                                                                                                                                                                                    $db->query($sql);
                                                                                                                                                                                                                                                                }

                                                                                                                                                                                                                                                                admin_log($_POST['cat_name'],
                                                                                                                                                                                                                                                                    'add',
                                                                                                                                                                                                                                                                    'category');
                                                                                                                                                                                                                                                                $dt_list = isset($_POST['document_title']) ? $_POST['document_title'] : array();
                                                                                                                                                                                                                                                                $dt_id = isset($_POST['dt_id']) ? $_POST['dt_id'] : array();
                                                                                                                                                                                                                                                                get_documentTitle_insert_update($dt_list,
                                                                                                                                                                                                                                                                    $cat_id,
                                                                                                                                                                                                                                                                    $dt_id);
                                                                                                                                                                                                                                                                clear_cache_files();
                                                                                                                                                                                                                                                            }
                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                        if ($_REQUEST['act'] == 'video_box') {
                                                                                                                                                                                                                                                            ...
                                                                                                                                                                                                                                                            if ($_REQUEST['act'] == 'goods_review_status') {
                                                                                                                                                                                                                                                                $json = new JSON();
                                                                                                                                                                                                                                                                $result = array(
                                                                                                                                                                                                                                                                    'error' => 0,
                                                                                                                                                                                                                                                                    'message' => '',
                                                                                                                                                                                                                                                                ...
                                                                                                                                                                                                                                                                if ($_REQUEST['act'] == 'goods_ordinary') {
                                                                                                                                                                                                                                                                    $json = new JSON();
                                                                                                                                                                                                                                                                    $result = array(
                                                                                                                                                                                                                                                                        'error' => 0,
                                                                                                                                                                                                                                                                        'message' => '',
                                                                                                                                                                                                                                                                    ...
                                                                                                                                                                                                                                                                    if ($_REQUEST['act'] == 'goods_virtual_card') {
                                                                                                                                                                                                                                                                        $json = new JSON();
                                                                                                                                                                                                                                                                        $result = array(
                                                                                                                                                                                                                                                                            'error' => 0,
                                                                                                                                                                                                                                                                            'message' => '',
