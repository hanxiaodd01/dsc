...
{
    return 'g.cat_id ' . db_create_in(array_unique(array_merge(array($cat),
            get_array_keys_cat($cat, 0, 'zc_category'))));
}
...
if ($_REQUEST['act'] == 'query') {
    ...
    $smarty->assign('page_count', $list['page_count']);
    $smarty->assign('arr_zc', $list['zc_projects']);
    make_json_result($smarty->fetch('zc_project_list.dwt'), '',
    ...
    if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            $end_time = !empty($_POST['promote_end_date']) ? strtotime(trim($_POST['promote_end_date'])) : date('Y-m-d',
                strtotime('+1 month'));
                ...
                if ($_REQUEST['act'] == 'del') {
                    ...
                    if ($_REQUEST['act'] == 'toggle_best') {
                        ...
                        if ($_REQUEST['act'] == 'product_list') {
                            ...
                            $action_link = array(
                                'href' => 'zc_project.php?act=add_product&id=' . $id,
                                'text' => $_LANG['add_zc_goods']
                            );
                            ...
                            if ($_REQUEST['act'] == 'add_product' || $_REQUEST['act'] == 'edit_product') {
                                ...
                                $action_link = array(
                                    'href' => 'zc_project.php?act=product_list&id=' . intval($_GET['id']),
                                    'text' => $_LANG['zc_goods_manage']
                                );
                                $smarty->assign('action_link', $action_link);
                                ...
                                if ($_REQUEST['act'] == 'insert_product') {
                                    admin_priv('zc_project_manage');
                                    $id = $_POST['item_id'];
                                    ...
                                    if ($_REQUEST['act'] == 'update_product') {
                                        ...
                                        if ($_REQUEST['act'] == 'del_product') {
                                            ...
                                            if ($_REQUEST['act'] == 'progress') {
                                                admin_priv('zc_project_manage');
                                                $item_id = $_GET['id'];
                                                $smarty->assign('ur_here', $_LANG['zc_progress_manage']);
                                                $action_link = array(
                                                    'href' => 'zc_project.php?act=add_evolve&id=' . $item_id,
                                                    'text' => $_LANG['add_zc_progress']
                                                );
                                                ...
                                                if ($_REQUEST['act'] == 'add_evolve' || $_REQUEST['act'] == 'edit_evolve') {
                                                    ...
                                                    $action_link = array(
                                                        'href' => 'zc_project.php?act=progress&id=' . intval($_GET['id']),
                                                        'text' => $_LANG['zc_progress_manage']
                                                    );
                                                    $smarty->assign('action_link', $action_link);
                                                    ...
                                                    if ($_REQUEST['act'] == 'insert_evolve') {
                                                        ...
                                                        if (have_file_upload()) {
                                                            for ($i = 0; $i < count($_FILES); $i++) {
                                                                if ($_FILES['img_' . $i]) {
                                                                    $evolve_img[] = $image->upload_image($_FILES['img_' . $i],
                                                                        $dir_evolve);
                                                                }
                                                            }
                                                        }
