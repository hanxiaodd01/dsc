...
function get_admin_userlist($ru_id)
{
    ...
    foreach ($list as $key => $val) {
        ...
        $list[$key]['parent_name'] = get_table_date('admin_user', 'user_id=\'' . $val['parent_id'] . '\'',
            array('user_name'), 2);
    }
    ...
    if ($_REQUEST['act'] == 'signin') {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            if ($_REQUEST['act'] == 'query') {
                ...
                $smarty->assign('record_count', $admin_list['record_count']);
                $smarty->assign('page_count', $admin_list['page_count']);
                make_json_result($smarty->fetch('privilege_list.dwt'), '',
                    array('filter' => $admin_list['filter'], 'page_count' => $admin_list['page_count']));
            } else {
                if ($_REQUEST['act'] == 'add') {
                    admin_priv('admin_manage');
                    $smarty->assign('ur_here', $_LANG['admin_add']);
                    $smarty->assign('action_link',
                        array('href' => 'privilege.php?act=list', 'text' => $_LANG['01_admin_list']));
                    ...
                    if ($_REQUEST['act'] == 'insert') {
                        ...
                        if ($_POST['user_name'] == $_LANG['buyer'] || $_POST['user_name'] == $_LANG['seller_alt']) {
                            $link[] = array(
                                'text' => $_LANG['invalid_name_notic'],
                                'href' => 'privilege.php?act=modif'
                            );
                        ...
                        sys_msg($_LANG['add'] . '&nbsp;' . $_POST['user_name'] . '&nbsp;' . $_LANG['action_succeed'], 0,
                            $link);
                        ...
                        if ($_REQUEST['act'] == 'edit') {
                            ...
                            $smarty->assign('action_link',
                                array('text' => $_LANG['01_admin_list'], 'href' => 'privilege.php?act=list'));
                            ...
                            if ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self') {
                                ...
                                if ($_REQUEST['act'] == 'update') {
                                    ...
                                    $nav_list = !empty($_POST['nav_list']) ? ', nav_list = \'' . @join(',',
                                            $_POST['nav_list']) . '\'' : '';
                                ...
                                if ($ru_id && $GLOBALS['_CFG']['sms_seller_signin'] == '1') {
                                    ...
                                    if ($adminru['ru_id'] == 0 && $template['template_content'] != '') {
                                        ...
                                        if ($shopinfo['seller_email'] && ($admin_name != '' || $_POST['new_password'] != '')) {
                                            ...
                                            send_mail($admin_name, $shopinfo['seller_email'],
                                ...
                                $link[] = array(
                                    'text' => strpos($g_link,
                                        'list') ? $_LANG['back_admin_list'] : $_LANG['modif_info'],
                                    'href' => $g_link
                                );
                                sys_msg($msg . '<script>parent.document.getElementById(\'header-frame\').contentWindow.document.location.reload();</script>',
                                    0, $link);
                            } else {
                                if ($_REQUEST['act'] == 'modif') {
                                    ...
                                    $user_info = $db->getRow($sql);
                                    $nav_arr = trim($user_info['nav_list']) == '' ? array() : explode(',',
                                        $user_info['nav_list']);
                                    $nav_lst = array();
                                    ...
                                    if ($_REQUEST['act'] == 'allot') {
                                        ...
                                        $admin_id = get_admin_id();
                                        $action_list = get_table_date('admin_user', 'user_id=\'' . $admin_id . '\'',
                                            array('action_list'), 2);
                                        ...
                                        if ($priv_arr) {
                                            ...
                                            while ($priv = $db->FetchRow($result)) {
                                                if (!empty($action_list) && $action_list != 'all' && !in_array($priv['action_code'],
                                                        $action_array)) {
                                                    continue;
                                                }
                                        ...
                                        if ($_REQUEST['act'] == 'update_allot') {
                                            ...
                                            sys_msg($_LANG['edit'] . '&nbsp;' . $admin_name . '&nbsp;' . $_LANG['action_succeed'],
                                                0, $link);
                                        } else {
                                            if ($_REQUEST['act'] == 'remove') {
                                                ...
                                                $url = 'privilege.php?act=query&' . str_replace('act=remove', '',
