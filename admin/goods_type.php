...
function get_goodstype($ru_id)
{
    ...
    foreach ($all as $key => $val) {
        ...
        if (0 < $val['suppliers_id']) {
            $all[$key]['user_name'] = get_table_date('suppliers', 'suppliers_id=\'' . $val['suppliers_id'] . '\'',
...
if ($_REQUEST['act'] == 'manage') {
    ...
    if ($GLOBALS['_CFG']['attr_set_up'] == 0) {
        if ($adminru['ru_id'] == 0) {
            $smarty->assign('action_link',
                array('text' => $_LANG['new_goods_type'], 'href' => 'goods_type.php?act=add'));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'cat_list') {
            ...
            $smarty->assign('action_link',
                array('text' => $_LANG['type_cart_add'], 'href' => 'goods_type.php?act=cat_add'));
            $smarty->assign('action_link1',
                array('text' => $_LANG['type_cart'], 'href' => 'goods_type.php?act=cat_list'));
            $smarty->assign('action_link2',
                array('text' => $_LANG['08_goods_type'], 'href' => 'goods_type.php?act=manage'));
            $smarty->assign('act_type', $_REQUEST['act']);
            $smarty->assign('level', $level);
            self_seller(BASENAME($_SERVER['PHP_SELF']), 'cat_list');
            $smarty->display('goods_type_cat.dwt');
        } else {
            if ($_REQUEST['act'] == 'cat_list_query') {
                ...
                if ($_REQUEST['act'] == 'cat_add' || $_REQUEST['act'] == 'cat_edit') {
                    ...
                    if ($_REQUEST['act'] == 'cat_insert' || $_REQUEST['act'] == 'cat_update') {
                        ...
                        if ($_REQUEST['act'] == 'cat_insert') {
                            ...
                            $db->autoExecute($ecs->table('goods_type_cat'), $cat_info, 'UPDATE',
                                'cat_id = \'' . $cat_id . '\'');
                        ...
                        if ($_REQUEST['act'] == 'remove_cat') {
                            ...
                            $url = 'goods_type.php?act=cat_list_query&' . str_replace('act=remove_cat', '',
    ...
    if ($_REQUEST['act'] == 'edit_type_name') {
        ...
        if ($_REQUEST['act'] == 'toggle_enabled') {
            ...
            if ($_REQUEST['act'] == 'add') {
                ...
                $smarty->assign('action_link',
                    array('href' => 'goods_type.php?act=manage', 'text' => $_LANG['goods_type_list']));
                ...
                if ($_REQUEST['act'] == 'insert') {
                    ...
                    if ($_REQUEST['act'] == 'edit') {
                        ...
                        if ($_REQUEST['act'] == 'update') {
                            ...
                            if ($db->autoExecute($ecs->table('goods_type'), $goods_type, 'UPDATE',
                                    'cat_id=\'' . $cat_id . '\'') !== false) {
                            ...
                            if ($_REQUEST['act'] == 'remove') {
                                ...
                                if ($exc->drop($id)) {
                                    ...
                                    $url = 'goods_type.php?act=query&' . str_replace('act=remove', '',
                                            $_SERVER['QUERY_STRING']);
                                    ecs_header('Location: ' . $url . "\n");
                                    exit();
                                } else {
