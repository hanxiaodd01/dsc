    ...
    if ($type == 0) {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            ...
            if ($watermark) {
                ...
                $image = $GLOBALS['image']->make_thumb(ROOT_PATH . $row['original_img'],
                    $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height'], $dir);
                ...
                $image = $GLOBALS['image']->add_watermark(ROOT_PATH . $image, '', $GLOBALS['_CFG']['watermark'],
            ...
            if ($thumb) {
                ...
                $goods_thumb = $GLOBALS['image']->make_thumb(ROOT_PATH . $row['original_img'],
                    $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height'], $dir);
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            ...
            if ($watermark && file_exists(ROOT_PATH . $row['img_original'])) {
                ...
                $image = $GLOBALS['image']->add_watermark($dir . $file_name, '', $GLOBALS['_CFG']['watermark'],
                    $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']);
            ...
            if ($thumb) {
                ...
                $thumb_url = $GLOBALS['image']->make_thumb(ROOT_PATH . $row['img_original'],
                    $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height'], $dir);
    ...
    if (!empty($_GET['get_goods'])) {
        ...
        if (isset($_GET['start'])) {
            ...
            $result = array(
                ...
                'cat_id' => $cat_id,
                'row' => array(
                    'new_page' => sprintf($_LANG['page_format'], 1),
            ...
            if ($result['module_no'] == 0) {
                ...
                if ($result['page'] <= ceil($count / $result['page_size'])) {
                    ...
                    if ($proc_thumb) {
                        process_image_ex($result['page'], $result['page_size'], $result['module_no'], $result['thumb'],
                            $result['watermark'], $result['change'], $result['silent']);
                    } else {
                        process_image($result['page'], $result['page_size'], $result['module_no'], $result['thumb'],
                            $result['watermark'], $result['change'], $result['silent']);
                    }
                ...
                if (($result['module_no'] == 1) && ($result['do_album'] == 1)) {
                    ...
                    if ($result['page'] <= ceil($count / $result['page_size'])) {
                        ...
                        if ($proc_thumb) {
                            process_image_ex($result['page'], $result['page_size'], $result['module_no'],
