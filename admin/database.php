...
function sql_import($sql_file)
{
    ...
    if ('4.1' < $db_ver) {
        for ($i = 0; $i < $ret_count; $i++) {
            ...
            if (!empty($ret[$i])) {
                if ((strpos($ret[$i], 'CREATE TABLE') !== false) && (strpos($ret[$i],
                            'DEFAULT CHARSET=' . str_replace('-', '', EC_CHARSET)) === false)) {
        ...
        if (($mask & 1) < 1) {
            ...
            foreach ($real_list as $file) {
                ...
                $list[] = array(
                    ...
                    'add_time' => $info['date'],
                    'vol' => $info['vol'],
                    'file_size' => num_bitunit($file_size),
                    'mark' => $mark
                );
            }
        }
...
if ($_REQUEST['act'] == 'dumpsql') {
    ...
    if (empty($tables)) {
        if (1 < $vol) {
            if (!@file_put_contents(ROOT_PATH . DATA_DIR . '/sqldata/' . $sql_file_name . '_' . $vol . '.sql',
                $dump->dump_sql)) {
            ...
            for ($i = 1; $i <= $vol; $i++) {
                $list[] = array(
                    'name' => $sql_file_name . '_' . $i . '.sql',
                    'href' => '../' . DATA_DIR . '/sqldata/' . $sql_file_name . '_' . $i . '.sql'
                );
            }
            ...
            $smarty->assign('list', array(
                array(
                    'name' => $sql_file_name . '.sql',
                    'href' => '../' . DATA_DIR . '/sqldata/' . $sql_file_name . '.sql'
                )
            ));
            $smarty->assign('title', $_LANG['backup_success']);
            $smarty->display('sql_dump_msg.dwt');
    ...
    if (preg_match('/_[0-9]+\\.sql$/', $file_name)) {
        if ($is_confirm == false) {
            sys_msg($_LANG['confirm_import'], 1, array(
                array(
                    'text' => $_LANG['also_continue'],
                    'href' => 'database.php?act=import&confirm=1&file_name=' . $file_name
                )
            ), false);
        }
    ...
    if (empty($_GET['mysql_ver_confirm'])) {
        if (empty($sql_info['mysql_ver'])) {
            ...
            if ($sql_info['mysql_ver'] != $mysql_ver_arr) {
                $lnk = array();
                $lnk[] = array(
                    'text' => $_LANG['confirm_ver'],
                    'href' => 'database.php?act=upload_sql&mysql_ver_confirm=1'
                );
...
if ($_REQUEST['act'] == 'optimize') {
    ...
    while ($row = $db->fetchRow($ret)) {
        ...
        $list[] = array(
            ...
            'type' => $type,
            'rec_num' => $row['Rows'],
            'rec_size' => sprintf(' %.2f KB', $row['Data_length'] / 1024),
            'rec_index' => $row['Index_length'],
            'rec_chip' => $row['Data_free'],
            'status' => $res['Msg_text'],
            'charset' => $charset
        );
    }
