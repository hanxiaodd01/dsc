...
function get_affiliate_ck()
{
    ...
    while ($rt = $GLOBALS['db']->fetch_array($query)) {
        ...
        if (!empty($rt['suid'])) {
            $rt['info'] = sprintf($GLOBALS['_LANG']['separate_info2'], $rt['suid'], $rt['auser'], $rt['money'],
                $rt['point']);
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('affiliate_ck_list.dwt'), '',
        ...
        if ($_REQUEST['act'] == 'del') {
            ...
            if ($_REQUEST['act'] == 'rollback') {
                ...
                if (!empty($stat)) {
                    ...
                    log_account_change($stat['user_id'], 0 - $stat['money'], 0, 0 - $stat['point'], 0,
                        $_LANG['loginfo']['cancel']);
                ...
                $links[] = array(
                    ...
                    'href' => 'affiliate_ck.php?act=list&uselastfilter=1'
                );
                sys_msg($_LANG['edit_ok'], 0, $links);
            } else {
                        ...
                        if (empty($separate_by)) {
                            ...
                            for ($i = 0; $i < $num; $i++) {
                                ...
                                if (empty($up_uid) || empty($row['user_name'])) {
                                    ...
                                    write_affiliate_log($oid, $up_uid, $row['user_name'], $setmoney, $setpoint,
                                        $separate_by);
                                }
                            }
                        } else {
