...
function get_template_region($tmp_name, $tmp_file, $lib = true)
{
    ...
    if (empty($regions)) {
        $matches = array();
        $result = preg_match_all('/(<!--\\s*TemplateBeginEditable\\sname=")([^"]+)("\\s*-->)/', $content, $matches,
            PREG_SET_ORDER);
        if ($result && (0 < $result)) {
    ...
    foreach ($regions as $key => $val) {
        ...
        if (preg_match(sprintf($pattern, $val), $content, $matches)) {
            $lib_matches = array();
            $result = preg_match_all('/([\\s|\\S]{0,20})(<!--\\s#BeginLibraryItem\\s")([^"]+)("\\s-->)/', $matches[2],
                $lib_matches, PREG_SET_ORDER);
            $i = 0;
            if ($result && (0 < $result)) {
                foreach ($lib_matches as $k => $v) {
                    ...
                    $libs[] = array(
                        'library' => $v[3],
                        'region' => $val,
                        'lib' => basename(substr($v[3], 0, strpos($v[3], '.'))),
                        'sort_order' => $i
                    );
                    $i++;
                }
...
function move_plugin_library($tmp_name, &$msg)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($rec)) {
        if (!@rename($source_dir . $row['library'], $target_dir . $row['library'])) {
            if (!@copy(ROOT_PATH . 'plugins/' . $row['code'] . '/temp' . $row['library'],
                $target_dir . $row['library'])) {
                $return_value = false;
