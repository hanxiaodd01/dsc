...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $single_id = 0, $files_type = 0)
{
    ...
    foreach ($image_descs as $key => $img_desc) {
        ...
        if ($flag) {
            ...
            if ($proc_thumb) {
                $thumb_url = $GLOBALS['image']->make_thumb(array('img' => $img_original, 'type' => 1),
            ...
            if ($proc_thumb && 0 < gd_version()) {
                ...
                $newname = dirname($img_original) . '/' . $GLOBALS['image']->random_filename() . substr(basename($img_original),
                        $pos);
                copy($img_original, $newname);
                $img_url = $newname;
                $GLOBALS['image']->add_watermark($img_url, '', $GLOBALS['_CFG']['watermark'],
                    $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']);
            }
            ...
            if (!empty($image_urls[$key]) && $image_urls[$key] != $GLOBALS['_LANG']['img_file'] && $image_urls[$key] != 'http://' && (strpos($image_urls[$key],
                        'http://') !== false || strpos($image_urls[$key], 'https://') !== false)) {
                if (get_http_basename($image_urls[$key], $admin_temp_dir)) {
                    ...
                    $img_wh = $GLOBALS['image']->get_width_to_height($down_img, $GLOBALS['_CFG']['image_width'],
                    ...
                    $goods_img = $GLOBALS['image']->make_thumb(array('img' => $down_img, 'type' => 1),
...
function handle_gallery_image_add(
    $goods_id,
    $image_files,
    $image_descs,
    $image_urls,
...
function get_product_info($product_id, $filed = '', $goods_model = 0, $is_attr = 0)
{
    ...
    if ($is_attr == 1) {
        if ($return_array['goods_attr']) {
            ...
            $return_array['goods_attr'] = get_product_attr_list($goods_attr_id, $return_array['goods_id'], $goods_model,
                $return_array['warehouse_id'], $return_array['area_id']);
        }
    }
...
function check_product_sn_exist(
    $product_sn,
    $product_id = 0,
    $ru_id = 0,
    $goods_model = 0,
...
function check_product_bar_code_exist(
    $product_bar_code,
    $product_id = 0,
    $ru_id = 0,
    $goods_model = 0,
...
function reformat_image_name($type, $goods_id, $source_img, $position = '')
{
    ...
    if ($position == 'source') {
        if (move_image_file(ROOT_PATH . $source_img,
            ROOT_PATH . $dir . '/' . $sub_dir . '/source_img/' . $img_name . $img_ext)) {
            return $dir . '/' . $sub_dir . '/source_img/' . $img_name . $img_ext;
        }
    } else {
        if ($position == 'thumb') {
            if (move_image_file(ROOT_PATH . $source_img,
                ROOT_PATH . $dir . '/' . $sub_dir . '/thumb_img/' . $img_name . $img_ext)) {
                return $dir . '/' . $sub_dir . '/thumb_img/' . $img_name . $img_ext;
            }
        } else {
            if (move_image_file(ROOT_PATH . $source_img,
                ROOT_PATH . $dir . '/' . $sub_dir . '/goods_img/' . $img_name . $img_ext)) {
