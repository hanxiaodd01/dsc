    ...
    if ($GLOBALS['db']->getOne($sql) || ($parent_id == 0)) {
        ...
        foreach ($res as $row) {
            ...
            $sql = 'SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ' . 'FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = b.brand_id AND g.user_id = 0 AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods_cat') . ' AS gc ON g.goods_id = gc.goods_id ' . ' WHERE ' . $children . ' OR ' . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id),
                    $cat_keys))) . ' AND b.is_show = 1 ' . 'GROUP BY b.brand_id HAVING goods_num > 0 ' . $where_having . ' ORDER BY b.sort_order, b.brand_id ASC';
            ...
            foreach ($brands_list as $key => $val) {
                ...
                $brands_list[$temp_key]['url'] = build_uri('category', array(
                    'cid' => $cat_id,
                    'bid' => $val['brand_id'],
                    'price_min' => $price_min,
                    'price_max' => $price_max,
        ...
        if ($res) {
            foreach ($res as $row) {
                ...
                if ($getrid == 0) {
                    ...
                    if ($table == 'merchants_category') {
                        $three_arr[$row['cat_id']]['url'] = build_uri('merchants_store',
                            array('cid' => $row['cat_id'], 'urid' => $row['user_id']), $row['cat_name']);
                    } else {
...
function get_default_collection_goods($user_id)
{
    ...
    setcookie('province', $province_id, gmtime() + (3600 * 24 * 30), $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
    setcookie('city', $city_id, gmtime() + (3600 * 24 * 30), $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);
    setcookie('district', $district_id, gmtime() + (3600 * 24 * 30), $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
...
function get_store_category_leve_list($cat_list = array(), $ru_id = 0, $is_url = 0)
{
    if ($cat_list) {
        foreach ($cat_list as $key => $row) {
            $cat_list[$key]['child_tree'] = get_category_store_child_list($row['cat_id'], $row['level'], $ru_id,
                $is_url);
        }
    }
