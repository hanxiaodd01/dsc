...
function get_bonus_type()
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        $bonus[$row['type_id']] = $row['type_name'] . ' [' . sprintf($GLOBALS['_CFG']['currency_format'],
                $row['type_money']) . ']';
    }
...
function get_pay_card_type($id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        $bonus['name'] = $row['type_name'] . ' [' . sprintf($GLOBALS['_CFG']['currency_format'],
                $row['type_money']) . ']';
    }
...
function area_list($region_id = 0, $type = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($type == 1) {
            $rw_id = get_table_date('region_warehouse', 'regionId = \'' . $row['region_id'] . '\'', array('region_id'),
                2);
...
function get_filter($param_str = '')
{
    ...
    if (isset($_GET['uselastfilter']) && isset($_COOKIE['ECSCP']['lastfilterfile']) && $_COOKIE['ECSCP']['lastfilterfile'] == sprintf('%X',
            crc32($filterfile))) {
        return array(
        ...
        for ($i = 0; $i < count($attr_values); $i++) {
            ...
            if (!empty($upFile)) {
                if (0 < $res['id']) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('attribute_img'), $other, 'UPDATE',
                        'attr_id = \'' . $attr_id . '\' and attr_values = \'' . $attr_values[$i]['values'] . '\'');
                } else {
        ...
        if (!empty($cat_name)) {
            ...
            if ($GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('category'), $cat) !== false) {
                ...
                if ($cat['show_in_nav'] == 1) {
                    ...
                    $vieworder += 2;
                    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('nav') . ' (name,ctype,cid,ifshow,vieworder,opennew,url,type)' . ' VALUES(\'' . $cat['cat_name'] . ('\', \'c\', \'' . $cat_id . '\',\'1\',\'' . $vieworder . '\',\'0\', \'') . build_uri('category',
                            array('cid' => $cat_id), $cat['cat_name']) . '\',\'middle\')';
                    $GLOBALS['db']->query($sql);
                }
...
function return_order_list()
{
    ...
    foreach ($row as $key => $value) {
        ...
        if ($return_number == $all_goods_number) {
            ...
            $row[$key]['discount_amount'] = number_format($value['should_return'] * $row[$key]['discount_percent_decimal'],
                2, '.', '');
        }
        ...
        if ($is_ajax == 1) {
            if ($filter['order_id_list']) {
                ...
                foreach ($order_arr as $key => $val) {
                    order_action($val['order_sn'], $val['order_status'], SS_RECEIVED, $val['pay_status'], '',
                        $_SESSION['admin_name'], 0, $val['confirm_take_time']);
                }
            }
...
function set_current_page()
{
    ...
    if ($app != 'index.php') {
        ...
        if (isset($output['act']) && (preg_match('/insert/', $output['act']) || preg_match('/update/',
                    $output['act']))) {
            $act_status = 0;
        }
...
function get_product_info_by_attr(
    $goods_id = 0,
    $attr_arr = array(),
    $goods_model = 0,
    $region_id = 0,
