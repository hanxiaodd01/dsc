...
function bill_detail_list()
{
    ...
    foreach ($row as $key => $value) {
        ...
        $row[$key]['format_return_amount'] = price_format($value['return_amount'] + $value['return_shippingfee'],
            false);
...
function commission_bill_list($ajax_bill = 0)
{
    ...
    foreach ($row as $key => $value) {
        $row[$key]['format_settleaccounts_time'] = local_date($GLOBALS['_CFG']['time_format'],
            $value['settleaccounts_time']);
        ...
        if (empty($value['chargeoff_status'])) {
            $detail = get_bill_amount_detail($value['id'], $value['seller_id'], $value['proportion'],
                $value['start_time'], $value['end_time'], $value['chargeoff_status'], $value['commission_model']);
            if ($detail && ($detail['notake_order_count'] == 0) && (1 <= $detail['take_order_count'])) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_commission_bill'), $other, 'UPDATE',
                    'id = \'' . $value['id'] . '\'');
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_bill_order'), $bill_order_other, 'UPDATE',
                    '1 ' . $where_bill_order);
...
function get_bill_amount_detail(
    $bill_id = 0,
    $seller_id = 0,
    $proportion = 100,
    $start_time = 0,
    ...
    $where_wait_order .= db_create_in(array(OS_CONFIRMED, OS_SPLITED),
            'oa.order_status') . ' AND oa.shipping_status ' . db_create_in(array(SS_SHIPPED)) . ' AND oa.pay_status ' . db_create_in(array(PS_PAYED));
    $where_wait_order .= ', ' . db_create_in(array(OS_CONFIRMED, OS_SPLITED),
            'o.order_status') . ' AND ' . db_create_in(array(SS_RECEIVED), 'o.shipping_status',
            'NOT') . ' AND IF(o.shipping_status = 2, ' . db_create_in(array(PS_PAYED), 'o.pay_status',
            'NOT') . ', ' . db_create_in(array(PS_PAYED),
            'o.pay_status') . ')' . ' AND ' . db_create_in(array(OS_RETURNED), 'o.order_status', 'NOT') . ')';
...
function bill_notake_order_list()
{
    ...
    foreach ($row as $key => $value) {
        ...
        $value['drp_money'] = $GLOBALS['db']->getOne('SELECT SUM(drp_money) AS drp_money FROM ' . $GLOBALS['ecs']->table('order_goods') . ' WHERE order_id = \'' . $order_id . '\'',
            true);
        $row[$key]['format_drp_money'] = price_format($value['drp_money'], false);
        $row[$key]['format_integral_money'] = price_format($value['integral_money'], false);
...
function merchants_is_settlement($ru_id = 0, $state = '')
{
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        $row[$i]['formated_brokerage_amount'] = price_format(($row[$i]['total_fee'] - $row[$i]['return_amount']) * $percent_value,
            true);
        ...
        if (file_exists(MOBILE_DRP)) {
            ...
            $row[$i]['formated_brokerage_amount'] = price_format(($brokerage_amount['total_fee'] - $row[$i]['return_amount']) * $percent_value,
                true);
            $row[$i]['formated_effective_amount'] = price_format($brokerage_amount['total_fee'] - $row[$i]['return_amount'],
                true);
            ...
            $row[$i]['formated_brokerage_amount'] = price_format(($row[$i]['total_fee'] - $row[$i]['return_amount']) * $percent_value,
                true);
            $row[$i]['formated_effective_amount'] = price_format($row[$i]['total_fee'] - $row[$i]['return_amount'],
                true);
