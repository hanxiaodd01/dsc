...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order) {
        $value_card = $GLOBALS['db']->getOne('SELECT use_val FROM ' . $GLOBALS['ecs']->table('value_card_record') . ' WHERE order_id = \'' . $order_id . '\'',
            true);
    ...
    foreach ($arr as $key => $value) {
        ...
        if (1 < count($currency_format)) {
            $goods_price = trim(get_final_price($value['goods_id'], $value['goods_number'], true, $attr_id,
                $value['warehouse_id'], $value['area_id'], 0, 0, $add_tocart), $currency_format[0]);
            ...
            $goods_price = get_final_price($value['goods_id'], $value['goods_number'], true, $attr_id,
                $value['warehouse_id'], $value['area_id'], 0, 0, $add_tocart);
        ...
        if ($value['extension_code'] == 'package_buy') {
            ...
            if (($value['extension_code'] == 'presale') || (1 < $value['rec_type'])) {
                ...
                if (($ru_type == 1) && (0 < $warehouse_id) && ($store_id == 0)) {
                    ...
                    $products = get_warehouse_id_attr_number($value['goods_id'], $value['goods_attr_id'],
                        $goodsInfo['user_id'], $warehouse_id, $area_id);
            ...
            if (defined('THEME_EXTENSION')) {
                $arr[$key]['goods_attr_text'] = get_goods_attr_info($value['goods_attr_id'], 'pice',
                    $value['warehouse_id'], $value['area_id'], 1);
            }
    ...
    if (0 < $num) {
        ...
        if ($row) {
            ...
            if (($GLOBALS['_CFG']['use_storage'] == 0) || ($num <= $goods_storage)) {
                $goods_price = get_final_price($goods_id, $num, true, $spec, $warehouse_id, $area_id, 0, 0,
                    $add_tocart);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (1 < count($currency_format)) {
            $goods_price = trim(get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id,
                $row['warehouse_id'], $row['area_id'], 0, 0, $add_tocart), $currency_format[0]);
    ...
    foreach ($shipping_list as $key => $val) {
        if (substr($val['shipping_code'], 0, 5) != 'ship_') {
            if ($GLOBALS['_CFG']['freight_model'] == 0) {
                        ...
                        if (!empty($cart_goods[0]['freight']) && ($cart_goods[0]['is_shipping'] == 0)) {
                            if ($cart_goods[0]['freight'] == 1) {
                                ...
                                if ($trow['freight_type']) {
                                    ...
                                    $goods_transport = $GLOBALS['ecs']->get_select_find_in_set(2, $consignee['city'],
                                        $transport, $transport_where, 'goods_transport_extend', 'area_id');
                ...
                $shipping_fee = (($shipping_count == 0) && ($cart_weight_price['free_shipping'] == 1) ? 0 : shipping_fee($val['shipping_code'],
                    $configure, $cart_weight_price['weight'], $cart_weight_price['amount'],
            ...
            while ($row_goods = $GLOBALS['db']->fetchRow($res_goods)) {
                ...
                if ($is_dec) {
                    change_goods_storage($row_goods['goods_id'], $row['product_id'],
                        0 - ($row['num'] * $row_goods['goods_number']), $row['warehouse_id'], $row['area_id'],
                        $order_id, $use_storage, $admin_id);
                } else {
                    if ($is_goods['is_real']) {
                        change_goods_storage($row_goods['goods_id'], $row['product_id'],
    ...
    foreach ($merchant_goods as $key => $row) {
        ...
        foreach ($user_cart_goods as $key1 => $row1) {
                    ...
                    if ($row1['is_gift'] == 0) {
                        ...
                        switch ($row2['act_type']) {
                            ...
                            case 1:
                                ...
                                $merchant_goods[$key]['new_list'][$row2['act_id']]['act_type_ext_format'] = number_format($row2['act_type_ext'],
                                    2);
                                break;
                        ...
                        $merchant_goods[$key]['new_list'][$row2['act_id']]['favourable_used'] = favourable_used($row2,
                            $cart_favourable);
            ...
            if (isset($sort_favourable['by_category']) && ($row1['extension_code'] != 'package_buy')) {
                ...
                if ((in_array($cat_id, $id_list) && ($row1['is_gift'] == 0)) || in_array($row1['is_gift'],
                        $favourable_id_list)) {
                    foreach ($sort_favourable['by_category'] as $key2 => $row2) {
                        $fav_act_range_ext = (!empty($row2['act_range_ext']) ? explode(',',
                            $row2['act_range_ext']) : array());
            ...
            if (isset($sort_favourable['by_brand']) && ($row1['extension_code'] != 'package_buy')) {
                ...
                if ((in_array(trim($brand_id),
                            $get_act_range_ext) && ($row1['is_gift'] == 0)) || in_array($row1['is_gift'],
                        $favourable_id_list)) {
            ...
            if (isset($sort_favourable['by_goods']) && ($row1['extension_code'] != 'package_buy')) {
                ...
                if (in_array($row1['goods_id'], $get_act_range_ext) || in_array($row1['is_gift'],
                        $favourable_id_list)) {
