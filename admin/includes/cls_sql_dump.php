    ...
    public function get_table_data($table, $pos)
    {
        ...
        for ($i = 0; $i < $cycle_time; $i++) {
            ...
            for ($j = 0; $j < $data_count; $j++) {
                ...
                $tmp_dump_sql = empty($tmp_str_pos) ? $tmp_dump_sql : substr($tmp_dump_sql, 0,
                        $tmp_str_pos - 1) . 'NULL' . substr($tmp_dump_sql, $tmp_str_pos + 5);
        ...
        foreach ($arr as $val) {
            ...
            if (0 < $pos) {
                ...
                if ($type == 'DATE') {
                    ...
                    if ($type == 'MYSQL SERVER VERSION') {
                        $sql_info['mysql_ver'] = $value;
                    } else {
                        if ($type == 'PHP VERSION') {
                            $sql_info['php_ver'] = $value;
                        } else {
                            if ($type == 'ECShop VERSION') {
