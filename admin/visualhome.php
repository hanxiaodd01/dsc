            ...
            if ($_REQUEST['act'] == 'file_put_visual') {
                ...
                if ($_REQUEST['act'] == 'edit_information') {
                    ...
                    if (isset($_FILES['ten_file']['error']) && $_FILES['ten_file']['error'] == 0 || !isset($_FILES['ten_file']['error']) && isset($_FILES['ten_file']['tmp_name']) && $_FILES['ten_file']['tmp_name'] != 'none') {
                        if (!check_file_type($_FILES['ten_file']['tmp_name'], $_FILES['ten_file']['name'],
                            $allow_file_types)) {
                            $result['error'] = 1;
                    ...
                    if (isset($_FILES['big_file']['error']) && $_FILES['big_file']['error'] == 0 || !isset($_FILES['big_file']['error']) && isset($_FILES['big_file']['tmp_name']) && $_FILES['big_file']['tmp_name'] != 'none') {
                        if (!check_file_type($_FILES['big_file']['tmp_name'], $_FILES['big_file']['name'],
                            $allow_file_types)) {
                            $result['error'] = 1;
                            $result['message'] = $_LANG['image_type_error'];
                            exit(json_encode($result));
                    ...
                    $html = write_static_file_cache('tpl_info', iconv('UTF-8', 'GB2312', $html), 'txt',
                        $file_dir . '/');
                                            ...
                                            if ($_REQUEST['act'] == 'bonusAdv') {
                                                ...
                                                if ($_FILES['advfile']) {
                                                    if (isset($_FILES['advfile']['error']) && $_FILES['advfile']['error'] == 0 || !isset($_FILES['advfile']['error']) && $_FILES['advfile']['tmp_name'] != 'none') {
                                                        if (!check_file_type($_FILES['advfile']['tmp_name'],
                                                            $_FILES['advfile']['name'], $allow_file_types)) {
                                                            $result['error'] = 1;
                                                            ...
                                                            if (move_upload_file($_FILES['advfile']['tmp_name'],
                                                                $file_name)) {
                                                ...
                                                if ($_REQUEST['act'] == 'delete_adv') {
                                                    ...
                                                    if ($_REQUEST['act'] == 'update_home_temp') {
                                                        require ROOT_PATH . '/includes/cls_json.php';
                                                        $json = new JSON();
                                                        $result = array('error' => '', 'message' => '');
