...
if ($_REQUEST['act'] == 'detail') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['06_undispose_booking'], 'href' => 'goods_booking.php?act=list_all'));
...
if ($_REQUEST['act'] == 'update') {
    ...
    if (!empty($_POST['send_email_notice']) || isset($_POST['remail'])) {
        ...
        if (send_mail($booking_info['link_man'], $booking_info['email'], $template['template_subject'], $content,
            $template['is_html'])) {
            $send_ok = 0;
        } else {
            $send_ok = 1;
