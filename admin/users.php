...
function user_update($user_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $args, 'update',
        'user_id=\'' . $user_id . '\'');
}
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link2',
        array('text' => $_LANG['12_users_export'], 'href' => 'javascript:download_userlist();'));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('users_list.dwt'), '',
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            if ($_REQUEST['act'] == 'insert') {
                    ...
                    if ($users->error == ERR_INVALID_USERNAME) {
                        ...
                        if ($users->error == ERR_USERNAME_NOT_ALLOW) {
                            ...
                            if ($users->error == ERR_USERNAME_EXISTS) {
                                ...
                                if ($users->error == ERR_INVALID_EMAIL) {
                                    $msg = $_LANG['email_invalid'];
                                } else {
                                    if ($users->error == ERR_EMAIL_NOT_ALLOW) {
                ...
                if (!empty($GLOBALS['_CFG']['register_points'])) {
                    log_account_change($_SESSION['user_id'], 0, 0, $GLOBALS['_CFG']['register_points'],
                        $GLOBALS['_CFG']['register_points'], $_LANG['register_points']);
                }
                ...
                foreach ($fields_arr as $val) {
                    ...
                    if (!empty($_POST[$extend_field_index])) {
                        $temp_field_content = 100 < strlen($_POST[$extend_field_index]) ? mb_substr($_POST[$extend_field_index],
                ...
                if ($_REQUEST['act'] == 'edit') {
                    ...
                    $smarty->assign('full_page', 1);
                    $smarty->assign('action_link2',
                        array('text' => $_LANG['03_users_list'], 'href' => 'users.php?act=list'));
                    $select_date = array();
                    $select_date['year'] = range(1956, date(Y));
                    $select_date['month'] = range(1, 12);
                    $select_date['day'] = range(1, 31);
        ...
        if ($_REQUEST['act'] == 'main_user') {
            ...
            if ($_REQUEST['act'] == 'remove') {
                ...
                if ($_REQUEST['act'] == 'address_list') {
                    ...
                    if (0 < $id) {
                        $smarty->assign('action_link2',
                            array('text' => $_LANG['address_list'], 'href' => 'users.php?act=list'));
                    }
                    ...
                    if ($_REQUEST['act'] == 'remove_parent') {
                        ...
                        if ($_REQUEST['act'] == 'aff_list') {
                            ...
                            for ($i = 0; $i < $temp_count; $i++) {
                                $user_list['user_list'][$i]['reg_time'] = local_date($_CFG['date_format'],
                            ...
                            $smarty->assign('action_link',
                                array('text' => $_LANG['back_note'], 'href' => 'users.php?act=edit&id=' . $auid));
                            assign_query_info();
                            ...
                            if ($_REQUEST['act'] == 'export') {
                                ...
                                if ($_REQUEST['act'] == 'users_log') {
                                    ...
                                    if ($_REQUEST['act'] == 'users_log_query') {
                                        $user_id = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
                                        $user_log = get_user_log();
                                        $smarty->assign('user_log', $user_log['list']);
                                        $smarty->assign('filter', $user_log['filter']);
                                        ...
                                        $smarty->assign('page_count', $user_log['page_count']);
                                        $smarty->assign('user_id', $user_id);
                                        make_json_result($smarty->fetch('users_log.dwt'), '', array(
                                            'filter' => $user_log['filter'],
                                            'page_count' => $user_log['page_count']
                                        ));
                                    } else {
                                        if ($_REQUEST['act'] == 'batch_log') {
                                            ...
                                            if (empty($_POST['checkboxes'])) {
                                                ...
                                                $link[] = array(
                                                    'text' => $_LANG['back'],
                                                    'href' => 'users.php?act=users_log&id=' . $user_id
                                                );
