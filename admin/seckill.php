...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link2',
        array('text' => $_LANG['seckill_time_bucket'], 'href' => 'seckill.php?act=time_bucket'));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'tb_query') {
            ...
            if ($_REQUEST['act'] == 'sg_query') {
                ...
                $smarty->assign('record_count', $list['record_count']);
                $smarty->assign('page_count', $list['page_count']);
                make_json_result($smarty->fetch('seckill_set_goods_info.dwt'), '',
                    array('filter' => $list['filter'], 'page_count' => $list['page_count']));
            } else {
                if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
                    ...
                    $smarty->assign('action_link',
                        array('href' => 'seckill.php?act=list', 'text' => $_LANG['seckill_list']));
                    ...
                    if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                        ...
                        if ($_REQUEST['act'] == 'insert') {
                            ...
                            if ($db->query($sql)) {
                                ...
                                sys_msg($_LANG['add'] . '&nbsp;' . $_POST['acti_title'] . '&nbsp;' . $_LANG['attradd_succed'],
                                    0, $link);
                            } else {
                                sys_msg($_LANG['add'] . '&nbsp;' . $_POST['acti_title'] . '&nbsp;' . $_LANG['attradd_failed'],
                                    1);
                            }
                        } else {
                            $is_only = $exc->is_only('acti_title', $_POST['acti_title'], 0,
                                'sec_id != \'' . $sec_id . '\'');
                            ...
                            $link[0]['text'] = $_LANG['back_list'];
                            $link[0]['href'] = 'seckill.php?act=list';
                            sys_msg($_LANG['edit'] . '&nbsp;' . $_POST['acti_title'] . '&nbsp;' . $_LANG['attradd_succed'],
                                0, $link);
                        }
                    } else {
                        if ($_REQUEST['act'] == 'time_add' || $_REQUEST['act'] == 'time_edit') {
                            ...
                            $smarty->assign('action_link', array(
                                'href' => 'seckill.php?act=time_bucket',
                                'text' => $_LANG['seckill_time_bucket']
                            ));
                            $smarty->assign('cfg_lang', $_CFG['lang']);
                            ...
                            if ($_REQUEST['act'] == 'time_insert' || $_REQUEST['act'] == 'time_update') {
                                ...
                                if ($_REQUEST['act'] == 'time_insert') {
                                    ...
                                    if ($db->query($sql)) {
                                        ...
                                        sys_msg($_LANG['add'] . '&nbsp;' . $title . '&nbsp;' . $_LANG['attradd_succed'],
                                            0, $link);
                                    } else {
                                        sys_msg($_LANG['add'] . '&nbsp;' . $title . '&nbsp;' . $_LANG['attradd_failed'],
                                            1);
                                    }
                                } else {
                                    ...
                                    $link[0]['text'] = $_LANG['back_list'];
                                    $link[0]['href'] = 'seckill.php?act=time_bucket';
                                    sys_msg($_LANG['edit'] . '&nbsp;' . $title . '&nbsp;' . $_LANG['attradd_succed'], 0,
                                        $link);
                                }
                            } else {
                                if ($_REQUEST['act'] == 'toggle_putaway') {
...
if ($_REQUEST['act'] == 'add_goods') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['set_seckill_goods'], 'href' => 'seckill.php?act=set_goods&sec_id=' . $sec_id));
    $smarty->assign('sec_id', $sec_id);
    $smarty->assign('tb_id', $tb_id);
    $smarty->assign('full_page', 1);
    $smarty->display('seckill_set_goods_info.dwt');
} else {
    if ($_REQUEST['act'] == 'sg_remove') {
        $id = intval($_REQUEST['id']);
        ...
        $url = 'seckill.php?act=sg_query&sec_id=' . $sec_id . '&tb_id=' . $tb_id . str_replace('act=sg_remove', '',
                ...
                if (isset($_POST['type'])) {
                    if ($_POST['type'] == 'batch_remove') {
                        ...
                        $links[] = array(
                            ...
                            'href' => 'seckill.php?act=list&' . list_link_postfix()
                        );
                        sys_msg($_LANG['batch_drop_ok'], 0, $links);
                    } else {
                        if ($_POST['type'] == 'review_to') {
                            ...
                            if ($db->query($sql)) {
                                $lnk[] = array(
                                    'text' => $_LANG['back_list'],
                                    'href' => 'seckill.php?act=list&seller_list=1&' . list_link_postfix()
                                );
                ...
                if ($_REQUEST['act'] == 'tb_remove') {
                    ...
                    if ($_REQUEST['act'] == 'goods_info') {
                        ...
                        if ($_REQUEST['act'] == 'changedgoods') {
                            ...
                            if ($_REQUEST['act'] == 'edit_sec_price') {
                                ...
                                if ($_REQUEST['act'] == 'edit_sec_num') {
                                    ...
                                    if ($_REQUEST['act'] == 'edit_sec_limit') {
                                        check_authz_json('seckill_manage');
                                        $id = intval($_POST['id']);
