...
if ($_REQUEST['act'] == 'warehouse_list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('goods_area_attr_list.htm'), '',
        ...
        if ($_REQUEST['act'] == 'product_list') {
            ...
            $region_name = get_table_date('region_warehouse', 'region_id = \'' . $area_id . '\'', array('region_name'),
                2);
            ...
            if ($_REQUEST['act'] == 'product_query') {
                ...
                $smarty->assign('goods_name', sprintf($_LANG['products_title'], $goods['goods_name']));
                $smarty->assign('goods_sn', sprintf($_LANG['products_title_2'], $goods['goods_sn']));
                $region_name = get_table_date('region_warehouse', 'region_id = \'' . $area_id . '\'',
                    array('region_name'), 2);
                $smarty->assign('region_name', $region_name);
                    ...
                    if ($_REQUEST['act'] == 'edit_bar_code') {
                        ...
                        if ($_REQUEST['act'] == 'edit_product_number') {
                            ...
                            if ($_REQUEST['act'] == 'product_remove') {
                                ...
                                if ($result) {
                                    $url = 'goods_warehouse_attr.php?act=product_query&warehouse_id=' . $_SESSION['product_area'] . '&' . str_replace('act=product_remove',
                                ...
                                if ($_REQUEST['act'] == 'product_add_execute') {
                                    ...
                                    if ($insert) {
                                        ...
                                        $link[] = array(
                                            'href' => 'goods_area_attr.php?act=product_list&goods_id=' . $product['goods_id'] . '&area_id=' . $product['area_id'],
