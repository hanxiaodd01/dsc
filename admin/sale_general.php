    ...
    if ($type != 0) {
        for ($i = 0; $i < count($data_list); $i++) {
            ...
            if ($filter['time_type'] == 1) {
                ...
                $data_list[$i]['shipping_time'] = local_date($GLOBALS['_CFG']['time_format'],
                    $data_list[$i]['shipping_time']);
            }
        }
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('library/sale_general.dwt'), '',
            array('filter' => $data['filter'], 'page_count' => $data['page_count']));
    } else {
        if ($_REQUEST['act'] == 'download') {
            $data = get_data_list(1);
            $data_list = $data['data_list'];
            $filename = str_replace(' ', '-', local_date($GLOBALS['_CFG']['time_format'], gmtime())) . '_' . rand(0,
                    1000);
