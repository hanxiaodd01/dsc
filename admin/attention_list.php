        ...
        if ($_REQUEST['act'] == 'addtolist') {
            ...
            if ($start < $count) {
                ...
                while ($rt = $db->fetch_array($query)) {
                    ...
                    $smarty->assign(array(
                        ...
                        'goods_url' => $preg_replace,
                        'shop_name' => $_CFG['shop_title'],
                        'send_date' => local_date($_CFG['time_format'], gmtime())
                    ));
                ...
                $links[] = array(
                    ...
                    'href' => 'attention_list.php?act=addtolist&id=' . $id . '&pri=' . $pri . '&start=' . $start
                );
            ...
            if ($_REQUEST['act'] == 'batch_addtolist') {
                ...
                if ($start < $count) {
                    ...
                    while ($rt = $db->fetch_array($query)) {
                        $time = time();
                        $preg_replace = $ecs->url() . build_uri('goods', array('gid' => $rt['goods_id']),
                                $rt['user_name']);
                        $smarty->assign(array(
                            'user_name' => $rt['user_name'],
                            'goods_name' => $rt['goods_name'],
                    ...
                    $links[] = array(
                        'text' => sprintf($_LANG['finish_list'], $start),
                        'href' => 'attention_list.php?act=batch_addtolist&date=' . $olddate . '&pri=' . $pri . '&start=' . $start
                    );
                    sys_msg($_LANG['finishing'], 0, $links);
                } else {
                    $links[] = array('text' => $_LANG['02_attention_list'], 'href' => 'attention_list.php?act=list');
