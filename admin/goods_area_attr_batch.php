        ...
        if ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
                ...
                if ($GLOBALS['db']->getOne($sql, true)) {
                    ...
                    $db->autoExecute($ecs->table('warehouse_area_attr'), $other, 'UPDATE',
                        'goods_id = \'' . $result['list']['goods_id'] . '\' AND area_id = \'' . $result['list']['area_id'] . '\' AND goods_attr_id = \'' . $result['list']['goods_attr_id'] . ('\' ' . $where));
