...
function get_psi_log()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (!empty($rows['product_id'])) {
            $product_info = get_table_date($table_products, 'product_id=\'' . $rows['product_id'] . '\'',
                array('product_number', 'goods_attr'));

            if (!empty($product_info)) {
                $rows['total_number'] = $product_info['product_number'];
                $goods_attr = explode('|', $product_info['goods_attr']);
                $sql = ' SELECT attr_value FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' WHERE goods_attr_id IN (' . implode(',',
                        $goods_attr) . ') ';
        ...
        $rows['warehouse_name'] = get_table_date('region_warehouse', 'region_id=\'' . $rows['warehouse_id'] . '\'',
            array('region_name'), 2);
        $rows['area_name'] = get_table_date('region_warehouse', 'region_id=\'' . $rows['area_id'] . '\'',
            array('region_name'), 2);
        $rows['suppliers_name'] = get_table_date('suppliers', 'suppliers_id=\'' . $rows['suppliers_id'] . '\'',
...
function get_select_purchase_goods($goods_ids = '')
{
    ...
    foreach ($row as $key => $val) {
        ...
        $row[$key]['product_list'] = get_goods_product_list($val['goods_id'], $goods['model_attr'], $region_id,
            $area_id, false);
...
if ($_REQUEST['act'] == 'purchase_operate') {
    ...
    if ($_REQUEST['act'] == 'inventory') {
        ...
        if ($_REQUEST['act'] == 'inventory_query') {
            ...
            if ($_REQUEST['act'] == 'add_sku') {
                ...
                $goods_info = get_table_date('goods', 'goods_id=\'' . $goods_id . '\'',
                    array('goods_name', 'goods_type', 'goods_number', 'bar_code', 'warn_number'));
                ...
                if (!empty($goods['model_attr'])) {
                    $region_info = get_table_date($goods_table,
                        'goods_id=\'' . $goods_id . '\' AND region_id=\'' . $region_id . '\'', array('region_number'));
                ...
                if ($_REQUEST['act'] == 'sku_query') {
                    ...
                    if (!empty($goods['model_attr'])) {
                        $region_info = get_table_date($goods_table,
                            'goods_id=\'' . $goods_id . '\' AND region_id=\'' . $region_id . '\'',
                            array('region_number'));
                        $goods_info['goods_number'] = $region_info['region_number'];
                    }
                    ...
                    $smarty->assign('type', $type);
                    make_json_result($smarty->fetch('library/' . $lbi . '.lbi'), '', array(
                        'pb_filter' => $product_list['filter'],
                        'pb_page_count' => $product_list['page_count'],
                        'class' => 'attrlistDiv'
                    ));
                } else {
                    if ($_REQUEST['act'] == 'save_inventory') {
                        check_authz_json('goods_psi');
                        $json = new JSON();
                        $result = array('error' => 0, 'message' => '', 'content' => '');
                        ...
                        if (0 < $goods_id) {
                            ...
                            if (!empty($goods_type)) {
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_inventory_logs'), $this_log,
                                    'INSERT');
                            }
                        }
                        ...
                        if ($_REQUEST['act'] == 'edit_warn_number') {
                            check_authz_json('goods_psi');
                            ...
                            if ($_REQUEST['act'] == 'edit_product_warn_number') {
                                check_authz_json('goods_psi');
                                ...
                                if ($_REQUEST['act'] == 'psi_log') {
                                    admin_priv('goods_psi');
                                    $smarty->assign('ur_here', $_LANG['stock_journal']);
                                    $smarty->assign('action_link',
                                        array('text' => $_LANG['export_excel'], 'href' => 'javascript:;'));
                                    $smarty->assign('full_page', 1);
                                    $goods_id = empty($_REQUEST['goods_id']) ? 0 : intval($_REQUEST['goods_id']);
                                    $list = get_psi_log();
                                    $smarty->assign('goods_list', $list['list']);
                                    ...
                                    if ($_REQUEST['act'] == 'psi_log_query') {
                                        ...
                                        if ($_REQUEST['act'] == 'edit_psi_log') {
                                            check_authz_json('goods_psi');
                                            $id = intval($_POST['id']);
                                            $number = intval($_POST['val']);

                                            if (!empty($number)) {
                                                $psi_log = get_table_date('goods_inventory_logs', 'id=\'' . $id . '\'',
                                                    array('*'));
                                            ...
                                            if ($_REQUEST['act'] == 'goods_info') {
                                                ...
                                                if ($_REQUEST['act'] == 'changedgoods') {
                                                    ...
                                                    if (!empty($goods_list)) {
                                                        foreach ($goods_list as $k => $v) {
                                                            ...
                                                            $goods_list[$k]['url'] = build_uri('goods',
                                                                array('gid' => $v['goods_id']), $v['goods_name']);
                                                            $goods_list[$k]['shop_price'] = price_format($v['shop_price']);
                                                    ...
                                                    if ($_REQUEST['act'] == 'add_purchase_goods') {
                                                        check_authz_json('goods_psi');
                                                        $goods_ids = empty($_REQUEST['goods_ids']) || $_REQUEST['goods_ids'] == 'undefined' ? '' : explode(',',
                                                            $_REQUEST['goods_ids']);
                                                        $selected = empty($_REQUEST['selected']) ? '' : explode(',',
                                                            $_REQUEST['selected']);
                                                        $result = array('error' => 0, 'message' => '', 'content' => '');

                                                        if ($selected) {
                                                            if ($goods_ids) {
                                                                $goods_ids_arr = array_unique(array_merge($goods_ids,
                                                                    $selected));
