...
function get_data_list($type = 0)
{
    ...
    if ($type != 0) {
        ...
        if ($result === false) {
            ...
            $filter['shipping_status'] = isset($_REQUEST['shipping_status']) ? explode(',',
                $_REQUEST['shipping_status']) : '-1';
...
$exc = new exchange($ecs->table('offline_store'), $db, 'id', 'stores_user', 'stores_name', 'is_confirm', 'stores_tel',
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        $smarty->assign('record_count', $offline_store['record_count']);
        $smarty->assign('page_count', $offline_store['page_count']);
        make_json_result($smarty->fetch('offline_store_list.dwt'), '',
            ...
            if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                ...
                if ($_REQUEST['act'] == 'insert') {
                    ...
                    if (isset($_FILES['stores_img']['error']) && $_FILES['stores_img']['error'] == 0 || !isset($_FILES['stores_img']['error']) && isset($_FILES['stores_img']['tmp_name']) && $_FILES['stores_img']['tmp_name'] != 'none') {
                        if (!check_file_type($_FILES['stores_img']['tmp_name'], $_FILES['stores_img']['name'],
                            $allow_file_types)) {
                ...
                if ($_REQUEST['act'] == 'order_stats') {
                    admin_priv('offline_store');
                    $smarty->assign('menu_select',
                        array('action' => '17_merchants', 'current' => '16_seller_users_real'));
                    $smarty->assign('ur_here', $_LANG['2_order_stats']);
                    $smarty->assign('action_link',
                        array('text' => $_LANG['12_offline_store'], 'href' => 'offline_store.php?act=list'));
                    $start_time = local_mktime(0, 0, 0, date('m'), 1, date('Y'));
                    $end_time = local_mktime(0, 0, 0, date('m'), date('t'), date('Y')) + 24 * 60 * 60 - 1;
                    ...
                    if ($_REQUEST['act'] == 'order_stats_query') {
                        ...
                        make_json_result($smarty->fetch('store_stats_order.dwt'), '',
                            array('filter' => $data['filter'], 'page_count' => $data['page_count']));
                    } else {
                        if ($_REQUEST['act'] == 'edit_stores_tel') {
                            ...
                            if ($_REQUEST['act'] == 'toggle_show') {
                                ...
                                if ($_REQUEST['act'] == 'edit_stores_opening_hours') {
                                    check_authz_json('offline_store');
                                    $id = intval($_POST['id']);
                                    $order = json_str_iconv(trim($_POST['val']));
                                    ...
                                    if ($_REQUEST['act'] == 'remove') {
                                        ...
                                        $url = 'offline_store.php?act=query&' . str_replace('act=remove', '',
                                        ...
                                        if ($_REQUEST['act'] == 'batch_remove') {
                                            ...
                                            if ($_REQUEST['batch_handle'] == 'open_batch' || $_REQUEST['batch_handle'] == 'off_batch') {
                                                ...
                                                if ($_REQUEST['batch_handle'] == 'open_batch') {
                                                    ...
                                                    if ($_REQUEST['batch_handle'] == 'off_batch') {
                                                        $is_confirm = 0;
                                                    }
                                                }
                                                ...
                                                if ($db->query($sql) == true) {
                                                    $link[] = array(
                                                        ...
                                                        'href' => 'offline_store.php?act=list&' . list_link_postfix()
                                                    );
                                                    sys_msg($_LANG['handle_succeed'], 0, $link);
                                                }
                                            } else {
                                                if ($_REQUEST['batch_handle'] == 'drop_batch') {
                                                    if (!empty($checkboxes)) {
                                            ...
                                            if ($_REQUEST['act'] == 'stat') {
                                                ...
                                                $smarty->assign('action_link',
                                                    array('text' => $_LANG['12_offline_store'], 'href' => $href));
                                                $smarty->assign('start_date', local_date('Y-m-d', $start_date));
                                                $smarty->assign('end_date', local_date('Y-m-d', $end_date));
                                                $smarty->display('offline_store_stat.dwt');
                                            } else {
                                                if ($_REQUEST['act'] == 'stat_filter') {
