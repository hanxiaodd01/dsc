...
function mass_sms_log()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res)) {
        ...
        $user_info = get_table_date('users', 'user_id=\'' . $rows['user_id'] . '\'',
            array('user_name', 'mobile_phone'));
    ...
    if ($_REQUEST['act'] == 'log_query') {
        ...
        if ($_REQUEST['act'] == 'remove_log') {
            ...
            if ($_REQUEST['act'] == 'send') {
                ...
                $user_info = get_table_date('users', 'user_id=\'' . $log_info['user_id'] . '\'',
                    array('user_name', 'mobile_phone'));
                $template_info = get_table_date('mass_sms_template', 'id=\'' . $log_info['template_id'] . '\'',
                    array('*'));
                $links[] = array(
                    'text' => $_LANG['back_list'],
                    'href' => 'mass_sms.php?act=log_list&template_id=' . $log_info['template_id']
                );
                ...
                if ($_REQUEST['act'] == 'batch_log') {
                    ...
                    $links[] = array(
                        'text' => $_LANG['back_list'],
                        'href' => 'mass_sms.php?act=log_list&template_id=' . $template_id
                    );
                        ...
                        if ($_REQUEST['act'] == 'batch_send') {
                            ...
                            if ($list) {
                                ...
                                if (!empty($list_array)) {
                                    ...
                                    $user_info = get_table_date('users', 'user_id=\'' . $log_info['user_id'] . '\'',
                                        array('user_name', 'mobile_phone'));
                                    $template_info = get_table_date('mass_sms_template',
                                        'id=\'' . $log_info['template_id'] . '\'', array('*'));
                                    $data = array();
                                    ...
                                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('mass_sms_log'), $data, 'UPDATE',
                                        'id = \'' . $first . '\'');
                                    $result['data'] = array_merge($log_info, $user_info, $data);
                                    $result['data']['last_send'] = local_date($GLOBALS['_CFG']['time_format'],
                                        $result['data']['last_send']);
