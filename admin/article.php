...
if ($_REQUEST['act'] == 'edit') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['03_article_list'], 'href' => 'article.php?act=list&' . list_link_postfix()));
...
if ($_REQUEST['act'] == 'update') {
    ...
    if ($file_url != '' && $old_url != '' && $old_url != $file_url && strpos($old_url,
    ...
    if ($exc->edit('title=\'' . $_POST['title'] . '\', cat_id=\'' . $_POST['article_cat'] . '\', article_type=\'' . $_POST['article_type'] . '\', is_open=\'' . $_POST['is_open'] . '\', author=\'' . $_POST['author'] . '\', author_email=\'' . $_POST['author_email'] . '\', keywords =\'' . $_POST['keywords'] . '\', file_url =\'' . $file_url . '\', open_type=\'' . $open_type . '\', content=\'' . $_POST['FCKeditor1'] . '\', link=\'' . $_POST['link_url'] . '\', description = \'' . $_POST['description'] . '\',sort_order = \'' . $sort_order . '\' ',
        $_POST['id'])) {
...
');
            ...
            if ($_REQUEST['act'] == 'del_file') {
                ...
                clear_all_files();
                $links[] = array(
                    'text' => $_LANG['back_edit_article_content'],
                    'href' => 'article.php?act=edit&id=' . $article_id
                );
    ...
    if ($_REQUEST['act'] == 'batch') {
        if (isset($_POST['type'])) {
            if ($_POST['type'] == 'button_remove') {
                ...
                $sql = 'SELECT file_url FROM ' . $ecs->table('article') . ' WHERE article_id ' . db_create_in(join(',',
                        $_POST['checkboxes'])) . ' AND file_url <> \'\'';
                $res = $db->query($sql);
