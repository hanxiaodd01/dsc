...
function zc_cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('zc_category'), $args, 'update',
        'cat_id=\'' . $cat_id . '\'');
}
...
if ($_REQUEST['act'] == 'list') {
    ...
    if (isset($_REQUEST['back_level']) && (0 < $_REQUEST['back_level'])) {
        ...
        $parent_id = $db->getOne('SELECT parent_id FROM ' . $ecs->table('zc_category') . ' WHERE cat_id = \'' . $parent_id . '\'',
            true);
    } else {
...
if ($_REQUEST['act'] == 'add') {
    ...
    $smarty->assign('action_link',
        array('href' => 'zc_category.php?act=list', 'text' => $_LANG['02_crowdfunding_cat']));
    $smarty->assign('form_act', 'insert');
    $smarty->assign('cat_info',
        array('is_show' => 1, 'is_group_show' => 0, 'is_search_show' => 0, 'is_search_show_layout' => 1));
...
if ($_REQUEST['act'] == 'edit') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['02_crowdfunding_cat'], 'href' => 'zc_category.php?act=list'));
