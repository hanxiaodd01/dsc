...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('magazine_list.dwt'), '',
            array('filter' => $magazinedb['filter'], 'page_count' => $magazinedb['page_count']));
    } else {
        if ($_REQUEST['act'] == 'add') {
            if (empty($_POST['step'])) {
                $smarty->assign('action_link',
                    array('text' => $_LANG['go_list'], 'href' => 'magazine_list.php?act=list'));
                ...
                if ($_POST['step'] == 2) {
                    ...
                    if (strpos($magazine_content, 'https://') === false && strpos($magazine_content,
                            'http://') === false) {
                        $magazine_content = str_replace('src=\\"', 'src=\\"http://' . $_SERVER['HTTP_HOST'],
                            $magazine_content);
                    }
            ...
            if ($_REQUEST['act'] == 'edit') {
                ...
                if (empty($_POST['step'])) {
                    ...
                    $smarty->assign(array(
                        'id' => $id,
                        'act' => 'edit',
                        'magazine_name' => $rt['template_subject'],
                        'magazine_content' => $rt['template_content']
                    ));
                        ...
                        if (!empty($template_id)) {
                            if (SEND_LIST == $rank_array[0]) {
                                ...
                                if ($start < $count) {
                                    ...
                                    $links[] = array(
                                        'text' => sprintf($_LANG['finish_list'], $start),
                                        'href' => 'magazine_list.php?act=addtolist&id=' . $id . '&pri=' . $pri . '&start=' . $start . '&send_rank=' . $send_rank
                                    );
                                    ...
                                    $links[] = array(
                                        'text' => $_LANG['04_magazine_list'],
                                        'href' => 'magazine_list.php?act=list'
                                    );
                                    sys_msg($_LANG['edit_ok'], 0, $links);
                                }
                            } else {
