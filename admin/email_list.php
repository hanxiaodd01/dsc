    ...
    if ($_REQUEST['act'] == 'export') {
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            if ($_REQUEST['act'] == 'batch_remove') {
                ...
                $sql = 'DELETE FROM ' . $ecs->table('email_list') . ' WHERE id ' . db_create_in(join(',',
                        $_POST['checkboxes']));
                $db->query($sql);
                ...
                if ($_REQUEST['act'] == 'batch_unremove') {
                    ...
                    $sql = 'UPDATE ' . $ecs->table('email_list') . ' SET stat = 1 WHERE stat <> 1 AND id ' . db_create_in(join(',',
                            $_POST['checkboxes']));
                    $db->query($sql);
                    $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'email_list.php?act=list');
                    ...
                    if ($_REQUEST['act'] == 'batch_exit') {
                        ...
                        $sql = 'UPDATE ' . $ecs->table('email_list') . ' SET stat = 2 WHERE stat <> 2 AND id ' . db_create_in(join(',',
                                $_POST['checkboxes']));
                        $db->query($sql);
                        $lnk[] = array('text' => $_LANG['back_list'], 'href' => 'email_list.php?act=list');
