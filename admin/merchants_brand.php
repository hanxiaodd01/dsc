...
if ($_REQUEST['act'] == 'list') {
    ...
    if (0 < $adminru['ru_id']) {
        $smarty->assign('action_link',
            array('text' => $_LANG['07_brand_add'], 'href' => 'merchants_brand.php?act=add'));
    }
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'edit') {
            ...
            $smarty->assign('action_link', array(
                'text' => $_LANG['07_merchants_brand'],
                'href' => 'merchants_brand.php?act=list&' . list_link_postfix()
            ));
            ...
            if ($_REQUEST['act'] == 'updata') {
                ...
                if (!empty($_FILES['brand_logo']['name'])) {
                    $img_name = 'data/septs_Image/' . basename($image->upload_image($_FILES['brand_logo'],
                            'septs_Image'));
                } else {
                    $img_name = '';
                }
                ...
                if (isset($_POST['link_brand']) && 0 < $adminru['ru_id']) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('admin_user'), $parent, 'UPDTAE',
                        'ru_id =' . $adminru['ru_id']);
                }
                ...
                if (isset($_POST['audit_status']) && $adminru['ru_id'] == 0) {
                    ...
                    if ($_POST['audit_status'] == 1) {
                        ...
                        if (0 < $linkBrand) {
                            ...
                            if ($lid) {
                                if ($audit_status == 1) {
                                    $goods_brand_id = $GLOBALS['db']->getOne('SELECT brand_id FROM ' . $GLOBALS['ecs']->table('link_brand') . (' WHERE bid =\'' . $id . '\''),
                                        true);
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('link_brand'), $link_brand, 'UPDTAE',
                                    'bid = \'' . $bid . '\'');
                            } else {
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('link_brand'), $link_brand,
                                    'INSERT');
                            }
                        }
                    }
                        ...
                        if ($_REQUEST['act'] == 'edit_sort_order') {
                            ...
                            if ($_REQUEST['act'] == 'toggle_show') {
                                ...
                                if ($_REQUEST['act'] == 'remove') {
                                    check_authz_json('merchants_brand');
                                    $id = intval($_GET['id']);
                                    $ru_id = $GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['ecs']->table('merchants_shop_brand') . (' WHERE bid =\'' . $id . '\''),
                                        true);
                                    $brand_id = $GLOBALS['db']->getOne('SELECT brand_id FROM ' . $GLOBALS['ecs']->table('link_brand') . (' WHERE bid =\'' . $id . '\''),
                                        true);
                                    ...
                                    $db->query($sql);
                                    dsc_unlink(ROOT_PATH . DATA_DIR . '/sc_file/seller_brand/seller_brand_' . $ru_id);
                                    $url = 'merchants_brand.php?act=query&' . str_replace('act=remove', '',
                                    ...
                                    if ($_REQUEST['act'] == 'drop_logo') {
                                        ...
                                        get_del_batch('', $brand_id, array('brandLogo'), 'bid', 'merchants_shop_brand',
                                            1);
                                        ...
                                        $link = array(
                                            array(
                                                ...
                                                'href' => 'merchants_brand.php?act=edit&id=' . $brand_id
                                            ),
                                            array(
                                                'text' => $_LANG['brand_list_lnk'],
                                                'href' => 'merchants_brand.php?act=list'
                                            )
                                        );
