...
function region_store_list()
{
    ...
    while ($rows = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($region_id) {
            $rows['region_name'] = get_table_date('region', 'region_id=\'' . $region_id . '\'', array('region_name'),
                2);
        }
            ...
            if ($_REQUEST['act'] == 'edit_rs_name') {
                ...
                if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
                    ...
                    $smarty->assign('action_link',
                        array('text' => $_LANG['01_region_store_manage'], 'href' => 'region_store.php?act=list'));
                    ...
                    if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                        ...
                        if (0 < $rs_id) {
                            $db->autoExecute($ecs->table('region_store'), $store_data, 'UPDATE',
                                'rs_id = \'' . $rs_id . '\'');
                        ...
                        if ($is_exist) {
                            $db->autoExecute($ecs->table('rs_region'), $region_data, 'UPDATE',
                                'rs_id = \'' . $rs_id . '\'');
                        } else {
                        ...
                        $db->autoExecute($ecs->table('admin_user'), array('rs_id' => 0), 'UPDATE',
                            'rs_id = \'' . $rs_id . '\'');
                        $db->autoExecute($ecs->table('admin_user'), $admin_data, 'UPDATE',
                            'user_id = \'' . $admin_data['user_id'] . '\'');
                        $link[] = array('text' => $_LANG['back_list'], 'href' => 'region_store.php?act=list');
                        sys_msg($msg, 0, $link);
                    } else {
                        if ($_REQUEST['act'] == 'admin_update') {
                            check_authz_json('region_store_manage');
                            $smarty->assign('region_admin', get_region_admin());
