    ...
    if ($result === false) {
        ...
        if ($filter['use_type'] == 1) {
            ...
            if ($filter['use_type'] == 2) {
                ...
                if ($filter['use_type'] == 3) {
                    $where .= ' AND fa.userFav_type = 1';
                } else {
                    if ($filter['use_type'] == 4) {
...
function get_range_goods($act_range, $act_range_ext_list, $create_in, $user_id = 0)
{
    ...
    $sql = 'SELECT goods_id FROM ' . $GLOBALS['ecs']->table('goods') . (' WHERE user_id = \'' . $user_id . '\' AND ') . db_create_in($id_list,
            $create_in);
    $res = $GLOBALS['db']->query($sql);
    $arr_goods_id = array();
            ...
            if ($_REQUEST['act'] == 'batch') {
                ...
                if (isset($_POST['type'])) {
                    if ($_POST['type'] == 'batch_remove') {
                        ...
                        clear_cache_files();
                        $links[] = array(
                            'text' => $_LANG['back_favourable_list'],
                            'href' => 'favourable.php?act=list&' . list_link_postfix()
                        );
                        ...
                        if ($_POST['type'] == 'review_to') {
                            ...
                            if ($db->query($sql)) {
                                $lnk[] = array(
                                    'text' => $_LANG['back_favourable_list'],
                                    'href' => 'favourable.php?act=list&seller_list=1&' . list_link_postfix()
                                );
                ...
                if ($_REQUEST['act'] == 'edit_sort_order') {
                    ...
                    if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
                        ...
                        while ($row = $db->fetchRow($res)) {
                            $row['checked'] = strpos(',' . $favourable['user_rank'] . ',',
                                    ',' . $row['rank_id'] . ',') !== false;
                            $user_rank_list[] = $row;
                        }
                        ...
                        if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                            ...
                            $act_range_ext = isset($_POST['act_range_ext']) && !empty($_POST['act_range_ext']) ? implode(',',
                                $_POST['act_range_ext']) : '';
                            ...
                            if ($is_add) {
                                ...
                                if ($_CFG['region_store_enabled']) {
                                    $favourable_info = get_table_info('favourable_activity',
                                        'act_id = \'' . $act_id . '\'',
                                        array('user_id', 'rs_id', 'userFav_type', 'userFav_type_ext', 'review_status'));
                                } else {
                                    $favourable_info = get_table_info('favourable_activity',
                                        'act_id = \'' . $act_id . '\'', array('user_id', 'review_status'));
                                }
                            }
                            ...
                            if ($_CFG['region_store_enabled']) {
                                ...
                                $mer_ids = get_favourable_merchants(intval($_POST['userFav_type']), $userFav_type_ext,
                                    $rs_id, 1);
                            ...
                            if (intval($_POST['act_type']) == FAT_GOODS && isset($_POST['gift_id'])) {
                                foreach ($_POST['gift_id'] as $key => $id) {
                                    $gift[] = array(
                                        'id' => $id,
                                        'name' => $_POST['gift_name'][$key],
                                        'price' => $_POST['gift_price'][$key]
                                    );
                                }
                            }
                            ...
                            if ($is_add) {
                                ...
                                $db->autoExecute($ecs->table('favourable_activity'), $favourable, 'UPDATE',
                            ...
                            if ($is_add) {
                                $links = array(
                                    array(
                                        'href' => 'favourable.php?act=add',
                                        'text' => $_LANG['continue_add_favourable']
                                    ),
                                ...
                                $links = array(
                                    array(
                                        'href' => 'favourable.php?act=edit&id=' . $favourable['act_id'] . '&ru_id=' . $ru_id,
                                        'text' => $_LANG['edit_favourable']
                                    ),
                                    array(
                                        'href' => 'favourable.php?act=list&' . list_link_postfix(),
                                        'text' => $_LANG['back_favourable_list']
                            ...
                            if ($_REQUEST['act'] == 'drop_thumb') {
                                ...
                                $link = array(
                                    array(
                                        'text' => $_LANG['edit_favourable'],
                                        'href' => 'favourable.php?act=edit&id=' . $act_id . '&ru_id=' . $ru_id
                                    ),
                                ...
                                if ($_REQUEST['act'] == 'search') {
                                    ...
                                    if ($filter->act_range == FAR_ALL) {
                                        ...
                                        if ($filter->act_range == FAR_CATEGORY) {
                                            if ($ru_id) {
                                            ...
                                            if ($filter->act_range == FAR_BRAND) {
                                                ...
                                                if ($arr) {
                                                    foreach ($arr as $key => $row) {
                                                        if ($ru_id) {
                                                            $arr[$key]['is_brand'] = get_seller_brand_count($row['id'],
                                                                $ru_id);
                                                        } else {
                                                            $arr[$key]['is_brand'] = 1;
                                                        }
                                    ...
                                    if ($_REQUEST['act'] == 'set_use_range') {
                                        ...
                                        if ($_REQUEST['act'] == 'changedrs') {
                                            require_once ROOT_PATH . '/includes/cls_json.php';
                                            $json = new JSON();
                                            $keyword = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
                                            $result = array('error' => 0, 'message' => '', 'content' => '');
                                            $rs_ids = isset($_REQUEST['rs_ids']) ? explode(',',
                                                $_REQUEST['rs_ids']) : '';
                                            ...
                                            if ($_REQUEST['act'] == 'changedmer') {
                                                require_once ROOT_PATH . '/includes/cls_json.php';
                                                $json = new JSON();
                                                $keyword = empty($_REQUEST['keyword']) ? '' : trim($_REQUEST['keyword']);
                                                $result = array('error' => 0, 'message' => '', 'content' => '');
                                                $mer_ids = isset($_REQUEST['mer_ids']) ? explode(',',
                                                    $_REQUEST['mer_ids']) : '';
                                                $type = isset($_REQUEST['type']) ? intval($_REQUEST['type']) : 0;
                                                $rs_id = empty($_REQUEST['rs_id']) ? 0 : intval($_REQUEST['rs_id']);
                                                ...
                                                if (!empty($mer_list)) {
                                                    foreach ($mer_list as $k => $v) {
                                                        ...
                                                        if (0 < $v['user_id'] && in_array($v['user_id'],
                                                                $mer_ids) && !empty($mer_ids)) {
                                                ...
                                                if ($_REQUEST['act'] == 'marketing_center') {
                                                    require_once ROOT_PATH . '/includes/cls_json.php';
                                                    $json = new JSON();
