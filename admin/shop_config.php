                    ...
                    if ($item) {
                        if ($item['type'] == 'file' && in_array($item['code'], $code_arr) && $item['value']) {
                            ...
                            if (strpos($item['value'], '../') === false) {
                                $item['value'] = get_image_path(0, $item['value']);
                            }
                        } else {
                            $item['del_img'] = 0;
                        }
    ...
    if ($_REQUEST['act'] == 'mail_settings') {
        ...
        if ($_REQUEST['act'] == 'post') {
            ...
            if (!in_array($type, $back_array)) {
                $update_arr = array(
                    'seller_email' => $_POST['value'][114],
                    'kf_qq' => $_POST['value'][109],
                    'kf_ww' => $_POST['value'][110],
                    'shop_title' => $_POST['value'][102],
                    'shop_keyword' => $_POST['value'][104],
                    'country' => $_POST['value'][105],
                    'province' => $_POST['value'][106],
                    'city' => $_POST['value'][107],
                    'district' => 0,
                    'shop_address' => $_POST['value'][108],
                    'kf_tel' => $_POST['value'][115],
                    'notice' => $_POST['value'][121]
                );
                ...
                if (isset($file['error']) && $file['error'] == 0 || !isset($file['error']) && $file['tmp_name'] != 'none') {
                    if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                        ...
                        if ($code == 'shop_logo') {
                            ...
                            $file_name = str_replace('{$template}', $_CFG['template'],
                                    $file_var_list[$code]['store_dirlogo']) . $info['logo'];
                        } else {
                            if ($code == 'business_logo') {
                                include_once 'includes/lib_template.php';
                                $info = get_template_info($_CFG['template']);
                                $file_name = str_replace('{$template}', $_CFG['template'],
                                        $file_var_list[$code]['store_dir']) . $info['business_logo'];
                            } else {
                                if ($code == 'watermark') {
                                    ...
                                    if ($code == 'wap_logo') {
                                        ...
                                        if ($code == 'two_code_logo') {
                                            ...
                                            if (in_array($code, $code_store_dir)) {
                                                $ext = !empty($file['name']) ? explode('.', $file['name']) : '';
                                                $ext = !empty($ext) ? array_pop($ext) : '';
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            ...
                            if ($file_name) {
                                ...
                                if ($olde_value != $file_name && $olde_value != '../images/errorImg.png' && $olde_value != '' && strpos($olde_value,
                                        'http://') === false && strpos($olde_value, 'https://') === false) {
            ...
            $httpData = array(
                ...
                'qq' => $row['qq'],
                'ww' => $row['ww'],
                'ym' => $row['service_phone'],
                'msn' => $row['msn'],
                'email' => $row['service_email'],
                'phone' => $row['sms_shop_mobile'],
                'icp' => $row['icp_number'],
                'version' => VERSION,
                'release' => RELEASE,
                'language' => $_CFG['lang'],
