...
function get_pzd_list($ru_id)
{
    ...
    foreach ($row as $k => $v) {
        if (0 < $v['ru_id']) {
            ...
            if (0 < $v['suppliers_id']) {
                $row[$k]['shop_name'] = get_table_date('suppliers', 'suppliers_id=\'' . $v['suppliers_id'] . '\'',
                    array('suppliers_name'), 2);
            } else {
...
function get_pic_album($album_id = 0)
{
    ...
    foreach ($res as $k => $v) {
        ...
        if (!empty($res[$k]['album_cover']) && (strpos($res[$k]['album_cover'],
                    'http://') === false && strpos($res[$k]['album_cover'], 'https://') === false)) {
    ...
    foreach ($row as $k => $v) {
        ...
        if (!empty($row[$k]['pic_file']) && (strpos($row[$k]['pic_file'],
                    'http://') === false && strpos($row[$k]['pic_file'], 'https://') === false)) {
...
if ($_REQUEST['act'] == 'list') {
    ...
    if (0 < $parent_id) {
        ...
        $smarty->assign('action_link1', array(
            'text' => $_LANG['return_to_superior'],
            'href' => 'gallery_album.php?act=list&parent_id=' . $parent_album_id . '&seller_list=' . $offline_store['filter']['seller_list']
        ));
    }
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        $smarty->assign('record_count', $offline_store['record_count']);
        $smarty->assign('page_count', $offline_store['page_count']);
        make_json_result($smarty->fetch('gallery_album.dwt'), '',
            array('filter' => $offline_store['filter'], 'page_count' => $offline_store['page_count']));
    } else {
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            $smarty->assign('action_link',
                array('text' => $_LANG['gallery_album'], 'href' => 'gallery_album.php?act=list'));
            ...
            if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                ...
                if ($_REQUEST['act'] == 'insert') {
                    ...
                    if (isset($_FILES['album_cover']['error']) && $_FILES['album_cover']['error'] == 0 || !isset($_FILES['album_cover']['error']) && isset($_FILES['album_cover']['tmp_name']) && $_FILES['album_cover']['tmp_name'] != 'none') {
                        if (!check_file_type($_FILES['album_cover']['tmp_name'], $_FILES['album_cover']['name'],
                            $allow_file_types)) {
                    ...
                    $is_only = $exc->is_only('album_mame', $album_mame, 0,
                        'ru_id = ' . $seller_id . (' AND album_id != \'' . $album_id . '\''));
                    ...
                    if ($old_url != '' && $old_url != $file_url && strpos($old_url,
                            'http://') === false && strpos($old_url, 'https://') === false) {
                ...
                if ($_REQUEST['act'] == 'view') {
                    ...
                    $smarty->assign('action_link', array(
                        'text' => '上传图片',
                        'spec' => 'ectype=\'addpic_album\'',
                        'href' => 'gallery_album.php?act=view&id=' . $album_id
                    ));
                    $smarty->assign('album_id', $album_id);
                    $album_info = get_goods_gallery_album(2, $album_id, array('suppliers_id,ru_id'));
                    $cat_select = gallery_cat_list(0, 0, false, 0, true, $album_info['ru_id'],
                        $album_info['suppliers_id']);
                    ...
                    if ($parent_album_id) {
                        $smarty->assign('action_link', array(
                            'text' => $_LANG['upload_image'],
                            'spec' => 'ectype=\'addpic_album\'',
                            'href' => 'gallery_album.php?act=view&id=' . $parent_album_id
                        ));
                    } else {
                        $smarty->assign('action_link', array(
                            'text' => $_LANG['upload_image'],
                            'spec' => 'ectype=\'addpic_album\'',
                            'href' => 'gallery_album.php?act=list'
                        ));
                    }
                    ...
                    if ($_REQUEST['act'] == 'pic_query') {
                        ...
                        $smarty->assign('record_count', $offline_store['record_count']);
                        $smarty->assign('page_count', $offline_store['page_count']);
                        make_json_result($smarty->fetch('pic_album.dwt'), '',
                            array('filter' => $offline_store['filter'], 'page_count' => $offline_store['page_count']));
                    } else {
                        if ($_REQUEST['act'] == 'remove') {
                            ...
                            if (0 < $album_count) {
                                ...
                                $res = $db->getOne($sql);
                                $old_url = get_goods_gallery_album(0, $album_id, array('album_cover'));
                                if ($old_url != '' && @strpos($old_url, 'http://') === false && @strpos($old_url,
                                        'https://') === false) {
                                    @unlink(ROOT_PATH . $old_url);
                                    $del_arr_img[] = $old_url;
                                    get_oss_del_file($del_arr_img);
                                ...
                                if (0 < $res) {
                                    ...
                                    $url = 'gallery_album.php?act=query&' . str_replace('act=remove', '',
                            ...
                            if ($_REQUEST['act'] == 'gallery_query') {
                                ...
                                $smarty->assign('record_count', $offline_store['record_count']);
                                $smarty->assign('page_count', $offline_store['page_count']);
                                make_json_result($smarty->fetch('pic_album.dwt'), '', array(
                                    'filter' => $offline_store['filter'],
                                    'page_count' => $offline_store['page_count']
                                ));
                            } else {
                                if ($_REQUEST['act'] == 'pic_remove') {
                                    require ROOT_PATH . '/includes/cls_json.php';
                                    $json = new JSON();
                                    $result = array('error' => '', 'content' => '', 'url' => '');
                                    $id = isset($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
                                    $pic_info = gallery_pic_album(2, $id,
                                        array('pic_file', 'pic_thumb', 'pic_image', 'album_id'));
                                    if ($pic_info['pic_file'] != '' && @strpos($pic_info['pic_file'],
                                            'http://') === false && @strpos($pic_info['pic_file'],
                                            'https://') === false) {
                                        $arr_img[] = $pic_info['pic_file'];
                                    }

                                    if ($pic_info['pic_thumb'] != '' && @strpos($pic_info['pic_thumb'],
                                            'http://') === false && @strpos($pic_info['pic_thumb'],
                                            'https://') === false) {
                                        $arr_img[] = $pic_info['pic_thumb'];
                                    }

                                    if ($pic_info['pic_image'] != '' && @strpos($pic_info['pic_image'],
                                            'http://') === false && @strpos($pic_info['pic_image'],
                                            'https://') === false) {
                                    ...
                                    if ($_REQUEST['act'] == 'remove_batch') {
                                        ...
                                        if (!empty($checkboxes)) {
                                            ...
                                            if (!empty($album_cover)) {
                                                foreach ($album_cover as $k => $v) {
                                                    if ($v['album_cover'] != '' && @strpos($v['album_cover'],
                                                            'http://') === false && @strpos($v['album_cover'],
                                                            'https://') === false) {
                                            ...
                                            if ($db->query($sql) == true) {
                                                ...
                                                $link[0] = array(
                                                    ...
                                                    'href' => 'gallery_album.php?act=list&' . list_link_postfix()
                                                );
                                                ...
                                                if (!empty($unremove_arr)) {
                                                    ...
                                                    if (!empty($album_mame_arr)) {
                                                        ...
                                                        $back_msg = sprintf($_LANG['unremove_succeed'],
                                                            stripslashes($album_mame_arr));
                                                    }
                                                }
                                        ...
                                        if ($_REQUEST['act'] == 'edit_sort_order') {
                                            ...
                                            if ($_REQUEST['act'] == 'upload_pic') {
                                                ...
                                                if ($_FILES['file']['name']) {
                                                    if ($realname) {
                                                        $extname = strtolower(substr($_FILES['file']['name'],
                                                ...
                                                if ($extname == 'mp4') {
                                                    ...
                                                    if (isset($_FILES['file']['error']) && $_FILES['file']['error'] == 0 || !isset($_FILES['file']['error']) && isset($_FILES['file']['tmp_name']) && $_FILES['file']['tmp_name'] != 'none') {
                                                        if (!check_file_type($_FILES['file']['tmp_name'],
                                                            $_FILES['file']['name'], $allow_file_types)) {
                                                            sys_msg($_LANG['invalid_file']);
                                                        }
                                                        ...
                                                        if ($proc_thumb && 0 < $image->gd_version() && $image->check_img_function($_FILES['file']['type'])) {
                                                            if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0) {
                                                                $goods_thumb = $image->make_thumb('../' . $original_img,
                                                                    ...
                                                                    '../data/gallery_album/' . $album_id . '/thumb_img/');
                                                ...
                                                if ($_REQUEST['act'] == 'goods_video') {
                                                    ...
                                                    if ($extname == 'mp4') {
                                                        ...
                                                        if ($upload->uploadFile('file')) {
                                                            ...
                                                            $result['goods_video_path'] = !empty($goods_video) ? get_image_path($goods_id,
                                                                $goods_video) : '';
                                                        } else {
                                                            $result['error'] = 1;
                                                            $result['massege'] = $upload->getErrorMsg();
                                                    ...
                                                    if ($_REQUEST['act'] == 'del_video') {
                                                        ...
                                                        if ($_REQUEST['act'] == 'batch') {
                                                                ...
                                                                if ($type == 'remove') {
                                                                    ...
                                                                    if (!empty($pic_info)) {
                                                                        foreach ($pic_info as $v) {
                                                                            if ($v['pic_file'] != '' && @strpos($v['pic_file'],
                                                                                    'http://') === false && @strpos($v['pic_file'],
                                                                                    'https://') === false) {
                                                                                dsc_unlink(ROOT_PATH . $v['pic_file']);
                                                                                $arr_img[] = $v['pic_file'];
                                                                            }

                                                                            if ($v['pic_thumb'] != '' && @strpos($v['pic_thumb'],
                                                                                    'http://') === false && @strpos($v['pic_thumb'],
                                                                                    'https://') === false) {
                                                                                dsc_unlink(ROOT_PATH . $v['pic_thumb']);
                                                                                $arr_img[] = $v['pic_thumb'];
                                                                            }

                                                                            if ($v['pic_image'] != '' && @strpos($v['pic_image'],
                                                                                    'http://') === false && @strpos($v['pic_image'],
                                                                                    'https://') === false) {
                                                                    ...
                                                                    if ($res) {
                                                                        $link[] = array(
                                                                            'text' => $_LANG['bank_list'],
                                                                            'href' => 'gallery_album.php?act=view&id=' . $old_album_id
                                                                        );
                                                            ...
                                                            if ($_REQUEST['act'] == 'move_pic') {
                                                                ...
                                                                $album_info = $db->getRow($sql);
                                                                $cat_select = gallery_cat_list(0, 0, false, 0, true,
                                                                    $album_info['ru_id'], $album_info['suppliers_id']);
                                                                ...
                                                                if ($_REQUEST['act'] == 'submit_pic') {
                                                                    ...
                                                                    $link[] = array(
                                                                        'text' => $_LANG['bank_list'],
                                                                        'href' => 'gallery_album.php?act=list&parent_id=' . $parent_album_id
                                                                    );
