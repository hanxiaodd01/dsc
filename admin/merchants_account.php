...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link6',
        array('text' => $_LANG['fund_details'], 'href' => 'merchants_account.php?act=account_log_list'));
    $smarty->assign('action_link2', array(
        'text' => $_LANG['presentation_record'],
        'href' => 'merchants_account.php?act=list&act_type=detail&log_type=4' . $action_link
    ));
    $smarty->assign('action_link1', array(
        'text' => $_LANG['recharge_record'],
        'href' => 'merchants_account.php?act=list&act_type=detail&log_type=3' . $action_link
    ));
    $smarty->assign('action_link4', array(
        'text' => $_LANG['settlement_record'],
        'href' => 'merchants_account.php?act=list&act_type=detail&log_type=2' . $action_link
    ));
    $smarty->assign('action_link5', array(
        'text' => $_LANG['thawing_record'],
        'href' => 'merchants_account.php?act=list&act_type=detail&log_type=5' . $action_link
    ));
    $smarty->assign('action_link', array(
        'text' => $_LANG['05_seller_account_log'],
        'href' => 'merchants_account.php?act=list&act_type=account_log' . $action_link
    ));
    $smarty->assign('action_link3', array(
        'text' => $_LANG['merchant_funds_list'],
        'href' => 'merchants_account.php?act=list&act_type=merchants_seller_account' . $action_link
    ));
    ...
    if ($_REQUEST['act_type'] == 'detail') {
        ...
        if ($_REQUEST['act_type'] == 'account_log') {
            ...
            if ($_REQUEST['act_type'] == 'merchants_seller_account') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act_type'] == 'detail' || $_REQUEST['act_type'] == 'account_log' || $_REQUEST['act_type'] == 'merchants_seller_account') {
            ...
            make_json_result($smarty->fetch($fetch . '.dwt'), '',
        ...
        if ($_REQUEST['act'] == 'check') {
            $smarty->assign('action_link2', array(
                ...
                'href' => 'merchants_account.php?act=list&act_type=detail'
            ));
            $smarty->assign('action_link', array(
                'text' => $_LANG['05_seller_account_log'],
            ...
            if ($_REQUEST['act'] == 'update_check') {
                ...
                if (0 < $log_status && $log_status <= 2) {
                    if ($log_info['log_type'] == 5) {
                        ...
                        if ($log_status == 1) {
                            ...
                            merchants_account_log($log_info['ru_id'], 0, $log_info['frozen_money'],
                                '【' . $_SESSION['admin_name'] . '】' . $_LANG['08_refuse_apply_for']);
                        }
                        ...
                        if ($log_info['seller_frozen'] < $log_info['amount'] && $log_info['payment_info']['pay_code'] != 'bank') {
                            ...
                            if ($certificate_img['name']) {
                                $certificate = $image->upload_image('', 'seller_account', '', 1,
                                    $certificate_img['name'], $certificate_img['type'], $certificate_img['tmp_name'],
                                    $certificate_img['error'], $certificate_img['size']);
                            }
                            ...
                            if ($log_info['payment_info']['pay_code'] == 'bank') {
                                ...
                                if ($log_info['deposit_mode'] == 1) {
                                    ...
                                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('account_log'),
                ...
                if ($_REQUEST['act'] == 'edit_seller') {
                    $smarty->assign('ur_here', $_LANG['adjust_merchant_account']);
                    $smarty->assign('action_link', array(
                        'text' => $_LANG['merchant_funds_list'],
                        'href' => 'merchants_account.php?act=list&act_type=merchants_seller_account'
                    ));
                    ...
                    setcookie('seller_account_cookie', $seller_account_cookie, gmtime() + 3600 * 24 * 30,
                    ...
                    if ($_REQUEST['act'] == 'insert') {
                        ...
                        $links = array(
                            array(
                                'href' => 'merchants_account.php?act=account_log_list&ru_id=' . $user_id,
                                'text' => $_LANG['account_list']
                            ),
                            array(
                                'href' => 'merchants_account.php?act=edit_seller&ru_id=' . $user_id,
                                'text' => $_LANG['add_account']
                            )
                        );
                        ...
                        if (!empty($sc_guid) && !empty($sc_rand) && isset($_COOKIE['seller_account_cookie'])) {
                            ...
                            $change_desc = sprintf($_LANG['seller_change_money'],
                                    $_SESSION['admin_name']) . $change_desc;
                            merchants_account_log($user_id, $user_money, $frozen_money, $change_desc, 3);
                            setcookie('seller_account_cookie', '', gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                                $GLOBALS['cookie_domain']);
                        }

                        sys_msg($_LANG['merchant_funds_list'], 0, $links);
                    } else {
                        if ($_REQUEST['act'] == 'account_log_list') {
                            $store_list = get_common_store_list();
                            $smarty->assign('store_list', $store_list);
                            $smarty->assign('ur_here', $_LANG['04_seller_detail']);
                            $smarty->assign('action_link6', array(
                                'text' => $_LANG['fund_details'],
                                'href' => 'merchants_account.php?act=account_log_list'
                            ));
                            $smarty->assign('action_link2', array(
                                'text' => $_LANG['presentation_record'],
                                'href' => 'merchants_account.php?act=list&act_type=detail&log_type=4'
                            ));
                            $smarty->assign('action_link1', array(
                                'text' => $_LANG['recharge_record'],
                                'href' => 'merchants_account.php?act=list&act_type=detail&log_type=3'
                            ));
                            $smarty->assign('action_link4', array(
                                'text' => $_LANG['settlement_record'],
                                'href' => 'merchants_account.php?act=list&act_type=detail&log_type=2'
                            ));
                            $smarty->assign('action_link5', array(
                                'text' => $_LANG['thawing_record'],
                                'href' => 'merchants_account.php?act=list&act_type=detail&log_type=5'
                            ));
                            $smarty->assign('action_link', array(
                                'text' => $_LANG['05_seller_account_log'],
                                'href' => 'merchants_account.php?act=list&act_type=account_log'
                            ...
                            if ($_REQUEST['act'] == 'account_query') {
                                ...
                                $smarty->assign('act_type', 'account_log_list');
                                assign_query_info();
                                make_json_result($smarty->fetch('account_log_list.dwt'), '',
                                    array('filter' => $list['filter'], 'page_count' => $list['page_count']));
                            } else {
                                if ($_REQUEST['act'] == 'change_amount') {
                                    ...
                                    if ($GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_account_log'),
                                        $result, 'UPDATE', 'log_id =' . $log_id)) {
                                        $result['error'] = 0;
                                    } else {
                                        $result['error'] = 1;
                                    }
