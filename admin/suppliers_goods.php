            ...
            if (isset($GLOBALS['shop_id']) && 0 < $GLOBALS['shop_id']) {
                foreach ($img_list as $key => $gallery_img) {
                    $gallery_img[$key]['img_url'] = get_image_path($gallery_img['goods_id'],
                        $gallery_img['img_original'], false, 'gallery');
                    $gallery_img[$key]['thumb_url'] = get_image_path($gallery_img['goods_id'],
                        $gallery_img['img_original'], true, 'gallery');
                }
            } else {
                foreach ($img_list as $key => $gallery_img) {
        ...
        if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($_FILES['goods_img']['tmp_name'] != '' && $_FILES['goods_img']['tmp_name'] != 'none') {
                ...
                if ($proc_thumb && 0 < $image->gd_version() && $image->check_img_function($_FILES['goods_img']['type'])) {
                    if ($_CFG['image_width'] != 0 || $_CFG['image_height'] != 0) {
                        $goods_img = $image->make_thumb('../' . $goods_img, $GLOBALS['_CFG']['image_width'],
                    ...
                    if (0 < intval($_CFG['watermark_place']) && !empty($GLOBALS['_CFG']['watermark'])) {
                        if ($image->add_watermark('../' . $goods_img, '', $GLOBALS['_CFG']['watermark'],
                                $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']) === false) {
                            sys_msg($image->error_msg(), 1, array(), false);
                        }

                        if ($_CFG['auto_generate_gallery']) {
                            if ($image->add_watermark('../' . $gallery_img, '', $GLOBALS['_CFG']['watermark'],
                    ...
                    if ($_CFG['auto_generate_gallery']) {
                        if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0) {
                            $gallery_thumb = $image->make_thumb('../' . $img, $GLOBALS['_CFG']['thumb_width'],
            ...
            if (isset($_FILES['goods_thumb']) && $_FILES['goods_thumb']['tmp_name'] != '' && isset($_FILES['goods_thumb']['tmp_name']) && $_FILES['goods_thumb']['tmp_name'] != 'none') {
                ...
                if ($proc_thumb && isset($_POST['auto_thumb']) && !empty($original_img)) {
                    if ($_CFG['thumb_width'] != 0 || $_CFG['thumb_height'] != 0) {
                        $goods_thumb = $image->make_thumb('../' . $original_img, $GLOBALS['_CFG']['thumb_width'],
                            $GLOBALS['_CFG']['thumb_height']);
                            ...
                            if ($_REQUEST['act'] == 'check_goods_sn') {
                                ...
                                if ($_REQUEST['act'] == 'edit_goods_price') {
                                    ...
                                    if ($_REQUEST['act'] == 'edit_goods_number') {
                                        ...
                                        if ($exc->edit('goods_number = \'' . $goods_num . '\', last_update=' . gmtime(),
                                            $goods_id)) {
                                            clear_cache_files();
                                        ...
                                        if ($_REQUEST['act'] == 'toggle_on_sale') {
                                            ...
                                            if ($_REQUEST['act'] == 'toggle_best') {
                                                ...
                                                if ($exc->edit('is_best = \'' . $is_best . '\', last_update=' . gmtime(),
                                                    $goods_id)) {
                                                    clear_cache_files();
                                                ...
                                                if ($_REQUEST['act'] == 'toggle_new') {
                                                    ...
                                                    if ($exc->edit('is_new = \'' . $is_new . '\', last_update=' . gmtime(),
                                                        $goods_id)) {
                                                        clear_cache_files();
                                                    ...
                                                    if ($_REQUEST['act'] == 'toggle_hot') {
                                                        ...
                                                        if ($exc->edit('is_hot = \'' . $is_hot . '\', last_update=' . gmtime(),
                                                            $goods_id)) {
                                                            clear_cache_files();
                                                        ...
                                                        if ($_REQUEST['act'] == 'edit_sort_order') {
                                                            ...
                                                            if ($_REQUEST['act'] == 'query') {
                                                                ...
                                                                $smarty->assign('record_count',
                                                                    $goods_list['record_count']);
                                                                $smarty->assign('page_count',
                                                                    $goods_list['page_count']);
                                                                $smarty->assign('list_type',
                                                                    $is_delete ? 'trash' : 'goods');
                                                                ...
                                                                if ($_REQUEST['act'] == 'remove') {
                                                                    ...
                                                                    if ($exc->edit('is_delete = 1', $goods_id)) {
                                                                        ...
                                                                        admin_log(addslashes($goods_name), 'trash',
                                                                            'goods');
                                                                        $url = 'goods.php?act=query&' . str_replace('act=remove',
...
');
                                                                    ...
                                                                    if ($_REQUEST['act'] == 'restore_goods') {
                                                                        $goods_id = intval($_REQUEST['id']);
                                                                        check_authz_json('remove_back');
                                                                        $exc->edit('is_delete = 0, add_time = \'' . gmtime() . '\'',
                                                                            $goods_id);
                                                                        clear_cache_files();
                                                                        $goods_name = $exc->get_name($goods_id);
                                                                        admin_log(addslashes($goods_name), 'restore',
                                                                            'goods');
                                                                        $url = 'goods.php?act=query&' . str_replace('act=restore_goods',
                                                                                '', $_SERVER['QUERY_STRING']);
                                                                        ecs_header('Location: ' . $url . '
');
                                                                        exit();
                                                                        ...
                                                                        if ($_REQUEST['act'] == 'drop_goods') {
                                                                            ...
                                                                            $exc->drop($goods_id);
                                                                            admin_log(addslashes($goods['goods_name']),
                                                                                'remove', 'goods');
                                                                            ...
                                                                            if ($goods['is_real'] != 1) {
                                                                                ...
                                                                                if (!$db->query($sql,
                                                                                        'SILENT') && $db->errno() != 1146) {
                                                                                    exit($db->error());
                                                                                }
                                                                            }

                                                                            clear_cache_files();
                                                                            $url = 'goods.php?act=query&' . str_replace('act=drop_goods',
                                                                                    '', $_SERVER['QUERY_STRING']);
                                                                            ecs_header('Location: ' . $url . '
');
                                                                            exit();
                                                                            ...
                                                                            if ($_REQUEST['act'] == 'get_attr') {
                                                                                ...
                                                                                $content = build_attr_html($goods_type,
                                                                                    $goods_id);
