...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'detail_query' || $_REQUEST['act'] == 'download')) {
    ...
    make_json_result($smarty->fetch('exchange_detail_list.dwt'), '',
        array('filter' => $exchange_detail_data['filter'], 'page_count' => $exchange_detail_data['page_count']));
}
...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'order_query' || $_REQUEST['act'] == 'order_download')) {
    ...
    make_json_result($smarty->fetch('give_integral_orders.dwt'), '',
        array('filter' => $order_list_data['filter'], 'page_count' => $order_list_data['page_count']));
} else {
    if ($_REQUEST['act'] == 'detail') {
        ...
        if ($_REQUEST['act'] == 'detail_query') {
            ...
            make_json_result($smarty->fetch('exchange_detail_list.dwt'), '',
                array('filter' => $exchange_detail['filter'], 'page_count' => $exchange_detail['page_count']));
        } else {
            if ($_REQUEST['act'] == 'exchange_goods') {
                ...
                if ($_REQUEST['act'] == 'goods_detail_query') {
                    $user_id = isset($_REQUEST['user_id']) ? intval($_REQUEST['user_id']) : 0;
                    $goods = get_exchange_list($user_id, $is_pagination = true);
                    $smarty->assign('filter', $goods['filter']);
                    $smarty->assign('record_count', $goods['record_count']);
                    ...
                    make_json_result($smarty->fetch('exchange_goods_detail_info.dwt'), '',
                        array('filter' => $goods['filter'], 'page_count' => $goods['page_count']));
                } else {
                    if ($_REQUEST['act'] == 'order_view') {
                        $smarty->assign('ur_here', $_LANG['give_integral_order_list']);
                        $smarty->assign('action_link',
                            array('href' => 'exchange_detail.php?act=export_orders&', 'text' => $_LANG['export']));
                        $order_list = give_integral_order_list();
