    ...
    if (!empty($position_info['position_model'])) {
        ...
        foreach ($init_model as $model) {
            if (strpos($ad_model, $model)) {
                ...
                $i++;
                $ad_model = str_replace(array('_' . $model . '_', '_' . $model, $model . '_', $model),
                    array('', '', '', ''), $ad_model);
            }
        }
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            if (!empty($pid)) {
                ...
                $another_pic = in_array($rec['ad_model'], array(
                    'recommend_category',
                    'recommend_merchants',
                    'expert_field_ad',
                    'category_top_default_brand'
                ));
                $title = in_array($rec['ad_model'], array(
                    'recommend_category',
                    'expert_field_ad',
            ...
            $smarty->assign('action_link',
                array('href' => 'ads.php?act=list' . '&pid=' . $pid, 'text' => $_LANG['ad_list']));
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                if ($_POST['media_type'] == '0') {
                    ...
                    if (!empty($_POST['img_url'])) {
                        ...
                        if ($image_url) {
                            if (!empty($image_url) && $image_url != $GLOBALS['_LANG']['img_file'] && $image_url != 'http://' && copy(trim($image_url),
                                    ROOT_PATH . 'data/afficheimg/' . basename($image_url))) {
                    ...
                    if ($_POST['media_type'] == '1') {
                        if (isset($_FILES['upfile_flash']['error']) && $_FILES['upfile_flash']['error'] == 0 || !isset($_FILES['upfile_flash']['error']) && isset($_FILES['ad_img']['tmp_name']) && $_FILES['upfile_flash']['tmp_name'] != 'none') {
                            ...
                            if (!empty($_POST['flash_url'])) {
                                if (substr(strtolower($_POST['flash_url']),
                                        strlen($_POST['flash_url']) - 4) != '.swf') {
                                    $link[] = array(
                                        'text' => $_LANG['go_back'],
                ...
                if ($_REQUEST['act'] == 'edit') {
                    ...
                    if ($ads_arr['media_type'] == '0') {
                        if (strpos($ads_arr['ad_code'], 'http://') === false && strpos($ads_arr['ad_code'],
                                'https://') === false) {
                    ...
                    if ($ads_arr['ad_bg_code']) {
                        if (strpos($ads_arr['ad_bg_code'], 'http://') === false && strpos($ads_arr['ad_bg_code'],
                                'https://') === false) {
                    ...
                    if ($_REQUEST['act'] == 'edit_ad_name') {
                        ...
                        if ($_REQUEST['act'] == 'update') {
                            ...
                            if ($type == 0) {
                                ...
                                if (isset($_FILES['ad_bg_img']['error']) && $_FILES['ad_bg_img']['error'] == 0 || !isset($_FILES['ad_bg_img']['error']) && isset($_FILES['ad_bg_img']['tmp_name']) && $_FILES['ad_bg_img']['tmp_name'] != 'none') {
                                    $bg_img_up_info = basename($image->upload_image($_FILES['ad_bg_img'],
                                        'afficheimg'));
                            ...
                            clear_cache_files();
                            $href[] = array(
                                'text' => $_LANG['back_ads_list'],
                                'href' => 'ads.php?act=list' . '&pid=' . $_POST['position_id']
                            );
                            sys_msg($_LANG['edit'] . ' ' . $_POST['ad_name'] . ' ' . $_LANG['attradd_succed'], 0,
                                $href);
                        } else {
                            if ($_REQUEST['act'] == 'add_js') {
                                ...
                                $smarty->assign('action_link',
                                    array('href' => 'ads.php?act=list', 'text' => $_LANG['ad_list']));
