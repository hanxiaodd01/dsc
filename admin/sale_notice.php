...
function sale_notice_list($ru_id)
{
    ...
    $statusArr = array(
        1 => $GLOBALS['_LANG']['has_been_sent'],
        2 => $GLOBALS['_LANG']['unsent'],
        3 => $GLOBALS['_LANG']['system_send_fail']
    );
...
if ($_REQUEST['act'] == 'view') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['depreciate_notice_list'], 'href' => 'sale_notice.php?act=list'));
    assign_query_info();
...
if ($_REQUEST['act'] == 'action') {
    ...
    if (isset($_POST['remail']) && !empty($detail[email])) {
        ...
        if (send_mail($detail['user_name'], $detail['email'], $template['template_subject'], $content,
            $template['is_html'])) {
...
if ($_REQUEST['act'] == 'batch') {
    ...
    if (isset($_POST['checkboxes'])) {
        switch ($action) {
            case 'remove':
                $db->query('DELETE FROM ' . $ecs->table('sale_notice') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                        'id'));
                break;
