...
if ($_REQUEST['act'] == 'view') {
    ...
    if (!$is_multi) {
        ...
        while ($val = $db->fetchRow($res)) {
            ...
            $from_xml .= '<set name=\'' . str_replace(array('http://', 'https://'), array('', ''),
                    $from) . '\' value=\'' . $val['access_count'] . '\' color=\'' . chart_color($key) . '\' />';
            $key++;
        }
    ...
    if (!$is_multi) {
        $filename = gmdate($_CFG['date_format'], $start_date + ($timezone * 3600)) . '_' . gmdate($_CFG['date_format'],
                $end_date + ($timezone * 3600));
        $smarty->assign('action_link', array(
            'text' => $_LANG['down_flow_stats'],
            'href' => 'flow_stats.php?act=download&filename=' . $filename . '&start_date=' . $start_date . '&end_date=' . $end_date
        ));
    }
