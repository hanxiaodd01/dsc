...
function wholesale_order_list()
{
    ...
    if ($result === false) {
        ...
        if (empty($filter['start_take_time']) || empty($filter['end_take_time'])) {
            if ($adminru['ru_id'] == 0) {
                $where_store = ' AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' AS og ' . ' WHERE o.order_id = og.order_id LIMIT 1) > 0 ' . ' AND (SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' AS oi2 WHERE oi2.main_order_id = o.order_id) = 0';
            }
        }
    ...
    foreach ($row as $key => $value) {
        ...
        $value['ru_id'] = $GLOBALS['db']->getOne(' SELECT ru_id FROM ' . $GLOBALS['ecs']->table('wholesale_order_goods') . ' WHERE order_id = \'' . $value['order_id'] . '\'',
            true);
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        $smarty->assign('action_link',
            array('href' => 'order.php?act=order_query', 'text' => $_LANG['03_order_query']));
        $smarty->assign('order_list', $list['order_list']);
        $smarty->assign('filter', $list['filter']);
        $smarty->assign('record_count', $list['record_count']);
        $smarty->assign('page_count', $list['page_count']);
        $sort_flag = sort_flag($list['filter']);
        $smarty->assign($sort_flag['tag'], $sort_flag['img']);
        make_json_result($smarty->fetch('wholesale_order_list.dwt'), '',
        ...
        if ($_REQUEST['act'] == 'info') {
            ...
            $weight_price = wholesale_order_weight_price($order['order_id']);
            $order['total_weight'] = $weight_price['formated_weight'];
            $date = array('order_id');
            $order_child = count(get_table_date('wholesale_order_info', 'main_order_id=\'' . $order['order_id'] . '\'',
                $date, 1));
            ...
            $goods_list = array();
            $goods_attr = array();
            $sql = ' SELECT o.*, c.measure_unit, w.act_id, w.goods_number AS storage, g.model_inventory, g.model_attr as model_attr, o.goods_attr, g.suppliers_id, p.product_sn,
            ...
            if (isset($_GET['print'])) {
                ...
                if (isset($_GET['shipping_print'])) {
                    ...
                    if ($shipping['print_model'] == 2) {
                        ...
                        $sql = 'select oi.ru_id from ' . $GLOBALS['ecs']->table('order_info') . ' as oi ' . ' where oi.order_id = \'' . $order['order_id'] . '\'';
                    ...
                    $smarty->assign('action_link', array(
                        'href' => 'wholesale_order.php?act=list&' . list_link_postfix(),
                        'text' => $_LANG['02_order_list']
                    ));
                    assign_query_info();
            ...
            if ($_REQUEST['act'] == 'get_goods_info') {
                ...
                $smarty->assign('goods_type', 'wholesale');
                $smarty->assign('goods_attr', $attr);
                $smarty->assign('goods_list', $goods_list);
...
');
                            ...
                            foreach ($order_sn_list as $order_sn) {
                                ...
                                if (0 < $order['ru_id']) {
                                    $date = array('order_id');
                                    $order_child = count(get_table_date('order_info',
                                        'main_order_id = \'' . $order['order_id'] . '\'', $date, 1));
                                ...
                                $order['order_time'] = local_date($_CFG['time_format'], $order['add_time']);
                                $order['pay_time'] = 0 < $order['pay_time'] ? local_date($_CFG['time_format'],
                                    $order['pay_time']) : $_LANG['ps'][PS_UNPAYED];
                                $order['shipping_time'] = 0 < $order['shipping_time'] ? local_date($_CFG['time_format'],
                                    $order['shipping_time']) : $_LANG['ss'][SS_UNSHIPPED];
                                ...
                                while ($row = $db->fetchRow($res)) {
                                    $products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'],
                                        $row['ru_id'], $row['warehouse_id'], $row['area_id'], $row['model_attr']);
                    ...
                    if ($_REQUEST['act'] == 'pay_order') {
                        ...
                        if (0 < $order_id) {
                            ...
                            if ($pay_status == 2) {
                                $result['error'] = 1;
                                $result['msg'] = $_LANG['not_pay_repeat'];
                            } else {
