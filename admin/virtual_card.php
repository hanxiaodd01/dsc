...
if ($_REQUEST['act'] == 'replenish') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['go_list'], 'href' => 'virtual_card.php?act=card&goods_id=' . $card['goods_id']));
    ...
    if ($_REQUEST['act'] == 'edit_replenish') {
        ...
        if (($card['crc32'] == 0) || ($card['crc32'] == crc32(AUTH_KEY))) {
            ...
            if ($card['crc32'] == crc32(OLD_AUTH_KEY)) {
        ...
        $data_time = array(
            'year' => local_date('Y', $card['end_date']),
            'month' => local_date('m', $card['end_date']),
            'day' => local_date('d', $card['end_date'])
        );
        $smarty->assign('data_year', $data_year);
        $smarty->assign('data_month', $data_month);
        $smarty->assign('data_day', $data_day);
        ...
        if ($_REQUEST['act'] == 'action') {
            ...
            if ($_POST['card_sn'] != $_POST['old_card_sn']) {
                ...
                if (0 < $db->GetOne($sql)) {
                    $link[] = array(
                        'text' => $_LANG['go_back'],
                        'href' => 'virtual_card.php?act=replenish&goods_id=' . $_POST['goods_id']
                    );
            ...
            if (empty($_POST['old_card_sn'])) {
                ...
                $link[] = array(
                    'text' => $_LANG['go_list'],
                    'href' => 'virtual_card.php?act=card&goods_id=' . $_POST['goods_id']
                );
                $link[] = array(
            ...
            if ($_REQUEST['act'] == 'card') {
                ...
                if ($_REQUEST['act'] == 'query_card') {
                    ...
                    if ($_REQUEST['act'] == 'batch_drop_card') {
                        ...
                        $sql = 'DELETE FROM ' . $ecs->table('virtual_card') . ' WHERE card_id ' . db_create_in(implode(',',
                                $_POST['checkboxes']));
                        ...
                        if ($db->query($sql)) {
                            ...
                            $link[] = array(
                                'text' => $_LANG['go_list'],
                                'href' => 'virtual_card.php?act=card&goods_id=' . $_REQUEST['goods_id']
                            );
                            sys_msg($_LANG['action_success'], 0, $link);
                        }
                    } else {
                        if ($_REQUEST['act'] == 'batch_card_add') {
                            ...
                            $smarty->assign('action_link', array(
                                'text' => $_LANG['virtual_card_list'],
                                'href' => 'goods.php?act=list&extension_code=virtual_card'
                            ));
                            $smarty->assign('goods_id', $_REQUEST['goods_id']);
                            ...
                            if ($_REQUEST['act'] == 'batch_confirm') {
                                ...
                                $smarty->assign('action_link', array(
                                    ...
                                    'href' => 'virtual_card.php?act=batch_card_add&goods_id=' . $_REQUEST['goods_id']
                                ));
                                        ...
                                        if ($_REQUEST['act'] == 'submit_change') {
                                            ...
                                            if (isset($_POST['old_string']) && isset($_POST['new_string'])) {
                                                ...
                                                while ($row = $db->fetchRow($res)) {
                                                    $row['card_sn'] = encrypt(decrypt($row['card_sn'],
                                                        $_POST['old_string']), $_POST['new_string']);
                                                    $row['card_password'] = encrypt(decrypt($row['card_password'],
                                                        $_POST['old_string']), $_POST['new_string']);
                                                    $row['crc32'] = $new_crc32;
                                                    $db->autoExecute($ecs->table('virtual_card'), $row, 'UPDATE',
                                                        'card_id = ' . $row['card_id']);
                                                }

                                                sys_msg($_LANG['change_key_ok'], 0, array(
                                                    array(
                                                        'href' => 'virtual_card.php?act=list',
                                                        'text' => $_LANG['virtual_card_list']
                                                    )
                                                ));
                                            }
                                            ...
                                            if ($_REQUEST['act'] == 'toggle_sold') {
                                                ...
                                                if ($_REQUEST['act'] == 'remove_card') {
                                                    ...
                                                    if ($db->query($sql, 'SILENT')) {
                                                        ...
                                                        $url = 'virtual_card.php?act=query_card&' . str_replace('act=remove',
                                                    ...
                                                    if ($_REQUEST['act'] == 'start_change') {
                                                        ...
                                                        if ($old_key != AUTH_KEY) {
                                                            ...
                                                            file_put_contents($f,
                                                                str_replace('\'AUTH_KEY\', \'' . AUTH_KEY . '\'',
                                                                    '\'AUTH_KEY\', \'' . $new_key . '\'',
                                                                    file_get_contents($f)));
                                                            file_put_contents($f,
                                                                str_replace('\'OLD_AUTH_KEY\', \'' . OLD_AUTH_KEY . '\'',
                                                                    '\'OLD_AUTH_KEY\', \'' . $old_key . '\'',
                                                                    file_get_contents($f)));
                                                            @fclose($fp);
                                                        }
                                                        ...
                                                        make_json_result(sprintf($GLOBALS['_LANG']['old_stat'],
                                                            $stat['all'], $stat['new'], $stat['old'],
                                                            $stat['unknown']));
                                                    } else {
                                                        if ($_REQUEST['act'] == 'on_change') {
                                                            ...
                                                            while ($row = $db->fetchRow($res)) {
                                                                ...
                                                                if (!$db->autoExecute($ecs->table('virtual_card'), $row,
                                                                    'UPDATE', 'card_id = ' . $row['card_id'])) {
                                                                    make_json_error($updated, 0,
                                                                        $_LANG['update_error'] . "\n" . $db->error());
                                                                }
                                                            ...
                                                            if (0 < $left_num) {
                                                                ...
                                                                make_json_result($updated,
                                                                    sprintf($_LANG['new_stat'], $stat['new'],
                                                                        $stat['unknown']));
                                                            }
                                                        }
                                                    }
