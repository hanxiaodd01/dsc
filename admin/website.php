...
function getWebsiteList()
{
    ...
    while ($file = readdir($openfn)) {
        if (($file != '.') && ($file != '..') && ($file != 'jntoo.php') && ($file != 'config') && ($file != 'tb_callback.php') && ($file != 'tb_index.php') && (substr($file,
                    strlen($file) - 4) == '.php') && (substr($file, 0, 3) != 'cls')) {
    ...
    if ($_REQUEST['act'] == 'batch') {
        ...
        switch ($type) {
            case 'create':
                ...
                $contents = 'new website( "' . $url . '" , "' . join('|', $t) . '" , "' . join('|',
                        $className) . '" , ' . ($show_name ? '"' . join('|',
                            $name) . '"' : 'false') . ' , ' . ($show_title ? '"' . $_LANG['qita'] . '"' : 'false') . ',' . ($show_help ? '"' . join('|',
                            $help) . '"' : 'false') . ' , ' . ($is_open ? 'true' : 'false') . ' ,"{$back_act}", "' . $date . '" )';
                $smarty->assign('evaljavascript', $contents);
                $smarty->assign('scriptsrc', $url . 'js/website.js');
                assign_query_info();
                $smarty->assign('action_link',
                    array('href' => 'website.php?act=list', 'text' => $_LANG['webstte_list']));
