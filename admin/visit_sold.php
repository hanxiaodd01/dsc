...
function click_sold_info($ru_id, $cat_id, $brand_id, $show_num)
{
    global $db;
    global $ecs;
    $where = ' WHERE o.order_id = og.order_id AND g.goods_id = og.goods_id ' . $ruCat . order_query_sql('finished',
            'o.');
...
if (($_REQUEST['act'] == 'list') || ($_REQUEST['act'] == 'download')) {
    ...
    $smarty->assign('action_link', array(
        'text' => $_LANG['download_visit_buy'],
        'href' => 'visit_sold.php?act=download&show_num=' . $show_num . '&cat_id=' . $cat_id . '&brand_id=' . $brand_id . '&show_num=' . $show_num
    ));
    assign_query_info();
