...
function category_get_goods(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($display == 'grid') {
            $arr[$row['goods_id']]['goods_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
...
$cache_id = sprintf('%X',
    crc32($cat_id . '-' . $display . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' . $_CFG['lang'] . '-' . $brand . '-' . $price_max . '-' . $price_min . '-' . $filter_attr_str . '-' . $ship . '-' . $self . '-' . $have));
...
if (isset($_REQUEST['act']) && $_REQUEST['act'] == 'load_more_goods' || !$smarty->is_cached($dwt_name . '.dwt',
        $cache_id)) {
...
');
    ...
    if (($cat_row['parent_id'] == 0 && $cat_row['is_top_style'] == 1) == false) {
        ...
        if (empty($g_price) && (0 < $price_min || 0 < $price_max)) {
            ...
            $parray = array();
            $parray['purl'] = build_uri('category', array(
                'cid' => $cat_id,
                'bid' => $brand,
                'price_min' => 0,
                'price_max' => 0,
    ...
    if ($cat_row['parent_id'] == 0 && $cat_row['is_top_style'] == 1) {
            ...
            if ($cat_row['top_style_tpl'] == 2) {
                ...
                foreach ($cats as $key => $val) {
                    foreach ($cats[$key]['children'] as $_k => $_v) {
                        $cats[$key]['children'][$_k]['goods_detail'] = get_cat_id_goods_list($_v['id'], $region_id,
                            $area_id, $area_city, 10);
        ...
        $cate_top_new_goods = get_category_recommend_goods('new', $children, $brand, $price_min, $price_max, $ext,
            $region_id, $area_id, $area_city, $topStyle['new']);
        $smarty->assign('cate_top_new_goods', $cate_top_new_goods);
        $cate_top_hot_goods = get_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext,
            $region_id, $area_id, $area_city, $topStyle['hot']);
        $smarty->assign('cate_top_hot_goods', $cate_top_hot_goods);
        $smarty->assign('cate_top_promote_goods',
            get_category_recommend_goods('promote', $children, $brand, $price_min, $price_max, $ext, $region_id,
                $area_id, $area_city, $topStyle['promote']));
        $cate_top_best_goods = get_category_recommend_goods('best', $children, $brand, $price_min, $price_max, $ext,
            $region_id, $area_id, $area_city, $topStyle['best']);
        ...
        $havealook = get_category_recommend_goods('rand', $children, $brand, $price_min, $price_max, $ext, $region_id,
            $area_id, $area_city, $start);
        ...
        $smarty->assign('best_goods',
            get_category_recommend_goods('best', $children, $brand, $price_min, $price_max, $ext, $region_id, $area_id,
                $area_city, $best_size, $best_num));
        $smarty->assign('promotion_goods',
            get_category_recommend_goods('promote', $children, $brand, $price_min, $price_max, $ext, $region_id,
                $area_id, $area_city, $best_size, $best_num));
        $smarty->assign('hot_goods',
            get_category_recommend_goods('hot', $children, $brand, $price_min, $price_max, $ext, $region_id, $area_id,
                $area_city, $best_size, $best_num));
        if (0 < $_COOKIE['cagtegory_goods_count' . $cat_id] && 1 < $page) {
            $count = $_COOKIE['cagtegory_goods_count' . $cat_id];
        } else {
            $count = get_cagtegory_goods_count($children, $brand, $price_min, $price_max, $ext, $region_id, $area_id,
                $area_city, $where_ext);
            setcookie('cagtegory_goods_count' . $cat_id, $count, gmtime() + 2 * 3600, $GLOBALS['cookie_path'],
        ...
        $goodslist = category_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order,
            $region_id, $area_id, $area_city, $where_ext, $region);
        ...
        assign_pager('category', $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max,
            $display, $filter_attr_str, '', '', '', '', $ubrand, '', $ship, $self, $have);
    }
