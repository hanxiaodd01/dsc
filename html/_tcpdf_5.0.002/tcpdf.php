...
if (!class_exists('TCPDF', false)) {
    ...
    class TCPDF
    {
            ...
            if ($this->sign) {
                ...
                if (empty($this->signature_data['extracerts'])) {
                    openssl_pkcs7_sign($tempdoc, $tempsign, $this->signature_data['signcert'],
                        array($this->signature_data['privkey'], $this->signature_data['password']), array(),
            ...
            if ($arrcode !== false) {
                foreach ($arrcode['bcode'] as $k => $v) {
                    ...
                    if ($v['t']) {
                        ...
                        $this->Rect($xpos, $ypos, $bw, ($v['h'] * $barh / $arrcode['maxh']), 'F', array(),
                            $style['fgcolor']);
                    }
                    $xpos += $bw;
                }
                                    ...
                                    if (($this->CurrentFont['type'] == 'TrueTypeUnicode') OR ($this->CurrentFont['type'] == 'cidfont0')) {
                                        ...
                                        $pmid = preg_replace_callback("/\[\(([^\)]*)\)\]/x",
                                            function ($matches, $spacew) {
                                                ...
                                                return "[(" . str_replace(chr(0) . chr(32),
                                                        ") " . sprintf("%.3F", $spacew) . " (", $matches[1]) . ")]";
                                            }, $pmidtemp);
        ...
        protected function startSVGElementHandler($parser, $name, $attribs)
        {
            ...
            if ($this->svgdefsmode AND !in_array($name,
                    array('clipPath', 'linearGradient', 'radialGradient', 'stop'))) {
            ...
            switch ($name) {
                ...
                case 'path':
                {
                    ...
                    if ($clipping) {
                        ...
                        $this->StartTransform();
                        $this->SVGTransform($tm);
                        $obstyle = $this->setSVGStyles($svgstyle, $prev_svgstyle, 0, 0, 1, 1, 'SVGPath',
                            array($d, 'CNZ'));
                        if (!empty($obstyle)) {
