...
if (!defined('__CLASS_HTML2PDF__')) {
    ...
    class HTML2PDF
    {
        ...
        protected function _DEBUG_add($name, $level = null)
        {
            ...
            $this->_DEBUG_stepline($name, number_format(($time - $this->_debugStartTime) * 1000, 1, '.', ' ') . ' ms',
                number_format(($time - $this->_debugLastTime) * 1000, 1, '.', ' ') . ' ms',
                number_format($usage / 1024, 1, '.', ' ') . ' Ko', number_format($peak / 1024, 1, '.', ' ') . ' Ko');
            $this->_debugLastTime = $time;
        ...
        {
            $txt = str_pad($name, 30, ' ', STR_PAD_RIGHT) . str_pad($timeTotal, 12, ' ',
                    STR_PAD_LEFT) . str_pad($timeStep, 12, ' ', STR_PAD_LEFT) . str_pad($memoryUsage, 15, ' ',
                    STR_PAD_LEFT) . str_pad($memoryPeak, 15, ' ', STR_PAD_LEFT);
        ...
        {
            $pdf = null;
            HTML2PDF::$_subobj = new HTML2PDF($this->_orientation, $this->_format, $this->_langue, $this->_unicode,
                $this->_encoding,
        ...
        protected function _createSubHTML(&$subHtml, $cellmargin = 0)
        {
            ...
            $subHtml->initSubHtml($this->_format, $this->_orientation, $marge, $this->_page, $this->_defList,
                $this->pdf->getMyLastPageGroup(), $this->pdf->getMyLastPageGroupNb());
        }
        ...
        protected function _tag_open_DIV($param, $other = 'div')
        {
            ...
            $this->_drawRectangle($this->parsingCss->value['x'], $this->parsingCss->value['y'],
                $this->parsingCss->value['width'], $this->parsingCss->value['height'],
                $this->parsingCss->value['border'], $this->parsingCss->value['padding'], 0,
            ...
            if ($overflow) {
                ...
                $this->pdf->clippingPathStart($this->parsingCss->value['x'] + $marge['l'],
                    $this->parsingCss->value['y'] + $marge['t'], $this->parsingCss->value['width'],
        ...
        protected function _tag_open_WRITE($param)
        {
            ...
            if (count($words)) {
                ...
                $this->pdf->setXY($this->pdf->getX(), $y + $dh + $dy);
                $this->pdf->Cell($align == 'L' ? $w : $this->parsingCss->value['width'], $h, $txt, 0, 0, $align, $fill,
                    $this->_isInLink);
                $this->pdf->setXY($this->pdf->getX(), $y);
                $this->_maxH = max($this->_maxH, $lh);
        ...
        protected function _tag_open_TABLE($param, $other = 'table')
        {
            ...
            if ($this->_subPart) {
                ...
                $this->_drawRectangle(HTML2PDF::$_tables[$param['num']]['curr_x'],
                    HTML2PDF::$_tables[$param['num']]['curr_y'], HTML2PDF::$_tables[$param['num']]['width'],
                ...
                if (!$this->_isInTfoot && (($this->pdf->getH() - $this->pdf->getbMargin()) < (HTML2PDF::$_tables[$param['num']]['td_y'] + HTML2PDF::$_tables[$param['num']]['marge']['b'] + $ty + $hfoot))) {
                    ...
                    if (isset(HTML2PDF::$_tables[$param['num']]['height'][HTML2PDF::$_tables[$param['num']]['page']])) {
                        ...
                        $this->_drawRectangle(HTML2PDF::$_tables[$param['num']]['curr_x'],
                            ...
                            $this->parsingCss->value['border'], $this->parsingCss->value['padding'],
        ...
        protected function _tag_open_TD($param, $other = 'td')
        {
            ...
            if ($this->_subPart) {
                ...
                for ($j = 0; $j < $rowspan; $j++) {
                    for ($i = 0; $i < $colspan; $i++) {
                        HTML2PDF::$_tables[$param['num']]['corr'][HTML2PDF::$_tables[$param['num']]['corr_y'] + $j][HTML2PDF::$_tables[$param['num']]['corr_x'] + $i] = 0 < ($i + $j) ? '' : array(
                            $x,
                            $y,
                            $colspan,
                            $rowspan
                ...
                $this->_drawRectangle(HTML2PDF::$_tables[$param['num']]['td_x'],
                    HTML2PDF::$_tables[$param['num']]['td_y'], HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['w'],
                    HTML2PDF::$_tables[$param['num']]['cases'][$y][$x]['h'], $this->parsingCss->value['border'],
                    $this->parsingCss->value['padding'], HTML2PDF::$_tables[$param['num']]['cellspacing'] * 0.5,
                    $this->parsingCss->value['background']);
        ...
        protected function _tag_open_INPUT($param)
        {
            ...
            switch ($param['type']) {
                case 'checkbox':
                    ...
                    $this->pdf->CheckBox($name, $w, $checked, $prop, array(), $param['value'] ? $param['value'] : 'Yes',
                        $x, $y);
                    break;
                ...
                case 'radio':
                    ...
                    $checked = isset($param['checked']) && ($param['checked'] == 'checked');
                    $this->pdf->RadioButton($name, $w, $prop, array(), $param['value'] ? $param['value'] : 'On',
                        $checked, $x, $y);
                    break;
