...
class HTML2PDF_parsingCss
{
    ...
    public function analyse($tagName, &$param, $legacy = null)
    {
        ...
        if ($noWidth && in_array($tagName,
                array('div', 'blockquote', 'fieldset')) && ($this->value['position'] != 'absolute')) {
    ...
    protected function _getReccursiveStyle($key, $lst, $next = null)
    {
        ...
        foreach ($lst[0] as $name) {
            ...
            if ((substr($key, 0 - strlen(' ' . $name)) == (' ' . $name)) && $this->_getReccursiveStyle($key, $lst,
                    $name)) {
                return true;
            }
        }
    ...
    public function convertToColor($css, &$res)
    {
        ...
        if (preg_match('/rgb\\([\\s]*([0-9%\\.]+)[\\s]*,[\\s]*([0-9%\\.]+)[\\s]*,[\\s]*([0-9%\\.]+)[\\s]*\\)/isU', $css,
            $match)) {
