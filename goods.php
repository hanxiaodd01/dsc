...
function get_linked_articles($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        $row['url'] = $row['open_type'] != 1 ? build_uri('article', array('aid' => $row['article_id']),
            $row['title']) : trim($row['file_url']);
        $row['add_time'] = local_date($GLOBALS['_CFG']['date_format'], $row['add_time']);
        $row['short_title'] = 0 < $GLOBALS['_CFG']['article_title_length'] ? sub_str($row['title'],
...
function get_also_bought($goods_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $arr[$key]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
            $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        ...
        if (0 < $row['promote_price']) {
            $arr[$key]['promote_price'] = bargain_price($row['promote_price'], $row['promote_start_date'],
...
{
    $sql = 'SELECT SUM(attr_price) FROM ' . $GLOBALS['ecs']->table('goods_attr') . (' WHERE goods_id=\'' . $goods_id . '\' AND ') . db_create_in($attr,
            'goods_attr_id');
...
function get_package_goods_list($goods_id)
{
    ...
    foreach ($res as $tempkey => $value) {
        ...
        $sql = 'SELECT ga.goods_attr_id, ga.attr_value
                ...
                AND a.attr_type = 1
                AND ' . db_create_in($goods_id_array,
                'goods_id') . ' ORDER BY a.sort_order, a.attr_id, ga.goods_attr_id';
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price') {
    ...
    if ($goods_id == 0) {
        ...
        $products = get_warehouse_id_attr_number($goods_id, $_REQUEST['attr'], $goods['user_id'], $warehouse_id,
            $area_id, $area_city);
        ...
        if ($attr_id) {
            $spec_price = get_final_price($goods_id, $number, true, $attr_id, $warehouse_id, $area_id, $area_city, 1, 0,
                0, $res['show_goods'], $product_promote_price);
        }
        ...
        $res['onload'] = $onload;
        $limit = 1;
        $area_position_list = get_goods_user_area_position($goods['user_id'], $city_id, $_REQUEST['attr'], $goods_id, 0,
            0, 1, 0, $limit);
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'in_stock') {
    ...
    setcookie('street_area', $street_list, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'in_warehouse') {
    ...
    $area_region = 0;
    setcookie('area_region', $area_region, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
    $res['goods_id'] = $goods_id;
    $json = new JSON();
    exit($json->encode($res));
...
');
        ...
        $smarty->assign('keywords',
            !empty($goods['keywords']) ? htmlspecialchars($goods['keywords']) : htmlspecialchars($_CFG['shop_keywords']));
        $smarty->assign('description',
            !empty($goods['goods_brief']) ? htmlspecialchars($goods['goods_brief']) : htmlspecialchars($_CFG['shop_desc']));
        ...
        if ($_CFG['two_code']) {
            ...
            $logo = empty($_CFG['two_code_logo']) ? $goods['goods_img'] : str_replace('../', '',
                $_CFG['two_code_logo']);
            $size = '200x200';
            $url = $ecs->url();
            ...
            if (!file_exists(ROOT_PATH . $filename)) {
                ...
                if ($logo !== false) {
                    ...
                    imagecopyresampled($QR, $logo, $from_width, $from_width, 0, 0, $logo_qr_width, $logo_qr_height,
        ...
        $smarty->assign('best_goods',
            get_recommend_goods('best', '', $region_id, $area_id, $area_city, $goods['user_id'], 1));
        $smarty->assign('new_goods',
            get_recommend_goods('new', '', $region_id, $area_id, $area_city, $goods['user_id'], 1));
        $smarty->assign('hot_goods',
            get_recommend_goods('hot', '', $region_id, $area_id, $area_city, $goods['user_id'], 1));
        ...
        $basic_info['province'] = get_table_date('region', 'region_id = \'' . $basic_info['province'] . '\'',
            $basic_date, 2);
        ...
        if ($goods['stages']) {
            foreach ($goods['stages'] as $k => $v) {
                $stages_arr[$v]['stages_one_price'] = round(($goods['shop_price'] + $shippingFee['shipping_fee']) * ($goods['stages_rate'] / 100) + ($goods['shop_price'] + $shippingFee['shipping_fee']) / $v,
                    2);
            }
...
if (!empty($_COOKIE['ECS']['history'])) {
    ...
    setcookie('ECS[history]', implode(',', $history), gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
} else {
...
if (!empty($_COOKIE['ECS']['list_history'])) {
    ...
    setcookie('ECS[list_history]', implode(',', $list_history), gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
} else {
    setcookie('ECS[list_history]', $goods_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
}
...
foreach ($seo as $key => $value) {
    $seo[$key] = str_replace(array('{sitename}', '{key}', '{name}', '{description}'),
        array($_CFG['shop_name'], $goods['keywords'], $goods['goods_name'], $goods['goods_brief']), $value);
}
