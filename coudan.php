...
if ($_REQUEST['act'] == 'index') {
    ...
    $sort = isset($_REQUEST['sort']) && in_array(trim(strtolower($_REQUEST['sort'])),
        array('goods_id', 'sales_volume', 'shop_price')) ? trim($_REQUEST['sort']) : 'goods_id';
    $order = isset($_REQUEST['order']) && in_array(trim(strtoupper($_REQUEST['order'])),
        array('ASC', 'DESC')) ? trim($_REQUEST['order']) : 'DESC';
    ...
    $pager = get_pager('coudan.php', array('id' => $active_id, 'sort' => $sort, 'order' => $order), $count, $page,
        $size);
    ...
    $favourable_goods_list = favourable_goods_list($_SESSION['user_rank'], $active_id, $sort, $order, $size, $page,
        $region_id, $area_info['region_id'], $area_city);
    ...
    if ($_REQUEST['act'] == 'ajax_update_cart') {
        ...
        if ($_REQUEST['act'] == 'delete_cart_fav_goods') {
