...
function convert_table($table)
{
    ...
    if ($convert_tables[$table] == 1) {
        ...
        if ($query) {
            ...
            $createtable = preg_replace('/CREATE TABLE `' . $table . '_bak`/i', 'CREATE TABLE `' . $table . '`',
                $createtable);
    ...
    if ($convert_tables[$table] == 2) {
        ...
        if ($count < $start + $rpp) {
            ...
            <input type="hidden" name="table_name" value="' . $tables_keys[0] . '" />';
            showmessage('正在转换数据表 ' . $table . ' 的第 ' . $start . ' - ' . ($count < $start + $rpp ? $count : $start + $rpp) . ' 条数据',
                '?step=start', 'form', $extra);
        }
    }
}
