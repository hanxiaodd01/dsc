...
function update_structure_automatically($next_ver, $cur_pos)
{
    ...
    $db_charset = strtolower(str_replace('-', '', EC_CHARSET));
    $se = new sql_executor($db, $db_charset, 'ecs_', $prefix, ROOT_PATH . 'data/upgrade_' . $next_ver . '.log',
...
function url()
{
    ...
    $curr = strpos(PHP_SELF, 'upgrade/') !== false ? preg_replace('/(.*)(upgrade)(\\/?)(.)*/i', '\\1',
...
function write_charset_config($lang, $charset)
{
    ...
    $s = insertconfig($s, '/\\?\\>/', '');
    $s = insertconfig($s, '/define\\(\'EC_TEMPLATE\',\\s*\'.*?\'\\);/i',
        'define(\'EC_TEMPLATE\', \'ecmoban_dsc2017\');');
    $s = insertconfig($s, '/define\\(\'EC_LANGUAGE\',\\s*\'.*?\'\\);/i', 'define(\'EC_LANGUAGE\', \'' . $lang . '\');');
    $s = insertconfig($s, '/define\\(\'EC_CHARSET\',\\s*\'.*?\'\\);/i',
        'define(\'EC_CHARSET\', \'' . $charset . '\');');
    $s = insertconfig($s, '/\\?\\>/', '?>');
...
function save_uc_config($config)
{
    ...
    global $ecs;
    $success = false;
    list($appauthkey, $appid, $ucdbhost, $ucdbname, $ucdbuser, $ucdbpw, $ucdbcharset, $uctablepre, $uccharset, $ucapi, $ucip) = explode('|',
        $config);
    $config_file = ROOT_PATH . 'data/config.php';
    $s = file_get_contents($config_file);
    $s = insertconfig($s, '/\\?\\>/', '');
