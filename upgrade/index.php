...
switch ($step) {
        ...
        if (empty($ucconfig)) {
            ...
            if ($ucconfig == '-1') {
                ...
                if (empty($appauthkey) || empty($appid)) {
                    ...
                    if ($succeed = save_uc_config($ucconfig . ('|' . $ucapi . '|' . $ucip))) {
                        $result['error'] = 0;
                        $result['message'] = 'OK';
                    } else {
                        $result['error'] = 1;
    ...
    case 'userimporttouc':
        ...
        while ($data = $db->fetch_array($query)) {
            ...
            if (!$uc_userinfo) {
                $ucdb->query('INSERT LOW_PRIORITY INTO ' . UC_DBTABLEPRE . ('members SET uid=\'' . $lastuid . '\', username=\'' . $data['username'] . '\', password=\'' . $password . '\', email=\'' . $data['email'] . '\', regip=\'' . $data['regip'] . '\', regdate=\'' . $data['regdate'] . '\', salt=\'' . $salt . '\''),
                    'SILENT');
                $ucdb->query('INSERT LOW_PRIORITY INTO ' . UC_DBTABLEPRE . ('memberfields SET uid=\'' . $lastuid . '\''),
                    'SILENT');
            } else {
                if ($merge_method == 1) {
                ...
                $ucdb->query('REPLACE INTO ' . UC_DBTABLEPRE . 'mergemembers SET appid=\'' . UC_APPID . ('\', username=\'' . $data['username'] . '\''),
                    'SILENT');
                $repeat_user[] = $data;
            }
        }
