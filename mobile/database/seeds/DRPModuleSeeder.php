...
3、开微店收入来源之三：您的下级分销商邀请他的朋友注册微店后，他朋友的下线会员购买分销商品，您即可获得“三级分销佣金”。',
        ...
        $result_isdistributionr = \Illuminate\Support\Facades\DB::table('drp_config')->where('code',
            'isdistribution')->get();
        ...
        if (!empty($result_isdistributionr)) {
            \Illuminate\Support\Facades\DB::table('drp_config')->where('code',
                'isdistribution')->update(array('warning' => '开启分成模式从上级分销商开始分成，否则从当前下单分销商分成'));
        }
        ...
        if (!isset($result['item'][0]['credit_t'])) {
            \Illuminate\Support\Facades\DB::table('drp_config')->where('code',
                'drp_affiliate')->update(array('value' => serialize($value)));
        }
    }
