...
class OrderRuIdSeeder extends \Illuminate\Database\Seeder
{
    ...
    private function addFiled()
    {
        if (!\Illuminate\Support\Facades\Schema::hasColumn('order_info', 'ru_id')) {
            \Illuminate\Support\Facades\Schema::table('order_info',
    ...
    private function mainOrder()
    {
        ...
        if ($order_list) {
            foreach ($order_list as $key => $val) {
                \Illuminate\Support\Facades\DB::table('order_info')->where('order_id',
                    $val->order_id)->update(array('main_count' => $val->main_count));
            }
        }
    }
    ...
    private function order()
    {
        ...
        if ($order_list) {
            foreach ($order_list as $key => $val) {
                $ru_id = \Illuminate\Support\Facades\DB::table('order_goods')->where('order_id',
                    $val->order_id)->value('ru_id');
                \Illuminate\Support\Facades\DB::table('order_info')->where('order_id',
                    $val->order_id)->update(array('ru_id' => $ru_id));
            }
        }
    }
}
