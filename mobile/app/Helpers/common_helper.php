...
function load_ecsconfig()
{
    ...
    if ($data === false) {
        ...
        $arr['no_picture'] = !empty($arr['no_picture']) ? str_replace('../', './',
            $arr['no_picture']) : 'images/no_picture.gif';
...
function get_promotion_info($goods_id = '', $ru_id = 0)
{
    ...
    foreach ($res as $data) {
        switch ($data['act_type']) {
            case GAT_SNATCH:
                ...
                $snatch[$data['act_id']]['time'] = sprintf(L('promotion_time'),
            ...
            case GAT_GROUP_BUY:
                ...
                $group[$data['act_id']]['time'] = sprintf(L('promotion_time'), local_date('Y-m-d', $data['start_time']),
                    local_date('Y-m-d', $data['end_time']));
            ...
            case GAT_AUCTION:
                ...
                $auction[$data['act_id']]['time'] = sprintf(L('promotion_time'),
                    local_date('Y-m-d', $data['start_time']), local_date('Y-m-d', $data['end_time']));
            ...
            case GAT_PACKAGE:
                ...
                $package[$data['act_id']]['time'] = sprintf(L('promotion_time'),
                    local_date('Y-m-d', $data['start_time']), local_date('Y-m-d', $data['end_time']));
    ...
    if (empty($goods_id)) {
        foreach ($res as $rows) {
            ...
            $favourable[$rows['act_id']]['time'] = sprintf(L('promotion_time'),
            ...
            if ($rows['act_range'] == FAR_ALL) {
                ...
                if ($rows['act_range'] == FAR_CATEGORY) {
                    ...
                    if (strpos(',' . $ids . ',', ',' . $category_id . ',') !== false) {
                        $favourable[$rows['act_id']]['act_name'] = $rows['act_name'];
                        $favourable[$rows['act_id']]['url'] = url('activity/index/detail',
                            array('id' => $rows['act_id']));
                        $favourable[$rows['act_id']]['time'] = sprintf(L('promotion_time'),
                            local_date('Y-m-d', $rows['start_time']), local_date('Y-m-d', $rows['end_time']));
                        $favourable[$rows['act_id']]['sort'] = $rows['start_time'];
                        $favourable[$rows['act_id']]['type'] = 'favourable';
...
function cate_history($size, $page, $sort, $order, $warehouse_id = 0, $area_id = 0, $ship = 0, $self = 0)
{
    ...
    if (!empty($_COOKIE['ECS']['list_history'])) {
        ...
        foreach ($res as $row) {
            ...
            $arr[$row['goods_id']]['store_url'] = build_uri('merchants_store', array('urid' => $row['user_id']),
                $arr[$row['goods_id']]['rz_shopName']);
...
function get_store_shop_list(
    ...
    foreach ($res as $row) {
        ...
        $arr[$row['shop_id']]['shop_url'] = build_uri('merchants_store', array('cid' => 0, 'urid' => $row['user_id']),
...
function get_store_shop_goods_list($keywords = '', $size, $page, $sort, $order, $warehouse_id, $area_id)
{
    ...
    foreach ($res as $row) {
        ...
        $arr[$row['goods_id']]['shop_url'] = build_uri('merchants_store', array('cid' => 0, 'urid' => $row['user_id']),
            $arr[$key]['shop_name']);
...
function get_category_brands_ad($cat_id)
{
    ...
    $sql = 'SELECT b.brand_id, b.brand_name,  b.brand_logo, COUNT(*) AS goods_num, IF(b.brand_logo > \'\', \'1\', \'0\') AS tag ' . 'FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = b.brand_id AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods_cat') . ' AS gc ON g.goods_id = gc.goods_id ' . ('WHERE (' . $children . ' OR ') . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id),
            array_keys(cat_list($cat_id, 0,
                0))))) . ') AND b.is_show = 1 ' . 'GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC LIMIT 0,12';
...
function get_seo_words($type = '', $cat_id = 0)
{
    ...
    if ($type) {
        if ($type == 'category' && 0 < $cat_id) {
            $res = dao('category')->field('cate_title as title, cate_keywords as keywords, cate_description as description')->where(array('cat_id' => $cat_id))->cache(true,
                12 * 3600)->find();
        } else {
            $res = dao('seo')->field('title, keywords, description')->where(array('type' => $type))->cache(true,
                12 * 3600)->find();
        }
    }
...
function cloud_confirmorder($order_id)
{
    if (0 < $order_id) {
        ...
        if ($cloud_order) {
            ...
            $cloud_orders['notifyUrl'] = str_replace('/mobile/', '/',
                    $root_url) . 'api.php?app_key=' . C('shop.cloud_dsc_appkey') . '&method=dsc.order.confirmorder.post&format=json&interface_type=1';
