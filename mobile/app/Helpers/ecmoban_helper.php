    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        ...
        foreach ($res as $row) {
            ...
            $sql = 'SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ' . $brand_select . 'FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . (' AS g ON g.brand_id = b.brand_id AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . $brand_tag_where . ' ') . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods_cat') . ' AS gc ON g.goods_id = gc.goods_id ' . (' WHERE ' . $children . ' OR ') . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id),
                    array_keys(cat_list($cat_id, 0,
                        0))))) . ' AND b.is_show = 1 ' . ('GROUP BY b.brand_id HAVING goods_num > 0 ' . $where_having . ' ORDER BY b.sort_order, b.brand_id ASC');
            ...
            if (!empty($full[$i])) {
                ...
                if (0 < $cs_id && 0 < $nid) {
                    ...
                    if ($id_res < 1) {
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($table), $other, 'UPDATE',
                            'id=\'' . $id[$i] . '\'');
                    }
                }
            }
...
function get_user_default($user_id = 0)
{
    ...
    $info['bonus'] = sprintf(L('user_bonus_info'), $user_bonus['bonus_count'],
            ...
            while (!empty($arr)) {
                foreach ($arr as $key => $value) {
                    ...
                    if ($value['parent_cid'] == $last_cat_id) {
                        ...
                        if ($last_cat_id < $value['parent_cid']) {
                            break;
                        }
                    }
                }
...
function get_goods_user_area_position(
    $ru_id = 0,
    $city_id = 0,
    $spec_arr = '',
    $goods_id = 0,
    ...
    if ($store_list) {
        ...
        foreach ($store_list as $key => $row) {
            ...
            if (is_spec($is_spec) == true) {
                $products = get_warehouse_id_attr_number($row['goods_id'], $spec_arr, $row['ru_id'], 0, 0, 0, '',
                    $row['id']);
