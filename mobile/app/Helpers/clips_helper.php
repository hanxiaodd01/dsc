...
function get_collection_store_list($user_id, $record_count, $limit = '')
{
    ...
    foreach ($res as $key => $row) {
        ...
        if (4 <= $store_list[$key]['commentrank']) {
            ...
            if (3 < $store_list[$key]['commentrank']) {
        ...
        if (4 <= $store_list[$key]['commentServer']) {
            ...
            if (3 < $store_list[$key]['commentServer']) {
        ...
        if (4 <= $store_list[$key]['commentdelivery']) {
            ...
            if (3 < $store_list[$key]['commentdelivery']) {
...
function get_collection_store($user_id, $record_count, $page, $pageFunc, $size = 5)
{
    ...
    foreach ($res as $key => $row) {
        ...
        $store_list[$key]['url'] = build_uri('merchants_store', array('cid' => 0, 'urid' => $row['ru_id']),
            $store_list[$key]['store_name']);
        $store_list[$key]['merch_cmt'] = get_merchants_goods_comment($row['ru_id']);
        $store_list[$key]['hot_goods'] = get_user_store_goods_list($row['ru_id'], $region_id, $area_id, 'store_hot');
        $store_list[$key]['new_goods'] = get_user_store_goods_list($row['ru_id'], $region_id, $area_id, 'store_new');
    }
...
function get_user_store_goods_list(
    $user_id,
    $region_id,
    $area_id,
    $type = '',
    ...
    foreach ($res as $key => $row) {
        ...
        $goods_list[$row['goods_id']]['zconments'] = get_conments_stars($mc_all, $mc_one, $mc_two, $mc_three, $mc_four,
            $mc_five);
    }
...
{
    $sql = 'SELECT COUNT(*) FROM {pre}user_account  WHERE user_id = \'' . $user_id . '\'  AND process_type ' . db_create_in(array(
            SURPLUS_SAVE,
            SURPLUS_RETURN
        ));
    $total = $GLOBALS['db']->getOne($sql);
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('user_account') . (' WHERE user_id = \'' . $user_id . '\'') . ' AND process_type ' . db_create_in(array(
            SURPLUS_SAVE,
            SURPLUS_RETURN
...
function get_account_log_info($user_id, $log_id = 0)
{
    ...
    if (!empty($log_id)) {
        ...
        if ($rows['is_paid'] == 0 && $rows['process_type'] == 0) {
            $rows['handle'] = '<a class="btn-submit box-flex" href="' . url('user/account/pay',
                    array('id' => $rows['id'], 'pid' => $pay_id)) . '">' . L('pay') . '</a>';
        }
    }
...
function get_comment_list($user_id, $page_size, $start)
{
    ...
    if ($to_article) {
        $sql = 'SELECT article_id , title FROM ' . $GLOBALS['ecs']->table('article') . ' WHERE ' . db_create_in($to_article,
                'article_id');
...
function get_user_order_comment_list($user_id, $type = 0, $sign = 0, $order_id = 0, $size = 0, $start = 0)
{
    ...
    if ($type == 1) {
        ...
        foreach ($res as $row) {
            ...
            $row['impression_list'] = !empty($row['goods_product_tag']) ? explode(',',
                $row['goods_product_tag']) : array();
