...
function order_fee(
    ...
    if (0 < $store_id || $store_type) {
        ...
        if (!empty($coupons) && $coupons['cou_type'] == 5 && $total['shipping_fee'] == 0) {
            $total['shipping_fee_old'] = get_order_shipping_fee($cart_goods_list, $consignee, $cart_value,
                $shipping_list, $step);
...
function get_combo_goods_info($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_area)
{
    ...
    $prod = $GLOBALS['db']->getRow($sql);
    if (is_spec($spec) && !empty($prod)) {
        $product_info = get_products_info($goods_id, $spec, $warehouse_area['warehouse_id'], $warehouse_area['area_id'],
            $warehouse_area['area_city']);
    }
    ...
    $result['goods_price'] = get_final_price($goods_id, $num, true, $spec, $warehouse_area['warehouse_id'],
        $warehouse_area['area_id'], $warehouse_area['area_city'], 0, 0, $add_tocart);
        ...
        if (1 < count($currency_format)) {
            if ($row['parent_id'] == 0) {
                $goods_price = trim(get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id,
                    $row['warehouse_id'], $row['area_id'], $row['area_city'], 0, 0, $add_tocart), $currency_format[0]);
            }
            ...
            $goods_price = get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $row['warehouse_id'],
                $row['area_id'], $row['area_city'], 0, 0, $add_tocart);
...
function compute_discount($type = 0, $newInfo = array(), $use_type = 0, $ru_id = 0, $flow_type = CART_GENERAL_GOODS)
{
    ...
    $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('favourable_activity') . (' WHERE start_time <= \'' . $now . '\'') . (' AND end_time >= \'' . $now . '\'') . ' AND review_status = 3 ' . ' AND CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'' . ' AND act_type ' . db_create_in(array(
            FAT_DISCOUNT,
            FAT_PRICE
        ));
...
function return_order_info($ret_id, $order_sn = '', $order_id = 0)
{
    ...
    if (!empty($order)) {
        ...
        $order['address_detail'] = str_replace('[', '',
            str_replace(']', '', get_consignee_info($order['order_id'], $order['address'])));
...
function return_order($order_id = 0, $size = 10, $page = 1)
{
    ...
    foreach ($res as $row) {
        ...
        $row['refound_detail_url'] = url('user/refound/detail',
            array('ret_id' => $row['ret_id'], 'order_id' => $row['order_id']));
        $goods_list[] = $row;
    }
    ...
    if (1 == $action_type) {
        ...
        if (2 == $action_type) {
            ...
            if (0 < $refound_pay_points) {
                log_account_change($order['user_id'], 0, 0, 0, $refound_pay_points,
                    ' 订单退款，退回订单 ' . $order['order_sn'] . ' 购买的积分');
            }
            ...
            if ($order['pay_status'] != PS_UNPAYED) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_return'), $return_status, 'UPDATE',
                    'rec_id = \'' . $rec_id . '\'');
            }
            ...
            if (C('shop.use_storage') == '1') {
                ...
                if (C('shop.stock_dec_time') == SDT_SHIP) {
                    ...
                    if (C('shop.stock_dec_time') == SDT_PLACE) {
...
function return_integral_rank($ret_id = 0, $user_id = 0, $order_sn = 0, $rec_id = 0, $refound_pay_points = 0)
{
    ...
    if (!empty($return_integral)) {
        ...
        log_account_change($user_id, 0, 0, '-' . $return_integral['rank_integral'], '-' . $gave_custom_points,
            sprintf($lang_return_order_gift_integral, $order_sn), ACT_OTHER, 1);
        return null;
    }
}
    ...
    if ($res) {
        foreach ($res as $favourable) {
            ...
            foreach ($favourable['gift'] as $key => $value) {
                $goods = dao('goods')->field('goods_id, goods_thumb, user_id as ru_id')->where(array(
                    'goods_id' => $value['id'],
                    'is_on_sale' => 1,
                    'is_delete' => 0
                ))->find();
    ...
    if (0 < $team_id) {
        ...
        if ($res['team_num'] <= $team_count['num']) {
            ...
            if (is_dir(APP_WECHAT_PATH)) {
                ...
                foreach ($team_order as $key => $vo) {
                    ...
                    $url = __HOST__ . url('team/goods/teamwait',
                            array('team_id' => $team_id, 'user_id' => $vo['user_id']));
        ...
        if (is_dir(APP_WECHAT_PATH)) {
            if (0 < $team_parent_id) {
                ...
                $url = __HOST__ . url('team/goods/teamwait',
                        array('team_id' => $team_id, 'user_id' => $_SESSION['user_id']));
