...
{
    $url = dao('category')->field('cat_id , cat_icon, cat_name, cat_alias_name , parent_id')->where(array(
        'parent_id' => $tree_id,
        'is_show' => 1
    ))->ORDER('sort_order')->select();

    foreach ($url as $key => $value) {
        $category = dao('category')->field('cat_id ,cat_name, cat_alias_name , parent_id')->where(array(
        ...
        foreach ($category as $k => $val) {
            $category[$k] = array(
                ...
                'cat_alias_name' => $val['cat_alias_name'],
                'url' => url('category/index/products', array('id' => $val['cat_id'])),
        ...
        $url[$key] = array(
            ...
            'cat_alias_name' => $value['cat_alias_name'],
            'url' => url('category/index/products', array('id' => $value['cat_id'])),
...
function group_buy_info($group_buy_id, $current_num = 0)
{
    ...
    $group_buy['store_url'] = build_uri('merchants_store', array('urid' => $group_buy['user_id']),
        $group_buy['rz_shopName']);
...
function goods_info($goods_id, $warehouse_id, $area_id, $select = array(), $attr_id = '', $area_city = 0)
{
    ...
    if (!empty($row)) {
        ...
        $row['goods_price'] = get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $warehouse_id,
            $area_id, $area_city, 0, 0, $add_tocart);
        $row['goods_weight'] = 0 < intval($row['goods_weight']) ? $row['goods_weight'] . L('kilogram') : $row['goods_weight'] * 1000 . L('gram');
        $row['goods_img'] = get_image_path($row['goods_img']);
...
function get_goods_fittings(
    ...
    foreach ($res as $row) {
        ...
        $minMax_price = get_goods_minMax_price($row['goods_id'], $warehouse_id, $area_id, $area_city,
            $row['goods_price'], $row['market_price']);
...
function get_goods_fittings_info(
    ...
    foreach ($res as $row) {
        ...
        $minMax_price = get_goods_minMax_price($row['goods_id'], $warehouse_id, $area_id, $area_city, $goods_price,
            $row['market_price']);
        $arr[$temp_index]['fittings_minPrice'] = $minMax_price['goods_min'];
        $arr[$temp_index]['fittings_maxPrice'] = $minMax_price['goods_max'];
        $arr[$temp_index]['market_minPrice'] = $minMax_price['market_min'];
        ...
        $attr_number = !empty($attr_number) ? $attr_number : 0;
        $arr[$temp_index]['goods_number'] = $attr_number;
        $arr[$temp_index]['properties'] = get_goods_properties($goods_id, $warehouse_id, $area_id, 0,
            $row['goods_attr_id']);
        $temp_index++;
    }
