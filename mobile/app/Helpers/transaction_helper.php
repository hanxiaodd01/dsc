...
function get_user_orders($user_id, $num = 10, $page = 1, $status = 0)
{
    ...
    foreach ($res as $key => $row) {
        ...
        $orderInfo = get_table_date('order_info',
            'order_id = \'' . $row['order_id'] . ('\' and user_id = \'' . $user_id . '\''), $date);
        ...
        if ($GLOBALS['_CFG']['open_delivery_time'] == 1) {
            if ($orderInfo['order_status'] == 5 && $orderInfo['shipping_status'] == 1 && $orderInfo['pay_status'] == 2) {
                ...
                if ($delivery_time < $noTime) {
                    ...
                    order_action($orderInfo['order_sn'], OS_SPLITED, SS_RECEIVED, PS_PAYED, $note, L('buyer'), 0,
                        gmtime());
                }
            }
        }
                ...
                if ($row['shipping_status'] == SS_SHIPPED) {
                    ...
                    if ($row['shipping_status'] == SS_RECEIVED) {
                        ...
                        if ($row['pay_status'] == PS_UNPAYED) {
                            @$row['handler'] = '<a class="btn-default-new br-5" href="' . url('user/order/detail',
        ...
        if (0 < $store_id && $row['shipping_status'] == SS_SHIPPED && $row['pay_status'] == PS_PAYED) {
            @$row['handler'] = '<a class="btn-default-new br-5 min-btn" href="' . url('user/order/affirmreceived',
