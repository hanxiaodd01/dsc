...
function get_ip_area_name($ip = '', $api = '新浪')
{
    ...
    if (strstr($area_name, '香港')) {
        ...
        if (strstr($area_name, '澳门')) {
            $area_name = '澳门';
        } else {
            if (strstr($area_name, '内蒙古')) {
                $area_name = '内蒙古';
            } else {
                if (strstr($area_name, '宁夏')) {
                    $area_name = '宁夏';
                } else {
                    if (strstr($area_name, '新疆')) {
                        $area_name = '新疆';
                    } else {
                        if (strstr($area_name, '西藏')) {
                            $area_name = '西藏';
                        } else {
                            if (strstr($area_name, '广西')) {
                                $area_name = '广西';
                            }
                        }
                    }
    ...
    if ($type == 'finished') {
        ...
        if ($type == 'await_ship') {
            return ' AND   ' . $alias . 'order_status ' . db_create_in(array(
                    ...
                    PS_PAYED,
                    PS_PAYING
                )) . ' OR ' . $alias . 'pay_id ' . db_create_in(get_payment_id_list(true)) . ') ';
        } else {
            if ($type == 'await_pay') {
                return ' AND   ' . $alias . 'order_status ' . db_create_in(array(
                        OS_CONFIRMED,
                        ...
                        SS_SHIPPED,
                        SS_RECEIVED
                    )) . ' OR ' . $alias . 'pay_id ' . db_create_in(get_payment_id_list(false)) . ') ';
            } else {
                if ($type == 'unconfirmed') {
                            ...
                            if ($type == 'shipped') {
                                ...
                                if ($type == 'to_confirm') {
                                    return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                                            ...
                                            OS_SPLITED,
                                            OS_SPLITING_PART
                                        )) . ' ' . ' AND ' . $alias . 'shipping_status = \'' . SS_SHIPPED . '\'' . ' AND ' . $alias . 'pay_status ' . db_create_in(array(
                                            PS_PAYED,
                                            PS_PAYING
                                        ));
                                } else {
    ...
    if ($dateType == 'VARCHAR') {
        ...
        if ($dateType == 'CHAR') {
            ...
            if ($dateType == 'INT') {
                ...
                if ($dateType == 'MEDIUMINT') {
                    $length = (empty($length) ? 11 : $length);
                    $codingType = '';
                    $coding = '';
                    $dateType = 'MEDIUMINT( ' . $length . ' ) UNSIGNED';
                } else {
                    if ($dateType == 'SMALLINT') {
                        $length = (empty($length) ? 11 : $length);
                        $codingType = '';
                        $coding = '';
...
function get_merchants_steps_fields_admin(
    $table,
    $date,
    $dateType,
    $length,
    ...
    for ($i = 0; $i < count($date); $i++) {
        if (!empty($date[$i])) {
            ...
            $failure = get_add_drop_fields($arr[$i]['date'], $newDate, $table, $type, $arr[$i]['dateType'],
                $arr[$i]['length'], $arr[$i]['notnull'], $arr[$i]['formName'], $arr[$i]['coding']);
    ...
    for ($i = 0; $i < count($form); $i++) {
        ...
        if ($form[$i] == 'input') {
            ...
            if ($form[$i] == 'textarea') {
                ...
                if ($form[$i] == 'radio') {
                    $arr[$i]['form'] = $form[$i] . ':' . implode(',', get_formType_arr($_POST['radio_checkbox_' . $i],
                            $_POST['rc_sort_' . $i])) . $formName_special;
                } else {
                    if ($form[$i] == 'checkbox') {
                        $arr[$i]['form'] = $form[$i] . ':' . implode(',',
                                get_formType_arr($_POST['radio_checkbox_' . $i],
                                    $_POST['rc_sort_' . $i])) . $formName_special;
                    } else {
                        if ($form[$i] == 'select') {
                            $arr[$i]['form'] = $form[$i] . ':' . implode(',',
                                    get_formType_arr($_POST['select_' . $i], '', 1)) . $formName_special;
                        } else {
                            if ($form[$i] == 'other') {
...
function get_merchants_steps_fields_centent_insert_update(
    $textFields,
    $fieldsDateType,
    $fieldsLength,
    $fieldsNotnull,
    ...
    if ($handler_type == 'update') {
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_steps_fields_centent'), $parent, 'UPDATE',
            'tid = \'' . $tid . '\'');
    } else {
...
function get_merchants_steps_title_insert_update(
    $fields_steps,
    $fields_titles,
    $titles_annotation,
    $steps_style,
    ...
    if (0 < $res) {
        ...
        if ($handler_type == 'update') {
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_steps_title'), $parent, 'UPDATE',
                'tid = \'' . $tid . '\'');
            return true;
        } else {
...
function get_fields_centent_info(
    $id,
    $textFields,
    $fieldsDateType,
    $fieldsLength,
                ...
                if ($chooseForm[0] == 'textarea') {
                    ...
                    if (($chooseForm[0] == 'radio') || ($chooseForm[0] == 'checkbox')) {
                        ...
                        if ($chooseForm[0] == 'select') {
                            ...
                            if ($chooseForm[0] == 'other') {
                                $otherForm = explode(',', $form_special[0]);
                                $arr[$i + 1]['otherForm'] = $otherForm[0];

                                if ($otherForm[0] == 'dateTime') {
                                    if ($webType == 'root') {
                                        $arr[$i + 1]['dateTimeForm'] = get_dateTimeForm_arr(explode('--',
                                            $otherForm[1]), explode(',', $arr[$i + 1]['titles_centents']));
                                    } else {
                                        $arr[$i + 1]['dateTimeForm'] = $otherForm[1];
                                    }
                                } else {
                                    if ($otherForm[0] == 'textArea') {
                                        if ($webType == 'root') {
                                            $arr[$i + 1]['textAreaForm'] = get_textAreaForm_arr(explode(',',
                                                $arr[$i + 1]['titles_centents']));
                                            $arr[$i + 1]['province_list'] = get_regions_steps(1,
                                                $arr[$i + 1]['textAreaForm']['country']);
                                            $arr[$i + 1]['city_list'] = get_regions_steps(2,
                                                $arr[$i + 1]['textAreaForm']['province']);
                                            $arr[$i + 1]['district_list'] = get_regions_steps(3,
                                                $arr[$i + 1]['textAreaForm']['city']);
                                        }
                                    }
                                }
                            }
...
function get_documentTitle_insert_update($dt_list, $cat_id, $dt_id = array())
{
    for ($i = 0; $i < count($dt_list); $i++) {
        ...
        if (!empty($dt_list[$i])) {
            ...
            if (0 < $catId) {
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_documenttitle'), $parent, 'UPDATE',
                    'dt_id = \'' . $dt_id[$i] . '\'');
            } else {
...
function get_root_merchants_steps_title($pid, $user_id)
{
    ...
    $brandFirstChar = (isset($_POST['ec_brandFirstChar']) ? htmlspecialchars(trim($_POST['ec_brandFirstChar'])) : substr($searchBrandEnInput,
        0, 1));
    ...
    foreach ($res as $key => $row) {
        ...
        $cententFields = get_fields_centent_info($centent['id'], $centent['textFields'], $centent['fieldsDateType'],
            $centent['fieldsLength'], $centent['fieldsNotnull'], $centent['fieldsFormName'], $centent['fieldsCoding'],
            $centent['fieldsForm'], $centent['fields_sort'], $centent['will_choose'], 'root', $user_id);
        ...
        if ($row['steps_style'] == 1) {
            ...
            if ($ec_shop_permanent != 1) {
                $ec_shop_expireDateStart = (empty($ec_shop_expireDateStart) ? local_date('Y-m-d H:i',
                    $shop_info['shop_expireDateStart']) : $ec_shop_expireDateStart);
                $ec_shop_expireDateEnd = (empty($ec_shop_expireDateEnd) ? local_date('Y-m-d H:i',
                    $shop_info['shop_expireDateEnd']) : $ec_shop_expireDateEnd);
            ...
            if (0 < $_SESSION['user_id']) {
                if (0 < $shop_id) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_information'), $parent, 'UPDATE',
                        'user_id = \'' . $_SESSION['user_id'] . '\'');
                } else {
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_information'), $parent,
                        'INSERT');
                }
            }
                ...
                if ($row['steps_style'] == 3) {
                    ...
                    if ((0 < $ec_shop_bid) || (0 < $brand_m['brand_id'])) {
                        ...
                        if (0 < $_SESSION['user_id']) {
                            ...
                            if ($_SESSION['user_id'] == $brand_info['user_id']) {
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_brand'), $parent,
                                    'UPDATE',
                                    'user_id = \'' . $_SESSION['user_id'] . '\' and bid = \'' . $ec_shop_bid . '\'');
                                get_shop_brand_file($qualificationNameInput, $qualificationImg, $expiredDateInput,
                                    $b_fid, $ec_shop_bid);
                            }
                        }
                    } else {
                        if (0 < $_SESSION['user_id']) {
                            ...
                            if (!empty($bank_name_letter)) {
                                ...
                                if (0 < $bRes) {
                                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_brand'), $parent,
                                        'UPDATE',
                                        'user_id = \'' . $_SESSION['user_id'] . '\' and bid = \'' . $bRes . '\'');
                                    get_shop_brand_file($qualificationNameInput, $qualificationImg, $expiredDateInput,
                                        $b_fid, $bRes);
                    ...
                    if ($row['steps_style'] == 4) {
                        ...
                        if (!empty($ec_rz_shopName)) {
                            ...
                            if (0 < $_SESSION['user_id']) {
                                if (0 < $shop_id) {
                                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_information'),
                                        $parent, 'UPDATE', 'user_id = \'' . $_SESSION['user_id'] . '\'');
                                } else {
                                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_information'),
                                        $parent, 'INSERT');
                                }
...
function get_merchants_dt_file_insert_update($cat_id, $dt_id, $permanentFile, $permanent_date)
{
    ...
    for ($i = 0; $i < count($cat_id); $i++) {
        ...
        $row = $GLOBALS['db']->getRow($sql);
        $pFile = $image->upload_image('', 'septs_Image', '', 1, $permanentFile['name'][$i], $permanentFile['type'][$i],
            $permanentFile['tmp_name'][$i], $permanentFile['error'][$i], $permanentFile['size'][$i]);
        ...
        if (0 < $row['dtf_id']) {
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_dt_file'), $parent, 'UPDATE',
                'cat_id = \'' . $cat_id[$i] . '\' and dt_id = \'' . $dt_id[$i] . '\' and user_id = \'' . $_SESSION['user_id'] . '\'');
        } else {
...
function get_shop_brand_file($qInput, $qImg, $eDinput, $b_fid, $ec_shop_bid)
{
    ...
    for ($i = 0; $i < count($qInput); $i++) {
        ...
        $qImg[$i] = $image->upload_image('', 'septs_Image', '', 1, $qImg['name'][$i], $qImg['type'][$i],
            $qImg['tmp_name'][$i], $qImg['error'][$i], $qImg['size'][$i]);
        ...
        if (!empty($qInput[$i])) {
            ...
            if (!empty($b_fid[$i])) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_shop_brandfile'), $parent, 'UPDATE',
                    'bid = \'' . $ec_shop_bid . '\' and b_fid = \'' . $b_fid[$i] . '\'');
            } else {
...
function get_add_childCategory_info($cat_id, $user_id)
{
    ...
    foreach ($res as $key => $row) {
        ...
        if ($cat_id != 0) {
            ...
            if ($ct_id <= 0) {
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_category_temporarydate'), $parent,
                    'INSERT');
            }
        }
    }
...
function get_goods_freight($goods, $warehouse_id = 0, $goods_region = array(), $buy_number = 1, $shipping_code)
{
    ...
    if ($goods['goods_transport']) {
        ...
        $order_transport = get_order_transport($goods_transport, $goods_region, $shipping['shipping_id'],
            $shipping_code);
    }

    if ($goods['goods_price']) {
        $street_configure = get_goods_freight_configure($goods, $warehouse_id, $goods_region['street'], $shipping_code);
        $district_configure = get_goods_freight_configure($goods, $warehouse_id, $goods_region['district'],
            $shipping_code);
        $city_configure = get_goods_freight_configure($goods, $warehouse_id, $goods_region['city'], $shipping_code);
        $province_configure = get_goods_freight_configure($goods, $warehouse_id, $goods_region['province'],
            $shipping_code);
        ...
        $configure_price = goods_shipping_fee($shipping_code, unserialize($configure), $goods['weight'],
            $goods['goods_price'], $goods['number']);
...
function get_insert_warehouse_goods(
    $goods_id = 0,
    $warehouse_name = array(),
    $warehouse_number = array(),
    $warehouse_price = array(),
    $warehouse_promote_price = array(),
    $user_id = 0
) {
    $add_time = gmtime();
...
function get_insert_warehouse_area_goods(
    $goods_id = 0,
    $area_name = array(),
    $region_number = array(),
    $region_price = array(),
    $region_promote_price = array(),
    $user_id = 0
) {
    $add_time = gmtime();
...
function get_insert_produts_warehouse($goods_list)
{
    ...
    for ($i = 0; $i < count($goods_list); $i++) {
        if (0 < $goods_list[$i]['goods_id']) {
            ...
            if (0 < $res) {
                $return = 1;
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('products_warehouse'), $other, 'UPDATE',
                    'goods_id = \'' . $other['goods_id'] . '\'' . ' and goods_attr = \'' . $other['goods_attr'] . '\'' . ' and warehouse_id = \'' . $other['warehouse_id'] . '\'');
            } else {
                $return = 0;
...
function get_insert_produts_area($goods_list)
{
    ...
    for ($i = 0; $i < count($goods_list); $i++) {
        if (0 < $goods_list[$i]['goods_id']) {
            ...
            if (0 < $res) {
                $return = 1;
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('products_area'), $other, 'UPDATE',
                    'goods_id = \'' . $other['goods_id'] . '\'' . ' and goods_attr = \'' . $other['goods_attr'] . '\'' . ' and area_id = \'' . $other['area_id'] . '\'');
            } else {
                $return = 0;
...
function get_insert_bacth_warehouse($goods_list)
{
    ...
    for ($i = 0; $i < count($goods_list); $i++) {
        if (0 < $goods_list[$i]['goods_id']) {
            ...
            if (0 < $res) {
                $return = 1;
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('warehouse_goods'), $other, 'UPDATE',
                    ' user_id = \'' . $other['user_id'] . '\' and goods_id = \'' . $other['goods_id'] . '\'' . ' and region_id = \'' . $other['region_id'] . '\'');
            } else {
                $return = 0;
    ...
    for ($i = 0; $i < count($goods_list); $i++) {
        if (0 < $goods_list[$i]['goods_id']) {
            ...
            if (0 < $res) {
                $arr['return'] = 1;
                $return = $arr;
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('warehouse_area_goods'), $other, 'UPDATE',
                    ' user_id = \'' . $other['user_id'] . '\' and goods_id = \'' . $other['goods_id'] . '\'' . ' and region_id = \'' . $other['region_id'] . '\'');
            } else {
                $arr['return'] = 0;
                $return = $arr;
    ...
    for ($i = 0; $i < count($goods_list); $i++) {
        if (0 < $goods_list[$i]['goods_id']) {
            ...
            if (0 < $res) {
                $arr['return'] = 1;
                $return = $arr;
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('warehouse_area_attr'), $other, 'UPDATE',
                    ' goods_id = \'' . $other['goods_id'] . '\' and area_id = \'' . $other['area_id'] . '\'' . ' and goods_attr_id = \'' . $other['goods_attr_id'] . '\'');
            } else {
                $arr['return'] = 0;
                $return = $arr;
...
function get_area_attr_price_insert($area_name, $goods_id, $goods_attr_id)
{
    ...
    for ($i = 0; $i < count($area_name); $i++) {
        if (!empty($area_name[$i])) {
            ...
            if (0 < $id) {
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('warehouse_area_attr'), $parent, 'UPDATE',
                    'goods_id = \'' . $goods_id . '\' and goods_attr_id = \'' . $goods_attr_id . '\' and area_id = \'' . $area_name[$i] . '\'');
            } else {
...
function get_warehouse_attr_price_insert($warehouse_name, $goods_id, $goods_attr_id)
{
    ...
    for ($i = 0; $i < count($warehouse_name); $i++) {
        if (!empty($warehouse_name[$i])) {
            ...
            if (0 < $id) {
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('warehouse_attr'), $parent, 'UPDATE',
                    'goods_id = \'' . $goods_id . '\' and goods_attr_id = \'' . $goods_attr_id . '\' and warehouse_id = \'' . $warehouse_name[$i] . '\'');
            } else {
    ...
    foreach ($newInfo as $key => $info) {
        ...
        do {
            ...
            if ($GLOBALS['_CFG']['send_service_email'] && ($seller_email != '') && ($GLOBALS['_CFG']['seller_email'] == 1)) {
                ...
                send_mail($GLOBALS['_CFG']['shop_name'], $seller_email, $tpl['template_subject'], $content,
                    $tpl['is_html']);
            }
...
function get_user_store_category($ru_id)
{
    ...
    foreach ($res as $key => $row) {
        $arr[$key] = $row;
        $arr[$key]['url'] = build_uri('merchants_store', array('cid' => $row['cat_id'], 'urid' => $ru_id),
            $row['cat_name']);
        $arr[$key]['opennew'] = 0;
    ...
    if ($type == 1) {
        ...
        if ($type == 2) {
            ...
            if (0 < $goods['promote_price']) {
                $promote_price = bargain_price($goods['promote_price'], $goods['promote_start_date'],
...
function get_min_or_max_values($arr)
{
    ...
    $newArr['fittings_minPrice'] = $arr[0]['fittings_minPrice'] + min(explode(',',
            substr($newArr['fittings_minPrice'], 0, -1)));
    $newArr['fittings_maxPrice'] = $arr[0]['fittings_maxPrice'] + $newArr['fittings_maxPrice'];
    $newArr['market_minPrice'] = $arr[0]['market_minPrice'] + min(explode(',',
            substr($newArr['market_minPrice'], 0, -1)));
        ...
        if (!empty($goodsInfo['freight'])) {
            if ($goodsInfo['freight'] == 1) {
                ...
                if ($transportInfo['freight_type']) {
                    ...
                    $goods_transport = $GLOBALS['ecs']->get_select_find_in_set(2, $region[2], $transport,
                        $transport_where, 'goods_transport_extend', 'area_id');
            ...
            if ($is_shipping) {
                if ($goodsInfo['is_shipping'] == 1) {
                    ...
                    if ($GLOBALS['_CFG']['freight_model'] == 0) {
                        $goods_shipping_fee = get_goods_shipping_fee($goodsInfo, $val, $sellerShippingInfo,
                            $warehouse_id);
                        ...
                        if ($GLOBALS['_CFG']['freight_model'] == 1) {
                            ...
                            $shipping_fee = get_goods_order_shipping_fee($goods_list, $region,
...
function get_goods_transport_tpl($goodsInfo = array(), $region = array(), $shippingInfo = array(), $goods_number = 1)
{
    ...
    if ($is_shipping) {
        ...
        $shippingFee = shipping_fee($shippingInfo['shipping_code'], $shippingInfo['configure'], $goods_weight,
            ...
            if ($row['freight'] && ($row['is_shipping'] == 0)) {
                if ($row['freight'] == 1) {
                    ...
                    if ($trow['freight_type'] == 1) {
                        ...
                        $goods_transport = $GLOBALS['ecs']->get_select_find_in_set(2, $consignee['city'], $transport,
                            $transport_where, 'goods_transport_extend', 'area_id');
                        $ship_transport = array('tid', 'ru_id', 'shipping_fee');
                        $ship_transport_where = ' AND ru_id = \'' . $row['ru_id'] . '\' AND tid = \'' . $row['tid'] . '\'';
                        $goods_ship_transport = $GLOBALS['ecs']->get_select_find_in_set(2, $shipping_id,
                            $ship_transport, $ship_transport_where, 'goods_transport_express', 'shipping_id');
...
{
    if ($GLOBALS['_CFG']['freight_model'] == 0) {
        $shippingFee = shipping_fee($val['shipping_code'], $val['configure'], $goodsInfo['goods_weight'],
        ...
        if ($GLOBALS['_CFG']['freight_model'] == 1) {
            ...
            $shipping_fee = get_goods_freight($goods, $warehouse_id, $goods_region, 1,
                $sellerShippingInfo['shipping_code']);
            $shippingFee = $shipping_fee['shipping_fee'];
...
function getSellerShippingFee($sellerOrderInfo = array(), $cart_goods)
{
    ...
    $val = $GLOBALS['db']->getRow($sql);
    $shippingFee = shipping_fee($val['shipping_code'], $val['configure'], $sellerOrderInfo['weight'],
        $sellerOrderInfo['goods_price'], $sellerOrderInfo['number']);
