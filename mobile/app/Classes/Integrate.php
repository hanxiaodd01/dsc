...
class Integrate
{
    ...
    public function __construct($cfg)
    {
        ...
        if (empty($cfg['db_host'])) {
            ...
            if (empty($cfg['is_latin1'])) {
                $this->db = new mysql($cfg['db_host'], $cfg['db_user'], $cfg['db_pass'], $cfg['db_name'],
                    $this->charset, null, $quiet);
            } else {
                $this->db = new mysql($cfg['db_host'], $cfg['db_user'], $cfg['db_pass'], $cfg['db_name'], 'latin1',
                    null, $quiet);
            }
        }
    ...
    public function edit_user($cfg)
    {
        ...
        if ($values) {
            $sql = 'UPDATE ' . $this->table($this->user_table) . ' SET ' . implode(', ',
                    $values) . ' WHERE ' . $this->field_name . '=\'' . $cfg['post_username'] . '\' LIMIT 1';
            $this->db->query($sql);
    ...
    public function remove_user($id)
    {
        ...
        if ($this->need_sync || (isset($this->is_ecshop) && $this->is_ecshop)) {
            ...
            $sql .= (is_array($post_id) ? db_create_in($post_id,
                'user_name') : 'user_name=\'' . $post_id . '\' LIMIT 1');
