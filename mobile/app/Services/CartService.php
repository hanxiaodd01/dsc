...
class CartService
{
    ...
    public function __construct(
        ...
        \App\Repositories\Activity\ActivityRepository $activityRepository,
        ...
        foreach ($merchant_goods as $key => $row) {
            ...
            if ($user_cart_goods) {
                foreach ($user_cart_goods as $key1 => $row1) {
                    ...
                    if (isset($sort_favourable['by_all']) && $row1['extension_code'] != 'package_buy' && substr($row1['extension_code'],
                            0, 7) != 'seckill') {
                            ...
                            if ($row2['userFav_type'] == 1 || $mer_ids) {
                                if ($row1['is_gift'] == 0) {
                                    if (isset($row1) && $row1) {
                                        ...
                                        $merchant_goods[$key]['new_list'][$row2['act_id']]['available'] = $this->favourableAvailable($row['user_id'],
                                            $row2, array(), $row1['ru_id']);
                                        $cart_favourable = $this->cartRepository->cartFavourable($row['user_id'],
                                            $row1['ru_id']);
                                        ...
                                        $merchant_goods[$key]['new_list'][$row2['act_id']]['favourable_used'] = $this->favourableUsed($row2,
                                            $cart_favourable);
                    ...
                    if (isset($sort_favourable['by_category']) && $row1['extension_code'] != 'package_buy' && substr($row1['extension_code'],
                            0, 7) != 'seckill') {
                            ...
                            foreach ($sort_favourable['by_category'] as $key2 => $row2) {
                                if (isset($row1) && $row1) {
                                    ...
                                    if (isset($row1) && $row1 && $row1['is_gift'] == $row2['act_id']) {
                                        if ($this->favourableAvailable($row['user_id'], $row2, array(),
                                            $row1['ru_id'])) {
                                            $merchant_goods[$key]['new_list'][$row2['act_id']]['act_cart_gift'][$row1['rec_id']] = $row1;
                                        } else {
                                            $is_gift = \App\Models\Cart::where('user_id',
                                                $row['user_id'])->where('is_gift', $row2['act_id'])->count();

                                            if ($is_gift) {
                                                \App\Models\Cart::where('user_id', $row['user_id'])->where('is_gift',
                                                    $row2['act_id'])->delete();
                                            }
                                        }
                                    }
                    ...
                    if (isset($sort_favourable['by_brand']) && $row1['extension_code'] != 'package_buy' && substr($row1['extension_code'],
                            0, 7) != 'seckill') {
                    ...
                    if (isset($sort_favourable['by_goods']) && $row1['extension_code'] != 'package_buy' && substr($row1['extension_code'],
                            0, 7) != 'seckill') {
