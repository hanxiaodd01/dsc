...
class GoodsService
{
    ...
    public function __construct(
        ...
        \App\Repositories\Goods\CollectGoodsRepository $collectGoodsRepository,
        ...
        \App\Repositories\Store\StoreRepository $StoreRepository,
    ...
    public function getGoodsList(
        ...
        $list = $this->goodsRepository->findBy('category', $categoryId, $page, $size, $field, $keywords, $sortKey,
            $sortVal, $proprietary, $price_min, $price_max, $brand, $province_id, $city_id, $county_id, $fil_key);
    ...
    public function goodsDetail($id, $uid)
    {
        ...
        if (!empty($goodsInfo['desc_mobile'])) {
            ...
            $goodsInfo['goods_desc'] = preg_replace('/<div[^>]*(tools)[^>]*>(.*?)<\\/div>(.*?)<\\/div>/is', '',
                $goodsInfo['desc_mobile']);
        } else {
            if (!empty($goodsInfo['goods_desc'])) {
                ...
                if ($open_oss == 1) {
                    ...
                    $goodsInfo['goods_desc'] = str_replace(array('src="/images/upload', 'src="images/upload'),
                        'src="' . $bucket_info['endpoint'] . 'images/upload', $goodsInfo['goods_desc']);
                } else {
                    $goodsInfo['goods_desc'] = str_replace(array('src="/images/upload', 'src="images/upload'),
                        'src="' . $rootPath . '/images/upload', $goodsInfo['goods_desc']);
                }
            } else {
        ...
        if (0 < $ruId) {
            ...
            $result['detail']['sellershopinfo']['logo_thumb'] = get_image_path(str_replace('../', '',
                $detail[0]['sellershopinfo']['logo_thumb']));
        }
    ...
    public function goodsPropertiesPrice($goods_id, $attr_id, $num = 1, $warehouse_id = 0, $area_id = 0, $store_id = 0)
    {
        ...
        $result['stock'] = $this->goodsRepository->goodsAttrNumber($goods_id, $attr_id, $warehouse_id, $area_id,
            $store_id);
        $result['market_price'] = $this->goodsRepository->goodsMarketPrice($goods_id, $attr_id, $warehouse_id,
            $area_id);
        $result['market_price_formated'] = price_format($result['market_price'], true);
        $result['qty'] = $num;
        $result['spec_price'] = $this->goodsRepository->goodsPropertyPrice($goods_id, $attr_id, $warehouse_id,
            $area_id);
        $result['spec_price_formated'] = price_format($result['spec_price'], true);
        $result['goods_price'] = $this->goodsRepository->getFinalPrice($goods_id, $num, true, $attr_id, $warehouse_id,
            $area_id);
    ...
    public function goodsShare($id, $uid, $path = '', $width = 430, $type = 'goods')
    {
        ...
        $user = array(
            'name' => isset($userInfo['nick_name']) ? $userInfo['nick_name'] : $userInfo['user_name'],
            'id' => $userInfo['id'],
            'pic' => get_image_path($userInfo['user_picture']),
            'shop_name' => $shop_name,
        ...
        $goods_cont = array(
            ...
            'pic' => get_image_path($goodsInfo['goods_thumb'])
        );
        $share['user'] = $user;
