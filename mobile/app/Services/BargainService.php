...
class BargainService
{
        ...
        foreach ($adsense as $row) {
            if (!empty($row['position_id'])) {
                $src = strpos($row['ad_code'], 'http://') === false && strpos($row['ad_code'],
                    'https://') === false ? 'data/afficheimg/' . $row['ad_code'] : $row['ad_code'];
    ...
    public function goodsDetail($id = 0, $user_id = 0, $bs_id = 0)
    {
        ...
        if (!empty($bs_id)) {
            ...
            if ($bargain_log['goods_attr_id']) {
                ...
                $goodsInfo['shop_price'] = $this->bargainRepository->getFinalPrice($goodsInfo['goods_id'], '', true,
                    $spec, '', '');
                $goodsInfo['target_price'] = $this->bargainRepository->bargainTargetPrice($id, $goodsInfo['goods_id'],
                    $spec, 0, 0, $goodsInfo['model_attr']);
    ...
    public function goodsPropertiesPrice(
        $bargain_id,
        $attr_id,
        $num = 1,
        $warehouse_id = 0,
        ...
        $result['stock'] = $this->goodsRepository->goodsAttrNumber($goodsInfo['goods_id'], $attr_id, $warehouse_id,
            $area_id, $store_id);
        $result['market_price'] = $this->goodsRepository->goodsMarketPrice($goodsInfo['goods_id'], $attr_id,
            $warehouse_id, $area_id);
        $result['market_price_formated'] = price_format($result['market_price'], true);
        $result['qty'] = $num;
        $result['spec_price'] = $this->goodsRepository->goodsPropertyPrice($goodsInfo['goods_id'], $attr_id,
            $warehouse_id, $area_id);
        $result['spec_price_formated'] = price_format($result['spec_price'], true);
        $result['goods_price'] = $goodsInfo['goods_price'];

        if (!empty($attr_id)) {
            $result['target_price'] = $this->bargainRepository->bargainTargetPrice($bargain_id, $goodsInfo['goods_id'],
                $attr_id, $warehouse_id, $area_id, $goodsInfo['model_attr']);
            $result['goods_price'] = $this->bargainRepository->getFinalPrice($goodsInfo['goods_id'], $num, true,
                $attr_id, $warehouse_id, $area_id);
        }
    ...
    public function addBargain($bargain_id, $attr_id, $user_id = 0, $warehouse_id = 0, $area_id = 0)
    {
        ...
        if (!empty($attr_id)) {
            $final_price = $this->bargainRepository->getFinalPrice($goodsInfo['goods_id'], '', true, $attr_id,
                $warehouse_id, $area_id);
        } else {
    ...
    public function goBargain($bargain_id = 0, $bs_id = 0, $user_id = 0, $form_id = '')
    {
        ...
        if ($bs_log['goods_attr_id']) {
            ...
            $bargain['target_price'] = $this->bargainRepository->bargainTargetPrice($bargain_id, $bargain['goods_id'],
                $spec, 0, 0, $bargain['model_attr']);
        }
