...
class FlowService
{
    ...
    public function __construct(
        \App\Repositories\Cart\CartRepository $cartRepository,
        \App\Repositories\Coupons\CouponsRepository $couponsRepository,
        \App\Repositories\User\AddressRepository $addressRepository,
        \App\Repositories\User\InvoiceRepository $invoiceRepository,
        \App\Repositories\Payment\PaymentRepository $paymentRepository,
        \App\Repositories\Shipping\ShippingRepository $shippingRepository,
        \App\Repositories\Drp\DrpRepository $drpRepository,
        ...
        \App\Repositories\Order\OrderInvoiceRepository $orderInvoiceRepository,
        \App\Repositories\Product\ProductRepository $productRepository,
        \App\Repositories\Order\OrderRepository $orderRepository,
        \App\Repositories\Order\OrderGoodsRepository $orderGoodsRepository,
        \App\Repositories\User\AccountRepository $accountRepository,
        \App\Repositories\Payment\PayLogRepository $payLogRepository,
        \App\Repositories\Region\RegionRepository $regionRepository,
        \App\Repositories\Shop\ShopRepository $shopRepository,
        \App\Repositories\Bargain\BargainRepository $bargainRepository,
        \App\Repositories\Team\TeamRepository $teamRepository,
    ...
    public function flowInfo($userId, $flow_type = 0, $bs_id = 0, $t_id = 0, $team_id = 0)
    {
        ...
        foreach ($result['cart_goods_list']['list'] as $key => $val) {
            $cou[$key] = $this->couponsRepository->UserCoupons($userId, true, $result['cart_goods_list']['order_total'],
                $val, true);
        }
...
', '', $this->shopConfigRepository->getShopConfigByCode('invoice_content')));
    ...
    public function changeCou($uc_id, $userId, $flow_type = 0)
    {
        ...
        if (empty($cart_goods_list)) {
            ...
            $coupons_info = $this->couponsRepository->getcoupons($userId, $uc_id,
                array('c.cou_id', 'c.cou_man', 'c.cou_type', 'c.ru_id', 'c.cou_money', 'cu.uc_id', 'cu.user_id'));
            ...
            if (!empty($coupons_info) && $cart_goods_list) {
                if ($coupons_info['cou_type'] == 5) {
                    ...
                    if ($coupons_info['cou_money'] < $cart_goods_list['order_total']) {
    ...
    private function arrangeCartGoods($userId, $flow_type)
    {
        ...
        foreach ($cartGoodsList['goods_list'] as $k => $v) {
            ...
            foreach ($v['goods'] as $key => $value) {
                ...
                $list[$v['ru_id']]['shop_list'][$key] = array(
                    'rec_id' => $value['rec_id'],
                    'user_id' => $v['user_id'],
                    'cat_id' => $value['cat_id'],
        ...
        return array(
            ...
            'order_total_formated' => price_format($totalAmount, false),
            'discount' => $discount
        );
    }
    ...
    public function submitOrder($args)
    {
        ...
        $order = array(
            ...
            'pay_id' => is_null($payment) ? 0 : $payment['pay_id'],
            'surplus' => isset($args['surplus']) ? floatval($args['surplus']) : 0,
            'integral' => isset($score) ? intval($score) : 0,
            'tax_id' => empty($args['postdata']['tax_id']) ? 0 : $args['postdata']['tax_id'],
            'inv_payee' => trim($args['postdata']['inv_payee']),
            'inv_content' => !trim($args['postdata']['inv_content']) ? 0 : trim($args['postdata']['inv_content']),
            'vat_id' => empty($args['postdata']['vat_id']) ? 0 : $args['postdata']['vat_id'],
            'invoice_type' => empty($args['postdata']['invoice_type']) ? 0 : $args['postdata']['invoice_type'],
            'froms' => '小程序',
            'referer' => 'wxapp',
        ...
        $total = $this->orderRepository->order_fee($order, $cart_goods['goods_list'], $consignee_info, $cart_good_ids,
            $order['shipping_id'], $consignee);

        if (0 < $args['uc_id']) {
            $coupons = $this->couponsRepository->getcoupons($userId, $args['uc_id'],
                array('c.cou_id', 'c.cou_man', 'c.cou_type', 'c.ru_id', 'c.cou_money', 'cu.uc_id', 'cu.user_id'));
        ...
        if ($flow_type == CART_TEAM_GOODS) {
            if (0 < $args['team_id']) {
                ...
                if (0 < $team_info['status']) {
                    ...
                    $arguments = array(
                        't_id' => $args['t_id'],
                        'goods_id' => $team_cart_goods['goods_id'],
                        'start_time' => gmtime(),
                        'status' => 0
                    );
        ...
        if (0 < $order['user_id'] && 0 < $order['integral']) {
            $this->accountRepository->logAccountChange(0, 0, 0, $order['integral'] * -1, trans('message.score.pay'),
                $order['order_sn'], $userId);
        }
        ...
        if ($flow_type != CART_BARGAIN_GOODS && $flow_type != CART_TEAM_GOODS && 1 < $ru_number) {
            $shipping = array(
                ...
                'shipping_fee_list' => isset($total['shipping_fee_list']) ? $total['shipping_fee_list'] : ''
            );
        ...
        foreach ($cart_goods as $cart_goods_key => $cart_goods_val) {
            if (0 < $cart_goods_val['cloud_id']) {
                ...
                if ($cart_goods_val['goods_attr_id']) {
                    ...
                    $products_info = $this->goodsRepository->getProductsAttrNumber($cart_goods_val['goods_id'],
                        $goods_attr_id, 0, 0, $goods['model_attr']);
                                    ...
                                    if (!empty($cart_goods[0]['freight']) && $cart_goods[0]['is_shipping'] == 0) {
                                        if ($cart_goods[0]['freight'] == 1) {
                                            ...
                                            if ($trow['freight_type']) {
                                                ...
                                                $transport_tpl = $this->get_goods_transport_tpl($cart_goods[0], $region,
                                                    $val, $cart_goods[0]['goods_number']);
                                                ...
                                                $goods_transport = $this->shopRepository->get_select_find_in_set(2,
                                                ...
                                                $goods_ship_transport = $this->shopRepository->get_select_find_in_set(2,
    ...
    private function get_goods_transport_tpl(
        $goodsInfo = array(),
        $region = array(),
        $shippingInfo = array(),
        $goods_number = 1
        ...
        if (empty($shippingInfo)) {
            ...
            if (!$shippingInfo) {
                ...
                $shippingInfo = $this->get_goods_transport_tpl_shipping($goodsInfo['tid'], $shippingInfo['shipping_id'],
                    $region);
            }
        } else {
            $is_goods = 0;
    ...
    private function get_goods_transport_tpl_shipping(
        $tid = 0,
        $shipping_id = 0,
        $region = array(),
        $type = 0,
        ...
        foreach ($goodsList as $key => $value) {
            ...
            foreach ($value['goods'] as $v) {
                ...
                $orderGoods[] = array(
                    ...
                    'area_id' => $v['area_id'],
                    'is_distribution' => $v['is_distribution'],
                    'dis_commission' => $v['dis_commission'],
                    'isdistribution' => $v['isdistribution']
                );
            }
        ...
        foreach ($arr as $key => $val) {
            ...
            if (0 < intval($this->shopConfigRepository->getShopConfigByCode('use_storage')) && $goodsExtendsionCode != 'package_buy') {
                ...
                if (!empty($row['product_id'])) {
                    @$product_number = $this->productRepository->findBy(array(
    ...
    public function shippingFee($args)
    {
        ...
        $coupons_info = $this->couponsRepository->getcoupons($this->userId, $uc_id,
            array('c.cou_id', 'c.cou_man', 'c.cou_type', 'c.ru_id', 'c.cou_money', 'cu.uc_id', 'cu.user_id'));
