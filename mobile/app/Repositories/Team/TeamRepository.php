...
class TeamRepository
{
    ...
    public function __construct(
        ...
        \App\Repositories\Goods\GoodsAttrRepository $goodsAttrRepository,
    ...
    {
        $time = gmtime();
        $res = \App\Models\TouchAd::select('ad_id', 'touch_ad.position_id', 'media_type', 'ad_link', 'ad_code',
            'ad_name')->with(array('position'))->join('touch_ad_position', 'touch_ad_position.position_id', '=',
            'touch_ad.position_id')->where('start_time', '<=', $time)->where('end_time', '>=',
            $time)->where('touch_ad_position.position_id', $position_id)->where('enabled',
            1)->limit($num)->get()->toArray();
    ...
    public function categoriesAdsense($tc_id = 0, $type = 'banner', $num = 3)
    {
        ...
        $res = \App\Models\TouchAd::select('ad_id', 'touch_ad.position_id', 'media_type', 'ad_link', 'ad_code',
            'ad_name')->with(array('position'))->join('touch_ad_position', 'touch_ad_position.position_id', '=',
            'touch_ad.position_id')->where('start_time', '<=', $time)->where('end_time', '>=',
            $time)->where('touch_ad_position.tc_id', $tc_id)->where('touch_ad_position.tc_type',
            $type)->where('touch_ad_position.ad_type', 'wxapp')->where('enabled', 1)->limit($num)->get()->toArray();
        $res = array_map(function ($v) {
            if (!empty($v['position'])) {
                $temp = array_merge($v, $v['position']);
                unset($temp['position']);
    ...
    {
        return \App\Models\TeamCategory::select('*')->where('parent_id', 0)->where('status', 1)->orderby('id',
            'asc')->get()->toArray();
    }

    public function teamCategoriesChild($tc_id = 0)
    {
        return \App\Models\TeamCategory::select('*')->where('parent_id', $tc_id)->where('status', 1)->orderby('id',
            'asc')->get()->toArray();
    }
    ...
    {
        $goods = \App\Models\TeamGoods::from('team_goods as tg')->select('g.goods_id', 'g.goods_name', 'g.shop_price',
            'g.goods_number', 'g.sales_volume', 'g.goods_thumb', 'tg.id', 'tg.team_price', 'tg.team_num',
            'tg.limit_num')->leftjoin('goods as g', 'g.goods_id', '=', 'tg.goods_id');
        ...
        $begin = ($page - 1) * $size;
        $list = $goods->where('tg.is_team', 1)->where('tg.is_audit', 2)->where('g.is_on_sale',
            1)->where('g.is_alone_sale', 1)->where('g.is_delete', 0)->where('g.review_status', '>',
            2)->offset($begin)->orderby('tg.id', 'desc')->limit($size)->get()->toArray();
    ...
    public function categoryGoodsList($tc_id = 0, $page = 1, $size = 10, $keywords = '', $sortKey = 0, $sortVal = '')
    {
        ...
        $begin = ($page - 1) * $size;
        $list = $goods->where('tg.tc_id', $tc_id)->where('tg.is_team', 1)->where('tg.is_audit',
            2)->where('g.is_on_sale', 1)->where('g.is_alone_sale', 1)->where('g.is_delete', 0)->where('g.review_status',
    ...
    {
        $res = \App\Models\TeamGoods::from('team_goods as tg')->select('tg.id', 'tg.goods_id', 'tg.team_price',
            'tg.team_num', 'tg.limit_num', 'tg.astrict_num', 'tg.is_audit', 'tg.is_team', 'tg.team_desc', 'g.user_id',
            'g.goods_sn', 'g.goods_name', 'g.is_real', 'g.is_shipping', 'g.is_on_sale', 'g.shop_price',
            'g.market_price', 'g.goods_thumb', 'g.goods_img', 'g.goods_number', 'g.sales_volume', 'g.goods_desc',
            'g.desc_mobile', 'g.goods_type', 'g.goods_brief', 'g.model_attr', 'g.review_status')->leftjoin('goods as g',
            'g.goods_id', '=', 'tg.goods_id')->where('tg.goods_id', $goods_id)->where('tg.is_team', 1)->first();
    ...
    public function teamNewGoods($type = 'is_new', $user_id = 0, $size = 10)
    {
        ...
        $list = $goods->where('tg.is_team', 1)->where('tg.is_audit', 2)->where('g.is_on_sale',
            1)->where('g.is_alone_sale', 1)->where('g.is_delete', 0)->where('g.review_status', '>',
            2)->where('g.user_id', $user_id)->orderby('tg.id', 'desc')->limit($size)->get()->toArray();
    ...
    {
        $list = \App\Models\TeamLog::from('team_log as tl')->select('tl.team_id', 'tl.goods_id', 'tl.start_time',
            'tl.status', 'tg.id', 'tg.validity_time', 'tg.team_price', 'tg.team_num', 'tg.limit_num', 'tg.astrict_num',
            'tg.is_team', 'g.goods_name')->leftjoin('team_goods as tg', 'tl.t_id', '=', 'tg.id')->leftjoin('goods as g',
            'tg.goods_id', '=', 'g.goods_id')->where('tl.team_id', $team_id)->first();
    ...
    {
        $list = \App\Models\OrderInfo::select('order_sn', 'user_id')->where('team_id',
            $team_id)->where('extension_code', 'team_buy')->where('pay_status', 2)->get()->toArray();
    ...
    {
        $list = \App\Models\TeamLog::from('team_log as tl')->select('tl.team_id', 'tl.goods_id', 'tl.start_time',
            'o.team_parent_id', 'tg.validity_time', 'tg.team_num')->leftjoin('order_info as o', 'tl.team_id', '=',
            'o.team_id')->leftjoin('team_goods as tg', 'tl.t_id', '=', 'tg.id')->where('tl.goods_id',
            $goods_id)->where('tl.status', '<', 1)->where('tl.is_show', 1)->where('o.extension_code',
            'team_buy')->where('o.team_parent_id', '>', 0)->where('o.pay_status', 2)->where('tg.is_team',
            1)->orderby('o.add_time', 'desc')->limit($size)->get()->toArray();
    ...
    {
        $info = \App\Models\TeamLog::from('team_log as tl')->select('tl.team_id', 'tl.start_time', 'tl.status',
            'o.user_id', 'o.team_parent_id', 'g.goods_id', 'g.goods_thumb', 'g.goods_img', 'g.goods_name',
            'g.goods_brief', 'tg.validity_time', 'tg.team_num', 'tg.team_price',
            'tg.is_team')->leftjoin('order_info as o', 'tl.team_id', '=', 'o.team_id')->leftjoin('goods as g',
            'tl.goods_id', '=', 'g.goods_id')->leftjoin('team_goods as tg', 'tl.t_id', '=',
            'tg.id')->where('tl.team_id', $team_id)->where('o.extension_code', 'team_buy')->where('o.team_parent_id',
            '>', 0)->first()->toArray();
        return $info;
    }
    ...
    {
        return \App\Models\OrderInfo::select('order_id', 'order_status', 'pay_status')->where('team_id',
            $team_id)->where('user_id', $user_id)->first()->toArray();
    }

    public function teamUserList($team_id = 0, $page = 1, $size = 5)
    {
        $begin = ($page - 1) * $size;
        $list = \App\Models\OrderInfo::from('order_info as o')->select('o.add_time', 'o.team_id', 'o.user_id',
            'o.team_parent_id', 'o.team_user_id')->leftjoin('users as u', 'o.user_id', '=', 'u.user_id')->where(array(
    ...
    {
        return \App\Models\OrderInfo::select('*')->where('team_id', $team_id)->where('user_id',
            $user_id)->where('extension_code', 'team_buy')->count();
    }
