...
class TeamRepository
{
	...
	public function categoryGoodsList($tc_id = 0, $page = 1, $size = 10, $keywords = '', $sortKey = 0, $sortVal = '')
	{
		...
		$begin = ($page - 1) * $size;
		$list = $goods->where('tg.tc_id', $tc_id)->where('tg.is_team', 1)->where('tg.is_audit', 2)->where('g.is_on_sale', 1)->where('g.is_alone_sale', 1)->where('g.is_delete', 0)->where('g.review_status', '>', 2)->offset($begin)->limit($size)->get()->toArray();
	...
	public function teamRankingList($page = 1, $size = 10, $type = 0)
	{
		...
		$begin = ($page - 1) * $size;
		$list = $goods->where('tg.is_team', 1)->where('tg.is_audit', 2)->where('g.is_on_sale', 1)->where('g.is_alone_sale', 1)->where('g.is_delete', 0)->where('g.review_status', '>', 2)->offset($begin)->limit($size)->get()->toArray();
	...
	public function teamNewGoods($type = 'is_new', $user_id = 0, $size = 10)
	{
		...
		$list = $goods->where('tg.is_team', 1)->where('tg.is_audit', 2)->where('g.is_on_sale', 1)->where('g.is_alone_sale', 1)->where('g.is_delete', 0)->where('g.review_status', '>', 2)->where('g.user_id', $user_id)->orderby('tg.id', 'desc')->limit($size)->get()->toArray();
