...
class GoodsRepository
{
    ...
    public function __construct(
        \App\Repositories\User\UserRankRepository $userRankRepository,
        \App\Services\AuthService $authService,
        \App\Repositories\User\MemberPriceRepository $memberPriceRepository,
        GoodsAttrRepository $goodsAttrRepository,
        VolumePriceRepository $volumePriceRepository,
        \App\Repositories\ShopConfig\ShopConfigRepository $shopConfigRepository
    ) {
        $this->setField();
    ...
    public function findBy(
        ...
        if (!empty($fil_key)) {
            $goods_fil_id = \App\Models\GoodsAttr::select('goods_id')->where('attr_value', 'like',
                '%' . $fil_key . '%')->get()->toArray();
        ...
        $res = $goods->where('is_on_sale', 1)->where('is_delete', 0)->where('review_status', '>',
            2)->offset($begin)->limit($size)->get()->toArray();
        return $res;
    }
    ...
    public function findByType($type = 'best', $size = 10)
    {
        ...
        $goods = \App\Models\Goods::select('goods_id', 'cat_id', 'user_cat', 'user_id', 'goods_sn', 'goods_name',
            'click_count', 'brand_id', 'provider_name', 'goods_number', 'market_price', 'shop_price', 'promote_price',
            'promote_start_date', 'promote_end_date', 'goods_thumb', 'goods_img', 'original_img')->where($type,
            1)->where('is_on_sale', 1)->where('is_delete', 0)->where('review_status', '>', 2)->orderBy('goods_id',
    ...
    {
        $res = \App\Models\Goods::select('goods_id', 'cat_id', 'goods_name', 'brand_id', 'shop_price as goods_price',
            'market_price', 'goods_number as stock', 'goods_desc', 'desc_mobile', 'goods_brief',
            'sales_volume as sales', 'goods_thumb', 'model_attr', 'goods_type', 'user_id', 'is_on_sale',
            'review_status', 'promote_price', 'product_price', 'product_promote_price', 'promote_start_date',
            'promote_end_date', 'goods_video', 'cloud_id')->where('goods_id', $id)->where('is_delete',
            0)->where('is_alone_sale', 1)->first();
...
', '<br />', $v['attr_value']);
    ...
    {
        return \App\Models\GoodsGallery::select('img_url')->where('goods_id', $id)->orderBy('img_id',
            'ASC')->get()->toArray();
    }

    public function goodsComment($id)
    {
        $res = \App\Models\Comment::select('comment_id as id', 'user_id', 'content', 'add_time',
            'comment_rank')->where('id_value', $id)->orderBy('comment_id', 'DESC')->get()->toArray();
        return $res;
    }
    ...
    {
        $goods = \App\Models\Goods::join('cart', 'goods.goods_id', '=', 'cart.goods_id')->where('cart.rec_id',
            $rec_id)->select('goods.goods_name', 'goods.goods_number', 'cart.product_id')->first();
    ...
    public function getFinalPrice(
        ...
        $goods = \App\Models\Goods::from('goods as g')->select('g.promote_price', 'g.promote_start_date',
            'g.promote_end_date', 'mp.user_price')->leftjoin('member_price as mp', 'mp.goods_id', '=',
            'g.goods_id')->where('g.goods_id', $goods_id)->where('g.is_delete', 0)->first()->toArray();
        ...
        if ($this->shopConfigRepository->getShopConfigByCode('add_shop_price') == 0) {
            $goods['promote_price'] = $this->goodsPropertyPrice($goods_id, $property, $warehouse_id, $area_id,
                'product_promote_price');
        }

        if (is_array($goods) && array_key_exists('promote_price', $goods) && 0 < $goods['promote_price']) {
            $promote_price = $this->bargainPrice($goods['promote_price'], $goods['promote_start_date'],
    ...
    public function goodsAttrNumber($goods_id, $attr_id, $warehouse_id = 0, $area_id = 0, $store_id = 0)
    {
        ...
        if (empty($products)) {
            $products = $this->goodsWarehouseNumber($goods_id, $warehouse_id, $area_id, $goods['model_attr'],
                $store_id);
    ...
    public function getProductsAttrNumber($goods_id, $attr_id, $warehouse_id, $area_id, $model_attr = 0, $store_id = 0)
    {
        ...
        if (0 < $store_id) {
            $product_number = \App\Models\StoreProducts::select('product_number')->where('goods_id',
                $goods_id)->where('store_id', $store_id);
        } else {
            if ($model_attr == 1) {
                $product_number = \App\Models\ProductsWarehouse::select('product_number')->where('goods_id',
                    $goods_id)->where('warehouse_id', $warehouse_id);
            } else {
                if ($model_attr == 2) {
                    $product_number = \App\Models\ProductsArea::select('product_number')->where('goods_id',
                        $goods_id)->where('area_id', $area_id);
                } else {
                    $product_number = \App\Models\Products::select('product_number', 'cloud_product_id',
                        'inventoryid')->where('goods_id', $goods_id);
                }
            }
        }
    ...
    {
        if (0 < $store_id) {
            $product_number = \App\Models\StoreGoods::select('goods_number')->where('goods_id',
                $goods_id)->where('goods_attr', $attr_id)->where('store_id', $store_id)->first();
        } else {
            if ($model_attr == 1) {
                $product_number = \App\Models\WarehouseGoods::select('region_number as product_number')->where('goods_id',
                    $goods_id)->where('region_id', $warehouse_id)->first();
            } else {
                if ($model_attr == 2) {
                    $product_number = \App\Models\WarehouseAreaGoods::select('region_number as product_number')->where('goods_id',
                        $goods_id)->where('region_id', $area_id)->first();
                } else {
                    $product_number = \App\Models\Goods::select('goods_number as product_number')->where('goods_id',
                        $goods_id)->first();
                }
            }
        }
    ...
    public function getProductsAttrPrice($goods_id, $attr_id, $warehouse_id, $area_id, $model_attr = 0)
    {
        ...
        if ($this->shopConfigRepository->getShopConfigByCode('goods_attr_price') == 1) {
            if ($model_attr == 1) {
                $product_price = \App\Models\ProductsWarehouse::select('product_price', 'product_promote_price',
                    'product_market_price')->where('goods_id', $goods_id)->where('warehouse_id', $warehouse_id);
            } else {
                if ($model_attr == 2) {
                    $product_price = \App\Models\ProductsArea::select('product_price', 'product_promote_price',
                        'product_market_price')->where('goods_id', $goods_id)->where('area_id', $area_id);
                } else {
                    $product_price = \App\Models\Products::select('product_price', 'product_promote_price',
                        'product_market_price')->where('goods_id', $goods_id);
                }
            }
    ...
    {
        if ($model_attr == 1) {
            $product_price = \App\Models\WarehouseGoods::select('warehouse_price as product_price',
                'warehouse_promote_price as product_promote_price')->where('goods_id', $goods_id)->where('region_id',
                $warehouse_id)->first();
        } else {
            if ($model_attr == 2) {
                $product_price = \App\Models\WarehouseAreaGoods::select('region_price as product_price',
                    'region_promote_price as product_promote_price')->where('goods_id', $goods_id)->where('region_id',
                    $area_id)->first();
            } else {
                $product_price = \App\Models\Goods::select('shop_price as product_price',
                    'promote_price as product_promote_price')->where('goods_id', $goods_id)->first();
            }
        }
    ...
    {
        $res = \App\Models\GoodsAttr::from('goods_attr as ga')->select('a.attr_type')->join('attribute as a',
            'ga.attr_id', '=', 'a.attr_id')->where('ga.goods_attr_id', $goods_attr_id)->first();
    ...
    public function getAttrImgFlie($goods_id, $attr_id = 0)
    {
        ...
        $res = \App\Models\GoodsAttr::select('attr_img_flie')->where('goods_id', $goods_id)->where('goods_attr_id',
            $attr_id)->first();
    ...
    {
        $screen = array();
        $screen['brand'] = \App\Models\Brand::select('brand_id', 'brand_name', 'brand_logo',
            'brand_desc')->where('is_show', 1)->groupby('brand_id')->groupby('sort_order')->orderBy('sort_order',
            'ASC')->get()->toArray();
        ...
        foreach ($regionList as $key => $value) {
            ...
            foreach ($regionList[$key]['region'] as $k => $v) {
                $regionList[$key]['region'][$k]['region'] = \App\Models\Region::where('parent_id',
                    $v['region_id'])->get()->toArray();
            }
        }
    ...
    public function goodsHistory($historylist, $page = 1, $size = 10)
    {
        ...
        foreach ($historylist as $key => $val) {
            $goods[$key] = \App\Models\Goods::select('goods_id', 'goods_name', 'goods_number', 'market_price',
                'shop_price', 'goods_thumb', 'goods_img')->where('goods_id', $val)->where('review_status', '>',
                2)->where('is_on_sale', 1)->where('is_delete', 0)->offset($begin)->limit($size)->get()->toArray();
