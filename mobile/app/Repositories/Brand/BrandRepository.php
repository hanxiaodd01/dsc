...
class BrandRepository
{
    public function getAllBrands()
    {
        ...
        $res = \App\Models\Brand::select('brand_id', 'brand_name', 'brand_logo', 'brand_desc')->where('is_show',
            1)->groupby('brand_id')->groupby('sort_order')->orderby('sort_order', 'ASC')->get()->toArray();
    ...
    {
        $brand = \App\Models\Brand::select('brand_id', 'brand_name')->where('is_show', 1)->where('is_delete',
            0)->where('brand_id', $id)->first()->toArray();
        return $brand;
    }

    private function goodsCountByBrand($brand_id)
    {
        $goodsNum = \App\Models\Goods::select()->where('brand_id', $brand_id)->where('is_on_sale',
            1)->where('is_alone_sale', 1)->where('is_delete', 0)->count();
        return $goodsNum;
    }
}
