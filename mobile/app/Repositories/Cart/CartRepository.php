...
class CartRepository
{
    ...
    public function __construct(
        ...
        \App\Repositories\Category\CategoryRepository $categoryRepository
    ) {
    ...
    public function cartInfo($rec_id, $user_id)
    {
        $cart = \App\Models\Cart::where('rec_id', $rec_id)->where('user_id', $user_id)->first();
    ...
    {
        $cart = \App\Models\Cart::select('rec_id', 'user_id', 'goods_id', 'goods_name', 'market_price', 'goods_price',
            'goods_number', 'goods_attr', 'ru_id')->get()->toArray();
        return $cart;
    }

    public function getCartByGoods($uid, $goodsId, $goodsAttr = '')
    {
        $cart = \App\Models\Cart::where('user_id', $uid)->where('goods_id', $goodsId)->where('goods_attr_id',
            $goodsAttr)->where('rec_type', 0)->first();
    ...
    public function goodsNumInCartChecked($id, $flow_type = 0)
    {
        $cart_list = \App\Models\Cart::where('user_id', $id)->where('rec_type', $flow_type)->where('is_checked',
            1)->sum('goods_number');
        return $cart_list;
    }
    ...
    public function getGoodsInCartByUser($id, $flow_type = 0, $is_checked = false)
    {
        $cart_list = \App\Models\Cart::select('cart.*')->with(array(
            'goods' => function ($query) {
                $query->select('goods_id', 'cat_id', 'brand_id', 'goods_name', 'goods_thumb', 'freight', 'tid',
                    'goods_weight', 'shipping_fee', 'cloud_id', 'cloud_goodsname', 'dis_commission', 'is_distribution');
            }
        ));

        if ($is_checked) {
            $cart_list = $cart_list->where('is_checked', 1);
        }

        $cart_list = $cart_list->where('user_id', $id)->where('rec_type', $flow_type)->orderBy('rec_id',
            'desc')->get()->toArray();
        $cart = array();
        ...
        foreach ($goods_list as $key => $row) {
            ...
            $row['goods']['is_checked'] = $row['is_checked'] == 1 ? true : false;
            $a = $row['ru_id'];
        ...
        foreach ($tmpArray as $key => $value) {
            foreach ($value['goods'] as $k) {
                if ($k['is_checked'] === false) {
                    $tmpArray[$key]['is_checked'] = false;
                    break;
                } else {
                    $tmpArray[$key]['is_checked'] = true;
                }
            }
        }

        foreach ($tmpArray as $key => $value) {
            $shipping = \App\Models\ShippingArea::select('shipping_area.*')->with(array(
    ...
    public function deleteGift($act_id, $uid)
    {
        return \App\Models\Cart::where('is_gift', $act_id)->where('user_id', $uid)->delete();
    }
    ...
    public function computeDiscountCheck($order_products)
    {
        ...
        $favourable_list = \App\Models\FavourableActivity::where('start_time', '<=', $now)->where('end_time', '>=',
            $now)->whereraw('CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'')->wherein('act_type',
            array(self::FAT_DISCOUNT, self::FAT_PRICE))->get()->toArray();
        ...
        foreach ($goods_list as $key => $good) {
            foreach ($good['goods'] as $k => $v) {
                ...
                $goods_list[$key]['price'] = $this->goodsRepository->getFinalPrice($v['goods_id'], $v['goods_number'],
                    true, $good_property);
        ...
        foreach ($favourable_list as $favourable) {
            ...
            if ($favourable['act_range'] == self::FAR_ALL) {
                ...
                if ($favourable['act_range'] == self::FAR_CATEGORY) {
                    $id_list = array();
                    ...
                    foreach ($goods_list as $goods) {
                        foreach ($goods['goods'] as $v) {
                            if (strpos(',' . $ids . ',', ',' . $v['cat_id'] . ',') !== false && $v['is_checked'] == 1) {
                                $total_amount += $v['goods_price'] * $v['goods_number'];
                            }
                        }
                    }
                } else {
                    if ($favourable['act_range'] == self::FAR_BRAND) {
                        foreach ($goods_list as $goods) {
                            foreach ($goods['goods'] as $v) {
                                if (strpos(',' . $favourable['act_range_ext'] . ',',
                                        ',' . $v['brand_id'] . ',') !== false && $v['is_checked'] == 1) {
                                    $total_amount += $v['goods_price'] * $v['goods_number'];
                                }
                            }
                        }
                    } else {
                        if ($favourable['act_range'] == self::FAR_GOODS) {
                            foreach ($goods_list as $goods) {
                                foreach ($goods['goods'] as $v) {
                                    if (strpos(',' . $favourable['act_range_ext'] . ',',
                                            ',' . $v['goods_id'] . ',') !== false && $v['is_checked'] == 1) {
                                        $total_amount += $v['goods_price'] * $v['goods_number'];
                                    }
                                }
                            }
            ...
            if (0 < $total_amount && $favourable['min_amount'] <= $total_amount && ($total_amount <= $favourable['max_amount'] || $favourable['max_amount'] == 0)) {
                if ($favourable['act_type'] == self::FAT_DISCOUNT) {
                    ...
                    if ($favourable['act_type'] == self::FAT_PRICE) {
    ...
    public function getGiveIntegral()
    {
        ...
        $allIntegral = \App\Models\Cart::from('cart as c')->select(array(
            'c.*',
            'g.give_integral as give_integral'
        ))->leftjoin('goods as g', 'c.goods_id', '=', 'g.goods_id')->where('c.goods_id', '>', 0)->where('c.parent_id',
            0)->where('c.rec_type', 0)->where('c.is_gift', 0)->where('c.user_id', $uid)->get()->toArray();
        $sum = 0;
    ...
    public function cartFavourableAmount(
        $user_id,
        $favourable,
    ...
    {
        $cart = \App\Models\Cart::select('goods_name')->where('user_id', $user_id)->wherein('goods_id',
            $is_gift_cart)->where('is_gift', $act_id)->where('rec_type', CART_GENERAL_GOODS)->get()->toArray();
        return $cart;
    }

    public function goodsNumInCartGift($id, $goods_id = 0)
    {
        $cart_list = \App\Models\Cart::where('user_id', $id)->where('goods_id', $goods_id)->where('is_gift',
            1)->sum('goods_number');
        return $cart_list;
    }

    public function updateChecked($uid, $rec_id = array())
    {
        if ($rec_id) {
            $res = \App\Models\Cart::where('user_id', $uid)->update(array('is_checked' => 0));

            foreach ($rec_id as $v) {
                $res = \App\Models\Cart::where('user_id', $uid)->where('rec_id', $v)->update(array('is_checked' => 1));
            }
        } else {
            $res = \App\Models\Cart::where('user_id', $uid)->update(array('is_checked' => 0));
        }
