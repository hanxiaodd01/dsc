...
class ActivityRepository
{
    ...
    {
        $list = \App\Models\FavourableActivity::select('*')->where('review_status', 3)->orderby('sort_order',
            'ASC')->orderby('end_time', 'DESC')->get()->toArray();
    ...
    public function activityListAll($ru_id = 0)
    {
        ...
        $activity = \App\Models\FavourableActivity::select('*')->where('start_time', '<=', $gmtime)->where('end_time',
            '>=', $gmtime);
        ...
        $list = $activity->where('review_status',
            3)->whereraw('CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'')->get()->toArray();
    ...
    public function activityRangeExt($ru_id = 0, $act_range)
    {
        ...
        $res = $activity->where('review_status', 3)->where('start_time', '<=', $gmtime)->where('end_time', '>=',
            $gmtime)->where('act_range',
            $act_range)->whereraw('CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'')->get()->toArray();
    ...
    {
        $list = \App\Models\FavourableActivity::select('*')->where('review_status', 3)->where('act_id',
            $id)->first()->toArray();
    ...
    public function activityGoods(
        $filter = array('goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0),
        $page,
        $size
    ) {
        $begin = ($page - 1) * $size;
        ...
        $list = $goods->where('g.is_on_sale', 1)->where('g.is_alone_sale', 1)->where('g.is_delete',
            0)->offset($begin)->orderby('g.sort_order', 'ASC')->limit($size)->get()->toArray();
    ...
    public function getActivitytype($act_id = 0)
    {
        ...
        $user_rank = $this->userRankRepository->getUserRankByUid();
        $user_rank = ',' . $user_rank['rank_id'] . ',';
        $activity = \App\Models\FavourableActivity::select('*')->where('review_status', 3)->where('act_id',
            $act_id)->where('start_time', '<=', $gmtime)->where('end_time', '>=',
            $gmtime)->whereraw('CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'')->first()->toArray();
    ...
    public function cartFavourableGoods($user_id = 0, $act_id = 0)
    {
        ...
        $user_rank = $this->userRankRepository->getUserRankByUid();
        $user_rank = ',' . $user_rank['rank_id'] . ',';
        $favourable = \App\Models\FavourableActivity::select('*')->where('review_status', 3)->where('act_id',
            $act_id)->where('start_time', '<=', $gmtime)->where('end_time', '>=',
            $gmtime)->whereraw('CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'')->first()->toArray();
