...
class DrpRepository
{
    ...
    public function __construct(
        ...
        \App\Repositories\Goods\GoodsRepository $goodsRepository,
    ...
    {
        if ($type === 0) {
            $res = \App\Models\DrpAffiliateLog::where('separate_type', '!=', '-1')->where('user_id',
                $uid)->sum('money');
        } else {
            if ($type === 1) {
                $res = \App\Models\DrpAffiliateLog::where('separate_type', '!=', '-1')->where('time', '>=',
                    mktime(0, 0, 0))->where('user_id', $uid)->sum('money');
            } else {
                $res = \App\Models\OrderGoods::from('order_goods as o')->leftjoin('drp_affiliate_log as a',
                    'o.order_id', '=', 'a.order_id')->where('a.separate_type', '!=', '-1')->where('a.user_id',
                    $uid)->sum('goods_price');
            }
        }
    ...
    {
        $res = \App\Models\Users::from('users as u')->select('u.user_id', 'u.user_name', 'u.nick_name',
            'u.user_picture', 'u.reg_time', 's.status', 's.audit')->leftjoin('drp_shop as s', 'u.user_id', '=',
            's.user_id')->where('u.drp_parent_id', $uid)->where('s.status', 1)->where('s.audit',
            1)->orderBy('u.reg_time', 'desc')->get()->toArray();
    ...
    public function drp_rank_info($user_id)
    {
        ...
        if (!empty($drp_info)) {
            ...
            $rank_info = \App\Models\DrpUserCredit::select('id', 'credit_name', 'min_money', 'max_money')->where('id',
                1)->first();
            $rank_info = $rank_info->toArray();

            if (0 < $drp_info['credit_id']) {
                $rank_info = \App\Models\DrpUserCredit::select('id', 'credit_name', 'min_money',
                    'max_money')->where('id', $drp_info['credit_id'])->first();
                $rank_info = $rank_info->toArray();
            }
        } else {
            $totals = \App\Models\OrderGoods::from('order_goods as o')->leftjoin('drp_affiliate_log as a', 'o.order_id',
                '=', 'a.order_id')->where('a.separate_type', '!=', -1)->where('a.user_id', $user_id)->sum('money');
            $goods_price = $totals ? $totals : 0;
            $rank_info = \App\Models\DrpUserCredit::select('id', 'credit_name', 'min_money',
                'max_money')->where('min_money', '<=', $goods_price)->where('max_money', '>', $goods_price)->first();
            $rank_info = $rank_info->toArray();
        }
    ...
    {
        $res = \App\Models\DrpLog::from('drp_log as dl')->select('dl.user_id', 'u.nick_name', 'u.user_name', 'dl.money',
            'o.*', 'dl.point', 'dl.drp_level', 'dl.is_separate')->leftjoin('order_info as o', 'o.order_id', '=',
            'dl.order_id')->leftjoin('wechat_user as w', 'w.ect_uid', '=', 'o.user_id')->leftjoin('users as u',
            'u.user_id', '=', 'o.user_id')->offset($corrent)->where('dl.user_id', $uid)->where('o.pay_status', 2);
        ...
        $res = $res->whereIn('dl.is_separate', array(0, 1))->orderBy('order_id',
    ...
    {
        $res = \App\Models\DrpLog::from('drp_log as dl')->select('dl.user_id', 'u.nick_name', 'u.user_name', 'dl.money',
            'o.*', 'dl.point', 'dl.drp_level')->leftjoin('order_info as o', 'o.order_id', '=',
            'dl.order_id')->leftjoin('wechat_user as w', 'w.ect_uid', '=', 'o.user_id')->leftjoin('users as u',
            'u.user_id', '=', 'o.user_id')->where('dl.user_id', $uid)->where('o.order_id',
            $order_id)->where('o.pay_status', 2)->first();
        return $res->toArray();
    }

    public function ordergoods($order_id)
    {
        $res = \App\Models\OrderGoods::from('order_goods as og')->select('og.rec_id', 'og.goods_id', 'og.goods_name',
            'og.goods_attr', 'og.goods_number', 'og.goods_price', 'og.drp_money',
            'g.goods_thumb')->leftjoin('goods as g', 'og.goods_id', '=', 'g.goods_id')->where('og.order_id',
            $order_id)->where('og.is_distribution', 1)->where('og.drp_money', '>', 0)->get()->toArray();
        return $res;
    }
    ...
    {
        $list = \App\Models\Goods::select('goods_id', 'cat_id', 'goods_name', 'shop_price',
            'goods_thumb')->where('is_real', 1)->where('is_on_sale', 1)->where('is_alone_sale',
            1)->where('review_status', '>', 2)->where('is_show', 1)->where('is_distribution', 1)->where('cat_id',
            $cat_id)->get()->toArray();

        foreach ($list as $k => $v) {
            $list[$k]['goods_thumb'] = get_image_path($v['goods_thumb']);
            $type = \App\Models\DrpType::select('id')->where('goods_id', $v['goods_id'])->where('user_id',
                $uid)->first();
    ...
    public function showgoods($uid, $corrent, $size, $type, $status = 0)
    {
        if ($type == 0) {
            ...
            if ($status == 2) {
                ...
                if ($status == 3) {
                    $time = gmtime();
                    $goods = $goods->where('promote_price', '>', 0)->where('promote_start_date', '<=',
                        $time)->where('promote_end_date', '>=', $time);
                }
            }

            $res = $goods->where('is_delete', 0)->where('is_real', 1)->where('is_on_sale', 1)->where('is_alone_sale',
                1)->where('review_status', '>', 2)->where('is_show', 1)->where('is_distribution', 1)->get()->toArray();

            foreach ($res as $k => $v) {
                $res[$k]['goods_thumb'] = get_image_path($v['goods_thumb']);
            }
        } else {
            $list = \App\Models\DrpType::select('goods_id', 'cat_id')->offset($corrent)->where('user_id',
                $uid)->where('type', $type)->limit($size)->get()->toArray();
            $res = array();

            if (!empty($list)) {
                foreach ($list as $k => $v) {
                    if ($type == 2) {
                        $goods = \App\Models\Goods::select('goods_id', 'cat_id', 'goods_name', 'shop_price',
                            'goods_thumb');
                        ...
                        $goods = $goods->where('is_delete', 0)->where('is_real', 1)->where('is_on_sale',
                            1)->where('is_alone_sale', 1)->where('review_status', '>', 2)->where('is_show',
                            1)->where('is_distribution', 1)->where('goods_id', $v['goods_id'])->first();
                        ...
                        $res[$k] = $goods->where('is_delete', 0)->where('is_real', 1)->where('is_on_sale',
                            1)->where('is_alone_sale', 1)->where('review_status', '>', 2)->where('is_show',
                            1)->where('is_distribution', 1)->where('cat_id', $v['cat_id'])->get()->toArray();
    ...
    {
        $res = \App\Models\DrpLog::from('drp_log as a')->select('o.order_sn', 'a.time', 'a.user_id', 'a.time',
            'a.money', 'a.point', 'a.separate_type', 'w.nickname', 'u.user_name',
            'a.is_separate')->leftjoin('order_info as o', 'o.order_id', '=', 'a.order_id')->leftjoin('wechat_user as w',
            'w.ect_uid', '=', 'o.user_id')->leftjoin('users as u', 'o.user_id', '=', 'u.user_id')->offset($corrent);
        ...
        $res = $res->where('a.user_id', $uid)->where('o.order_status', 1)->where('o.pay_status',
            2)->limit($size)->get()->toArray();
    ...
    {
        $res = \App\Models\Users::from('users as u')->select('u.drp_parent_id')->leftjoin('drp_shop as ds',
            'u.drp_parent_id', '=', 'ds.user_id')->where('u.user_id', $uid)->where('ds.audit', 1)->where('ds.status',
            1)->first();
    ...
    {
        $res = \App\Models\Article::select('title', 'content')->where('is_open', 1)->where('cat_id',
            1000)->orderBy('add_time', 'desc')->get()->toArray();
        return $res;
    }
    ...
    public function drpOrderInfo()
    {
        ...
        $model = \App\Models\OrderInfo::MainOrderCount()->where('order_id', '>', $last_oid)->where('drp_is_separate',
            0);
    ...
    {
        $row = \App\Models\OrderInfo::from('order_info as o')->select('o.order_sn', 'o.drp_is_separate',
            'o.user_id')->leftjoin('order_goods as og', 'o.order_id', '=', 'og.order_id')->where('o.order_id',
            $order_id)->first();
    ...
    {
        $user = \App\Models\Users::from('users as u')->select('u.drp_parent_id as user_id',
            'ds.id as drp_shop_id')->leftjoin('drp_shop as ds', 'ds.user_id', '=',
            'u.drp_parent_id')->where('u.user_id', $user_id)->first();
    ...
    {
        $user = \App\Models\Users::from('users as u')->select('u.user_id',
            'ds.id as drp_shop_id')->leftjoin('drp_shop as ds', 'u.user_id', '=', 'ds.user_id')->where('ds.user_id',
            $user_id)->first();
