...
class BargainRepository
{
    ...
    {
        $info = \App\Models\BargainStatisticsLog::select('*');

        if (0 < $bs_id) {
            $info->where('id', $bs_id);
        }

        $tatistics_og = $info->where('bargain_id', $bargain_id)->where('user_id', $user_id)->where('status',
            0)->first();
    ...
    {
        $bargain_info = \App\Models\BargainStatistics::select('*')->where('bs_id', $bs_id)->where('user_id',
            $user_id)->first();
    ...
    public function bargainPositions($tc_type = 'weapp', $num = 3)
    {
        ...
        $res = \App\Models\TouchAd::select('ad_id', 'touch_ad.position_id', 'media_type', 'ad_link', 'ad_code',
            ...
            $time)->where('touch_ad_position.position_id', $tc_type)->where('enabled',
    ...
    {
        $time = gmtime();
        $goods = \App\Models\BargainGoods::from('bargain_goods as bg')->select('bg.id', 'bg.start_time',
            'bg.goods_price', 'bg.end_time', 'bg.target_price', 'bg.total_num', 'g.goods_id', 'g.goods_name',
            'g.shop_price', 'g.market_price', 'g.goods_thumb', 'g.goods_img')->leftjoin('goods as g', 'g.goods_id', '=',
            'bg.goods_id');
        ...
        $begin = ($page - 1) * $size;
        $list = $goods->where('bg.status', 0)->where('bg.is_audit', 2)->where('bg.is_delete', 0)->where('bg.start_time',
            '<=', $time)->where('bg.end_time', '>=', $time)->where('g.is_on_sale', 1)->where('g.is_alone_sale',
            1)->where('g.is_delete', 0)->where('g.review_status', '>', 2)->offset($begin)->orderby('bg.id',
    ...
    {
        $res = \App\Models\BargainGoods::from('bargain_goods as bg')->select('bg.id', 'bg.goods_id', 'bg.goods_price',
            'bg.start_time', 'bg.end_time', 'bg.target_price', 'bg.min_price', 'bg.max_price', 'bg.total_num',
            'bg.status', 'bg.bargain_desc', 'g.user_id', 'g.goods_sn', 'g.goods_name', 'g.is_real', 'g.is_shipping',
            'g.is_on_sale', 'g.shop_price', 'g.market_price', 'g.goods_thumb', 'g.goods_img', 'g.goods_number',
            'g.goods_desc', 'g.desc_mobile', 'g.goods_type', 'g.goods_brief', 'g.model_attr',
            'g.review_status')->leftjoin('goods as g', 'g.goods_id', '=', 'bg.goods_id')->where('bg.id',
            $bargain_id)->first();
    ...
    {
        $list = \App\Models\BargainStatistics::select('user_id', 'add_time', 'subtract_price')->where('bs_id',
            $bs_id)->orderby('add_time', 'desc')->get();
    ...
    public function getFinalPrice(
        ...
        $goods = \App\Models\Goods::from('goods as g')->select('g.shop_price', 'g.promote_price',
            'g.promote_start_date', 'g.promote_end_date', 'mp.user_price')->leftjoin('member_price as mp',
            'mp.goods_id', '=', 'g.goods_id')->where('g.goods_id', $goods_id)->where('g.is_delete',
            0)->first()->toArray();
    ...
    public function bargainTargetPrice(
        $bargain_id = 0,
        $goods_id = 0,
        $attr_id = array(),
        $warehouse_id = 0,
        ...
        if ($this->shopConfigRepository->getShopConfigByCode('goods_attr_price') == 1) {
            if ($model_attr == 1) {
                $product_price = \App\Models\ProductsWarehouse::select('product_id', 'product_price',
                    'product_promote_price', 'product_market_price')->where('goods_id', $goods_id)->where('goods_attr',
                    $attr_id)->where('warehouse_id', $warehouse_id)->first()->toArray();
            } else {
                if ($model_attr == 2) {
                    $product_price = \App\Models\ProductsArea::select('product_id', 'product_price',
                        'product_promote_price', 'product_market_price')->where('goods_id',
                        $goods_id)->where('goods_attr', $attr_id)->where('area_id', $area_id)->first()->toArray();
                } else {
                    $product_price = \App\Models\Products::select('product_id', 'product_price',
                        'product_promote_price', 'product_market_price')->where('goods_id',
                        $goods_id)->where('goods_attr', $attr_id)->first()->toArray();
                }
            }

            if ($product_price['product_id']) {
                $res = \App\Models\ActivityGoodsAttr::select('target_price')->where('bargain_id',
                    $bargain_id)->where('goods_id', $goods_id)->where('product_id',
                    $product_price['product_id'])->first()->toArray();
            }
    ...
    {
        \App\Models\BargainStatisticsLog::where('id', $bs_id)->update(array(
            'count_num' => $count_num,
    ...
    {
        $begin = ($page - 1) * $size;
        $goods = \App\Models\BargainStatisticsLog::from('bargain_statistics_log as bsl')->select('bg.id',
            'bg.target_price', 'bg.total_num', 'g.goods_id', 'g.goods_name', 'g.shop_price', 'g.goods_thumb',
            'g.goods_img')->leftjoin('bargain_goods as bg', 'bsl.bargain_id', '=', 'bg.id')->leftjoin('goods as g',
            'bg.goods_id', '=', 'g.goods_id')->offset($begin)->where('bsl.user_id', $user_id)->orderby('bsl.add_time',
            'desc')->limit($size)->get()->toArray();
        return $goods;
    }
