...
class OrderRepository
{
    ...
    public function __construct(
        ...
        \App\Repositories\Bonus\BonusTypeRepository $bonusTypeRepository,
    ...
    {
        $model = \App\Models\OrderInfo::MainOrderCount()->select('*')->where('user_id', $id)->where('order_status',
            '<>', OS_CANCELED)->where('extension_code', '');
    ...
    {
        $model = \App\Models\OrderInfo::select('order_id')->with(array(
            'goods' => function ($query) use ($goodsId) {
                $query->leftjoin('goods', 'goods.goods_id', '=', 'order_goods.goods_id')->leftjoin('comment',
                    'comment.rec_id', '=', 'order_goods.rec_id')->where('goods.goods_id',
                    $goodsId)->select('order_goods.order_id', 'order_goods.goods_id', 'order_goods.goods_name',
                    'order_goods.goods_attr', 'order_goods.goods_price', 'goods.goods_thumb', 'order_goods.goods_price',
                    'comment.rec_id');
            }
    ...
    {
        $order = \App\Models\OrderInfo::select('order_sn')->where('user_id', $uid)->where('main_order_id',
            $orderId)->get()->toArray();
        return $order;
    }
    ...
    public function orderPay($uid, array $orderId)
    {
        $array = array(
            ...
            'pay_status' => PS_PAYED,
            'pay_time' => gmtime(),
            'money_paid' => \Illuminate\Support\Facades\DB::Raw('order_amount'),
            'order_amount' => 0
        );
    ...
    {
        $goods = \App\Models\OrderGoods::where('order_id', $orderId)->select('goods.goods_thumb',
            'order_goods.goods_price', 'order_goods.goods_number', 'order_goods.goods_id', 'order_goods.goods_name',
            'order_goods.goods_sn', 'order_goods.ru_id', 'order_goods.drp_money')->join('goods', 'goods.goods_id', '=',
            'order_goods.goods_id')->get();
    ...
    public function getOrderByUserId($id, $status = 0, $type = '', $page = 0, $size = 10)
    {
        ...
        $order = $model->select(array(
            ...
            'goods' => function ($query) {
                $query->leftjoin('goods', 'goods.goods_id', '=', 'order_goods.goods_id')->select('order_goods.order_id',
                    'order_goods.goods_number', 'order_goods.goods_id', 'order_goods.goods_name',
                    'order_goods.goods_attr', 'order_goods.goods_price', 'goods.goods_thumb', 'goods.user_id');
            }
    ...
    public function changeOrderGoodsStorage($order_id, $is_dec = true, $storage = 0)
    {
        switch ($storage) {
            case 0:
                $res = \App\Models\OrderGoods::where('order_id', $order_id)->where('is_real',
                    1)->groupBy('goods_id')->groupBy('product_id')->select(array(
                    'sum(send_number) as num',
                    'goods_id,max(extension_code) as extension_code',
                    'product_id'
                ))->get()->toArray();
                break;
    ...
    public function change_goods_storage($good_id, $product_id, $number = 0)
    {
        ...
        if (!empty($product_id)) {
            $products_query = \App\Models\Products::where('goods_id', $good_id)->where('product_id',
                $product_id)->first();
    ...
    public function order_fee($order, $goods, $consignee, $cart_good_id = 0, $shipping, $consignee_id)
    {
        ...
        if (0 < count($shippingArr) && 0 < $total['real_goods_count']) {
            ...
            foreach ($shippingArr as $k => $v) {
                ...
                $shipRes = app('App\\Services\\flowService')->getRuShippngInfo($newGoodsListShip, $cart_value,
                    $temp[0]);
