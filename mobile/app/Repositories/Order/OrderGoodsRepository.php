...
class OrderGoodsRepository
{
    public function insertOrderGoods($goods, $orderId = 0)
    {
        foreach ($goods as $v) {
            ...
            $is_distribution = isset($v['is_distribution']) ? $v['is_distribution'] * $v['isdistribution'] : 0;
            ...
            $orderGoods->drp_money = !isset($v['dis_commission']) || !isset($v['is_distribution']) ? 0 : $v['dis_commission'] * $v['is_distribution'] * $v['goods_price'] * $v['goods_number'] / 100 * $v['isdistribution'];
            $orderGoods->save();
        }
    }
    ...
    {
        $goods = \App\Models\OrderGoods::from('order_goods as og')->select('og.rec_id')->leftjoin('goods as g',
            'g.goods_id', '=', 'og.goods_id')->where('og.order_id', $oid)->where('g.cloud_id', $gid)->first();
    ...
    {
        $goods = \App\Models\OrderCloud::from('order_cloud as oc')->select('oc.parentordersn', 'og.goods_number',
            'og.goods_price')->leftjoin('order_goods as og', 'og.rec_id', '=', 'oc.rec_id')->where('og.order_id',
            $oid)->first();
