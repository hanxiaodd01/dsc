...
class ArticleRepository
{
    public function all($cat_id, $columns = array('*'), $page, $size)
    {
        ...
        $res['article'] = $article->offset($current)->limit($size)->orderBy('sort_order', 'ASC')->orderBy('add_time',
            'DESC')->get()->toArray();
    ...
    public function articleCatInfo($id)
    {
        ...
        $article_cat = \App\Models\Article::join('article_cat', 'article.cat_id', '=',
            'article_cat.cat_id')->where($where)->select('article_cat.cat_id', 'article_cat.cat_name')->first();
