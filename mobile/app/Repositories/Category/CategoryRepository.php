...
class CategoryRepository
{
    ...
    {
        $goods = \App\Models\Goods::select('goods_id', 'goods_sn', 'goods_name')->where('cat_id',
            $id)->get()->toArray();
        return $goods;
    }
    ...
    private function getTree($uid, $tree_id = 0, $top = 0)
    {
        ...
        if (0 < $count || $tree_id == 0) {
            $res = \App\Models\Category::select('cat_id', 'cat_name', 'touch_icon', 'parent_id', 'cat_alias_name',
                'is_show')->where('parent_id', $tree_id)->where('is_show', 1)->with(array(
                'goods' => function ($query) {
                    $query->select('goods_id', 'cat_id', 'goods_thumb')->where('is_on_sale', 1)->where('is_delete',
                        0)->orderby('sort_order', 'ASC')->orderby('goods_id', 'DESC');
                }
            ))->orderby('sort_order', 'ASC')->orderby('cat_id', 'ASC')->get()->toArray();

            foreach ($res as $k => $row) {
                if ($row['is_show']) {
                    $type = \App\Models\DrpType::select('id')->where('cat_id', $row['cat_id'])->where('user_id',
                        $uid)->first();
    ...
    public function catList($cat_id = 0)
    {
        ...
        if (0 < $count) {
            $res = \App\Models\Category::select('cat_id', 'cat_name', 'touch_icon', 'parent_id', 'cat_alias_name',
                'is_show')->where('parent_id', $cat_id)->where('is_show', 1)->orderby('sort_order',
                'ASC')->orderby('cat_id', 'ASC')->get()->toArray();
