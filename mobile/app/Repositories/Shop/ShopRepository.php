...
class ShopRepository
{
    ...
    {
        $list = \App\Models\SellerShopinfo::select('ru_id', 'shop_name', 'shop_logo', 'shopname_audit')->with(array(
            'MerchantsShopInformation' => function ($query) {
    ...
    {
        $time = local_gettime();
        $ads = \App\Models\TouchAd::select('ad_id', 'touch_ad.position_id', 'media_type', 'ad_link', 'ad_code',
            'ad_name')->with(array('position'))->join('touch_ad_position', 'touch_ad_position.position_id', '=',
            'touch_ad.position_id')->where('start_time', '<=', $time)->where('touch_ad_position.tc_type',
            'banner')->where('touch_ad_position.ad_type', $tc_type)->where('touch_ad_position.tc_id',
            0)->where('end_time', '>=', $time)->where('enabled', 1);
    ...
    public function get_select_find_in_set(
        $is_db = 0,
        $select_id,
        $select_array = array(),
        $where = '',
    ...
    public function getAd()
    {
        ...
        foreach ($position_id as $key => $value) {
            $res['ad'][$key] = \App\Models\TouchAd::select('ad_id', 'position_id', 'media_type', 'ad_link', 'ad_code',
                'ad_name')->where('position_id', $value)->get()->toArray();
    ...
    public function getStore()
    {
        ...
        foreach ($store as $key => $val) {
            ...
            $store[$key]['goods'] = \App\Models\Goods::select('goods_id', 'goods_name', 'goods_thumb')->where('user_id',
                $val['user_id'])->where('is_on_sale', '1')->where('review_status', '>', 2)->where('is_alone_sale',
                '1')->limit(3)->orderBy('sort_order', 'ASC')->get()->toArray();
    ...
    {
        $res = \App\Models\Article::select('title', 'content', 'article_id')->where('is_open', 1)->where('cat_id', '>',
            0)->limit(10)->orderBy('add_time', 'desc')->get()->toArray();
        return $res;
    }
}
