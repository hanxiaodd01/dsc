...
class UserRepository
{
    ...
    {
        $user = \App\Models\Users::where('user_id', $uid)->select('user_id as id', 'user_name', 'nick_name', 'sex',
            'birthday', 'user_money', 'frozen_money', 'pay_points', 'rank_points', 'address_id', 'qq', 'mobile_phone',
            'user_picture')->first();
    ...
    {
        $connectUser = \App\Models\Users::select('users.user_id')->leftjoin('connect_user', 'connect_user.user_id', '=',
            'users.user_id')->where('connect_user.open_id', $unionid)->first();
    ...
    {
        $list = \App\Models\WechatUser::from('wechat_user as wu')->select('wu.openid')->leftjoin('connect_user as cu',
            'cu.open_id', '=', 'wu.unionid')->where('cu.user_id', $uid)->first();
