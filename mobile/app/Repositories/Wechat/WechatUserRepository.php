...
class WechatUserRepository
{
    ...
    public function getWechatConfig($ru_id = 0)
    {
        ...
        $wechat = \App\Models\Wechat::where($where)->select('id', 'name', 'orgid', 'weixin', 'token', 'appid',
            'appsecret', 'type', 'status')->first();
    ...
    {
        $wechat = $this->getWechatConfig();
        $wechatuser = \App\Models\WechatUser::where('unionid', $unionid)->where('wechat_id',
            $wechat['id'])->select('subscribe', 'openid', 'nickname', 'sex', 'city', 'country', 'province',
            'headimgurl', 'unionid', 'ect_uid')->first();
