...
class GoodsController extends \App\Api\Controllers\Controller
{
    ...
    public function goodsList(\Illuminate\Http\Request $request)
    {
        $this->validate($request,
            array('page' => 'required|int', 'warehouse_id' => 'required|integer', 'area_id' => 'required|integer'));
        $list = $this->goodsService->getGoodsList($request->get('id'), $request->get('keyword'), $request->get('page'),
            $request->get('per_page'), $request->get('sort_key'), $request->get('sort_value'),
            $request->get('warehouse_id'), $request->get('area_id'), $request->get('proprietary'),
            $request->get('price_min'), $request->get('price_max'), $request->get('brand'),
            $request->get('province_id'), $request->get('city_id'), $request->get('county_id'),
            $request->get('fil_key'));
    ...
    public function property(\Illuminate\Http\Request $request)
    {
        ...
        $price = $this->goodsService->goodsPropertiesPrice($request->get('id'), $request->get('attr_id'),
