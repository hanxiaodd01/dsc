...
class TeamController extends \App\Api\Controllers\Controller
{
    ...
    public function __construct(
        \App\Services\TeamService $teamService,
        \App\Services\AuthService $authService,
        \App\Services\GoodsService $goodsService
    ) {
    ...
    public function teamList(\Illuminate\Http\Request $request)
    {
        $this->validate($request,
            array('page' => 'required|integer', 'size' => 'required|integer', 'tc_id' => 'required|integer'));
    ...
    public function categoryList(\Illuminate\Http\Request $request)
    {
        ...
        $list = $this->teamService->categoryGoodsList($request->get('tc_id'), $request->get('page'),
            $request->get('size'), $request->get('keyword'), $request->get('sort_key'), $request->get('sort_value'));
        return $this->apiReturn($list);
    }
    ...
    public function teamRanking(\Illuminate\Http\Request $request)
    {
        ...
        $list = $this->teamService->teamRankingList($request->get('page'), $request->get('size'),
            $request->get('type'));
        return $this->apiReturn($list);
    }
    ...
    public function teamProperty(\Illuminate\Http\Request $request)
    {
        ...
        $price = $this->teamService->goodsPropertiesPrice($request->get('goods_id'), $request->get('attr_id'),
            $request->get('num'), $request->get('warehouse_id'), $request->get('area_id'));
    ...
    {
        $this->validate($request,
            array('team_id' => 'required|int', 'page' => 'required|integer', 'size' => 'required|integer'));
    ...
    {
        $this->validate($request,
            array('type' => 'required|int', 'page' => 'required|integer', 'size' => 'required|integer'));
        $uid = $this->authService->authorization();
        if (isset($uid['error']) && 0 < $uid['error']) {
            $uid = 0;
        }

        $list = $this->teamService->teamUserOrder($uid, $request->get('type'), $request->get('page'),
            $request->get('size'));
        return $this->apiReturn($list);
    }
}
