            ...
            if ($row['freight'] && $row['is_shipping'] == 0) {
                if ($row['freight'] == 1) {
                    ...
                    if (isset($trow['freight_type']) && $trow['freight_type'] == 0) {
                        ...
                        $goods_transport = app('App\\Repositories\\Shop\\ShopRepository')->get_select_find_in_set(2,
                            $consignee['city'], $transport, $transport_where, 'goods_transport_extend', 'area_id');

                        if ($goods_transport) {
                            $ship_transport = array('tid', 'ru_id', 'shipping_fee');
                            $ship_transport_where = ' AND ru_id = \'' . $row['ru_id'] . '\' AND tid = \'' . $row['tid'] . '\'';
                            $goods_ship_transport = app('App\\Repositories\\Shop\\ShopRepository')->get_select_find_in_set(2,
                                $shipping_id, $ship_transport, $ship_transport_where, 'goods_transport_express',
                                'shipping_id');
                        }
