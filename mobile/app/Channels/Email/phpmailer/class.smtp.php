...
class SMTP
{
    ...
    public function connect($host, $port = null, $timeout = 30, $options = array())
    {
        ...
        $this->edebug('Connection: opening to ' . $host . ':' . $port . ', t=' . $timeout . ', opt=' . var_export($options,
                true), self::DEBUG_CONNECTION);
        $errno = 0;
        $errstr = '';
    ...
    public function authenticate($username, $password, $authtype = null, $realm = '', $workstation = '')
    {
        ...
        switch ($authtype) {
            case 'PLAIN':
                ...
                if (!$this->sendCommand('User & Password', base64_encode("\x00" . $username . "\x00" . $password),
                    235)) {
                    return false;
                }
    ...
    protected function sendCommand($command, $commandstring, $expect)
    {
        ...
        if (preg_match('/^([0-9]{3})[ -](?:([0-9]\\.[0-9]\\.[0-9]) )?/', $this->last_reply, $matches)) {
            ...
            $detail = preg_replace('/' . $code . '[ -]' . ($code_ex ? str_replace('.', '\\.',
                        $code_ex) . ' ' : '') . '/m', '', $this->last_reply);
        } else {
