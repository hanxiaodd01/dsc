...
class alipay
{
	...
	{
		$payData = array('body' => $order['order_sn'], 'subject' => $order['order_sn'], 'order_no' => make_trade_no($order['log_id'], $order['order_amount']), 'timeout_express' => gmtime() + 3600 * 24 * 9, 'amount' => $order['order_amount'], 'return_param' => (string) $order['log_id'], 'client_ip' => isset($_SERVER['REMOTE_ADDR']) ? $_SERVER['REMOTE_ADDR'] : '127.0.0.1', 'goods_type' => 1, 'store_id' => '');
