        ...
        if (IS_AJAX) {
            ...
            if (!empty($store_list)) {
                foreach ($store_list as $k => $v) {
                    $store_list[$k]['map_url'] = url('offline_store/index/map', array(
                        'address' => $v['province'] . $v['city'] . $v['district'] . trim($v['stores_address']),
                    ...
                    if (is_spec($is_spec) == true) {
                        $products = get_warehouse_id_attr_number($v['goods_id'], $spec_arr, $v['ru_id'], 0, 0, '',
                            $v['id']);
