...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
                ...
                if (isset($_SESSION['user_id']) && 0 < $user_id && $_SESSION['user_id'] == $user_id && !empty($res['unionid'])) {
                    ...
                    if (!empty($_SESSION['unionid']) && isset($_SESSION['unionid']) || $res['unionid']) {
                        if (!empty($refer) && $refer == 'user') {
                            $this->redirect('oauth/index/bindregister',
                                array('type' => $type, 'back_url' => $back_url));
                        }
                        ...
                        show_message(L('msg_author_register_error'), L('msg_go_back'), url('user/login/index'),
                            'error');
                    }
                }
            } else {
    ...
    protected function UserBind($res, $user_id, $type)
    {
        ...
        if ($users && !empty($res['unionid'])) {
            $connect_user_id = dao('connect_user')->where(array(
                'open_id' => $res['unionid'],
    ...
    {
        $wxinfo = dao('wechat')->field('id, token, appid, appsecret, encodingaeskey')->where(array(
            'default_wx' => 1,
            'status' => 1
        ))->find();
        ...
        if ($wxinfo) {
            ...
            if (file_exists($file)) {
                ...
                $fromusername = $_SESSION['openid'];
                $data = $wechat->returnData($fromusername, $addons);
                ...
                if (0 < $new_user_id) {
                    ...
                    if (!empty($from_connect_user)) {
                        foreach ($from_connect_user as $key => $value) {
                            dao('connect_user')->where('user_id = ' . $value['user_id'])->setField('user_id',
                                $new_user_id);
                        }
                    }
                    ...
                    if (is_dir(APP_WECHAT_PATH)) {
                        ...
                        if (!empty($from_wechat_user)) {
                            dao('wechat_user')->where('ect_uid = ' . $from_wechat_user['ect_uid'])->setField('ect_uid',
                                $new_user_id);
                        }
                    }
                    ...
                    if ($res == true) {
                        $this->users->logout();
                        $back_url = empty($back_url) ? url('user/index/index') : $back_url;
                        show_message(L('logout'), array(L('back_up_page'), '返回首页'), array($back_url, url('/')),
                            'success');
                    }
