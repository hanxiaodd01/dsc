...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        $guess_goods = get_guess_goods($_SESSION['user_id'], 1, 1, 10, $this->region_id, $this->area_id,
            $this->area_city);
                ...
                if (0 < count($goods_arr)) {
                    if ($status == 0) {
                        ...
                        exit(json_encode(array(
                            'msg' => L('already_attention_check_shop'),
                            'status' => 1,
                            'error' => 0
                        )));
                    } else {
        ...
        if (IS_AJAX) {
            ...
            if ($xiangouInfo['is_xiangou'] == 1 && $start_date < $nowTime && $nowTime < $end_date) {
                ...
                if ($row['goods_number'] < $goods_number) {
                    ...
                    $result['error'] = 1;
                    $result['msg'] = sprintf(L('stock_insufficiency'), $row['goods_name'], $row['goods_number'],
                        $row['goods_number']);
                    exit(json_encode($result));
                }
                ...
                if (0 < intval($GLOBALS['_CFG']['use_storage']) && $row['extension_code'] != 'package_buy') {
                    ...
                    if (!empty($row['product_id'])) {
                        ...
                        if ($product_number < $goods_number) {
                            ...
                            $result['error'] = 2;
                            $result['msg'] = sprintf(L('stock_insufficiency'), $row['goods_name'], $product_number,
                                $product_number);
                            exit(json_encode($result));
                        }
                    } else {
            ...
            if (0 < $goods_number) {
                if (0 < $row['group_number'] && $row['group_number'] < $goods_number && !empty($row['group_id'])) {
                    $result['error'] = 1;
                    $result['msg'] = sprintf(L('group_stock_insufficiency'), $row['goods_name'], $row['group_number'],
                        $row['group_number']);
                    exit(json_encode($result));
                }
                ...
                if ($row['extension_code'] == 'package_buy') {
                    ...
                    $goods_price = get_final_price($row['goods_id'], $goods_number, true, $attr_id, $warehouse_id,
    ...
    public function actionCartLabelCount()
    {
        if (IS_POST) {
            ...
            $cart_goods = cart_goods(CART_GENERAL_GOODS, $rec_id, 0, $this->region_id, $this->area_id,
                $this->area_city);
            $discount = compute_discount(3, $rec_id);
            $fav_amount = $discount['discount'];
            ...
            if (0 < $type) {
                ...
                $merchant_goods_list = cart_by_favourable($cart_goods['goods_list'],
                    $cart_goods['total']['cart_value']);
                $this->assign('goods_list', $merchant_goods_list);
                $this->assign('total', $cart_goods['total']);
                $this->assign('cart_value', $cart_goods['total']['cart_value']);
    ...
    public function actionAddToCartCombo()
    {
        if (IS_POST) {
            ...
            if (!is_numeric($goods->number) || intval($goods->number) <= 0) {
                ...
                $res = addto_cart_combo($goods_id, $number, $goods->spec, $parent_id, $goods->group_id,
                    $cart_combo_extends, $parent_attr);
                ...
                if ($res == true) {
                    ...
                    $combo_goods_info = get_combo_goods_info($goods_id, $number, $goods->spec, $parent_id,
                        $warehouse_area);
            ...
            if ($fittings) {
                $goods_info = get_goods_fittings_info($parent_id, $warehouse_id, $area_id, $area_city,
                    $goods->group_id);
    ...
    public function actionDelInCartCombo()
    {
        if (IS_POST) {
            ...
            if (empty($combo_goods['shop_price'])) {
                $shop_price = get_final_price($parent_id, 1, true, $goods->goods_attr, $warehouse_id, $area_id,
                    $area_city);
                    ...
                    if ($rows['act_range'] == FAR_ALL) {
                        ...
                        if ($rows['act_range'] == FAR_CATEGORY) {
                            ...
                            if ($rows['act_range'] == FAR_BRAND) {
                                $rows['act_range_ext'] = return_act_range_ext($rows['act_range_ext'],
                                    $rows['userFav_type'], $rows['act_range']);
