...
{
    $sql = 'select count(b.goods_id) from {pre}order_info as o  LEFT JOIN {pre}order_goods  as b on o.order_id=b.order_id  where user_id=\'' . $user_id . '\' ' . ' AND o.order_status ' . db_create_in(array(
            OS_CONFIRMED,
            OS_SPLITED
        )) . ' AND o.shipping_status ' . db_create_in(array(
            SS_SHIPPED,
            SS_RECEIVED
...
function historys($record_count = 0, $limit = '')
{
    ...
    if (!empty($_COOKIE['ECS']['history_goods'])) {
        ...
        foreach ($query as $key => $row) {
            ...
            $goods['short_name'] = 0 < C('shop.goods_name_length') ? sub_str($row['goods_name'],
                C('shop.goods_name_length')) : $row['goods_name'];
...
function zc_get_user_orders($user_id, $size = 10, $page = 1, $status = 0)
{
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['order_status'] == OS_UNCONFIRMED) {
            $row['handler'] = '<a class="btn-default" href="' . url('user/crowd/cancel',
                    array('order_id' => $row['order_id'])) . '" onclick="if (!confirm(\'' . L('confirm_cancel') . '\')) return false;">' . L('cancel') . '</a>';
        } else {
            if ($row['order_status'] == OS_SPLITED) {
                if ($row['shipping_status'] == SS_SHIPPED) {
                    @$row['handler'] = '<a class="btn-default" href="' . url('user/crowd/affirmreceived',
                            array('order_id' => $row['order_id'])) . '" onclick="if (!confirm(\'' . L('confirm_received') . '\')) return false;">' . L('received') . '</a>';
                } else {
                    if ($row['shipping_status'] == SS_RECEIVED) {
                        @$row['handler'] = '<a class="btn-default">' . L('ss_received') . '</a>';
                    } else {
                        if ($row['pay_status'] == PS_UNPAYED) {
                            @$row['handler'] = '<a class="btn-default" href="' . url('user/crowd/detail',
                                    array('order_id' => $row['order_id'])) . '" >' . L('pay_money') . '</a>';
                        } else {
                            @$row['handler'] = '<a  class="btn-default" href="' . url('user/crowd/detail',
                                    array('order_id' => $row['order_id'])) . '">' . L('view_order') . '</a>';
                        }
                    }
                }
            } else {
        ...
        $arr[] = array(
            ...
            'title' => $row['title'],
            'title_img' => get_zc_image_path($row['title_img']),
            'content' => $row['content'],
            'price' => $row['price'],
            'zc_status' => $zc_status,
...
function zc_cancel_order($order_id, $user_id = 0)
{
    ...
    if ($GLOBALS['db']->query($sql)) {
        order_action($order['order_sn'], OS_CANCELED, $order['shipping_status'], PS_UNPAYED, L('buyer_cancel'),
            'buyer');
...
function zc_get_order_detail($order_id, $user_id = 0)
{
    ...
    if (0 < $order['shipping_time'] && in_array($order['shipping_status'], array(SS_SHIPPED, SS_RECEIVED))) {
        $order['shipping_time'] = sprintf($GLOBALS['_LANG']['shipping_time'],
            local_date($GLOBALS['_CFG']['time_format'], $order['shipping_time']));
    } else {
        $order['shipping_time'] = '';
    }
...
function get_coupons_lists($num = 2, $page = 1, $status = 0)
{
    ...
    foreach ($tab as &$v) {
        ...
        $v['to_use_url'] = $v['is_use'] == 0 ? url('category/index/products',
            array('id' => 0, 'intro' => '', 'cou_id' => $v['cou_id'])) : '';
    }
...
function get_all_return_order($order_id = 0, $size = 10, $page = 1)
{
    ...
    $time = gmtime();
    $sql = 'SELECT count(o.order_id) as num FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.user_id = \'' . $_SESSION['user_id'] . '\' ' . ' AND o.order_status IN (\'' . OS_CONFIRMED . '\',\'' . OS_SPLITED . '\', \'' . OS_SPLITING_PART . '\', \'' . OS_RETURNED_PART . '\', \'' . OS_ONLY_REFOUND . '\') ' . ' AND o.pay_status IN (\'' . PS_PAYED . '\',\'' . PS_PAYED_PART . '\',\'' . PS_REFOUND_PART . '\') ' . ' AND o.is_delete = 0 AND o.is_zc_order = 0 ' . $where . ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ' . ' ';
    $total = $GLOBALS['db']->getOne($sql);
    $sql = 'SELECT o.order_id, o.order_sn, o.add_time, o.extension_id, o.order_status, o.pay_status, o.shipping_status, o.shipping_time FROM ' . $GLOBALS['ecs']->table('order_info') . ' AS o ' . ' WHERE o.user_id = \'' . $_SESSION['user_id'] . '\' ' . ' AND o.order_status IN (\'' . OS_CONFIRMED . '\',\'' . OS_SPLITED . '\', \'' . OS_SPLITING_PART . '\', \'' . OS_RETURNED_PART . '\', \'' . OS_ONLY_REFOUND . '\') ' . ' AND o.pay_status IN (\'' . PS_PAYED . '\',\'' . PS_PAYED_PART . '\',\'' . PS_REFOUND_PART . '\') ' . ' AND o.is_delete = 0 AND o.is_zc_order = 0 ' . $where . ' AND (SELECT count(*) FROM ' . $GLOBALS['ecs']->table('order_info') . ' as oi2 where oi2.main_order_id = o.order_id) = 0 ' . ' group by o.order_id ORDER BY o.add_time DESC';
    ...
    foreach ($order as $key => $val) {
        ...
        foreach ($goods as $gkey => $gval) {
            ...
            $goods[$gkey]['apply_return_url'] = '#';

            if ($val['pay_status'] == PS_PAYED) {
                $goods[$gkey]['apply_return_url'] = url('user/refound/apply_return',
                    array('rec_id' => $gval['rec_id'], 'order_id' => $gval['order_id']));
                if (!empty($val['shipping_time']) && 0 < $sign_time && $val['order_status'] != OS_CANCELED) {
                    $day = ($time - $val['shipping_time']) / 3600 / 24;
...
function get_complaint_list($num = 10, $page = 1, $where = '', $is_complaint = 0)
{
    ...
    foreach ($res as $key => $row) {
        ...
        $arr[] = array(
            ...
            'url' => url('user/order/complaint_apply',
                array('order_id' => $row['order_id'], 'complaint_id' => $row['is_complaint']))
        );
    }
