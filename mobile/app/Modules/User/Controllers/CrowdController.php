...
class CrowdController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionDetail()
    {
        ...
        if ($order['order_status'] == OS_UNCONFIRMED) {
            $order['handler'] = '<span class="box-flex text-right"></span><a class="btn-default box-flex" type="button" href="' . url('user/crowd/cancel',
                    array('order_id' => $order['order_id'])) . '" onclick="if (!confirm(\'' . L('confirm_cancel') . '\')) return false;">' . L('cancel') . '</a>';
        } else {
            if ($order['order_status'] == OS_SPLITED) {
                if ($order['shipping_status'] == SS_SHIPPED) {
                    @$order['handler'] = '<a class="btn-submit" href="' . url('user/crowd/affirmreceived',
                            array('order_id' => $order['order_id'])) . '" onclick="if (!confirm(\'' . L('confirm_received') . '\')) return false;">' . L('received') . '</a>';
                } else {
