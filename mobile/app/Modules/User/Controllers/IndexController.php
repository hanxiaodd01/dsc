...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionEditPassword()
    {
        if (IS_POST) {
            ...
            if ($new_passwords !== $new_password) {
                show_message(L('confirm_password_invalid'), L('back_retry_answer'), url('user/index/edit_password'),
                    'warning');
            }
            ...
            if (!$this->users->check_user($user_info['user_name'], $old_password)) {
                show_message(L('first_password_error'), L('back_retry_answer'), url('user/index/edit_password'),
                    'warning');
            }
            ...
            if ($this->users->edit_user(array(
                ...
                show_message(L('edit_profile_success'), L('back_login'),
                    url('user/login/index', array('back_act' => $this->back_act)), 'success');
            }
        }
    ...
    public function actionAddCommentImg()
    {
        ...
        $img_num = dao('comment_img')->field('id')->where(array(
            'user_id' => $user_id,
            'order_id' => $order_id,
            'goods_id' => $goods_id
        ))->select();
    ...
    public function actionCollectionList()
    {
        if (IS_AJAX) {
            ...
            exit(json_encode(array(
                'goods_list' => $collection_goods['goods_list'],
                'totalPage' => $collection_goods['totalpage']
            )));
        }
    ...
    public function actionAffiliate()
    {
            ...
            if (IS_AJAX) {
                ...
                if ($rt) {
                    foreach ($rt as $k => $v) {
                        ...
                        $rt[$k]['order_sn'] = substr($v['order_sn'], 0,
                                strlen($v['order_sn']) - 5) . '***' . substr($v['order_sn'], -2, 2);
        ...
        if (!file_exists($share_img)) {
            ...
            $img->open($qrcode_bg)->water($qrcode_logo, array(($bg_width - $logo_width) / 2, $bg_height / 2),
                100)->save($share_img);
    ...
    public function actionInvForm()
    {
        ...
        if (IS_POST) {
            ...
            if ($vat_id && empty($id)) {
                exit(json_encode(array(
                    'status' => 'y',
                    'info' => '您已提交过增票资质申请，请勿重复提交！',
                    'url' => url('user/index/inv_info')
                )));
            } else {
                if (empty($content['id'])) {
