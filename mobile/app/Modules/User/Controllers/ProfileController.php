...
');
    ...
    public function actionRealname()
    {
        if (IS_POST) {
            ...
            if ($real_id && $count_user == 1) {
                ...
                dao('users_real')->data($real_user)->where(array(
                    'real_id' => $real_id,
                    'user_id' => $this->user_id
                ))->save();
                ...
                if (!empty($users)) {
                    if (!empty($users['mobile_phone'])) {
                        ...
                        $json_result = array(
                            'error' => 1,
                            'msg' => '请先填写并验证手机号',
                            'url' => url('user/profile/user_edit_mobile')
                        );
                        exit(json_encode($json_result));
                    }
                } else {
    ...
    public function actionAccountRelation()
    {
        if (IS_POST) {
            ...
            if (0 < $bind_user_id && !empty($_SESSION['unionid'])) {
                ...
                if ($connect_user_id == 0 && $bind_user_id != $this->user_id) {
                    $res = dao('connect_user')->data(array('user_id' => $bind_user_id))->where(array(
                        'user_id' => $this->user_id,
                        'connect_code' => 'sns_wechat'
                    ))->save();
        ...
        if (!empty($_SESSION['unionid'])) {
            ...
            $main_user_id = dao('wechat_user')->where(array(
            ...
            if (!empty($main_user_info)) {
                ...
                $main_user_info['user_picture'] = dao('wechat_user')->where(array(
                    'unionid' => $_SESSION['unionid'],
                    'wechat_id' => $wechat_id
                ))->getField('headimgurl');
            }
        ...
        if (IS_AJAX) {
            ...
            if (!empty($relation_user_id)) {
                ...
                if (!empty($userinfo)) {
                    ...
                    dao('connect_user')->data($data)->where(array(
