...
class OrderController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionDetail()
    {
        ...
        $orderInfo = get_table_date('order_info',
            'order_id = \'' . $order_id . '\' and user_id = \'' . $this->user_id . '\'', $date);
                ...
                if ($order['shipping_status'] == SS_SHIPPED) {
                    ...
                    if ($order['shipping_status'] == SS_RECEIVED) {
                        ...
                        if ($order['pay_status'] == PS_UNPAYED) {
                            @$order['handler'] = '<span class="box-flex text-right"></span><a class="btn-submit" href="' . url('user/order/detail',
                                    array('order_id' => $order['order_id'])) . '" >' . L('pay_money') . '</a>';
                        } else {
        ...
        if (isset($order['point_id']) && !empty($order['point_id'])) {
            ...
            if ($order['point']) {
                $order['point']['pickDate'] = local_date('Y',
                        strtotime($order['add_time'])) . '年' . $order['shipping_datestr'];
            }
        }
...
');
        ...
        if ($user_info['user_money'] < $order['order_amount']) {
            show_message(L('balance_not_enough'), L('back_up_page'),
                url('user/order/detail', array('order_id' => $order_id)));
        }
    ...
    public function actionComplaintList()
    {
        if (IS_AJAX) {
            ...
            if ($status == 0) {
                $where_confirmed = ' AND oi.order_status ' . db_create_in(array(
                        OS_CONFIRMED,
                        OS_SPLITED,
                        OS_SPLITING_PART
                    )) . '  ' . 'AND IF(oi.pay_status = ' . PS_PAYED . ', IF(oi.shipping_status = ' . SS_RECEIVED . ', oi.shipping_status = \'' . SS_RECEIVED . ('\' AND (\'' . $time . '\'- oi.confirm_take_time) < \'' . $dealy_time . '\', ') . db_create_in(array(SS_RECEIVED),
                        'oi.shipping_status', 'NOT') . ') ';
                $where_confirmed .= 'AND oi.pay_status ' . db_create_in(array(PS_PAYED)) . ', IF(oi.shipping_status = ' . SS_RECEIVED . ', ' . db_create_in(array(SS_RECEIVED),
                        'oi.shipping_status') . (' AND (\'' . $time . '\'- oi.confirm_take_time) < \'' . $dealy_time . '\', ') . db_create_in(array(SS_UNSHIPPED),
                        'oi.shipping_status', 'NOT') . '))' . $where_zc_order;
            }

            $complaint_list = get_complaint_list($size, $page, $where_confirmed, $status);
            exit(json_encode(array(
                'order_list' => $complaint_list['list'],
                'totalPage' => $complaint_list['totalPage']
            )));
        }
