...
class LoginController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (empty($back_act)) {
            if (empty($back_act) && isset($GLOBALS['_SERVER']['HTTP_REFERER'])) {
                $back_act = strpos($GLOBALS['_SERVER']['HTTP_REFERER'],
                    url('user/index/index')) ? url('user/index/index') : $GLOBALS['_SERVER']['HTTP_REFERER'];
            } else {
        ...
        if (is_wechat_browser() && (empty($_SESSION['unionid']) || empty($_SESSION['user_id']))) {
            ...
            if ($auth_config) {
                $this->redirect('oauth/index/index',
                    array('type' => 'wechat', 'refer' => 'user', 'back_url' => urlencode($back_act)));
            }
        }
            ...
            if (!empty($users)) {
                ...
                if (register($username, $password, $email, $other) !== false) {
                    ...
                    exit(json_encode(array(
                        'status' => 'y',
                        'info' => sprintf($message_tips, $username),
                        'url' => $back_act
                    )));
                } else {
    ...
    public function actionEditForgetPassword()
    {
        if (IS_POST) {
            ...
            show_message(L('edit_error'), L('retrieve_password'),
                url('user/login/get_password_phone', array('enabled_sms' => 2)), 'info');
        }
    ...
    public function actionEditPassword()
    {
        if (IS_POST) {
            ...
            if (!empty($mobile) && base64_encode($user_info['mobiles']) == $mobile || $user_info && (!empty($code) && md5($user_info['user_id'] . C('hash_code') . $user_info['reg_time']) == $code) || 0 < $_SESSION['user_id'] && $_SESSION['user_id'] == $user_id && $this->load->user->check_user($_SESSION['user_name'],
                    $old_password)) {
    ...
    public function actionLogout()
    {
        if ((!isset($this->back_act) || empty($this->back_act)) && isset($_SERVER['HTTP_REFERER'])) {
            $this->back_act = stripos($_SERVER['HTTP_REFERER'],
                'profile') ? url('user/index/index') : $_SERVER['HTTP_REFERER'];
        } else {
            $this->back_act = url('user/login/index');
        }

        $this->users->logout();
        show_message(L('logout'), array(L('back_up_page'), L('back_home_lnk')), array($this->back_act, url('/')),
            'success');
    }
    ...
    public function actionRegister()
    {
        if (IS_POST) {
            ...
            if (register($username, $password, $email, $other) !== false) {
                ...
                exit(json_encode(array(
                    'status' => 'y',
                    'info' => sprintf(L('register_success'), $username),
                    'url' => $back_act
                )));
            } else {
