...
class AccountController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionDeposit()
    {
        ...
        $buyer_recharge = floatval(C('shop.buyer_recharge'));
    ...
    public function actionAccountRaply()
    {
        ...
        $buyer_cash = floatval(C('shop.buyer_cash'));
    ...
    public function actionAccount()
    {
        ...
        if ($surplus['process_type'] == 1) {
            ...
            if (0 < $surplus['rec_id']) {
                ...
                log_account_change($this->user_id, $amount, $frozen_money, 0, 0,
                    '【' . L('application_withdrawal') . '】' . $surplus['user_note'], ACT_ADJUSTING, 0,
                    $surplus['deposit_fee']);
        ...
        if (IS_POST) {
            ...
            if ($result == 0) {
                exit(json_encode(array(
                    'status' => 'y',
                    'info' => L('add_value_card_sucess'),
                    'url' => url('user/account/value_card')
                )));
            }
        }
    ...
    public function actionPayValueCard()
    {
        ...
        if (IS_POST) {
            ...
            if ($result == 0) {
                exit(json_encode(array(
                    'status' => 'y',
                    'info' => L('use_pay_card_sucess'),
                    'url' => url('user/account/value_card_info', array('vid' => $vid))
                )));
            }
