...
class RefoundController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionList()
    {
        ...
        if (IS_AJAX) {
            ...
            exit(json_encode(array(
                'refound_list' => $refound_list['list'],
                'totalPage' => $refound_list['totalpage']
            )));
        }
    ...
    public function actionSubmitReturn()
    {
            ...
            if (!empty($images_list)) {
                foreach ($images_list as $k => $v) {
                    if ($v) {
                        ...
                        if (!empty($img) && (strpos($img, 'http://') === false && strpos($img,
                                    'https://') === false && strpos($img, 'errorImg.png') === false)) {
        ...
        $order_return = array(
            ...
            'goods_attr' => $order_goods['goods_attr'],
            'attr_val' => $attr_val,
        ...
        do {
            ...
            $query = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_return'), $order_return, 'INSERT', '',
                'SILENT');
        ...
        if ($query) {
            ...
            $query = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('return_goods'), $return_goods, 'INSERT', '',
                'SILENT');
            ...
            if (0 < $images_count) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('return_images'), $images, 'UPDATE',
                    'rec_id = \'' . $rec_id . '\' and user_id = \'' . $_SESSION['user_id'] . '\'');
            }
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_return_extend'), $order_return_extend, 'INSERT',
                '', 'SILENT');
            ...
            show_message(L('Apply_Success_Prompt'), L('See_returnlist'),
                url('detail', array('ret_id' => $return['ret_id'])), 'success', true);
        } else {
            show_message(L('Apply_Abnormal'), '', '', 'error', true);
        }
    }
    ...
    public function actionDetail()
    {
        ...
        foreach ($shipping_list as $key => $val) {
            ...
            $shipping_fee = $shipping_count == 0 && $cart_weight_price['free_shipping'] == 1 ? 0 : shipping_fee($val['shipping_code'],
                unserialize($val['configure']), $cart_weight_price['weight'], $cart_weight_price['amount'],
