    ...
    protected function init_oauth()
    {
        ...
        if (is_wechat_browser() && empty($_SESSION['unionid']) && $module_name != 'oauth') {
            ...
            if ($auth_config) {
                ...
                if (isset($HTTP_REFERER) && !empty($HTTP_REFERER) && strpos($HTTP_REFERER,
                        'mp.weixinbridge.com/mp/wapredirect?url') !== false) {
                ...
                $this->redirect('oauth/index/index',
                    array('type' => 'wechat', 'back_url' => urlencode(html_in($back_url))));
            }
        }
    }
...
', $shop_config['kf_ww']) : '';
    ...
    public function get_wechat_share_content($share_data = array())
    {
        if (!empty($share_data['img'])) {
            $share_img = strtolower(substr($share_data['img'], 0,
                4)) == 'http' ? $share_data['img'] : __STATIC__ . $share_data['img'];
        } else {
            $share_img = !!C('shop.wap_logo') ? __STATIC__ . '/' . ltrim(C('shop.wap_logo'),
                    '../') : elixir('img/wxsdk.png', true);
        }
...
',
                '
'
            ), '', html_in($share_data['desc'])) : C('shop.shop_desc'),
