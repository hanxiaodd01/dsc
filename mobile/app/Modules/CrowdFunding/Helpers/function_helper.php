    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            $arr[$row['topic_id']]['topic_content'] = str_replace('\\r\\n', '<br />',
                htmlspecialchars($row['topic_content']));
            $arr[$row['topic_id']]['add_time'] = get_time_past($row['add_time'], gmtime());
            $arr[$row['topic_id']]['url'] = U('crowd_funding/index/topic',
                array('id' => $row['pid'], 'topic_id' => $row['topic_id'], 'action' => 'comment'));
        ...
        foreach ($shipping_list as $key => $val) {
            if ($GLOBALS['_CFG']['freight_model'] == 0) {
                ...
                $shipping_fee = (($shipping_count == 0) && ($cart_weight_price['free_shipping'] == 1) ? 0 : shipping_fee($val['shipping_code'],
                    unserialize($val['configure']), $cart_weight_price['weight'], $goods['price'], $goods['number']));
                $shipping_list[$key]['free_money'] = price_format($shipping_cfg['free_money'], false);
            } else {
                if ($GLOBALS['_CFG']['freight_model'] == 1) {
                    $shipping_cfg = unserialize_config($val['configure']);
                    $shipping_fee = goods_shipping_fee($val['shipping_code'], unserialize($val['configure']), 0,
...
function zc_get_ru_shippng_info($cart_goods, $consignee = '')
{
    ...
    foreach ($shipping_list as $key => $val) {
        if ($GLOBALS['_CFG']['freight_model'] == 0) {
            $shipping_cfg = unserialize_config($val['configure']);
            $shipping_fee = (($shipping_count == 0) && ($cart_weight_price['free_shipping'] == 1) ? 0 : shipping_fee($val['shipping_code'],
                unserialize($val['configure']), $cart_weight_price['weight'], $cart_goods['price'],
                $cart_goods['number']));
            $shipping_list[$key]['free_money'] = price_format($shipping_cfg['free_money'], false);
        } else {
            if ($GLOBALS['_CFG']['freight_model'] == 1) {
                ...
                $shipping_cfg = unserialize_config($val['configure']);
                $shipping_fee = goods_shipping_fee($val['shipping_code'], unserialize($val['configure']), 0,
                    $cart_goods['price'], $cart_goods['number']);
        ...
        $shipping_list[$key]['insure_formated'] = strpos($val['insure'], '%') === false ? price_format($val['insure'],
            false) : $val['insure'];
