...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function getWepCheckGiftLogin($gift_sn, $gift_pwd)
    {
        ...
        $gift_count = dao('user_gift_gard')->where(array(
            'gift_sn' => $gift_sn,
            'goods_id' => 0,
            'is_delete' => 1
        ))->count();
        ...
        if ($gift_sn) {
            $result = dao('user_gift_gard')->where(array(
                'gift_sn' => $gift_sn,
            ...
            if ($result) {
                $row = dao('gift_gard_type')->where(array(
                    'review_status' => 3,
        ...
        if ($row) {
            ...
            if ($row['gift_end_date'] <= $time) {
                return array(
                    'error' => 1,
                    'content' => L('gift_gard_overdue_time') . local_date('Y-m-d H:i:s', $row['gift_end_date'])
                );
            } else {
                if ($time <= $row['gift_start_date']) {
                    return array(
                        'error' => 1,
                        'content' => L('gift_gard_Use_time') . local_date('Y-m-d H:i:s', $row['gift_start_date'])
                    );
                }
            }
        } else {
    ...
    public function actionTake()
    {
        ...
        if ($gift_sn) {
            $pwd = dao('user_gift_gard')->where(array(
                'gift_sn' => $gift_sn,
                'goods_id' => 0,
                'is_delete' => 1
            ))->find();
