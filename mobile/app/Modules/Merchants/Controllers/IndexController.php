...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        if (IS_POST) {
            ...
            if (!empty($data['fid'])) {
                dao('merchants_steps_fields')->where(array(
                    'fid' => $data['fid'],
    ...
    public function actionShop()
    {
        if (IS_POST) {
            ...
            if ($res == true) {
                dao('merchants_category_temporarydate')->data(array('is_add' => 1))->where(array(
                    'user_id' => $this->user_id,
                    'is_add' => 0
                ))->save();
                exit(json_encode(array(
                    'status' => 'y',
                    'info' => L('add_success') . L('wait_audit'),
                    'url' => url('merchants/index/audit')
                )));
            } else {
