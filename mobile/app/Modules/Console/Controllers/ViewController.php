...
class ViewController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionDefault()
    {
        if (IS_POST) {
            ...
            if ($ru_id) {
                ...
                $index = dao('touch_page_view')->where(array(
                    'ru_id' => 0,
                    'type' => 'index',
                    'default' => 1
                ))->getField('id');
            ...
            if (!empty($goods_id)) {
                ...
                if ($goods === false) {
                    foreach ($goods_id as $key => $val) {
                        $row = dao('goods')->field('goods_id ,  goods_name , model_attr, product_promote_price, promote_start_date, promote_end_date,  sales_volume ,sales_volume_base ,market_price , shop_price, goods_thumb, goods_img, goods_number ')->where(array(
                            'goods_id' => $val,
                            'is_on_sale' => 1,
                            'is_delete' => 0
                        ))->find();
                        ...
                        if ($row) {
                            ...
                            $goods[$key]['url'] = build_uri('goods', array('gid' => $row['goods_id']),
                                $row['goods_name']);
                        } else {
                ...
                if ($product === false) {
                    ...
                    $product = category_get_goods($children, $type, $brand, $user_id, '', $warehouse_id, $area_id,
                        $number, $this->area_city);
    ...
    public function actionView()
    {
        if (IS_POST) {
            ...
            $view['data'] = str_replace('../data/gallery_album/original_img/5951ceab15b33.jpg',
                get_image_path(ltrim('../data/gallery_album/original_img/5951ceab15b33.jpg', '../')), $view['data']);
    ...
    public function actionSearch()
    {
        if (IS_POST) {
            ...
            $view = dao('touch_page_view')->field('id, pic, title, default, type')->where(array(
                'title' => $title,
                'ru_id' => $ru_id,
                'default' => $default
            ))->order('update_at DESC')->select();
    ...
    public function actionGoodsList()
    {
        if (IS_POST) {
            ...
            foreach ($goodslist as $key => $val) {
                $goodslist[$key]['url'] = url('goods/index/index',
