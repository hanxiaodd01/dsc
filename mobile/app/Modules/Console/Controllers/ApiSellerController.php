...
class ApiSellerController extends \App\Modules\Admin\Controllers\EditorSellerController
{
        ...
        if (IS_POST) {
            ...
            foreach ($goods_list as $key => $val) {
                ...
                if (0 < $val['promote_price']) {
                    $promote_price = bargain_price($val['promote_price'], $val['promote_start_date'],
                ...
                $product[$key]['url'] = url('goods/index/index',
                    array('id' => $val['goods_id'], 'u' => $_SESSION['user_id']));
        ...
        if (IS_POST) {
            ...
            if ($type == 'thumb') {
                ...
                foreach ($pic as $key => $value) {
                    ...
                    $thumb[$key]['tree'] = dao('gallery_album')->field('album_id, album_mame')->where(array(
                        'ru_id' => $ru_id,
                        'parent_album_id' => $value['album_id']
    ...
    public function actionUrl()
    {
        if (IS_POST) {
            ...
            if ($type == 'category') {
                $category = dao('merchants_category')->field('cat_id , cat_name , parent_id')->where(array(
                    'parent_id' => 0,
                    'is_show' => 1,
                    'user_id' => $ru_id
                ));
                ...
                foreach ($merchants_category as $key => $value) {
                    $merchants_category[$key]['url'] = url('store/index/pro_list',
                        array('ru_id' => $ru_id, 'cat_id' => $value['cat_id']));
                }
                        ...
                        if ($number == 0) {
                            ...
                            if (0 < $number) {
                                $view = dao('touch_page_view')->field('id , type ,  title ,  pic ,thumb_pic , default ')->where(array(
                                    ...
                                    'ru_id' => $ru_id,
                                    'page_id' => $page_id
                                ))->order('update_at DESC')->limit($number)->find();
                            }
                        }
                    } else {
                        if ($default == 3) {
            ...
            if (IS_POST) {
                ...
                if ($id) {
                    ...
                    if (count($num) == 1) {
                        ...
                        $keep = array(
                            'data' => !empty($_POST['data']) ? $_POST['data'] : $num[0]['data'],
                            'pic' => $pic,
                            'update_at' => $time
                        );
