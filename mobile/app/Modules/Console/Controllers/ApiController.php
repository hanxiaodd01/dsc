...
class ApiController extends \App\Modules\Admin\Controllers\EditorController
{
        ...
        if (IS_POST) {
            ...
            if (!empty($goods_id)) {
                ...
                foreach ($goods_id as $key => $val) {
                    $row = dao('goods')->field('goods_id ,  goods_name , model_attr, product_promote_price, promote_start_date, promote_end_date,  sales_volume ,market_price , shop_price, goods_thumb, goods_img, goods_number ')->where(array(
                        'goods_id' => $val,
                        'is_on_sale' => 1,
                        'is_delete' => 0
                    ))->find();
    ...
    {
        $url = dao('category')->field('cat_id , cat_name , parent_id')->where(array(
            'parent_id' => 0,
            'is_show' => 1
        ))->select();

        foreach ($url as $key => $value) {
            $category = dao('category')->field('cat_id , cat_name , parent_id')->where(array(
                'parent_id' => $value['cat_id'],
                'is_show' => 1
            ))->select();

            foreach ($category as $key2 => $value2) {
                $category2 = dao('category')->field('cat_id , cat_name , parent_id')->where(array(
                    'parent_id' => $value2['cat_id'],
                    'is_show' => 1
                ))->select();
                $category[$key2] = array(
                    'cat_id' => $value2['cat_id'],
                    'cat_name' => $value2['cat_name'],
                    'parent_id' => $value2['parent_id'],
                    'child_tree' => $category2
                );
            }
    ...
    public function actionThumb()
    {
        if (IS_POST) {
            ...
            if ($type == 'thumb') {
                $pic = dao('gallery_album')->field('*')->where(array(
                    'ru_id' => $ru_id,
                    'parent_album_id' => 0
                ...
                if ($type == 'img') {
                    ...
                    $img = dao('pic_album')->field('pic_id ,pic_name, pic_file')->where(array(
                        'album_id' => $album_id,
                        'ru_id' => $ru_id
                    ))->order('add_time DESC')->limit($current, $pageSize)->select();
        ...
        if (IS_POST) {
            ...
            if ($type == 'function') {
                ...
                $store = array(
                    ...
                    'cat_name' => '店铺街',
                    'parent_id' => 0,
                    'url' => url('store/index/index')
                );
                $brand = array(
                    'cat_id' => '5',
                    'cat_name' => '品牌街',
                    'parent_id' => 0,
                    'url' => url('brand/index/index')
                );
                $community = array(
                    'cat_id' => '6',
                    'cat_name' => '微社区',
                    'parent_id' => 0,
                    'url' => url('community/index/index')
                );
                ...
                if ($type == 'activity') {
                    $groupbuy = array(
                        ...
                        'cat_name' => '团购',
                        'parent_id' => 0,
                        'url' => url('groupbuy/index/index')
                    );
                    $exchange = array(
                        'cat_id' => 2,
                        'cat_name' => '积分',
                        'parent_id' => 0,
                        'url' => url('exchange/index/index')
                    );
                    $crowd_funding = array(
                        'cat_id' => 3,
                        'cat_name' => '众筹',
                        'parent_id' => 0,
                        'url' => url('crowd_funding/index/index')
                    );
                    $topic = array(
                        'cat_id' => 4,
                        'cat_name' => '专题',
                        'parent_id' => 0,
                        'url' => url('topic/index/index')
                    );
                    $activity = array(
                        'cat_id' => 5,
                        'cat_name' => '促销活动',
                        'parent_id' => 0,
                        'url' => url('activity/index/index')
                    );
                    $auction = array(
                        'cat_id' => 6,
                        'cat_name' => '拍卖',
                        'parent_id' => 0,
                        'url' => url('auction/index/index')
                    );
                    $seckill = array(
                        'cat_id' => 7,
                        'cat_name' => '秒杀',
                    ...
                    $team = array(
                        ...
                        'cat_name' => '拼团',
                        'parent_id' => 0,
                        'url' => url('team/index/index')
                    );
                    $package = array(
                        'cat_id' => 9,
                        'cat_name' => '超值礼包',
                        'parent_id' => 0,
                        'url' => url('package/index/index')
                    );
                    $url = array(
                        $groupbuy,
                        $exchange,
                    ...
                    if ($type == 'category') {
                        ...
                        foreach ($url as $key => $value) {
                            ...
                            foreach ($category as $key2 => $value2) {
                                ...
                                foreach ($category2 as $key3 => $value3) {
                                    $category2[$key3]['url'] = url('category/index/products',
                                        array('id' => $value3['cat_id']));
                                }

                                $category[$key2] = array(
                                    'cat_id' => $value2['cat_id'],
                                    'cat_name' => $value2['cat_name'],
                                    'url' => url('category/index/products', array('id' => $value2['cat_id'])),
                                    'parent_id' => $value2['parent_id'],
                                    'child_tree' => $category2,
                        ...
                        if ($type == 'article') {
                            $article = dao('article_cat')->field('cat_id , cat_name , parent_id')->where(array('parent_id' => 0))->limit($current,
                                $pageSize)->select();
                            ...
                            foreach ($article as $key => $value) {
                                $list[$key] = array(
                                    ...
                                    'cat_name' => $value['cat_name'],
                                    'parent_id' => $value['parent_id'],
                                    'url' => url('article/index/index', array('cat_id' => $value['cat_id']))
                                );
                            }
    ...
    public function actionAddCollect()
    {
        ...
        if (!empty($shopid) && 0 < $_SESSION['user_id']) {
            $status = dao('collect_store')->field('user_id, rec_id')->where(array(
                'ru_id' => $shopid,
    ...
    public function actionKeep()
    {
        ...
        if (empty($id)) {
            ...
            if (empty($new)) {
                $index = unserialize(str_replace('<?php exit("no access");', '',
                    file_get_contents(ROOT_PATH . 'storage/app/diy/index.php')));
                ...
                if (!empty($index)) {
                    ...
                    $new = dao('touch_page_view')->where(array(
                        'ru_id' => '0',
                        'type' => 'index',
                        'default' => 1
                    ))->getField('id');
                    ...
                    if (empty($new)) {
                        $index = str_replace('<?php exit("no access");', '',
                            file_get_contents(ROOT_PATH . 'storage/app/diy/default.php'));
                        $keep = array(
                            'ru_id' => 0,
                            'type' => 'index',
                            'page_id' => 0,
    ...
    public function actionView()
    {
        if (IS_POST) {
            ...
            if ($id) {
                ...
                if ($topic_id) {
                    $view = dao('touch_page_view')->field('type, thumb_pic, data, default')->where(array(
                        'id' => $topic_id,
                        'type' => 'topic'
                    ))->find();
                } else {
                    if ($default < 2) {
                        if ($number == 0) {
                            $view = dao('touch_page_view')->field('id , type ,  title ,  pic ,thumb_pic , default ')->where(array(
                                'default' => $default,
                                'ru_id' => $ru_id,
                                'page_id' => $page_id
                            ...
                            if (0 < $number) {
                                $view = dao('touch_page_view')->field('id , type ,  title ,  pic ,thumb_pic , default ')->where(array(
                                    ...
                                    'ru_id' => $ru_id,
                                    'page_id' => $page_id
                                ))->order('update_at DESC')->limit($number)->select();
                            }
                        }
                    } else {
                        if ($default == 3) {
                            ...
                            $view = dao('touch_page_view')->field('id , type ,  title , data,  pic ,thumb_pic , default ')->where(array(
                                'ru_id' => $ru_id,
                                'type' => $type
                            ))->order('update_at DESC')->select();
                        }
            ...
            if (empty($view)) {
                ...
                if (empty($new)) {
                    $data = str_replace('<?php exit("no access");', '',
                        file_get_contents(ROOT_PATH . 'storage/app/diy/default.php'));
        ...
        if (IS_POST) {
            ...
            if ($id) {
                ...
                if ($res) {
                    ...
                    $keep = array(
                        'data' => !empty($_POST['data']) ? $_POST['data'] : $res['data'],
                        'pic' => $pic,
                        'update_at' => $time
                    );
            ...
            if ($id) {
                ...
                if (count($num) == 1) {
                    ...
                    $keep = array(
                        'id' => $id,
                        'ru_id' => $ru_id,
                        'title' => !empty($_POST['title']) ? $_POST['title'] : $num[0]['title'],
                        'thumb_pic' => $pic,
                        'description' => !empty($description) ? $description : $num[0]['description'],
                        'update_at' => $time
                    );
                ...
                if ($num < 1) {
                    $keep = array(
                        ...
                        'page_id' => $page_id,
                        'thumb_pic' => !empty($res['url']['file']['savename']) ? $res['url']['file']['savename'] : '',
    ...
    public function actionSearch()
    {
        if (IS_POST) {
            ...
            $where = 'g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 AND msi.shop_close = 1 NAD msi.merchants_audit ';
