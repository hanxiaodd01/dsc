...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionList()
    {
        ...
        if (IS_AJAX) {
            ...
            $sort = isset($_REQUEST['sort']) && in_array(trim(strtolower(I('sort'))),
                array('shop_price', 'start_time', 'act_id')) ? I('sort', '',
                array('htmlspecialchars', 'trim')) : $default_sort_order_type;
            $order = isset($_REQUEST['order']) && in_array(trim(strtoupper(I('order'))),
                array('ASC', 'DESC')) ? I('order', '', array('htmlspecialchars', 'trim')) : $default_sort_order_method;
        ...
        if (IS_AJAX) {
            ...
            foreach ($res as $key => $val) {
                ...
                if ($now <= $val['start_time']) {
                    $res[$key]['status'] = 1;
                    $res[$key]['short_format_date'] = short_format_date($val['start_time']);
                } else {
                    if ($val['end_time'] < $now) {
                        $res[$key]['status'] = 3;
                    } else {
                        $res[$key]['short_format_date'] = short_format_date($val['end_time']);
                    }
                }
            }
...
', $basic_info['kf_qq']) : '';
        ...
        $this->assign('best_goods',
            get_recommend_goods('best', '', $this->region_id, $this->area_info['region_id'], $this->area_city,
                $goods['user_id'], 1, 'presale'));
        $this->assign('new_goods',
            get_recommend_goods('new', '', $this->region_id, $this->area_info['region_id'], $this->area_city,
                $goods['user_id'], 1, 'presale'));
        $this->assign('hot_goods',
            get_recommend_goods('hot', '', $this->region_id, $this->area_info['region_id'], $this->area_city,
                $goods['user_id'], 1, 'presale'));
    ...
    public function actionPrice()
    {
        ...
        if ($this->goods_id == 0) {
            ...
            $products = get_warehouse_id_attr_number($this->goods_id, $_REQUEST['attr'], $goods['user_id'],
                $warehouse_id, $area_id, $this->area_city);
            ...
            $spec_price = get_final_price($this->goods_id, $number, true, $attr_id, $warehouse_id, $area_id,
                $this->area_city, 1, 0, 0, $res['show_goods'], $product_promote_price);
...
');
    ...
    protected function get_pre_goods(
        $cat_id,
        $status = 0,
        $sort = 'cat_id',
        $order = 'DESC',
