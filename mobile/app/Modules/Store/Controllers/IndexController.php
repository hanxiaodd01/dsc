...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
                    ...
                    foreach ($store_list as $key) {
                        ...
                        if ($goods) {
                            foreach ($goods as $gkey) {
                                $goodsarr[] = get_goods_info($gkey['goods_id'], $this->region_id,
                                    $this->area_info['region_id'], $this->area_city);
                            }
                        } else {
                            $goodsarr = 0;
                        }
                        ...
                        $store[] = array(
                            ...
                            'commentrank' => $merchants_goods_comment['cmt']['commentRank']['zconments']['score'],
                            'commentrank_bg' => $this->cmt($merchants_goods_comment['cmt']['commentRank']['zconments']['score']),
                            'commentserver' => $merchants_goods_comment['cmt']['commentServer']['zconments']['score'],
                            'commentserver_bg' => $this->cmt($merchants_goods_comment['cmt']['commentServer']['zconments']['score']),
                            'commentdelivery' => $merchants_goods_comment['cmt']['commentDelivery']['zconments']['score'],
                            'commentdelivery_bg' => $this->cmt($merchants_goods_comment['cmt']['commentDelivery']['zconments']['score']),
                            ...
                            'gaze_status' => $status,
                            'goods' => $goodsarr,
                            'title' => $title = 0 < count($goods) ? '爆款商品' : '',
                            'distance' => $distance
                        );
                    }
                }
    ...
    public function actionShopInfo()
    {
        ...
        foreach ($seo as $key => $value) {
            $seo[$key] = html_in(str_replace(array('{sitename}', '{key}', '{shopname}', '{description}'),
                array(C('shop.shop_name'), $data['shop_keyword'], $data['shop_title'], $data['street_desc']), $value));
        }
        ...
        $share_data = array(
            ...
            'desc' => $description,
            'link' => '',
            'img' => get_wechat_image_path(ltrim($data['logo_thumb'], '../'))
        );
    ...
    public function actionProList()
    {
        ...
        if (IS_AJAX) {
            ...
            $list = store_get_goods($children, $bid, 0, 0, '', $size, $page, $sort, $order, $ru_id, $this->region_id,
                $this->area_info['region_id'], $keyword, $type, $this->area_city);
            $maxpage = store_get_goods($children, $bid, 0, 0, '', 0, 0, $sort, $order, $ru_id, $this->region_id,
                $this->area_info['region_id'], $keyword, $type, $this->area_city);
    ...
    public function actionShopAbout()
    {
        ...
        if ($info['license_comp_adress']) {
            ...
            if (!empty($adress)) {
                ...
                foreach ($adress as $v) {
                    $license_comp_adress .= get_table_date('region', 'region_id=\'' . $v . '\'', array('region_name'),
                        2);
                }
            }
