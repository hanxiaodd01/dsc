...
function get_also_bought($goods_id)
{
    ...
    foreach ($res as $key => $row) {
        ...
        $arr[$key]['short_name'] = 0 < C('shop.goods_name_length') ? sub_str($row['goods_name'],
...
{
    $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) ' . 'FROM {pre}order_info AS o, {pre}order_goods  AS g ' . 'WHERE o.order_id = g.order_id ' . 'AND o.order_status = \'' . OS_CONFIRMED . '\' ' . 'AND o.shipping_status ' . db_create_in(array(
            SS_SHIPPED,
            SS_RECEIVED
    ...
    if (0 < $sales_count) {
        $sql = 'SELECT DISTINCT SUM(goods_number) AS num ' . 'FROM {pre}order_info  AS o,  {pre}order_goods  AS g ' . 'WHERE o.order_id = g.order_id ' . 'AND o.order_status = \'' . OS_CONFIRMED . '\' ' . 'AND o.shipping_status ' . db_create_in(array(
                SS_SHIPPED,
                SS_RECEIVED
            )) . ' AND o.pay_status ' . db_create_in(array(
                PS_PAYED,
...
{
    $sql = 'SELECT IFNULL(SUM(g.goods_number), 0) as count ' . 'FROM {pre}order_info AS o, {pre}order_goods AS g ' . 'WHERE o . order_id = g . order_id ' . ' AND o . order_status = \'' . OS_CONFIRMED . '\'' . ' AND o . shipping_status ' . db_create_in(array(
            SS_SHIPPED,
            SS_RECEIVED
        )) . ' AND o . pay_status ' . db_create_in(array(
            PS_PAYED,
