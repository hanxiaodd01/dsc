...
');
        ...
        foreach ($seo as $key => $value) {
            $seo[$key] = html_in(str_replace(array('{sitename}', '{key}', '{name}', '{description}'),
                array(C('shop.shop_name'), $goods['keywords'], $goods['goods_name'], $goods['goods_brief']), $value));
        }
        ...
        if (!empty($default_attr_id)) {
            ...
            $shop_price = get_final_price($this->goods_id, 1, true, $default_attr_id, $this->region_id, $this->area_id,
                $this->area_city, 0, 0, $add_tocart);
        ...
        $this->assign('kefu', $this->kefu($goods['goods_id'], $goods['user_id']));
        $basic_date = array('region_name');
        $basic_info['province'] = !empty($basic_info['province']) ? get_table_date('region',
            'region_id = \'' . $basic_info['province'] . '\'', $basic_date, 2) : '';
        $basic_info['city'] = !empty($basic_info['city']) ? get_table_date('region',
                'region_id= \'' . $basic_info['city'] . '\'', $basic_date, 2) . '市' : '';
        ...
        $new_goods = get_recommend_goods('new', '', $this->region_id, $this->area_id, $this->area_city,
            $goods['user_id']);
        ...
        foreach ($coupont as $key => $value) {
            ...
            if (0 < $_SESSION['user_id']) {
                $user_num = dao('coupons_user')->where(array(
                    'cou_id' => $value['cou_id'],
        ...
        if (empty($goods['desc_mobile']) && !empty($goods['goods_desc'])) {
            if (C('shop.open_oss') == 1) {
                ...
                $goods_desc = str_replace(array('src="/images/upload', 'src="images/upload'),
                    'src="' . $bucket_info['endpoint'] . 'images/upload', $goods['goods_desc']);
            } else {
                $goods_desc = str_replace(array('src="/images/upload', 'src="images/upload'),
                    'src="' . __STATIC__ . '/images/upload', $goods['goods_desc']);
            }
        }
    ...
    public function actionNewGoods()
    {
        ...
        $this->response(array(
            ...
            'cart_num' => $cart_num,
            'spec' => $spec,
            'properties' => $properties['spe'],
            'share_data' => $this->get_wechat_share_content($share_data),
            'type' => $attr
        ));
    }
        ...
        if (IS_AJAX) {
            ...
            if ($goods_url && $goods_img) {
                ...
                if (!file_exists($avatar)) {
                    ...
                    if ($headimg === false) {
                        if (strtolower(substr($users['user_picture'], 0, 4)) == 'http' && strpos($users['user_picture'],
                                __STATIC__) !== false) {
                ...
                $goods_price = price_format($goods['original_shop_price']);

                if (!file_exists($goods_thumb)) {
                    if (C('shop.open_oss') == 1 || strtolower(substr($goods_img, 0, 4)) == 'http' && strpos($goods_img,
                            __STATIC__) === false) {
                    ...
                    $img->open($goodsthumb)->thumb($bg_width, $bg_width,
                        \Think\Image::IMAGE_THUMB_FILLED)->save($goods_thumb);
                }

                $fonts_path = dirname(ROOT_PATH) . '/data/attached/fonts/msyh.ttf';
                $font_color = '#555555';
                $img->open($bgImg)->water($goods_thumb, array(0, 0), 100)->text($goods_title_first, $fonts_path, 20,
                    $font_color, array(40, $bg_width + 20))->save($outImg);
                $img->open($outImg)->text($goods_price, $fonts_path, 28, '#EC5151',
                    array(40, $bg_width + 70))->save($outImg);
                ...
                $img->open($outImg)->water($qrcode, array($qr_left, $qr_top), 100)->text($text_description, $fonts_path,
                    18, '#999999', array($text_left, $text_top))->save($outImg);
    ...
    public function actionPrice()
    {
        ...
        if ($this->goods_id == 0) {
            ...
            $products = get_warehouse_id_attr_number($this->goods_id, $attr, $goods['user_id'], $warehouse_id, $area_id,
                $area_city);
            ...
            $spec_price = get_final_price($this->goods_id, $number, true, $attr_id, $warehouse_id, $area_id, $area_city,
                1, 0, 0, $res['show_goods'], $product_promote_price);
        ...
        $area_position_list = get_goods_user_area_position($goods['user_id'], $this->city_id, $attr, $this->goods_id, 0,
            0, 1, 0, '');
    ...
    public function actionFittingPrice()
    {
        if (IS_AJAX) {
            ...
            if ($goods_id == 0) {
                ...
                $products = get_warehouse_id_attr_number($goods_id, $attr, $goods['user_id'], $warehouse_id, $area_id,
                    $area_city);
                $attr_number = isset($products['product_number']) ? $products['product_number'] : 0;
                $product_promote_price = isset($products['product_promote_price']) ? $products['product_promote_price'] : 0;
                ...
                $shop_price = get_final_price($goods_id, $number, true, $attr_id, $warehouse_id, $area_id, $area_city,
                    0, 0, $add_tocart, 0, $product_promote_price);
                if ($GLOBALS['_CFG']['add_shop_price'] == 0 && empty($spec_price) && empty($prod)) {
                    $spec_price = $shop_price;
                }
            ...
            if ($type == 1 && $onload == 'onload') {
                ...
                if (0 < $group_count) {
                    ...
                    if ($fittings_list) {
                        ...
                        $goods_fittings = get_goods_fittings_info($goods_id, $warehouse_id, $area_id, $area_city, '', 1,
                            '', $fittings_attr);
                ...
                if (0 < $combo_goods['combo_number']) {
                    ...
                    $fittings = get_goods_fittings(array($parent_id), $warehouse_id, $area_id, $area_city, $group_id,
                        1);
                } else {
                ...
                if (0 < $combo_goods['combo_number']) {
                    ...
                    $res['fittings_minMax'] = price_format($fittings_interval['fittings_min']) . '-' . number_format($fittings_interval['fittings_max'],
                            2, '.', '');
                    $res['market_minMax'] = price_format($fittings_interval['market_min']) . '-' . number_format($fittings_interval['market_max'],
                            2, '.', '');

                    if ($fittings_interval['save_minPrice'] == $fittings_interval['save_maxPrice']) {
                        $res['save_minMaxPrice'] = price_format($fittings_interval['save_minPrice']);
                    } else {
                        $res['save_minMaxPrice'] = price_format($fittings_interval['save_minPrice']) . '-' . number_format($fittings_interval['save_maxPrice'],
                                2, '.', '');
                    }
                }
            }
            ...
            if ($type == 1 && $onload != 'onload' || $type == 0) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('cart_combo'), $cart_combo_data, 'UPDATE',
