...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        $position = dao('touch_ad_position')->where(array(
            'ad_type' => 'seckill',
            'tc_type' => 'banner'
        ))->order('position_id desc')->find();
        ...
        if (empty($goods_info['desc_mobile']) && !empty($goods_info['goods_desc'])) {
            if (C('shop.open_oss') == 1) {
                ...
                $goods_desc = str_replace(array('src="/images/upload', 'src="images/upload'),
                    'src="' . $bucket_info['endpoint'] . 'images/upload', $goods_info['goods_desc']);
            } else {
                $goods_desc = str_replace(array('src="/images/upload', 'src="images/upload'),
                    'src="' . __STATIC__ . '/images/upload', $goods_info['goods_desc']);
            }
        }
...
');
        ...
        if ($specs) {
            $products = get_warehouse_id_attr_number($seckill['goods_id'], $specs, $seckill['user_id'],
                $this->region_id, $this->area_id, $this->area_city);
        }
