        ...
        if (count($arr) < 4) {
            ...
            foreach ($stb as $k => $v) {
                if ($sec_id_tomorrow) {
                    ...
                    $arr['tmr' . $k]['frist_end_time'] = local_date('Y-m-d H:i:s',
                        local_strtotime($v['end_time']) + $day);
...
function seckill_goods_results($id = '', $page = 1, $size = 10, $tomorrow = 0)
{
    ...
    if ($res) {
        foreach ($res as $k => $v) {
            ...
            $res[$k]['percent'] = !empty($v['sec_num']) ? ceil($res[$k]['sales_volume']['valid_goods'] / ($v['sec_num'] + $res[$k]['sales_volume']['valid_goods']) * 100) : 100;
