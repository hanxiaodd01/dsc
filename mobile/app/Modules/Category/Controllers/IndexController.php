...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionProducts()
    {
        ...
        if (IS_AJAX) {
            ...
            if ($goodslist === false) {
                $goodslist = category_get_goods($this->keywords, $this->children, $this->intro, $this->brand,
                    $this->price_min, $this->price_max, $this->ext, $this->size, $this->page, $this->sort, $this->order,
                    $this->region_id, $this->area_id, $this->ubrand, $this->hasgoods, $this->promotion, $this->cou_id,
                    $this->area_city);
        ...
        foreach ($seo as $key => $value) {
            $seo[$key] = html_in(str_replace(array('{sitename}', '{key}', '{name}', '{description}'),
                array(C('shop.shop_name'), $cat_info['keywords'], $cat_info['cat_name'], $cat_info['cat_desc']),
                $value));
        }
    ...
    public function actionCartId()
    {
        if (IS_POST) {
            ...
            $cart_id = dao('cart')->field('rec_id, goods_number')->where(array(
                'goods_id' => $goods_id,
                'user_id' => $user_id,
    ...
    public function actionGoods()
    {
        if (IS_POST) {
            ...
            $goodslist = category_get_goods('', $this->children, $type, $this->brand, $this->price_min,
                $this->price_max, $this->ext, $pageSize, $currentPage, $this->sort, $this->order, $this->region_id,
                $this->area_id, $this->ubrand, $this->hasgoods, $this->promotion);
    ...
    protected function init_params()
    {
        ...
        $this->display = in_array($goods_display,
            $display_array) ? $goods_display : (isset($_COOKIE['display']) ? $_COOKIE['display'] : $default_display_type);
        ...
        if (0 < $this->cat['filter_attr']) {
            ...
            foreach ($this->cat_filter_attr as $key => $value) {
                $sql = 'SELECT a.attr_id, a.attr_name, a.attr_cat_type FROM ' . $this->ecs->table('attribute') . ' AS a ' . (' WHERE a.attr_id = \'' . $value . '\' LIMIT 1');
                ...
                if ($attributeInfo) {
                    ...
                    $sql = 'SELECT a.attr_id, MIN(a.goods_attr_id) AS goods_id, a.attr_value AS attr_value, a.color_value FROM ' . $this->ecs->table('goods_attr') . ' AS a ' . (' WHERE a.attr_id=\'' . $value . '\' ') . ' GROUP BY a.attr_value';
                    ...
                    foreach ($attr_list as $k => $v) {
                        ...
                        if (!empty($this->filter_attr[$key])) {
                            ...
                            $all_attr_list[$key]['select_attr_name'] = !empty($all_attr_list[$key]['select_attr']) ? rtrim($all_attr_list[$key]['select_attr'],
                                '、') : L('all_attribute');
                        }
                    }
                }
