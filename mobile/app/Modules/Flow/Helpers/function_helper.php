        ...
        if (0 < intval($GLOBALS['_CFG']['use_storage']) && $goods['extension_code'] != 'package_buy' && $store_id == 0 && $row['cloud_id'] == 0) {
            ...
            if (!empty($row['product_id'])) {
                ...
                if ($product_number < $val) {
                    show_message(sprintf(L('stock_insufficiency'), $row['goods_name'], $product_number,
                        $product_number), '', '', 'warning');
                    exit();
                }
            } else {
                if ($row['goods_number'] < $val) {
                    show_message(sprintf(L('stock_insufficiency'), $row['goods_name'], $row['goods_number'],
                        $row['goods_number']), '', '', 'warning');
                    exit();
                }
            }
        } else {
            if (0 < intval($GLOBALS['_CFG']['use_storage']) && 0 < $store_id && $row['cloud_id'] == 0) {
                ...
                $products = get_warehouse_id_attr_number($row['goods_id'], $goods['goods_attr_id'], $goodsInfo['ru_id'],
                    0, 0, 0, '', $store_id);
                ...
                if ($row['goods_number'] < $val) {
                    show_message(sprintf(L('stock_store_shortage'), $row['goods_name'], $row['goods_number'],
                        $row['goods_number']), '', '', 'warning');
                    exit();
                }
            } else {
                if (0 < intval($GLOBALS['_CFG']['use_storage']) && $goods['extension_code'] == 'package_buy') {
                    ...
                    if (0 < $row['cloud_id']) {
                        ...
                        if ($cloud_number < $val) {
                            show_message(sprintf(L('stock_insufficiency'), $row['goods_name'], $cloud_number,
                                $cloud_number), '', '', 'warning');
                            exit();
                        }
                    }
                }
