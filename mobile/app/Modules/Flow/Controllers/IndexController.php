...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
    ...
    public function actionIndex()
    {
        ...
        $cart_goods_list = cart_goods($flow_type, $cart_value, 1, $this->region_id, $this->area_id, $this->area_city,
            $consignee, $store_id);
        ...
        $total = order_fee($order, $cart_goods, $consignee, 0, $cart_value, 0, $cart_goods_list, $this->region_id,
            $this->area_id, $store_id);
        ...
        if ((!isset($use_integral) || $use_integral == '1') && 0 < $_SESSION['user_id'] && 0 < $user_info['pay_points'] && ($flow_type == CART_GENERAL_GOODS || $flow_type == CART_ONESTEP_GOODS)) {
            $order_max_integral = flow_available_points($cart_value, $flow_type, $this->region_id, $this->area_id,
                $this->area_city);
        ...
        if ((!isset($use_coupons) || $use_coupons == '1') && ($flow_type == CART_GENERAL_GOODS || $flow_type == CART_ONESTEP_GOODS)) {
            $user_coupons = get_user_coupons_list($_SESSION['user_id'], true, $total['goods_price'], $cart_goods, true,
                $cart_ru_id);
...
', '', $GLOBALS['_CFG']['invoice_content']));
    ...
    public function actionGetUserCouon()
    {
        ...
        $disabled_coupons_list = get_user_coupons_list($_SESSION['user_id'], true, '', false, true, $cart_ru_id,
            'cart');
    ...
    public function actionDone()
    {
        ...
        if ($done_cart_value && $GLOBALS['_CFG']['stock_dec_time'] == SDT_PLACE) {
            ...
            if ($solve_info) {
                if (1 < $solve_info['solve_num']) {
                    $ogoods_list = array();
                    $goods_orec = array();
                    $sql = 'SELECT rec_id, goods_number, user_id, goods_id, CONCAT(goods_id, \'|\', model_attr, \'|\', goods_attr_id) AS cart_info, CONCAT(goods_id, \'|\', model_attr, \'|\', goods_attr_id, \'|\', warehouse_id, \'|\', area_id) AS warehouse_area FROM {pre}cart WHERE rec_id ' . db_create_in($solve_info['orec_id']) . ' AND is_real = 1' . ' AND rec_type ' . db_create_in(array(
                            CART_GENERAL_GOODS,
        ...
        if (0 < $user_id) {
            ...
            $flow_points = flow_available_points($_SESSION['cart_value'], $flow_type, $this->region_id, $this->area_id,
                $this->area_city);
        ...
        if (0 < $order['bonus_id']) {
            ...
            if (empty($bonus) || $bonus['user_id'] != $user_id || 0 < $bonus['order_id'] || cart_amount(true,
                    $flow_type, $_SESSION['cart_value']) < $bonus['min_goods_amount']) {
                $order['bonus_id'] = 0;
            }
        } else {
            if (isset($_POST['bonus_sn'])) {
                ...
                $now = gmtime();
                if (empty($bonus) || 0 < $bonus['user_id'] || 0 < $bonus['order_id'] || cart_amount(true, $flow_type,
                        $_SESSION['cart_value']) < $bonus['min_goods_amount'] || $bonus['use_end_date'] < $now) {
                } else {
                    if (0 < $user_id) {
        ...
        if (0 < $order['uc_id']) {
            ...
            if (empty($coupons) || $coupons['user_id'] != $user_id || $coupons['is_use'] == 1 || cart_amount(true,
                    $flow_type, $_SESSION['cart_value']) < $coupons['cou_man']) {
                $order['uc_id'] = 0;
            }
        }

        $cart_goods_list = cart_goods($flow_type, $_SESSION['cart_value'], 1, $this->region_id, $this->area_id,
            $this->area_city, $consignee, $store_id);

        if (empty($cart_goods_list)) {
            show_message(L('no_goods_in_cart'), L('back_home'), './', 'warning');
        }

        if (($flow_type == CART_GENERAL_GOODS || $flow_type == CART_ONESTEP_GOODS) && cart_amount(true, $flow_type,
                $_SESSION['cart_value']) < C('shop.min_goods_amount')) {
        ...
        $total = order_fee($order, $cart_goods, $consignee, 1, $_SESSION['cart_value'], $pay_type, $cart_goods_list,
            $this->region_id, $this->area_id, $store_id);
        ...
        if (0 < $order['user_id'] && (0 < $order['surplus'] || 0 < $order['integral'])) {
            ...
            log_account_change($order['user_id'], $order_surplus, 0, 0, $order_integral,
                    ...
                    if ($this->db->getOne($sql) <= 0) {
                        ...
                        if (0 < $order['user_id']) {
                            ...
                            log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']),
                                ...
                                sprintf(L('order_gift_integral'), $order['order_sn']));
        ...
        if ($order['is_surplus'] == 1 && $order['order_amount'] <= 0 && 0 < $order['surplus']) {
            ...
            if (1 < $child_order && $child_order_info) {
                foreach ($child_order_info as $key => $child) {
                    order_action($child['order_sn'], OS_CONFIRMED, SS_UNSHIPPED, PS_PAYED, L('flow_surplus_pay'),
                        L('buyer'));
                }
            }
...
');
        ...
        if (IS_AJAX) {
            ...
            if (empty($cart_goods_list) || !check_consignee_info($consignee, $flow_type)) {
                ...
                if (!empty($_SESSION['flow_order']['cou_id'])) {
                    ...
                    $coupons_info = get_coupons($cou_id, array(
                        'c.cou_id',
                        'c.cou_man',
                        'c.cou_type',
                        'c.ru_id',
    ...
    public function actionSelectPayment()
    {
        if (IS_AJAX) {
            ...
            $cart_goods_list = cart_goods($flow_type, $_SESSION['cart_value'], 1, $this->region_id, $this->area_id,
                $this->area_city, $consignee, $store_id);
            if (empty($cart_goods_list) || !check_consignee_info($consignee,
                    $flow_type) && $_SESSION['store_id'] <= 0) {
                if (empty($cart_goods_list)) {
                    $result['error'] = 1;
                } else {
                    if (!check_consignee_info($consignee, $flow_type)) {
                ...
                $cart_goods = get_new_group_cart_goods($cart_goods_list);
                $total = order_fee($order, $cart_goods, $consignee, 0, $_SESSION['cart_value'], 0, $cart_goods_list, 0,
                    0, $store_id);
                $this->assign('order', $order);
                $this->assign('total', $total);
        ...
        if (IS_AJAX) {
            ...
            if (empty($cart_goods_list) || !check_consignee_info($consignee,
                ...
                $total = order_fee($order, $cart_goods, $consignee, 0, $_SESSION['cart_value'], 0, $cart_goods_list, 0,
                    0, $store_id);
                $this->assign('total', $total);
                $this->assign('total_integral', cart_amount(false, $flow_type,
    ...
    public function actionChangeCoupont()
    {
        ...
        if (empty($cart_goods_list) || !check_consignee_info($consignee, $flow_type) && $_SESSION['store_id'] <= 0) {
            ...
            $order = flow_order_info();
            $coupons_info = get_coupons($cou_id,
    ...
    public function actionChangeIntegral()
    {
        ...
        if (empty($cart_goods_list) || !check_consignee_info($consignee, $flow_type)) {
            ...
            if (0 < $is_integral) {
                ...
                $flow_points = flow_available_points($_SESSION['cart_value'], $flow_type, $this->region_id,
                    $this->area_id, $this->area_city);
                $user_points = $user_info['pay_points'];
