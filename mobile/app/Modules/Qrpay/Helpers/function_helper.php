...
{
    $res = dao('qrpay_discounts')->field('min_amount, discount_amount, max_discount_amount')->where(array(
        'status' => 1,
        'id' => $id
    ))->find();
...
{
    $res = dao('qrpay_log')->where(array('id' => $order_id, 'is_settlement' => 0, 'pay_status' => 1))->find();
    ...
    if (!empty($res)) {
        if (0 < $res['ru_id']) {
            ...
            dao('qrpay_log')->data(array('is_settlement' => 1))->where(array(
                'id' => $order_id,
                'ru_id' => $res['ru_id']
            ))->save();
