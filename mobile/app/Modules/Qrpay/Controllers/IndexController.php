...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
	...
	private function getPayData($order)
	{
		...
		if ($this->pay_code == 'alipay') {
			$payData = array('body' => $order['pay_order_sn'], 'subject' => !empty($order['pay_desc']) ? '【' . $order['pay_desc'] . '】' . $order['pay_order_sn'] : $order['pay_order_sn'], 'order_no' => $order['pay_order_sn'] . 'Q' . $order['id'], 'timeout_express' => gmtime() + 3600 * 24 * 9, 'amount' => $order['pay_amount'], 'return_param' => 'qr' . $order['id'], 'client_ip' => $this->get_client_ip(), 'goods_type' => 1, 'store_id' => '');
		}

		if ($this->pay_code == 'wxpay') {
			$payData = array('body' => $order['pay_order_sn'], 'subject' => !empty($order['pay_desc']) ? '【' . $order['pay_desc'] . '】' . $order['pay_order_sn'] : $order['pay_order_sn'], 'order_no' => $order['pay_order_sn'] . 'Q' . $order['id'], 'timeout_express' => gmtime() + 3600 * 24 * 9, 'amount' => $order['pay_amount'], 'return_param' => 'qr' . $order['id'], 'client_ip' => $this->get_client_ip(), 'openid' => $order['openid']);
		}
