...
class IndexController extends \App\Modules\Base\Controllers\FrontendController
{
        ...
        if (IS_AJAX) {
            if (!empty($this->qrpay_info)) {
                ...
                if (0 < $this->qrpay_info['ru_id']) {
                    $shop_name = dao('merchants_shop_information')->alias('a')->join(C('DB_PREFIX') . 'seller_shopinfo b on a.user_id = b.ru_id')->where(array(
                        'ru_id' => $this->qrpay_info['ru_id'],
                        'b.shop_close' => 1
                    ))->getField('shop_title');
                } else {
    ...
    public function actionCallback()
    {
        ...
        $id = isset($order['id']) ? dao('qrpay_log')->where(array('id' => $order['id']))->getField('qrpay_id') : input('get.id',
            0, 'intval');
        $this->assign('id', $id);
    ...
    private function getPayData($order)
    {
        ...
        if ($this->pay_code == 'alipay') {
            $payData = array(
                ...
                'timeout_express' => gmtime() + 3600 * 24 * 9,
