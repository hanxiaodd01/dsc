...
class Kefu extends \Think\Model
{
    ...
    static public function getAdmin($admin = 0)
    {
        ...
        $adminUser = \App\Models\AdminUser::where('user_id', $admin)->with(array(
    ...
    {
        $service = \App\Models\ImService::where('user_id', $id)->where('status', 1)->select('id', 'nick_name',
            'chat_status')->first();
    ...
    {
        $service = \App\Models\ImService::where('id', $id)->where('status', 1)->select('id', 'nick_name', 'chat_status',
            'user_id')->first();
    ...
    static public function getServiceList($ruId, $sid)
    {
        ...
        $adminUser = \App\Models\AdminUser::where('ru_id', $ruId)->select(array('user_id'))->with(array(
            'Service' => function ($query) {
    ...
    {
        $waitMessage = \App\Models\ImDialog::select('id', 'customer_id', 'services_id', 'origin', 'goods_id',
            'store_id', 'start_time')->where('services_id', 0)->where('store_id', $ru_id)->where('status',
            1)->orderBy('start_time', 'DESC')->groupby('customer_id')->get()->toArray();
        $total = 0;
        ...
        foreach ($waitMessage as $k => $v) {
            ...
            $res = \App\Models\ImMessage::where('from_user_id', $v['customer_id'])->where('to_user_id',
                0)->where('status', 1)->orderBy('add_time', 'desc')->get()->toArray();
            ...
            unset($res);
            $res = $temp;
            $waitMessage[$k]['num'] = \App\Models\ImMessage::where('from_user_id',
                $v['customer_id'])->where('to_user_id', 0)->where('status', 1)->orderBy('add_time', 'desc')->count();
            ...
            $res = \App\Models\Users::where('user_id', $v['customer_id'])->select('user_name', 'user_picture',
                'nick_name')->first();
    ...
    {
        $messageList = \App\Models\ImDialog::select('id', 'customer_id', 'services_id', 'origin', 'goods_id',
            'store_id', 'status')->where('services_id', $service['id'])->orderBy('start_time',
            'DESC')->get()->toArray();
        $temp = array();
        ...
        foreach ($messageList as $k => $v) {
            ...
            $res = \App\Models\ImMessage::where('dialog_id', $v['id'])->where('status',
                1)->select('message')->orderBy('add_time', 'DESC')->get()->toArray();
            ...
            if (0 < $messageList[$k]['goods_id']) {
                $res = \App\Models\Goods::where('goods_id', $v['goods_id'])->select('goods_name', 'goods_thumb',
                    'shop_price')->first();
    ...
    {
        \App\Models\ImMessage::where(array(
            'from_user_id' => $serviceId,
            'to_user_id' => $customId
        ))->orWhere(array('to_user_id' => $serviceId, 'from_user_id' => $customId))->update(array('status' => 0));
    }
    ...
    static public function getHistory($uid, $tid, $keyword = '', $time = '', $page = 1, $size = 20)
    {
        ...
        $list = M('im_message')->where($where)->order('add_time DESC, id DESC')->field('id, message, add_time, from_user_id, user_type')->limit($start,
            $size);
        $list = $list->select();
        ...
        foreach ($list as $k => $v) {
            if ($v['user_type'] == 1) {
                ...
                if ($v['user_type'] == 2) {
                    $res = \App\Models\Users::where('user_id', $v['from_user_id'])->select('user_name',
                        'nick_name')->first();
    ...
    {
        $reply = \App\Models\ImConfigure::where('ser_id', $id)->where('type', $type)->select('id', 'type', 'is_on',
            'content');
    ...
    {
        $dialog = \App\Models\ImDialog::where('customer_id', $data['customer_id'])->where('services_id',
            $data['services_id'])->where('goods_id', $data['goods_id'])->where('store_id',
            $data['store_id'])->orderBy('id', 'DESC')->where('status', 1)->limit(1)->get();
        $dialog = $dialog[0];
    ...
    {
        $dialog = \App\Models\ImDialog::where('customer_id', $cid)->where('services_id', $fid)->orderBy('id',
            'DESC')->limit(1)->get();
        $dialog = $dialog[0];
    ...
    {
        \App\Models\ImMessage::where('from_user_id', $cusId)->where('to_user_id', '')->where('user_type',
            2)->update(array('to_user_id' => $serId, 'status' => 0));
    ...
    {
        $ImMessage = \App\Models\ImMessage::where('from_user_id', $cusId)->where('user_type', 2)->orderBy('add_time',
            'DESC')->limit(1)->get();
    ...
    {
        \App\Models\ImDialog::where('customer_id', $tid)->where('services_id', $uid)->orderBy('start_time',
            'DESC')->update(array('end_time' => time(), 'status' => 2));
    }

    static public function closeOldWindow($expire)
    {
        $dialog = \App\Models\ImDialog::where('end_time', '<', time() - $expire)->where('status', 1)->where('end_time',
            '>', 0)->where('services_id', '<>', 0)->distinct()->orderBy('start_time', 'DESC')->get();
        $temp = array();
    ...
    {
        $goods = \App\Models\Goods::select('goods_id', 'goods_name', 'goods_thumb', 'shop_price')->where('goods_id',
            $gid)->first()->toArray();
            ...
            foreach ($links[0] as $url) {
                if (preg_match('/^(http|https)/is', $url)) {
                    if (preg_match('/(goods|goods.php)/i', $url)) {
                        ...
                        if (!empty($goods_id)) {
                            $res = \App\Models\Goods::where('goods_id', $goods_id)->select('goods_name', 'goods_thumb',
                                'shop_price')->first();
