...
class Page
{
    ...
    public function show($url = '', $total = 0, $perPage = 10, $pageBarNum = 10, $mode = 1)
    {
        ...
        switch ($mode) {
            case 1:
                return $pager = array(
                    'page_first' => $this->firstPage(),
                    'page_prev' => $this->prePage(),
                    'page_next' => $this->nextPage(),
                    'page_last' => $this->lastPage(),
                    'page_number' => $this->showselect(),
                    'page' => $this->nowIndex,
    ...
    private function _setUrl($url = '')
    {
        ...
        if (!empty($url) && preg_match('/\\{page\\}/', $url)) {
            ...
            if (($url = preg_replace('/' . preg_quote($page_str, '/') . '(\\d{1,})/', $page_str . '{page}',
                    $this->requestUri)) == $this->requestUri) {
                if (($url = str_replace($this->pageSuffix, $page_str . '{page}' . $this->pageSuffix,
                        $this->requestUri)) == $this->requestUri) {
