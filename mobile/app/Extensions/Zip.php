...
class Zip
{
    ...
    private function file()
    {
        ...
        return $data . $ctrldir . $this->eof_ctrl_dir . pack('v', sizeof($this->ctrl_dir)) . pack('v',
                sizeof($this->ctrl_dir)) . pack('V', strlen($ctrldir)) . pack('V', strlen($data)) . "\x00\x00";
    }
    ...
    protected function ExtractFile($header, $to, $zip)
    {
        ...
        for ($i = 0; $i < (count($pth) - 1); $i++) {
            ...
            if ((!is_dir($mydir) && @mkdir($mydir,
                        511)) || ((($mydir == ($to . $header['filename'])) || (($mydir == $to) && ($this->total_folders == 0))) && is_dir($mydir))) {
                @chmod($mydir, 511);
                $this->total_folders++;
            }
        }
