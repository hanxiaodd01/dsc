...
class Dispatcher
{
    ...
    static public function dispatch()
    {
        ...
        if (MODULE_NAME && (defined('BIND_MODULE') || !in_array_case(MODULE_NAME,
                    C('MODULE_DENY_LIST'))) && (is_dir(APP_PATH . MODULE_NAME) || is_dir(COMPONENT_PATH . MODULE_NAME) || is_dir(PLAYGROUND_PATH . MODULE_NAME))) {
            // 定义当前模块路径
            if (is_dir(PLAYGROUND_PATH . MODULE_NAME)) {
                define('MODULE_PATH', PLAYGROUND_PATH . MODULE_NAME . '/');
            } else {
                if (is_dir(COMPONENT_PATH . MODULE_NAME)) {
        ...
        if ('' != $_SERVER['PATH_INFO'] && (!C('URL_ROUTER_ON') || !Route::check())) {   // 检测路由规则 如果没有则按默认规则调度URL
            ...
            // 检查禁止访问的URL后缀
            if (C('URL_DENY_SUFFIX') && preg_match('/\.(' . trim(C('URL_DENY_SUFFIX'), '.') . ')$/i',
                    $_SERVER['PATH_INFO'])) {
