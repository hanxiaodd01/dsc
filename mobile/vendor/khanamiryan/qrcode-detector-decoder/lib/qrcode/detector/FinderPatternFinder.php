...
class FinderPatternFinder
{
    ...
    protected final function handlePossibleCenter($stateCount, $i, $j, $pureBarcode)
    {
        ...
        if (!is_nan($centerI)) {
            // Re-cross check
            $centerJ = $this->crossCheckHorizontal(intval($centerJ), intval($centerI), $stateCount[2],
                $stateCountTotal);
            if (!is_nan($centerJ) &&
                (!$pureBarcode || $this->crossCheckDiagonal(intval($centerI), intval($centerJ), $stateCount[2],
                        $stateCountTotal))
            ) {
