...
class Worker
{
    ...
    protected static function displayUI()
    {
        ...
        foreach (static::$_workers as $worker) {
            static::safeEcho(str_pad($worker->user, static::$_maxUserNameLength + 2) . str_pad($worker->name,
                    ...
                    static::$_maxSocketNameLength + 2) . str_pad(' ' . $worker->count,
    ...
    protected static function formatStatusData()
    {
        ...
        foreach ($info as $key => $value) {
            ...
            if (preg_match('/^[0-9]+/', $value, $pid_math)) {
                ...
                if (preg_match('/^\S+?\s+?\S+?\s+?\S+?\s+?\S+?\s+?\S+?\s+?\S+?\s+?\S+?\s+?(\S+?)\s+?/', $value,
                    $match)) {
    ...
    public static function forkWorkersForWindows()
    {
        ...
        if (isset($argv[1]) && $argv[1] === '-q') {
            ...
            // Display UI.
            echo str_pad($worker->name, 21) . str_pad($worker->getSocketName(), 36) . str_pad($worker->count,
                    10) . "[ok]\n";
            $worker->listen();
            $worker->run();
            exit("@@@child exit@@@\r\n");
