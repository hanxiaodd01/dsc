...
class PHPExcel_Calculation_Financial
{
    ...
    public static function COUPNUM($settlement, $maturity, $frequency, $basis = 0)
    {
        ...
        $daysBetweenSettlementAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($settlement, $maturity,
                $basis) * 365;
    ...
    public static function DDB($cost, $salvage, $life, $period, $factor = 2.0)
    {
        ...
        if ((is_numeric($cost)) && (is_numeric($salvage)) && (is_numeric($life)) && (is_numeric($period)) && (is_numeric($factor))) {
            ...
            $previousDepreciation = 0;
            for ($per = 1; $per <= $period; ++$per) {
                $depreciation = min(($cost - $previousDepreciation) * ($factor / $life),
    ...
    public static function TBILLPRICE($settlement, $maturity, $discount)
    {
        ...
        if (is_numeric($discount)) {
            ...
            if (PHPExcel_Calculation_Functions::getCompatibilityMode() == PHPExcel_Calculation_Functions::COMPATIBILITY_OPENOFFICE) {
                ++$maturity;
                $daysBetweenSettlementAndMaturity = PHPExcel_Calculation_DateTime::YEARFRAC($settlement,
                        $maturity) * 360;
                if (!is_numeric($daysBetweenSettlementAndMaturity)) {
                    //	return date error
                    return $daysBetweenSettlementAndMaturity;
     ...
     * @param array of float    $values        A series of cash flows that corresponds to a schedule of payments in
     *                 dates. The first payment is optional and corresponds to a cost or payment that occurs at the
     *                 beginning of the investment. If the first value is a cost or payment, it must be a negative
     *                 value. All succeeding payments are discounted based on a 365-day year. The series of values must
     ...
     *                 The first payment date indicates the beginning of the schedule of payments. All other dates must
