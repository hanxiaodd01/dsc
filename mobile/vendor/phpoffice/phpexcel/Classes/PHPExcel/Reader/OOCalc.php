...
//		}
        ...
        if ($zip->open($pFilename) === true) {
            ...
            if ($stat && ($stat['size'] <= 255)) {
                ...
                $xml = simplexml_load_string($this->securityScan($zip->getFromName('META-INF/manifest.xml')),
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        $xml = new XMLReader();
        $res = $xml->xml($this->securityScanFile('zip://' . realpath($pFilename) . '#content.xml'), null,
...
//		echo '<h1>Meta Information</h1>';
        $xml = simplexml_load_string($this->securityScan($zip->getFromName("meta.xml")), 'SimpleXMLElement',
            PHPExcel_Settings::getLibXmlLoaderOptions());
                    ...
                    case 'user-defined' :
                        ...
                        foreach ($propertyValueAttributes as $key => $value) {
                            if ($key == 'name') {
                                ...
                                switch ($value) {
                                    case 'date'    :
                                        $propertyValue = PHPExcel_DocumentProperties::convertProperty($propertyValue,
                                            'date');
...
//		echo '<h1>Workbook Content</h1>';
        $xml = simplexml_load_string($this->securityScan($zip->getFromName("content.xml")), 'SimpleXMLElement',
            PHPExcel_Settings::getLibXmlLoaderOptions());
        $namespacesContent = $xml->getNamespaces(true);
//		echo '<pre>';
...
//									echo 'Value Type is '.$cellDataOfficeAttributes['value-type'].'<br />';
                                    switch ($cellDataOfficeAttributes['value-type']) {
                                        ...
                                        case 'date' :
                                            ...
                                            $dataValue = PHPExcel_Shared_Date::FormattedPHPToExcel($year, $month, $day,
                                                $hour, $minute, $second);
                                        ...
                                        case 'time' :
                                            $type = PHPExcel_Cell_DataType::TYPE_NUMERIC;
                                            $dataValue = PHPExcel_Shared_Date::PHPToExcel(strtotime('01-01-1970 ' . implode(':',
...
//									echo 'Formula: ', $cellDataFormula, PHP_EOL;
                                    ...
                                    foreach ($temp as &$value) {
                                        ...
                                        if ($tKey = !$tKey) {
                                            $value = preg_replace('/\[([^\.]+)\.([^\.]+):\.([^\.]+)\]/Ui', '$1!$2:$3',
                                                $value);    //  Cell range reference in another sheet
                                            $value = preg_replace('/\[([^\.]+)\.([^\.]+)\]/Ui', '$1!$2',
                                                $value);       //  Cell reference in another sheet
                                            $value = preg_replace('/\[\.([^\.]+):\.([^\.]+)\]/Ui', '$1:$2',
                                                $value);    //  Cell range reference
                                            $value = preg_replace('/\[\.([^\.]+)\]/Ui', '$1',
                                                $value);                  //  Simple cell reference
                                    ...
                                    for ($i = 0; $i < $colRepeats; ++$i) {
                                        ...
                                        if ($type !== PHPExcel_Cell_DataType::TYPE_NULL) {
                                            for ($rowAdjust = 0; $rowAdjust < $rowRepeats; ++$rowAdjust) {
                                                $rID = $rowID + $rowAdjust;
                                                $objPHPExcel->getActiveSheet()->getCell($columnID . $rID)->setValueExplicit((($hasCalculatedValue) ? $cellDataFormula : $dataValue),
                                                    $type);
                                                if ($hasCalculatedValue) {
