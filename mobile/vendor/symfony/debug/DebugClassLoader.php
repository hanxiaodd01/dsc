...
class DebugClassLoader
{
    ...
    {
        $exists = null === $file || \class_exists($class, false) || \interface_exists($class,
                false) || \trait_exists($class, false);
        ...
        if ($exists) {
            ...
            if ($parent = \get_parent_class($class)) {
                ...
                if (isset(self::$final[$parent])) {
                    @trigger_error(sprintf('The "%s" class is considered final%s. It may change without further notice as of its next major version. You should not extend it from "%s".',
                        $parent, self::$final[$parent], $name), E_USER_DEPRECATED);
                }
            }
            ...
            foreach ($parentAndTraits + $this->getOwnInterfaces($name, $parent) as $use) {
                ...
                if (isset(self::$deprecated[$use]) && \strncmp($ns, $use, $len)) {
                    $type = class_exists($name, false) ? 'class' : (interface_exists($name,
                        false) ? 'interface' : 'trait');
                    $verb = class_exists($use, false) || interface_exists($name,
                        false) ? 'extends' : (interface_exists($use, false) ? 'implements' : 'uses');

                    @trigger_error(sprintf('The "%s" %s %s "%s" that is deprecated%s.', $name, $type, $verb, $use,
                        self::$deprecated[$use]), E_USER_DEPRECATED);
                }
                if (isset(self::$internal[$use]) && \strncmp($ns, $use, $len)) {
                    @trigger_error(sprintf('The "%s" %s is considered internal%s. It may change without further notice. You should not use it from "%s".',
                        $use,
                        class_exists($use, false) ? 'class' : (interface_exists($use, false) ? 'interface' : 'trait'),
                        self::$internal[$use], $name), E_USER_DEPRECATED);
                }
            }
            ...
            foreach ($refl->getMethods(\ReflectionMethod::IS_PUBLIC | \ReflectionMethod::IS_PROTECTED) as $method) {
                ...
                if ($isClass && $parent && isset(self::$finalMethods[$parent][$method->name])) {
                    ...
                    @trigger_error(sprintf('The "%s::%s()" method is considered final%s. It may change without further notice as of its next major version. You should not extend it from "%s".',
                        $declaringClass, $method->name, $message, $name), E_USER_DEPRECATED);
                }
                ...
                foreach ($parentAndTraits as $use) {
                    if (isset(self::$internalMethods[$use][$method->name])) {
                        ...
                        if (\strncmp($ns, $declaringClass, $len)) {
                            @trigger_error(sprintf('The "%s::%s()" method is considered internal%s. It may change without further notice. You should not extend it from "%s".',
                                $declaringClass, $method->name, $message, $name), E_USER_DEPRECATED);
                        }
                    }
                }
            ...
            if (isset(self::$php7Reserved[\strtolower($refl->getShortName())])) {
                @trigger_error(sprintf('The "%s" class uses the reserved name "%s", it will break on PHP 7 and higher',
                    $name, $refl->getShortName()), E_USER_DEPRECATED);
            }
        }
