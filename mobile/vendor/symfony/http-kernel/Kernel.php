...
abstract class Kernel implements KernelInterface, RebootableInterface, TerminableInterface
{
    ...
    public function getBundle($name, $first = true/*, $noDeprecation = false */)
    {
        ...
        if (!$first && !$noDeprecation) {
            @trigger_error(sprintf('Passing "false" as the second argument to %s() is deprecated as of 3.4 and will be removed in 4.0.',
    ...
    public function setClassCache(array $classes)
    {
        ...
        file_put_contents(($this->warmupDir ?: $this->getCacheDir()) . '/classes.map',
    ...
    {
        file_put_contents(($this->warmupDir ?: $this->getCacheDir()) . '/annotations.map',
    ...
    protected function doLoadClassCache($name, $extension)
    {
        ...
        if (!$this->booted && is_file($cacheDir . '/classes.map')) {
            ClassCollectionLoader::load(include($cacheDir . '/classes.map'), $cacheDir, $name, $this->debug, false,
                $extension);
        }
    }
    ...
    protected function initializeBundles()
    {
        ...
        foreach ($this->registerBundles() as $bundle) {
            ...
            if ($parentName = $bundle->getParent()) {
                @trigger_error('Bundle inheritance is deprecated as of 3.4 and will be removed in 4.0.',
                    E_USER_DEPRECATED);
        ...
        if (!$fresh) {
            if ($this->debug) {
                ...
                $previousHandler = $previousHandler ?: set_error_handler(function ($type, $message, $file, $line) use (
                    ...
                    if (E_USER_DEPRECATED !== $type && E_DEPRECATED !== $type) {
                        return $previousHandler ? $previousHandler($type & ~E_WARNING, $message, $file,
                            $line) : E_WARNING === $type;
                    }
            ...
            try {
                ...
                if (!$this->debug) {
                    ...
                    file_put_contents($cacheDir . '/' . $class . 'Deprecations.log',
                        serialize(array_values($collectedLogs)));
                    file_put_contents($cacheDir . '/' . $class . 'Compiler.log',
    ...
    protected function getEnvParameters()
    {
        if (0 === func_num_args() || func_get_arg(0)) {
            @trigger_error(sprintf('The %s() method is deprecated as of 3.3 and will be removed in 4.0. Use the %%env()%% syntax to get the value of any environment variable from configuration files instead.',
                __METHOD__), E_USER_DEPRECATED);
        }
        ...
        foreach ($_SERVER as $key => $value) {
            if (0 === strpos($key, 'SYMFONY__')) {
                @trigger_error(sprintf('The support of special environment variables that start with SYMFONY__ (such as "%s") is deprecated as of 3.3 and will be removed in 4.0. Use the %%env()%% syntax instead to get the value of environment variables in configuration files.',
    ...
    protected function dumpContainer(ConfigCache $cache, ContainerBuilder $container, $class, $baseClass)
    {
        ...
        $content = $dumper->dump(array(
            ...
            'as_files' => true,
            'debug' => $this->debug,
            'inline_class_loader_parameter' => \PHP_VERSION_ID >= 70000 && !$this->loadClassCache && !class_exists(ClassCollectionLoader::class,
                false) ? 'container.dumper.inline_class_loader' : null,
        ));
