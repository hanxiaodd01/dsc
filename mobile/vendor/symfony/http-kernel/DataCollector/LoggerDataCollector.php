...
class LoggerDataCollector extends DataCollector implements LateDataCollectorInterface
{
    ...
    public function __construct($logger = null, $containerPathPrefix = null)
    {
        if (null !== $logger && $logger instanceof DebugLoggerInterface) {
            if (!method_exists($logger, 'clear')) {
                @trigger_error(sprintf('Implementing "%s" without the "clear()" method is deprecated since Symfony 3.4 and will be unsupported in 4.0 for class "%s".',
                    DebugLoggerInterface::class, \get_class($logger)), E_USER_DEPRECATED);
            }
    ...
    private function getContainerDeprecationLogs()
    {
        ...
        foreach (unserialize(file_get_contents($file)) as $log) {
            $log['context'] = array(
                'exception' => new SilencedErrorContext($log['type'], $log['file'], $log['line'], $log['trace'],
                    $log['count'])
            );
    ...
    private function getContainerCompilerLogs()
    {
        ...
        foreach (file($file, FILE_IGNORE_NEW_LINES) as $log) {
            $log = explode(': ', $log, 2);
            if (!isset($log[1]) || !preg_match('/^[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*+(?:\\\\[a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*+)++$/',
                    $log[0])) {
    ...
    private function sanitizeLogs($logs)
    {
        ...
        foreach ($logs as $log) {
            ...
            $errorId = md5("{$exception->getSeverity()}/{$exception->getLine()}/{$exception->getFile()}\0{$message}",
                true);
    ...
    private function isSilencedOrDeprecationErrorLog(array $log)
    {
        ...
        if ($exception instanceof \ErrorException && in_array($exception->getSeverity(),
                array(E_DEPRECATED, E_USER_DEPRECATED), true)) {
            return true;
        }
