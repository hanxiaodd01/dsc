...
class ParserTest extends TestCase
{
    ...
    public function testSpecifications($expected, $yaml, $comment, $deprecated)
    {
        ...
        if ($deprecated) {
            ...
            $this->assertContains(true !== $deprecated ? $deprecated : 'Using the comma as a group separator for floats is deprecated since version 3.2 and will be removed in 4.0 on line 1.',
                $deprecations[0]);
        }
    }
    ...
    /**
     * @group                      legacy
     * @expectedDeprecationMessage Using the Yaml::PARSE_KEYS_AS_STRINGS flag is deprecated since version 3.4 as it
     *                             will be removed in 4.0. Quote your keys when they are evaluable
     * @dataProvider               getNonStringMappingKeysData
     */
    public function testNonStringMappingKeys($expected, $yaml, $comment)
    {
        $this->assertSame($expected, var_export($this->parser->parse($yaml, Yaml::PARSE_KEYS_AS_STRINGS), true),
            $comment);
    }
...
bar: 1
EOF;
        $this->assertEquals(array('foo' => new B(), 'bar' => 1), $this->parser->parse($input, Yaml::PARSE_OBJECT),
...
EOF;
    ...
    {
        $this->assertEquals(array('foo' => new B(), 'bar' => 1), $this->parser->parse($yaml, Yaml::PARSE_OBJECT),
            '->parse() is able to parse objects');
    }
...
EOD;
     ...
     * @expectedDeprecation Duplicate key "%s" detected whilst parsing YAML. Silent handling of duplicate mapping keys
     *                      in YAML is deprecated %s and will throw \Symfony\Component\Yaml\Exception\ParseException in
     *                      4.0 on line %d. throws \Symfony\Component\Yaml\Exception\ParseException in 4.0
     */
...
EOF;
    ...
    /**
     * @group               legacy
     * @expectedDeprecation Implicit casting of numeric key to string is deprecated since version 3.3 and will throw
     *                      \Symfony\Component\Yaml\Exception\ParseException in 4.0. Quote your evaluable mapping keys
     *                      instead on line 2.
     */
...
EOF;
    ...
    /**
     * @group               legacy
     * @expectedDeprecation Implicit casting of non-string key to string is deprecated since version 3.3 and will throw
     *                      \Symfony\Component\Yaml\Exception\ParseException in 4.0. Quote your evaluable mapping keys
     *                      instead on line 1.
     */
...
EOT;
    ...
    public function testParserThrowsExceptionWithCorrectLineNumber($lineNumber, $yaml)
    {
        if (method_exists($this, 'expectException')) {
            ...
            $this->expectExceptionMessage(sprintf('Unexpected characters near "," at line %d (near "bar: "123",").',
                $lineNumber));
        } else {
...
EOF;
    ...
    {
        $this->assertEquals(new TaggedValue('foo', array('foo' => 'bar')),
...
YAML
            ...
            'mappings' => array(
                new TaggedValue('foo', array(
                    'foo' => new TaggedValue('quz', array('bar')),
                    'quz' => new TaggedValue('foo', array('quz' => 'bar'))
                )),
                <<<YAML
!foo
foo: !quz [bar]
...
YAML
            ...
            'inline' => array(
                array(
                    new TaggedValue('foo', array('foo', 'bar')),
                    new TaggedValue('quz',
                        array('foo' => 'bar', 'quz' => new TaggedValue('bar', array('one' => 'bar'))))
                ),
                <<<YAML
- !foo [foo, bar]
...
YAML
    ...
    /**
     * @expectedException \Symfony\Component\Yaml\Exception\ParseException
     * @expectedExceptionMessage Tags support is not enabled. Enable the `Yaml::PARSE_CUSTOM_TAGS` flag to use
     *                           "!iterator" at line 1 (near "!iterator [foo]").
     */
    ...
    /**
     * @group               legacy
     * @expectedDeprecation Using the unquoted scalar value "!iterator foo" is deprecated since version 3.3 and will be
     *                      considered as a tagged value in 4.0. You must quote it on line 1.
     */
    ...
    /**
     * @group               legacy
     * @expectedDeprecation Starting an unquoted string with a question mark followed by a space is deprecated since
     *                      version 3.3 and will throw \Symfony\Component\Yaml\Exception\ParseException in 4.0 on line
     *                      1.
     */
...
YAML;
    ...
    /**
     * @group               legacy
     * @expectedDeprecation The !php/const: tag to indicate dumped PHP constants is deprecated since version 3.4 and
     *                      will be removed in 4.0. Use the !php/const (without the colon) tag instead on line 2.
     * @expectedDeprecation The !php/const: tag to indicate dumped PHP constants is deprecated since version 3.4 and
     *                      will be removed in 4.0. Use the !php/const (without the colon) tag instead on line 4.
     * @expectedDeprecation The !php/const: tag to indicate dumped PHP constants is deprecated since version 3.4 and
     *                      will be removed in 4.0. Use the !php/const (without the colon) tag instead on line 5.
     */
...
YAML;
    ...
    /**
     * @group               legacy
     * @expectedDeprecation Using the Yaml::PARSE_KEYS_AS_STRINGS flag is deprecated since version 3.4 as it will be
     *                      removed in 4.0. Quote your keys when they are evaluable instead.
     */
