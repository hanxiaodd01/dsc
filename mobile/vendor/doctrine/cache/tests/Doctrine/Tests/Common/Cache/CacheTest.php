...
abstract class CacheTest extends \Doctrine\Tests\DoctrineTestCase
{
    ...
    public function testSetContainsFetchDelete($value)
    {
        ...
        if (is_object($value)) {
            $this->assertEquals($value, $cache->fetch('key'),
                'Objects retrieved from the cache must be equal but not necessarily the same reference');
        } else {
    ...
    public function testCacheKeyIsCaseSensitive()
    {
        ...
        $cache->delete('KEY');
        $this->assertTrue($cache->contains('key',
            'Deleting cache item with different case must not affect other cache item'));
    }
