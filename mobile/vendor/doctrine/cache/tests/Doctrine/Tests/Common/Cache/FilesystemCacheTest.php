...
class FilesystemCacheTest extends BaseFileCacheTest
{
    ...
    public function testCacheInSharedDirectoryIsPerExtension()
    {
        ...
        $this->assertSame(11, $cache1->fetch('key1'),
            'Cache value must not be influenced by a different cache in the same directory but different extension');
        ...
        $this->assertSame(21, $cache2->fetch('key1'),
            'flushAll() must not remove items with a different extension in a shared directory');
