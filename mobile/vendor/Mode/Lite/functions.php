...
function import($class, $baseUrl = '', $ext = EXT)
{
    ...
    if (empty($baseUrl)) {
        ...
        } elseif (in_array($class_strut[0],
                array('Think', 'Org', 'Behavior', 'Com', 'Vendor')) || is_dir(LIB_PATH . $class_strut[0])) {
            // 系统类库包和第三方类库包
            $baseUrl = LIB_PATH;
        } else { // 加载其他模块的类库
            $baseUrl = APP_PATH;
...
function cookie($name = '', $value = '', $option = null)
{
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            setcookie($name, '', time() - 3600, $config['path'], $config['domain'], $config['secure'],
                $config['httponly']);
