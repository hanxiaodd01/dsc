...
function get_cat_goods($id, $ru_id, $num = 8)
{
    ...
    foreach ($res as $idx => $row) {
        ...
        foreach ($child_cat_res as $key => $value) {
            ...
            $child_cat[$key]['url'] = build_uri('merchants_category', array('cid' => $value['cat_id']),
                $value['cat_name']);
        }
        ...
        foreach ($goods_res as $goods_idx => $goods_row) {
            if (0 < $goods_row['promote_price']) {
                $promote_price = bargain_price($goods_row['promote_price'], $goods_row['promote_start_date'],
            ...
            $goods[$goods_idx]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($goods_row['goods_name'],
                $GLOBALS['_CFG']['goods_name_length']) : $goods_row['goods_name'];
            ...
            $goods[$goods_idx]['url'] = build_uri('goods', array('gid' => $goods_row['goods_id']),
...
if (!$smarty->is_cached($dwt, $cache_id)) {
    ...
    assign_pager('merchants_index', $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max,
