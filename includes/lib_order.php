...
function available_payment_list($support_cod, $cod_fee = 0, $is_online = false, $order_amount = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $row['format_pay_fee'] = strpos($row['pay_fee'],
            '%') !== false ? $row['pay_fee'] : price_format($row['pay_fee'], false);
        $modules[] = $row;
    }
...
function order_info($order_id, $order_sn = '')
{
    ...
    if ($order) {
        ...
        $order['formated_profit_amount'] = price_format($order['total_fee'] - $order['cost_amount'] - $order['shipping_fee'],
            false);
        ...
        if (empty($order['confirm_take_time']) && $order['order_status'] == OS_CONFIRMED && $order['shipping_status'] == SS_RECEIVED && $order['shipping_status'] == PS_PAYED) {
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $other, 'UPDATE',
...
function order_fee(
    ...
    if (isset($order['uc_id']) && !empty($order['uc_id'])) {
        $coupons = get_coupons($order['uc_id'],
            array('c.cou_id', 'c.cou_man', 'c.cou_type', 'c.ru_id', 'c.cou_money', 'cu.uc_id'));
    ...
    $total['will_get_bonus'] = $order['extension_code'] == 'exchange_goods' ? 0 : price_format(get_total_bonus(),
        false);
...
function get_order_shipping_fee(
    $cart_goods,
    $consignee = '',
    $cart_value = '',
    $shipping_list = '',
            ...
            foreach ($cart_goods as $key => $row) {
                ...
                if ($shipping) {
                    if (!empty($step) && in_array($step, $step_array)) {
                        ...
                        if (isset($row['tmp_shipping_id']) && $row['tmp_shipping_id'] && in_array($row['tmp_shipping_id'],
                                $str_shipping)) {
                            $have_shipping = 1;
                        } else {
                            $have_shipping = 0;
...
function update_manual($goods_id, $num)
{
    ...
    if ($res) {
        return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('intelligent_weight'), $num, 'UPDATE',
            'goods_id = \'' . $goods_id . '\'');
    } else {
...
function update_return_num($goods_id, $return_num)
{
    ...
    if ($res) {
        ...
        return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('intelligent_weight'), $return_num, 'UPDATE',
            'goods_id = \'' . $goods_id . '\'');
    } else {
        return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('intelligent_weight'), $return_num, 'INSERT',
            'goods_id = \'' . $goods_id . '\'');
    }
}
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $order, 'UPDATE',
        'order_id = \'' . $order_id . '\'');
}
...
function cart_goods(
    ...
    foreach ($arr as $key => $value) {
        $currency_format = !empty($GLOBALS['_CFG']['currency_format']) ? explode('%',
            $GLOBALS['_CFG']['currency_format']) : '';
        $attr_id = !empty($value['goods_attr_id']) ? explode(',', $value['goods_attr_id']) : '';

        if (1 < count($currency_format)) {
            $goods_price = trim(get_final_price($value['goods_id'], $value['goods_number'], true, $attr_id,
                $value['warehouse_id'], $value['area_id'], $value['area_city'], 0, 0, $add_tocart),
                $currency_format[0]);
            $cart_price = trim($value['goods_price'], $currency_format[0]);
        } else {
            $goods_price = get_final_price($value['goods_id'], $value['goods_number'], true, $attr_id,
                $value['warehouse_id'], $value['area_id'], $value['area_city'], 0, 0, $add_tocart);
        ...
        if ($value['extension_code'] == 'package_buy') {
            ...
            if ($value['rec_type'] == 1) {
                ...
                if ($value['rec_type'] == 5) {
                    ...
                    $arr[$key]['url'] = build_uri('presale', array('act' => 'view', 'presaleid' => $presale['act_id']),
                        $presale['act_name']);
                } else {
                    if ($value['rec_type'] == 4) {
                        $arr[$key]['url'] = build_uri('exchange_goods', array('gid' => $value['goods_id']),
                            $value['goods_name']);
                    } else {
                        $arr[$key]['url'] = build_uri('goods', array('gid' => $value['goods_id']),
                            $value['goods_name']);
                    }
                }
            }
            ...
            if ($value['extension_code'] == 'presale' || 1 < $value['rec_type']) {
                ...
                if ($ru_type == 1 && 0 < $warehouse_id && $store_id == 0) {
                    ...
                    $products = get_warehouse_id_attr_number($value['goods_id'], $value['goods_attr_id'],
                        $goodsInfo['user_id'], $warehouse_id, $area_id, $area_city);
            ...
            if (defined('THEME_EXTENSION')) {
                $arr[$key]['goods_attr_text'] = get_goods_attr_info($value['goods_attr_id'], 'pice',
                    $value['warehouse_id'], $value['area_id'], $value['area_city'], 1);
            }
            ...
            if (0 < $store_id) {
                ...
                $goodsInfo = $GLOBALS['db']->getRow($sql);
                $products = get_warehouse_id_attr_number($value['goods_id'], $value['goods_attr_id'],
                    $goodsInfo['ru_id'], 0, 0, 0, '', $store_id);
                $attr_number = $products['product_number'];
...
function set_cloud_order_goods($cart_goods = array(), $order = array())
{
    ...
    if (!empty($order_detaillist)) {
        ...
        $order_request['area'] = get_table_date('region', 'region_id=\'' . $order['district'] . '\'',
            array('region_name'), 2);
        $order_request['city'] = get_table_date('region', 'region_id=\'' . $order['city'] . '\'', array('region_name'),
            2);
        $order_request['province'] = get_table_date('region', 'region_id=\'' . $order['province'] . '\'',
            array('region_name'), 2);
...
function addto_cart(
    ...
    $goods_price = get_final_price($goods_id, $num, true, $spec, $warehouse_id, $area_id, $area_city, 0, 0,
        $add_tocart);
...
function addto_cart_combo(
    ...
    if ($GLOBALS['_CFG']['use_storage'] == 1) {
        ...
        if (is_spec($spec) && !empty($prod)) {
            if (!empty($spec)) {
                if ($product_info['product_number'] < $num) {
                    $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['shortage'], $product_info['product_number']),
                        ERR_OUT_OF_STOCK);
                    return false;
                }
            }
...
function get_insert_group_main(
    $goods_id,
    $num = 1,
    $goods_spec = array(),
    $parent = 0,
...
function get_combo_goods_info($goods_id, $num = 1, $spec = array(), $parent = 0, $warehouse_area)
{
    ...
    $result['fittings_price'] = isset($result['fittings_price']) ? $result['fittings_price'] : get_final_price($goods_id,
        $num, true, $spec);
...
';
        ...
        $sql = 'SELECT ga.goods_attr_id, a.attr_name, ga.attr_value, ' . ' IF(g.model_attr < 1, ga.attr_price, IF(g.model_attr < 2, wap.attr_price, wa.attr_price)) as attr_price ' . 'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS ga ' . $leftJoin . ' left join ' . $GLOBALS['ecs']->table('attribute') . ' AS a ' . 'on a.attr_id = ga.attr_id ' . 'WHERE ' . db_create_in($arr,
                'ga.goods_attr_id') . ' ORDER BY a.sort_order, a.attr_id, ga.goods_attr_id';
        $res = $GLOBALS['db']->query($sql);
...
{
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $user, 'UPDATE',
        'user_id = \'' . $user_id . '\'');
}
...
function return_card_money($order_id = 0, $ret_id = 0, $return_sn = '')
{
    ...
    if ($row) {
        ...
        order_action($order_info['order_sn'], $order_info['order_status'], $order_info['shipping_status'],
            $order_info['pay_status'], $note, null, 0, $time);
    }
}
...
function get_query_vcard_return($order_id)
{
    ...
    if ($res) {
        foreach ($res as $key => $row) {
            ...
            $res[$key]['action_note'] = isset($res[$key]['action_note'][0]) && !empty($res[$key]['action_note'][0]) ? explode('：',
                $res[$key]['action_note'][0]) : '';
...
function get_cart_goods($cart_value = '', $type = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (1 < count($currency_format)) {
            $goods_price = trim(get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id,
                $row['warehouse_id'], $row['area_id'], $area_city, 0, 0, $add_tocart), $currency_format[0]);
            $cart_price = trim($row['goods_price'], $currency_format[0]);
        } else {
            $goods_price = get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $row['warehouse_id'],
                $row['area_id'], $area_city, 0, 0, $add_tocart);
        ...
        if ($warehouse_id && $row['extension_code'] != 'package_buy') {
            ...
            $goodsInfo = $GLOBALS['db']->getRow($sql);
            $products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'], $goodsInfo['user_id'],
            ...
            foreach ($shipping_list as $key => $val) {
                if (substr($val['shipping_code'], 0, 5) != 'ship_') {
                    if ($GLOBALS['_CFG']['freight_model'] == 0) {
                        if ($cart_goods) {
                            if (count($cart_goods) == 1) {
                                ...
                                if (!empty($cart_goods[0]['freight']) && $cart_goods[0]['is_shipping'] == 0) {
                                    if ($cart_goods[0]['freight'] == 1) {
                                        ...
                                        if ($trow['freight_type']) {
                                            ...
                                            $transport_tpl = get_goods_transport_tpl($cart_goods[0], $region, $val,
                                                $cart_goods[0]['goods_number']);
                                            ...
                                            $goods_transport = $GLOBALS['ecs']->get_select_find_in_set(2,
                                                $consignee['city'], $transport, $transport_where,
                                                'goods_transport_extend', 'area_id');
                                            $ship_transport = array('tid', 'ru_id', 'shipping_fee');
                                            $ship_transport_where = ' AND ru_id = \'' . $cart_goods[0]['ru_id'] . '\' AND tid = \'' . $cart_goods[0]['tid'] . '\'';
                                            $goods_ship_transport = $GLOBALS['ecs']->get_select_find_in_set(2,
                                                $val['shipping_id'], $ship_transport, $ship_transport_where,
                                                'goods_transport_express', 'shipping_id');
                                ...
                                $order_transpor = get_order_transport($cart_goods, $consignee, $val['shipping_id'],
                                    $val['shipping_code']);
                    ...
                    if (isset($val['insure']) && $val['insure']) {
                        $shipping_list[$key]['insure_formated'] = strpos($val['insure'],
                            '%') === false ? price_format($val['insure'], false) : $val['insure'];
                    }
...
function merge_order($from_order_sn, $to_order_sn)
{
    ...
    if ($from_order['order_status'] != OS_UNCONFIRMED && $from_order['order_status'] != OS_CONFIRMED) {
        ...
        if ($from_order['pay_status'] != PS_UNPAYED) {
            return sprintf($GLOBALS['_LANG']['ps_not_unpayed'], $from_order_sn);
        } else {
            if ($from_order['shipping_status'] != SS_UNSHIPPED) {
    ...
    if ($to_order['order_status'] != OS_UNCONFIRMED && $to_order['order_status'] != OS_CONFIRMED) {
        ...
        if ($to_order['pay_status'] != PS_UNPAYED) {
            return sprintf($GLOBALS['_LANG']['ps_not_unpayed'], $to_order_sn);
        } else {
            if ($to_order['shipping_status'] != SS_UNSHIPPED) {
    ...
    if (0 < $order['shipping_id']) {
        ...
        $order['shipping_fee'] = shipping_fee($shipping_area['shipping_code'], unserialize($shipping_area['configure']),
            $weight_price['weight'], $weight_price['amount'], $weight_price['number']);

        if (0 < $order['insure_fee']) {
            $order['insure_fee'] = shipping_insure_fee($shipping_area['shipping_code'], $order['goods_amount'],
                $shipping_area['insure']);
        }
    }
    ...
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_goods') . (' SET order_id = \'' . $order_id . '\' ') . 'WHERE order_id ' . db_create_in(array(
            $from_order['order_id'],
            $to_order['order_id']
        ));
    $GLOBALS['db']->query($sql);
    ...
    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE order_id ' . db_create_in(array(
            $from_order['order_id'],
            $to_order['order_id']
        ));
    $GLOBALS['db']->query($sql);
    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('pay_log') . ' WHERE order_id ' . db_create_in(array(
            $from_order['order_id'],
            $to_order['order_id']
        ));
    $GLOBALS['db']->query($sql);
...
function change_order_goods_storage(
    $order_id,
    $is_dec = true,
    $storage = 0,
    $use_storage = 0,
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        if ($row['extension_code'] != 'package_buy') {
            if ($is_dec) {
                change_goods_storage($row['goods_id'], $row['product_id'], 0 - $row['num'], $row['warehouse_id'],
                    $row['area_id'], $order_id, $use_storage, $admin_id, $store_id);
            } else {
                change_goods_storage($row['goods_id'], $row['product_id'], $row['num'], $row['warehouse_id'],
                    $row['area_id'], $order_id, $use_storage, $admin_id, $store_id);
            }
            ...
            while ($row_goods = $GLOBALS['db']->fetchRow($res_goods)) {
                ...
                if ($is_dec) {
                    change_goods_storage($row_goods['goods_id'], $row['product_id'],
                        0 - $row['num'] * $row_goods['goods_number'], $row['warehouse_id'], $row['area_id'], $order_id,
                        $use_storage, $admin_id);
                } else {
                    if ($is_goods['is_real']) {
                        change_goods_storage($row_goods['goods_id'], $row['product_id'],
                            $row['num'] * $row_goods['goods_number'], $row['warehouse_id'], $row['area_id'], $order_id,
                            $use_storage, $admin_id);
                    }
                }
            }
...
{
    if ($type == 'finished') {
        return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                OS_CONFIRMED,
                OS_RETURNED_PART,
                OS_SPLITED
            )) . (' AND ' . $alias . 'shipping_status ') . db_create_in(array(
                SS_SHIPPED,
                SS_RECEIVED
    ...
    if ($type == 'confirm_take') {
        return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                ...
                OS_RETURNED_PART,
                OS_SPLITED
            )) . (' AND ' . $alias . 'shipping_status ') . db_create_in(array(SS_RECEIVED)) . (' AND ' . $alias . 'pay_status ') . db_create_in(array(PS_PAYED)) . ' ';
    }
    ...
    if ($type == 'confirm_wait_goods') {
        return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                OS_CONFIRMED,
                OS_SPLITED
            )) . (' AND ' . $alias . 'shipping_status ') . db_create_in(array(SS_SHIPPED)) . (' AND ' . $alias . 'pay_status ') . db_create_in(array(PS_PAYED)) . ' ';
    } else {
        if ($type == 'await_ship') {
            return ' AND   ' . $alias . 'order_status ' . db_create_in(array(
                    OS_CONFIRMED,
                    ...
                    SS_PREPARING,
                    SS_SHIPPED_ING
                )) . (' AND ( ' . $alias . 'pay_status ') . db_create_in(array(
                    PS_PAYED,
                    PS_PAYING
                )) . (' OR ' . $alias . 'pay_id ') . db_create_in(payment_id_list(true)) . ') ';
        } else {
            if ($type == 'await_pay') {
                return ' AND   ' . $alias . 'order_status ' . db_create_in(array(
                        OS_CONFIRMED,
                        OS_SPLITED
                    )) . (' AND   ' . $alias . 'pay_status = \'') . PS_UNPAYED . '\'' . (' AND ( ' . $alias . 'shipping_status ') . db_create_in(array(
                        SS_SHIPPED,
                        SS_RECEIVED
                    )) . (' OR ' . $alias . 'pay_id ') . db_create_in(payment_id_list(false)) . ') ';
            } else {
                if ($type == 'unconfirmed') {
                    ...
                    if ($type == 'unprocessed') {
                        return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                                OS_UNCONFIRMED,
                                OS_CONFIRMED
                            )) . (' AND ' . $alias . 'shipping_status = \'') . SS_UNSHIPPED . '\'' . (' AND ' . $alias . 'pay_status = \'') . PS_UNPAYED . '\' ';
                    } else {
                        if ($type == 'unpay_unship') {
                            return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                                    OS_UNCONFIRMED,
                                    OS_CONFIRMED
                                )) . (' AND ' . $alias . 'shipping_status ') . db_create_in(array(
                            ...
                            if ($type == 'shipped') {
                                return ' AND ' . $alias . 'order_status = \'' . OS_CONFIRMED . '\'' . (' AND ' . $alias . 'shipping_status ') . db_create_in(array(
                                        SS_SHIPPED,
                                        SS_RECEIVED
                                    )) . ' ';
                            } else {
                                if ($type == 'real_pay') {
                                    return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                                            ...
                                            OS_SPLITED,
                                            OS_RETURNED_PART
                                        )) . (' AND ' . $alias . 'shipping_status <> ') . SS_UNSHIPPED . (' AND ' . $alias . 'pay_status ') . db_create_in(array(
                                            PS_PAYED,
                                            PS_PAYING
                                        )) . ' ';
                                } else {
...
function compute_discount($type = 0, $newInfo = array(), $use_type = 0, $ru_id = 0)
{
    ...
    $sql = 'SELECT *' . 'FROM ' . $GLOBALS['ecs']->table('favourable_activity') . (' WHERE review_status = 3 AND start_time <= \'' . $now . '\'') . (' AND end_time >= \'' . $now . '\'') . ' AND CONCAT(\',\', user_rank, \',\') LIKE \'%' . $user_rank . '%\'' . ' AND act_type ' . db_create_in(array(
            FAT_DISCOUNT,
            FAT_PRICE
        ));
    ...
    foreach ($favourable_list as $favourable) {
        ...
        if ($favourable['act_range'] == FAR_ALL) {
            ...
            if ($GLOBALS['_CFG']['region_store_enabled']) {
                $mer_ids = get_favourable_merchants($favourable['userFav_type'], $favourable['userFav_type_ext'],
                    $favourable['rs_id'], 1);
                $rs_label = false;
                        ...
                        if (in_array($goods['ru_id'], $mer_ids) || $rs_label) {
                            if ($use_type == 1) {
                                ...
                                if ($favourable['userFav_type'] == 1) {
                                    ...
                                    if ($favourable['user_id'] == $goods['ru_id'] && $rs_label || in_array($goods['ru_id'],
                                            $mer_ids)) {
                                        $total_amount += $goods['subtotal'];
                                    }
                                }
            ...
            if ($favourable['act_range'] == FAR_CATEGORY) {
                ...
                if ($favourable['act_range'] == FAR_BRAND) {
                    $favourable['act_range_ext'] = return_act_range_ext($favourable['act_range_ext'],
                        $favourable['userFav_type'], $favourable['act_range']);

                    foreach ($goods_list as $goods) {
                        if ($goods['act_id'] == $favourable['act_id']) {
                            if (strpos(',' . $favourable['act_range_ext'] . ',',
                                    ',' . $goods['brand_id'] . ',') !== false) {
                                if ($use_type == 1) {
                                    if ($favourable['user_id'] == $goods['ru_id']) {
                                        $total_amount += $goods['subtotal'];
                                    }
                    ...
                    if ($favourable['act_range'] == FAR_GOODS) {
                        if ($GLOBALS['_CFG']['region_store_enabled']) {
                            $mer_ids = get_favourable_merchants($favourable['userFav_type'],
                                $favourable['userFav_type_ext'], $favourable['rs_id']);
                            $where = '';
...
function add_package_to_cart($package_id, $num = 1, $warehouse_id, $area_id, $type)
{
    ...
    foreach ($package['goods_list'] as $key => $val) {
        if (!$val['stock_number'] || $val['stock_number'] < $goods_number * $val['goods_number']) {
            ...
            if ($val['stock_number'] < $num * $val['goods_number']) {
                $is_fail = 3;
                $goods_name = $val['goods_name'];
                break;
            }
...
function return_order_info($ret_id = 0, $order_sn = '', $order_id = 0)
{
    ...
    if ($order) {
        ...
        $order['formated_return_amount'] = price_format($order['should_return'] + $order['return_shipping_fee'] - $order['discount_amount'],
            false);
    ...
    foreach ($merchant_goods as $key => $row) {
        ...
        if ($user_cart_goods) {
            foreach ($user_cart_goods as $goods_key => $goods_row) {
                ...
                if (isset($sort_favourable['by_all']) && $goods_row['extension_code'] != 'package_buy' && substr($goods_row['extension_code'],
                        0, 7) != 'seckill') {
                    foreach ($sort_favourable['by_all'] as $fav_key => $fav_row) {
                        if ($goods_row['act_id'] == $fav_row['act_id'] || empty($goods_row['act_id'])) {
                            ...
                            if ($GLOBALS['_CFG']['region_store_enabled']) {
                                $mer_ids = get_favourable_merchants($fav_row['userFav_type'],
                                    $fav_row['userFav_type_ext'], $fav_row['rs_id'], 1, $goods_row['ru_id']);
                            }
                                    ...
                                    if (isset($goods_row) && $goods_row) {
                                        ...
                                        switch ($fav_row['act_type']) {
                                            ...
                                            case 1:
                                                ...
                                                $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type_ext_format'] = number_format($fav_row['act_type_ext'],
                                                    2);
                                                break;
                                        ...
                                        $merchant_goods[$key]['new_list'][$fav_row['act_id']]['available'] = favourable_available($fav_row,
                                            $cart_value, $goods_row['ru_id']);
                                        ...
                                        $merchant_goods[$key]['new_list'][$fav_row['act_id']]['favourable_used'] = favourable_used($fav_row,
                                            $cart_favourable);
                                        ...
                                        if (empty($goods_row['act_id'])) {
                                            update_cart_goods_fav($goods_row['rec_id'], $_SESSION['user_id'],
                                                $fav_row['act_id']);
                                        }

                                        $goods_row['favourable_list'] = get_favourable_info($goods_row['goods_id'],
                                            $goods_row['ru_id'], $goods_row);
                ...
                if (isset($sort_favourable['by_category']) && $goods_row['extension_code'] != 'package_buy' && substr($goods_row['extension_code'],
                        0, 7) != 'seckill') {
                    ...
                    if (in_array($cat_id, $id_list) && $goods_row['is_gift'] == 0 || in_array($goods_row['is_gift'],
                            $favourable_id_list)) {
                        foreach ($sort_favourable['by_category'] as $fav_key => $fav_row) {
                            if (isset($goods_row) && $goods_row && ($goods_row['act_id'] == $fav_row['act_id'] || empty($goods_row['act_id']))) {
                                $fav_act_range_ext = !empty($fav_row['act_range_ext']) ? explode(',',
                                    $fav_row['act_range_ext']) : array();
                                ...
                                if ($goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0 && in_array($cat_id,
                                        $fav_act_range_ext)) {
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_id'] = $fav_row['act_id'];
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_name'] = $fav_row['act_name'];
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type'] = $fav_row['act_type'];
                ...
                if (isset($sort_favourable['by_brand']) && $goods_row['extension_code'] != 'package_buy' && substr($goods_row['extension_code'],
                        0, 7) != 'seckill') {
                    ...
                    if (in_array(trim($brand_id),
                            $get_act_range_ext) && $goods_row['is_gift'] == 0 || in_array($goods_row['is_gift'],
                            $favourable_id_list)) {
                        foreach ($sort_favourable['by_brand'] as $fav_key => $fav_row) {
                            $act_range_ext_str = ',' . $fav_row['act_range_ext'] . ',';
                            $brand_id_str = ',' . $brand_id . ',';
                            if (isset($goods_row) && $goods_row && ($goods_row['act_id'] == $fav_row['act_id'] || empty($goods_row['act_id']))) {
                                if ($goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0 && strstr($act_range_ext_str,
                                        trim($brand_id_str))) {
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_id'] = $fav_row['act_id'];
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_name'] = $fav_row['act_name'];
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type'] = $fav_row['act_type'];
                ...
                if (isset($sort_favourable['by_goods']) && $goods_row['extension_code'] != 'package_buy' && substr($goods_row['extension_code'],
                        0, 7) != 'seckill') {
                    ...
                    if (in_array($goods_row['goods_id'], $get_act_range_ext) || in_array($goods_row['is_gift'],
                            $favourable_id_list)) {
                        foreach ($sort_favourable['by_goods'] as $fav_key => $fav_row) {
                            ...
                            if (isset($goods_row) && $goods_row && ($goods_row['act_id'] == $fav_row['act_id'] || empty($goods_row['act_id']))) {
                                if (strstr($act_range_ext_str,
                                        $goods_id_str) && $goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0) {
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_id'] = $fav_row['act_id'];
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_name'] = $fav_row['act_name'];
                                    $merchant_goods[$key]['new_list'][$fav_row['act_id']]['act_type'] = $fav_row['act_type'];
...
function favourable_list($user_rank, $user_id = -1, $fav_id = 0, $act_sel_id = array(), $ru_id = -1)
{
    ...
    while ($favourable = $GLOBALS['db']->fetchRow($res)) {
        ...
        $favourable['act_type_desc'] = sprintf($GLOBALS['_LANG']['fat_ext'][$favourable['act_type']],
        ...
        $favourable['act_range_ext'] = return_act_range_ext($favourable['act_range_ext'], $favourable['userFav_type'],
            $favourable['act_range']);
...
function cart_favourable_amount(
    $favourable,
    $act_sel_id = array('act_sel_id' => '', 'act_pro_sel_id' => '', 'act_sel' => ''),
    $ru_id = -1
) {
    if (!empty($_SESSION['user_id'])) {
        $c_sess = ' c.user_id = \'' . $_SESSION['user_id'] . '\' ';
        ...
        if ($favourable['act_range'] == FAR_CATEGORY) {
            ...
            if ($favourable['act_range'] == FAR_BRAND) {
                ...
                if ($favourable['userFav_type'] == 1 && $id_list) {
                    ...
                    $id_list = return_act_range_ext($favourable['act_range_ext'], $favourable['userFav_type'],
                        $favourable['act_range']);
...
function get_act_range_ext($user_rank, $user_id = 0, $act_range)
{
    ...
    foreach ($res as $key => $row) {
        if ($row['act_range'] == FAR_GOODS && $GLOBALS['_CFG']['region_store_enabled']) {
            ...
            $row['act_range_ext'] = return_act_range_ext($row['act_range_ext'], $row['userFav_type'],
                $row['act_range']);
    ...
    foreach ($merchant_goods as $key => $row) {
        ...
        if ($row['ru_id'] == $favourable_activity['user_id']) {
            foreach ($user_cart_goods as $goods_key => $goods_row) {
                ...
                if (!empty($act_sel_id)) {
                    $goods_row['sel_checked'] = strstr(',' . $act_sel_id['act_sel_id'] . ',',
                        ',' . $goods_row['rec_id'] . ',') ? 1 : 0;
                }
                ...
                if ($goods_row['act_id'] == $favourable_activity['act_id'] || empty($goods_row['act_id'])) {
                    ...
                    if ($favourable_activity['act_range'] == FAR_CATEGORY && $goods_row['extension_code'] != 'package_buy') {
                        ...
                        if (in_array(trim($cat_id),
                                $id_list) && $goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0 || $goods_row['is_gift'] == $favourable_activity['act_id']) {
                            if ($goods_row['act_id'] == $favourable_activity['act_id'] || empty($goods_row['act_id'])) {
                    ...
                    if ($favourable_activity['act_range'] == FAR_BRAND && $goods_row['extension_code'] != 'package_buy') {
                        ...
                        $brand_id = $goods_row['brand_id'];
                        if (in_array(trim($brand_id),
                                $get_act_range_ext) && $goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0 || $goods_row['is_gift'] == $favourable_activity['act_id']) {
                            if ($goods_row['act_id'] == $favourable_activity['act_id'] || empty($goods_row['act_id'])) {
                    ...
                    if ($favourable_activity['act_range'] == FAR_GOODS && $goods_row['extension_code'] != 'package_buy') {
                        $get_act_range_ext = get_act_range_ext($_SESSION['user_rank'], $row['ru_id'], 3);
                        if (in_array($goods_row['goods_id'],
                                $get_act_range_ext) || $goods_row['is_gift'] == $favourable_activity['act_id']) {
                            if ($goods_row['act_id'] == $favourable_activity['act_id'] || empty($goods_row['act_id'])) {
                                $act_range_ext_str = ',' . $favourable_activity['act_range_ext'] . ',';
                                $goods_id_str = ',' . $goods_row['goods_id'] . ',';
                                if (strstr($act_range_ext_str,
                                        trim($goods_id_str)) && $goods_row['is_gift'] == 0 && $goods_row['parent_id'] == 0) {
                                    $favourable_box['act_id'] = $favourable_activity['act_id'];
                                    $favourable_box['act_name'] = $favourable_activity['act_name'];
                                    $favourable_box['act_type'] = $favourable_activity['act_type'];
                                    ...
                                    $cart_favourable = cart_favourable($goods_row['ru_id']);
                                    $favourable_box['cart_favourable_gift_num'] = empty($cart_favourable[$favourable_activity['act_id']]) ? 0 : intval($cart_favourable[$favourable_activity['act_id']]);
                                    $favourable_box['favourable_used'] = favourable_used($favourable_box,
                                        $cart_favourable);
                                    $favourable_box['left_gift_num'] = intval($favourable_activity['act_type_ext']) - (empty($cart_favourable[$favourable_activity['act_id']]) ? 0 : intval($cart_favourable[$favourable_activity['act_id']]));
...
function return_user_surplus_integral_bonus($order)
{
    if (0 < $order['user_id'] && 0 < $order['surplus']) {
        ...
        log_account_change($order['user_id'], $surplus, 0, 0, 0,
            sprintf($GLOBALS['_LANG']['return_order_surplus'], $order['order_sn']), ACT_OTHER, 1);
    ...
    if (0 < $order['user_id'] && 0 < $order['integral']) {
        log_account_change($order['user_id'], 0, 0, 0, $order['integral'],
            sprintf($GLOBALS['_LANG']['return_order_integral'], $order['order_sn']), ACT_OTHER, 1);
    }
...
function checked_pay_Invalid_order($pay_effective_time = 0)
{
    ...
    $sql = 'SELECT oi.order_id , oi.order_sn , oi.user_id,oi.surplus,oi.money_paid,oi.bonus_id,oi.integral_money,oi.bonus FROM ' . $GLOBALS['ecs']->table('order_info') . (' AS oi WHERE oi.add_time < ' . $time . ' ') . order_query_sql('unpay_unship',
            'oi.') . $where . 'AND (SELECT p.pay_code FROM ' . $GLOBALS['ecs']->table('payment') . 'AS p WHERE p.pay_id = oi.pay_id) NOT IN(\'bank\',\'cod\',\'post\')';
    ...
    if (!empty($order_list)) {
        foreach ($order_list as $k => $v) {
            ...
            order_action($v['order_sn'], OS_INVALID, SS_UNSHIPPED, PS_UNPAYED,
                $GLOBALS['_LANG']['pay_effective_Invalid']);
...
function get_favourable_info($goods_id = 0, $ru_id = 0, $goods = array())
{
    ...
    if (empty($goods_id)) {
        foreach ($res as $rows) {
            ...
            $favourable[$rows['act_id']]['time'] = sprintf($GLOBALS['_LANG']['promotion_time'],
                local_date('Y-m-d', $rows['start_time']), local_date('Y-m-d', $rows['end_time']));
        ...
        foreach ($res as $rows) {
            if ($rows['act_range'] == FAR_ALL) {
                ...
                if ($GLOBALS['_CFG']['region_store_enabled']) {
                    $mer_ids = get_favourable_merchants($rows['userFav_type'], $rows['userFav_type_ext'],
                        $rows['rs_id'], 1, $ru_id);
                }
                ...
                if ($rows['act_range'] == FAR_CATEGORY) {
                    $id_list = array();
                    ...
                    $list_array = !empty($list_array) ? array_merge($raw_id_list,
                        $list_array[$rows['act_id']]) : $raw_id_list;
                    $id_list = arr_foreach($list_array);
                    $id_list = array_unique($id_list);
                    $ids = join(',', array_unique($id_list));
                    ...
                    if ($rows['act_range'] == FAR_BRAND) {
                        $rows['act_range_ext'] = return_act_range_ext($rows['act_range_ext'], $rows['userFav_type'],
                            $rows['act_range']);
                        ...
                        if ($rows['act_range'] == FAR_GOODS) {
                            if (strpos(',' . $rows['act_range_ext'] . ',', ',' . $goods_id . ',') !== false) {
                                $mer_ids = true;

                                if ($GLOBALS['_CFG']['region_store_enabled']) {
                                    $mer_ids = get_favourable_merchants($rows['userFav_type'],
                                        $rows['userFav_type_ext'], $rows['rs_id'], 1, $ru_id);
                                }
                                ...
                                if ($mer_ids) {
                                    ...
                                    $favourable[$rows['act_id']]['url'] = 'activity.php';
                                    $favourable[$rows['act_id']]['time'] = sprintf($GLOBALS['_LANG']['promotion_time'],
                                        local_date('Y-m-d', $rows['start_time']),
                                        local_date('Y-m-d', $rows['end_time']));
                                    $favourable[$rows['act_id']]['sort'] = $rows['start_time'];
                                    $favourable[$rows['act_id']]['type'] = 'favourable';
                                    $favourable[$rows['act_id']]['act_type'] = $rows['act_type'];
                                }
