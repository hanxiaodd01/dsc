...
class integrate
{
    ...
    public function __construct($cfg)
    {
        ...
        if (empty($cfg['db_host'])) {
            ...
            if (empty($cfg['is_latin1'])) {
                $this->db = new cls_mysql($cfg['db_host'], $cfg['db_user'], $cfg['db_pass'], $cfg['db_name'],
                    $this->charset, null, $quiet);
            } else {
                $this->db = new cls_mysql($cfg['db_host'], $cfg['db_user'], $cfg['db_pass'], $cfg['db_name'], 'latin1',
                    null, $quiet);
            }
        }
    ...
    public function add_user(
        ...
        $values = array(
            $post_username,
            $registerMode_info['mobile_phone'],
            $registerMode_info['email'],
            $post_password
        );
        ...
        $sql = 'INSERT INTO ' . $this->table($this->user_table) . ' (' . implode(',',
    ...
    public function edit_user($cfg, $forget_pwd = '0')
    {
        ...
        if ($values) {
            $sql = 'UPDATE ' . $this->table($this->user_table) . ' SET ' . implode(', ',
                    $values) . ' WHERE ' . $this->field_id . '=\'' . $cfg['user_id'] . '\' LIMIT 1';
            $this->db->query($sql);
    ...
    public function remove_user($id)
    {
        ...
        if ($this->need_sync || isset($this->is_ecshop) && $this->is_ecshop) {
            ...
            if ($col) {
                $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET parent_id = 0 WHERE ' . db_create_in($col,
                        'parent_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('users') . ' WHERE ' . db_create_in($col, 'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'SELECT order_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $col_order_id = $GLOBALS['db']->getCol($sql);

                if ($col_order_id) {
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE ' . db_create_in($col_order_id,
                            'order_id');
                    $GLOBALS['db']->query($sql);
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('order_goods') . ' WHERE ' . db_create_in($col_order_id,
                            'order_id');
                    $GLOBALS['db']->query($sql);
                }

                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('booking_goods') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('collect_goods') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('feedback') . ' WHERE ' . db_create_in($col, 'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('user_address') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('user_bonus') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('user_account') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('tag') . ' WHERE ' . db_create_in($col, 'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('account_log') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('users_auth') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);
                $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('connect_user') . ' WHERE ' . db_create_in($col,
                        'user_id');
                $GLOBALS['db']->query($sql);

                if (file_exists(MOBILE_WECHAT)) {
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('wechat_user') . ' WHERE ' . db_create_in($col,
                            'ect_uid');
                    $GLOBALS['db']->query($sql);
                }

                if (file_exists(MOBILE_DRP)) {
                    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET drp_parent_id = 0 WHERE ' . db_create_in($col,
                            'drp_parent_id');
                    $GLOBALS['db']->query($sql);
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('drp_affiliate_log') . ' WHERE ' . db_create_in($col,
                            'user_id');
                    $GLOBALS['db']->query($sql);
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('drp_log') . ' WHERE ' . db_create_in($col,
                            'user_id');
                    $GLOBALS['db']->query($sql);
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('drp_shop') . ' WHERE ' . db_create_in($col,
                            'user_id');
                    $GLOBALS['db']->query($sql);
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('drp_type') . ' WHERE ' . db_create_in($col,
                            'user_id');
                    $GLOBALS['db']->query($sql);
                    $sql = 'DELETE FROM ' . $GLOBALS['ecs']->table('drp_transfer_log') . ' WHERE ' . db_create_in($col,
                            'user_id');
                    $GLOBALS['db']->query($sql);
                }
            }
    ...
    public function sync($username, $password = '', $md5password = '')
    {
        ...
        if (empty($profile)) {
            ...
            if (empty($values)) {
                ...
                $sql = 'UPDATE ' . $GLOBALS['ecs']->table('users') . ' SET ' . implode(', ',
                        $values) . (' WHERE user_name=\'' . $username . '\'');
                $GLOBALS['db']->query($sql);
                return true;
            }
        }
    ...
    public function get_points($username)
    {
        ...
        if ($fileds) {
            $sql = 'SELECT ' . $this->field_id . ', ' . implode(', ',
                    $fileds) . ' FROM ' . $this->table($this->user_table) . ' WHERE ' . $this->field_name . ('=\'' . $username . '\'');
            $row = $this->db->getRow($sql);
            return $row;
        } else {
            return false;
    ...
    public function set_points($username, $credits)
    {
        ...
        if ($set) {
            ...
            $sql = 'UPDATE ' . $this->table($this->user_table) . ' SET ' . implode(', ',
                    $tmp) . ' WHERE ' . $this->field_name . (' = \'' . $username . '\'');
            $this->db->query($sql);
        }
    ...
    public function test_conflict($user_list)
    {
        ...
        $sql = 'SELECT ' . $this->field_name . ' FROM ' . $this->table($this->user_table) . ' WHERE ' . db_create_in($user_list,
                $this->field_name);
