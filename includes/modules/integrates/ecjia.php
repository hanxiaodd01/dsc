...
class ecjia extends ucenter
{
    public function login($username, $password, $remember = null, $is_oath = 1)
    {
        ...
        $local_user = DB::table('users')->select('user_id', 'user_name', 'password', 'email', 'mobile_phone',
            'ec_salt')->where(key($condition), current($condition))->first();
        ...
        if (0 < $uid) {
            $connect_user = DB::table('connect_user')->where('connect_code', $this->connectType())->where('open_id',
                $uid)->leftJoin('users', 'users.user_id', 'connect_user.user_id')->first();
            ...
            if (is_null($connect_user)) {
                ...
                DB::table('connect_user')->insert(array(
                ...
                DB::table('users')->where('user_id', collect($local_user)->get('user_id'))->update(array(
                    'password' => $password,
                    'ec_salt' => $ec_salt
                ));
