				...
				if ($pay_log['order_type'] == PAY_ORDER) {
					...
					if ($user_account) {
						if ($user_account['is_paid'] == 0) {
							...
							$smsParams = array('user_name' => $user_info['user_name'], 'username' => $user_info['user_name'], 'user_money' => $user_info['user_money'], 'usermoney' => $user_info['user_money'], 'op_time' => local_date('Y-m-d H:i:s', gmtime()), 'optime' => local_date('Y-m-d H:i:s', gmtime()), 'add_time' => local_date('Y-m-d H:i:s', gmtime()), 'addtime' => local_date('Y-m-d H:i:s', gmtime()), 'examine' => $GLOBALS['_LANG']['through'], 'process_type' => $GLOBALS['_LANG']['surplus_type_0'], 'processtype' => $GLOBALS['_LANG']['surplus_type_0'], 'fmt_amount' => $user_account['amount'], 'fmtamount' => $user_account['amount'], 'mobile_phone' => $user_info['mobile_phone'] ? $user_info['mobile_phone'] : '', 'mobilephone' => $user_info['mobile_phone'] ? $user_info['mobile_phone'] : '');
					...
					if (0 < $child_num && 0 < $order_id) {
						...
						foreach ($order_res as $row) {
							...
							if ($suppliers && $GLOBALS['_CFG']['sms_order_payed'] == '1' && $suppliers['mobile_phone'] != '') {
								...
								$smsParams = array('shop_name' => $shop_name, 'shopname' => $shop_name, 'order_sn' => $wholesale_order['order_sn'], 'ordersn' => $wholesale_order['order_sn'], 'consignee' => $wholesale_order['consignee'], 'order_region' => $order_region, 'orderregion' => $order_region, 'address' => $wholesale_order['address'], 'order_mobile' => $wholesale_order['mobile'], 'ordermobile' => $wholesale_order['mobile'], 'mobile_phone' => $suppliers['mobile_phone'], 'mobilephone' => $suppliers['mobile_phone']);
