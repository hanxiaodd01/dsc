        ...
        if (0 < $log_id) {
            ...
            if ($pay_log && $pay_log['is_paid'] == 0) {
                ...
                if ($pay_log['order_type'] == PAY_ORDER) {
                    ...
                    foreach ($order_id_arr as $o_key => $o_val) {
                        ...
                        if ($order['extension_code'] == 'presale') {
                            ...
                            if ($order['pay_status'] == 0) {
                                ...
                                if (0 < $order_id) {
                                    order_action($order_sn, OS_CONFIRMED, SS_UNSHIPPED, PS_PAYED_PART, $note,
                                        $GLOBALS['_LANG']['buyer']);
                                ...
                                if ($order_id) {
                                    order_action($order_sn, OS_CONFIRMED, SS_UNSHIPPED, PS_PAYED, $note,
                                        $GLOBALS['_LANG']['buyer']);
                        ...
                        if ($main_order_id == 0 && 0 < $child_num && 0 < $order_id) {
                            ...
                            foreach ($order_res as $row) {
                                ...
                                order_action($row['order_sn'], OS_CONFIRMED, SS_UNSHIPPED, $pay_status, $note,
                                    $GLOBALS['_LANG']['buyer']);
                            }
                        }
                                    ...
                                    if ($order['shipping_id'] == -1 || empty($order['shipping_id'])) {
                                        ...
                                        if ($GLOBALS['db']->getOne($sql) <= 0) {
                                            ...
                                            if (0 < $order['user_id']) {
                                                ...
                                                log_account_change($order['user_id'], 0, 0,
                                                    intval($integral['rank_points']),
                                                    intval($integral['custom_points']),
                                                    sprintf($GLOBALS['_LANG']['order_gift_integral'],
                                                        $order['order_sn']));
                    ...
                    if ($pay_log['order_type'] == PAY_SURPLUS) {
                        ...
                        if ($user_account) {
                            if ($user_account['is_paid'] == 0) {
                                ...
                                log_account_change($user_account['user_id'], $user_account['amount'], 0, 0, 0,
                                    $GLOBALS['_LANG']['surplus_type_0'], ACT_SAVING);
                                ...
                                $smsParams = array(
                                    ...
                                    'add_time' => local_date('Y-m-d H:i:s', gmtime()),
                                    'addtime' => local_date('Y-m-d H:i:s', gmtime()),
                                    'examine' => $GLOBALS['_LANG']['through'],
                                    'process_type' => $GLOBALS['_LANG']['surplus_type_0'],
                                    'processtype' => $GLOBALS['_LANG']['surplus_type_0'],
                        ...
                        if ($pay_log['order_type'] == PAY_APPLYTEMP) {
                            ...
                            Import_temp($seller_template_apply['temp_code'], $new_suffix,
                                $seller_template_apply['ru_id']);
                            ...
                            if ($pay_log['order_type'] == PAY_APPLYGRADE) {
                                ...
                                if ($pay_log['order_type'] == PAY_TOPUP) {
                                    ...
                                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_account_log'), $log,
                                        'INSERT');
                                } else {
                                    if ($pay_log['order_type'] == PAY_WHOLESALE) {
                                        ...
                                        if (0 < $child_num && 0 < $order_id) {
                                            ...
                                            foreach ($order_res as $row) {
                                                ...
                                                $GLOBALS['db']->query($sql);
                                                $sql = 'SELECT * FROM ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' WHERE order_id = \'' . $row['order_id'] . '\'';
                                                $wholesale_order = $GLOBALS['db']->getRow($sql);
                                                $sql = 'SELECT user_id, mobile_phone FROM ' . $GLOBALS['ecs']->table('suppliers') . ' WHERE suppliers_id = \'' . $wholesale_order['suppliers_id'] . '\'';
                                                $suppliers = $GLOBALS['db']->getRow($sql);
                                                if ($suppliers && $GLOBALS['_CFG']['sms_order_payed'] == '1' && $suppliers['mobile_phone'] != '') {
                                                    $shop_name = get_shop_name($suppliers['user_id'], 1);
                                                    $order_region = get_flow_wholesale_user_region($row['order_id']);
                                                    $smsParams = array(
                                                        'shop_name' => $shop_name,
                                                        'shopname' => $shop_name,
                                                        'order_sn' => $wholesale_order['order_sn'],
                                                        'ordersn' => $wholesale_order['order_sn'],
                                                        'consignee' => $wholesale_order['consignee'],
                                                        'order_region' => $order_region,
                                                        'orderregion' => $order_region,
                                                        'address' => $wholesale_order['address'],
                                                        'order_mobile' => $wholesale_order['mobile'],
                                                        'ordermobile' => $wholesale_order['mobile'],
                                                    ...
                                                    if ($GLOBALS['_CFG']['sms_type'] == 0) {
                                                        ...
                                                        if (1 <= $GLOBALS['_CFG']['sms_type']) {
                                                            $result = sms_ali($smsParams, 'sms_order_payed');
                                                            $sms_send[$suppliers['user_id']] = $result;
                                                        }
                                                    }
                                                }
                                            }
                                        ...
                                        if ($sms_send && $GLOBALS['_CFG']['sms_order_payed'] == '1' && 1 <= $GLOBALS['_CFG']['sms_type']) {
                                            $GLOBALS['ecs']->ali_yu($sms_send, 1);
                                        }
                                    }
                                }
                        ...
                        if (virtual_goods_ship($virtual_goods, $msg, $order['order_sn'], true)) {
                            if ($order['shipping_id'] == -1 || empty($order['shipping_id'])) {
                                ...
                                if ($GLOBALS['db']->getOne($sql) <= 0) {
                                    update_order($order_id,
                                        array('shipping_status' => SS_SHIPPED, 'shipping_time' => gmtime()));
                                    order_action($order_sn, OS_CONFIRMED, SS_SHIPPED, $order['pay_status'], $note,
                                        $GLOBALS['_LANG']['buyer']);
