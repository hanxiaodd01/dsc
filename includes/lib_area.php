...
if ($_REQUEST['act'] == 'select_regionChild') {
    ...
    if ($region->type == 0) {
        if (empty($city_list)) {
            setcookie('province', $region->region_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
        }
    } else {
        if ($region->type == 1) {
            setcookie('type_province', $region->region_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
        } else {
            if ($region->type == 2) {
                setcookie('type_city', $region->region_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                    $GLOBALS['cookie_domain']);
            }
        }
    }
    ...
    if (empty($city_list)) {
        $result['city_list'] = 1;
        setcookie('province', $_COOKIE['type_province'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
            $GLOBALS['cookie_domain']);
        setcookie('city', $_COOKIE['type_city'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
            $GLOBALS['cookie_domain']);
    }
    ...
    if ($_REQUEST['act'] == 'select_district_list') {
        include_once 'includes/cls_json.php';
        $result = array('error' => 0, 'message' => '', 'content' => '', 'ra_id' => '', 'region_id' => '');
        $json = new JSON();
        $_POST['region'] = strip_tags(urldecode($_POST['region']));
        ...
        if ($region->type == 0) {
            setcookie('province', $province, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
            setcookie('city', $region->region_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
            $district_id = 0;
            ...
            setcookie('street', $street_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
            setcookie('street_area', $street_list, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
            $region_top = get_warehouse_goods_region($province);
            setcookie('area_region', $region_top['region_id'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
            ...
            setcookie('type_district', $region->region_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
        }
...
if ($type_province) {
    ...
    if ($provinceT_list) {
        ...
        if ($city_district_list) {
            ...
            if ($cityT_list['parent_id'] == $type_province) {
                $_COOKIE['province'] = $type_province;
