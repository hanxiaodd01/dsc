...
function get_purchase_list($filter = array(), $size = 10, $page = 1, $sort = 'add_time', $order = 'DESC')
{
    ...
    if (isset($filter['is_finished'])) {
        ...
        if ($filter['is_finished'] == 0) {
            ...
            if ($filter['is_finished'] == 1) {
                $where .= ' AND end_time < \'' . $now . '\' ';
            } else {
                if ($filter['is_finished'] == 2) {
                }
            }
        }
    }
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $row['goods_number'] = get_table_date('wholesale_purchase_goods',
            'purchase_id = \'' . $row['purchase_id'] . '\'', array('SUM(goods_number)'), 2);
        ...
        $row['url'] = build_uri('wholesale_purchase', array('gid' => $row['purchase_id'], 'act' => 'info'),
            $row['subject']);
        $arr[] = $row;
    }
...
function get_purchase_info($purchase_id = 0)
{
    ...
    if ($purchase_info) {
        ...
        $purchase_info['user_name'] = get_table_date('users', 'user_id = \'' . $purchase_info['user_id'] . '\'',
            array('user_name'), 2);
...
{
    $data = get_table_date('users_real',
        'user_id=\'' . $user_id . '\' AND user_type=\'' . $user_type . '\' AND review_status=1', array('real_id'), 2);
...
function get_wholesale_product_info($product_id, $filed = '', $goods_model = 0, $is_attr = 0)
{
    ...
    if ($is_attr == 1) {
        if ($return_array['goods_attr']) {
            ...
            $return_array['goods_attr'] = get_wholesale_product_attr_list($goods_attr_id, $return_array['goods_id'],
                $goods_model, $return_array['warehouse_id'], $return_array['area_id']);
        }
    }
...
function get_wholesale_product_attr_list(
    $goods_attr_id = 0,
    $goods_id = 0,
    $goods_model = 0,
    $warehouse_id = 0,
    ...
    foreach ($ru_ids as $key => $val) {
        ...
        foreach ($goods_ids as $a => $g) {
            ...
            $goods_data = get_table_date('wholesale', 'goods_id=\'' . $g . '\'',
                array('act_id', 'goods_id, goods_name, price_model, goods_price', 'moq'));
...
function get_wholesale_child_cat($cat_id = 0, $type = 0)
{
    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        ...
        foreach ($res as $row) {
            ...
            $cat_arr[$row['cat_id']]['url'] = build_uri('wholesale_cat',
                array('act' => 'list', 'cid' => $row['cat_id']), $row['cat_name']);
    ...
    if ($GLOBALS['db']->getOne($sql) || $tree_id == 0) {
        ...
        foreach ($res as $row) {
            ...
            if ($ru_id) {
                ...
                $three_arr[$row['cat_id']]['url'] = build_uri('wholesale_cat',
                    array('act' => 'list', 'cid' => $row['cat_id']), $row['cat_name']);
            }
        ...
        if ($order->type == 'text') {
            ...
            if ($order->type == 'dateTime' || $order->type == 'order_status') {
                if ($order->idTxt == 'wholesale_submitDate') {
                    ...
                    if ($order->idTxt == 'wholesale_status_list') {
                        $date_keyword = $order->date_keyword;
