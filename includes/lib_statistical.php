...
function shop_sale_stats($page = 0)
{
    ...
    if ($result === false) {
        ...
        $filter['start_date'] = empty($_REQUEST['start_date']) ? '' : (0 < strpos($_REQUEST['start_date'],
            '-') ? local_strtotime($_REQUEST['start_date']) : $_REQUEST['start_date']);
        $filter['end_date'] = empty($_REQUEST['end_date']) ? '' : (0 < strpos($_REQUEST['end_date'],
            '-') ? local_strtotime($_REQUEST['end_date']) : $_REQUEST['end_date']);
...
function shop_area_stats($page = 0)
{
    ...
    foreach ($row as $key => $value) {
        ...
        $row[$key]['province_name'] = get_table_date('region', 'region_id=\'' . $province_id . '\'',
            array('region_name'), 2);
    }
...
function get_statistical_user_rank()
{
    ...
    $no_rank_user_num = get_table_date('users', 'user_rank' . db_create_in($rank_list, '', 'NOT'), array('COUNT(*)'),
        2);
    ...
    if ($cat_list) {
        ...
        foreach ($cat_list as $key => $val) {
            ...
            if ($search_data['type'] == 'order_fee') {
                ...
                if ($search_data['type'] == 'order_num') {
                    $series_data[] = intval($data['order_num']);
                } else {
                    if ($search_data['type'] == 'order_goods_num') {
...
function settlement_total_stats()
{
    ...
    $admin_all['is_settlement'] = empty($admin_commission['is_settlement']) ? '0.00' : sprintf('%.2f',
        $admin_commission['is_settlement']);
    $admin_all['no_settlement'] = empty($admin_commission['no_settlement']) ? '0.00' : sprintf('%.2f',
        $admin_commission['is_settlement']);
    $admin_all['valid_fee'] = empty($admin_commission['valid_fee']) ? '0.00' : sprintf('%.2f',
        $admin_commission['valid_fee']);
    $admin_all['refund_fee'] = empty($admin_commission['refund_fee']) ? '0.00' : sprintf('%.2f',
        $admin_commission['refund_fee']);
