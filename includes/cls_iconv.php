...
class Chinese
{
    ...
    public $config = array(
        ...
        'GBtoUTF8_table' => 'gb_utf8.php',
        'BIG5toUTF8_table' => 'big5_utf8.php'
    );
    ...
    public function __construct($dir = './')
    {
        ...
        if ('5.0' <= PHP_VERSION && function_exists('mb_convert_encoding') && function_exists('mb_list_encodings')) {
            ...
            if (in_array('UTF-8', $encodings) == true && in_array('BIG-5', $encodings) == true && in_array('CP936',
                    $encodings) == true) {
    ...
    public function Convert($source_lang, $target_lang, $source_string = '')
    {
        ...
        if (($this->iconv_enabled || $this->mbstring_enabled) && !($this->config['source_lang'] == 'GBK' && $this->config['target_lang'] == 'BIG-5')) {
            if ($this->config['target_lang'] != 'UNICODE') {
                $string = $this->_convert_iconv_mbstring($this->SourceText, $this->config['target_lang'],
                ...
                while ($text) {
                    if (127 < ord(substr($text, 0, 1))) {
                        if ($this->config['source_lang'] != 'UTF-8') {
                            $char = $this->_convert_iconv_mbstring(substr($text, 0, 2), 'UTF-8',
                                $this->config['source_lang']);
                        } else {
