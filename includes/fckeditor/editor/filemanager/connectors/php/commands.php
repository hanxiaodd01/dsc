...
function FileUpload($resourceType, $currentFolder, $sCommand)
{
    ...
    if (isset($_FILES['NewFile']) && !is_null($_FILES['NewFile']['tmp_name'])) {
        ...
        if (isset($Config['HtmlExtensions'])) {
            if (!IsHtmlExtension($sExtension,
                    $Config['HtmlExtensions']) && (($detectHtml = DetectHtml($oFile['tmp_name'])) === true)) {
                $sErrorNumber = '202';
            }
        }
        ...
        if (!$sErrorNumber && IsAllowedExt($sExtension, $resourceType)) {
                ...
                if (is_file($sFilePath)) {
                    ...
                    if (($sExtension == 'jpg') || ($sExtension == 'jpeg') || ($sExtension == 'png') || ($sExtension == 'gif') || ($sExtension == 'bmp')) {
                        ...
                        if ((0 < intval($GLOBALS['_CFG']['watermark_place'])) && !empty($GLOBALS['_CFG']['watermark'])) {
                            $image->add_watermark($sFilePath, '', '../../../../../' . $GLOBALS['_CFG']['watermark'],
            ...
            if (file_exists($sFilePath)) {
                if (isset($isImageValid) && ($isImageValid === -1) && (IsImageValid($sFilePath,
                            $sExtension) === false)) {
                    @unlink($sFilePath);
                    $sErrorNumber = '202';
                } else {
    ...
    if (is_array($_FILES['NewFile']['name'])) {
        foreach ($_FILES['NewFile']['name'] as $key => $value) {
            if (!empty($_FILES['NewFile']['tmp_name'][$key])) {
                ...
                if (isset($Config['HtmlExtensions'])) {
                    if (!IsHtmlExtension($sExtension,
                            $Config['HtmlExtensions']) && (($detectHtml = DetectHtml($_FILES['NewFile']['tmp_name'][$key])) === true)) {
                        $sErrorNumber = '202';
                    }
                }
