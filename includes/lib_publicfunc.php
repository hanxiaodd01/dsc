    ...
    if (is_array($warehouse_area)) {
        for ($i = 0; $i < count($warehouse_area); $i++) {
            if (!empty($warehouse_area[$i])) {
                ...
                if (0 < $id) {
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($table), $parent, 'UPDATE',
                        'goods_id = \'' . $goods_id . '\' and goods_attr_id = \'' . $goods_attr_id . '\' ' . $where);
                } else {
        ...
        if (is_array($goods_attr_id)) {
            for ($i = 0; $i < count($goods_attr_id); $i++) {
                if (!empty($goods_attr_id[$i])) {
                    ...
                    if (0 < $id) {
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($table), $parent, 'UPDATE',
                            'goods_id = \'' . $goods_id . '\' and goods_attr_id = \'' . $goods_attr_id[$i] . ('\' ' . $where));
                    } else {
                        $parent['admin_id'] = $admin_id;
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($table), $parent, 'INSERT');
                    }
...
function set_default_filter($goods_id = 0, $cat_id = 0, $user_id = 0, $cat_type_show = 0, $table = 'category')
{
    ...
    $GLOBALS['smarty']->assign('filter_category_list',
        get_category_list($cat_id, 0, $seller_shop_cat, $user_id, 2, $table));
...
function get_add_edit_goods_cat_list(
    $goods_id = 0,
    $cat_id = 0,
    $table = 'category',
    $sin_prefix = '',
    ...
    if ($_REQUEST['act'] == 'add') {
        ...
        if ($_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
            ...
            for ($i = 0; $i < count($parent_cat_list); $i++) {
                if ($sin_prefix) {
                    $select_category_html .= insert_seller_select_category(pos($parent_cat_list),
                        next($parent_cat_list), $i, 'user_cat', 0, $table, array(), $user_id);
                } else {
                    $select_category_html .= insert_select_category(pos($parent_cat_list), next($parent_cat_list), $i,
                        'cat_id', 0, $table, $seller_shop_cat);
                }
            }
    ...
    if ($attr_arr) {
        ...
        foreach ($attr_arr as $key => $row) {
            if ($row) {
                ...
                $attr_info = get_goods_attr_id($where_select,
                    array('ga.goods_id', 'ga.attr_value', 'a.attr_id', 'a.attr_type'), 2, 1);
    ...
    foreach ($table_list as $key => $table) {
        if ($type) {
            ...
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($table), $other, 'UPDATE',
                'goods_id = 0 AND admin_id = \'' . $admin_id . '\'');
        } else {
...
{
    $diff_data = array();
    $changelog = get_table_date('seller_shopinfo_changelog', 'ru_id=\'' . $ru_id . '\'',
        array('data_key', 'data_value'), 1);
...
function get_goods_product_list(
    $goods_id = 0,
    $model = 0,
    $warehouse_id = 0,
    $area_id = 0,
...
function return_integral_rank($ret_id = 0, $user_id = 0, $order_sn = 0, $rec_id = 0, $refound_pay_points = 0)
{
    ...
    if (!empty($return_integral)) {
        log_account_change($user_id, 0, 0, '-' . $return_integral['rank_integral'], '-' . $gave_custom_points,
            sprintf($GLOBALS['_LANG']['return_order_gift_integral'], $order_sn), ACT_OTHER, 1);
        return null;
    }
}
        ...
        while (!empty($arr)) {
            foreach ($arr as $key => $value) {
                ...
                if ($value['parent_album_id'] == $last_cat_id) {
                    ...
                    if ($last_cat_id < $value['parent_album_id']) {
                        break;
                    }
                }
            }
...
{
    $shipping_conf = get_table_date('kdniao_eorder_config',
        'shipping_id=\'' . $shipping_id . '\' and ru_id=\'' . $ru_id . '\'', array('*'));
...
{
    $data = get_table_date('kdniao_customer_account',
        'shipping_id=\'' . $shipping_id . '\' and ru_id=\'' . $ru_id . '\'', array('*'));
    return $data;
}
...
function get_order_delayed_list($ru_id = 0)
{
    ...
    $arr = array(
        'order_delay_list' => $order_delayed_list,
        'filter' => $filter,
        'page_count' => $filter['page_count'],
        'record_count' => $filter['record_count']
    );
