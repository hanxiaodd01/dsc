...
function edit_profile($profile)
{
    ...
    if (!$GLOBALS['user']->edit_user($cfg)) {
        if ($GLOBALS['user']->error == ERR_EMAIL_EXISTS) {
            ...
            if ($GLOBALS['user']->error == ERR_PHONE_EXISTS) {
    ...
    if (!empty($profile['other'])) {
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $profile['other'], 'UPDATE',
...
function get_user_orders(
    ...
    if ($order == 'auction' || $action == 'auction' || $order == 'auction_order_recycle') {
        $user_order = new Pager($record_count, $pagesize, '', $id, 0, $page, 'user_auction_order_gotoPage', 1, 0, 0,
            $config);
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (0 < $pay_effective_time && $row['pay_status'] == PS_UNPAYED && in_array($row['order_status'],
                array(OS_UNCONFIRMED, OS_CONFIRMED)) && in_array($row['shipping_status'],
                array(SS_UNSHIPPED, SS_PREPARING)) && !in_array($pay_code, array('bank', 'cod', 'post'))) {
            ...
            if ($pay_effective_time < gmtime() - $row['add_time']) {
                ...
                order_action($row['order_sn'], OS_INVALID, SS_UNSHIPPED, PS_UNPAYED,
        ...
        if ($row['order_status'] == OS_RETURNED) {
            ...
            if ($row['order_status'] == OS_ONLY_REFOUND) {
                $row['order_status'] = $GLOBALS['_LANG']['os'][$row['order_status']] . '<br />' . $GLOBALS['_LANG']['ps'][$row['pay_status']];
            } else {
        ...
        if (0 < $sign_time) {
            if ($row['user_pay'] == PS_UNPAYED) {
                $where_log = ' AND ' . db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART),
                        'order_status') . ' AND ' . db_create_in(array(SS_RECEIVED),
                        'shipping_status') . ' AND ' . db_create_in(array(PS_UNPAYED), 'pay_status');
            } else {
                if ($row['user_pay'] == PS_PAYED) {
                    $where_log = ' AND ' . db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART),
                            'order_status') . ' AND ' . db_create_in(array(
                            SS_UNSHIPPED,
                            SS_SHIPPED,
                            SS_RECEIVED,
                            SS_SHIPPED_PART,
                            OS_SHIPPED_PART
                        ), 'shipping_status') . ' AND ' . db_create_in(array(PS_PAYED), 'pay_status');
                }
            }
...
function cancel_order($order_id, $user_id = 0)
{
    ...
    if ($GLOBALS['db']->query($sql)) {
        order_action($order['order_sn'], OS_CANCELED, $order['shipping_status'], PS_UNPAYED,
            $GLOBALS['_LANG']['buyer_cancel'], 'buyer');
            ...
            if ($order['shipping_status'] != SS_SHIPPED) {
                ...
                if ($GLOBALS['db']->query($sql)) {
                    order_action($order['order_sn'], $order['order_status'], SS_RECEIVED, $order['pay_status'], '',
                        $GLOBALS['_LANG']['buyer'], 0, $confirm_take_time);
                    $seller_id = $GLOBALS['db']->getOne('SELECT ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . (' WHERE order_id = \'' . $order_id . '\''),
                        true);
...
function affirm_return_received($order_id, $user_id = 0)
{
    ...
    if (0 < $user_id && $return_order['user_id'] != $user_id) {
        ...
        if ($return_order['return_status'] == 4) {
            ...
            if ($GLOBALS['db']->query($sql)) {
                order_action($order['return_sn'], $order['return_status'], SS_RECEIVED, '', '',
                    $GLOBALS['_LANG']['buyer']);
                return true;
            } else {
                exit($GLOBALS['db']->errorMsg());
...
function save_consignee($consignee, $default = false)
{
    if (0 < $consignee['address_id']) {
        $res = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_address'), $consignee, 'UPDATE',
            'address_id = ' . $consignee['address_id'] . ' AND `user_id`= \'' . $_SESSION['user_id'] . '\'');
    } else {
...
function update_address($address, $default = 0)
{
    ...
    if (0 < $address_id) {
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('user_address'), $address, 'UPDATE',
            'address_id = ' . $address_id . ' AND user_id = ' . $address['user_id']);
    } else {
...
function get_order_detail($order_id, $user_id = 0)
{
    ...
    if (0 < $order['confirm_time'] && ($order['order_status'] == OS_CONFIRMED || $order['order_status'] == OS_SPLITED || $order['order_status'] == OS_SPLITING_PART)) {
        $order['confirm_time'] = sprintf($GLOBALS['_LANG']['confirm_time'],
            local_date($GLOBALS['_CFG']['time_format'], $order['confirm_time']));
    } else {
        $order['confirm_time'] = '';
    }

    if (0 < $order['pay_time'] && $order['pay_status'] != PS_UNPAYED) {
        $order['pay_time'] = sprintf($GLOBALS['_LANG']['pay_time'],
            local_date($GLOBALS['_CFG']['time_format'], $order['pay_time']));
    } else {
        $order['pay_time'] = '';
    }
    ...
    if (!empty($order['confirm_take_time'])) {
        $order['confirm_take_time'] = sprintf($GLOBALS['_LANG']['confirm_time'],
            local_date($GLOBALS['_CFG']['time_format'], $order['confirm_take_time']));
    } else {
...
function save_order_address($address, $user_id)
{
    ...
    if ($row) {
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $address, 'UPDATE',
