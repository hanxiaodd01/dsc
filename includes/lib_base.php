    ...
    if (empty($cer_url)) {
        if (file_exists(ROOT_PATH . 'temp/static_caches/cat_goods_config.php')) {
            ...
            $url_data = array(
                ...
                'qq' => $GLOBALS['_CFG']['qq'],
                'ww' => $GLOBALS['_CFG']['ww'],
                'ym' => $GLOBALS['_CFG']['service_phone'],
                'msn' => $GLOBALS['_CFG']['msn'],
                'email' => $GLOBALS['_CFG']['service_email'],
                'phone' => $GLOBALS['_CFG']['sms_shop_mobile'],
...
function send_mail($name, $email, $subject, $content, $type = 0, $notification = false)
{
    ...
    if ($GLOBALS['_CFG']['mail_service'] == 0 && function_exists('mail')) {
        ...
        $res = @mail($email, '=?' . $charset . '?B?' . base64_encode($subject) . '?=', $content,
            implode("\r\n", $headers));
            ...
            if ($smtp->connect() && $smtp->send($send_params)) {
                ...
                if (empty($err_msg)) {
                    ...
                    if (strpos($err_msg, 'Failed to connect to server') !== false) {
                        $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['smtp_connect_failure'],
                            $params['host'] . ':' . $params['port']));
                    } else {
                        if (strpos($err_msg, 'AUTH command failed') !== false) {
                            $GLOBALS['err']->add($GLOBALS['_LANG']['smtp_login_failure']);
                        } else {
                            if (strpos($err_msg, 'bad sequence of commands') !== false) {
                        ...
                        if (substr($str, 0, 8) == "�PNG\r\n\x1a\n") {
                            ...
                            if (substr($str, 0, 2) == 'BM' && $extname != 'txt') {
                                ...
                                if ((substr($str, 0, 3) == 'CWS' || substr($str, 0, 3) == 'FWS') && $extname != 'txt') {
                                    ...
                                    if (substr($str, 0, 4) == "\xd0\xcf\x11\xe0") {
                                        ...
                                        if (substr($str, 0, 4) == "PK\x03\x04") {
                                            if (substr($str, 512, 4) == "\xec\xa5\xc1\x00" || $extname == 'docx') {
                                                ...
                                                if (substr($str, 512, 2) == "\t\x08" || $extname == 'xlsx') {
                                                    ...
                                                    if (substr($str, 512,
                                                            4) == "\xfd\xff\xff\xff" || $extname == 'pptx') {
                                                        $format = 'pptx';
                                                    } else {
                                                        $format = 'zip';
                                                    }
        ...
        foreach ($get as $key => $row) {
            ...
            if ($row && !is_array($row)) {
                ...
                if (strpos($lower_row, '</script>') !== false) {
                    ...
                    if (strpos($lower_row, 'alert') !== false) {
                        $get[$key] = '';
                    } else {
                        if (strpos($lower_row, 'updatexml') !== false || strpos($lower_row,
                                'extractvalue') !== false || strpos($lower_row, 'floor') !== false) {
                            $get[$key] = '';
                        } else {
                            $get[$key] = $row;
                        }
