...
function insert_query_info()
{
    ...
    return sprintf($GLOBALS['_LANG']['query_info'], $GLOBALS['db']->queryCount, $query_time,
            $online_count) . $gzip_enabled . $memory_usage . $cron_method;
}
...
function insert_history()
{
    ...
    if (!empty($_COOKIE['ECS']['history'])) {
        ...
        while ($row = $GLOBALS['db']->fetch_array($query)) {
            ...
            $goods['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
...
function insert_get_adv_child($arr)
{
    ...
    if ($index_brand == 'index_brand_banner') {
        ...
        if ($index_brand == 'index_group_banner') {
            $val = $GLOBALS['smarty']->fetch('library/index_group_banner.lbi');
        } else {
            if ($index_brand == 'index_banner_group') {
                if (!defined('THEME_EXTENSION')) {
                    $prom_ad = array();
    ...
    if ($top_style_cate_banner == 'top_style_elec_banner') {
        ...
        if ($top_style_cate_banner == 'top_style_food_banner') {
            $val = $GLOBALS['smarty']->fetch('library/cat_top_ad.lbi');
        }
    }
    ...
    if ($top_style_cate_row == 'top_style_elec_row') {
        ...
        if ($top_style_cate_row == 'top_style_food_row') {
            $val = $GLOBALS['smarty']->fetch('library/top_style_food.lbi');
        }
    }
    ...
    if ($brand_list_right_ad == 'brand_list_right_ad') {
        ...
        if ($brand_list_right_ad == 'category_top_banner') {
    ...
    if ($category_all_left == 'category_all_left') {
        ...
        if ($category_all_left == 'category_top_left') {
