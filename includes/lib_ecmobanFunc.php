...
function get_taobao_api($ip = 0)
{
    ...
    if (!is_array($str) || $ip == '127.0.0.1') {
        if (!empty($GLOBALS['_CFG']['shop_city'])) {
            ...
            if (!empty($GLOBALS['_CFG']['shop_province'])) {
...
function get_qq_api($ip = 0)
{
    ...
    if (!empty($str['result']['ad_info']['city'])) {
        ...
        if (!empty($str['result']['ad_info']['province'])) {
...
function get_order_query_sql($type = 'finished', $alias = '')
{
    if ($type == 'finished') {
        return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                OS_CONFIRMED,
                OS_SPLITED
            )) . (' AND ' . $alias . 'shipping_status ') . db_create_in(array(SS_RECEIVED)) . (' AND ' . $alias . 'pay_status ') . db_create_in(array(
                PS_PAYED,
                PS_PAYING
            )) . ' ';
    } else {
                            ...
                            if ($type == 'shipped') {
                                ...
                                if ($type == 'to_confirm') {
                                    return ' AND ' . $alias . 'order_status ' . db_create_in(array(
                                            ...
                                            OS_SPLITED,
                                            OS_SPLITING_PART
                                        )) . ' ' . (' AND ' . $alias . 'shipping_status >= ') . SS_UNSHIPPED . ' AND oi.shipping_status <> ' . SS_RECEIVED . (' AND ' . $alias . 'pay_status ') . db_create_in(array(
                                            PS_PAYED,
                                            PS_PAYING
                                        ));
                                } else {
                        ...
                        if ($category[$i][1]) {
                            ...
                            foreach ($child_tree as $key => $row) {
                                ...
                                if ($ru_id) {
                                    ...
                                    $category[$i]['child_tree'][$key]['url'] = build_uri('category',
        ...
        if (0 < $goods_list[$i]['goods_id']) {
            ...
            if (0 < $res) {
                ...
                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('warehouse_area_goods'), $other, 'UPDATE',
                    ' user_id = \'' . $other['user_id'] . '\' and goods_id = \'' . $other['goods_id'] . '\'' . ' and city_id = \'' . $other['region_id'] . '\'');
            } else {
                $arr['return'] = 0;
                $return = $arr;
...
function get_warehouse_id_attr_number(
    $goods_id = 0,
    $attr_id = '',
    $seller_id = 0,
    $warehouse_id = 0,
    ...
    if (empty($attr_id)) {
        ...
        foreach ($attr_arr as $key => $val) {
            $attr_type = get_goods_attr_id(array('goods_id' => $goods_id, 'goods_attr_id' => $val),
                array('a.attr_type'));
    ...
    foreach ($newInfo as $key => $info) {
        ...
        do {
            ...
            if (isset($row['uc_id']) && !empty($row['uc_id'])) {
                $couponsInfo = get_coupons($row['uc_id'],
                    array('c.cou_id', 'c.cou_man', 'c.cou_type', 'c.ru_id', 'c.cou_money', 'cu.uc_id'), $row['user_id'],
                    $key);
            }
            ...
            if ($virtual_goods && $flow_type != CART_GROUP_BUY_GOODS) {
                if (virtual_goods_ship($virtual_goods, $msg, $order_sn, true)) {
                    ...
                    if ($GLOBALS['db']->getOne($sql) <= 0) {
                        update_order($arr[$key][$j]['order_id'],
...
function get_seller_shipping_order(
    $ru_id = array(),
    $shipping_id = array(),
    $shipping_name = array(),
    $shipping_code = array(),
...
function get_order_goods_toInfo($order_id = 0)
{
    ...
    foreach ($res as $key => $row) {
        ...
        if ($row['og_extension_code'] == 'presale') {
            $arr[$key]['url'] = build_uri('presale', array('act' => 'view', 'presaleid' => $extension_id),
                $row['goods_name']);
        } else {
            if ($row['oi_extension_code'] == 'group_buy') {
                $arr[$key]['url'] = build_uri('group_buy', array('gbid' => $extension_id));
            } else {
                if ($row['oi_extension_code'] == 'snatch') {
                    $arr[$key]['url'] = build_uri('snatch', array('sid' => $extension_id));
                } else {
                    if ($row['oi_extension_code'] == 'seckill') {
                        $arr[$key]['url'] = build_uri('seckill', array('act' => 'view', 'secid' => $extension_id));
                    } else {
                        if ($row['oi_extension_code'] == 'auction') {
                            $arr[$key]['url'] = build_uri('auction', array('auid' => $extension_id));
                        } else {
                            if ($row['oi_extension_code'] == 'exchange_goods') {
...
function get_goods_area_shipping($goods_list, $shipping_id = 0, $shipping_code = '', $consignee = array())
{
    ...
    foreach ($tid_arr2 as $key => $row) {
        ...
        if ($trow && $trow['freight_type'] == 1) {
            ...
            if (!empty($configure)) {
                $tid_arr2[$key]['shipping_fee'] = shipping_fee($shipping_code, $configure, $row['weight'],
        ...
        foreach ($bt_info as $k => $val) {
            if ($bt_info[$k]['is_stages'] == 1) {
                ...
                if ($repay_date <= $over_date) {
                    if ($is_jiesuan) {
                        show_message($GLOBALS['_LANG']['bt_forbid_pay'], $GLOBALS['_LANG']['bt_go_refund'],
                            'user.php?act=baitiao');
                        exit();
                    } else {
                        $result['error'] = 1;
                ...
                if ($val['repay_date'] <= $over_date) {
                    if ($is_jiesuan) {
                        show_message($GLOBALS['_LANG']['bt_overdue'], $GLOBALS['_LANG']['bt_go_refund'],
                            'user.php?act=baitiao');
                        exit();
                    } else {
                        $result['error'] = 1;
...
function get_user_coupons_list(
    $user_id = '',
    $is_use = false,
    $total = '',
    $cart_goods = false,
