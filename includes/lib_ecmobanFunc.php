...
function get_order_query_sql($type = 'finished', $alias = '')
{
	if ($type == 'finished') {
		...
		return ' AND ' . $alias . 'order_status ' . db_create_in(array(OS_CONFIRMED, OS_SPLITED, OS_SPLITING_PART)) . ' ' . (' AND ' . $alias . 'shipping_status >= ') . SS_UNSHIPPED . ' AND oi.shipping_status <> ' . SS_RECEIVED . (' AND ' . $alias . 'pay_status ') . db_create_in(array(PS_PAYED, PS_PAYING));
	}
	else {
