        ...
        if (in_array($filename, array(
            ...
            if (!empty($cat_arr)) {
                ...
                foreach ($cat_arr as $kval => $val) {
                    ...
                    if ($type == 'article_cat') {
                        $ur_here .= '<span class="arrow">></span>' . '<span> <a href="' . build_uri($type, $args,
                                $val['cat_name']) . '">' . htmlspecialchars($val['cat_name']) . '</a>' . '</span>';
                    }
                    ...
                    if ($val['parent_id'] == 0 && !defined('THEME_EXTENSION')) {
                        ...
                        $ur_here .= '<span class="filter-tag"><a href="' . build_uri($type, $args,
                                $val['cat_name']) . '">' . $val['cat_name'] . '</a><i class="sc-icon-right"></i></span>';
                        ...
                        if ($val['cat_tree']) {
                            ...
                            foreach ($val['cat_tree'] as $ckey => $crow) {
                                $ur_here .= '<li><a href="' . build_uri($type, array('cid' => $crow['cat_id']),
                                        $crow['cat_name']) . '" title="' . $crow['cat_name'] . '">' . $crow['cat_name'] . '</a></li>';
                                $cat_arr[$kval]['cat_tree'][$ckey]['url'] = build_uri($type,
                                    array('cid' => $crow['cat_id']), $crow['cat_name']);
                            }
...
function get_shop_help()
{
    ...
    foreach ($res as $key => $row) {
        ...
        $arr[$row['cat_id']]['article'][$key]['short_title'] = 0 < $GLOBALS['_CFG']['article_title_length'] ? sub_str($row['title'],
            $GLOBALS['_CFG']['article_title_length']) : $row['title'];
        $arr[$row['cat_id']]['article'][$key]['url'] = $row['open_type'] != 1 ? build_uri('article',
            array('aid' => $row['article_id']), $row['title']) : trim($row['file_url']);
    }
...
function assign_pager(
    ...
    switch ($app) {
        ...
        case 'exchange':
            $uri_args = array(
                'cid' => $cat,
                'integral_min' => $price_min,
                'integral_max' => $price_max,
                'sort' => $sort,
                'order' => $order,
                'display' => $display_type
            );
    ...
    if ($pager['styleid'] == 0) {
        ...
        if (!empty($url_format)) {
            ...
            $pager['page_first'] = 1 < $page - $_offset && $_pagenum < $page_count ? build_uri($app, $uri_args, '', 1,
                $keywords) : '';
...
function is_spider($record = true)
{
    ...
    foreach ($searchengine_bot as $key => $value) {
        if (strpos($spider, $value) !== false) {
            ...
            if ($record === true) {
                $GLOBALS['db']->autoReplace($GLOBALS['ecs']->table('searchengine'),
                    array('date' => local_date('Y-m-d'), 'searchengine' => $spider, 'count' => 1), array('count' => 1));
            }
...
function get_os()
{
    ...
    if (strpos($agent, 'win') !== false) {
        if (strpos($agent, 'nt 5.1') !== false) {
            ...
            if (strpos($agent, 'nt 5.2') !== false) {
                $os = 'Windows 2003';
            } else {
                if (strpos($agent, 'nt 5.0') !== false) {
                    $os = 'Windows 2000';
                } else {
                    if (strpos($agent, 'nt 6.0') !== false) {
                        $os = 'Windows Vista';
                    } else {
                        if (strpos($agent, 'nt') !== false) {
                            $os = 'Windows NT';
                        } else {
                            if (strpos($agent, 'win 9x') !== false && strpos($agent, '4.90') !== false) {
                                ...
                                if (strpos($agent, '98') !== false) {
                                    ...
                                    if (strpos($agent, '95') !== false) {
                                        ...
                                        if (strpos($agent, '32') !== false) {
                                            $os = 'Windows 32';
                                        } else {
                                            if (strpos($agent, 'ce') !== false) {
                                                $os = 'Windows CE';
                                            }
                                        }
                                    }
                ...
                if (strpos($agent, 'sun') !== false && strpos($agent, 'os') !== false) {
                    ...
                    if (strpos($agent, 'ibm') !== false && strpos($agent, 'os') !== false) {
                        ...
                        if (strpos($agent, 'mac') !== false && strpos($agent, 'pc') !== false) {
                            ...
                            if (strpos($agent, 'powerpc') !== false) {
                                $os = 'PowerPC';
                            } else {
                                if (strpos($agent, 'aix') !== false) {
                                    $os = 'AIX';
                                } else {
                                    if (strpos($agent, 'hpux') !== false) {
                                        $os = 'HPUX';
                                    } else {
                                        if (strpos($agent, 'netbsd') !== false) {
                                            $os = 'NetBSD';
                                        } else {
                                            if (strpos($agent, 'bsd') !== false) {
                                                $os = 'BSD';
                                            } else {
                                                if (strpos($agent, 'osf1') !== false) {
                                                    $os = 'OSF1';
                                                } else {
                                                    if (strpos($agent, 'irix') !== false) {
                                                        $os = 'IRIX';
                                                    } else {
                                                        if (strpos($agent, 'freebsd') !== false) {
                                                            $os = 'FreeBSD';
                                                        } else {
                                                            if (strpos($agent, 'teleport') !== false) {
                                                                $os = 'teleport';
                                                            } else {
                                                                if (strpos($agent, 'flashget') !== false) {
                                                                    $os = 'flashget';
                                                                } else {
                                                                    if (strpos($agent, 'webzip') !== false) {
                                                                        $os = 'webzip';
                                                                    } else {
                                                                        if (strpos($agent, 'offline') !== false) {
                                                                            $os = 'offline';
                                                                        } else {
                                                                            $os = 'Unknown';
                                                                        }
...
function visit_stats()
{
    ...
    setcookie('ECS[visit_times]', $visit_times, $time + 86400 * 365, $GLOBALS['cookie_path'],
    ...
    if (!empty($_SERVER['HTTP_ACCEPT_LANGUAGE'])) {
        ...
        $lang = addslashes($pos !== false ? substr($_SERVER['HTTP_ACCEPT_LANGUAGE'], 0,
            $pos) : $_SERVER['HTTP_ACCEPT_LANGUAGE']);
    } else {
        $lang = '';
    }
...
function save_searchengine_keyword($domain, $path)
{
                ...
                if (strpos($domain, '114.vnet.cn') !== false && preg_match('/kw=([^&]*)/i', $path, $regs)) {
                    ...
                    if (strpos($domain, 'iask.com') !== false && preg_match('/k=([^&]*)/i', $path, $regs)) {
                        ...
                        if (strpos($domain, 'soso.com') !== false && preg_match('/w=([^&]*)/i', $path, $regs)) {
                            ...
                            if (strpos($domain, 'sogou.com') !== false && preg_match('/query=([^&]*)/i', $path,
                                    $regs)) {
                                $searchengine = 'SOGOU';
                                $keywords = urldecode($regs[1]);
                            } else {
                                if (strpos($domain, 'so.163.com') !== false && preg_match('/q=([^&]*)/i', $path,
                                        $regs)) {
                                    $searchengine = 'NETEASE';
                                    $keywords = urldecode($regs[1]);
                                } else {
                                    if (strpos($domain, 'yodao.com') !== false && preg_match('/q=([^&]*)/i', $path,
                                            $regs)) {
                                        $searchengine = 'YODAO';
                                        $keywords = urldecode($regs[1]);
                                    } else {
                                        if (strpos($domain, 'zhongsou.com') !== false && preg_match('/word=([^&]*)/i',
                                                $path, $regs)) {
                                            $searchengine = 'ZHONGSOU';
                                            $keywords = urldecode($regs[1]);
                                        } else {
                                            if (strpos($domain,
                                                    'search.tom.com') !== false && preg_match('/w=([^&]*)/i', $path,
                                                    $regs)) {
                                                $searchengine = 'TOM';
                                                $keywords = urldecode($regs[1]);
                                            } else {
                                                if (strpos($domain, 'live.com') !== false && preg_match('/q=([^&]*)/i',
                                                        $path, $regs)) {
                                                    $searchengine = 'MSLIVE';
                                                    $keywords = urldecode($regs[1]);
                                                } else {
                                                    if (strpos($domain,
                                                            'tw.search.yahoo.com') !== false && preg_match('/p=([^&]*)/i',
                                                            $path, $regs)) {
                                                        $searchengine = 'YAHOO TAIWAN';
                                                        $keywords = urldecode($regs[1]);
                                                    } else {
                                                        if (strpos($domain,
                                                                'cn.yahoo.') !== false && preg_match('/p=([^&]*)/i',
                                                                $path, $regs)) {
                                                            $searchengine = 'YAHOO CHINA';
                                                            $keywords = urldecode($regs[1]);
                                                        } else {
                                                            if (strpos($domain,
                                                                    'yahoo.') !== false && preg_match('/p=([^&]*)/i',
                                                                    $path, $regs)) {
                                                                $searchengine = 'YAHOO';
                                                                $keywords = urldecode($regs[1]);
                                                            } else {
                                                                if (strpos($domain,
                                                                        'msn.com.tw') !== false && preg_match('/q=([^&]*)/i',
                                                                        $path, $regs)) {
                                                                    $searchengine = 'MSN TAIWAN';
                                                                    $keywords = urldecode($regs[1]);
                                                                } else {
                                                                    if (strpos($domain,
                                                                            'msn.com.cn') !== false && preg_match('/q=([^&]*)/i',
                                                                            $path, $regs)) {
                                                                        $searchengine = 'MSN CHINA';
                                                                        $keywords = urldecode($regs[1]);
                                                                    } else {
                                                                        if (strpos($domain,
                                                                                'msn.com') !== false && preg_match('/q=([^&]*)/i',
                                                                                $path, $regs)) {
                                                                            $searchengine = 'MSN';
                                                                            $keywords = urldecode($regs[1]);
                                                                        }
    ...
    if (!empty($keywords)) {
        ...
        $GLOBALS['db']->autoReplace($GLOBALS['ecs']->table('keywords'), array(
...
function upload_file($upload, $type)
{
    if (!empty($upload['tmp_name'])) {
        $ftype = check_file_type($upload['tmp_name'], $upload['name'],
            '|png|jpg|jpeg|gif|doc|xls|txt|zip|ppt|pdf|rar|docx|xlsx|pptx|');
        ...
        foreach ($res as $row) {
            ...
            if ($xiangouInfo['is_xiangou'] == 1 && $start_date < $nowTime && $nowTime < $end_date) {
                ...
                if ($xiangouInfo['xiangou_num'] <= $orderGoods['goods_number']) {
                    ...
                    if ($xiangouInfo['xiangou_num'] <= $cart_number) {
...
function assign_comment($id, $type, $page = 1, $cmtType = 0)
{
    ...
    foreach ($res as $key => $row) {
        ...
        $arr[$row['comment_id']]['buy_goods'] = get_user_buy_goods_order($row['id_value'], $row['user_id'],
            $row['order_id']);
        ...
        if (strpos($arr[$row['comment_id']]['user_picture'],
                'http://') === false && strpos($arr[$row['comment_id']]['user_picture'], 'https://') === false) {
        ...
        $arr[$row['comment_id']]['goods_attr'] = get_table_date('order_goods', 'rec_id=\'' . $row['rec_id'] . '\'',
            array('goods_attr'), 2);
    }
...
function assign_template($ctype = '', $catlist = array(), $merchant_id = 0)
{
    ...
    if (isset($GLOBALS['_CFG']['floor_nav_type'])) {
        if ($GLOBALS['_CFG']['floor_nav_type'] == 1) {
            ...
            if ($GLOBALS['_CFG']['floor_nav_type'] == 2) {
                $smarty->assign('floor_nav_type', 'two');
            } else {
                if ($GLOBALS['_CFG']['floor_nav_type'] == 3) {
                    $smarty->assign('floor_nav_type', 'sthree');
                } else {
                    if ($GLOBALS['_CFG']['floor_nav_type'] == 4) {
    ...
    if (!empty($_GET['u']) && $config['on'] == 1) {
        if (!empty($config['config']['expire'])) {
            if ($config['config']['expire_unit'] == 'hour') {
                ...
                if ($config['config']['expire_unit'] == 'day') {
                    $c = 24;
                } else {
                    if ($config['config']['expire_unit'] == 'week') {
                        $c = 24 * 7;
                    } else {
                        $c = 1;
                    }
            ...
            setcookie('ecshop_affiliate_uid', intval($_GET['u']), gmtime() + 3600 * 24, $GLOBALS['cookie_path'],
                $GLOBALS['cookie_domain']);
        }
    }
}
...
function article_categories_tree($cat_id = 0, $cat_type = 1)
{
    ...
    foreach ($res as $row) {
        ...
        if ($row['child_id'] != null) {
            ...
            $cat_arr[$row['cat_id']]['children'][$row['child_id']]['url'] = build_uri('article_cat',
                array('acid' => $row['child_id']), $row['child_name']);
...
function get_navigator($ctype = '', $catlist = array())
{
    ...
    foreach ($navlist['middle'] as $k => $v) {
        $condition = empty($ctype) ? strpos($v['url'], $cur_url) !== false : strpos($v['url'],
                $cur_url) !== false && strlen($cur_url) == strlen($v['url']);
