...
class ECS
{
    ...
    public function url()
    {
        $curr = strpos(PHP_SELF, ADMIN_PATH . '/') !== false ? preg_replace('/(.*)(' . ADMIN_PATH . ')(\\/?)(.)*/i',
    ...
    public function seller_url($path = '')
    {
        ...
        $curr = strpos(PHP_SELF, $path . '/') !== false ? preg_replace('/(.*)(' . $path . ')(\\/?)(.)*/i', '\\1',
    ...
    {
        $curr = strpos(PHP_SELF, STORES_PATH . '/') !== false ? preg_replace('/(.*)(' . STORES_PATH . ')(\\/?)(.)*/i',
            '\\1', dirname(PHP_SELF)) : dirname(PHP_SELF);
        $root = str_replace('\\', '/', $curr);
