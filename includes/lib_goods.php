...
function get_top10($cats = 0, $presale = '', $ru_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    for ($count = count($arr); $i < $count; $i++) {
        $arr[$i]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($arr[$i]['goods_name'],
        ...
        if (0 < $arr[$i]['promote_price']) {
            $promote_price = bargain_price($arr[$i]['promote_price'], $arr[$i]['promote_start_date'],
...
function get_recommend_goods(
    ...
    if (empty($type_goods[$type])) {
        ...
        if ($rec_type == 0) {
            ...
            if ($rec_type == 1) {
                $type2lib = array(
                    'best' => 'recommend_best_goods',
                    'new' => 'recommend_new_goods',
                    'hot' => 'recommend_hot_goods'
                );
            }
        }
        ...
        foreach ($result as $idx => $row) {
            ...
            $goods[$idx]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
...
function get_goods_info($goods_id, $warehouse_id = 0, $area_id = 0, $area_city = 0, $select = array())
{
    ...
    if ($row !== false) {
        ...
        $row['promote_end_time'] = !empty($row['gmt_end_time']) ? local_date($GLOBALS['_CFG']['time_format'],
            $row['gmt_end_time']) : 0;
        ...
        $row['end_time'] = !empty($manage_info['endtime']) ? local_date($GLOBALS['_CFG']['time_format'],
            $manage_info['endtime']) : '';
        return $row;
    } else {
        return false;
...
', '<br />', $row['attr_value']);
        ...
        if ($row['attr_type'] == 0) {
            ...
            $row['img_flie'] = isset($attr_info['attr_img']) && !empty($attr_info['attr_img']) ? get_image_path($row['attr_id'],
    ...
    if (!empty($attr)) {
        foreach ($attr['lnk'] as $key => $val) {
            ...
            foreach ($res as $row) {
                ...
                $lnk[$key]['goods'][$row['goods_id']]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
                    $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
                $lnk[$key]['goods'][$row['goods_id']]['goods_thumb'] = get_image_path($row['goods_id'],
                ...
                $lnk[$key]['goods'][$row['goods_id']]['url'] = build_uri('goods', array('gid' => $row['goods_id']),
                    $row['goods_name']);
            }
        }
    }
    ...
    foreach ($res as $key => $value) {
        if ($key == 0) {
            ...
            foreach ($goods_res as $idx => $row) {
                ...
                $goods_res[$idx]['shop_price'] = price_format($row['shop_price']);
                $goods_res[$idx]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
                    $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
...
function spec_price($spec, $goods_id = 0, $warehouse_area = array())
{
    if (!empty($spec)) {
        ...
        if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
            ...
            foreach ($spec as $key => $val) {
                $attr_type = get_goods_attr_id(array('goods_attr_id' => $val, 'goods_id' => $goods_id),
                    array('a.attr_type'));
            ...
            if ($attr_type_spec) {
                $attr_type_spec_price = get_goods_attr_price($goods_id, $model_attr, $attr_type_spec, $warehouse_id,
                    $area_id);
            }
...
function group_buy_info($group_buy_id, $current_num = 0, $path = '')
{
    ...
    $group_buy['goods_video_path'] = !empty($group_buy['goods_video']) ? get_image_path($goods_id,
        $group_buy['goods_video']) : '';
...
function group_buy_status($group_buy)
{
    ...
    if ($group_buy['is_finished'] == 0) {
        ...
        if ($group_buy['is_finished'] == GBS_SUCCEED) {
            ...
            if ($group_buy['is_finished'] == GBS_FAIL) {
                $status = GBS_FAIL;
            }
        }
    }
...
function goods_info(
    ...
    if (!empty($row)) {
        ...
        $row['goods_price'] = get_final_price($row['goods_id'], $row['goods_number'], true, $attr_id, $warehouse_id,
            $area_id, $area_city, 0, $presale, $add_tocart);
        $row['goods_weight'] = 0 < intval($row['goods_weight']) ? $row['goods_weight'] . $GLOBALS['_LANG']['kilogram'] : $row['goods_weight'] * 1000 . $GLOBALS['_LANG']['gram'];
...
function get_goods_fittings(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $arr[$temp_index]['parent_short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['parent_name'],
            $GLOBALS['_CFG']['goods_name_length']) : $row['parent_name'];
        ...
        $arr[$temp_index]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
            $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
        ...
        $minMax_price = get_goods_minMax_price($row['goods_id'], $warehouse_id, $area_id, $row['goods_price'],
            $row['market_price']);
        ...
        $arr[$temp_index]['attr_id'] = !empty($row['goods_attr_id']) ? str_replace(',', '|',
            $row['goods_attr_id']) : '';
        $arr[$temp_index]['goods_number'] = get_goods_fittings_gnumber($row['goods_number'], $row['goods_id'],
        ...
        if (empty($row['goods_attr_id'])) {
            ...
            $products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'], $row['goods_name'],
                $warehouse_id, $area_id, $area_city);
        ...
        $arr[$temp_index]['properties'] = get_goods_properties($row['goods_id'], $warehouse_id, $area_id, $area_city, 0,
            $row['goods_attr_id']);
...
function get_goods_fittings_info(
    $goods_id = 0,
    $warehouse_id = 0,
    $area_id = 0,
    $area_city = 0,
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($GLOBALS['_CFG']['add_shop_price'] == 0) {
            ...
            $goods_price = get_final_price($fittings_goods, 1, true, $fittings_attr, $warehouse_id, $area_id,
                $area_city, 0, 0, $add_tocart);
        } else {
        ...
        $minMax_price = get_goods_minMax_price($row['goods_id'], $warehouse_id, $area_id, $goods_price,
            $row['market_price']);
        $arr[$temp_index]['fittings_minPrice'] = $minMax_price['goods_min'];
        $arr[$temp_index]['fittings_maxPrice'] = $minMax_price['goods_max'];
        $arr[$temp_index]['market_minPrice'] = $minMax_price['market_min'];
        ...
        $products = get_warehouse_id_attr_number($goods_id, $row['goods_attr_id'], $row['goods_name'], $warehouse_id,
            $area_id, $area_city);
        $attr_number = $products['product_number'];
        ...
        $attr_number = !empty($attr_number) ? $attr_number : 0;
        $arr[$temp_index]['goods_number'] = $attr_number;
        $arr[$temp_index]['properties'] = get_goods_properties($goods_id, $warehouse_id, $area_id, $area_city,
            $row['goods_attr_id']);
        $temp_index++;
    }
    ...
    if (0 < $cat_id) {
        ...
        foreach ($res as $row) {
            ...
            if ($row['child_id'] != null) {
                ...
                $cat_arr[$row['cat_id']]['children'][$row['child_id']]['url'] = build_uri('category',
                    array('cid' => $row['child_id']), $row['child_name']);
...
function get_cat_id_goods_list($cat_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0, $num = '')
{
    ...
    $sql .= ' AND (c.parent_id =' . $cat_id . ' OR g.cat_id = ' . $cat_id . ' OR g.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id),
            $cat_keys))) . ')';
    ...
    if ($res) {
        foreach ($res as $k => $v) {
            ...
            if ($i) {
                ...
                $res[$k]['list_url'] = build_uri('seckill', array('act' => 'list', 'secid' => $v['id']),
                    $v['goods_name']);
        ...
        if ($soon) {
            foreach ($soon as $k => $v) {
                ...
                if ($i) {
                    ...
                    $soon[$k]['url'] = build_uri('seckill', array('act' => 'view', 'secid' => $v['id']),
                        $v['goods_name']);
                    $res[$k]['list_url'] = build_uri('seckill', array('act' => 'list', 'secid' => $v['id']),
                        $v['goods_name']);
                    $res[$k]['goods_thumb'] = get_image_path($v['id'], $v['goods_thumb'], true);
...
function seckill_goods_results($date = '', $left_join = '', $where = '', $day = 0)
{
    ...
    if ($res) {
        foreach ($res as $k => $v) {
            ...
            if (0 < $day) {
                $res[$k]['url'] = build_uri('seckill', array('act' => 'view', 'secid' => $v['id'], 'tmr' => 1),
                    $v['goods_name']);
            } else {
                $res[$k]['url'] = build_uri('seckill', array('act' => 'view', 'secid' => $v['id']), $v['goods_name']);
            }
