    ...
    if ($GLOBALS['db']->getOne($sql) || $parent_id == 0) {
        ...
        foreach ($res as $row) {
            ...
            foreach ($articles as $key => $val) {
                $articles[$key]['url'] = $val['open_type'] != 1 ? build_uri('article',
                    array('aid' => $val['article_id']), $val['title']) : trim($val['file_url']);
            }

            $cat_keys = get_array_keys_cat($cat_id);
            $sql = 'SELECT b.brand_id, b.brand_name, b.brand_logo, COUNT(*) AS goods_num ' . 'FROM ' . $GLOBALS['ecs']->table('brand') . 'AS b ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON g.brand_id = b.brand_id AND g.is_on_sale = 1 AND g.is_alone_sale = 1 AND g.is_delete = 0 ' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods_cat') . ' AS gc ON g.goods_id = gc.goods_id ' . (' WHERE ' . $children . ' OR ') . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id),
                    $cat_keys))) . ' AND b.is_show = 1 ' . 'GROUP BY b.brand_id HAVING goods_num > 0 ORDER BY b.sort_order, b.brand_id ASC';
            $brands_list = $GLOBALS['db']->getAll($sql);

            foreach ($brands_list as $key => $val) {
                $temp_key = $key;
                $brands_list[$temp_key]['brand_name'] = $val['brand_name'];
                $brands_list[$temp_key]['url'] = build_uri('category',
                    array('cid' => $cat_id, 'bid' => $val['brand_id']), $cat['cat_name']);
...
', '', $row['cat_name'])), ENT_QUOTES);
                    ...
                    if ($table == 'merchants_category') {
                        ...
                        $three_arr[$row['cat_id']]['url'] = build_uri('category', array('cid' => $row['cat_id']),
                            $row['cat_name']);
                    }
                    ...
                    $three_arr[$row['cat_id']]['select'] = str_repeat('&nbsp;',
                        $three_arr[$row['cat_id']]['level'] * 4);
                }

                if (isset($row['cat_id']) != null) {
                    if ($row['parent_id'] != 0) {
                        $three_arr[$row['cat_id']]['cat_id'] = get_child_tree_pro($row['cat_id'], $level + 1, $table,
                            $getrid);
                    } else {
                        $three_arr[$row['cat_id']]['cat_id'] = get_child_tree_pro($row['cat_id'], $level, $table,
                            $getrid);
                    }
                }
...
function get_conments_stars(
    ...
    $arr = array(
        'score' => sprintf('%.2f', round($percentage / 20, 2)),
        'badReview' => round($badReview, 2) * 100,
        'middlReview' => round($middleReview, 2) * 100,
        'goodReview' => round($goodReview, 2) * 100,
        'allReview' => $percentage,
        'badmen' => $badmen,
        'middlemen' => $middlemen,
        'goodmen' => $goodmen,
        'allmen' => $allmen
    ...
    if (100 < $review) {
        ...
        if ($maxReview == $arr['badReview']) {
            ...
            if ($maxReview == $arr['middlReview']) {
                $arr['middlReview'] = $arr['middlReview'] - $review;
            } else {
                if ($maxReview == $arr['goodReview']) {
...
function get_merchants_goods_comment($ru_id)
{
    ...
    if ($seller_cmt) {
        ...
        @$arr['cmt']['commentRank']['zconments'] = get_conments_stars($arr['commentRank']['mc_all'],
            $arr['commentRank']['mc_one'], $arr['commentRank']['mc_two'], $arr['commentRank']['mc_three'],
            $arr['commentRank']['mc_four'], $arr['commentRank']['mc_five'], 'goods');
        @$arr['cmt']['commentServer']['zconments'] = get_conments_stars($arr['commentServer']['mc_all'],
            $arr['commentServer']['mc_one'], $arr['commentServer']['mc_two'], $arr['commentServer']['mc_three'],
            $arr['commentServer']['mc_four'], $arr['commentServer']['mc_five'], 'service');
        @$arr['cmt']['commentDelivery']['zconments'] = get_conments_stars($arr['commentDelivery']['mc_all'],
            $arr['commentDelivery']['mc_one'], $arr['commentDelivery']['mc_two'], $arr['commentDelivery']['mc_three'],
            $arr['commentDelivery']['mc_four'], $arr['commentDelivery']['mc_five'], 'shipping');
        @$arr['cmt']['all_zconments']['score'] = sprintf('%.2f',
            ($arr['cmt']['commentRank']['zconments']['score'] + $arr['cmt']['commentServer']['zconments']['score'] + $arr['cmt']['commentDelivery']['zconments']['score']) / 3);
        @$arr['cmt']['all_zconments']['allReview'] = round(($arr['cmt']['commentRank']['zconments']['allReview'] + $arr['cmt']['commentServer']['zconments']['allReview'] + $arr['cmt']['commentDelivery']['zconments']['allReview']) / 3,
            2);
...
function get_order_pdf_goods($order_id = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        if ($row['is_real'] == 0) {
            ...
            if (file_exists($filename)) {
                ...
                if (!empty($_LANG[$row['extension_code'] . '_link'])) {
                    $row['goods_name'] = $row['goods_name'] . sprintf($_LANG[$row['extension_code'] . '_link'],
                            $row['goods_id'], $order_id);
                }
            }
        }
        ...
        $products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'], $row['ru_id'],
            $row['warehouse_id'], $row['area_id'], $row['model_attr']);
...
function get_merchants_navigator($ru_id = 0, $ctype = '', $catlist = array())
{
    ...
    foreach ($navlist['middle'] as $k => $v) {
        $condition = empty($ctype) ? strpos($cur_url, $v['url']) === 0 : strpos($cur_url,
                $v['url']) === 0 && strlen($cur_url) == strlen($v['url']);
        ...
        if (substr($v['url'], 0, 8) == 'category') {
            ...
            if (substr($v['url'], 0, 15) == 'merchants_store') {
                if ($v['cid']) {
...
function get_user_default($user_id = 0)
{
    ...
    $info['bonus'] = sprintf($GLOBALS['_LANG']['user_bonus_info'], $user_bonus['bonus_count'],
        price_format($user_bonus['bonus_value'], false));
...
function get_cart_goods_combined_freight($goods, $type = 0, $region = '', $ru_id = 0, $shipping_id = 0)
{
    ...
    if ($type == 1) {
        ...
        foreach ($arr as $key => $row) {
            ...
            foreach ($row as $warehouse => $rows) {
                @$arr[$key][$warehouse]['shipping'] = get_goods_freight($rows, $rows['warehouse_id'], $region,
                    $rows['goods_number'], $shipping_code);
            }
        }
        ...
        if ($type == 2) {
            ...
            foreach ($new_arr as $key => $row) {
                ...
                @$new_arr[$key]['shipping'] = get_goods_freight($row, $row['warehouse_id'], $order, $row['number'],
                    $shipping_code);
...
function get_shop_name($ru_id = 0, $type = 0)
{
    ...
    if ($type == 3) {
        ...
        $shop_information['rz_shopName'] = str_replace(array('旗舰店', '专卖店', '专营店'), '',
            $shop_information['rz_shopName']);
    }
    ...
    if ($shopinfo['shopname_audit'] == 1) {
        if ($shopinfo['check_sellername'] == 1) {
            ...
            if ($shopinfo['check_sellername'] == 2) {
        ...
        if ($type == 2) {
            ...
            if ($type == 3) {
                if ($shop_information['shopNameSuffix']) {
                    if (strpos($shop_name,
                            $shop_information['shopNameSuffix']) === false && $shopinfo['check_sellername'] == 1) {
...
function get_user_buy_goods_order($goods_id, $user_id, $order_id)
{
    ...
    $buy_goods['add_time'] = !empty($buy_goods['add_time']) ? local_date($GLOBALS['_CFG']['time_format'],
        $buy_goods['add_time']) : '';
    return $buy_goods;
}
...
function get_goods_attr_order($goods_attr_id)
{
    if ($goods_attr_id) {
        ...
        if (!empty($goods_attr_id)) {
            $fmt = '%s：%s <br/>';
            $sql = 'SELECT ga.goods_attr_id, a.attr_name, ga.attr_value ' . 'FROM ' . $GLOBALS['ecs']->table('goods_attr') . ' AS ga ' . ' left join ' . $GLOBALS['ecs']->table('attribute') . ' AS a ' . 'on a.attr_id = ga.attr_id ' . 'WHERE ' . db_create_in($goods_attr_id,
                    'ga.goods_attr_id') . ' ORDER BY a.sort_order, a.attr_id, ga.goods_attr_id';
            $res = $GLOBALS['db']->query($sql);
...
function get_reply_list($goods_id, $comment_id, $type = 0, $reply_page = 1, $libType = 0, $reply_size = 2)
{
    if ($type == 1) {
        ...
        $reply_comment = new Pager($reply_count, $reply_size, '', $id, 0, $reply_page, 'reply_comment_gotoPage', 1,
            $libType, 1);
        $limit = $reply_comment->limit;
...
function get_history_goods($goods_id = 0, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    if (!empty($_COOKIE['ECS']['history'])) {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            ...
            $arr[$row['goods_id']]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
            ...
            while (!empty($arr)) {
                foreach ($arr as $key => $value) {
                    ...
                    if ($value['parent_id'] == $last_cat_id) {
                        ...
                        if ($last_cat_id < $value['parent_id']) {
                            break;
                        }
                    }
                }
...
function presale_status($presale)
{
    ...
    if ($presale['is_finished'] == 0) {
        ...
        if ($presale['is_finished'] == GBS_SUCCEED) {
            $status = GBS_SUCCEED;
        } else {
            if ($presale['is_finished'] == GBS_FAIL) {
                $status = GBS_FAIL;
            }
        }
    }
...
function get_guess_goods($user_id, $history = 0, $page = 1, $limit = 5, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    if ($query) {
        while ($query && ($row = $GLOBALS['db']->fetch_array($query))) {
            ...
            $guess_goods[$row['goods_id']]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
                $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
            ...
            $guess_goods[$row['goods_id']]['url'] = build_uri('goods', array('gid' => $row['goods_id']),
                $row['goods_name']);
...
function get_self_point($district, $point_id = 0, $limit = 100)
{
    ...
    foreach ($list as $key => $val) {
        ...
        if ($shipping_dateStr) {
            ...
            $list[$key]['shipping_dateStr'] = date('m', strtotime(' +1day')) . '月' . date('d',
                    strtotime(' +1day')) . '日&nbsp;【周' . transition_date(date('Y-m-d', strtotime(' +1day'))) . '】';
        }
    }
...
function sms_ali($params = array(), $send_time = '', $test = array())
{
    ...
    if ($sms_type == 2) {
        ...
        if ($sms_type == 3) {
            $result = array(
                'TemplateContent' => !empty($test['temp_content']) ? $test['temp_content'] : $row['temp_content'],
                'TemplateParam' => json_encode($smsParams),
                'PhoneNumbers' => $params['mobile_phone']
            );
        } else {
...
function get_order_bill_log($other)
{
    ...
    if ($GLOBALS['db']->getOne($sql, true)) {
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_bill_order'), $other, 'UPDATE',
            'bill_id = \'' . $other['bill_id'] . '\' AND order_id = \'' . $other['order_id'] . '\'');
    } else {
    ...
    foreach ($goods_list as $key => $row) {
        $parent_id = $GLOBALS['db']->getOne('SELECT parent_id FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE order_id = \'' . $row['order_id'] . '\'',
            true);
        ...
        if ($GLOBALS['db']->getOne($sql, true)) {
            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_bill_goods'), $goods, 'UPDATE',
                'rec_id = \'' . $row['rec_id'] . '\' AND order_id = \'' . $row['order_id'] . '\'');
        } else {
...
function get_brand_url($brand_id = 0)
{
    ...
    if ($res) {
        ...
        $res['brand_logo'] = empty($res['brand_logo']) ? str_replace(array('../'), '',
            ...
            if ($row['pay_num'] == 0 && 0 < $row['stages_total'] && $row['is_stages'] == 1) {
                for ($i = 1; $i <= $row['stages_total']; $i++) {
                    ...
                    if (!$GLOBALS['db']->getOne($sql, true)) {
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('baitiao_pay_log'), $pay_log_other,
                            'INSERT');
                    }
                }
                ...
                if ($row['stages_total'] == $bt_pay_count) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('baitiao_log'), $baitiao_log_other, 'UPDATE',
                        'log_id = \'' . $row['log_id'] . '\'');
                }
            }
