...
class cls_template
{
                ...
                if ($cache_id && $this->caching) {
                    ...
                    if ($cache_id) {
                        ...
                        if ($GLOBALS['_CFG']['open_memcached'] == 1) {
                            ...
                            if (file_put_contents($hash_dir . '/' . $cachename . '.php', '<?php exit;?>' . $data . $out,
                                    LOCK_EX) === false) {
    ...
    public function smarty_prefilter_preCompile($source)
    {
        ...
        if ($file_type == '.dwt') {
            ...
            $source = preg_replace('/(<link\\shref=["|\'])(?:\\.\\/|\\.\\.\\/)?(css\\/)?([a-z0-9A-Z_]+\\.css["|\']\\srel=["|\']stylesheet["|\']\\stype=["|\']text\\/css["|\'])/i',
                '\\1' . $tmp_dir . '\\2\\3', $source);
            $source = preg_replace('/(<script\\s(?:type|language)=["|\']text\\/javascript["|\']\\ssrc=["|\'])(?:\\.\\/|\\.\\.\\/)?(js\\/[a-z0-9A-Z_\\-\\.]+\\.(?:js|vbs)["|\']><\\/script>)/',
                '\\1' . $tmp_dir . '\\2', $source);
    ...
    public function html_select_date($arr)
    {
        ...
        if ($arr['display_months'] != 'false') {
            ...
            for ($i = 1; $i <= 12; $i++) {
                $out .= $i == $month ? '<option value="' . $i . '" selected>' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>' : '<option value="' . $i . '">' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>';
            }
        }
        ...
        if ($arr['display_days'] != 'false') {
            ...
            for ($i = 1; $i <= 31; $i++) {
                $out .= $i == $day ? '<option value="' . $i . '" selected>' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>' : '<option value="' . $i . '">' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>';
            }
        }
    ...
    public function html_select_time($arr)
    {
        ...
        if (!isset($arr['display_hours'])) {
            ...
            for ($i = 0; $i <= 23; $i++) {
                $out .= $i == $hour ? '<option value="' . $i . '" selected>' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>' : '<option value="' . $i . '">' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>';
            }
        ...
        if (!isset($arr['display_minutes'])) {
            ...
            for ($i = 0; $i <= 59; $i++) {
                $out .= $i == $minute ? '<option value="' . $i . '" selected>' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>' : '<option value="' . $i . '">' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>';
            }
        ...
        if (!isset($arr['display_seconds'])) {
            ...
            for ($i = 0; $i <= 59; $i++) {
                $out .= $i == $second ? '<option value="' . $i . '" selected>' . str_pad($i, 2, '0',
                        STR_PAD_LEFT) . '</option>' : '<option value="' . $i . '">' . $i . '</option>';
            }
