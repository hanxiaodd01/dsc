...
function assign_query_info()
{
    ...
    $GLOBALS['smarty']->assign('query_info',
        sprintf($GLOBALS['_LANG']['query_info'], $GLOBALS['db']->queryCount, $query_time));
    if ($GLOBALS['_LANG']['memory_info'] && function_exists('memory_get_usage')) {
        $GLOBALS['smarty']->assign('memory_info',
            sprintf($GLOBALS['_LANG']['memory_info'], memory_get_usage() / 1048576));
    }
    ...
    if ($row === false) {
        ...
        foreach ($row as $key => $val) {
            if ($val['site_url'] && 8 < strlen($val['site_url'])) {
                ...
                $row[$key]['url'] = build_uri($app, array('cid' => $cat, 'bid' => $val['brand_id']),
                    $val['brand_name']);
            }
    ...
    foreach ($res as $data) {
        switch ($data['act_type']) {
            case GAT_SNATCH:
                ...
                $snatch[$data['act_id']]['time'] = sprintf($GLOBALS['_LANG']['promotion_time'],
                    local_date('Y-m-d', $data['start_time']), local_date('Y-m-d', $data['end_time']));
            ...
            case GAT_GROUP_BUY:
                ...
                $group[$data['act_id']]['time'] = sprintf($GLOBALS['_LANG']['promotion_time'],
                    local_date('Y-m-d', $data['start_time']), local_date('Y-m-d', $data['end_time']));
            ...
            case GAT_AUCTION:
                ...
                $auction[$data['act_id']]['time'] = sprintf($GLOBALS['_LANG']['promotion_time'],
                    local_date('Y-m-d', $data['start_time']), local_date('Y-m-d', $data['end_time']));
...
function order_action(
    ...
    if (is_null($username)) {
        $username = $GLOBALS['db']->getOne('SELECT user_name FROM ' . $GLOBALS['ecs']->table('admin_user') . (' WHERE user_id = \'' . $admin_id . '\''),
            true);
    }
...
function virtual_card_shipping($goods, $order_sn, &$msg, $process = 'other')
{
    ...
    foreach ($arr as $virtual_card) {
        ...
        if ($virtual_card['crc32'] == 0 || $virtual_card['crc32'] == crc32(AUTH_KEY)) {
            ...
            if ($virtual_card['crc32'] == crc32(OLD_AUTH_KEY)) {
    ...
    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('virtual_card') . ' SET ' . 'is_saled = 1 ,' . ('order_sn = \'' . $order_sn . '\' ') . 'WHERE ' . db_create_in($card_ids,
            'card_id');
...
function virtual_card_result($order_sn, $goods)
{
    ...
    while ($row = $GLOBALS['db']->FetchRow($res)) {
        if ($row['crc32'] == 0 || $row['crc32'] == crc32(AUTH_KEY)) {
            ...
            if ($row['crc32'] == crc32(OLD_AUTH_KEY)) {
                    ...
                    if ($order_res && 0 < $order_res['order_id']) {
                        ...
                        if (0 < $main_order_res) {
                            foreach ($main_order_res as $key => $row) {
                                $order_surplus_desc = sprintf($GLOBALS['_LANG']['return_order_surplus'],
                                    $row['order_sn']);
                                $order_integral_desc = sprintf($GLOBALS['_LANG']['return_order_integral'],
                                    $row['order_sn']);
                                $main_change_desc = sprintf($GLOBALS['_LANG']['return_order_surplus'],
                                    $row['order_sn']);
...
{
    if (!empty($image) && (strpos($image, 'http://') === false && strpos($image, 'https://') === false && strpos($image,
                'errorImg.png') === false)) {
        if ($GLOBALS['_CFG']['open_oss'] == 1) {
            $bucket_info = get_bucket_info();
    ...
    if ($return < 4) {
        if (!empty($image) && (strpos($image, 'http://') === false && strpos($image,
                    'https://') === false && strpos($image, 'errorImg.png') === false)) {
            if ($return == 1) {
    ...
    if ($retain) {
        ...
        if ($return == 4) {
            $GLOBALS['_CFG']['no_picture'] = isset($GLOBALS['_CFG']['no_picture']) && !empty($GLOBALS['_CFG']['no_picture']) ? str_replace('../',
                '', $GLOBALS['_CFG']['no_picture']) : '';
        }
...
function get_for_purchasing_goods(
    $start_date = 0,
    $end_date = 0,
    $goods_id = 0,
    $user_id = 0,
...
function cate_history(
    ...
    if (!empty($_COOKIE['ECS']['list_history'])) {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            ...
            $arr[$row['goods_id']]['brand_url'] = build_uri('brand', array('bid' => $row['brand_id']),
                $row['brand_name']);
            ...
            $arr[$row['goods_id']]['zconments'] = get_conments_stars($mc_all, $mc_one, $mc_two, $mc_three, $mc_four,
                $mc_five);
    ...
    if (!empty($_COOKIE['ECS']['sec_history'])) {
        ...
        while ($row = $GLOBALS['db']->fetchRow($res)) {
            ...
            $sec_arr[$row['goods_id']]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
                $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
            ...
            $sec_arr[$row['goods_id']]['url'] = build_uri('seckill', array('act' => 'view', 'secid' => $row['id']),
                $row['goods_name']);
            $sec_arr[$row['goods_id']]['brand_url'] = build_uri('brand', array('bid' => $row['brand_id']),
                $row['brand_name']);
...
function get_store_shop_list(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $arr[$row['shop_id']]['store_shop_url'] = build_uri('merchants_store_shop', array('urid' => $row['user_id']),
            $arr[$row['shop_id']]['shop_name']);

        if ($goods_type == 0) {
            $arr[$row['shop_id']]['goods_count'] = get_shop_goods_count_list($row['user_id'], $warehouse_id, $area_id,
                $area_city);
            $arr[$row['shop_id']]['goods_list'] = get_shop_goods_count_list($row['user_id'], $warehouse_id, $area_id,
                $area_city, 1);
        }
...
function get_store_shop_goods_list($keywords = '', $size, $page, $sort, $order, $warehouse_id, $area_id, $area_city = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        if ($row) {
            ...
            $arr[$row['goods_id']]['goods_name'] = $row['goods_name'];
            $arr[$row['goods_id']]['goods_thumb'] = get_image_path($row['goods_id'], $row['goods_thumb'], true);
            $arr[$row['goods_id']]['goods_url'] = build_uri('goods', array('gid' => $row['goods_id']),
                $row['goods_name']);
            $arr[$row['goods_id']]['is_shipping'] = $row['is_shipping'];
            $sql = 'select * from ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' where ru_id=\'' . $row['user_id'] . '\'';
            $basic_info = $GLOBALS['db']->getRow($sql);
...
function get_shop_goods_count_list(
    $user_id,
    $warehouse_id,
    $area_id,
    $area_city = 0,
    ...
    if ($type == 1) {
        ...
        foreach ($res as $key => $row) {
            ...
            $arr[$key]['shop_url'] = build_uri('merchants_store', array('cid' => 0, 'urid' => $row['user_id']),
                $arr[$key]['shop_name']);
            $cmt_count = $GLOBALS['db']->getOne('SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('comment') . ' where id_value =\'' . $row['goods_id'] . '\' AND status = 1 AND parent_id = 0');
...
function get_shop_goods_cmt_list(
    $user_id,
    $warehouse_id,
    $area_id,
    $area_city,
...
function get_user_bouns_new_list(
    $user_id = 0,
    $page = 1,
    $type = 0,
    $pageFunc = '',
...
function get_cart_info($type = 0)
{
    ...
    if ($type == 1) {
        ...
        if ($type == 2) {
            ...
            $GLOBALS['smarty']->assign('str',
                sprintf($GLOBALS['_LANG']['cart_info'], $number, price_format($amount, false)));
...
', $row['category_links']);
        ...
        foreach ($res as $key2 => $val) {
            ...
            $arr[$row['cat_id']]['child_two'][$key2]['url'] = build_uri('category', array('cid' => $val['cat_id']),
                $val['cat_name']);
        }
    }
...
function insert_select_category(
    $cat_id = 0,
    $child_cat_id = 0,
    $cat_level = 0,
    $select_jsId = 'cat_parent_id',
    $type = 0,
    $table = 'category',
    $seller_shop_cat = array()
) {
...
function insert_seller_select_category(
    $cat_id = 0,
    $child_cat_id = 0,
    $cat_level = 0,
    $select_jsId = 'cat_parent_id',
    ...
    if ($brand_list === false && empty($keyword)) {
        ...
        foreach ($res as $key => $val) {
            ...
            if (0 < $val['is_brand']) {
                ...
                $brand_list[$key]['letter'] = !empty($val['brand_first_char']) ? $val['brand_first_char'] : strtoupper(substr($pin->Pinyin($val['brand_name'],
                    EC_CHARSET), 0, 1));
            } else {
            ...
            foreach ($brand_list as $key => $val) {
                if (!empty($letter) && empty($keyword)) {
                    if ($letter == 'QT' && !$brand_list[$key]['letter']) {
                        ...
                        if ($letter == $brand_list[$key]['letter']) {
                            $arr[$key] = $val;
                        }
                    }
                } else {
...
function get_filter_goods_list(
    $filter = array('goods_ids' => '', 'cat_ids' => '', 'brand_ids' => '', 'user_id' => 0, 'mer_ids' => ''),
    $size = 10,
    $page = 1,
    $sort = 'sort_order',
    $order = 'ASC',
...
function set_prevent_token($cookie = '')
{
    if ($cookie) {
        ...
        setcookie($cookie, $prevent_cookie, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
...
function judge_seller_grade_expiry($ru_id = 0)
{
    if (0 < $ru_id) {
        ...
        $end_time = local_date('Y',
                $seller_grade['add_time']) + $seller_grade['year_num'] . '-' . local_date('m-d H:i:s',
                $seller_grade['add_time']);
