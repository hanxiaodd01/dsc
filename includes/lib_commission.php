...
function get_bill_detail($select = array())
{
    ...
    if ($row) {
        ...
        if ($row['chargeoff_status'] == 1 || $row['chargeoff_status'] == 3) {
            ...
            if (0 < $order_id) {
                $row['settle_accounts'] = get_order_take_brokerage($row['seller_id'], $order_id,
                    $row['chargeoff_time']);
            } else {
                $row['settle_accounts'] = 0;
            }
        } else {
            if ($row['chargeoff_status'] == 2) {
        ...
        if ($value['repair_order'] == 0) {
            if (empty($value['chargeoff_status'])) {
                ...
                if ($detail && $value['end_time'] < $gmtime) {
                    ...
                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('seller_bill_order'), $bill_order_other,
                        'UPDATE', '1 ' . $where_bill_order);
                ...
                if ($value['chargeoff_status'] == 1) {
                    $where_order = ' AND ' . db_create_in(array(OS_UNCONFIRMED),
                            'sbo.shipping_status') . ' AND ' . db_create_in(array(PS_UNPAYED),
                            'sbo.pay_status') . ' AND ' . db_create_in(array(OS_RETURNED), 'sbo.order_status');
...
function get_bill_order_amount(
    $seller_id,
    $start_time,
    $end_time,
    $commission_model,
...
function get_alone_goods_rate($order_id = 0, $ru_id = 0, $order = array())
{
    ...
    if ($goods_list) {
        ...
        foreach ($row['goods'] as $key => $goods) {
            ...
            $actual_return = $GLOBALS['db']->getOne($sql);
            $row[$key]['should_amount'] = number_format(($row[$key]['goods_amount'] - $actual_return) * $goods['commission_rate'],
                2, '.', '');
...
function get_weeks_list($month)
{
    ...
    for ($i = 1; $i < $end_date; $i = $i + 7) {
        ...
        $weekinfo[] = array(
            local_date('Y-m-d', local_strtotime($month . '-' . $i . ' -' . ($w - 1) . ' days')),
            local_date('Y-m-d', local_strtotime($month . '-' . $i . ' +' . (7 - $w) . ' days'))
        );
    }
    ...
    if ($mintime && $maxtime) {
        ...
        if ($min_year < $max_year) {
            ...
            if ($day_number && $server && $server['day_number'] < $day_number) {
                ...
                for ($i = 0; $i <= $number; $i++) {
                    $year_start = local_date('Y-m-d', local_strtotime(local_date('Y-m-d',
                            $bill['min_time'])) + (($i + 1) * ($server['day_number'] - 1) - $server['day_number'] + 1) * 24 * 60 * 60);
                    $year_end = local_date('Y-m-d', local_strtotime(local_date('Y-m-d',
                            $bill['min_time'])) + ($i + 1) * ($server['day_number'] - 1) * 24 * 60 * 60);
