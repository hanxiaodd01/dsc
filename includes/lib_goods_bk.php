...
function get_goods_properties(
    ...
    foreach ($res as $row) {
        ...
        if ($row['attr_type'] == 0) {
            ...
            $row['img_flie'] = !empty($attr_info['attr_img']) ? get_image_path($row['attr_id'], $attr_info['attr_img'],
                true) : '';
...
function auction_info($act_id, $config = false, $path = '')
{
    ...
    if (1 < $auction['status_no']) {
        $sql = 'SELECT COUNT(*)' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE extension_code = \'auction\'' . (' AND extension_id = \'' . $act_id . '\'') . ' AND order_status ' . db_create_in(array(
                OS_CONFIRMED,
                OS_UNCONFIRMED
            ));
...
function get_goods_fittings(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $arr[$temp_index]['properties'] = get_goods_properties($row['goods_id'], $warehouse_id, $area_id,
            $row['goods_attr_id']);
...
function get_goods_fittings_info(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($GLOBALS['_CFG']['add_shop_price'] == 0) {
            ...
            $goods_price = get_final_price($fittings_goods, 1, true, $fittings_attr, $warehouse_id, $area_id, 0, 0,
                $add_tocart);
        } else {
        ...
        $attr_number = !empty($attr_number) ? $attr_number : 0;
        $arr[$temp_index]['goods_number'] = $attr_number;
        $arr[$temp_index]['properties'] = get_goods_properties($goods_id, $warehouse_id, $area_id,
            $row['goods_attr_id']);
        $temp_index++;
    }
