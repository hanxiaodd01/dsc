    ...
    if ($register_mode == 1) {
        if (empty($other['mobile_phone'])) {
            ...
            if (!is_mobile($other['mobile_phone'])) {
                $GLOBALS['err']->add(sprintf($GLOBALS['_LANG']['msg_mobile_invalid'],
                    htmlspecialchars($other['mobile_phone'])));
            }
        }
    }
    ...
    if (!$GLOBALS['user']->add_user($username, $password, $user_registerMode)) {
        ...
        if (!empty($GLOBALS['_CFG']['register_points'])) {
            log_account_change($_SESSION['user_id'], 0, 0, $GLOBALS['_CFG']['register_points'],
                $GLOBALS['_CFG']['register_points'], $GLOBALS['_LANG']['register_points']);
        }
                ...
                if (!empty($affiliate['config']['level_register_all'])) {
                    if (!empty($affiliate['config']['level_register_up'])) {
                        ...
                        if ($rank_points + $affiliate['config']['level_register_all'] <= $affiliate['config']['level_register_up']) {
                            log_account_change($up_uid, 0, 0, $affiliate['config']['level_register_all'], 0,
                                sprintf($GLOBALS['_LANG']['register_affiliate'], $_SESSION['user_id'], $username));
                        }
                    } else {
                        log_account_change($up_uid, 0, 0, $affiliate['config']['level_register_all'], 0,
                            $GLOBALS['_LANG']['register_affiliate']);
                    }
                }
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('users'), $update_data, 'UPDATE',
