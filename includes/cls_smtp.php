...
class smtp
{
    ...
    {
        if (is_resource($this->connection) && $this->send_data('HELO ' . $this->helo) && substr($error = $this->get_data(),
                0, 3) === '250') {
            return true;
        } else {
    ...
    {
        if (is_resource($this->connection) && $this->send_data('EHLO ' . $this->helo) && substr($error = $this->get_data(),
                0, 3) === '250') {
            return true;
        } else {
    ...
    {
        if (is_resource($this->connection) && $this->send_data('AUTH LOGIN') && substr($error = $this->get_data(), 0,
                3) === '334' && $this->send_data(base64_encode($this->user)) && substr($error = $this->get_data(), 0,
                3) === '334' && $this->send_data(base64_encode($this->pass)) && substr($error = $this->get_data(), 0,
                3) === '235') {
            return true;
        } else {
    ...
    {
        if ($this->is_connected() && $this->send_data('MAIL FROM:<' . $from . '>') && substr($this->get_data(), 0,
                2) === '250') {
            return true;
        } else {
            return false;
    ...
    {
        if ($this->is_connected() && $this->send_data('RCPT TO:<' . $to . '>') && substr($error = $this->get_data(), 0,
                2) === '25') {
            return true;
        } else {
