            ...
            if ($_REQUEST['act'] == 'import_taobao') {
                ...
                if ($_REQUEST['act'] == 'act_export_ecshop') {
                    ...
                    if ($result['page'] <= ceil($count / $result['page_size'])) {
                        ...
                        $res = $db->SelectLimit($sql, $result['page_size'],
...
';
                            ...
                            if (!empty($row['original_img']) && is_file(ROOT_PATH . $row['original_img'])) {
                                $zip->add_file(file_get_contents(ROOT_PATH . $row['original_img']),
                                    $row['original_img']);
                            }
                    ...
                    if ($_REQUEST['act'] == 'act_export_step_search') {
                        admin_priv('goods_export');
                        @set_time_limit(300);
                        include_once ROOT_PATH . 'includes/cls_json.php';
                        $json = new JSON();
...
';
                                    if (!empty($row['goods_img']) && is_file(ROOT_PATH . $row['goods_img'])) {
                                        $zip->add_file(file_get_contents(ROOT_PATH . $row['goods_img']),
                                            $row['goods_img']);
                                    }
                                }
                                            ...
                                            if ($_REQUEST['act'] == 'get_goods_list') {
                                                ...
                                                if ($_REQUEST['act'] == 'act_export_taobao V4.6') {
                                                    ...
                                                    $goods_value = array(
                                                        ...
                                                        'new_goods_img' => '',
                                                        'video' => '',
                                                        'marketing_property_mix' => '',
                                                        'user_input_ID_numbers' => '',
                                                        'input_user_name_value' => '',
                                                        'sellers_code' => '',
                                                        'another_of_marketing_property' => '',
                                                        'charge_type' => '0',
                                                        'treasure_number' => '',
                                                        'ID_number' => ''
                                                    );
...
';
                                                    ...
                                                    while ($row = $db->fetchRow($res)) {
                                                        if (!empty($row['goods_img']) && is_file(ROOT_PATH . $row['goods_img'])) {
                                                            $row['new_goods_img'] = preg_replace('/(^images\\/)+(.*)(.gif|.jpg|.jpeg|.png)$/',
                                                                '${2}.tbi', $row['goods_img']);
                                                            @copy(ROOT_PATH . $row['goods_img'],
                                                                ROOT_PATH . 'images\\/' . $row['new_goods_img']);

                                                            if (is_file(ROOT_PATH . 'images\\/' . $row['new_goods_img'])) {
                                                                $zip->add_file(file_get_contents(ROOT_PATH . 'images\\/' . $row['new_goods_img']),
                                                                    $row['new_goods_img']);
                                                        ...
                                                        if (!empty($row['new_goods_img'])) {
                                                            $row['new_goods_img'] = str_ireplace('/', '\\',
                                                                $row['new_goods_img'], $row['new_goods_img']);
                                                            $row['new_goods_img'] = str_ireplace('.tbi', '',
                                                                $row['new_goods_img'], $row['new_goods_img']);
