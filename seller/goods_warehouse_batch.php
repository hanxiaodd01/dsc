        ...
        if ($_FILES['file']['name']) {
            ...
            if (0 < count($data)) {
                ...
                if ($goods_id) {
                    $link[] = array(
                        'href' => 'goods.php?act=edit&goods_id=' . $goods_id . '&extension_code=',
                        'text' => $_LANG['03_goods_edit']
                    );
                    $link[] = array(
                        'href' => 'goods_warehouse_batch.php?act=add&goods_id=' . $goods_id,
                        'text' => $_LANG['back_warehouse_batch_list']
                    );
                } else {
                    $link[] = array(
                        'href' => 'goods_warehouse_batch.php?act=add',
                        'text' => $_LANG['back_warehouse_batch_list']
                    );
                }
            ...
            if (isset($_LANG['upload_warehouse'])) {
                if (($_GET['charset'] == 'zh_cn') || ($_GET['charset'] == 'zh_tw')) {
                    ...
                    if (0 < count($area_info)) {
                        for ($i = 0; $i < count($area_info); $i++) {
                            $data .= join(',', array(
                                    $area_info[$i]['goods_name'],
                                    $area_info[$i]['area_name'],
                                    $area_info[$i]['number'],
                                    $area_info[$i]['minnumber'],
                                    $area_info[$i]['price'],
                                    $area_info[$i]['promote_price']
                                )) . "\t\n";
                        }
                    }
