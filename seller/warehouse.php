...
function get_region_type_area($type = 1)
{
    ...
    foreach ($res as $key => $row) {
        $arr[$key] = $row;
        $region_id = get_table_date('region_warehouse', 'regionId = \'' . $row['region_id'] . '\'', array('region_id'),
            2);
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link2', array(
        'href' => 'warehouse.php?act=ship_list',
        'text' => $_LANG['warehouse_shipping_tpl'],
        'class' => 'icon-edit'
    ));
    ...
    if (0 < $region_id) {
        ...
        $action_link = array(
            ...
            'href' => 'warehouse.php?act=list&pid=' . $parent_id,
            'class' => 'icon-reply'
        );
    } else {
        $action_link = '';
    ...
    if ($_REQUEST['act'] == 'ship_list') {
        admin_priv('warehouse_manage');
        $smarty->assign('action_link', array(
            'text' => $_LANG['09_warehouse_management'],
            'href' => 'warehouse.php?act=list',
            'class' => 'icon-reply'
        ));
        $smarty->assign('primary_cat', $_LANG['11_system']);
        ...
        if ($_REQUEST['act'] == 'remove_tpl') {
            ...
            if ($_REQUEST['act'] == 'multi_remove') {
                ...
                $links[0] = array(
                    'href' => 'warehouse.php?act=tpl_list&shipping_id=' . intval($_REQUEST['shipping_id']),
                    'text' => $_LANG['go_back']
                );
...
if ($_REQUEST['act'] == 'tpl_list') {
    ...
    $smarty->assign('action_link2', array(
        'href' => 'warehouse.php?act=ship_tpl&shipping_id=' . $shipping_id,
        'text' => $_LANG['new_warehouse_shipping_tpl']
    ));
    $smarty->assign('action_link',
        array('href' => 'warehouse.php?act=ship_list', 'text' => $_LANG['return_delivery_list']));
    $smarty->assign('full_page', 1);
    assign_query_info();
    $smarty->assign('current', 'warehouse');
    ...
    if ($_REQUEST['act'] == 'ship_tpl') {
        ...
        if (!empty($row)) {
            ...
            $smarty->assign('shipping_area',
                array('shipping_id' => $_REQUEST['shipping_id'], 'shipping_code' => $shipping['shipping_code']));
        }

        $smarty->assign('action_link', array(
            'href' => 'warehouse.php?act=tpl_list&shipping_id=' . $shipping_id,
            'text' => '返回模板列表',
            'class' => 'icon-reply'
        ));
        ...
        if ($_REQUEST['act'] == 'freight_tpl_insert') {
            ...
            if (empty($tpl_status) || empty($id)) {
                ...
                $db->autoExecute($ecs->table('warehouse_freight_tpl'), $other, 'UPDATE',
                    ' id= ' . $id . ' and user_id = ' . $adminru['ru_id'] . ' and shipping_id= ' . $shipping_id);
                ...
                if ($_REQUEST['act'] == 'edit_area_name') {
                    ...
                    if ($_REQUEST['act'] == 'drop_area') {
                        ...
                        if ($_REQUEST['act'] == 'freight') {
                            ...
                            if ($_REQUEST['act'] == 'freight_insert') {
                                ...
                                if ($id) {
                                    $db->autoExecute($ecs->table('warehouse_freight'), $other, 'UPDATE',
                                        'id=\'' . $id . '\'');
                                ...
                                if ($_REQUEST['act'] == 'get_freight_area') {
                                    check_authz_json('warehouse_manage');
