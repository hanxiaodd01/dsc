...
function get_type_list($ru_id)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $row['effective_date'] = local_date($GLOBALS['_CFG']['time_format'],
                $row['gift_start_date']) . '～' . local_date($GLOBALS['_CFG']['time_format'], $row['gift_end_date']);
        $arr[] = $row;
    }
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['gift_gard_type_add'], 'href' => 'gift_gard.php?act=add', 'class' => 'icon-plus'));
...
if ($_REQUEST['act'] == 'confirm_ship') {
    ...
    if (!empty($id)) {
        if ($db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('user_gift_gard') . (' WHERE status = \'3\' AND gift_gard_id=\'' . $id . '\''))) {
            ...
            if ($db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('user_gift_gard') . (' WHERE status = \'1\' AND gift_gard_id=\'' . $id . '\''))) {
                $sql = 'UPDATE ' . $ecs->table('user_gift_gard') . (' SET status=\'2\' WHERE gift_gard_id=\'' . $id . '\'');
                $db->query($sql);
                $val = 2;
            } else {
                if ($db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('user_gift_gard') . (' WHERE status = \'2\' AND gift_gard_id=\'' . $id . '\''))) {
...
if ($_REQUEST['act'] == 'handle_log') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['take_list'], 'href' => 'gift_gard.php?act=take_list', 'class' => 'icon-reply'));
...
if ($_REQUEST['act'] == 'add') {
    ...
    $smarty->assign('action_link',
        array('href' => 'gift_gard.php?act=list', 'text' => $_LANG['gift_gard_type_list'], 'class' => 'icon-reply'));
...
if ($_REQUEST['act'] == 'gift_add') {
    ...
    $smarty->assign('action_link',
        array('href' => 'gift_gard.php?act=bonus_list&bonus_type=' . $gift_id, 'text' => $_LANG['gift_gard_list']));
    $smarty->assign('action', 'add');
...
if ($_REQUEST['act'] == 'edit') {
    ...
    $smarty->assign('action_link', array(
        'href' => 'gift_gard.php?act=list&' . list_link_postfix(),
        'text' => $_LANG['gift_gard_type_list'],
        'class' => 'icon-reply'
    ));
...
if ($_REQUEST['act'] == 'bonus_list') {
    ...
    $smarty->assign('action_link2', array(
        'href' => 'gift_gard.php?act=gift_add&bonus_type=' . intval($_REQUEST['bonus_type']),
        'text' => $_LANG['gift_gard_add'],
        'class' => 'icon-plus'
    ));
    $smarty->assign('action_link3', array(
        'href' => 'gift_gard.php?act=export_gift_gard&bonus_type=' . intval($_REQUEST['bonus_type']),
        'text' => $_LANG['gift_gard_export'],
...
if ($_REQUEST['act'] == 'take_list') {
    ...
    $smarty->assign('ur_here', $_LANG['gift_gard_list']);
    $smarty->assign('action_link',
        array('href' => 'gift_gard.php?act=take_excel', 'text' => '导出订单', 'class' => 'icon-download-alt'));
    $store_list = get_common_store_list();
    $smarty->assign('store_list', $store_list);
    $list = get_bonus_list($adminru['ru_id']);
    $page_count_arr = seller_page($list, $_REQUEST['page']);
...
if ($_REQUEST['act'] == 'batch') {
    ...
    if (isset($_POST['checkboxes'])) {
        ...
        if (isset($_POST['drop'])) {
            ...
            if (isset($_POST['configure_goods'])) {
                assign_query_info();
                $smarty->assign('primary_cat', $_LANG['02_promotion']);
                $smarty->assign('ur_here', $_LANG['gift_from_goods']);
                $smarty->assign('action_link', array(
                    'href' => 'gift_gard.php?act=bonus_list&bonus_type=' . $bonus_type_id,
                    'text' => $_LANG['gift_gard_list'],
                    'class' => 'icon-reply'
                ));
