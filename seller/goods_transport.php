...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['add_transport'], 'href' => 'goods_transport.php?act=add', 'class' => 'icon-plus'));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            ...
            $smarty->assign('action_link', array(
                'href' => 'goods_transport.php?act=list',
                'text' => $_LANG['goods_transport'],
                'class' => 'icon-reply'
            ));
            assign_query_info();
                        ...
                        if ($_REQUEST['act'] == 'edit_title') {
                            ...
                            if ($_REQUEST['act'] == 'add_area') {
                                ...
                                if ($_REQUEST['act'] == 'drop_area') {
                                    ...
                                    $tid = get_table_date('goods_transport_extend', 'id=\'' . $id . '\'', array('tid'),
                                        2);
                                    $exc_extend->drop($id);
                                    $smarty->assign('transport_area', get_transport_area($tid));
                                    $html = $smarty->fetch('library/goods_transport_area.lbi');
                                    ...
                                    if ($_REQUEST['act'] == 'edit_area_fee') {
                                        ...
                                        if ($_REQUEST['act'] == 'edit_area') {
                                            $id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
                                            $tid = get_table_date('goods_transport_extend', 'id=\'' . $id . '\'',
                                                array('tid'), 2);
                                            $province_selected = get_table_date('goods_transport_extend',
                                                'id=\'' . $id . '\'', array('top_area_id'), 2);
                                            $province_selected = explode(',', $province_selected);
                                            $city_selected = get_table_date('goods_transport_extend',
                                                'id=\'' . $id . '\'', array('area_id'), 2);
                                            ...
                                            foreach ($province as $key => $val) {
                                                $child_num = 0;
                                                $other_num = 0;
                                                $province[$key]['is_selected'] = in_array($val['region_id'],
                                                    $province_selected) ? 1 : 0;
                                                $city = get_regions(2, $val['region_id']);

                                                foreach ($city as $k => $v) {
                                                    $city[$k]['is_selected'] = in_array($v['region_id'],
                                                        $city_selected) ? 1 : 0;
                                                    $city[$k]['is_disabled'] = in_array($v['region_id'],
                                                        $city_disabled) ? 1 : 0;
                                            ...
                                            if ($_REQUEST['act'] == 'save_area') {
                                                ...
                                                $db->autoExecute($ecs->table('goods_transport_extend'), $data, 'UPDATE',
                                                    'id = \'' . $id . '\'');
                                                $smarty->assign('transport_area', get_transport_area($tid));
                                                $html = $smarty->fetch('library/goods_transport_area.lbi');
                                                make_json_result($html);
                                            } else {
                                                if ($_REQUEST['act'] == 'add_express') {
                                                    $data = array();
                                                    $data['tid'] = empty($_REQUEST['tid']) ? 0 : intval($_REQUEST['tid']);
                                                    $data['ru_id'] = $adminru['ru_id'];
                                                    $data['admin_id'] = $admin_id;
                                                    $db->autoExecute($ecs->table('goods_transport_express'), $data,
                                                        'INSERT');
                                                    $smarty->assign('transport_express',
                                                        get_transport_express($data['tid']));
                                                    $html = $smarty->fetch('library/goods_transport_express.lbi');
                                                    make_json_result($html);
                                                } else {
                                                    if ($_REQUEST['act'] == 'drop_express') {
                                                        $id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
                                                        $tid = get_table_date('goods_transport_express',
                                                            'id=\'' . $id . '\'', array('tid'), 2);
                                                        $exc_express->drop($id);
                                                        $smarty->assign('transport_express',
                                                        ...
                                                        if ($_REQUEST['act'] == 'edit_express_fee') {
                                                            $id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
                                                            $shipping_fee = empty($_REQUEST['fee']) ? 0 : (double)$_REQUEST['fee'];

                                                            if ($exc_express->edit('shipping_fee = \'' . $shipping_fee . '\'',
                                                                $id)) {
                                                                clear_cache_files();
                                                                make_json_result($shipping_fee);
                                                            }
                                                        } else {
                                                            if ($_REQUEST['act'] == 'edit_express') {
                                                                $id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
                                                                $tid = get_table_date('goods_transport_express',
                                                                    'id=\'' . $id . '\'', array('tid'), 2);
                                                                $express_selected = get_table_date('goods_transport_express',
                                                                    'id=\'' . $id . '\'', array('shipping_id'), 2);
                                                                ...
                                                                foreach ($shipping_list as $k => $v) {
                                                                    ...
                                                                    $shipping_list[$k]['is_selected'] = in_array($v['shipping_id'],
                                                                        $express_selected) ? 1 : 0;
                                                                    $shipping_list[$k]['is_disabled'] = in_array($v['shipping_id'],
                                                                        $express_disabled) ? 1 : 0;
                                                                }
                                                                ...
                                                                if ($_REQUEST['act'] == 'save_express') {
                                                                    ...
                                                                    if ($_REQUEST['act'] == 'get_shipping_tem') {
                                                                        ...
                                                                        if (!empty($row)) {
                                                                            ...
                                                                            foreach ($fields as $key => $val) {
                                                                                ...
                                                                                if ($val['name'] == 'fee_compute_mode') {
                                                                                    $smarty->assign('fee_compute_mode',
                                                                                        $val['value']);
                                                                                    unset($fields[$key]);
                                                                                } else {
                                                                            ...
                                                                            if (empty($item_fee)) {
                                                                                $field = array(
                                                                                    'name' => 'item_fee',
                                                                                    'value' => '0',
                                                                                    'label' => empty($_LANG['item_fee']) ? '' : $_LANG['item_fee']
                                                                                );
                                                                            ...
                                                                            $smarty->assign('shipping_area', array(
                                                                        ...
                                                                        if ($_REQUEST['act'] == 'the_national') {
                                                                            ...
                                                                            if ($_REQUEST['act'] == 'add_shipping_tpl') {
                                                                                ...
                                                                                $rId = empty($_REQUEST['regions']) ? '' : implode(',',
                                                                                    $_REQUEST['regions']);
                                                                                $tid = empty($_REQUEST['tid']) ? 0 : intval($_REQUEST['tid']);
                                                                                $id = empty($_REQUEST['id']) ? 0 : intval($_REQUEST['id']);
                                                                                $regionId = $rId;
                                                                                ...
                                                                                if ($shipping_id == 0 || empty($regionId)) {
                                                                                    ...
                                                                                    if (0 < $res) {
                                                                                        $db->autoExecute($ecs->table('goods_transport_tpl'),
                                                                                            $other, 'UPDATE',
                                                                                            'id = \'' . $id . '\'');
                                                                                    } else {
                                                                                ...
                                                                                if ($tpl_id && isset($_SESSION[$tid]['tpl_id']) && !empty($_SESSION[$tid]['tpl_id'])) {
                                                                                    $_SESSION[$tid]['tpl_id'] = array_merge($tpl_id,
                                                                                        $_SESSION[$tid]['tpl_id']);
                                                                                } else {
