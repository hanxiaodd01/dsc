...
if ($_REQUEST['act'] == 'list') {
    ...
    if (0 < $adminru['ru_id']) {
        $smarty->assign('action_link', array(
            'text' => $_LANG['seller_upgrade_list'],
            'href' => 'seller_apply.php?act=list&ru_id=' . $adminru['ru_id'],
            'class' => 'icon-book'
        ));
    }
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        $smarty->assign('record_count', $seller_garde['record_count']);
        $smarty->assign('page_count', $seller_garde['page_count']);
        make_json_result($smarty->fetch('merchants_upgrade.dwt'), '',
            array('filter' => $seller_garde['filter'], 'page_count' => $seller_garde['page_count']));
    } else {
        if ($_REQUEST['act'] == 'application_grade' || $_REQUEST['act'] == 'edit') {
            ...
            $smarty->assign('action_link',
                array('text' => $_LANG['09_merchants_upgrade'], 'href' => 'merchants_upgrade.php?act=list'));
                ...
                if (0 < $seller_apply_info['pay_id'] && $seller_apply_info['is_paid'] == 0 && $seller_apply_info['pay_status'] == 0) {
                    ...
                    if ($payment_info === false) {
                        ...
                        if (substr($payment_info['pay_code'], 0, 4) == 'pay_') {
                            ...
                            $apply['log_id'] = get_paylog_id($seller_apply_info['allpy_id'],
                                $pay_type = PAY_APPLYGRADE);
            ...
            if ($seller_grade) {
                $seller_grade['end_time'] = date('Y',
                        $seller_grade['add_time']) + $seller_grade['year_num'] . '-' . date('m-d H:i:s',
            ...
            if ($_REQUEST['act'] == 'insert_submit' || $_REQUEST['act'] == 'update_submit') {
                ...
                if (0 < $pay_id && 0 < $payable_amount) {
                    ...
                    $smarty->assign('action_link',
                        array('text' => $_LANG['application_grade'], 'href' => 'merchants_upgrade.php?act=list'));
                    ...
                    if ($payment_info['pay_code'] == 'balance') {
                        ...
                        if ($payable_amount < $user_money) {
                            ...
                            $db->query($sql);
                            log_account_change($adminru['ru_id'], $payable_amount * -1, 0, 0, 0,
                                $_LANG['record_id'] . $apply_sn . $_LANG['seller_level_apply_pay']);
                        } else {
                                ...
                                if (0 < $seller_apply_info['pay_id'] && 0 < $seller_apply_info['payable_amount']) {
                                    ...
                                    if ($payment_info['pay_code'] == 'balance') {
                                        ...
                                        if ($seller_apply_info['payable_amount'] < $user_money) {
                                            ...
                                            $sql = 'UPDATE ' . $ecs->table('pay_log') . 'SET is_paid = 1 WHERE order_id = \'' . $apply_id . '\' AND order_type = \'' . PAY_APPLYGRADE . '\'';
                                            $db->query($sql);
                                            log_account_change($adminru['ru_id'],
                                                $seller_apply_info['payable_amount'] * -1, 0, 0, 0,
                                                sprintf($_LANG['seller_apply'], $seller_apply_info['apply_sn']));
                                        } else {
                                            $links[] = array(
                                                'text' => $_LANG['return_apply_list'],
                                                'href' => 'merchants_upgrade.php?act=list'
                                            );
                                    ...
                                    $smarty->assign('amount',
                                        price_format($seller_apply_info['payable_amount'], false));
                                    $smarty->assign('order', $apply_info);
                                    $smarty->display('seller_done.dwt');
                                } else {
                                    $links[] = array(
                        ...
                        if ($_REQUEST['act'] == 'suppliers_apply') {
                            ...
                            if ($_REQUEST['act'] == 'supplier_info') {
                                ...
                                if (empty($_FILES['front_of_id_card']['size'])) {
                                    ...
                                    $supplier_info['front_of_id_card'] = $image->upload_image($_FILES['front_of_id_card'],
                                        'idcard');
                                ...
                                if (empty($_FILES['reverse_of_id_card']['size'])) {
                                    ...
                                    $supplier_info['reverse_of_id_card'] = $image->upload_image($_FILES['reverse_of_id_card'],
                                        'idcard');
                                ...
                                if (empty($_FILES['suppliers_logo']['size'])) {
                                    ...
                                    $supplier_info['suppliers_logo'] = $image->upload_image($_FILES['suppliers_logo'],
                                        'idcard');
                                ...
                                if ($count_suppliers && $action == 'update') {
                                    if ($db->autoExecute($ecs->table('suppliers'), $supplier_info, 'UPDATE',
                                        'user_id=\'' . $user_id . '\' ')) {
                                        $link[] = array(
                                            'text' => $_LANG['back'],
                                            'href' => 'merchants_upgrade.php?act=suppliers_apply'
                                        );
