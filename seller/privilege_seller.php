...
if ($_REQUEST['act'] == 'list') {
    ...
    if (0 < $adminru['ru_id']) {
        $smarty->assign('action_link',
            array('href' => 'privilege_seller.php?act=add', 'text' => $_LANG['admin_add'], 'class' => 'icon-plus'));
    }
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $smarty->assign('action_link', array(
                'href' => 'privilege_seller.php?act=list',
                'text' => $_LANG['02_admin_seller'],
                'class' => 'icon-reply'
            ));
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                if ($_REQUEST['act'] == 'edit') {
                    ...
                    if ($_REQUEST['act'] == 'update' || $_REQUEST['act'] == 'update_self') {
                        ...
                        $link[] = array(
                            'text' => strpos($g_link, 'list') ? $_LANG['back_admin_list'] : $_LANG['modif_info'],
                            'href' => $g_link
                        );
                        ...
                        if ($_REQUEST['act'] == 'allot') {
                            ...
                            $smarty->assign('menu_select',
                                array('action' => '10_priv_admin', 'current' => '02_admin_seller'));
                            $priv_str = $db->getOne('SELECT action_list FROM ' . $ecs->table('admin_user') . (' WHERE user_id = \'' . $_GET['id'] . '\''));
                            ...
                            if ($_REQUEST['act'] == 'update_allot') {
                                ...
                                if ($_REQUEST['act'] == 'remove') {
                                    ...
                                    $url = 'privilege_seller.php?act=query&' . str_replace('act=remove', '',
