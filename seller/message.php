...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['send_msg'], 'href' => 'message.php?act=send', 'class' => 'icon-plus'));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'send') {
            ...
            $smarty->assign('action_link',
                array('text' => $_LANG['msg_list'], 'href' => 'message.php?act=list', 'class' => 'icon-reply'));
            ...
            if ($_REQUEST['act'] == 'insert') {
                ...
                if ($_REQUEST['act'] == 'edit') {
                    ...
                    $smarty->assign('action_link',
                        array('href' => 'message.php?act=list', 'text' => $_LANG['msg_list']));
                    ...
                    if ($_REQUEST['act'] == 'update') {
                        ...
                        if ($_REQUEST['act'] == 'view') {
                            ...
                            $smarty->assign('menu_select',
                                array('action' => '10_priv_admin', 'current' => 'admin_message'));
                            $msg_arr = array();
                            ...
                            if ($msg_arr['readed'] == 0) {
                                ...
                                $db->query($sql);
                                $msg_arr['read_time'] = local_date($GLOBALS['_CFG']['time_format'],
                                    $msg_arr['read_time']);
                            }
                                ...
                                if ($_REQUEST['act'] == 're_msg') {
                                    ...
                                    if ($_REQUEST['act'] == 'drop_msg') {
                                        ...
                                        if ($_REQUEST['act'] == 'remove') {
                                            ...
                                            $sql = 'UPDATE ' . $ecs->table('admin_message') . ' SET deleted=1 ' . (' WHERE message_id=' . $id . ' AND (sender_id=\'' . $_SESSION['seller_id'] . '\' OR receiver_id=\'' . $_SESSION['seller_id'] . '\')');
                                            $db->query($sql);
                                            $url = 'message.php?act=query&' . str_replace('act=remove', '',
