...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_category'), $args, 'update',
        'cat_id=\'' . $cat_id . '\'');
}
...
if ($_REQUEST['act'] == 'list') {
    ...
    if (isset($_REQUEST['back_level']) && 0 < $_REQUEST['back_level']) {
        ...
        $_REQUEST['parent_id'] = $db->getOne('SELECT parent_id FROM ' . $ecs->table('merchants_category') . ' WHERE cat_id = \'' . $_REQUEST['parent_id'] . '\'',
            true);
    } else {
    ...
    $smarty->assign('action_link', array(
        'href' => 'category_store.php?act=add&parent_id=' . $_REQUEST['parent_id'],
...
if ($_REQUEST['act'] == 'update') {
    ...
    if ($db->autoExecute($ecs->table('merchants_category'), $cat, 'UPDATE', 'cat_id=\'' . $cat_id . '\'')) {
        ...
        if ($cat['show_in_nav'] != $dat['show_in_nav']) {
            if ($cat['show_in_nav'] == 1) {
                ...
                if (empty($nid)) {
                    $vieworder = $db->getOne('SELECT max(vieworder) FROM ' . $ecs->table('merchants_nav') . ' WHERE type = \'middle\'');
                    $vieworder += 2;
                    $uri = build_uri('merchants_store', array('urid' => $adminru['ru_id'], 'cid' => $cat_id),
                        $cat['cat_name']);
        ...
        $link[] = array(
            ...
            'href' => 'category_store.php?act=list&parent_id=' . $cat['parent_id']
        );
