...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('href' => 'presale.php?act=add', 'text' => $_LANG['add_presale'], 'class' => 'icon-plus'));

    if ($adminru['ru_id'] == 0) {
        $smarty->assign('presale_cat_link',
            array('href' => 'presale_cat.php?act=list', 'text' => $_LANG['presale_cate_list']));
    }
            ...
            if ($_REQUEST['act'] == 'insert_update') {
                            ...
                            while ($row = $db->fetchRow($res)) {
                                ...
                                if ($presale['deposit'] <= $order['surplus'] + $order['money_paid']) {
                                    ...
                                    if (0 < $order['insure_fee']) {
                                        ...
                                        $order['insure_fee'] = shipping_insure_fee($shipping['shipping_code'],
                                            $goods_amount, $shipping['insure']);
                                    }
                                    ...
                                    if (0 < $money) {
                                        ...
                                        order_refund($order, 1,
                                            $_LANG['cancel_order_reason'] . ':' . $order['order_sn']);
                                    }
                        ...
                        if (isset($_POST['fail'])) {
                            ...
                            if (0 < $presale['valid_order']) {
                                ...
                                while ($order = $db->fetchRow($res)) {
                                    ...
                                    if (0 < $money) {
                                        ...
                                        $order['order_amount'] = $money;
                                        order_refund($order, 1,
                                            $_LANG['cancel_order_reason'] . ':' . $order['order_sn'], $money);
                                    }
                            ...
                            if (isset($_POST['mail'])) {
                                ...
                                while ($order = $db->fetchRow($res)) {
                                    ...
                                    $smarty->assign('shop_url',
                                        $ecs->seller_url() . 'user.php?act=order_detail&order_id=' . $order['order_id']);
                                ...
                                $act_name = empty($_POST['act_name']) ? $goods_name : sub_str($_POST['act_name'], 0,
                                    255, false);
                                ...
                                if (0 < $presale_id) {
                                    ...
                                    $db->autoExecute($ecs->table('presale_activity'), $presale, 'UPDATE',
                                        'act_id = \'' . $presale_id . '\'');
                                    admin_log(addslashes($goods_name) . '[' . $presale_id . ']', 'edit', 'presale');
                                    $links = array(
                                        array(
                                            'href' => 'presale.php?act=list&' . list_link_postfix(),
                ...
                if ($_REQUEST['act'] == 'batch_drop') {
                    ...
                    if ($_REQUEST['act'] == 'search_goods') {
                        ...
                        $default_arr = array(
                            'goods_id' => 0,
                            'goods_name' => $_LANG['search_goods_creat_option_list'],
                            'shop_price' => 0
                        );
                        $arr[] = $default_arr;
                        ...
                        if ($_REQUEST['act'] == 'get_price') {
                            ...
                            if ($_REQUEST['act'] == 'edit_deposit') {
                                ...
                                if ($_REQUEST['act'] == 'remove') {
                                    ...
                                    if ($presale['ru_id'] != $adminru['ru_id']) {
                                        $url = 'presale.php?act=query&' . str_replace('act=remove', '',
                                    ...
                                    $db->query($sql);
                                    admin_log(addslashes($presale['goods_name']) . '[' . $id . ']', 'remove',
                                        'presale');
                                    clear_cache_files();
                                    $url = 'presale.php?act=query&' . str_replace('act=remove', '',
                                            $_SERVER['QUERY_STRING']);
