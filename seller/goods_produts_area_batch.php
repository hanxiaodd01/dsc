...
if ($_REQUEST['act'] == 'add') {
    ...
    if (0 < $goods_id) {
        $smarty->assign('action_link', array(
            'text' => $_LANG['back_to_goods_detail'],
        ...
        if ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (isset($_SESSION['goods_list']) && $_SESSION['goods_list']) {
                ...
                if (!empty($result['list']['goods_attr'])) {
                    ...
                    if ($product_id) {
                        $db->autoExecute($ecs->table('products_area'), $other, 'UPDATE',
                        ...
                        if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
                            ...
                            $db->autoExecute($ecs->table('goods'), $goods_other, 'UPDATE',
                                'goods_id = \'' . $result['list']['goods_id'] . ('\' AND product_id = \'' . $product_id . '\' AND product_table = \'products_area\''));
                        }
                ...
                if (isset($_LANG['upload_product'])) {
                    if ($_GET['charset'] == 'zh_cn' || $_GET['charset'] == 'zh_tw') {
                        ...
                        $region_name = get_table_date('region_warehouse', 'region_id = \'' . $area_id . '\'',
                            array('region_name'), 2);
                        $city_name = get_table_date('region_warehouse', 'region_id = \'' . $city_id . '\'',
                            array('region_name'), 2);
                        $attr_info = get_list_download($goods_info['goods_sn'], array($region_name, $city_name),
                            $_attribute, count($_attribute), $model);
