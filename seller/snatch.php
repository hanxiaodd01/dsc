...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['snatch_add'], 'href' => 'snatch.php?act=add', 'class' => 'icon-plus'));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $snatch = array(
                ...
                'end_time' => $end_time,
                'option' => '<li><a href="javascript:;" data-value="0" class="ftx-01">' . $_LANG['make_option'] . '</a></li>'
            );
            $smarty->assign('snatch', $snatch);
            $smarty->assign('ur_here', $_LANG['snatch_add']);
            $smarty->assign('action_link',
                array('text' => $_LANG['02_snatch_list'], 'href' => 'snatch.php?act=list', 'class' => 'icon-reply'));
                    ...
                    if ($_REQUEST['act'] == 'edit_snatch_name') {
                        ...
                        if ($_REQUEST['act'] == 'remove') {
                            ...
                            if ($snatch['user_id'] != $adminru['ru_id']) {
                                $url = 'snatch.php?act=query&' . str_replace('act=remove', '',
                            ...
                            if ($_REQUEST['act'] == 'edit') {
                                ...
                                $smarty->assign('menu_select',
                                    array('action' => '02_promotion', 'current' => '02_snatch_list'));
                                $smarty->assign('primary_cat', $_LANG['02_promotion']);
                                ...
                                $smarty->assign('action_link', array(
                                    'text' => $_LANG['02_snatch_list'],
                                    'href' => 'snatch.php?act=list&' . list_link_postfix(),
                                    'class' => 'icon-reply'
                                ));
                                ...
                                if ($_REQUEST['act'] == 'update') {
                                    ...
                                    $db->autoExecute($ecs->table('goods_activity'), $record, 'UPDATE',
                                        'act_id = \'' . $act_id . '\' AND act_type = ' . GAT_SNATCH);
                                    ...
                                    if ($_REQUEST['act'] == 'view') {
                                        ...
                                        if ($_REQUEST['act'] == 'query_bid') {
                                            ...
                                            $sort_flag = sort_flag($bid_list['filter']);
                                            $smarty->assign($sort_flag['tag'], $sort_flag['img']);
                                            make_json_result($smarty->fetch('snatch_view.htm'), '', array(
                                                'filter' => $bid_list['filter'],
                                                'page_count' => $bid_list['page_count']
                                            ));
                                        } else {
