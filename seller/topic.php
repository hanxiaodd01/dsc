...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['topic_add'], 'href' => 'topic.php?act=add', 'class' => 'icon-plus'));
...
');
            ...
            if ($_REQUEST['act'] == 'delete') {
                admin_priv('topic_manage');
                get_del_batch($_POST['checkboxes'], intval($_GET['id']), array('topic_img', 'title_pic'), 'topic_id',
                    'topic', 1);
...
');
                ...
                if ($_REQUEST['act'] == 'query') {
                    ...
                    make_json_result($smarty->fetch($tpl), '',
                        array('filter' => $topic_list['filter'], 'page_count' => $topic_list['page_count']));
                } else {
