            ...
            if (empty($order)) {
                ...
                if ($shipping['print_model'] == 2) {
                    ...
                    if (!empty($shipping['shipping_print'])) {
                        ...
                        $create_html = create_html($shipping['shipping_print'], $adminru['ru_id'], $simulation_print,
                            '', 2);
                        ...
                        if (!empty($_LANG['shipping_print'])) {
                            $data['error'] = 0;
                            $simulation_print = 'simulation_print.html';
                            $create_html = create_html($_LANG['shipping_print'], $adminru['ru_id'], $simulation_print,
                                '', 2);
                            $dir = ROOT_PATH . 'data/' . $simulation_print;
                            $data['content'] = $GLOBALS['smarty']->fetch($dir);
                            exit(json_encode($data));
