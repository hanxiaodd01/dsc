...
function get_sales($day_num)
{
    ...
    if ($day_num == 1) {
        $date_start = local_mktime(0, 0, 0, local_date('m', gmtime()), local_date('d', gmtime()),
            local_date('Y', gmtime()));
        $date_end = local_mktime(23, 59, 59, local_date('m', gmtime()), local_date('d', gmtime()),
            local_date('Y', gmtime()));
    } else {
        if ($day_num == 2) {
...
{
    $where = ' AND oi.order_status ' . db_create_in(array(
            OS_CONFIRMED,
            OS_SPLITED
        )) . '  AND oi.shipping_status = \'' . SS_RECEIVED . '\' AND oi.pay_status ' . db_create_in(array(
            PS_PAYED,
            PS_PAYING
        ));
...
function surplus_time($ru_id)
{
    if ($_SESSION['verify_time']) {
        ...
        if ($enter_overtime <= $time) {
            ...
            if ($enter_overtime < $one_month_later) {
                ...
                if ($enter_overtime < $two_month_later) {
                    $link[] = array('text' => $GLOBALS['_LANG']['back_list'], 'href' => 'index.php');
...
if ($_REQUEST['act'] == '') {
    ...
    $smarty->assign('new_repay',
        $db->getOne('SELECT COUNT(*) FROM ' . $ecs->table('user_account') . ' WHERE process_type = ' . SURPLUS_RETURN . ' AND is_paid = 0 '));
    ...
    if ($_REQUEST['act'] == 'merchants_first') {
        ...
        if ($_REQUEST['act'] == 'merchants_second') {
                ...
                if (isset($file['error']) && $file['error'] == 0 || !isset($file['error']) && $file['tmp_name'] != 'none') {
                    if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            ...
                            sys_msg(sprintf($_LANG['msg_upload_failed'], $file['name'],
                ...
                if (isset($file['error']) && $file['error'] == 0 || !isset($file['error']) && $file['tmp_name'] != 'none') {
                    if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            include_once ROOT_PATH . '/includes/cls_image.php';
                            $image = new cls_image($_CFG['bgcolor']);
                            $goods_thumb = $image->make_thumb($file_name, 120, 120,
            ...
            if (0 < $domain_id) {
                $db->autoExecute($ecs->table('seller_domain'), $seller_domain, 'UPDATE',
                    'ru_id=\'' . $adminru['ru_id'] . '\'');
            } else {
                ...
                if (isset($file['error']) && $file['error'] == 0 || !isset($file['error']) && $file['tmp_name'] != 'none') {
                    if (!check_file_type($file['tmp_name'], $file['name'], $allow_file_types)) {
                        ...
                        if (move_upload_file($file['tmp_name'], $file_name)) {
                            include_once ROOT_PATH . '/includes/cls_image.php';
                            $image = new cls_image($_CFG['bgcolor']);
                            $qrcode_thumb = $image->make_thumb($file_name, 120, 120,
                            ...
                            if (!empty($qrcode_thumb)) {
                                ...
                                if (0 < $qrcode_count) {
                                    if (!empty($qrcode_thumb)) {
                                        ...
                                        $db->autoExecute($ecs->table('seller_qrcode'), $qrcode_other, 'UPDATE',
                                            'ru_id = \'' . $adminru['ru_id'] . '\'');
                                    }
                                } else {
                                    $qrcode_other = array(
            ...
            $db->autoExecute($ecs->table('admin_user'), $admin_user, 'UPDATE',
            ...
            if ($data_op == 'add') {
                ...
                if (!$store) {
                    ...
                    if (!empty($diff_data)) {
                        foreach ($diff_data as $key => $val) {
                            ...
                            if ($db->getOne($sql)) {
                                $GLOBALS['db']->autoExecute($ecs->table('seller_shopinfo_changelog'), $changelog,
                                    'update', 'ru_id=\'' . $adminru['ru_id'] . ('\' AND data_key = \'' . $key . '\''));
                            } else {
                ...
                if (!empty($diff_data)) {
                    ...
                    $db->autoExecute($ecs->table('seller_shopinfo'), $review_status, 'UPDATE',
                        'ru_id=\'' . $adminru['ru_id'] . '\'');
                    ...
                    foreach ($diff_data as $key => $val) {
                        ...
                        if ($db->getOne($sql)) {
                            $GLOBALS['db']->autoExecute($ecs->table('seller_shopinfo_changelog'), $changelog, 'update',
                                'ru_id=\'' . $adminru['ru_id'] . ('\' AND data_key = \'' . $key . '\''));
                        } else {
                            $db->autoExecute($ecs->table('seller_shopinfo_changelog'), $changelog, 'INSERT');
                        }
                    }
                        ...
                        if ($_REQUEST['act'] == 'check_order') {
                            ...
                            if ($_REQUEST['act'] == 'check_bill') {
                                ...
                                if ($cfg_checkbill === false) {
                                    write_static_cache('checkbill_number_' . $adminru['ru_id'], $checkbil_array,
                                        '/data/sc_file/seller_bill/');
                                } else {
                                    ...
                                    if ($cfg_checkbill[$day_time][$adminru['ru_id']]['checkbill_number'] < $checkbill_number) {
                                        ...
                                        write_static_cache('checkbill_number_' . $adminru['ru_id'], $cfg_checkbill,
                                            '/data/sc_file/seller_bill/');
                                    }
                                }
                                                ...
                                                if ($row['cycle'] == 5) {
                                                    ...
                                                    if ($row['cycle'] == 4) {
                                                        ...
                                                        if ($row['cycle'] == 3) {
                                                            ...
                                                            if (empty($day_array)) {
                                                                $nowMonth = $nowMonth - 1;
                                                                $days = cal_days_in_month(CAL_GREGORIAN, $nowMonth,
                                                                    $nowYear);
                                ...
                                if ($_REQUEST['act'] == 'out_check_bill') {
                                    $result = array();
                                    $bill_list = commission_bill_list(1);
                                    make_json_result('', '', $result);
                                } else {
                                    if ($_REQUEST['act'] == 'change_user_menu') {
                                        $adminru = get_admin_ru_id();
                                        ...
                                        if (!$change) {
                                            $user_menu[] = $action;
                                            $sql = ' UPDATE ' . $GLOBALS['ecs']->table('seller_shopinfo') . ' set user_menu = \'' . implode(',',
                                                    $user_menu) . '\' WHERE ru_id = \'' . $adminru['ru_id'] . '\' ';
                                        ...
                                        if ($_REQUEST['act'] == 'clear_cache') {
                                            ...
                                            if ($_REQUEST['act'] == 'tengxun_coordinate') {
                                                $result = array('error' => 0, 'message' => '', 'content' => '');
                                                ...
                                                $region = get_seller_region(array(
                                                    'province' => $province,
                                                    'city' => $city,
                                                    'district' => $district
                                                ));
