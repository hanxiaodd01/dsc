...
if ($_REQUEST['act'] == 'add') {
    ...
    if (admin_priv('goods_export')) {
        $tab_menu[] = array(
            'curr' => 0,
            'text' => $_LANG['14_goods_export'],
            'href' => 'goods_export.php?act=goods_export'
        );
    }
    ...
    if ($_REQUEST['act'] == 'upload') {
        ...
        if ($_REQUEST['act'] == 'ajax_insert_data') {
            admin_priv('goods_batch');
            ...
            if (isset($_POST['checked']) && !empty($guid)) {
                        ...
                        if (!empty($field_value)) {
                            if (in_array($field, array('goods_number', 'warn_number', 'integral'))) {
                                ...
                                if ($field == 'brand_name') {
                                    ...
                                    if (in_array($field, array('goods_weight', 'market_price', 'shop_price'))) {
                ...
                write_static_cache('goods_batch-' . $guid, $goods_list,
                    DATA_DIR . '/sc_file/goods_batch/' . $adminru['ru_id'] . '/');
            }
            ...
            if ($_REQUEST['act'] == 'goods_batch_separate') {
                ...
                $guid = isset($_POST['guid']) && !empty($_POST['guid']) ? trim($_POST['guid']) : '';
                $goods_list = read_static_cache('goods_batch-' . $guid,
                    DATA_DIR . '/sc_file/goods_batch/' . $adminru['ru_id'] . '/');
                            ...
                            if (!empty($goods['original_img']) || !empty($goods['goods_img']) || !empty($goods['goods_thumb'])) {
                                ...
                                if (!empty($goods['original_img'])) {
                                    if ($_CFG['auto_generate_gallery']) {
                                        ...
                                        $goods_gallery['img_original'] = reformat_image_name('gallery', $goods_id, $img,
                                            'source');
                                    }

                                    if ($_CFG['retain_original_img']) {
                                        $original_img = reformat_image_name('goods', $goods_id, $goods['original_img'],
                                            'source');
                                    } else {
                                ...
                                if (!empty($goods['goods_img'])) {
                                    if ($_CFG['auto_generate_gallery'] && !empty($gallery_img)) {
                                        $goods_gallery['img_url'] = reformat_image_name('gallery', $goods_id,
                                            $gallery_img, 'goods');
                                    }
                                ...
                                if (!empty($goods['goods_thumb'])) {
                                    if ($_CFG['auto_generate_gallery']) {
                                        ...
                                        $goods_gallery['thumb_url'] = reformat_image_name('gallery_thumb', $goods_id,
                                            $gallery_thumb, 'thumb');
                                    }

                                    $goods_thumb = reformat_image_name('goods_thumb', $goods_id, $goods['goods_thumb'],
                                        'thumb');
                                }
                                ...
                                $db->autoExecute($ecs->table('goods'), $goodsOther, 'UPDATE',
                                    'goods_id = \'' . $goods_id . '\'');
                ...
                if (0 < $goods_id) {
                    ...
                    admin_log('[' . $GLOBALS['_LANG']['goods_id'] . ':' . $goods_id . ']' . $goods['goods_name'],
                        'batch_upload', 'goods');
                } else {
                ...
                if ($_REQUEST['act'] == 'select') {
                    ...
                    if (admin_priv('goods_batch')) {
                        $tab_menu[] = array(
                            'curr' => 1,
                            'text' => $_LANG['15_batch_edit'],
                            'href' => 'goods_batch.php?act=select'
                        );
                        $tab_menu[] = array(
                            'curr' => 0,
                            'text' => $_LANG['13_batch_add'],
                            'href' => 'goods_batch.php?act=add'
                        );
                    }
                    ...
                    if ($_REQUEST['act'] == 'edit') {
                        ...
                        $smarty->assign('menu_select',
                            array('action' => '02_cat_and_goods', 'current' => '15_batch_edit'));
                                ...
                                if (!empty($_POST['goods_id'])) {
                                    foreach ($_POST['goods_id'] as $goods_id) {
                                        ...
                                        $goods_info = get_table_date('goods', 'goods_id=\'' . $goods_id . '\'',
                                            array('shop_price', 'promote_price'));
                                        ...
                                        $db->autoExecute($ecs->table('goods'), $goods, 'UPDATE',
                                            'goods_id = \'' . $goods_id . '\'');
                                            ...
                                            foreach ($_POST['rank_id'] as $rank_id) {
                                                ...
                                                if (0 < $db->getOne($sql)) {
                                                    if ($rank['user_price'] < 0) {
                                                        ...
                                                        $db->autoExecute($ecs->table('member_price'), $rank, 'UPDATE',
                                                            'goods_id = \'' . $goods_id . '\' AND user_rank = \'' . $rank_id . '\'');
                                                    }
                                                } else {
                                                    if (0 <= $rank['user_price']) {
                                                ...
                                                if (trim($_POST['member_price'][$rank_id]) != '') {
                                                    ...
                                                    if (0 < $db->getOne($sql)) {
                                                        if ($rank['user_price'] < 0) {
                                                            ...
                                                            $db->autoExecute($ecs->table('member_price'), $rank,
                                                                'UPDATE',
                                                                'goods_id = \'' . $goods_id . '\' AND user_rank = \'' . $rank_id . '\'');
                                                        }
                                                    } else {
