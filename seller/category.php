...
function cat_update($cat_id, $args)
{
    ...
    return $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('category'), $args, 'update',
        'cat_id=\'' . $cat_id . '\'');
}
    ...
    if ($res) {
        foreach ($res as $k => $row) {
            ...
            if ($extension_goods_array) {
                $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE is_delete = 0 AND user_id = \'' . $adminru['ru_id'] . '\'' . ' AND review_status >= 3 AND is_real = 1 ' . ' AND (cat_id ' . db_create_in($cat_keys) . ' OR goods_id ' . db_create_in($cat_keys) . ')';
            } else {
                $sql = 'SELECT count(*) FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE is_delete = 0 AND user_id = \'' . $adminru['ru_id'] . '\'' . ' AND review_status >= 3 AND is_real = 1 ' . ' AND cat_id ' . db_create_in($cat_keys);
            }
...
if ($_REQUEST['act'] == 'list') {
    ...
    $tab_menu[] = array(
        'curr' => 0,
        'text' => $_LANG['03_store_category_list'],
        'href' => 'category_store.php?act=list'
    );
    ...
    if (isset($_REQUEST['back_level']) && 0 < $_REQUEST['back_level']) {
        ...
        $parent_id = $db->getOne('SELECT parent_id FROM ' . $ecs->table('category') . (' WHERE cat_id = \'' . $parent_id . '\''),
            true);
    } else {
    ...
    if ($_REQUEST['act'] == 'ajax_cache_list') {
...
if ($_REQUEST['act'] == 'edit') {
    ...
    if ($_REQUEST['act'] == 'titleFileView') {
        ...
        $smarty->assign('action_link',
            array('href' => 'category.php?act=edit&cat_id=' . $cat_id, 'text' => $_LANG['go_back']));
        assign_query_info();
        ...
        if ($_REQUEST['act'] == 'title_update') {
            ...
            if ($_REQUEST['act'] == 'add_category') {
                ...
                if (cat_exists($category, $parent_id)) {
                    ...
                    for ($i = 0; $i < count($parent_cat_list); $i++) {
                        $select_category_html .= insert_select_category(pos($parent_cat_list), next($parent_cat_list),
                            $i, 'cat_id');
                    }
