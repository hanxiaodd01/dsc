...
function dialog_list($id, $val = 0)
{
    ...
    foreach ($res as $k => $v) {
        ...
        $res[$k]['end_time'] = empty($v['end_time']) ? $GLOBALS['_LANG']['have_not_end'] : date('Y-m-d H:i:s',
            $v['end_time']);
    }
...
if ($_REQUEST['act'] == 'list') {
    ...
    if (0 < $adminru['ru_id']) {
        $smarty->assign('action_link',
            array('href' => 'privilege_kefu.php?act=add', 'text' => $_LANG['add_kefu'], 'class' => 'icon-plus'));
    }
                    ...
                    if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                        ...
                        if ($_REQUEST['act'] == 'dialog_list') {
                            ...
                            $smarty->assign('menu_select',
                                array('action' => '10_priv_admin', 'current' => 'kefu_list'));
                            $smarty->assign('action_link', array(
                                'href' => 'privilege_kefu.php?act=list',
                                'text' => $_LANG['kefu_list'],
                                'class' => 'icon-reply'
                            ));
                            assign_query_info();
                            ...
                            if ($_REQUEST['act'] == 'dialog_list_ajax') {
                                $id = empty($_POST['id']) ? 0 : intval($_POST['id']);
                                ...
                                if ($_REQUEST['act'] == 'message_list_ajax') {
                                    $id = empty($_POST['id']) ? 0 : intval($_POST['id']);
...
//                                        require dirname(__FILE__) . '/../mobile/thinker/phpoffice/phpexcel/Classes/PHPExcel.php';
                                        ...
                                        foreach ($list as $k => $v) {
                                            $excel->getActiveSheet()->setCellValue($letter[0] . $k,
                                                strip_tags($v['message']));
                                            $excel->getActiveSheet()->setCellValue($letter[1] . $k, $v['add_time']);

                                            if ($v['user_type'] == 1) {
                                                $excel->getActiveSheet()->setCellValue($letter[2] . $k,
                                                    $dialog['user_name']);
                                            } else {
                                                if ($v['user_type'] == 2) {
                                                    $excel->getActiveSheet()->setCellValue($letter[2] . $k,
                                        ...
                                        if ($_REQUEST['act'] == 'get_message_by_date') {
                                            $customer_id = empty($_POST['customer_id']) ? 0 : intval($_POST['customer_id']);
                                            $service_id = empty($_POST['service_id']) ? 0 : intval($_POST['service_id']);
                                            $page = empty($_POST['page']) ? 0 : intval($_POST['page']);
                                            $start_time = empty($_POST['start_time']) ? 0 : strip_tags(trim($_POST['start_time']));
                                            $message = message_list($customer_id, $service_id, $page, '',
