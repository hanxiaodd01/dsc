...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        make_json_result($smarty->fetch('goods_auto.dwt'), '',
        ...
        if ($_REQUEST['act'] == 'del') {
            ...
            if ($_REQUEST['act'] == 'edit_starttime') {
                ...
                $db->autoReplace($ecs->table('auto_manage'),
                    array('item_id' => $id, 'type' => 'goods', 'starttime' => $time),
                    array('starttime' => (string)$time));
                clear_cache_files();
                ...
                if ($_REQUEST['act'] == 'edit_endtime') {
                    ...
                    $db->autoReplace($ecs->table('auto_manage'),
                        array('item_id' => $id, 'type' => 'goods', 'endtime' => $time),
                        array('endtime' => (string)$time));
                    clear_cache_files();
                    make_json_result(stripslashes($_POST['val']), '', array('act' => 'goods_auto', 'id' => $id));
                } else {
                    if ($_REQUEST['act'] == 'batch_start') {
                        ...
                        foreach ($_POST['checkboxes'] as $id) {
                            $db->autoReplace($ecs->table('auto_manage'),
                                array('item_id' => $id, 'type' => 'goods', 'starttime' => $_POST['date']),
                                array('starttime' => (string)$_POST['date']));
                        }
                        ...
                        if ($_REQUEST['act'] == 'batch_end') {
                            ...
                            foreach ($_POST['checkboxes'] as $id) {
                                $db->autoReplace($ecs->table('auto_manage'),
                                    array('item_id' => $id, 'type' => 'goods', 'endtime' => $_POST['date']),
                                    array('endtime' => (string)$_POST['date']));
                            }
