...
function bargain_get_product_info_by_attr(
    $bargain_id = 0,
    $goods_id = 0,
    $attr_arr = array(),
    $goods_model = 0,
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('href' => 'bargain.php?act=add', 'text' => $_LANG['add_bargain_goods'], 'class' => 'icon-plus'));
                ...
                if (0 < $id) {
                    ...
                    if ($product_id) {
                        foreach ($product_id as $key => $value) {
                            ...
                            $db->autoExecute($ecs->table('activity_goods_attr'), $attr_data, 'UPDATE',
                    ...
                    $links = array(
                        array(
                            'href' => 'bargain.php?act=list&' . list_link_postfix(),
                            'text' => $_LANG['return_bargain_list']
                        )
                    );
...
if ($_REQUEST['act'] == 'bargain_log') {
    ...
    if ($_REQUEST['act'] == 'bargain_log_query') {
...
if ($_REQUEST['act'] == 'bargain_statistics_list') {
    ...
    $tab_menu[] = array(
        'curr' => 1,
        'text' => $_LANG['friends_and_relatives'],
        'href' => 'bargain.php?act=bargain_statistics_list'
    );
    $smarty->assign('tab_menu', $tab_menu);
    $id = $_REQUEST['id'];
                    ...
                    if ($_REQUEST['act'] == 'set_attribute_table' || $_REQUEST['act'] == 'goods_attribute_query') {
                        ...
                        if (0 < count($attr_arr)) {
                            ...
                            foreach ($attr_group as $key => $val) {
                                $group = array();
                                $product_info = bargain_get_product_info_by_attr($bargain_id, $goods_id, $val,
                                    $goods_model, $region_id);
