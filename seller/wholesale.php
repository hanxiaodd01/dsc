...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('href' => 'wholesale.php?act=add', 'text' => $_LANG['add_wholesale'], 'class' => 'icon-plus'));
            ...
            if ($_REQUEST['act'] == 'batch') {
                if (empty($_POST['checkboxes'])) {
                    ...
                    if (isset($_POST['drop'])) {
                        ...
                        clear_cache_files();
                        $links[] = array(
                            'text' => $_LANG['back_wholesale_list'],
                            'href' => 'wholesale.php?act=list&' . list_link_postfix()
                        );
                ...
                if ($_REQUEST['act'] == 'toggle_enabled') {
                    ...
                    if (($_REQUEST['act'] == 'add') || ($_REQUEST['act'] == 'edit')) {
                        ...
                        $smarty->assign('menu_select',
                            array('action' => 'supply_and_demand', 'current' => '01_wholesale'));
                        ...
                        if (isset($wholesale['is_promote']) && ($wholesale['is_promote'] == '0')) {
                            ...
                            $wholesale['start_time'] = local_date($GLOBALS['_CFG']['time_format'],
                                $wholesale['start_time']);
                            $wholesale['end_time'] = local_date($GLOBALS['_CFG']['time_format'],
                                $wholesale['end_time']);
                        }
                        ...
                        $smarty->assign('goods_type_list',
                            goods_type_list($goods_type, $wholesale['goods_id'], 'array'));
                        $smarty->assign('goods_type_name',
                            $GLOBALS['db']->getOne(' SELECT cat_name FROM ' . $GLOBALS['ecs']->table('goods_type') . ' WHERE cat_id = \'' . $goods_type . '\' '));
                        $volume_price_list = get_wholesale_volume_price_list($wholesale['goods_id']);
                        $smarty->assign('volume_price_list', $volume_price_list);
                        $smarty->assign('action_link',
                            array('href' => $href, 'text' => $_LANG['wholesale_list'], 'class' => 'icon-reply'));
                        ...
                        if (($_REQUEST['act'] == 'insert') || ($_REQUEST['act'] == 'update')) {
                            ...
                            if (intval($_POST['price_model']) && isset($_POST['volume_number']) && isset($_POST['volume_price'])) {
                                handle_wholesale_volume_price($goods_id, intval($_POST['price_model']),
                                    $_POST['volume_number'], $_POST['volume_price'], $_POST['id']);
                            }
                            $wholesale = array(
                            ...
                            if ($is_add) {
                                ...
                                $db->autoExecute($ecs->table('wholesale'), $wholesale, 'UPDATE',
                                    ...
                                    foreach ($attr_value_list as $attr_value => $info) {
                                        if ($info['sign'] == 'insert') {
                                            ...
                                            if ($info['sign'] == 'update') {
                                                ...
                                                $ecs->get_del_find_in_set($info['goods_attr_id'], $where, $table,
                                                    'goods_attr', '|');
                            ...
                            foreach ($product['product_sn'] as $key => $value) {
                                ...
                                if ($product['product_id'][$key]) {
                                    $goods_product = get_wholesale_product_info($product['product_id'][$key],
                                        'product_number', $goods_model);
                                    if ($goods_product['product_number'] != $product['product_number'][$key]) {
                                        ...
                                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_inventory_logs'),
                                            $logs_other, 'INSERT');
                                    }
                                    ...
                                    $goods_attr_id = handle_wholesale_goods_attr($product['goods_id'], $id_list,
                                        $is_spec_list, $value_price_list);
                                    ...
                                    if (check_wholesale_goods_attr_exist($goods_attr, $product['goods_id'], 0,
                                        $region_id)) {
                                        continue;
                                    }
                            ...
                            if ($is_add) {
                                $links = array(
                                    array(
                                        'href' => 'wholesale.php?act=add',
                                        'text' => $_LANG['continue_add_wholesale']
                                    ),
                            ...
                            if ($_REQUEST['act'] == 'search_goods') {
                                ...
                                if ($_REQUEST['act'] == 'get_goods_info') {
                                    ...
                                    if ($_REQUEST['act'] == 'get_attribute') {
                                        ...
                                        $attribute = set_wholesale_goods_attribute($goods_type, $goods_id, $model,
                                            'wholesale_goods_attr');
                                        ...
                                        if ($_REQUEST['act'] == 'drop_product') {
                                            ...
                                            if (($_REQUEST['act'] == 'set_attribute_table') || ($_REQUEST['act'] == 'wholesale_attribute_query')) {
                                                ...
                                                $goods_id = (empty($_REQUEST['goods_id']) ? 0 : intval($_REQUEST['goods_id']));
                                                $goods_type = (empty($_REQUEST['goods_type']) ? 0 : intval($_REQUEST['goods_type']));
                                                $attr_id_arr = (empty($_REQUEST['attr_id']) ? array() : explode(',',
                                                    $_REQUEST['attr_id']));
                                                $attr_value_arr = (empty($_REQUEST['attr_value']) ? array() : explode(',',
                                                    $_REQUEST['attr_value']));
                                                ...
                                                $group_attr = array(
                                                    ...
                                                    'attr_value' => empty($attr_value_arr) ? '' : implode(',',
                                                        $attr_value_arr),
                                                ...
                                                if (0 < count($attr_arr)) {
                                                    ...
                                                    foreach ($attr_arr as $key => $val) {
                                                        ...
                                                        foreach ($val as $k => $v) {
                                                            $data = get_wholesale_goods_attr_id(array(
                                                                'attr_id' => $key,
                                                                'attr_value' => $v,
                                                                'goods_id' => $goods_id
                                                            ), array('ga.*, a.attr_type'), array(1, 2), 1);
                                                            if (!($data)) {
                                                    ...
                                                    $products_list = $ecs->page_array($filter['page_size'],
                                                        $filter['page'], $attr_group, 0, $filter);
                                                    $filter = $products_list['filter'];
                                                    $attr_group = $products_list['list'];
                                                    foreach ($attr_group as $key => $val) {
                                                        $group = array();
                                                        $product_info = get_wholesale_product_info_by_attr($goods_id,
                                                            $val, $goods_model, $region_id);
                                                    ...
                                                    if ($_REQUEST['act'] == 'set_attribute_table') {
                                                        ...
                                                        make_json_result($smarty->fetch('library/wholesale_attribute_query.lbi'),
                                                            '', array(
                                                                'filter' => $products_list['filter'],
                                                                'page_count' => $products_list['page_count']
                                                            ));
                                                    }
                                                }
                                                $smarty->assign('group_attr', $result['group_attr']);
                                                ...
                                                if ($_REQUEST['act'] == 'edit_product_number') {
                                                    ...
                                                    if ($product_id) {
                                                        ...
                                                        $product = get_product_info($product_id,
                                                            'product_number, goods_id' . $filed, $goods_model);
                                                    ...
                                                    if ($_REQUEST['act'] == 'edit_attr_sort') {
                                                        check_authz_json('goods_manage');
                                                        ...
                                                        if ($_REQUEST['act'] == 'edit_attr_price') {
                                                            check_authz_json('goods_manage');
                                                            $goods_attr_id = intval($_REQUEST['id']);
                                                            ...
                                                            if ($_REQUEST['act'] == 'edit_product_sn') {
                                                                ...
                                                                if ($_REQUEST['act'] == 'edit_attr_sort') {
                                                                    ...
                                                                    if ($_REQUEST['act'] == 'attr_input_type') {
                                                                        ...
                                                                        $goods_attr = get_dialog_wholesale_goods_attr_type($attr_id,
                                                                            $goods_id);
                                                                        ...
                                                                        if ($_REQUEST['act'] == 'insert_attr_input') {
                                                                            ...
                                                                            if ($_REQUEST['act'] == 'del_goods_attr') {
                                                                                ...
                                                                                if ($_REQUEST['act'] == 'add_attr_img') {
                                                                                    ...
                                                                                    $goods_attr_info = get_table_date('wholesale_goods_attr',
                                                                                    ...
                                                                                    $smarty->assign('goods_info',
                                                                                        $goods_info);
                                                                                    $smarty->assign('attr_info',
                                                                                        $attr_info);
                                                                                    $smarty->assign('goods_attr_info',
                                                                                        $goods_attr_info);
                                                                                    $smarty->assign('goods_attr_name',
                                                                                        $goods_attr_name);
                                                                                    $smarty->assign('goods_id',
                                                                                        $goods_id);
                                                                                    $smarty->assign('attr_id',
                                                                                        $attr_id);
                                                                                    $smarty->assign('goods_attr_id',
                                                                                        $goods_attr_id);
                                                                                    ...
                                                                                    if ($_REQUEST['act'] == 'insert_attr_img') {
                                                                                        ...
                                                                                        if ($other['attr_checked'] == 1) {
                                                                                            $db->autoExecute($ecs->table('wholesale_goods_attr'),
                                                                                                array('attr_checked' => 0),
                                                                                                'UPDATE',
                                                                                        ...
                                                                                        if ($_REQUEST['act'] == 'drop_attr_img') {
                                                                                            ...
                                                                                            $other['attr_img_flie'] = '';
                                                                                            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('wholesale_goods_attr'),
                                                                                                $other, 'UPDATE',
