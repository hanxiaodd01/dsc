...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link2',
        array('href' => 'shipping.php?act=list', 'text' => $_LANG['03_shipping_list'], 'class' => 'icon-reply'));
    ...
    if ($_REQUEST['act'] == 'add' && !empty($_REQUEST['shipping'])) {
        ...
        $smarty->assign('action_link', array(
            'href' => 'shipping_area.php?act=list&shipping=' . $_REQUEST['shipping'],
            'text' => $_LANG['09_region_area_management'],
            'class' => 'icon-reply'
        ));
        ...
        $smarty->assign('shipping_area',
            array('shipping_id' => $_REQUEST['shipping'], 'shipping_code' => $shipping['shipping_code']));
        ...
        if ($_REQUEST['act'] == 'insert') {
            ...
            if (0 < $db->getOne($sql)) {
                ...
                $lnk[] = array(
                    ...
                    'href' => 'shipping_area.php?act=list&shipping=' . $_POST['shipping']
                );
                $lnk[] = array(
                    'text' => $_LANG['add_continue'],
                    'href' => 'shipping_area.php?act=add&shipping=' . $_POST['shipping']
                );
            ...
            if ($_REQUEST['act'] == 'edit') {
                ...
                $smarty->assign('action_link', array(
                    'href' => 'shipping_area.php?act=list&shipping=' . $row['shipping_id'],
                    'text' => $_LANG['09_region_area_management'],
                    'class' => 'icon-reply'
                ));
                assign_query_info();
                ...
                if ($_REQUEST['act'] == 'update') {
                    ...
                    if ($_REQUEST['act'] == 'multi_remove') {
                        ...
                        $links[0] = array(
                            'href' => 'shipping_area.php?act=list&shipping=' . intval($_REQUEST['shipping']),
                            'text' => $_LANG['go_back']
                        );
                        ...
                        if ($_REQUEST['act'] == 'edit_area') {
                            ...
                            if (!$exc->is_only('shipping_area_name', $val, $id,
                                'shipping_id = \'' . $shipping_id . '\' and ru_id=\'' . $adminru['ru_id'] . '\'')) {
                            ...
                            if ($_REQUEST['act'] == 'remove_area') {
                                ...
                                if ($_REQUEST['act'] == 'remove_point') {
                                    ...
                                    if ($db->query($sql)) {
                                        $data = array(
                                            'error' => 2,
                                            'message' => $_LANG['delete_success_alt'],
                                            'content' => ''
                                        );
                                        admin_log($name, 'remove', 'shipping_area');
                                    } else {
                                        $data = array(
                                            'error' => 0,
                                            'message' => $_LANG['js_languages']['confirm_delete_fail'],
                                            'content' => ''
                                        );
                                    }
