...
{
    return preg_match('/,*' . $authz . ',*/',
            $_SESSION['seller_action_list']) || $_SESSION['seller_action_list'] == 'all';
}
        ...
        if (!empty($cat_name)) {
            ...
            if ($GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('merchants_category'), $cat) !== false) {
                ...
                if ($cat['show_in_nav'] == 1) {
                    ...
                    $vieworder += 2;
                    $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('merchants_nav') . ' (name,ctype,cid,ifshow,vieworder,opennew,url,type)' . ' VALUES(\'' . $cat['cat_name'] . ('\', \'c\', \'' . $cat_id . '\',\'1\',\'' . $vieworder . '\',\'0\', \'') . build_uri('merchants_store',
                            array('cid' => $cat_id, 'urid' => $ru_id), $cat['cat_name']) . '\',\'middle\')';
                    $GLOBALS['db']->query($sql);
                }
