...
if (!isset($_REQUEST['act'])) {
    ...
    if (($_REQUEST['act'] == 'login' || $_REQUEST['act'] == 'logout' || $_REQUEST['act'] == 'signin') && strpos(PHP_SELF,
            '/privilege.php') === false) {
        $_REQUEST['act'] = '';
    } else {
        if (($_REQUEST['act'] == 'forget_pwd' || $_REQUEST['act'] == 'reset_pwd' || $_REQUEST['act'] == 'get_pwd') && strpos(PHP_SELF,
                '/get_password.php') === false) {
            $_REQUEST['act'] = '';
        }
    }
}
...
if ($_REQUEST['act'] != 'login' && $_REQUEST['act'] != 'signin' && $_REQUEST['act'] != 'forget_pwd' && $_REQUEST['act'] != 'reset_pwd' && $_REQUEST['act'] != 'check_order') {
    ...
    if (!empty($_SERVER['HTTP_REFERER']) && strpos(preg_replace('/:\\d+/', '', $_SERVER['HTTP_REFERER']),
            $admin_path) === false && strpos(preg_replace('/:\\d+/', '', $_SERVER['HTTP_REFERER']),
            $mobile_path) === false) {
        if (!empty($_REQUEST['is_ajax'])) {
            make_json_error($_LANG['priv_error']);
        } else {
...
$admin_info = get_table_date('admin_user', 'user_id=\'' . intval($_SESSION['seller_id']) . '\'',
    array('user_name', 'admin_user_img'));
...
if ($row['domain_name'] && $row['is_enable']) {
    ...
    if (!empty($head_shop_url) && (strpos($head_shop_url, 'http://') === false && strpos($head_shop_url,
                'https://') === false)) {
