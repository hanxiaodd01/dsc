...
function get_flash_xml()
{
    ...
    if (file_exists(ROOT_PATH . DATA_DIR . '/flash_data.xml')) {
        ...
        if (!preg_match_all('/item_url="([^"]+)"\\slink="([^"]+)"\\stext="([^"]*)"\\ssort="([^"]*)"/', $flash_data, $t,
            PREG_SET_ORDER)) {
...
function set_flash_focus($tplname, $flashdata)
{
    ...
    write_static_file_cache('data',
        $jsdata . "\n" . $jsdata2['url'] . ";\n" . $jsdata2['link'] . ";\n" . $jsdata2['txt'] . ';', 'js',
        ROOT_PATH . DATA_DIR . '/flashdata/' . $tplname . '/');
    return true;
}
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            if (empty($_POST['step'])) {
                ...
                if (isset($width_height['width']) || isset($width_height['height'])) {
                    $smarty->assign('width_height',
                        sprintf($_LANG['width_height'], $width_height['width'], $width_height['height']));
                }
            ...
            if ($_REQUEST['act'] == 'edit') {
                ...
                if (empty($_POST['step'])) {
                    ...
                    $smarty->assign('action_link',
                        array('text' => $_LANG['go_url'], 'href' => 'flashplay.php?act=list'));
                    $smarty->assign('rt', $rt);
                    ...
                    if ($_POST['step'] == 2) {
                        if (empty($_POST['img_url'])) {
                            $links[] = array(
                                'text' => $_LANG['return_edit'],
                                'href' => 'flashplay.php?act=edit&id=' . $id
                            );
                            sys_msg($_LANG['link_empty'], 0, $links);
                        }
                ...
                if ($_REQUEST['act'] == 'install') {
                    ...
                    if ($_REQUEST['act'] == 'custom_list') {
                        $group_list = array(
                            'sys' => array(
                                'text' => $_LANG['system_set'],
                                'url' => $_CFG['index_ad'] == 'cus' ? 'javascript:system_set();void(0);' : 'flashplay.php?act=list'
                            ),
                        ...
                        $smarty->assign('uri', $uri);
                        $smarty->assign('ur_here', $_LANG['flashplay']);
                        $smarty->assign('action_link_special',
                            array('text' => $_LANG['add_flash'], 'href' => 'flashplay.php?act=custom_add'));
                        $ad = array(
                            'ad_name' => '',
                            'ad_type' => 0,
                            'ad_url' => 'http://',
                        ...
                        if ($_REQUEST['act'] == 'custom_add') {
                            ...
                            $smarty->assign('action_link_special',
                                array('text' => $_LANG['add_flash'], 'href' => 'flashplay.php?act=custom_add'));
                            $smarty->assign('action_link',
                                array('text' => $_LANG['ad_play_url'], 'href' => 'flashplay.php?act=custom_list'));
                            $ad = array(
                                'ad_name' => '',
                                'ad_type' => 0,
                                'ad_url' => 'http://',
                            ...
                            if ($_REQUEST['act'] == 'custom_insert') {
                                ...
                                if (empty($_POST['ad']) || empty($_POST['content']) || empty($_POST['ad']['ad_name'])) {
                                    $links[] = array(
                                        'text' => $_LANG['back'],
                                        'href' => 'flashplay.php?act=custom_list'
                                    );
                                ...
                                if ($ad_img['ad_img']['name'] && (0 < $ad_img['ad_img']['size'])) {
                                    ...
                                    if (!empty($filter['content']['url'])) {
                                        if (strstr($filter['content']['url'],
                                                'http') && !strstr($filter['content']['url'],
                                                $_SERVER['SERVER_NAME'])) {
                                    ...
                                    if ($_REQUEST['act'] == 'custom_status') {
                                        ...
                                        if (modfiy_ad_status($id, $ad_status)) {
                                            ...
                                            $group_list = array(
                                                'sys' => array(
                                                    'text' => $_LANG['system_set'],
                                                    'url' => $shop_config['value'] == 'cus' ? 'javascript:system_set();void(0);' : 'flashplay.php?act=list'
                                                ),
                                                'cus' => array('text' => $_LANG['custom_set'], 'url' => '')
                                            );
                                            $ad_list = ad_list();
                                            ...
                                            $smarty->assign('uri', $uri);
                                            $smarty->assign('ur_here', $_LANG['flashplay']);
                                            $smarty->assign('action_link_special', array(
                                                'text' => $_LANG['add_flash'],
                                                'href' => 'flashplay.php?act=custom_add'
                                            ));
                                            $ad = array(
                                                'ad_name' => '',
                                                'ad_type' => 0,
                                        ...
                                        if ($_REQUEST['act'] == 'custom_edit') {
                                            ...
                                            $width_height = get_width_height();
                                            $smarty->assign('width_height',
                                                sprintf($_LANG['width_height'], $width_height['width'],
                                                    $width_height['height']));
                                            $smarty->assign('group_selected', $_CFG['index_ad']);
                                            $smarty->assign('uri', $uri);
                                            $smarty->assign('ur_here', $_LANG['flashplay']);
                                            $smarty->assign('action_link', array(
                                                'text' => $_LANG['ad_play_url'],
                                                'href' => 'flashplay.php?act=custom_list'
                                            ));
                                            ...
                                            if ($_REQUEST['act'] == 'custom_update') {
                                                ...
                                                $db->autoExecute($ecs->table('ad_custom'), $ad, 'UPDATE',
                                                    'ad_id = ' . $ad_info['ad_id'], 'SILENT');
