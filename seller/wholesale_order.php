...
function wholesale_order_list()
{
    ...
    if ($result === false) {
        ...
        $filter['start_take_time'] = empty($_REQUEST['start_take_time']) ? '' : (0 < strpos($_REQUEST['start_take_time'],
            '-') ? local_strtotime($_REQUEST['start_take_time']) : $_REQUEST['start_take_time']);
        $filter['end_take_time'] = empty($_REQUEST['end_take_time']) ? '' : (0 < strpos($_REQUEST['end_take_time'],
            '-') ? local_strtotime($_REQUEST['end_take_time']) : $_REQUEST['end_take_time']);
    ...
    foreach ($row as $key => $value) {
        ...
        $value['ru_id'] = $GLOBALS['db']->getOne(' SELECT ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' WHERE order_id = \'' . $value['order_id'] . '\'',
            true);
...
function download_orderlist($result)
{
    ...
    if ($adminru['ru_id'] < 1) {
        $data_name = $GLOBALS['_LANG']['goods_steps_name'] . ',';
        $data_cnt = $GLOBALS['_LANG']['exist'];
    }

    $data = i($GLOBALS['_LANG']['order_id'] . ',' . $data_name . $GLOBALS['_LANG']['wholesale_order_php_download_orderlist'] . '
');
    $count = count($result);
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'order_export') {
        ...
        if ($_REQUEST['act'] == 'query') {
            ...
            $smarty->assign('page_count_arr', $page_count_arr);
            $sort_flag = sort_flag($list['filter']);
            make_json_result($smarty->fetch('wholesale_store_order.dwt'), '',
            ...
            if ($_REQUEST['act'] == 'info') {
                ...
                $date = array('order_id');
                $order_child = count(get_table_date('wholesale_order_info',
            ...
            WHERE o.order_id = \'' . $order['order_id'] . '\' ');
                ...
                if (isset($_GET['print'])) {
                    ...
                    if (isset($_GET['shipping_print'])) {
                        ...
                        $smarty->assign('menu_select',
                            array('action' => 'supply_and_demand', 'current' => '02_wholesale_order'));
                ...
                if ($_REQUEST['act'] == 'remove') {
                    ...
                    $id = intval($_GET['id']);
                    $exc->drop($id);
                    $goods_list = get_table_date('wholesale_purchase_goods', 'purchase_id=\'' . $id . '\'',
                        array('goods_id', 'goods_img'), 1);
                    ...
                    $url = 'wholesale_purchase.php?act=query&' . str_replace('act=remove', '',
                        ...
                        if ($_REQUEST['act'] == 'get_goods_info') {
                            ...
                            if ($_REQUEST['act'] == 'pay_order') {
                                ...
                                if (0 < $order_id) {
                                    ...
                                    if ($pay_status == 2) {
                                        $result['error'] = 1;
                                        $result['msg'] = $_LANG['not_repeat_pay'];
                                    } else {
