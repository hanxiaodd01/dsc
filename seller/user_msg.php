...
if ($_REQUEST['act'] == 'add') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['order_detail'], 'href' => 'order.php?act=info&order_id=' . $order_id));
...
if ($_REQUEST['act'] == 'list_all') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        $smarty->assign('current', 'user_msg');
        make_json_result($smarty->fetch('msg_list.dwt'), '',
            array('filter' => $msg_list['filter'], 'page_count' => $msg_list['page_count']));
    } else {
...
if ($_REQUEST['act'] == 'batch') {
    ...
    if (isset($_POST['checkboxes'])) {
        switch ($action) {
            case 'remove':
                $db->query('DELETE FROM ' . $ecs->table('feedback') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                        'msg_id'));
                $db->query('DELETE FROM ' . $ecs->table('feedback') . ' WHERE ' . db_create_in($_POST['checkboxes'],
                        'parent_id'));
                break;
            ...
            case 'allow':
                $db->query('UPDATE ' . $ecs->table('feedback') . ' SET msg_status = 1  WHERE ' . db_create_in($_POST['checkboxes'],
                        'msg_id'));
                break;

            case 'deny':
                $db->query('UPDATE ' . $ecs->table('feedback') . ' SET msg_status = 0,msg_area =1  WHERE ' . db_create_in($_POST['checkboxes'],
                        'msg_id'));
                break;
    ...
    if ($_REQUEST['act'] == 'view') {
        ...
        $smarty->assign('action_link',
            array('text' => $_LANG['08_unreply_msg'], 'href' => 'user_msg.php?act=list_all'));
        assign_query_info();
        $smarty->assign('current', 'user_msg');
        ...
        if ($_REQUEST['act'] == 'action') {
            ...
            if (!empty($_POST['send_email_notice']) || isset($_POST['remail'])) {
                ...
                if (send_mail($message_info['user_name'], $message_info['user_email'], $template['template_subject'],
