...
if (empty($_REQUEST['act'])) {
    ...
    if ($_REQUEST['act'] == 'kdniao_print') {
        ...
        $smarty->assign('kdniao_printer',
            get_table_date('seller_shopinfo', 'ru_id=\'' . $adminru['ru_id'] . '\'', array('kdniao_printer'), 2));
...
if ($_REQUEST['act'] == 'order_print_setting') {
    ...
    $smarty->assign('action_link',
        array('text' => $_LANG['order_print_setting_add'], 'href' => 'tp_api.php?act=order_print_setting_add'));
    ...
    if ($_REQUEST['act'] == 'order_print_setting_query') {
        ...
        if ($_REQUEST['act'] == 'order_print_setting_remove') {
            check_authz_json('order_print_setting');
            $id = intval($_GET['id']);
            $exc = new exchange($ecs->table('order_print_setting'), $db, 'id', 'ru_id');
            $exc->drop($id);
            $url = 'tp_api.php?act=order_print_setting_query&' . str_replace('act=order_print_setting_remove', '',
                    $_SERVER['QUERY_STRING']);
            ecs_header('Location: ' . $url . "\n");
            exit();
        } else {
            if ($_REQUEST['act'] == 'edit_order_printer') {
                check_authz_json('order_print_setting');
                $id = intval($_POST['id']);
                $val = trim($_POST['val']);
                ...
                if ($_REQUEST['act'] == 'edit_print_width') {
                    check_authz_json('order_print_setting');
                    $id = intval($_POST['id']);
                    $val = trim($_POST['val']);
                    ...
                    if ($_REQUEST['act'] == 'edit_sort_order') {
                        ...
                        if ($_REQUEST['act'] == 'toggle_order_is_default') {
                            check_authz_json('order_print_setting');
                            $id = intval($_POST['id']);
                            ...
                            if ($_REQUEST['act'] == 'order_print_setting_add' || $_REQUEST['act'] == 'order_print_setting_edit') {
                                ...
                                $smarty->assign('menu_select',
                                    array('action' => '19_merchants_store', 'current' => 'order_print_setting'));
                                ...
                                $smarty->assign('action_link', array(
                                    'text' => $_LANG['order_print_setting'],
                                    'href' => 'tp_api.php?act=order_print_setting'
                                ));
                                assign_query_info();
                                ...
                                if ($_REQUEST['act'] == 'order_print_setting_insert' || $_REQUEST['act'] == 'order_print_setting_update') {
                                    ...
                                    if (empty($data['width'])) {
                                        $print_size = get_table_date('order_print_size',
                                    ...
                                    if (0 < $id) {
                                        $db->autoExecute($ecs->table('order_print_setting'), $data, 'UPDATE',
                                            'id = \'' . $id . '\'');
                                    ...
                                    if ($data['is_default']) {
                                        $db->autoExecute($ecs->table('order_print_setting'), array('is_default' => 0),
                                            'UPDATE', 'id <> \'' . $id . '\'');
                                    }
                                    ...
                                    if ($_REQUEST['act'] == 'order_print') {
                                        admin_priv('order_view');
                                        $print_specification = get_table_date('order_print_setting',
                                            'ru_id=\'' . $adminru['ru_id'] . '\' AND is_default=\'1\'',
                                            array('specification'), 2);

                                        if (empty($print_specification)) {
                                            $print_specification = get_table_date('order_print_setting',
                                                'ru_id=\'' . $adminru['ru_id'] . '\' ORDER BY sort_order, id',
                                                array('specification'), 2);
                                        }

                                        $print_size_info = get_table_date('order_print_size',
                                            'specification=\'' . $print_specification . '\'', array('*'));
                                        $print_size_list = get_table_date('order_print_setting',
                                            'ru_id=\'' . $adminru['ru_id'] . '\' ORDER BY sort_order, id', array('*'),
                                            1);
                                        $print_spec_info = get_table_date('order_print_setting',
                                            'specification=\'' . $print_specification . '\'', array('*'));
                                        ...
                                        $action_id = get_table_date('admin_action', 'action_code=\'supply_and_demand\'',
                                            array('action_id'), 2);
                                        ...
                                        $smarty->assign('order_print_logo',
                                            $ecs->seller_url() . ADMIN_PATH . '/' . $order_print_logo);
                                        $part_html = array();
                                        ...
                                        if ($_REQUEST['act'] == 'change_order_print') {
                                            ...
                                            $order_sn = empty($_REQUEST['order_sn']) ? '' : trim($_REQUEST['order_sn']);
                                            $order_type = empty($_REQUEST['order_type']) ? 'order' : trim($_REQUEST['order_type']);
                                            $action_id = get_table_date('admin_action',
                                                'action_code=\'supply_and_demand\'', array('action_id'), 2);
                                            if ($order_type == 'order' || empty($action_id)) {
                                                $table = $GLOBALS['ecs']->table('order_info');
                                            } else {
                                                $table = $GLOBALS['ecs']->table('wholesale_order_info');
