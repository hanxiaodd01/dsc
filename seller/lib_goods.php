...
function goods_list($is_delete, $real_goods = 1, $conditions = '')
{
    ...
    if ($result === false) {
        ...
        if ($filter['cat_type'] == 'seller') {
            $where = (0 < $filter['cat_id'] ? ' AND (' . get_children($filter['cat_id'], 0, 0, 'merchants_category',
                    'g.user_cat') . ')' : '');
        } else {
            $where = (0 < $filter['cat_id'] ? ' AND (' . get_children($filter['cat_id']) . ' OR ' . get_children($filter['cat_id'],
                    1) . ')' : '');
        }
