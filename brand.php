...
$cache_id = sprintf('%X',
    crc32($brand_id . '-' . $mbid . '-' . $display . '-' . $price_min . '-' . $price_max . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' . $_CFG['lang'] . '-' . $cate . '-' . $ship . '-' . $self));
...
if (!$smarty->is_cached('brand_list.dwt', $cache_id)) {
    ...
    $smarty->assign('promotion_goods',
        brand_recommend_goods('promote', $brand_id, $cate, $region_id, $area_id, $area_city));
    $smarty->assign('brand', $brand_info);
    $count = goods_count_by_brand($brand_id, $mbid, $cate, $act, $ship, $price_min, $price_max, $region_id, $area_id,
        $self);
    $goodslist = brand_get_goods($brand_id, $mbid, $cate, $size, $page, $sort, $order, $region_id, $area_id, $area_city,
        $act, $ship, $price_min, $price_max, $self);
    ...
    assign_pager('brand', $cate, $count, $size, $sort, $order, $page, '', $brand_id, $price_min, $price_max, $display,
        '', '', '', 0, '', '', $act, $ship, $self, $mbid);
