...
if (empty($_REQUEST['act'])) {
    ...
    if ($_REQUEST['act'] == 'ajax_return_images') {
        ...
        if (!empty($user_id) && 0 < $db->getOne($sql)) {
            ...
            $img_thumb = $image->make_thumb($img_file, $GLOBALS['_CFG']['single_thumb_width'],
                $GLOBALS['_CFG']['single_thumb_height'], DATA_DIR . '/cmt_img/' . date('Ym') . '/thumb/');
        ...
        if ($_REQUEST['act'] == 'del_pictures') {
            ...
            if ($_REQUEST['act'] == 'ajax_return_images_list') {
                $sql = 'select id, comment_img, img_thumb from ' . $ecs->table('comment_img') . ' where user_id = \'' . $_SESSION['user_id'] . ('\' and order_id = \'' . $order_id . '\' and goods_id = \'' . $goods_id . '\' order by id desc');
                $img_list = $db->getAll($sql);
                ...
                if ($_REQUEST['act'] == 'comm_order_goods') {
                    $result = array('error' => 0, 'message' => '', 'content' => '');
                    ...
                    if ($_REQUEST['act'] == 'satisfaction_degree') {
                        ...
                        if ($result) {
                            ...
                            foreach ($goods_id as $gid) {
                                ...
                                $num = array(
                                    ...
                                    'merchants_comment_number' => $comment_seller_num
                                );
                            ...
                            if ($_REQUEST['act'] == 'add_useful') {
                                ...
                                if ($_REQUEST['act'] == 'comment_reply') {
                                    ...
                                    if (!isset($_SESSION['user_id']) || $_SESSION['user_id'] == 0) {
                                        ...
                                        if ($comment_user == $_SESSION['user_id']) {
                                        } else {
                                    ...
                                    if ($result['err_no'] != 1) {
                                        $reply = get_reply_list($goods_id, $comment_id, $type, $reply_page, $libType,
                                            $size);
