...
function get_linked_goods($goods_id, $warehouse_id = 0, $area_id = 0, $area_city = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (0 < $row['promote_price']) {
            $arr[$row['goods_id']]['promote_price'] = bargain_price($row['promote_price'], $row['promote_start_date'],
    ...
    if (!$smarty->is_cached($template . '.dwt', $cache_id)) {
        if (0 < $count && $template == 'group_buy_list') {
            ...
            if (!$_CFG['category_load_type']) {
                $pager = get_pager('group_buy.php',
                    array('act' => 'list', 'keywords' => $keywords, 'sort' => $sort, 'order' => $order), $count, $page,
                    $size);
        ...
        foreach ($seo as $key => $value) {
            $seo[$key] = str_replace(array('{sitename}', '{key}', '{description}'),
                array($_CFG['shop_name'], $_CFG['shop_keywords'], $_CFG['shop_desc']), $value);
        }
        ...
        if ($_REQUEST['act'] == 'getInfo') {
            ...
            if ($_REQUEST['act'] == 'view') {
                ...
                if (!$smarty->is_cached('group_buy_goods.dwt', $cache_id)) {
                    ...
                    if (defined('THEME_EXTENSION')) {
                        $position = assign_ur_here($group_buy['cat_id'], $group_buy['goods_name'], array(), '',
                            $group_buy['user_id']);
                    } else {
                ...
                foreach ($seo as $key => $value) {
                    $seo[$key] = str_replace(array('{name}', '{description}'),
                        array($group_buy['act_name'], $group_buy['act_desc']), $value);
                }
                ...
                if ($_REQUEST['act'] == 'price') {
                    ...
                    if ($goods_id == 0) {
                        ...
                        $products = get_warehouse_id_attr_number($goods_id, $_REQUEST['attr'], $goods['user_id'],
                    ...
                    if ($_REQUEST['act'] == 'buy') {
                        ...
                        if ($goods_attr) {
                            ...
                            $prod = $GLOBALS['db']->getRow($sql);
                            $products = get_warehouse_id_attr_number($goods['goods_id'], $goods_attr, $goods['user_id'],
                        ...
                        $start_date = $group_buy['xiangou_start_date'];
                        $end_date = $group_buy['xiangou_end_date'];
                        $order_goods = get_for_purchasing_goods($start_date, $end_date, $group_buy['goods_id'],
                            $_SESSION['user_id'], 'group_buy');
