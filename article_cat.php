...
$smarty->assign('feed_url',
    $_CFG['rewrite'] == 1 ? 'feed-typearticle_cat' . $cat_id . '.xml' : 'feed.php?type=article_cat' . $cat_id);
$seo = get_seo_words('article');

foreach ($seo as $key => $value) {
    $seo[$key] = str_replace(array('{sitename}', '{key}', '{name}', '{description}', '{article_class}'),
        array($_CFG['shop_name'], $meta['keywords'], $meta['cat_name'], $meta['cat_desc'], $cat_list[0]['cat_name']),
        $value);
}
