...
function get_order_detection_list()
{
    ...
    if ($row) {
        foreach ($row as $key => $value) {
            ...
            order_action($value['order_sn'], $value['order_status'], SS_RECEIVED, $value['pay_status'], $note,
                $operator, 0, $confirm_take_time);
            ...
            $seller_id = $GLOBALS['db']->getOne('SELECT ru_id FROM ' . $GLOBALS['ecs']->table('order_goods') . ' WHERE order_id = \'' . $value['order_id'] . '\'',
                true);
            $value_card = $GLOBALS['db']->getOne('SELECT use_val FROM ' . $GLOBALS['ecs']->table('value_card_record') . ' WHERE order_id = \'' . $value['order_id'] . '\'',
                true);
