...
function check_gift_login($gift_sn, $gift_pwd, $remember = null)
{
    ...
    if (!$GLOBALS['db']->getOne($sql)) {
        $_SESSION['gift_sn'] = '';
        show_message($GLOBALS['_LANG']['gift_gard_used'], $GLOBALS['_LANG']['gift_gard_login'], 'gift_gard.php',
            'error');
        return false;
    }
    ...
    if (empty($result)) {
        $_SESSION['gift_sn'] = '';
        show_message($GLOBALS['_LANG']['password_error'], $GLOBALS['_LANG']['back_gift_login'],
            'gift_gard.php?act=gift_login', 'error');
        return false;
    }
    ...
    if ($row) {
        ...
        if ($row['gift_end_date'] <= $time) {
            $_SESSION['gift_sn'] = '';
            show_message($GLOBALS['_LANG']['gift_gard_overdue_time'] . local_date('Y-m-d H:i:s', $row['gift_end_date']),
                $GLOBALS['_LANG']['back_gift_login'], 'gift_gard.php?act=gift_login', 'error');
            return false;
        } else {
            if ($time <= $row['gift_start_date']) {
                $_SESSION['gift_sn'] = '';
                show_message($GLOBALS['_LANG']['gift_gard_Use_time'] . local_date('Y-m-d H:i:s',
                        $row['gift_start_date']), $GLOBALS['_LANG']['back_gift_login'], 'gift_gard.php?act=gift_login',
                    'error');
                return false;
            }
        }
        ...
        $_SESSION['gift_sn'] = '';
        show_message($GLOBALS['_LANG']['not_gift_gard'], $GLOBALS['_LANG']['back_gift_login'],
            'gift_gard.php?act=gift_login', 'error');
        return false;
    }
...
if ($_REQUEST['act'] == 'check_gift') {
    ...
    if (isset($_POST['captcha'])) {
        if (empty($captcha_str)) {
            show_message($_LANG['cmt_lang']['captcha_not_null'], $_LANG['relogin_lnk'], 'javascript:history.go(-1);',
                'error');
        }
...
if ($_REQUEST['act'] == 'list') {
    ...
    $sort = isset($_REQUEST['sort']) && in_array(trim(strtolower($_REQUEST['sort'])),
        array('gift_gard_id')) ? trim($_REQUEST['sort']) : $default_sort_order_type;
    ...
    $cache_id = sprintf('%X',
        crc32($cat_id . '-' . $display . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' . $_CFG['lang']));
    ...
    if ($_REQUEST['act'] == 'take_view') {
        ...
        if ($_REQUEST['act'] == 'check_take') {
            ...
            if ($_REQUEST['act'] == 'edit_Consignee') {
