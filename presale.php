...
function get_pre_cat()
{
    ...
    foreach ($cat_res as $key => $row) {
        ...
        $cat_res[$key]['cat_url'] = build_uri('presale', array('act' => 'category', 'cid' => $row['cat_id']),
            $row['cat_name']);
    }
...
function get_pre_category($act = 'new', $status = 0)
{
    ...
    foreach ($res as $key => $row) {
        $res[$key]['cat_id'] = $row['cat_id'];
        $res[$key]['cat_name'] = $row['cat_name'];
        $res[$key]['url'] = build_uri('presale', array('act' => $act, 'cid' => $row['cat_id'], 'status' => $status),
            $row['cat_name']);
    }
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'price') {
    ...
    if ($goods_id == 0) {
        ...
        $spec_price = get_final_price($goods_id, $number, true, $attr_id, $warehouse_id, $area_id, $area_city, 1, 0, 0,
            $res['show_goods'], $product_promote_price);
        ...
        if ($_REQUEST['act'] == 'new') {
            ...
            if ($_REQUEST['act'] == 'advance') {
                ...
                $sort = isset($_REQUEST['sort']) && in_array(trim(strtolower($_REQUEST['sort'])),
                    array('shop_price', 'start_time', 'act_id')) ? trim($_REQUEST['sort']) : $default_sort_order_type;
                ...
                if ($_REQUEST['act'] == 'category') {
                    ...
                    $cat_id = isset($_REQUEST['cat_id']) && 0 < intval($_REQUEST['cat_id']) ? intval($_REQUEST['cat_id']) : 0;
                    $status = isset($_REQUEST['status']) && 0 < intval($_REQUEST['status']) ? intval($_REQUEST['status']) : 0;
                    $goods = get_pre_goods($cat_id, $price_min, $price_max, $start_time, $end_time, $sort, $status,
                        $order);
                    ...
                    if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'guess_goods') {
                        ...
                        if ($_REQUEST['act'] == 'view') {
                            ...
                            if (!$smarty->is_cached('presale_goods.dwt', $cache_id)) {
                                ...
                                $position = assign_ur_here($presale['pa_catid'], $presale['goods_name'], array(), '',
                                    $presale['user_id']);
                                $smarty->assign('page_title', $position['title']);
                                $smarty->assign('ur_here', $position['ur_here']);
                                $smarty->assign('helps', get_shop_help());
                                $smarty->assign('best_goods',
                                    get_recommend_goods('best', '', $region_id, $area_id, $area_city, $goods['user_id'],
                                        1, 'presale'));
                                $smarty->assign('new_goods',
                                    get_recommend_goods('new', '', $region_id, $area_id, $area_city, $goods['user_id'],
                                        1, 'presale'));
                                $smarty->assign('hot_goods',
                                    get_recommend_goods('hot', '', $region_id, $area_id, $area_city, $goods['user_id'],
                                        1, 'presale'));
                            ...
                            $basic_info['province'] = get_table_date('region',
                            ...
                            if ($_REQUEST['act'] == 'buy') {
                                $goods_attr = isset($_POST['goods_attr_id']) && !empty($_POST['goods_attr_id']) ? dsc_addslashes($_POST['goods_attr_id'],
                                    0) : '';
                                ...
                                if ($goods_attr) {
                                    ...
                                    $prod = $GLOBALS['db']->getRow($sql);
                                    $products = get_warehouse_id_attr_number($goods['goods_id'], $goods_attr,
                                        $goods['user_id'], $region_id, $area_id, $area_city);
                                ...
                                if ($goods['is_xiangou'] == 1 && $start_date < $nowTime && $nowTime < $end_date) {
                                    if ($goods['xiangou_num'] <= $presale['total_goods']) {
                                        ...
                                        show_message($message, $_LANG['back_to_presale'],
                                            'presale.php?id=' . $presale['act_id'] . '&act=view');
                                    } else {
                                        if (0 < $goods['xiangou_num']) {
