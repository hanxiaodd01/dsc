...
function brand_recommend_goods(
    $type,
    $brand,
    $cat = 0,
    $warehouse_id = 0,
    ...
    foreach ($result as $row) {
        if ($type == 'best' && $row['is_best'] == 1 || $type == 'new' && $row['is_new'] == 1 || $type == 'hot' && $row['is_hot'] == 1 || $type == 'promote' && $row['is_promote'] == 1 && $row['promote_start_date'] <= $time && $time <= $row['promote_end_date']) {
            ...
            $goods[$idx]['short_style_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
...
function brand_related_cat($brand, $act)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        $row['url'] = build_uri('brandn', array('cid' => $row['cat_id'], 'bid' => $brand, 'act' => $act),
            $row['cat_name']);
        $arr[] = $row;
    }
...
if ($act == 'index') {
    $smarty->assign('best_goods',
        brand_recommend_goods('best', $brand_id, $cate, $region_id, $area_id, $area_city, $act));
    $smarty->assign('hot_goods',
        brand_recommend_goods('hot', $brand_id, $cate, $region_id, $area_id, $area_city, $act));
    ...
    if ($act == 'new') {
        ...
        assign_pager('brandn', $cate, $goods['record_count'], $size, $sort, $order, $page, '', $brand_id, $price_min,
            $price_max, $display, '', '', '', 0, '', '', $act, $is_ship, $is_self);
        ...
        if ($act == 'hot') {
            ...
            assign_pager('brandn', $cate, $goods['record_count'], $size, $sort, $order, $page, '', $brand_id,
                $price_min, $price_max, $display, '', '', '', 0, '', '', $act, $is_ship, $is_self);
            ...
            if ($act == 'cat') {
                ...
                $smarty->assign('new_goods',
                    brand_recommend_goods('new', $brand_id, $cate, $region_id, $area_id, $area_city, $act));
                $goodslist = brand_get_goods($brand_id, $cate, $size, $page, $sort, $order, $region_id, $area_id,
                    $area_city, $act, $is_ship, $price_min, $price_max, $is_self);
                $count = goods_count_by_brand($brand_id, $cate, $act, $is_ship, $price_min, $price_max, $region_id,
                    $area_id, $is_self);
                assign_pager('brandn', $cate, $count, $size, $sort, $order, $page, '', $brand_id, $price_min,
                    $price_max, $display, '', '', '', 0, '', '', $act, $is_ship, $is_self);
                $smarty->assign('goods_list', $goodslist);
                $seo = get_seo_words('brand');

                foreach ($seo as $key => $value) {
                    $seo[$key] = str_replace(array('{sitename}', '{description}', '{name}'),
                        array($position['title'], $brand_info['brand_desc'], $brand_info['brand_name']), $value);
                }
                ...
                if ($act == 'change_index') {
                    ...
                    $best_rand = brand_recommend_goods('best', $brand_id, $cate, $region_id, $area_id, $area_city, $act,
                        'rand');
                    ...
                    if ($act == 'collect') {
                            ...
                            if ($script_name != '') {
                                if ($script_name == 'category') {
                                    ...
                                    if ($script_name == 'search' || $script_name == 'merchants_shop') {
                                        ...
                                        if ($script_name == 'merchants_store_shop') {
                        ...
                        if ($act == 'cancel') {
                            ...
                            if ($type == 0) {
                                ...
                                if ($type == 1) {
                                    if ($GLOBALS['db']->query($sql) === false) {
                                        show_message($GLOBALS['db']->errorMsg(), $GLOBALS['_LANG']['back'], $ecs->url,
                                            'error');
                                    } else {
                            ...
                            if ($act == 'get_brand_cat_goods') {
                                include_once 'includes/cls_json.php';
                                $json = new JSON();
                                $result = array('error' => 0, 'content' => '');
                                $goodslist = brand_get_goods($brand_id, $cate, $size, $page, $sort, $order, $region_id,
                                    $area_id, $area_city, $act, $is_ship, $price_min, $price_max, $is_self);
                                $count = goods_count_by_brand($brand_id, $cate, $act, $is_ship, $price_min, $price_max,
                                    $region_id, $area_id, $is_self);
                                assign_pager('brandn', $cate, $count, $size, $sort, $order, $page, '', $brand_id,
                                    $price_min, $price_max, $display, '', '', '', 0, '', '', $act, $is_ship, $is_self);
                                $smarty->assign('goods_list', $goodslist);
