...
$code_config = array(
    'imageW' => $captcha_width,
    'imageH' => $captcha_height,
...
if (isset($_REQUEST['captcha'])) {
    if ($_REQUEST['captcha'] == 'is_common') {
        ...
        if ($_REQUEST['captcha'] == 'is_login') {
            ...
            if ($_REQUEST['captcha'] == 'is_register_email') {
                $code_config['seKey'] = 'register_email';
            } else {
                if ($_REQUEST['captcha'] == 'is_register_phone') {
                    $code_config['seKey'] = 'mobile_phone';
                } else {
                    if ($_REQUEST['captcha'] == 'is_discuss') {
                        $code_config['seKey'] = 'captcha_discuss';
                    } else {
                        if ($_REQUEST['captcha'] == 'is_user_comment') {
                            $code_config['seKey'] = 'user_comment';
                        } else {
                            if ($_REQUEST['captcha'] == 'is_get_password') {
                                $code_config['seKey'] = 'get_password';
                            } else {
                                if ($_REQUEST['captcha'] == 'is_get_phone_password') {
                                    $code_config['seKey'] = 'get_phone_password';
                                } else {
                                    if ($_REQUEST['captcha'] == 'get_pwd_question') {
                                        $code_config['seKey'] = 'psw_question';
                                    } else {
                                        if ($_REQUEST['captcha'] == 'is_bonus') {
                                            $code_config['seKey'] = 'bonus';
                                        } else {
                                            if ($_REQUEST['captcha'] == 'is_value_card') {
                                                $code_config['seKey'] = 'value_card';
                                            } else {
                                                if ($_REQUEST['captcha'] == 'is_pay_card') {
                                                    $code_config['seKey'] = 'pay_card';
                                                } else {
                                                    if ($_REQUEST['captcha'] == 'admin_login') {
                                                        $code_config['seKey'] = 'admin_login';
                                                    } else {
                                                        if ($_REQUEST['captcha'] == 'change_password_s') {
                                                            $code_config['seKey'] = 'change_password_s';
                                                        } else {
                                                            if ($_REQUEST['captcha'] == 'change_password_f') {
