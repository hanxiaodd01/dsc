    ...
    if ($string !== false) {
        ...
        if ($string !== false) {
            ...
            if (@$string['keywords'] == '优惠券') {
                header('location:http://' . str_replace(strrchr($_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'], '/'), '',
                        $_SERVER['HTTP_HOST'] . $_SERVER['PHP_SELF'] . '/coupons.php?act=coupons_index'));
            }
        } else {
            $string = array();
        }
...
$display = isset($_REQUEST['display']) && in_array(trim(strtolower($_REQUEST['display'])), array(
    ...
    'grid',
    'text'
)) ? trim($_REQUEST['display']) : (isset($_SESSION['display_search']) ? $_SESSION['display_search'] : 'list');
...
if ($search_type == 1) {
    if ($display == 'list') {
        ...
        $sort = isset($_REQUEST['sort']) && in_array(trim(strtolower($_REQUEST['sort'])),
            array('shop_id', 'goods_number', 'sales_volume')) ? trim($_REQUEST['sort']) : $default_sort_order_type;
    } else {
...
if ($_REQUEST['act'] == 'advanced_search') {
    ...
    if ($search_type == 0) {
        $smarty->assign('best_goods',
            get_recommend_goods('best', '', $region_id, $area_info['region_id'], $area_city, 0, 1));
    ...
    if ($search_type == 0) {
        ...
        if (1 < $cat['grade']) {
            ...
            foreach ($price_grade as $key => $val) {
                if ($val['sn'] != '') {
                    ...
                    $price_grade[$temp_key]['url'] = $brands[$temp_key]['url'] = build_uri('search', array(
                        'bid' => $brand,
                        'chkw' => $_REQUEST['keywords'],
        ...
        if ($children) {
            $brands_list_children = '(' . $children . ' OR ' . 'gc.cat_id ' . db_create_in(array_unique(array_merge(array($cat_id),
                    $cat_keys))) . ') AND';
        } else {
        ...
        foreach ($brands_list as $key => $val) {
            ...
            $brands[$temp_key]['url'] = build_uri('search', array(
                'bid' => $val['brand_id'],
                'chkw' => $_REQUEST['keywords'],
                'price_min' => $price_min,
                'price_max' => $price_max,
        ...
        if (empty($g_price) && (0 < $price_min || 0 < $price_max)) {
            ...
            $parray = array();
            $parray['purl'] = build_uri('search', array(
                'bid' => $brand,
                'chkw' => $_REQUEST['keywords'],
                'price_min' => 0,
                'price_max' => 0,
        ...
        while ($row = $db->FetchRow($res)) {
            ...
            if ($row['act_id']) {
                ...
                $arr[$idx]['purl'] = build_uri('presale', array('act' => 'view', 'presaleid' => $row['act_id']),
                    $row['goods_name']);
                ...
                foreach ($arr_keyword as $key => $val_keyword) {
                    $act_name_keyword = preg_replace('/(>.*)(' . $val_keyword . ')(.*<)/Ui',
                        '$1<font style=\'color:#ec5151;\'>' . $val_keyword . '</font>$3', $act_name_keyword);
                }
            ...
            if ($display == 'grid') {
                ...
                foreach ($arr_keyword as $key => $val_keyword) {
                    if (preg_match('/(>.*)(' . $val_keyword . ')(.*<)/Ui', $goods_name_keyword)) {
                        $goods_name_keyword = preg_replace('/(>.*)(' . $val_keyword . ')(.*<)/Ui',
                            '$1<font style=\'color:#ec5151;\'>' . $val_keyword . '</font>$3', $row['goods_name']);
                    }
                }
                ...
                foreach ($arr_keyword as $key => $val_keyword) {
                    if (preg_match('/(>.*)(' . $val_keyword . ')(.*<)/Ui', $goods_name_keyword)) {
                        $goods_name_keyword = preg_replace('/(>.*)(' . $val_keyword . ')(.*<)/Ui',
                            '$1<font style=\'color:#ec5151;\'>' . $val_keyword . '</font>$3', $goods_name_keyword);
                    }
                }
            ...
            if (!defined('THEME_EXTENSION')) {
                $shippingFee = goodsShippingFee($row['goods_id'], $region_id, $area_info['region_id'], $area_city,
                    $region);
        ...
        $smarty->assign('guess_goods',
            get_guess_goods($_SESSION['user_id'], 1, 1, 7, $region_id, $area_info['region_id'], $area_city));
        ...
        if ($search_type == 1) {
            ...
            if ($display == 'list') {
                ...
                $shop_count = $count;
                $size = 10;
                $store_shop_list = get_store_shop_list(1, $keywords, $count, $size, $page, $sort, $order, $region_id,
                    $area_id, $area_city);
                ...
                if ($display == 'grid' || $display == 'text') {
                    ...
                    $shop_goods_list = get_store_shop_goods_list($keywords, $size, $page, $sort, $order, $region_id,
                        $area_id, $area_city);
