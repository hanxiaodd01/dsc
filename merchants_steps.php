...
if ($step == 'addChildCate') {
    ...
    if ($step == 'addChildCate_checked') {
        ...
        if ($step == 'deleteChildCate_checked') {
...
if (!$smarty->is_cached('merchants_steps.dwt')) {
    ...
    if (1 < $sid && $sid < 4) {
        ...
        if ($process['process_title'] == $_LANG['07_brand_add']) {
            ...
            if ($process['process_title'] == $_LANG['new_brand']) {
                $smarty->assign('pid_key', $pid_key - 1);
            } else {
                $smarty->assign('pid_key', $pid_key + 1);
            }
        ...
        if (0 < $process['id']) {
            ...
            $sql = 'SELECT COUNT(*) FROM ' . $ecs->table('merchants_steps_process') . ' WHERE process_title ' . db_create_in(array(
                    '添加品牌',
                    '添加品牌'
                )) . ' AND is_show = 1';
