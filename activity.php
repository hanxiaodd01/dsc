...
function get_range_gift_list($act_id, $type)
{
    ...
    if ($type == 1) {
        ...
        if ($type == 0) {
            ...
            foreach ($res as $k => $v) {
                $arr[$k] = $v;
                $arr[$k]['thumb'] = get_image_path($v['id'],
                    $GLOBALS['db']->getOne('SELECT goods_thumb FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE goods_id = \'' . $v['id'] . '\''),
                    true);
                $arr[$k]['url'] = build_uri('goods', array('gid' => $v['id']), $v['name']);
            }
        }
    ...
    if (defined('THEME_EXTENSION')) {
        ...
        if ($act == 'view') {
            ...
            if ($GLOBALS['_CFG']['region_store_enabled']) {
                $filter['mer_ids'] = get_favourable_merchants($row['userFav_type'], $row['userFav_type_ext'],
                    $row['rs_id']);
            }
            ...
            switch ($row['act_type']) {
                case 0:
                    ...
                    if (is_array($row['gift'])) {
                        foreach ($row['gift'] as $k => $v) {
                            $row['gift'][$k]['thumb'] = get_image_path($v['id'],
                                $db->getOne('SELECT goods_thumb FROM ' . $ecs->table('goods') . ' WHERE goods_id = \'' . $v['id'] . '\''),
                                true);
                        }
                    }
            ...
            $pager = get_pager('activity.php', array('act' => 'view', 'act_id' => $act_id),
                $filter_array['record_count'], $page, $size);
