...
class OssClientMultipartUploadTest extends TestOssClientBase
{
    public function testInvalidDir()
    {
        try {
            ...
            $this->assertEquals('parameter error: abc/ds/s/s/notexitst is not a directory, please check it',
                $e->getMessage());
        }
    }
    ...
    public function testMultipartUploadBigFile()
    {
        ...
        try {
            $this->ossClient->multiuploadFile($this->bucket, $object, $bigFileName,
                array(\OSS\OssClient::OSS_PART_SIZE => 1));
    ...
    public function testCopyPart()
    {
        ...
        $copyId = 1;
        $eTag = $this->ossClient->uploadPartCopy($this->bucket, $copiedObject, $this->bucket, $object, $copyId,
            $upload_id);
    ...
    public function testAbortMultipartUpload()
    {
        ...
        foreach ($pieces as $i => $piece) {
            ...
            $up_options = array(
                \OSS\OssClient::OSS_FILE_UPLOAD => $upload_file,
                \OSS\OssClient::OSS_PART_NUM => $i + 1,
                \OSS\OssClient::OSS_SEEK_TO => $from_pos,
                \OSS\OssClient::OSS_LENGTH => ($to_pos - $from_pos) + 1,
                \OSS\OssClient::OSS_CHECK_MD5 => $is_check_md5
            );
        ...
        try {
            $listMultipartUploadInfo = $listMultipartUploadInfo = $this->ossClient->listMultipartUploads($this->bucket,
                $options);
    ...
    public function testPutObjectsByDir()
    {
        ...
        sleep(1);
        $this->assertTrue($this->ossClient->doesObjectExist($this->bucket,
            'samples/codes/' . 'OssClientMultipartUploadTest.php'));
    }
