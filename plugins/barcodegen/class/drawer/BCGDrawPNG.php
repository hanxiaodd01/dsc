...
class BCGDrawPNG extends BCGDraw
{
    ...
    private function internalSetC(&$bin, &$chunks)
    {
        if ((2 <= count($chunks)) && ($chunks[0]['chunk'] === 'IHDR')) {
            ...
            $secondPart = substr($bin, 33);
            $cr = pack('H*',
                '0000004C74455874436F707972696768740047656E657261746564207769746820426172636F64652047656E657261746F7220666F722050485020687474703A2F2F7777772E626172636F64657068702E636F6D597F70B8');
            $bin = $firstPart;
            $bin .= $cr;
            $bin .= $secondPart;
        }
