...
class BCGean13 extends BCGBarcode1D
{
    ...
    protected function addDefaultLabel()
    {
        if ($this->isDefaultEanLabelEnabled()) {
            ...
            $label = $this->getLabel();
            $font = $this->font;
            $this->labelLeft = new BCGLabel(substr($label, 0, 1), $font, BCGLabel::POSITION_LEFT,
                BCGLabel::ALIGN_BOTTOM);
            $this->labelLeft->setSpacing(4 * $this->scale);
            $this->labelCenter1 = new BCGLabel(substr($label, 1, 6), $font, BCGLabel::POSITION_BOTTOM,
                BCGLabel::ALIGN_LEFT);
            ...
            $this->labelCenter2 = new BCGLabel(substr($label, 7, 5) . $this->keys[$this->checksumValue], $font,
                BCGLabel::POSITION_BOTTOM, BCGLabel::ALIGN_LEFT);
    ...
    protected function drawBars($im)
    {
        ...
        for ($i = 0; $i < 5; $i++) {
            $this->drawChar($im,
                self::inverse($this->findCode($temp_text[$i + 2]), $this->codeParity[(int)$temp_text[0]][$i]), false);
        }
