...
class BCGgs1128 extends BCGcode128
{
    ...
    private function formatGs1128()
    {
        ...
        for ($i = 0; $i < $c; $i++) {
            ...
            if (isset($this->identifiersAi[$this->identifiersId[$i]])) {
                ...
                if (isset($this->identifiersId[$i][3])) {
        ...
        if (($this->noLengthLimit === false) && (self::MAX_GS1128_CHARS < (strlen(str_replace('~F1', chr(29),
                        $formatedText)) - 1))) {
            throw new BCGParseException('gs1128',
                'The barcode can\'t contain more than ' . self::MAX_GS1128_CHARS . ' characters.');
        }
    ...
    private function parseContent($text)
    {
        ...
        $id = ($isFormated ? $this->findIdFormated($id, $yAlreadySet, $realNameId) : $this->findIdNotFormated($id,
            $yAlreadySet, $realNameId));
    ...
    private function checkConformity(&$content, $id, $realNameId)
    {
        ...
        if ($nbCharContent < ($minlengthContent - $checksumChar)) {
            if ($minlengthContent === $maxlengthContent) {
                throw new BCGParseException('gs1128',
                    'The value of "' . $id . '" must contain ' . $minlengthContent . ' character(s).');
            } else {
                throw new BCGParseException('gs1128',
                    'The value of "' . $id . '" must contain between ' . $minlengthContent . ' and ' . $maxlengthContent . ' character(s).');
            }
        }
