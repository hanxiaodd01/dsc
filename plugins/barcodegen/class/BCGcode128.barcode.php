...
class BCGcode128 extends BCGBarcode1D
{
                    ...
                    if ($text[$pos + 1] === 'F') {
                        if (isset($text[$pos + 2])) {
                            ...
                            if ((1 <= $v) && ($v <= 4)) {
                                ...
                                throw new BCGParseException('code128',
                                    'Bad ~F. You must provide a number from 1 to 4.');
                            }
                        } else {
        ...
        if ($this->tilde) {
            ...
            while (($pos = strpos($text, '~', $previousPos)) !== false) {
                ...
                if ($simpleTilde && ($currentMode !== 'B')) {
                    throw new BCGParseException('code128',
                        'The Table ' . $currentMode . ' doesn\'t contain the character ~.');
                }

                if (($tildeData !== '~F1') && ($currentMode === 'C')) {
                    throw new BCGParseException('code128',
                        'The Table C doesn\'t contain the function ' . $tildeData . '.');
                }
    ...
    private function getSequence($text, &$starting_text)
    {
        ...
        for ($x = 0; $x < $xLen; $x++) {
            ...
            if ($this->tilde && ($input === '~')) {
                ...
                if ($nextNumber || (($pos !== false) && isset($text[$x + 1]) && (strpos($this->keysC,
                                $text[$x + 1]) !== false))) {
    ...
    private function createBinaryStream($text, $seq)
    {
        ...
        if ($this->starting_text === 'A') {
            ...
            if ($this->starting_text === 'B') {
                $currentEncoding = 1;
                $indcheck[] = self::KEY_STARTB;
                $this->lastTable = 'B';
            } else {
                if ($this->starting_text === 'C') {
                    $currentEncoding = 2;
