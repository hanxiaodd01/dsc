...
abstract class BCGBarcode
{
            ...
            if (($label->getPosition() === BCGLabel::POSITION_LEFT) || ($label->getPosition() === BCGLabel::POSITION_RIGHT)) {
                ...
                if ($alignment === BCGLabel::ALIGN_LEFT) {
                    ...
                    if ($alignment === BCGLabel::ALIGN_CENTER) {
                        ...
                        if ($alignment === BCGLabel::ALIGN_RIGHT) {
    ...
    {
        foreach ($this->labels as $label) {
            $label->draw($im, (($x1 + $this->offsetX) * $this->scale) + $this->pushLabel[0],
    ...
    protected function drawPixel($im, $x, $y, $color = self::COLOR_FG)
    {
        ...
        imagefilledrectangle($im, $xR, $yR, ($xR + $this->scale) - 1, ($yR + $this->scale) - 1,
    ...
    {
        if ($this->scale === 1) {
            imagefilledrectangle($im, $x1 + $this->offsetX + $this->pushLabel[0],
                $y1 + $this->offsetY + $this->pushLabel[1], $x2 + $this->offsetX + $this->pushLabel[0],
                $y2 + $this->offsetY + $this->pushLabel[1], $this->getColor($im, $color));
        } else {
            imagefilledrectangle($im, (($x1 + $this->offsetX) * $this->scale) + $this->pushLabel[0],
                (($y1 + $this->offsetY) * $this->scale) + $this->pushLabel[1],
            ...
            imagefilledrectangle($im, (($x1 + $this->offsetX) * $this->scale) + $this->pushLabel[0],
                ...
                $this->getColor($im, $color));
            imagefilledrectangle($im, (($x2 + $this->offsetX) * $this->scale) + $this->pushLabel[0],
                (($y1 + $this->offsetY) * $this->scale) + $this->pushLabel[1],
                ((($x2 + $this->offsetX) * $this->scale) + $this->pushLabel[0] + $this->scale) - 1,
                ((($y2 + $this->offsetY) * $this->scale) + $this->pushLabel[1] + $this->scale) - 1,
                $this->getColor($im, $color));
        ...
        foreach ($this->labels as $label) {
            ...
            if (isset($labels[$position])) {
                ...
                if (($position === BCGLabel::POSITION_LEFT) || ($position === BCGLabel::POSITION_RIGHT)) {
                    ...
                    if ($savedDimension[$searchTB] < $dimension[$searchTB]) {
                        $labels[$position] = $label;
                    }
                }
            } else {
