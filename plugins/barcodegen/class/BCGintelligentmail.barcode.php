...
class BCGintelligentmail extends BCGBarcode1D
{
    ...
    static private $characterTable2 = array(
        3,
        6144,
        5,
        5120,
    ...
    public function setTrackingCode($barcodeIdentifier, $serviceTypeIdentifier, $mailerIdentifier, $serialNumber)
    {
        ...
        if (($barcodeIdentifierSecondNumber !== '0') && ($barcodeIdentifierSecondNumber !== '1') && ($barcodeIdentifierSecondNumber !== '2') && ($barcodeIdentifierSecondNumber !== '3') && ($barcodeIdentifierSecondNumber !== '4')) {
            throw new BCGArgumentException('Barcode Identifier second digit must be a number between 0 and 4.',
                'barcodeIdentifier');
        }

        if (($serviceTypeIdentifier < 0) || (999 < $serviceTypeIdentifier)) {
            throw new BCGArgumentException('Service Type Identifier must be between 0 and 999.',
        ...
        if (($mailerIdentifierLength === 9) && (6 < strlen($serialNumber))) {
            throw new BCGArgumentException('If the Serial Number has more than 6 digits, the Mailer Identifier must be lower than 900000.',
                'mailerIdentifier');
        }

        if ($mailerIdentifierLength === 9) {
            if (($mailerIdentifierLength < 0) || (999999999 < $mailerIdentifier)) {
                throw new BCGArgumentException('Mailer Identifier must be between 0 and 999999999.',
                    'mailerIdentifier');
            }
        }
    ...
    {
        parent::parse($text);
        $number = self::executeStep1($this->text, $this->barcodeIdentifier, $this->serviceTypeIdentifier,
            $this->mailerIdentifier, $this->serialNumber);
    ...
    protected function validate()
    {
        if (($this->barcodeIdentifier === null) || ($this->serviceTypeIdentifier === null) || ($this->mailerIdentifier === null) || ($this->serialNumber === null)) {
            throw new BCGParseException('intelligentmail',
                'The tracking code must be set before calling the parse method.');
        }
    ...
    static private function executeStep1(
        $text,
        $barcodeIdentifier,
        $serviceTypeIdentifier,
        $mailerIdentifier,
        ...
        $number = self::conversionTrackingCode($number, $barcodeIdentifier, $serviceTypeIdentifier, $mailerIdentifier,
            $serialNumber);
        return $number;
    }
    ...
    static private function conversionTrackingCode(
        $number,
        $barcodeIdentifier,
        $serviceTypeIdentifier,
        $mailerIdentifier,
