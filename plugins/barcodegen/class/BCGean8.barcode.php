...
class BCGean8 extends BCGBarcode1D
{
    ...
    protected function addDefaultLabel()
    {
        if ($this->isDefaultEanLabelEnabled()) {
            ...
            $label = $this->getLabel();
            $font = $this->font;
            $this->labelLeft = new BCGLabel(substr($label, 0, 4), $font, BCGLabel::POSITION_BOTTOM,
                BCGLabel::ALIGN_LEFT);
            $labelLeftDimension = $this->labelLeft->getDimension();
            $this->labelLeft->setOffset(((($this->scale * 30) - $labelLeftDimension[0]) / 2) + ($this->scale * 2));
            $this->labelRight = new BCGLabel(substr($label, 4, 3) . $this->keys[$this->checksumValue], $font,
