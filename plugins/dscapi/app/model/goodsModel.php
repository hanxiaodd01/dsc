...
abstract class goodsModel extends \app\func\common
{
    ...
    public function get_select_list($table, $select, $where, $page_size, $page, $sort_by, $sort_order)
    {
        ...
        if ($table == 'warehouse_area_goods') {
            if (strpos($where, 'province_name') !== false) {
                ...
                if (strpos($where, 'city_name') !== false) {
                        ...
                        if ($table == $config['gll']) {
                            ...
                            if ($table == $config['ga']) {
                                ...
                                if ($table == $config['gtt']) {
                                    ...
                                    if ($table == $config['gtes']) {
                                        $info['id'] = $id;
                                    } else {
                                        if ($table == $config['gted']) {
                                            $info['id'] = $id;
                                        }
                                    }
                                }
            ...
            foreach ($select as $k => $v) {
                ...
                if (0 < $goods_id) {
                    ...
                    if (!empty($goods_gallery_list)) {
                        foreach ($goods_gallery_list as $gallery_list_key => $gallery_list_val) {
                            if ($gallery_list_val['img_original'] != '' && strpos($gallery_list_val['img_original'],
                                    'http://') === false && strpos($gallery_list_val['img_original'],
                                    'https://') === false) {
                                @unlink(ROOT_PATH . $gallery_list_val['img_original']);
                            }

                            if ($gallery_list_val['img_url'] != '' && strpos($gallery_list_val['img_url'],
                                    'http://') === false && strpos($gallery_list_val['img_url'],
                                    'https://') === false) {
                                @unlink(ROOT_PATH . $gallery_list_val['img_url']);
                            }

                            if ($gallery_list_val['thumb_url'] != '' && strpos($gallery_list_val['thumb_url'],
                                    'http://') === false && strpos($gallery_list_val['thumb_url'],
                                    'https://') === false) {
                ...
                if ($v['goodsDetailList']) {
                    ...
                    foreach ($v['goodsDetailList'] as $detail_key => $detail_val) {
                        if (!empty($detail_val['imagePath']) && $detail_val['imagePath'] != 'http://' && (strpos($detail_val['imagePath'],
                                    'http://') !== false || strpos($detail_val['imagePath'], 'https://') !== false)) {
                ...
                if ($v['goodsNavigateList']) {
                    ...
                    foreach ($v['goodsNavigateList'] as $gallery_key => $gallery_val) {
                        $i++;
                        if (!empty($gallery_val['navigateImage']) && $gallery_val['navigateImage'] != 'http://' && (strpos($gallery_val['navigateImage'],
                                    'http://') !== false || strpos($gallery_val['navigateImage'],
                                    'https://') !== false)) {
                            if (\app\func\common::get_http_basename($gallery_val['navigateImage'], $admin_temp_dir)) {
                                ...
                                $img_wh = \app\func\common::get_width_to_height($down_img,
                                ...
                                $goods_img = \app\func\common::make_thumb(array('img' => $down_img, 'type' => 1),
                                    $GLOBALS['_CFG']['image_width'], $GLOBALS['_CFG']['image_height']);

                                if ($proc_thumb) {
                                    $thumb_url = \app\func\common::make_thumb(array('img' => $down_img, 'type' => 1),
                                        $GLOBALS['_CFG']['thumb_width'], $GLOBALS['_CFG']['thumb_height']);
                                    $thumb_url = \app\func\common::reformat_image_name('gallery_thumb', $id, $thumb_url,
                                        'thumb');
                                } else {
                                    $thumb_url = \app\func\common::make_thumb(array('img' => $down_img, 'type' => 1));
                                    $thumb_url = \app\func\common::reformat_image_name('gallery_thumb', $id, $thumb_url,
                                ...
                                $img_original = \app\func\common::reformat_image_name('gallery', $id, $down_img,
                                    'source');
                                ...
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_gallery'), $goods_gallery,
                                    'INSERT');
                                ...
                                if ($i == 1) {
                                    ...
                                    $sql_where = ' goods_id = \'' . $id . '\'';
                                    $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table($table), $goods_arr, 'UPDATE',
                                        $sql_where);
                                }
                    ...
                    if (!empty($specDesc)) {
                        foreach ($specDesc as $specDesc_key => $specDesc_val) {
                            ...
                            if ($attr_id == 0 || $attr_id == '') {
                                $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('attribute'), $specDesc_arr,
                                    'INSERT');
                            ...
                            if (!empty($specDesc_val['value'])) {
                                foreach ($specDesc_val['value'] as $value_key => $value_val) {
                                    ...
                                    if ($goods_attr_id == 0 || $goods_attr_id == '') {
                                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('goods_attr'), $value_arr,
                                            'INSERT');
                    ...
                    if (!empty($order)) {
                        foreach ($order as $k => $v) {
                            $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'),
                                array('order_status' => OS_INVALID), 'UPDATE', 'order_id = \'' . $v['order_id'] . '\'');
                            $log_time = gmtime();
    ...
    public function return_user_surplus_integral_bonus($order)
    {
        if (0 < $order['user_id'] && 0 < $order['surplus']) {
            ...
            \app\func\common::log_account_change($order['user_id'], $surplus, 0, 0, 0,
                '由于取消、无效或退货操作，退回支付订单 ' . $order['order_sn'] . ' 时使用的预付款', ACT_OTHER);
        ...
        if (0 < $order['user_id'] && 0 < $order['integral']) {
            \app\func\common::log_account_change($order['user_id'], 0, 0, 0, $order['integral'],
                '由于取消、无效或退货操作，退回支付订单' . $order['order_sn'] . ' 时使用的积分', ACT_OTHER);
        }
        ...
        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('order_info'), $arr, 'UPDATE',
