...
abstract class orderModel extends \app\func\common
{
    ...
    public function get_where($val = array(), $alias = '')
    {
        ...
        if ($val['seller_id'] != -1) {
            $conditions .= ' AND ' . $alias . '.ru_id ' . \app\func\base::db_create_in($val['seller_id']);
    ...
    public function get_confirmorder($table, $order_select, $format)
    {
        ...
        if (!empty($order_select)) {
            ...
            $query = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('delivery_order'), $_delivery, 'INSERT', '',
                'SILENT');
            ...
            if (0 < $delivery_id) {
                ...
                if (!empty($goods_list)) {
                    ...
                    foreach ($goods_list as $key => $val) {
                        ...
                        $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('delivery_goods'), $delivery_goods, 'INSERT',
                            '', 'SILENT');
                        $split_action_note .= sprintf($orderLang['conf_message']['split_action_note'], $val['goods_sn'],
                                $val['goods_number']) . '<br/>';
        ...
        if ($delivery_order) {
            ...
            $delivery_order['formated_insure_fee'] = \app\func\common::price_format($delivery_order['insure_fee'],
                false);
            $delivery_order['formated_shipping_fee'] = \app\func\common::price_format($delivery_order['shipping_fee'],
                false);
            $delivery_order['formated_add_time'] = local_date($GLOBALS['_CFG']['time_format'],
                $delivery_order['add_time']);
            $delivery_order['formated_update_time'] = local_date($GLOBALS['_CFG']['time_format'],
                $delivery_order['update_time']);
        }
        ...
        $_delivery['status'] = 0;
        $query = $GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('delivery_order'), $_delivery, 'UPDATE',
        ...
        if ($order_finish) {
            if (0 < $order['user_id']) {
                ...
                \app\func\common::log_account_change($order['user_id'], 0, 0, intval($integral['rank_points']),
                    intval($integral['custom_points']),
                    sprintf($orderLang['conf_message']['order_gift_integral'], $order['order_sn']));
            ...
            if ($cfg == '1') {
                ...
                $GLOBALS['smarty']->assign('confirm_url',
                    $GLOBALS['ecs']->url() . 'user.php?act=order_detail&order_id=' . $order['order_id']);
                $GLOBALS['smarty']->assign('send_msg_url',
                    $GLOBALS['ecs']->url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
