...
class attribute extends \app\model\attributeModel
{
    ...
    public function get_goodstype_list($table)
    {
        ...
        $result = \app\model\attributeModel::get_select_list($this->table, $this->select, $this->where,
            $this->page_size, $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\attributeModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->attributeLangList, $this->format);
        return $result;
    }
    ...
    public function get_goodstype_info($table)
    {
        ...
        if (strlen($this->where) != 1) {
            $result = \app\model\attributeModel::get_info_common_data_fs($result, $this->attributeLangList,
                $this->format);
        } else {
    ...
    public function get_goodstype_insert($table)
    {
        ...
        if ($select) {
            if (!isset($select['cat_id'])) {
                if (isset($select['cat_name']) && !empty($select['cat_name'])) {
                    ...
                    if (!$info) {
                        return \app\model\attributeModel::get_insert($this->table, $this->attribute_select,
                            $this->format);
                    } else {
        ...
        if (in_array($error, array(
            \languages\attributeLang::INSERT_CANNOT_PARAM_FAILURE,
            \languages\attributeLang::INSERT_KEY_PARAM_FAILURE
        ))) {
            $attributeLang['msg_failure'][$error]['failure'] = sprintf($attributeLang['msg_failure'][$error]['failure'],
                $string);
        }
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($cat_name) {
                            ...
                            return \app\model\attributeModel::get_update($this->table, $this->attribute_select,
