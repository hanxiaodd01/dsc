...
class goods extends \app\model\goodsModel
{
    ...
    public function get_goods_list($table)
    {
        ...
        $result = \app\model\goodsModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\goodsModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->goodsLangList, $this->format);
        return $result;
    }
    ...
    public function get_goods_insert($table)
    {
                ...
                if (isset($select['goods_sn']) && !empty($select['goods_sn'])) {
                    ...
                    if (!$info) {
                        if (isset($select['goods_name']) && !empty($select['goods_name'])) {
                            ...
                            if (!$info) {
                                return \app\model\goodsModel::get_insert($this->table, $this->goods_select,
                                    $this->format);
                            } else {
        ...
        if (in_array($error,
            array(\languages\goodsLang::INSERT_CANNOT_PARAM_FAILURE, \languages\goodsLang::INSERT_KEY_PARAM_FAILURE))) {
            $goodsLang['msg_failure'][$error]['failure'] = sprintf($goodsLang['msg_failure'][$error]['failure'],
                $string);
        }
                    ...
                    if (!$info) {
                        ...
                        if ($goods_sn) {
                            ...
                            if ($goods_name) {
                                ...
                                return \app\model\goodsModel::get_update($this->table, $this->goods_select,
                                    $this->where, $this->format, $info);
                            }
                        }
                    }
                } else {
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($region_sn) {
                            ...
                            return \app\model\goodsModel::get_update($this->table, $this->goods_select, $this->where,
                                $this->format, $info);
                        }
                    }
                } else {
                    ...
                    if (!$info) {
                        ...
                        if ($is_region == 1) {
                            if ($region_sn) {
                                ...
                                return \app\model\goodsModel::get_update($this->table, $select, $this->where,
                                    $this->format, $info);
                            }
                        } else {
            ...
            if (!isset($select['tid'])) {
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        return \app\model\goodsModel::get_more_update($this->table, $this->goods_select, $this->where,
                            $this->format, $info);
                    }
                } else {
                    $error = \languages\goodsLang::UPDATE_NULL_PARAM_FAILURE;
