...
class order extends \app\model\orderModel
{
    ...
    public function get_order_list($table)
    {
        ...
        $result = \app\model\orderModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order, $this->alias);
        $result = \app\model\orderModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->orderLangList, $this->format);
        return $result;
    }
    ...
    public function get_order_insert($table)
    {
        ...
        if (in_array($error,
            array(\languages\orderLang::INSERT_CANNOT_PARAM_FAILURE, \languages\orderLang::INSERT_KEY_PARAM_FAILURE))) {
            $orderLang['msg_failure'][$error]['failure'] = sprintf($orderLang['msg_failure'][$error]['failure'],
                $string);
        }
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($order_sn) {
                            ...
                            return \app\model\orderModel::get_update($this->table, $this->order_select, $this->where,
                                $this->format, $info);
                        }
                    }
                } else {
