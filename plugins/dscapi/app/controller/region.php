...
class region extends \app\model\regionModel
{
    ...
    public function get_region_list($table)
    {
        ...
        $result = \app\model\regionModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\regionModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->regionLangList, $this->format);
        return $result;
    }
    ...
    public function get_region_insert($table)
    {
            ...
            if (!isset($select['region_id'])) {
                if (isset($select['region_name']) && !empty($select['region_name'])) {
                    if (isset($select['region_name']) && !empty($select['region_name'])) {
                        ...
                        if (!$info) {
                            return \app\model\regionModel::get_insert($this->table, $this->region_select,
                                $this->format);
                        } else {
        ...
        if (in_array($error, array(
            \languages\regionLang::INSERT_CANNOT_PARAM_FAILURE,
            \languages\regionLang::INSERT_KEY_PARAM_FAILURE
        ))) {
            $regionLang['msg_failure'][$error]['failure'] = sprintf($regionLang['msg_failure'][$error]['failure'],
                $string);
        }
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($region_name) {
                            ...
                            return \app\model\regionModel::get_update($this->table, $this->region_select, $this->where,
                                $this->format, $info);
                        }
                    }
                } else {
