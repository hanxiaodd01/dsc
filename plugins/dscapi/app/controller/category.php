...
class category extends \app\model\categoryModel
{
    ...
    public function get_category_list($table)
    {
        ...
        $result = \app\model\categoryModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\categoryModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->categoryLangList, $this->format);
        return $result;
    }
    ...
    public function get_category_info($table)
    {
        ...
        if (strlen($this->where) != 1) {
            $result = \app\model\categoryModel::get_info_common_data_fs($result, $this->categoryLangList,
                $this->format);
        } else {
    ...
    public function get_category_insert($table)
    {
        ...
        if ($select) {
            if (!isset($select['cat_id'])) {
                if (isset($select['cat_name']) && !empty($select['cat_name'])) {
                    ...
                    if (!$info) {
                        return \app\model\categoryModel::get_insert($this->table, $this->category_select,
                            $this->format);
                    } else {
        ...
        if (in_array($error, array(\languages\categoryLang::INSERT_CANNOT_PARAM_FAILURE))) {
            $categoryLang['msg_failure'][$error]['failure'] = sprintf($categoryLang['msg_failure'][$error]['failure'],
                $string);
        }
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($cat_name) {
                            ...
                            return \app\model\categoryModel::get_update($this->table, $this->category_select,
