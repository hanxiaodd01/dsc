...
class connect extends \app\model\connectModel
{
    ...
    public function get_connect_list($table)
    {
        ...
        $result = \app\model\connectModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\connectModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->connectLangList, $this->format);
        return $result;
    }
    ...
    public function get_connect_insert($table)
    {
        ...
        if (in_array($error, array(
            \languages\connectLang::INSERT_CANNOT_PARAM_FAILURE,
            \languages\connectLang::INSERT_KEY_PARAM_FAILURE
        ))) {
            $connectLang['msg_failure'][$error]['failure'] = sprintf($connectLang['msg_failure'][$error]['failure'],
                $string);
        }

        $common_data = array(
            'result' => 'failure',
            'msg' => $connectLang['msg_failure'][$error]['failure'],
            'error' => $connectLang['msg_failure'][$error]['error'],
            'format' => $this->format,
            'info' => $info
        );
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($openid) {
                            ...
                            return \app\model\connectModel::get_update($this->table, $this->connect_select,
