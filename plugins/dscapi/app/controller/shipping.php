...
class shipping extends \app\model\shippingModel
{
    ...
    public function get_shipping_list($table)
    {
        ...
        $result = \app\model\shippingModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\shippingModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->shippingLangList, $this->format);
        return $result;
    }
    ...
    public function get_shipping_details($table)
    {
        ...
        if (strlen($this->where) != 1) {
            $result = \app\model\shippingModel::get_info_common_data_fs($result, $this->shippingLangList,
                $this->format);
        } else {
    ...
    public function get_shipping_insert($table)
    {
                ...
                if (isset($select['shipping_code']) && !empty($select['shipping_code'])) {
                    ...
                    if (!$info) {
                        if (isset($select['shipping_name']) && !empty($select['shipping_name'])) {
                            ...
                            if (!$info) {
                                return \app\model\shippingModel::get_insert($this->table, $this->shipping_select,
                                    $this->format);
                            } else {
        ...
        if (in_array($error, array(
            \languages\shippingLang::INSERT_CANNOT_PARAM_FAILURE,
            \languages\shippingLang::INSERT_KEY_PARAM_FAILURE
        ))) {
            $shippingLang['msg_failure'][$error]['failure'] = sprintf($shippingLang['msg_failure'][$error]['failure'],
                $string);
        }
                    ...
                    if (!$info) {
                        ...
                        if ($shipping_code) {
                            ...
                            if ($shipping_name) {
                                ...
                                return \app\model\shippingModel::get_update($this->table, $this->shipping_select,
