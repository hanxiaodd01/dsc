...
class warehouse extends \app\model\warehouseModel
{
    ...
    public function get_warehouse_list($table)
    {
        ...
        $result = \app\model\warehouseModel::get_select_list($this->table, $this->select, $this->where,
            $this->page_size, $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\warehouseModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->warehouseLangList, $this->format);
        return $result;
    }
    ...
    public function get_warehouse_info($table)
    {
        ...
        if (strlen($this->where) != 1) {
            $result = \app\model\warehouseModel::get_info_common_data_fs($result, $this->warehouseLangList,
                $this->format);
        } else {
    ...
    public function get_warehouse_insert($table)
    {
                ...
                if (isset($select['region_code']) && !empty($select['region_code'])) {
                    ...
                    if (!$info) {
                        if (isset($select['region_name']) && !empty($select['region_name'])) {
                            ...
                            if (!$info) {
                                return \app\model\warehouseModel::get_insert($this->table, $this->warehouse_select,
                                    $this->format);
                            } else {
        ...
        if (in_array($error, array(
            \languages\warehouseLang::INSERT_CANNOT_PARAM_FAILURE,
            \languages\warehouseLang::INSERT_KEY_PARAM_FAILURE
        ))) {
            $warehouseLang['msg_failure'][$error]['failure'] = sprintf($warehouseLang['msg_failure'][$error]['failure'],
                $string);
        }
                    ...
                    if (!$info) {
                        ...
                        if ($region_code) {
                            ...
                            if ($region_name) {
                                ...
                                return \app\model\warehouseModel::get_update($this->table, $this->warehouse_select,
