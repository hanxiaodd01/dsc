...
class brand extends \app\model\brandModel
{
    ...
    public function get_brand_list($table)
    {
        ...
        $result = \app\model\brandModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\brandModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->brandLangList, $this->format);
        return $result;
    }
    ...
    public function get_brand_insert($table)
    {
        ...
        if (in_array($error,
            array(\languages\brandLang::INSERT_CANNOT_PARAM_FAILURE, \languages\brandLang::INSERT_KEY_PARAM_FAILURE))) {
            $brandLang['msg_failure'][$error]['failure'] = sprintf($brandLang['msg_failure'][$error]['failure'],
                $string);
        }
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($goods_sn) {
                            ...
                            return \app\model\brandModel::get_update($this->table, $this->brand_select, $this->where,
                                $this->format, $info);
                        }
                    }
                } else {
