...
class product extends \app\model\productModel
{
    ...
    public function get_product_list($table)
    {
        ...
        $result = \app\model\productModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\productModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->productLangList, $this->format);
        return $result;
    }
    ...
    public function get_product_insert($table)
    {
        ...
        if (in_array($error, array(
            \languages\productLang::INSERT_CANNOT_PARAM_FAILURE,
            \languages\productLang::INSERT_KEY_PARAM_FAILURE
        ))) {
            $productLang['msg_failure'][$error]['failure'] = sprintf($productLang['msg_failure'][$error]['failure'],
                $string);
        }
                ...
                if (strlen($this->where) != 1) {
                    ...
                    if (!$info) {
                        ...
                        if ($info) {
                            ...
                            return \app\model\productModel::get_update($this->table, $this->product_select,
