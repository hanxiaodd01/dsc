...
class user extends \app\model\userModel
{
    ...
    public function get_user_list($table)
    {
        ...
        $result = \app\model\userModel::get_select_list($this->table, $this->select, $this->where, $this->page_size,
            $this->page, $this->sort_by, $this->sort_order);
        $result = \app\model\userModel::get_list_common_data($result, $this->page_size, $this->page,
            $this->userLangList, $this->format);
        return $result;
    }
    ...
    public function get_user_insert($table)
    {
                ...
                if (isset($select['mobile_phone']) && !empty($select['mobile_phone'])) {
                    ...
                    if (!$info) {
                        if (isset($select['user_name']) && !empty($select['user_name'])) {
                            ...
                            if (!$info) {
                                return \app\model\userModel::get_insert($this->table, $this->user_select,
                                    $this->format);
                            } else {
        ...
        if (in_array($error,
            array(\languages\userLang::INSERT_CANNOT_PARAM_FAILURE, \languages\userLang::INSERT_KEY_PARAM_FAILURE))) {
                    ...
                    if (!$info) {
                        ...
                        if ($mobile_phone) {
                            ...
                            if ($user_name) {
                                ...
                                return \app\model\userModel::get_update($this->table, $this->user_select, $this->where,
                                    $this->format, $info);
                            }
                        }
                    }
