        ...
        if (!empty($str)) {
            ...
            if (count($ex_rec) <= 1) {
                if ($type == 1) {
                    $str = addslashes($str);
                    $str = strtolower($str);
                    if (strpos($str, 'updatexml') !== false || strpos($str, 'extractvalue') !== false || strpos($str,
                            'floor') !== false) {
                        $str = '';
                    } else {
                        if ((strpos($str, 'or') !== false || strpos($str, 'and') !== false) && !in_array($str,
                                $str_arr)) {
                            $str = '';
                        }
                    }
                ...
                foreach ($ex_rec as $key => $row) {
                    if ($type == 1) {
                        $row = addslashes($row);
                        $row = strtolower($row);
                        if (strpos($row, 'updatexml') !== false || strpos($row,
                                'extractvalue') !== false || strpos($row, 'floor') !== false) {
                            $row = '';
                        } else {
                            if ((strpos($row, 'or') !== false || strpos($row, 'and') !== false) && !in_array($str,
                                    $str_arr)) {
                                $row = '';
                            }
                        }
