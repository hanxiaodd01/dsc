    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (0 < $row['product_id']) {
            $products = get_warehouse_id_attr_number($row['goods_id'], $row['goods_attr_id'], $row['ru_id'],
                $row['warehouse_id'], $row['area_id'], $row['model_attr'], $store_id);
            ...
            if ($row['model_inventory'] == 1) {
                ...
                if ($row['model_inventory'] == 2) {
                    $row['storage'] = get_warehouse_area_goods($row['area_id'], $row['goods_id'],
                        'warehouse_area_goods');
                }
            }
        }
    ...
    foreach ($row as $key => $value) {
        if ($value['goods_attr'] != '') {
            ...
            foreach ($goods_attr_array as $_attr) {
                $goods_attr[] = sprintf($format[0], $_goods_attr[$_attr]['attr_name'],
                    $_goods_attr[$_attr]['attr_value'], $_goods_attr[$_attr]['attr_price']);
            }
...
if ($_REQUEST['act'] == 'list') {
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        if ($_REQUEST['act'] == 'deal_store_order') {
            ...
            if ($_REQUEST['act'] == 'pick_goods') {
                ...
                if ($_REQUEST['act'] == 'grab_order') {
                    ...
                    if (empty($store_order_info['store_id'])) {
                        $has_stock = true;
                        $goods_list = get_table_date('order_goods',
                            'order_id=\'' . $store_order_info['order_id'] . '\'',
                            array('goods_id', 'goods_attr_id', 'goods_number'), 1);

                        foreach ($goods_list as $key => $val) {
                            if (empty($val['goods_attr_id'])) {
                                $goods_number = get_table_date('store_goods',
                                    'store_id=\'' . $store_id . '\' AND goods_id=\'' . $val['goods_id'] . '\'',
                                    array('goods_number'), 2);
                            } else {
                                $goods_attr = str_replace(',', '|', $val['goods_attr_id']);
                                $goods_number = get_table_date('store_products',
                                    'store_id=\'' . $store_id . '\' AND goods_id=\'' . $val['goods_id'] . '\' AND goods_attr=\'' . $goods_attr . '\'',
                                    array('product_number'), 2);
                            }
                    ...
                    if ($_REQUEST['act'] == 'delivery') {
                        ...
                        if ($invoice_no) {
                            ...
                            if ($goods_list) {
                                foreach ($goods_list as $key => $goods_value) {
                                    ...
                                    if ($goods_value['extension_code'] == 'package_buy' && 0 < count($goods_value['package_goods_list'])) {
                                        $goods_list[$key]['package_goods_list'] = package_goods($goods_value['package_goods_list'],
                                            $goods_value['goods_number'], $goods_value['order_id'],
                                            $goods_value['extension_code'], $goods_value['goods_id']);
                            ...
                            $query = $db->autoExecute($ecs->table('delivery_order'), $_delivery, 'INSERT', '',
                                'SILENT');
                                            ...
                                            if ($value['extension_code'] == 'package_buy') {
                                                foreach ($value['package_goods_list'] as $pg_key => $pg_value) {
                                                    $delivery_pg_goods = array(
                                                        ...
                                                        'send_number' => $value['goods_number'],
                                                        'parent_id' => $value['goods_id'],
                                                        'extension_code' => $value['extension_code'],
                                                        'is_real' => $pg_value['is_real']
                                                    );
                                                    $query = $db->autoExecute($ecs->table('delivery_goods'),
                                                        $delivery_pg_goods, 'INSERT', '', 'SILENT');
                                                    $sql = 'UPDATE ' . $GLOBALS['ecs']->table('order_goods') . '
                SET send_number = ' . $value['goods_number'] . '
                WHERE order_id = \'' . $value['order_id'] . '\'
                AND goods_id = \'' . $pg_value['goods_id'] . '\' ';
                  ...
                  WHERE delivery_id = \'' . $delivery_order['delivery_id'] . '\'';
                            ...
                            for ($i = 0; $i < count($delivery_stock_result); $i++) {
                                $products = get_warehouse_id_attr_number($delivery_stock_result[$i]['goods_id'],
                                    $delivery_stock_result[$i]['goods_attr_id'], $delivery_stock_result[$i]['ru_id'],
                                    $delivery_stock_result[$i]['warehouse_id'], $delivery_stock_result[$i]['area_id'],
                                    $delivery_stock_result[$i]['model_attr'], $store_id);
                                ...
                                foreach ($delivery_stock_result as $value) {
                                    if ($value['is_real'] != 0) {
                                        if (!empty($value['goods_attr_id'])) {
                                            ...
                                            if (is_spec($spec)) {
                                                $product_info = get_products_info($value['goods_id'], $spec,
                                                    $value['warehouse_id'], $value['area_id'], $store_id);
                            ...
                            $query = $db->autoExecute($ecs->table('delivery_order'), $_delivery, 'UPDATE',
                                'delivery_id = ' . $delivery_id, 'SILENT');

                            if (!$query) {
                                $links[] = array(
                                    'text' => $_LANG['delivery_sn'] . $_LANG['detail'],
                            ...
                            update_order($order_id, $arr);
                            order_action($order['order_sn'], OS_CONFIRMED, $shipping_status, $order['pay_status'],
                                $action_note, $stores_user, 1);
                            ...
                            if ($order_finish) {
                                ...
                                if ($cfg == '1') {
                                    ...
                                    $smarty->assign('confirm_url',
                                        $ecs->stores_url() . 'user.php?act=order_detail&order_id=' . $order['order_id']);
                                    $smarty->assign('send_msg_url',
                                        $ecs->stores_url() . 'user.php?act=message_list&order_id=' . $order['order_id']);
                                    $content = $smarty->fetch('str:' . $tpl['template_content']);

                                    if (!send_mail($order['consignee'], $order['email'], $tpl['template_subject'],
                                        $content, $tpl['is_html'])) {
                                    ...
                                    if ($GLOBALS['_CFG']['sms_type'] == 0) {
                                        ...
                                        if (1 <= $GLOBALS['_CFG']['sms_type']) {
                                            ...
                                            if ($result) {
                                                ...
                                                make_json_response('', 0, '阿里大鱼短信配置异常',
                                                    array('url' => 'order.php?act=list'));
                                            }
                                        }
                                    }
                                }
