...
function handle_gallery_image($goods_id, $image_files, $image_descs, $image_urls, $single_id = 0, $files_type = 0)
{
    ...
    foreach ($image_descs as $key => $img_desc) {
        ...
        if ($flag) {
            if ($proc_thumb) {
                $thumb_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'][$key],
            ...
            if (!empty($image_urls[$key]) && $image_urls[$key] != $GLOBALS['_LANG']['img_file'] && $image_urls[$key] != 'http://' && copy(trim($image_urls[$key]),
                    ROOT_PATH . 'temp/' . basename($image_urls[$key]))) {
                $image_url = trim($image_urls[$key]);
                $down_img = ROOT_PATH . 'temp/' . basename($image_url);

                if ($proc_thumb) {
                    $thumb_url = $GLOBALS['image']->make_thumb($down_img, $GLOBALS['_CFG']['thumb_width'],
...
function handle_gallery_image_add(
    ...
    if (!empty($_SESSION['thumb_img_id' . $_SESSION['store_user_id']])) {
        $_SESSION['thumb_img_id' . $_SESSION['store_user_id']] = array_merge($thumb_img_id,
            $_SESSION['thumb_img_id' . $_SESSION['store_user_id']]);
    } else {
