...
$sto = new exchange($ecs->table('offline_store'), $db, 'id', 'stores_user', 'stores_name', 'is_confirm', 'stores_tel',
...
if ($_REQUEST['act'] == 'list') {
    ...
    $smarty->assign('action_link',
        array('href' => 'store_assistant.php?act=add', 'text' => $_LANG['store_assistant_add']));
    $smarty->assign('action_link2',
        array('href' => 'store_assistant.php?act=message_edit', 'text' => $_LANG['store_message_edit']));
    ...
    if ($_REQUEST['act'] == 'query') {
        ...
        $smarty->assign('record_count', $list['record_count']);
        $smarty->assign('page_count', $list['page_count']);
        make_json_result($smarty->fetch('store_assistant.dwt'), '',
            array('filter' => $list['filter'], 'page_count' => $list['page_count']));
    } else {
        if ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit') {
            store_priv('user_manage');
            $smarty->assign('action_link',
                array('href' => 'store_assistant.php?act=list', 'text' => $_LANG['store_assistant_list']));
            ...
            $smarty->assign('page_title',
                $_REQUEST['act'] == 'add' ? $_LANG['add_assistant'] : $_LANG['edit_assistant']);
            ...
            if ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
                ...
                if ($_REQUEST['act'] == 'insert') {
                    ...
                    if ($db->query($sql) == true) {
                        ...
                        make_json_response('', 1, $_LANG['add_succeed'],
                            array('url' => 'store_assistant.php?act=list'));
                    }
                } else {
                    $id = !empty($_REQUEST['id']) ? intval($_REQUEST['id']) : 0;
                ...
                if ($_REQUEST['act'] == 'remove') {
                    ...
                    if ($_REQUEST['act'] == 'message_edit') {
                        ...
                        if ($_REQUEST['act'] == 'message_update') {
                            store_priv('user_manage');
                            ...
                            if (!$is_only) {
                                make_json_response('', 0, $_LANG['title_exist'],
                                    array('url' => 'store_assistant.php?act=message_edit'));
                            }
                            ...
                            $link[0]['text'] = $_LANG['bank_list'];
                            $link[0]['href'] = 'store_assistant.php?act=list';
                            make_json_response('', 1, $_LANG['edit_succeed'],
                                array('url' => 'store_assistant.php?act=message_edit'));
                        }
                    }
                }
