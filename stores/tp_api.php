    ...
    if (isset($order_info['order_id'])) {
        ...
        if ($goods_list) {
            foreach ($goods_list as $key => $val) {
                ...
                if ($order_type == 'order') {
                    $products = get_warehouse_id_attr_number($val['goods_id'], $val['goods_attr_id'], $val['ru_id'],
                        $val['warehouse_id'], $val['area_id'], $val['model_attr']);
                }

                if (isset($products) && isset($products['bar_code'])) {
                    $bar_code = $products['bar_code'];
                } else {
                    $bar_code = get_table_date('goods', 'goods_id = \'' . $val['goods_id'] . '\'', array('bar_code'),
                        2);
                }
                                ...
                                if ($_REQUEST['act'] == 'order_print_setting_insert' || $_REQUEST['act'] == 'order_print_setting_update') {
                                    ...
                                    if ($_REQUEST['act'] == 'order_print') {
                                        ...
                                        $smarty->assign('order_print_logo',
                                            $ecs->stores_url() . ADMIN_PATH . '/' . $order_print_logo);
                                        $part_html = array();
