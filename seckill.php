...
function get_top_seckill_goods()
{
    ...
    foreach ($look_top_list as $key => $look_top) {
        ...
        $look_top['url'] = build_uri('seckill', array('act' => 'view', 'secid' => $look_top['id']),
            $look_top['goods_name']);
...
function get_merchant_seckill_goods($sec_goods_id, $ru_id)
{
    ...
    foreach ($merchant_seckill as $key => $row) {
        ...
        $merchant_seckill[$key]['url'] = build_uri('seckill', array('act' => 'view', 'secid' => $row['id']),
            $row['goods_name']);
    }
...
');
        ...
        if (!empty($_COOKIE['ECS']['sec_history'])) {
            ...
            setcookie('ECS[sec_history]', implode(',', $sec_history), gmtime() + 3600 * 24 * 30,
                $GLOBALS['cookie_path'], $GLOBALS['cookie_domain']);
        } else {
            setcookie('ECS[sec_history]', $seckill_id, gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
...
');
                ...
                if ($goods_attr) {
                    ...
                    $prod = $GLOBALS['db']->getRow($sql);
                    $products = get_warehouse_id_attr_number($seckill['goods_id'], $goods_attr, $seckill['user_id'],
                ...
                $start_date = local_strtotime($seckill['begin_time']);
                $end_date = local_strtotime($seckill['end_time']);
                $order_goods = get_for_purchasing_goods($start_date, $end_date, $seckill['goods_id'],
                    $_SESSION['user_id'], 'seckill');
                $restrict_amount = $seckill['sec_limit'];
                $order_number = $order_goods['goods_number'] + $number;
                if (0 < $order_goods['goods_number'] && $restrict_amount <= $order_goods['goods_number']) {
                    show_message($GLOBALS['_LANG']['js_languages']['common']['Already_buy'] . $order_goods['goods_number'] . $GLOBALS['_LANG']['js_languages']['common']['Already_buy_two']);
                    exit();
                } else {
                    if (0 < $order_goods['goods_number'] && $restrict_amount < $order_number) {
                        $buy_num = $restrict_amount - $order_goods['goods_number'];
                        show_message($GLOBALS['_LANG']['js_languages']['common']['Already_buy'] . $buy_num . $GLOBALS['_LANG']['js_languages']['common']['jian']);
                        exit();
                    } else {
                        if ($restrict_amount < $number) {
                            show_message($GLOBALS['_LANG']['js_languages']['common']['Purchase_quantity']);
                            exit();
                        }
                    }
                }
