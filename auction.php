...
function auction_list($keywords, $sort, $order, $size, $page, $top_children = array())
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if (1 < $auction['status_no']) {
            $sql = 'SELECT COUNT(*)' . ' FROM ' . $GLOBALS['ecs']->table('order_info') . ' WHERE extension_code = \'auction\'' . (' AND extension_id = \'' . $auction['act_id'] . '\'') . ' AND order_status ' . db_create_in(array(
                    OS_CONFIRMED,
                    OS_UNCONFIRMED
                ));
...
function get_exchange_recommend_goods($type = '', $cats = '', $min = 0, $max = 0, $ext)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $auction[$idx]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
...
if ($_REQUEST['act'] == 'list') {
    ...
    $sort = isset($_REQUEST['sort']) && in_array(trim(strtolower($_REQUEST['sort'])),
        array('act_id', 'start_time', 'end_time')) ? trim($_REQUEST['sort']) : $default_sort_order_type;
...
if ($_REQUEST['act'] == 'load_more_goods') {
    ...
    if ($_REQUEST['act'] == 'view') {
        ...
        $basic_info['province'] = get_table_date('region', 'region_id = \'' . $goods['province'] . '\'', $basic_date,
            2);
        ...
        if ($_REQUEST['act'] == 'bid') {
            ...
            if (!$is_ok) {
                ...
                if ($bid_price < $min_price) {
                    show_message(sprintf($_LANG['au_your_lowest_price'], price_format($min_price, false)), '', '',
                        'error');
                }
            }
            ...
            if (0 < $auction['deposit']) {
                ...
                log_account_change($user_id, -1 * $auction['deposit'], $auction['deposit'], 0, 0,
