...
if ($act == 'delHistory') {
    ...
    setcookie('ECS[history]', implode(',', $one_history), gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
    setcookie('ECS[list_history]', implode(',', $two_history), gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
        $GLOBALS['cookie_domain']);
...
assign_pager('history_list', 0, $count, $size, $sort, $order, $page, '', '', '', '', '', '', '', '', '', '', '', '',
    $ship, $self);
