...
function API_AddCategory($post)
{
    ...
    if ($GLOBALS['db']->autoExecute($GLOBALS['ecs']->table('category'), $cat) !== false) {
        ...
        if ($cat['show_in_nav'] == 1) {
            ...
            $vieworder += 2;
            $sql = 'INSERT INTO ' . $GLOBALS['ecs']->table('nav') . ' (name, ctype, cid, ifshow, vieworder, opennew, url, type)' . ' VALUES(\'' . $cat['cat_name'] . '\', \'c\', \'' . $insert_id . '\',\'1\',\'' . $vieworder . '\',\'0\', \'' . build_uri('category',
                    array('cid' => $insert_id), $cat['cat_name']) . '\',\'middle\')';
...
function API_AddGoods($post)
{
    ...
    if (!empty($_POST['goods_img']['Data'])) {
        ...
        if ((0 < $image->gd_version()) && $image->check_img_function($img_property[2])) {
            if (($GLOBALS['_CFG']['image_width'] != 0) || ($GLOBALS['_CFG']['image_height'] != 0)) {
                $goods_img = $image->make_thumb(ROOT_PATH . '/' . $goods_img, $GLOBALS['_CFG']['image_width'],
            ...
            if ((0 < intval($GLOBALS['_CFG']['watermark_place'])) && !empty($GLOBALS['_CFG']['watermark'])) {
                if ($image->add_watermark(ROOT_PATH . '/' . $goods_img, '', $GLOBALS['_CFG']['watermark'],
                ...
                if ($image->add_watermark(ROOT_PATH . '/' . $gallery_img, '', $GLOBALS['_CFG']['watermark'],
                        $GLOBALS['_CFG']['watermark_place'], $GLOBALS['_CFG']['watermark_alpha']) === false) {
                    client_show_message(213);
                }
            }
            ...
            if (($_CFG['thumb_width'] != 0) || ($_CFG['thumb_height'] != 0)) {
                $gallery_thumb = $image->make_thumb(ROOT_PATH . '/' . $img, $GLOBALS['_CFG']['thumb_width'],
    ...
    if (!empty($_POST['goods_thumb']['Data'])) {
        ...
        if (isset($_POST['auto_thumb']) && !empty($original_img)) {
            if (($_CFG['thumb_width'] != 0) || ($_CFG['thumb_height'] != 0)) {
                $goods_thumb = $image->make_thumb(ROOT_PATH . '/' . $original_img, $GLOBALS['_CFG']['thumb_width'],
                    $GLOBALS['_CFG']['thumb_height']);
