...
function store_get_goods(
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        if ($display == 'grid') {
            $arr[$idx]['goods_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
...
function get_store_goods_count(
    $children,
    $brand = 0,
    $min = 0,
    $max = 0,
...
function get_hot_new_best_goods(
    $children = '',
    $type = '',
    $merchant_id,
    $warehouse_id = 0,
    ...
    if (!empty($type)) {
        ...
        foreach ($res as $key => $row) {
            ...
            if ($display == 'grid') {
                $arr[$key]['goods_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
                    $GLOBALS['_CFG']['goods_name_length']) : $row['goods_name'];
            } else {
...
if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'in_warehouse') {
    ...
    if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'in_stock') {
        ...
        if (!empty($_REQUEST['act']) && $_REQUEST['act'] == 'ajax_collect_store') {
            ...
            if ($_REQUEST['act'] == 'merchants_licence') {
                assign_template();
                ...
                if ($basic_info['license_comp_adress']) {
                    ...
                    if (!empty($adress)) {
                        ...
                        foreach ($adress as $v) {
                            $license_comp_adress .= get_table_date('region', 'region_id=\'' . $v . '\'',
                                array('region_name'), 2);
                        }
                    }
                ...
                if ($basic_info['company_located']) {
                    ...
                    if (!empty($adress)) {
                        ...
                        foreach ($adress as $v) {
                            $company_located .= get_table_date('region', 'region_id=\'' . $v . '\'',
                                array('region_name'), 2);
                        }
                    }
...
foreach ($seo as $key => $value) {
    $seo[$key] = str_replace(array('{sitename}', '{key}', '{shopname}', '{description}'),
        array($_CFG['shop_name'], $store_info['shop_keyword'], $store_info['shop_title'], $store_info['street_desc']),
        $value);
}
...
if ($order_area['province'] && 0 < $user_id) {
    ...
    if (0 < $user_area['province']) {
        ...
        setcookie('province', $user_area['province'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
            $GLOBALS['cookie_domain']);
...
if ($order_area['province'] && 0 < $user_id) {
    ...
    if (0 < $user_area['city']) {
        ...
        setcookie('city', $user_area['city'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
            $GLOBALS['cookie_domain']);
    } else {
...
if ($order_area['province'] && 0 < $user_id) {
    ...
    if (0 < $user_area['district']) {
        ...
        setcookie('district', $user_area['district'], gmtime() + 3600 * 24 * 30, $GLOBALS['cookie_path'],
            $GLOBALS['cookie_domain']);
    } else {
...
if ($is_cache) {
    $cache_id = sprintf('%X',
        crc32($cat_id . '-' . $merchant_id . '-' . $display . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . $_SESSION['user_rank'] . '-' . $_CFG['lang'] . '-' . $brand . '-' . $price_max . '-' . $price_min . '-' . $filter_attr_str . '-' . $keyword));
    $not = '';
} else {
    $cache_id = '';
    $not = 'not';
...
');
    ...
    $count = get_store_goods_count($children, $brand, $price_min, $price_max, $ext, $merchant_id, $region_id,
        $area_info['region_id'], $area_city, $keyword);
    ...
    $goodslist = store_get_goods($children, $brand, $price_min, $price_max, $ext, $size, $page, $sort, $order,
        $merchant_id, $region_id, $area_info['region_id'], $area_city, $keyword);
    ...
    assign_pager('merchants_store', $cat_id, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max,
        $display, $filter_attr_str, '', '', $merchant_id, $keyword);
