...
<div class="footer">
            ...
            <div class="help-list">
                ...
                <div class="help-item">
                    ...
                    <ul>
                        ...
                        <li><a href="{$item.url}" title="{$item.title|escape:html}"
                               target="_blank">{$item.short_title}</a></li>
                        {/if}
                        <!-- {/foreach} -->
                    </ul>
                    </dl>
                ...
                <div class="help-ctn">
                    ...
                    <div class="help-ctn-mb">
                        ...
                        <a id="IM" IM_type="dsc" onclick="openWin(this)" href="javascript:;" class="btn-ctn"><i
                                    class="iconfont icon-csu"></i>咨询客服</a>
                        {else}
                        {if $basic_info.kf_type eq 1}
                        <a href="http://www.taobao.com/webww/ww.php?ver=3&touid={$basic_info.kf_ww}&siteid=cntaobao&status=1&charset=utf-8"
                           class="btn-ctn" target="_blank"><i class="iconfont icon-csu"></i>{$lang.con_cus_service}</a>
                        {else}
                        <a href="http://wpa.qq.com/msgrd?v=3&uin={$basic_info.kf_qq}&site=qq&menu=yes" class="btn-ctn"
                           target="_blank"><i class="iconfont icon-csu"></i>{$lang.con_cus_service}</a>
                        {/if}
                        {/if}
                    </div>
                </div>
    ...
    <div class="dsc-copyright">
        <div class="w w1200">
            ...
            <p class="copyright_info">
                ...
                <a href="{$link.url}" target="_blank" title="{$link.name}"><img src="{$link.logo}" alt="{$link.name}"
                                                                                border="0"/></a>
                <!--结束图片类型的友情链接{/foreach}-->
                <!-- {if $txt_links} -->
            ...
            <b>{$lang.icp_number}:<a href="http://www.miibeian.gov.cn/" target="_blank">{$icp_number}</a> <a
                        href="http://www.dscmall.cn/" target="_blank">111111</a></b>
            <!-- 结束ICP 证书{/if} -->
    ...
    <!--隐藏域-->
    <div class="hidden">
        <input type="hidden" name="seller_kf_IM" value="{$shop_information.is_IM}" rev="{$site_domain}"
               ru_id="{$smarty.get.merchant_id}"/>
