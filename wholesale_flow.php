            ...
            if ($order_info['pay_status'] != 2) {
                if ($payment_info['pay_code'] == 'balance') {
                    ...
                    if ($order['order_amount'] < $user_money) {
                        ...
                        $db->query($sql);
                        log_account_change($order['user_id'], $order['order_amount'] * -1, 0, 0, 0,
                            sprintf($_LANG['pay_who_order'], $order_info['order_sn']));
                        ...
                        if (0 < $child_num) {
                            ...
                            foreach ($order_res as $row) {
                                ...
                                $sql = 'UPDATE ' . $GLOBALS['ecs']->table('wholesale_order_info') . ' SET pay_status = 2, ' . (' pay_time = \'' . $time . '\', ') . (' pay_fee = \'' . $child_pay_fee . '\' ') . 'WHERE order_id = \'' . $row['order_id'] . '\'';
                                $GLOBALS['db']->query($sql);
                            }
                        }
            ...
            if ($_REQUEST['step'] == 'remove') {
                ...
                if ($_REQUEST['step'] == 'batch_remove') {
                    ...
                    if ($_REQUEST['step'] == 'ajax_update_cart') {
                        ...
                        if ($_REQUEST['step'] == 'update_rec_num') {
                            require_once ROOT_PATH . 'includes/cls_json.php';
                            $json = new JSON();
                            $result = array('error' => 0, 'message' => '', 'content' => '');
                            $rec_id = empty($_REQUEST['rec_id']) ? 0 : intval($_REQUEST['rec_id']);
                            $rec_num = empty($_REQUEST['rec_num']) ? 0 : intval($_REQUEST['rec_num']);
                            $cart_info = get_table_date('wholesale_cart', 'rec_id=\'' . $rec_id . '\'',
                                array('goods_id', 'goods_attr_id'));

                            if (empty($cart_info['goods_attr_id'])) {
                                $goods_number = get_table_date('wholesale',
                                    'goods_id=\'' . $cart_info['goods_id'] . '\'', array('goods_number'), 2);
                            } else {
                                $set = get_find_in_set(explode(',', $cart_info['goods_attr_id']));
                                $goods_number = get_table_date('wholesale_products',
                                    'goods_id=\'' . $cart_info['goods_id'] . '\' ' . $set, array('product_number'), 2);
                            }
