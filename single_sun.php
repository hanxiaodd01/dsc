...
if ($_REQUEST['act'] == 'check_comm') {
    ...
    if ($_REQUEST['act'] == 'discuss_show') {
        ...
        if (!$smarty->is_cached('goods_discuss_show.dwt', $cache_id)) {
            ...
            $position = assign_ur_here($goodsInfo['cat_id'], $goodsInfo['goods_name'], array($discuss['dis_title']),
                $goodsInfo['goods_url']);
        ...
        if ($_REQUEST['act'] == 'add_discuss') {
            ...
            if (empty($_POST['referenceType'])) {
                show_message($_LANG['discuss_type'], $_LANG['back_page_up'], 'category_discuss.php?id=' . $goods_id,
                    'error');
            }

            if (empty($_POST['commentTitle'])) {
                show_message($_LANG['title_Remarks'], $_LANG['back_page_up'], 'category_discuss.php?id=' . $goods_id,
                    'error');
            }

            if (empty($_POST['content'])) {
                show_message($_LANG['content_null'], $_LANG['back_page_up'], 'category_discuss.php?id=' . $goods_id,
                    'error');
            }
            ...
            if (!empty($dis_id)) {
                ...
                show_message($_LANG['Submit_fail'], $_LANG['back_page_up'], 'category_discuss.php?id=' . $goods_id,
                    'error');
                exit();
            }
        } else {
            if ($_REQUEST['act'] == 'ajax_verify') {
