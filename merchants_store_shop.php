...
$cache_id = sprintf('%X',
    crc32($_SESSION['user_rank'] . '-' . $sort . '-' . $order . '-' . $page . '-' . $size . '-' . $merchant_id . '-' . $_CFG['lang']));
...
if (!$smarty->is_cached('merchants_shop.dwt', $cache_id)) {
    ...
    $goods_list = get_shop_goods_cmt_list($merchant_id, $region_id, $area_id, $area_city, $price_min, $price_max, $page,
        $size, $sort, $order);
    ...
    assign_pager('merchants_store_shop', 0, $count, $size, $sort, $order, $page, '', $brand, $price_min, $price_max,
        'list', $filter_attr_str, '', '', $merchant_id);
