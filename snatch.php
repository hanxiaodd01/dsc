...
function get_myprice($id)
{
    ...
    if (!empty($_SESSION['user_id'])) {
        ...
        if ($my_price_time) {
            $sql = 'SELECT bid_price , count(*) AS num FROM ' . $GLOBALS['ecs']->table('snatch_log') . ('  WHERE snatch_id =\'' . $id . '\' AND bid_price ') . db_create_in(join(',',
                    $my_price)) . ' GROUP BY bid_price HAVING num = 1';
...
function get_price_list($id)
{
    ...
    while ($row = $GLOBALS['db']->FetchRow($res)) {
        ...
        $price_list[] = array(
            'bid_price' => price_format($row['bid_price'], false),
            'user_name' => $row['user_name'],
            'bid_date' => local_date('Y-m-d H:i', $row['bid_time'])
        );
    }
...
function get_snatch_list($keywords = '', $size, $page, $sort, $order, $warehouse_id, $area_id, $area_city = 0)
{
    ...
    while ($row = $GLOBALS['db']->FetchRow($res)) {
        ...
        $snatch_list[] = array(
            ...
            'price_list_count' => count(get_price_list($row['snatch_id'])),
...
function get_snatch($id)
{
    ...
    if ($goods) {
        ...
        $goods['snatch_time'] = sprintf($GLOBALS['_LANG']['snatch_start_time'], $goods['start_time'],
            $goods['end_time']);
...
function get_exchange_recommend_goods($type = '', $warehouse_id, $area_id, $area_city = 0)
{
    ...
    while ($row = $GLOBALS['db']->fetchRow($res)) {
        ...
        $snatch[$idx]['short_name'] = 0 < $GLOBALS['_CFG']['goods_name_length'] ? sub_str($row['goods_name'],
...
if ($_REQUEST['act'] == 'list') {
    ...
    $sort = isset($_REQUEST['sort']) && in_array(trim(strtolower($_REQUEST['sort'])),
        array('snatch_id', 'end_time', 'start_time')) ? trim($_REQUEST['sort']) : $default_sort_order_type;
    ...
    if (!$_CFG['category_load_type']) {
        $pager = get_pager('snatch.php',
            array('act' => 'list', 'keywords' => $keywords, 'sort' => $sort, 'order' => $order), $count, $page, $size);
    ...
    if ($_REQUEST['act'] == 'load_more_goods') {
        ...
        if ($_REQUEST['act'] == 'main') {
            ...
            $basic_info['province'] = get_table_date('region', 'region_id = \'' . $goods['province'] . '\'',
                $basic_date, 2);
            $basic_info['city'] = get_table_date('region', 'region_id= \'' . $goods['city'] . '\'', $basic_date,
                    2) . '市';
...
if ($_REQUEST['act'] == 'bid') {
    ...
    $goods_attr = isset($_REQUEST['goods_attr_id']) && !empty($_REQUEST['goods_attr_id']) ? dsc_addslashes($_REQUEST['goods_attr_id'],
        0) : '';
    ...
    if ($goods_attr) {
        ...
        $products = get_warehouse_id_attr_number($goods['goods_id'], $goods_attr, $goods['user_id'], $warehouse_id,
            $area_id, $area_city);
    ...
    log_account_change($_SESSION['user_id'], 0, 0, 0, 0 - $row['cost_points'],
        sprintf($_LANG['snatch_log'], $row['snatch_name']));
