    ...
    if ($step == 'stepTwo') {
        ...
        if ($step == 'stepThree') {
            if (!is_array($process)) {
                ...
                if ($db->getOne($sql)) {
                    show_message($_LANG['Settled_Prompt'], $_LANG['Return_last_step'],
                        'merchants_steps.php?step=' . $step . '&pid_key=' . $noWkey);
                    exit();
                } else {
                ...
                if ($db->getOne($sql)) {
                    show_message($_LANG['Settled_Prompt_name'], $_LANG['Return_last_step'],
                        'merchants_steps.php?step=' . $step . '&pid_key=' . $noWkey);
                    exit();
                } else {
                    $sql = 'update ' . $ecs->table('merchants_shop_information') . ' set steps_audit = 1' . ' where user_id = \'' . $_SESSION['user_id'] . '\'';
